...
- Removed unsupported fixed claims such as guaranteed accuracy, speedup, and exact training times.
...
## 9. Text-generation fixes

### `guides/projects/text_generation/main.py`
- Fixed examples that supplied `temperature`/`top_p` without enabling sampling.
- Made multi-sequence generation actually sample distinct candidates.
- Corrected the tokenization demonstration so token IDs and token strings have matching lengths.
- Clarified that `temperature` and `top_p` affect sampling, not deterministic decoding.
- Renamed the custom function's misleading `max_words` argument to `max_new_tokens`.
- Added basic validation for generation parameters.
- Updated the explanation of `top_p`, `temperature`, and `do_sample` to match their actual semantics.
- Fixed the optional DistilGPT-2 example to explicitly enable sampling.

## 10. Neural-network visualization fixes

### `guides/projects/neural_network_basics/main.py`
- Fixed the decision-boundary visualization using mixed coordinate systems.
- The model is trained on standardized features, so the decision grid and plotted points now use the same standardized coordinate system.
- Added stratification to the train/test split.
- Made raw-vs-scaled data handling explicit for new prediction points.
- Simplified prediction code and removed unnecessary `.data` usage.
- Updated labels so the plotted coordinate system is accurately described.

## 11. Remaining high-priority audit work
...
## 12. Knowledge-base date incident
...
## 13. Recommended V2 validation gates
...
[ ] Sampling parameters have the intended decoding mode
[ ] Visualization coordinates match the model's preprocessing pipeline
```

## 14. Important philosophy for V2
