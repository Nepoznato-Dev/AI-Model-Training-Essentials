---
# فراداده
عنوان: "پیکربندی خط لوله CI/CD"
توضیحات: "GitHub Actions، GitLab CI، Jenkins، الگوهای YAML خط لوله"
دسته بندی: "مرجع سریع"
نسخه: "1.0.0"
وضعیت: "فعال"
# مشارکت
...
# طبقه بندی
برچسب‌ها: [cicd، خط لوله، پیکربندی، مرجع سریع]
سطح سختی: "مبتدی"
پیش نیاز: []
...
|---------|-------------|
| `on: push`| در هر فشار |
|  __محافظت شده_1__ | در روابط عمومی باز، به روز رسانی، بازگشایی |
| `on: schedule`| جدول زمانی مبتنی بر کرون |
|  __محافظت شده_3__ | ماشه دستی |
...
|  __محافظت شده_1__ | واگذاری کار به مرحله |
| `script`| دستورات برای اجرا |
|  __محافظت شده_3__ | دستورات قبل از اسکریپت اصلی اجرا می شوند |
...
|  __محافظت شده_6__ | نسخه انعطاف پذیرتر فقط/به جز |
| `variables`| تعریف متغیرهای CI/CD |
| `cache`| فایل های کش بین اجرای خط لوله |
|  __محافظت شده_9__ | فایل هایی برای ارسال بین کارها |
...
|  __محافظت شده_1__ | نام شعبه یا برچسب |
| `$CI_PIPELINE_ID`| شناسه خط لوله |
|  __محافظت شده_3__ | شناسه شغلی |
| `$CI_PROJECT_DIR`| مسیر کامل پروژه |
| `$CI_REGISTRY`| آدرس رجیستری کانتینر |
|  __محافظت شده_6__ | نام شعبه پیش فرض |
---
...
| **استقرار صحنه سازی** | استقرار در محیط صحنه سازی |
...
| **Node.js (npm)** |  __محافظت شده_3__ | `actions/setup-node`با کش داخلی |
| **جاوا (Maven)** | `~/.m2/repository`| حافظه پنهان با کلید از`pom.xml`هش |
| **جاوا (Gradle)** | `~/.gradle/caches`| حافظه پنهان با کلید از`build.gradle`هش |
| **برو** |  __محافظت شده_9__ | حافظه پنهان با کلید از`go.sum`هش |
| **زنگ (محموله)** | `~/.cargo/registry`| حافظه پنهان با کلید از`Cargo.lock`هش |
| **داکر** | ذخیره سازی لایه داکر | `docker/build-push-action`با حافظه پنهان از |
---
...
| **تست های پوسته پوسته** | تست های پوسته پوسته قرنطینه؛ رفع علت اصلی؛ سعی مجدد با`retry:`|
