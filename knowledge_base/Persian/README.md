...
**سازمان:** 10 دایرکتوری موضوعی + 34 مرجع زبان برنامه نویسی
---
...
| [embedded_systems_and_iot.md](01_coding_and_technology/embedded_systems_and_iot.md)| میکروکنترلرها، حسگرها، پروتکل‌های RTOS، IoT، محاسبات لبه |
...
| دارت | `programming_languages/dart/`| | اسکالا | `programming_languages/scala/`|
| هاسکل | `programming_languages/haskell/`| | جولیا | `programming_languages/julia/`|
| لوا | `programming_languages/lua/`| | پرل | `programming_languages/perl/`|
| ارلنگ/اکسیر | `programming_languages/erlang_and_elixir/`| | OCaml | `programming_languages/ocaml/`|
| پرولوگ | `programming_languages/prolog/`| | Lisp/Clojure | `programming_languages/lisp_and_clojure/`|
| آدا | `programming_languages/ada/`| | مونتاژ | `programming_languages/assembly/`|
| متلب | `programming_languages/matlab/`| | فرترن | `programming_languages/fortran/`|
| COBOL | `programming_languages/cobol/`| | Shell/PowerShell | `programming_languages/shell_and_powershell/`|
| دلفی/پاسکال | `programming_languages/delphi_object_pascal/`| | خراش | `programming_languages/scratch/`|
...
4. زبانی را در`01_coding_and_technology/programming_languages/python/`انتخاب کنید
...
## سلب مسئولیت مهم
- **اطلاعات پزشکی:** محتوای`medicine_and_healthcare.md`فقط برای اهداف آموزشی است و نباید جایگزین توصیه های پزشکی حرفه ای شود.
- **اطلاعات حقوقی:** محتوای موجود در`law_and_legal_systems.md`اطلاعاتی است و به منزله مشاوره حقوقی نیست.
- **اطلاعات مالی:** مطالب در`finance_and_investing.md`آموزشی است و نباید به عنوان توصیه مالی در نظر گرفته شود.
- **پیش بینی های آینده:** محتوای`08_future_and_trends/`حاوی اطلاعات و پیش بینی های احتمالی است که ممکن است تغییر کند.
---

## دستورالعمل های استفاده
### برای سیستم های هوش مصنوعی
- همه فایل‌ها از عناوین سلسله مراتبی واضح استفاده می‌کنند (عنوان `#`، بخش‌های `##`، بخش‌های فرعی `###`)
- جداول برای مقایسه ساختار یافته در سراسر استفاده می شود
- نمونه کد شامل برجسته کردن نحو است
- ارجاعات متقابل از پیوندهای نشانه گذاری نسبی استفاده می کنند
### برای خوانندگان انسانی
- فایل ها با لحن محاوره ای و خنثی نوشته می شوند
- جداول و مقایسه های ساختار یافته به جای پاراگراف های متراکم استفاده می شود
- مثال ها و قیاس های دنیای واقعی به پایه گذاری مفاهیم انتزاعی کمک می کند
- هر فایل شامل یک بخش خلاصه است
---

## مشارکت و فراداده
هر فایل محتوایی در این پایگاه دانش (همه چیز به جز این`README.md`) با یک بلوک frontmatter YAML باز می شود. از همان الگوی ردیابی مشارکت‌کننده استفاده شده توسط[skills](../../skills/skill-creator.md)پیروی می‌کند، بنابراین تألیف، ویرایش‌ها و نظرات در هر دو مجموعه قابل ردیابی است. برای گردش کار مشارکت عمومی - انشعاب، انشعاب، درخواست‌های کشش، قرار دادن فایل، و قراردادهای نام‌گذاری - ریشه[CONTRIBUTING.md](../../CONTRIBUTING.md)را ببینید.
### طرحواره Frontmatter
هر فایل با این بلوک (بین`---`جداکننده) شروع می شود که به پنج بخش نظر داده شده تقسیم می شود:
```yaml
---
# Metadata
...
| `estimated_reading_time`| محاسبه شده از تعداد خطوط |
### ویرایش یک فایل
وقتی یک فایل پایگاه دانش را تغییر می‌دهید، موضوع اصلی آن را همگام نگه دارید:
1. **برآمدگی`version`** دنبال [SemVer](https://semver.org/):
   - وصله (`1.0.x`): غلط املایی، قالب بندی، اصلاحات جزئی
   - جزئی (`1.x.0`): بخش های جدید، محتوای گسترده، نمونه های جدید
   - عمده ( `x.0.0`): بازنویسی کامل یا بازسازی
2. **یک ورودی گزارش تغییرات اضافه کنید** (ابتدا جدیدترین) که در آن تغییراتی را که تغییر داده اید شرح دهد:   ```yaml
   changelog:
     - version: "1.1.0"
       date: "2026-08-05"
       author: "Your Name"
     ...
     - version: "1.0.0"
       ...
       changes: "Added YAML frontmatter metadata for contributor tracking"
   ```
3. **`last_modified`** را به تاریخ تغییر خود به روز کنید
4. **اگر اولین ویرایش شما در فایل است، خود را به`contributors`اضافه کنید:   ```yaml
   contributors:
     - name: "Your Name"
...
### چک لیست کیفیت
قبل از ارسال فایل پایگاه دانش جدید یا ویرایش شده، بررسی کنید:
- [ ] مقدمه YAML کامل و معتبر است (همه پنج بخش موجود، بین`---`جداکننده)
- [ ] `title`، `description`، `category`، `version`،`status`تنظیم شده اند
- [ ] حداقل یک نویسنده در`authors`فهرست شده است 
- [ ]`changelog`ورودی دارد که تغییرات را مستند می کند (ابتدا جدیدترین)
- [ ]`last_modified`تاریخ امروز را نشان می دهد
- [ ]`tags`شامل 3-6 کلمه کلیدی مرتبط به اضافه کلمه کلیدی دسته تمیز
- [ ]`difficulty_level`تنظیم شده است (مبتدی / متوسط / پیشرفته)
- [ ]`estimated_reading_time`برای طول محتوا واقع بینانه است
- [ ] محتوای بدنه از[Usage Guidelines](#usage-guidelines)بالا پیروی می کند
- [ ] بدون ارجاع متقابل شکسته
---

*این پایگاه دانش یک مجموعه اسناد زنده است که به طور مداوم بهبود یافته و گسترش می یابد.*