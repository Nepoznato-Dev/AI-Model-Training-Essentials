...
- فیلدهای غیرمنتظره (<<INLINE_CODE_15>>> یا `additionalProperties: false` در طرحواره JSON) را رد کنید.
...
- از فایل‌های قفل بسته (<<INLINE_CODE_28>>>، `Cargo.lock`، `go.sum`) برای اطمینان از ساخت‌های قابل تکرار استفاده کنید.
...
### سخت شدن سیستم عامل
- به‌روزرسانی‌های امنیتی را مرتباً اعمال کنید (<<INLINE_CODE_31>>>، `yum update`).
