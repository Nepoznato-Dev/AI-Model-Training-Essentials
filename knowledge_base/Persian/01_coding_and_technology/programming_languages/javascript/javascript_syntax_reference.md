...
---
# جاوا اسکریپت - مرجع نحو
این سند یک مرجع جامع و ساختار یافته برای جاوا اسکریپت ارائه می دهد. این مرجع اصلی جاوا اسکریپت را با تمرکز بر الگوهای نحوی جامع، جداول عملگر و مکانیک داخلی تکمیل می کند.
---
...
| `/`| بخش | `7 / 2`| `3.5`| |
| `%`| باقی مانده | `7 % 2`| `1`| |
| `**`| توانمندی | `2 ** 10`| `1024`| ES2016 |
| `++`| افزایش | `x++`/`++x`| | پس و پیش افزایشی |
| `--`| کاهش | `x--`/`--x`| | پس از کاهش و پیش کاهش |
| `+x`,`-x`| Unary | `+"42"`→`42`| تایپ اجبار به شماره |
### مقایسه و برابری
| اپراتور | نام | مثال | یادداشت ها |
|----------|------|---------|-------|
| `===`| برابری دقیق | `1 === 1`| بدون اجبار - همیشه این |
| `!==`| نابرابری شدید | `1 !== "1"`| `true`— انواع مختلف |
| `==`| چکیده برابری | `1 == "1"`| نوع اجبار — اجتناب از |
| `!=`| نابرابری چکیده | `1 != "1"`| `false`— اجتناب از |
| `<`,`>`,`<=`,`>=`| رابطه ای | `"b" > "a"`| فرهنگ نویسی برای رشته ها |
### عملگرهای منطقی و پوچ
| اپراتور | نام | مثال | یادداشت ها |
|----------|------|---------|-------|
| `&&`| منطقی و | `a && b`| اولین مقدار نادرست یا آخرین مقدار |
| `\|\|`| منطقی یا | `a \|\| b`| اولین حقیقت یا آخرین مقدار |
| `!`| منطقی نه | `!true`| `false`|
| `??`| Nullish Coalescing | `a ?? b`|`b`را فقط در صورتی برمی‌گرداند که`a``null` /`undefined`باشد |
| `?.`| زنجیربندی اختیاری | `obj?.prop?.method()`| اتصال کوتاه به`undefined`|
### عملگرهای بیتی
| اپراتور | نام | مثال | یادداشت ها |
|----------|------|---------|-------|
| `&`| و | `5 & 3`| `1`|
...
| 2 | `new`(بدون ارگ) | راست |
| 3 | `++``--` (پست ثابت) | N/A |
| 4 | `!``~``+x``-x``typeof``void``delete``++``--`(pree) | راست |
| 5 | `**`| راست |
| 6 | `*``/``%`| چپ |
| 7 | `+``-` | چپ |
| 8 | `<<``>>``>>>`| چپ |
...
| مجموعه | سفارش داد | قابل تغییر | تکراری | نوع کلید | جستجو |
|------------|---------|--------|------------|----------|--------|
| `Array`| بله | بله | بله | فهرست (تعداد) | O(n) |
| `Object`| بله (درج) | بله | کلیدها: خیر | رشته/نماد | O(1) میانگین |
| `Map`| بله (درج) | بله | کلیدها: خیر | هر | O(1) میانگین |
| `Set`| بله (درج) | بله | نه | — | O(1) میانگین |
| `WeakMap`| نه | بله | کلیدها: خیر | فقط شی | O(1) میانگین |
| `WeakSet`| نه | بله | نه | فقط شی | O(1) میانگین |
### آرایه ها
```javascript
// Creation
...
## خلاصه
نحو جاوا اسکریپت در ظاهر به طرز فریبنده ای ساده است اما دارای عمق قابل توجهی است. این زبان از یک ابزار اسکریپت نویسی ساده به یک زبان چند پارادایم با کلاس ها، ماژول ها، ژنراتورها، پراکسی ها و یک مدل غیرهمگام پیچیده تبدیل شده است. درک اولویت اپراتور، حلقه رویداد، زنجیره های نمونه اولیه و تفاوت های ظریف اتصال`this`توسعه دهندگان شایسته را از متخصصان جدا می کند. جاوا اسکریپت مدرن (ES2020+) با زنجیره اختیاری، ادغام بی اثر،`Promise.allSettled`و انتظار سطح بالا همچنان به کاهش صفحه دیگ ادامه می دهد و در عین حال سازگاری با عقب را حفظ می کند.