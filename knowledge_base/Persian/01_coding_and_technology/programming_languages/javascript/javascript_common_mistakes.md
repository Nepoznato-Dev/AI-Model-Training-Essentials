...
# جاوا اسکریپت - اشتباهات رایج و ضد الگوها
این سند رایج ترین اشتباهات، تله ها و ضد الگوها را در جاوا اسکریپت فهرست می کند. هر ورودی رویکرد نادرست را نشان می دهد، توضیح می دهد که چرا شکست می خورد، و راه حل صحیح را ارائه می دهد.
---

## 1. سردرگمی اتصال `this`
```javascript
...
## 3.`var`بالا بردن و محدوده مسائل
```javascript
...
## 9. اشیاء آرایه مانند آرایه نیستند
```javascript
...
## 10. Anti-Pattern: جهش پارامترهای تابع
```javascript
...
## 11. درک نکردن حلقه رویداد و وظایف کوچک
```javascript
...
## 12. Anti-Pattern: با استفاده از `eval()`
```javascript
...
## 13. فراموش کردن پاکسازی شنوندگان رویداد
```javascript
// ❌ WRONG — memory leak
class Component {
  mount() {
...
## 14. کپی کم عمق در مقابل کپی عمیق
```javascript
...
## خلاصه
انعطاف‌پذیری جاوا اسکریپت با تله‌هایی همراه است: اتصال`this`به سایت تماس بستگی دارد،`==`اجبار را انجام می‌دهد، وعده‌ها نیاز به رسیدگی به خطا دارند، و ترتیب حلقه رویداد (وظایف کوچک قبل از ماکروتسک) غیر واضح است. قوانین مدرن ساده هستند: از`const`/`let`هرگز`var`استفاده کنید،`===`را به`==`ترجیح دهید، همیشه رد وعده ها را مدیریت کنید، از توابع پیکان برای پاسخ به تماس ها استفاده کنید، از جهش اجتناب کنید، و شنوندگان رویداد را پاک کنید. پیروی از این قوانین اکثر اشکالات جاوا اسکریپت را از بین می برد.