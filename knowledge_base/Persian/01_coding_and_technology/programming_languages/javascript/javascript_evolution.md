...
-->
# جاوا اسکریپت - تاریخچه نسخه و تکامل
## جدول زمانی
| نسخه | سال | تم کلید |
|---------|------|-----------|
| ES1 | 1997 | چاپ اول (Netscape) |
| ES2 | 1998 | تغییرات جزئی سرمقاله |
| ES3 | 1999 | Regex، `try/catch`،`switch`|
| ES4 | — | رها شده (خیلی جاه طلبانه) |
| ES5 | 2009 | حالت سختگیرانه، JSON،`map/filter/reduce`|
| ES5.1 | 2011 | اصلاحات سرمقاله |
| ES6/2015 | 2015 | **عناوین**: `let/const`، فلش ها، کلاس ها، وعده ها، ماژول ها |
| ES7/2016 | 2016 | `**`,`Array.includes`|
| ES8/2017 | 2017 | `async/await`,`Object.entries`, حافظه مشترک |
| ES9/2018 | 2018 | استراحت/گسترش،`for-await-of`,`Promise.finally`|
| ES10/2019 | 2019 | `Array.flat`,`Object.fromEntries`,`BigInt`|
| ES11/2020 | 2020 | `??`,`?.`,`Promise.allSettled`, دینامیک`import()`|
| ES12/2021 | 2021 | `String.replaceAll`,`Promise.any`, انتساب منطقی |
| ES13/2022 | 2022 |`await`سطح بالا، فیلدهای کلاس، `Array.at()`،`#private`|
| ES14/2023 | 2023 |  `Array.findLast`، hashbang، نمادها به عنوان کلیدهای WeakMap |
| ES15/2024 | 2024 | `Promise.withResolvers`,`Object.groupBy`,`Atomics.waitAsync`|
| ES16/2025 | 2025 | کمک کننده های Iterator، روش های `Set`،`import attributes`|
## نقاط عطف اصلی
### عصر مرورگر (1995–2009)
- **1995**: برندن ایچ جاوا اسکریپت را در 10 روز در نت اسکیپ ایجاد می کند
- **1997**: ECMA-262 زبان را استاندارد می کند
- **1999**: ES3 - نسخه ای که برای یک دهه تسلط داشت
- **2005**: انقلاب AJAX (Google Maps، Gmail) - JS جدی می شود
- **2006**: جی کوئری توسعه بین مرورگرها را ساده می کند
- **2008**: موتور V8 (Chrome) - انقلاب عملکرد JS آغاز می شود
- **2009**: Node.js - جاوا اسکریپت از مرورگر فرار می کند
### عصر مدرن (2015–اکنون)
- **2015**: ES6 - بزرگترین به روز رسانی تا کنون.  `let/const`، توابع پیکان، کلاس‌ها، وعده‌ها، قالب‌های واقعی، تخریب ساختار، ماژول‌ها، `Symbol`، `Map/Set`، ژنراتورها،`for...of`
- **2017**:`async/await`- برنامه نویسی ناهمزمان قابل خواندن می شود
- **2020**: زنجیره اختیاری`?.`و ادغام خنثی`??`
- **2022**:`await`سطح بالا، فیلدهای کلاس خصوصی
- **2024**: `Promise.withResolvers`، `Object.groupBy`
## ES6 (2015) - انفجار بزرگ
```javascript
// Before ES6
var name = "Alice";
