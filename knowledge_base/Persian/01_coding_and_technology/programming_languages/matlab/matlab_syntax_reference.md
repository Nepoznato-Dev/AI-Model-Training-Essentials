...
|----------|------|---------|-------|
| `+``-``*``/``^`| حسابی | `A * B`| ضرب ماتریسی |
| `.*``./``.^`| عنصر عاقلانه | `A .* B`| عملیات آرایه |
| `'`| انتقال | `A'`| انتقال مزدوج |
| `.'`| جابجایی غیر مزدوج | `A.'`| |
| `\`| تقسیم چپ | `A \ b`| حل تبر = b |
| `/`| تقسیم سمت راست | `b / A`| xA = b |
| `==``~=` | برابری | `A == B`| عنصر عاقلانه |
| `<``>``<=``>=` | مقایسه | `A > 0`| عنصر عاقلانه |
| `&``\|``~`| منطقی (عنصر) | `A & B`| |
| `&&``\|\|` | اتصال کوتاه | `a && b`| فقط اسکالر |
| `:`| کولون/محدوده | `1:10`| `start:step:stop`|
| `;`| جداکننده ردیف / سرکوب | `[1; 2; 3]`| |
| `,`| جداکننده ستون | `[1, 2, 3]`| |
---
...
## خلاصه
نحو MATLAB ماتریس اول است - هر متغیر یک آرایه است و عملیات برای جبر خطی بهینه شده است. عملگر کولون، نمایه سازی منطقی، و عملیات بردار، حلقه ها را برای اکثر کارهای عددی حذف می کنند. سیستم ترسیم ارقام با کیفیت انتشار با حداقل کد تولید می کند. جعبه ابزار MATLAB را در هر حوزه مهندسی گسترش می دهد. در حالی که پایتون به برخی از قلمروها نفوذ کرده است، MATLAB استانداردی برای محاسبات مهندسی، مدل‌سازی مبتنی بر سیمولینک و نمونه‌سازی سریع است.