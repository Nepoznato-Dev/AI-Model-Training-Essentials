...
| | `-`| تفریق | `A - B`|
| | `*`| ضرب | `A * B`|
| | `/`| تقسیم واقعی | `A / B`(همیشه واقعی را برمی گرداند) |
| | `div`| تقسیم عدد صحیح | `A div B`(عدد صحیح را برمی‌گرداند) |
| | `mod`| مدول | `A mod B`|
| **مقایسه** | `=`| برابر | `A = B`|
| | `<>`| برابر نیست | `A <> B`|
| | `<`| کمتر از | `A < B`|
...
| | `<=`| کمتر یا مساوی | `A <= B`|
| | `>=`| بزرگتر یا مساوی | `A >= B`|
| **منطقی** | `and`| منطقی و | `A and B`|
| | `or`| منطقی یا | `A or B`|
| | `not`| منطقی نه | `not A`|
| | `xor`| اختصاصی OR | `A xor B`|
| **رشته** | `+`| الحاق | `'Hello' + ' ' + 'World'`|
| ** مجموعه ** | `in`| عضویت | `X in [1, 2, 3]`|
| | `+`| اتحادیه | `SetA + SetB`|
| | `-`| تفاوت | `SetA - SetB`|
| | `*`| تقاطع | `SetA * SetB`|
| **تکالیف** | `:=`| تخصیص مقدار | `X := 42`|
| **آدرس** | `@`| آدرس-از | `P := @X`|
| | `^`| ارجاع | `P^`|
---
...
## خلاصه
Delphi/Object Pascal ترکیبی از نحو تمیز و قابل خواندن با ویژگی های قدرتمند OOP است. کلاس ها، رابط ها، ژنریک ها و سوابق یک جعبه ابزار کامل برای توسعه برنامه ارائه می دهند. بلوک‌های آغاز/پایان صریح، متغیرهای تایپ‌شده و شناسه‌های بدون حروف کوچک و بزرگ، زبان را قابل دسترسی می‌سازد. چارچوب های VCL/FMX زبان را با کتابخانه های مؤلفه های بصری برای توسعه سریع برنامه گسترش می دهند. برای برنامه‌های کاربردی ویندوز سازمانی و بخش‌های جلویی پایگاه داده، دلفی یک انتخاب سازنده باقی می‌ماند.