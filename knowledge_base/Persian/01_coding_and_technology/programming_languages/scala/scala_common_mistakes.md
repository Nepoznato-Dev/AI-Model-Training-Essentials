...
-->
# Scala - اشتباهات رایج و ضد الگوها
این سند رایج ترین اشتباهات، تله ها و ضد الگوها در اسکالا را با اصلاحات فهرست می کند.
---

## 1. استفاده از`var`به جای `val`
```scala
...
## 2. عدم استفاده از تطبیق الگو برای انواع مهر و موم شده
```scala
// ❌ WRONG — if/else chain
...
## 6.`Option`به درستی مدیریت نمی شود
```scala
...
## 8. Erasure را با Generics تایپ کنید
```scala
...
## خلاصه
قدرت اسکالا با مسئولیت همراه است:`val`را به`var`ترجیح دهید، از تطبیق الگو برای انواع مهر و موم شده استفاده کنید،`Option`را به درستی مدیریت کنید (هرگز`.get`را صدا نکنید)، به جای فیوچرهای تودرتو از برای درک استفاده کنید، و از null اجتناب کنید. Scala 3 بسیاری از این الگوها را با enums، روش های توسعه و انواع اتحادیه ساده می کند. اصل کلیدی: اگر کامپایلر می تواند آن را بررسی کند، اجازه دهید کامپایلر آن را بررسی کند.