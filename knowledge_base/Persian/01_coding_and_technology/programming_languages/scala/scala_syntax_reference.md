...
# Scala - مرجع نحو
این سند یک مرجع نحوی جامع و ساختار یافته برای Scala (3.x با یادداشت‌هایی در مورد تفاوت‌های 2.x) ارائه می‌کند. این مرجع اصلی Scala را با تمرکز بر الگوهای نحوی جامع، سیستم نوع، ساختارهای برنامه‌نویسی کاربردی و اصطلاحات Scala تکمیل می‌کند.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| حسابی | `a + b`| |
| `**`| توان (BigDecimal) | `BigDecimal(2) ** 10`| برای Int | تعبیه نشده است
| `==``!=` | برابری | `a == b`|`.equals()`را فراخوانی می‌کند — بی خطر |
| `eq``ne` | برابری مرجع | `a eq b`| همان نمونه |
| `<``>``<=``>=` | مقایسه | `a >= b`| نیاز به`Ordered`|
| `&&``\|\|``!`| منطقی | `a && b`| اتصال کوتاه |
| `&``\|``^``~` | بیتی | `a & b`| |
| `<<``>>``>>>`| شیفت | `a << 2`| |
| `+:``:+``::``:::` | مجموعه عملیات | `1 +: list`| اگر به`:`ختم می‌شود، راست تداعی کننده |
| `->`| ایجاد تاپل | `"key" -> "value"`| `("key", "value")`|
| `=>`| تابع/فلش | `x => x + 1`| لامبدا یا نگاشت تایپ |
| `<-`| ژنراتور | `x <- List(1,2,3)`| برای درک مطلب |
### قوانین اپراتور
```scala
...
## خلاصه
نحو اسکالا برنامه نویسی شی گرا و کاربردی را تحت یک سیستم نوع قدرتمند یکسان می کند. تطبیق الگو، برای درک، و کلاس های موردی، مدل سازی داده های بیانی را ارائه می دهند. صفات ترکیب انعطاف پذیر را بدون محدودیت های تک وراثتی امکان پذیر می کند. سیستم نوع - با ژنریک، واریانس، مرزهای نوع و انواع غیرشفاف - در زمان کامپایل خطاها را می‌گیرد در حالی که انتزاع‌های هزینه صفر را امکان‌پذیر می‌کند. Scala 3 زبان را با enums، روش‌های توسعه، داده/استفاده و انواع غیرشفاف ساده می‌کند، و زبان را در عین حفظ قدرت کامل، قابل دسترس‌تر می‌کند.