...
|----------|------|---------|-------|
| `+``-``*``/``^`| حسابی | `2^10`| |
| `%%`| مدول | `7 %% 3`| برمی گرداند 1 |
| `%/%`| تقسیم عدد صحیح | `7 %/% 3`| بازگشت 2 |
| `==``!=` | برابری | `x == y`| بردار |
| `<``>``<=``>=` | مقایسه | `x >= y`| بردار |
| `&``\|``!`| منطقی (بردار) | `x & y`| عنصر عاقلانه |
| `&&``\|\|` | منطقی (اسکالر) | `x && y`| فقط عنصر اول |
| `%in%`| عضویت | `x %in% y`| |
| `<-``=` | تکلیف | `x <- 10`| `<-`اصطلاحی است |
| `->`| حق اختصاص | `10 -> x`| به ندرت استفاده می شود |
| `<<-`| اختصاص جهانی | `x <<- 10`| اختصاص دادن در env والد |
| `$`| استخراج عنصر | `df$column`| |
| `[``[[` | نمایه سازی | `x[1]`/`x[[1]]`| `[`همان نوع را برمی گرداند.  عصاره`[[`|
| `%>%`| لوله (magrittr) | `x %>% f()`| |
| `\|>`| لوله (پایه R 4.1+) | `x \|> f()`| لوله بومی |
### عملیات بردار
```r
...
## خلاصه
نحو R برای محاسبات آماری و تجزیه و تحلیل داده ها طراحی شده است. ماهیت برداری عملیات ها حلقه ها را برای اکثر وظایف حذف می کند. اکوسیستم tidyverse (dplyr، ggplot2، tidyr) یک دستور زبان سازگار و قابل خواندن برای دستکاری و تجسم داده ها ارائه می دهد. قابلیت‌های مدل‌سازی آماری R - از آزمون‌های t پایه تا مدل‌های اثرات مختلط - از نظر عمقی بی‌نظیر هستند. برای علم داده، آمار و تحقیقات قابل تکرار، R همچنان یک ابزار ضروری است.