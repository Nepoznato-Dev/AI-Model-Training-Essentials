...
## Bash - اپراتورها و متغیرها
### اپراتورهای اصلی
| اپراتور | نام | مثال | یادداشت ها |
|----------|------|---------|-------|
| `=`| تکلیف | `x=10`| هیچ فضایی در اطراف`=`وجود ندارد |
| `$var`| بسط متغیر | `echo $HOME`| |
| `${var}`| انبساط مهاربندی شده | `${HOME}/docs`| ابهام می کند |
| `$(cmd)`| تعویض دستور | `$(date +%Y)`| ارجح بر بکتیک |
| `$((expr))`| حسابی | `$((2 + 3))`| |
| `${var:-default}`| مقدار پیش فرض | `${PORT:-8080}`| اگر تنظیم نشده یا خالی |
| `${var:=default}`| اختصاص پیش فرض | `${count:=0}`| |
| `${#var}`| طول رشته | `${#name}`| |
| `${var%%pattern}`| حذف طولانی ترین پسوند | `${file%%.*}`| حذف پسوند |
| `${var##pattern}`| حذف طولانی ترین پیشوند | `${path##*/}`| دریافت نام فایل |
| `"``"` | نقل قول دوگانه | `"$var"`| اجازه گسترش |
| `'``'` | نقل قول تکی | `'$var'`| رشته تحت اللفظی |
### اپراتورهای تست
```bash
# File tests
...
## PowerShell - توابع و الگوهای پیشرفته
```powershell
# Basic function
...
## خلاصه
Bash و PowerShell دو پارادایم از اسکریپت نویسی پوسته را نشان می دهند. Bash متن را لوله می کند - هر فرمان رشته ها را تبدیل می کند. PowerShell اشیاء را لوله می کند - هر دستور داده های ساختار یافته را با ویژگی ها و روش ها تولید می کند. Bash بر Linux/macOS و DevOps تسلط دارد. PowerShell برای مدیریت ویندوز و به طور فزاینده ای بین پلتفرمی ضروری است. هر دو ابزار ضروری در یک پشته فناوری مدرن هستند. اسکریپت های پوسته چسبی هستند که سیستم ها را به هم متصل می کنند، گردش کار را خودکار می کنند و کارها را انجام می دهند.