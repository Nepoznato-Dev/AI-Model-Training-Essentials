...
# Shell & PowerShell - اشتباهات رایج و ضد الگوها
این سند رایج ترین اشتباهات، تله ها و ضد الگوها در Shell/Bash و PowerShell را با اصلاحات فهرست می کند.
---

## 1. متغیرهای نقل قول نشده (Bash)
```bash
...
## 5. PowerShell: درون یابی رشته ای
```powershell
...
## 8. PowerShell: اشیاء خط لوله در مقابل رشته ها
```powershell
...
## خلاصه
تله های اسکریپت نویسی پوسته: همیشه متغیرها را در Bash نقل قول کنید، از`[[ ]]`بر روی`[ ]`استفاده کنید، از`set -euo pipefail`استفاده کنید، کدهای خروج را بررسی کنید، و محدوده زیر پوسته را درک کنید. تله‌های پاورشل: از نقل قول‌های دوگانه برای درونیابی استفاده کنید، از`-eq`استفاده کنید نه `==`، و به یاد داشته باشید که اشیاء خط لوله دارای ویژگی‌هایی هستند. هر دو به نقل قول دقیق و رسیدگی صریح خطا پاداش می دهند.