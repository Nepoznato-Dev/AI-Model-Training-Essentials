...
---
# TypeScript - اشتباهات رایج و ضد الگوها
این سند رایج ترین اشتباهات، تله ها و ضد الگوها را در TypeScript فهرست می کند. هر ورودی رویکرد نادرست را نشان می دهد، توضیح می دهد که چرا شکست می خورد، و راه حل صحیح را ارائه می دهد.
---

## 1. استفاده بیش از حد از `any`
```typescript
...
## 2. به جای تایپ گاردها، اظهارات را تایپ کنید
```typescript
...
## 4. سردرگمی رابط ها در مقابل انواع
```typescript
...
## 9. ضد الگو: انواع تکراری (عدم استفاده از ژنریک)
```typescript
...
## 12. عدم استفاده از اتحادیه های تبعیض آمیز
```typescript
...
## خلاصه
سیستم تایپ اسکریپت قدرتمند است اما نیاز به نظم و انضباط دارد. گناه اصلی رسیدن به`any`است - زمانی که نوع آن واقعاً ناشناخته است، از`unknown`استفاده کنید و با محافظ‌های نوع محدود است. از اظهارات غیر تهی (`!`) با مدیریت صحیح تعریف نشده اجتناب کنید. از رابط ها برای اشکال شیء، انواع برای اتحادیه ها و انواع محاسبه شده استفاده کنید. اتحادیه های تبعیض آمیز کل کلاس های باگ را حذف می کنند.`strict: true`را از روز اول فعال کنید — سختگیری مقاوم سازی دردناک است. هدف این است که دولت های غیرقانونی را در سطح نوع غیرقانونی نشان دهیم.