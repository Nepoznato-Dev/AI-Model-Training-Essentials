...
# SQL - تاریخچه نسخه و تکامل
## جدول زمانی
| نسخه | سال | تم کلید |
|---------|------|-----------|
| دنباله | 1974 | زبان تحقیقاتی اصلی IBM (چمبرلین و بویس) |
| SQL-86 | 1986 | **اولین استاندارد ANSI** (SQL-86) |
| SQL-89 | 1989 | بازبینی جزئی (محدودیت های یکپارچگی) |
| SQL-92 | 1992 | **عناوین**:`JOIN`, subqueries,`CASE`,`COALESCE`|
| SQL:1999 | 1999 | **عبارات منظم**، پرس و جوهای بازگشتی، محرک ها، BLOB |
| SQL:2003 | 2003 | **توابع پنجره**، XML، ستون های`GENERATED`|
| SQL:2006 | 2006 | پشتیبانی از XML،`MERGE`|
| SQL:2008 | 2008 |  تریگرهای `INSTEAD OF`، `TRUNCATE`،`ORDER BY`در بازدید |
| SQL:2011 | 2011 | **داده های زمانی** (`AS OF`,`FOR SYSTEM_TIME`),`FETCH FIRST`|
| SQL:2016 | 2016 | **پشتیبانی از JSON**، تشخیص الگوی ردیف |
| SQL: 2019 | 2019 | **توابع جدول چندشکلی**،`LISTAGG`|
| SQL:2023 | 2023 | ** عملیات`JSON_TABLE`**، `SET`، بهبود آرایه |
## نقاط عطف اصلی
### SEQUEL و Early SQL (1974-1986)
- **1974**: دونالد چمبرلین و ریموند بویس SEQUEL را در تحقیقات IBM ایجاد کردند.
- **هدف**: دستکاری پرس و جو برای System R (پایگاه داده رابطه ای)
- به دلیل تضاد علامت تجاری به SQL (زبان جستجوی ساختاریافته) تغییر نام داد
- **1986**: اولین استاندارد ANSI (SQL-86)
- **1987**: ISO SQL-87 را پذیرفته است
### SQL-92 - The Foundation (1992)
- **مهمترین استاندارد** - همه SQLهای مدرن از این نشات می گیرند
- `INNER JOIN`، `LEFT JOIN`،`RIGHT JOIN`
- سوالات فرعی (تودرتو با `SELECT`)
- بیان `CASE`
- `COALESCE`،`NULLIF`
- محدودیت های `UNIQUE`، `CHECK`
- تعریف طرحواره (`CREATE SCHEMA`)
### SQL:1999 - SQL مدرن آغاز می شود (1999)
- عبارات منظم (`LIKE`، `SIMILAR TO`)
- پرس و جوهای بازگشتی (`WITH RECURSIVE`)
- محرک ها
- BLOB/CLOB (اشیاء بزرگ باینری/شخصیتی)
- انواع تعریف شده توسط کاربر (UDT)
-`ORDER BY`در سوالات فرعی
### SQL:2003 — Analytics Revolution (2003)
- **عملکردهای پنجره**:`ROW_NUMBER()`,`RANK()`,`DENSE_RANK()`,`LAG()`,`LEAD()`,`SUM() OVER()`
- نوع داده XML و توابع
-`GENERATED ALWAYS AS IDENTITY`
-`SAVEPOINT`(کنترل تراکنش)
- توابع هش
### SQL:2011 - داده های زمانی (2011)
- **جدول زمانی**:`FOR SYSTEM_TIME AS OF`,`VERSIONING`
-`FETCH FIRST n ROWS ONLY`(استاندارد `LIMIT`)
- صفحه بندی`OFFSET`/ `FETCH`
### SQL: 2016–2023 - JSON & Beyond (2016–اکنون)
- **2016**: نوع داده JSON، `JSON_VALUE`، `JSON_QUERY`،`JSON_EXISTS`
- **2019**: توابع جدول چند شکلی،`LISTAGG`
- **2023**:`JSON_TABLE`(نمای رابطه ای JSON)، عملیات `SET`، بهبود آرایه
## تکامل نحو
```sql
