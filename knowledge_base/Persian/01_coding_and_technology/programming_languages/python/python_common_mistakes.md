...
-->
# Python - اشتباهات رایج و ضد الگوها
این سند رایج ترین اشتباهات، تله ها و ضد الگوها را در پایتون فهرست می کند. هر ورودی رویکرد نادرست را نشان می دهد، توضیح می دهد که چرا شکست می خورد، و راه حل صحیح را ارائه می دهد. درک این مشکلات به شما کمک می‌کند کد پایتونیک قوی‌تری بنویسید.
---

## 1. آرگومان های پیش فرض قابل تغییر
```python
...
**چرا این اتفاق می افتد:** آرگومان های پیش فرض یک بار در زمان تعریف تابع ارزیابی می شوند، نه در هر فراخوانی. یک پیش‌فرض قابل تغییر (لیست، دستور، مجموعه) در همه فراخوان‌ها به اشتراک گذاشته می‌شود.
---

## 2. اصلاح فهرست در حین تکرار
```python
...
## 4. استفاده از`type()`به جای `isinstance()`
```python
...
## 5. عدم استفاده از مدیران زمینه برای منابع
```python
...
## 8. عدم درک قوانین محدوده پایتون (LEGB)
```python
...
## 10. Anti-Pattern: استفاده از`except:`Bare به جز
```python
...
## 11. برگرداندن مراجع قابل تغییر به حالت داخلی
```python
...
## 12. عدم استفاده از`setdefault`یا `collections.defaultdict`
```python
...
## 15. ضد الگو: اشیاء خدا / کلاس های بیش از حد
```python
...
## خلاصه
سادگی پایتون فریبنده است – پیش‌فرض‌های قابل تغییر، بسته شدن دیرهنگام الزام‌آور، قوانین محدوده و مدیریت منابع، همگی رفتارهای ظریفی دارند که توسعه‌دهندگان را به دام می‌اندازند. اصول کلیدی که باید به خاطر بسپارید: پیش‌فرض‌ها یک بار ارزیابی می‌شوند،`is`هویت را بررسی می‌کند نه برابری، همیشه از مدیران زمینه برای منابع استفاده می‌کند، استثناهای خاص را می‌گیرد و ترکیب را بر اشیاء خدا ترجیح می‌دهد. نوشتن کد پایتونیک به معنای درک این مشکلات و استفاده از ویژگی‌های زبان (مدیران زمینه، درک، مولدها، ماژول `collections`) است.