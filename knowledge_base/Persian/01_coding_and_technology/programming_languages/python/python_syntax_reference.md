...
---
# Python - مرجع نحو
این سند یک مرجع جامع و ساختار یافته برای Python ارائه می دهد. این مرجع اصلی پایتون را با تمرکز بر الگوهای نحوی جامع، جداول عملگر و مکانیک داخلی تکمیل می کند.
---
...
|----------|------|---------|--------|-------|
| `+`| اضافه | `3 + 2`| `5`| همچنین الحاق رشته/لیست |
| `-`| تفریق | `3 - 2`| `1`| همچنین نفی واحدی |
| `*`| ضرب | `3 * 2`| `6`| همچنین تکرار رشته/لیست |
| `/`| بخش واقعی | `7 / 2`| `3.5`| همیشه شناور برمی گردد |
| `//`| بخش طبقه | `7 // 2`| `3`| به سمت بی نهایت منفی می گردد |
| `%`| مدول | `7 % 2`| `1`| علامت منطبق بر مقسوم علیه |
| `**`| توانمندی | `2 ** 10`| `1024`| راست انجمنی |
### مقایسه و عملگرهای بولی
| اپراتور | نام | مثال | یادداشت ها |
|----------|------|---------|-------|
| `==`| برابر | `x == y`| مقایسه ارزش (نه هویت) |
| `!=`| برابر نیست | `x != y`| |
| `<`,`>`,`<=`,`>=`| سفارش | `x >= y`| زنجیرپذیر:`a < b < c`|
| `is`| هویت | `x is y`| همان شی در حافظه |
| `is not`| نه هویت | `x is not None`| ارجح بر`!= None`|
| `in`| عضویت | `x in collection`| O(1) برای مجموعه ها/دیکت ها، O(n) برای لیست ها |
| `and`| منطقی و | `a and b`| اتصال کوتاه؛ آخرین مقدار true/falsy |
| `or`| منطقی یا | `a or b`| اتصال کوتاه؛ اولین مقدار true/falsy |
| `not`| منطقی نه | `not x`| |
### عملگرهای بیتی
| اپراتور | نام | مثال | یادداشت ها |
|----------|------|---------|-------|
...
| تقدم | اپراتورها | انجمنی |
|------------|-----------|---------------|
| 1 (بالاترین) | `()`| گروه بندی |
| 2 | `**`| راست |
| 3 | `~`,`+x`,`-x`| Unary |
| 4 | `*`,`/`,`//`,`%`| چپ |
| 5 | `+`,`-`| چپ |
...
| 8 | `^`| چپ |
| 9 | `\|`| چپ |
| 10 | `==`,`!=`,`<`,`>`,`<=`,`>=`,`is`, XQZXQZ2 | زنجیر شونده |
| 11 | `not`| راست |
| 12 | `and`| چپ |
| 13 (کمترین) | `or`| چپ |
### تکلیف و تکلیف افزوده
```python
...
## ساختارهای داده
### نمای کلی مجموعه های داخلی
| مجموعه | سفارش داد | قابل تغییر | تکراری | نحو | جستجو |
|------------|---------|--------|------------|--------|--------|
| `list`| بله | بله | بله | `[1, 2, 3]`| O(n) |
| `tuple`| بله | نه | بله | `(1, 2, 3)`| O(n) |
| `dict`| بله (3.7+) | بله | کلیدها: خیر | `{"k": "v"}`| O(1) میانگین |
| `set`| نه | بله | نه | `{1, 2, 3}`| O(1) میانگین |
| `frozenset`| نه | نه | نه | `frozenset({1,2})`| O(1) میانگین |
### لیست ها
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
### سیستم و پروتکل ها را تایپ کنید
```python
from typing import (
...
## خلاصه
این مرجع دستوری مجموعه کامل ویژگی های پایتون را پوشش می دهد - از عملگرهای اصلی و جریان کنترل از طریق فرابرنامه نویسی پیشرفته، توصیفگرها و سیستم نوع. سینتکس پایتون عمدا حداقل و قابل خواندن است، اما عمق آن قابل توجه است: تطبیق الگو، ژنراتورهای همگام، پروتکل ها و متاکلاس ها ابزارهای قدرتمندی را برای برنامه های پیچیده فراهم می کنند. این زبان همچنان به تکامل خود ادامه می‌دهد و هر نسخه ویژگی‌هایی را اضافه می‌کند که بیانگر را بهبود می‌بخشد و در عین حال سازگاری با عقب را حفظ می‌کند.