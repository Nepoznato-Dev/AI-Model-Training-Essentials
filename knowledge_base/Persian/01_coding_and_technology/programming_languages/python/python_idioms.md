...
## خلاصه
کد پایتونیک از ذن پایتون پیروی می کند: خوانا، صریح، ساده و مسطح. اصطلاحات کلیدی عبارتند از: درک فهرست/دیکت/مجموعه، مدیران زمینه (`with`)، مدیریت خطای EAFP،`enumerate`/`zip`برای تکرار،`dataclass`برای محفظه‌های داده، نکات نوع، باز کردن بسته‌بندی، و بررسی درستی/نادرستی. جامعه پایتون برای خوانایی ارزش قائل است و "باید یک راه واضح برای انجام آن وجود داشته باشد." از PEP 8 برای سبک پیروی کنید، از`ruff`برای پرده‌بندی استفاده کنید و در صورت امکان کتابخانه استاندارد را بر راه‌حل‌های شخص ثالث ترجیح دهید.