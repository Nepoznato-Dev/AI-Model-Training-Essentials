...
---
# ج - اشتباهات رایج و ضد الگوها
این سند رایج‌ترین اشتباهات، تله‌ها و ضد الگوها را در C فهرست می‌کند. هر ورودی رویکرد نادرست را نشان می‌دهد، توضیح می‌دهد که چرا شکست می‌خورد و راه‌حل صحیح را ارائه می‌دهد. C به شما کنترل کامل می دهد - که به معنای مسئولیت کامل در قبال ایمنی حافظه، بررسی محدوده ها و رفتار نامشخص است.
---

## 1. سرریز بافر با`gets()`/ `strcpy()`
```c
...
## 8. آسیب پذیری های رشته را قالب بندی کنید
```c
...
## خلاصه
خطرات C شناخته شده هستند اما همیشه وجود دارند: سرریز بافر، نشانگرهای آویزان، سرریز اعداد صحیح، حملات رشته های قالب بندی، و نشت حافظه. نظم این است: همیشه مقادیر برگشتی را بررسی کنید، همیشه آنچه را که اختصاص می دهید آزاد کنید، همیشه از توابع محدود استفاده کنید (`snprintf`، `strncpy`، `fgets`)، هرگز از`gets()`استفاده نکنید، همه متغیرها را مقداردهی اولیه کنید، و از ابزارهایی مانند Valgrind و AddressS استفاده کنید. C به برنامه نویسان دقیق با عملکرد و کنترل بی بدیل پاداش می دهد - اما بی دقتی را با رفتارهای تعریف نشده و آسیب پذیری های امنیتی مجازات می کند.