...
## خلاصه
اصطلاحات C تاکید دارند: مدیریت دقیق حافظه، برنامه‌ریزی دفاعی، ساختارهای مات برای محصور کردن، ماکروها با do-while، نقاط خروجی تکی با پاک‌سازی goto، و ویژگی‌های مدرن C11/C23.`clang-format`را برای قالب‌بندی،`clang-tidy`را برای لینتینگ دنبال کنید و همیشه با`-Wall -Wextra -Werror`کامپایل کنید. در طول آزمایش از ضد عفونی کننده ها (ASan، UBSan) استفاده کنید. جامعه C برای سادگی، عملکرد و کنترل صریح ارزش قائل است - "به برنامه نویس اعتماد کنید."