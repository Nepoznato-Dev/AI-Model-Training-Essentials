...
| `*`| ضرب | `3 * 2`| `6`| |
| `/`| بخش | `7 / 2`| `3`| برش عدد صحیح برای انواع int |
| `%`| مدول | `7 % 2`| `1`| فقط برای انواع عدد صحیح |
| `++`| افزایش | `i++`/`++i`| | پس و پیش افزایشی |
| `--`| کاهش | `i--`/`--i`| | پس از کاهش و پیش کاهش |
...
| `\|`| یا | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `~`| مکمل | `~0`| همه بیت ها برگردانده شدند |
| `<<`| Shift چپ | `1 << 3`| `8`|
| `>>`| شیفت راست | `8 >> 3`| `1`(پیاده سازی برای امضا شده) |
### اپراتورهای اشاره گر و آدرس
| اپراتور | نام | مثال | یادداشت ها |
|----------|------|---------|-------|
| `&`| آدرس-از | `&x`| نشانگر را به x | برمی گرداند
| `*`| ارجاع | `*ptr`| مقدار دسترسی در نشانگر |
| `->`| دسترسی اعضا از طریق اشاره گر | `p->x`| معادل`(*p).x`|
| `.`| دسترسی اعضا | `s.field`| دسترسی مستقیم اعضای ساختار |
| `[]`| زیرنویس آرایه | `arr[i]`| معادل`*(arr + i)`|
### اولویت اپراتور (بیشترین به کمترین)
| تقدم | اپراتورها | انجمنی |
|------------|-----------|---------------|
| 1 (بالاترین) | `()``[]``->``.` | از چپ به راست |
| 2 | `!``~``+``-` (یکی)`*``&``sizeof``(type)` | راست به چپ |
| 3 | `*``/``%`| از چپ به راست |
| 4 | `+``-` | از چپ به راست |
| 5 | `<<``>>` | از چپ به راست |
| 6 | `<``<=``>``>=` | از چپ به راست |
| 7 | `==``!=` | از چپ به راست |
| 8 | `&`(بیتی و) | از چپ به راست |
| 9 | `^`| از چپ به راست |
| 10 | `\|`| از چپ به راست |
| 11 | `&&`| از چپ به راست |
| 12 | `\|\|`| از چپ به راست |
| 13 | `? :`(سه تایی) | راست به چپ |
| 14 | `=``+=``-=``*=``/=``%=``&=``\|=``*=``>>=` | راست به چپ |
| 15 (کمترین) | `,`(کاما) | از چپ به راست |
---
...
## خلاصه
سینتکس C حداقل و نزدیک به سخت افزار است - اشاره گرها، مدیریت حافظه دستی، و یک لایه انتزاعی نازک بر روی دستورالعمل های ماشین. قدرت این زبان از همین سادگی ناشی می‌شود: تقریباً هیچ هزینه پنهان، سربار زمان اجرا و کنترل کامل بر طرح‌بندی حافظه وجود ندارد. پیش پردازنده، در حالی که قدرتمند است، برای استفاده ایمن نیاز به نظم و انضباط دارد. C مدرن (C11/C17/C23) ویژگی‌های ایمنی مهمی مانند `_Static_assert`، `_Generic`، `nullptr`، و`typeof`را اضافه کرده است و زبان را به‌طور تدریجی ایمن‌تر می‌کند و در عین حال فلسفه اصلی خود را برای اعتماد به برنامه‌نویس حفظ می‌کند.