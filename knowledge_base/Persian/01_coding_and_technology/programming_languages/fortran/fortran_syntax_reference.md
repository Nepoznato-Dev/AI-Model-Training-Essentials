...
# Fortran - مرجع نحو
این سند یک مرجع نحوی جامع و ساختار یافته برای Modern Fortran (2008/2018) ارائه می دهد. این مرجع اصلی Fortran را با تمرکز بر الگوهای نحوی جامع، عملیات آرایه، ماژول‌ها، OOP و محاسبات با کارایی بالا تکمیل می‌کند.
---
...
| `<``>``<=``>=` | مقایسه | `a >= b`| |
| `.and.``.or.``.not.`| منطقی | `a .and. b`| |
| `.eqv.``.neqv.` | معادل منطقی | `a .eqv. b`| |
| `//`| الحاق رشته | `'hello' // ' world'`| |
---
...
## خلاصه
نحو مدرن فرترن تمیز، ساختار یافته و برای محاسبات عددی بهینه شده است. عملیات آرایه حلقه ها را برای اکثر کارهای ریاضی حذف می کند. ماژول ها کپسوله سازی و مدیریت فضای نام را فراهم می کنند. ویژگی های OOP (انواع مشتق شده، وراثت، چندشکلی) سازماندهی کد تمیز را فعال می کند. Coarray ها برنامه نویسی موازی بومی را بدون کتابخانه های خارجی ارائه می دهند. نقاط قوت فرترن - عملکرد، عملیات آرایه، و دقت عددی - برای محاسبات علمی بی‌نظیر باقی می‌مانند. برای HPC، فیزیک محاسباتی و شبیه سازی مهندسی، فرترن همچنان یک نیروگاه است.