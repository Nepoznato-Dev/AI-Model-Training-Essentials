...
-->
# Lua - اشتباهات رایج و ضد الگوها
این سند رایج ترین اشتباهات، تله ها و ضد الگوها در Lua را با اصلاحات فهرست می کند.
---
...
## 4. متغیرهای جهانی به صورت پیش فرض
```lua
...
## 7. عدم استفاده از Metatables برای OOP
```lua
...
## خلاصه
مینیمالیسم Lua تله‌هایی ایجاد می‌کند: نمایه‌سازی مبتنی بر 1، فقط`nil`و`false`نادرست هستند، جداول مرجع هستند (نه مقادیر)، متغیرها به طور پیش‌فرض سراسری هستند و`pairs`نامرتب است در حالی که`ipairs`مرتب شده است. روش Lua این است: همیشه از`local`استفاده کنید، از`ipairs`برای آرایه ها و`pairs`برای فرهنگ لغت ها، از`table.concat`برای ساخت رشته ها و استفاده از جدول های متقابل برای OOP استفاده کنید. سادگی Lua قدرت آن است - اما به موارد لبه احترام بگذارید.