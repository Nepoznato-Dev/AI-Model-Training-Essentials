...
-->
# جولیا - مرجع نحو
این سند یک مرجع نحوی ساختار یافته و جامع برای جولیا (1.x) فراهم می‌کند. این مرجع اصلی جولیا را با تمرکز بر الگوهای نحوی جامع، ارسال چندگانه، ماکروها، فرابرنامه‌نویسی و محاسبات علمی تکمیل می‌کند.
---
...
|----------|------|---------|-------|
| `+``-``*``/``^`| حسابی | `2^10`| |
| `÷`| تقسیم عدد صحیح | `7 ÷ 3`| همانند`div(7, 3)`|
| `%``\` | مدول/باقیمانده | `7 % 3`| |
| `==``!=` | برابری | `a == b`| برابری ارزش |
| `===`| هویت | `a === b`| همان شی |
| `<``>``<=``>=` | مقایسه | `a >= b`| زنجیرپذیر:`1 < x < 10`|
| `&&``\|\|``!`| منطقی | `a && b`| اتصال کوتاه |
| `&``\|``⊻``~` | بیتی | `a & b`| `⊻`XOR است |
| `:`| محدوده | `1:10`| `start:step:stop`|
| `..`| فاصله | `1..10`| نیاز به IntervalSets |
| `∈``∉``⊆`| تنظیم عضویت | `x ∈ S`| `in(x, S)`|
| `∘`| ترکیب | `f ∘ g`| `(f ∘ g)(x) == f(g(x))`|
| `\|>`| لوله | `x \|> f`| `f(x)`|
| `=>`| جفت | `a => b`| برای دیکته |
| `...`| اسپلت | `f(args...)`| گسترش مجموعه |
...
## خلاصه
نحو جولیا تمیز، ریاضی و گویا است. ارسال چندگانه به توابع اجازه می دهد تا بر اساس همه انواع آرگومان رفتار متفاوتی داشته باشند و سازماندهی کد زیبا را ممکن می سازد. ماکروها فرابرنامه نویسی قدرتمندی را بدون پیچیدگی ماکروهای پیش پردازنده به سبک C ارائه می دهند. سیستم نوع - با انواع پارامتریک، انواع انتزاعی، و انواع اتحادیه - انعطاف پذیری و عملکرد را امکان پذیر می کند. نمایه سازی مبتنی بر 1 جولیا، درک، و نحو انجام بلوک آن را برای محاسبات ریاضی و علمی طبیعی می کند. این زبان از طریق کامپایل JIT با استنتاج نوع به وعده خود "راه رفتن مانند پایتون، اجرا مانند C" خود می رسد.