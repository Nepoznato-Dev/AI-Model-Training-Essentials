...
|----------|------|---------|-------|
| `+``-``*``/``%`| حسابی | `a + b`| |
| `..`| محدوده | `1..10`| محدوده فراگیر |
| `until`| محدوده نیمه باز | `1 until 10`| `1..9`|
| `downTo`| محدوده نزولی | `10 downTo 1`| |
| `==``!=` | برابری ساختاری | `a == b`|`.equals()`|
| `===``!==` | برابری مرجع | `a === b`| همان نمونه |
| `<``>``<=``>=` | مقایسه | `a >= b`| نیاز به`Comparable`|
| `&&``\|\|``!`| منطقی | `a && b`| اتصال کوتاه |
| `?:`| اپراتور الویس | `a ?: b`| اگر`a`تهی باشد،`b`را برمی گرداند |
| `?.`| تماس ایمن | `a?.b`| اگر`a`تهی باشد |
| `!!`| ادعای غیر تهی | `a!!`| NPE را در صورت تهی می اندازد |
| `as`| بازیگران | `a as String`| پرتاب ClassCastException |
| `as?`| بازیگران ایمن | `a as? String`| در صورت عدم موفقیت |
| `in`| عضویت | `x in list`| |
| `!in`| عضو نیست | `x !in list`| |
| `is`| بررسی تایپ | `x is String`| بازیگران هوشمند پس از بررسی |
| `!is`| نوع | `x !is String`| |
...
## خلاصه
نحو کاتلین مختصر، ایمن و قابل همکاری با جاوا است. امنیت تهی یک کلاس کامل از اشکالات را در زمان کامپایل حذف می کند. Coroutine ها روشی طبیعی برای نوشتن کدهای ناهمزمان ارائه می دهند. کلاس های مهر و موم شده، کلاس های داده، و توابع پسوند، مدل سازی دامنه بیانی را امکان پذیر می کنند. قدرت این زبان از تاثیر متقابل ویژگی‌های آن ناشی می‌شود - توابع دامنه، DSL، کلاس‌های درون خطی، و ژنریک‌های اصلاح‌شده برای تولید کدی که نوشتن آن هم امن و هم لذت‌بخش است.