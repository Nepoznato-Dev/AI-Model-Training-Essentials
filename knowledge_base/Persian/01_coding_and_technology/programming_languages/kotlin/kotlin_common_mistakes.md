...
-->
# کاتلین - اشتباهات رایج و ضد الگوها
این سند رایج ترین اشتباهات، تله ها و ضد الگوها را در کاتلین فهرست می کند. هر ورودی رویکرد نادرست را نشان می دهد، توضیح می دهد که چرا شکست می خورد، و راه حل صحیح را ارائه می دهد.
---

## 1. انواع پلتفرم و NullPointerException
```kotlin
...
## 3.`data class`بدون`equals`مناسب / `hashCode`
```kotlin
...
## 4. عدم استفاده صحیح از توابع Scope
```kotlin
...
## 7. جامع بودن کلاس مهر و موم شده
```kotlin
...
## 8. تابع برنامه افزودنی سایه زدن
```kotlin
...
## 9. عدم استفاده از`by lazy`برای مقداردهی اولیه
```kotlin
...
## 10. جریان در مقابل سردرگمی LiveData
```kotlin
...
## خلاصه
مختصر بودن کاتلین می‌تواند مشکلات ظریفی را پنهان کند: انواع پلتفرم از interop جاوا، خرابی `lateinit`، مجموعه‌های شکسته کلاس‌های داده قابل تغییر، و لغو برنامه‌های روزمره. روش Kotlin به این صورت است: از ایمنی پوچ استفاده نکنید (هرگز از`!!`استفاده نکنید)، از تابع دامنه مناسب استفاده کنید (`let` برای تبدیل،`apply`برای پیکربندی،`also`برای عوارض جانبی)، استفاده از کلاس های مهر و موم شده با`let`جامع، و Writein. Kotlin اصطلاحی (نه جاوا با نحو Kotlin). کامپایلر راهنمای شماست - اگر هشدار داد، گوش کنید.