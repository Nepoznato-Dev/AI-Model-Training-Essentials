...
---
# برو - اشتباهات رایج و ضد الگوها
این سند رایج ترین اشتباهات، تله ها و ضد الگوها را در Go فهرست می کند. هر ورودی رویکرد نادرست را نشان می دهد، توضیح می دهد که چرا شکست می خورد، و راه حل صحیح را ارائه می دهد.
---

## 1. ضبط متغیر حلقه در گوروتین ها
```go
...
## 5. اشاره گر صفر در روش های ساختاری
```go
...
## 7. Anti-Pattern: بازگشت نابهنگام متغیرهای نامگذاری شده
```go
...
## 11. عدم استفاده از`sync.Once`برای مقداردهی اولیه
```go
...
## خلاصه
سادگی Go نقطه قوت آن است، اما تله‌های ظریفی دارد: نشت‌های گوروتین از کانال‌های بافر نشده، برش معنایی پیوست که اصل را تغییر نمی‌دهد، تماس‌های معوقی که در حلقه‌ها انباشته می‌شوند، و تکرار نقشه‌ای که عمداً تصادفی شده است. ضرب المثل های Go این راه را راهنمایی می کنند: هر خطا را مدیریت کنید، رابط ها را بپذیرید اما ساختارها را برگردانید، از گیرنده های اشاره گر برای انواع mutex استفاده کنید، و اولویت های اولیه همزمانی (`sync.Once`، `errgroup`، لغو متن) را به مدیریت گوروتین دستی ترجیح دهید. ابتدا کد ساده بنویسید - به وضوح به جای زیرکی پاداش دهید.