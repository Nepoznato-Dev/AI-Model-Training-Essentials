...
|----------|------|---------|-------|
| `+``-``*``/``%``**` | حسابی | `2 ** 10`| `**`قدرت است |
| `==``!=``===``<=>` | برابری | `a == b`| `===`برابری حروف است. `<=>`سفینه فضایی است |
| `eql?`| برابری ارزش | `a.eql?(b)`| سختگیرتر از`==`(نوع چک) |
| `equal?`| هویت | `a.equal?(b)`| همان شی؟ |
| `<``>``<=``>=` | مقایسه | `a >= b`| |
| `&&``\|\|``!`| منطقی | `a && b`| اتصال کوتاه؛ آخرین مقدار ارزیابی شده |
| `and``or``not`| منطقی (تقدم پایین) | `a and b`| اجتناب کنید — استفاده از`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | بیتی | `a & b`| |
| `=~``!~` | بازی Regex | `str =~ /pattern/`| برمی گرداند شاخص یا صفر |
| `..``...` | محدوده | `1..10`| `..`شامل؛ `...`اختصاصی |
| `?.`| ناوبری ایمن | `user&.name`| اگر کاربر nil باشد، صفر را برمی‌گرداند
| `[]``[]=` | دسترسی عنصر | `arr[0]`| |
### حقیقت
```ruby
...
## خلاصه
نحو Ruby برای شادی توسعه دهندگان طراحی شده است - مانند انگلیسی خوانده می شود و به بیان پاداش می دهد. قدرت این زبان از مدل شی آن (همه چیز یک شی است)، بلوک‌ها و بسته‌ها، قابلیت‌های فرابرنامه‌نویسی، و ماژول Enumerable می‌آید که یک API پردازش مجموعه غنی را فراهم می‌کند. Ruby 3.x تطبیق الگو، امضاهای نوع RBS و Ractor را برای همزمانی اضافه کرده است، در حالی که فلسفه اصلی زبان را حفظ می کند: برنامه نویسان را سازنده و خوشحال کنید.