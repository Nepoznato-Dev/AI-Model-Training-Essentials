...
## اپراتورها و عبارات
| دسته بندی | اپراتور | توضیحات | مثال |
|----------|----------|------------|---------|
| **حساب** | `+`| اضافه | `a + b`|
| | `-`| تفریق | `a - b`|
| | `*`| ضرب | `a * b`|
| | `/`| تقسیم (دوبل برمی گرداند) | `a / b`|
| | `\`| تقسیم عدد صحیح | `a \ b`|
| | `Mod`| مدول | `a Mod b`|
| | `^`| توانمندی | `a ^ b`|
| **مقایسه** | `=`| برابر | `a = b`|
...
| | `<=`| کمتر یا مساوی | `a <= b`|
| | `>=`| بزرگتر یا مساوی | `a >= b`|
| **منطقی** | `AndAlso`| اتصال کوتاه و | `a AndAlso b`|
| | `OrElse`| اتصال کوتاه یا | `a OrElse b`|
| | `Not`| منطقی نه | `Not a`|
| | `And`| بیتی و | `a And b`|
| | `Or`| بیتی OR | `a Or b`|
| | `Xor`| بیتی XOR | `a Xor b`|
| **رشته** | `&`| الحاق | `"Hello" & " " & "World"`|
| | `+=`| ضمیمه | `s &= " more"`|
| **تکالیف** | `=`| اختصاص | `x = 42`|
| **تهی** | `Is`| برابری مرجع | `obj Is Nothing`|
| | `IsNot`| نابرابری مرجع | `obj IsNot Nothing`|
| **نوع** | `TryCast`| بازیگران ایمن (هیچ چیز را برمی گرداند) | `TryCast(obj, String)`|
| | `CType`| تبدیل | `CType(obj, Integer)`|
| | `DirectCast`| بازیگران مستقیم (پرتاب روی شکست) | `DirectCast(obj, String)`|
---
...
## خلاصه
سینتکس ویژوال بیسیک تمیز و شبیه به انگلیسی است و آن را به یکی از خواناترین زبان های برنامه نویسی تبدیل می کند. VB.NET دسترسی کامل به اکوسیستم دات نت شامل LINQ برای پرس و جوی داده، همگام سازی/انتظار برای عملیات همزمان، ژنریک برای ایمنی نوع، و OOP جامع با کلاس ها، رابط ها و وراثت را فراهم می کند. کلمات کلیدی پرمخاطب و عدم حساسیت زبان (End If، End Sub، End Class) خوانایی زبان را افزایش می دهد. برای برنامه های کاربردی ویندوز سازمانی، اتوماسیون اداری (VBA) و نمونه سازی سریع در دات نت، VB.NET یک زبان سازنده و توانا باقی می ماند.