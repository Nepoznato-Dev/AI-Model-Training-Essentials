...
|----------|------|---------|-------|
| `+`| اضافه | `3 + 2`| همچنین الحاق`String`از طریق`format!`|
| `-`| تفریق | `3 - 2`| |
| `*`| ضرب | `3 * 2`| |
| `/`| بخش | `7 / 2`| تقسیم عدد صحیح برای اعداد صحیح:`3`|
| `%`| باقی مانده | `7 % 2`| `1`|
| `-x`| نفی | `-5`| |
### مقایسه و عملگرهای منطقی
| اپراتور | نام | مثال | یادداشت ها |
|----------|------|---------|-------|
| `==`| برابر | `x == y`| نیاز به`PartialEq`|
| `!=`| برابر نیست | `x != y`| |
| `<`,`>`,`<=`,`>=`| سفارش | `x >= y`| نیاز به`PartialOrd`|
...
| `\|`| یا | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `!`| نه | `!0b1010u8`| مکمل بیتی |
| `<<`| Shift چپ | `5 << 1`| `10`|
| `>>`| شیفت راست | `5 >> 1`| `2`|
### اولویت اپراتور (بیشترین به کمترین)
| تقدم | اپراتورها | یادداشت ها |
|------------|-----------|-------|
| 1 (بالاترین) |  مسیرهای`()``.` :: | |
| 2 | فراخوانی روش، دسترسی میدانی | Auto-deref |
| 3 | `-`(نفی)،`!`,`*`(deref) | |
| 4 | `as`(نوع ریخته گری) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
| 14 | `..``..=` | محدوده |
| 15 | `=``+=` و غیره | تکلیف |
| 16 (کمترین) | `return``break``continue`| |
---
...
## ساختارهای داده
### نمای کلی انواع داخلی
| نوع | پشته / پشته | اندازه | تغییرپذیری | یادداشت ها |
|------|-----------|------|-----------|-------|
| اولیه (`i32`,`f64`,`bool`,`char`) | پشته | ثابت | غیر قابل تغییر به طور پیش فرض | `Copy`|
| `[T; N]`(آرایه) | پشته | ثابت | قابل تغییر با`mut`| با اندازه ثابت، اگر T`Copy`باشد،`Copy`|
| `Vec<T>`| پشته | پویا | قابل تغییر با`mut`| قابل رشد |
| `String`| پشته | پویا | قابل تغییر با`mut`| UTF-8 قابل رشد |
| `&[T]`(برش) | پشته (ptr چربی) | نمای پویا | بستگی به`&`یا`&mut`دارد | نمای قرضی |
| `HashMap<K, V>`| پشته | پویا | قابل تغییر با`mut`| O(1) میانگین جستجو |
| `BTreeMap<K, V>`| پشته | پویا | قابل تغییر با`mut`| O(log n) جستجو، سفارش داده شده |
| `HashSet<T>`| پشته | پویا | — | O(1) میانگین، بدون تکرار |
### بردارها
```rust
// Creation
...
### ویژگی های استاندارد رایج کتابخانه
```rust
...
## خلاصه
نحو Rust بر پایه ضمانت‌های ایمنی که در زمان کامپایل اجرا می‌شوند ساخته شده است. سیستم مالکیت، چندشکلی مبتنی بر ویژگی، تطبیق الگو، و انتزاعات با هزینه صفر، یک طرح منسجم را تشکیل می‌دهند که در آن کامپایلر یک شریک سخت‌گیر اما منصف است. از جریان کنترل اولیه از طریق ویژگی‌های پیشرفته مانند ماکروها، کد ناامن و نشانگرهای هوشمند، هر ساختاری در خدمت برنامه‌نویسی سیستم‌های ایمن و کارآمد است. درک تأثیر متقابل بین طول عمر، قرض گرفتن و سیستم نوع، کلید تسلط بر Rust است.