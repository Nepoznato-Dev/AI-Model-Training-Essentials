...
## خلاصه
اصطلاحات Rust تاکید دارند: مالکیت و قرض گرفتن،`Result`/`Option`برای مدیریت خطا، فهرست کردن با روش‌ها، ویژگی‌ها برای انتزاع، زنجیره تکرار، اشاره‌گرهای هوشمند برای حالت مشترک، و انتزاع‌های بدون هزینه.`rustfmt`را برای قالب‌بندی،`clippy`را برای پرده‌بندی، و دستورالعمل‌های Rust API را دنبال کنید. جامعه Rust برای ایمنی، عملکرد و درستی ارزش قائل است - "اگر کامپایل شود، کار می کند."`&str`را به`String`در آرگومان های تابع ترجیح دهید، از`?`برای انتشار خطا استفاده کنید و ویژگی های مشترک را با`#[derive(Debug, Clone, ...)]`استخراج کنید.