...
## 2. خام`new`/`delete`به جای اشاره گرهای هوشمند
```cpp
...
## 6. عدم استفاده صحیح از `std::move`
```cpp
...
## 7. Anti-Pattern: استفاده از`using namespace std;`در هدرها
```cpp
...
## 8. رفتار تعریف نشده از سرریز امضا شده
```cpp
...
## 10. Anti-Pattern: مدیریت منابع دستی (Pre-RAII)
```cpp
...
## 11. قالب Bloat و Long Compile Times
```cpp
...
## 12. عدم استفاده از`[[nodiscard]]`و `[[maybe_unused]]`
```cpp
...
## خلاصه
پیچیدگی C++ فرصت‌های زیادی برای اشتباهات ایجاد می‌کند: قانون پنج، برش شی، باطل کردن تکرارکننده، ارجاعات آویزان، و رفتار نامشخص از سرریز امضا شده. C++ مدرن پادزهر است: از نشانگرهای هوشمند به جای خام`new`/ `delete`، RAII برای همه منابع،`std::vector`و`std::string`به جای آرایه‌های خام استفاده کنید، معناشناسی را برای کارایی حرکت دهید، و`delete`را به مقدار بازگشتی یا catchign. دستورالعمل‌های C++ Core به دلایلی وجود دارند - آنها را دنبال کنید و از ابزارهایی مانند Clang-Tidy و ضدعفونی‌کننده‌ها برای پیدا کردن چیزهایی که از دست می‌دهید استفاده کنید.