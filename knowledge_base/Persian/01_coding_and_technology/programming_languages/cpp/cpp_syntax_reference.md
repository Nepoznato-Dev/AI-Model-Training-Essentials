...
## اپراتورها و عبارات
### عملگرهای محاسباتی و مقایسه
| اپراتور | نام | مثال | یادداشت ها |
|----------|------|---------|-------|
| `+``-``*``/``%`| حسابی | `a + b`| همان C |
| `++``--` | افزایش/کاهش | `++i`,`i++`| ترجیح پیش افزایش |
| `==``!=``<``>``<=``>=` | مقایسه | `a <=> b`| `<=>`مقایسه سه طرفه است (C++20) |
...
| اپراتور | نام | مثال | یادداشت ها |
|----------|------|---------|-------|
| `::`| وضوح محدوده | `std::cout`| |
| `.*``->*` | اشاره گر به عضو | `obj.*pmf`| |
| `<=>`| سفینه فضایی (C++20) | `a <=> b`|`strong_ordering`|
| `co_await`| تعلیق کوروتین | `co_await expr`| C++20 |
| `co_yield`| بازده کوروتین | `co_yield value`| C++20 |
| `co_return`| بازگشت کوروتین | `co_return value`| C++20 |
...
## جریان را کنترل کنید
### اتصالات ساختاری و الگوهای مدرن
```cpp
...
## ظروف و الگوریتم های STL
### نمای کلی کانتینر
| کانتینر | سربرگ | سفارش داد | جستجو | یادداشت ها |
|-----------|--------|---------|--------|-------|
| `vector`| `<vector>`| بله | O(n) | انتخاب پیش فرض برای دنباله ها |
| `deque`| `<deque>`| بله | O(n) | فشار / پاپ سریع در هر دو انتها |
| `list`| `<list>`| بله | O(n) | لیست پیوندی دوگانه |
| `array`| `<array>`| بله | O(n) | اندازه ثابت، تخصیص پشته |
| `set`/`multiset`| `<set>`| بله | O(log n) | درخت سرخ سیاه |
| `map`/`multimap`| `<map>`| بله (با کلید) | O(log n) | کلید-مقدار درخت قرمز-سیاه |
| `unordered_set`| `<unordered_set>`| نه | O(1) میانگین | جدول هش |
| `unordered_map`| `<unordered_map>`| نه | O(1) میانگین | جدول هش |
### الگوریتم های کلیدی
```cpp
#include <algorithm>
#include <numeric>
...
## خلاصه
سینتکس C++ از دستکاری نشانگر سطح پایین تا فرابرنامه‌نویسی قالب سطح بالا و همگام‌سازی مبتنی بر کوروتین را در بر می‌گیرد. تکامل زبان از طریق C++17/20/23 به طور چشمگیری ایمنی (اشاره‌گرهای هوشمند، `std::optional`، مفاهیم)، بیان (پیوندهای ساختاریافته، محدوده‌ها، `std::format`) و عملکرد (مفاهیمی که انتزاع‌های بدون هزینه را امکان‌پذیر می‌سازد، بدون حاشیه‌ها را به‌طور چشمگیری بهبود بخشیده است). فلسفه اصلی بدون تغییر باقی می‌ماند: شما نباید برای چیزی که استفاده نمی‌کنید هزینه بپردازید، و زبان باید از هر سطحی از انتزاع از فلز خالی گرفته تا زبان‌های خاص دامنه پشتیبانی کند.