...
-->
# جاوا - اشتباهات رایج و ضد الگوها
این سند رایج ترین اشتباهات، تله ها و ضد الگوها را در جاوا فهرست می کند. هر ورودی رویکرد نادرست را نشان می دهد، توضیح می دهد که چرا شکست می خورد، و راه حل صحیح را ارائه می دهد.
---

## 1.`equals()`در مقابل`==`برای اشیاء
```java
...
## 2. اصلاح یک مجموعه در طول تکرار
```java
...
## 3. عدم بستن منابع (با منابع امتحان کنید)
```java
...
## 7. عدم مدیریت InterruptedException
```java
...
## 8. بازگشت تهی به جای مجموعه های خالی
```java
...
## 9. اشیاء قابل تغییر به عنوان کلید HashMap
```java
...
## 10. ضد الگو: گرفتن`Exception`یا `Throwable`
```java
...
## 11. عدم لغو`hashCode`هنگام لغو `equals`
```java
...
## 12. شرایط مسابقه با قفل دوبار بررسی شده
```java
...
## خلاصه
پرحرفی جاوا می‌تواند اشکالات ظریفی را پنهان کند:`==`در مقابل `equals()`، اصلاح همزمان، نشت منابع، تله‌های عملکرد خودکار و قرارداد معادل/هش کد. روش مدرن جاوا این است: از`Objects.equals()`برای مقایسه ایمن تهی استفاده کنید، همیشه از try-with-sources استفاده کنید، مجموعه های خالی را به جای null برگردانید، استثناهای خاص را بگیرید، و از اصل مسئولیت واحد پیروی کنید. ویژگی‌های جاوا 8+ (جریان‌ها، اختیاری، مراجع روش‌ها) در عین حال که کد را ایمن‌تر و گویاتر می‌کند، صفحه بخاری را کاهش می‌دهد.