...
## 3. عدم استفاده از اعلان‌های نوع
```php
...
## 4. متغیرهای متغیر و دسترسی پویا
```php
...
## 8. عدم استفاده از Composer Autoloading
```php
...
## 10. ضد الگو: منطق تجاری در نماها
```php
...
## خلاصه
انعطاف پذیری PHP هم نقطه قوت و هم ضعف آن است. قوانین اصلی: همیشه از مقایسه دقیق استفاده کنید (`===`)، از عبارات آماده برای SQL استفاده کنید،`declare(strict_types=1)`را فعال کنید، از Composer برای بارگیری خودکار استفاده کنید، شناسه های جلسه را پس از احراز هویت بازسازی کنید، و منطق کسب و کار را از نمایش ها دور نگه دارید. پی‌اچ‌پی مدرن (8.x) با انواع سخت‌گیرانه، انواع اتحاد، عبارات مطابقت، و آرگومان‌های نام‌گذاری شده بسیار متفاوت از PHP 5 است - ویژگی‌های مدرن را در بر می‌گیرد.