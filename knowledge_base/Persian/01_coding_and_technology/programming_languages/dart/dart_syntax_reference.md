...
|----------|------|---------|-------|
| `+``-``*``/``%``~/` | حسابی | `7 ~/ 3`| `~/`یک تقسیم عدد صحیح |
| `==``!=` | برابری | `a == b`| |
| `<``>``<=``>=` | مقایسه | `a >= b`| |
| `&&``\|\|``!`| منطقی | `a && b`| اتصال کوتاه |
| `??`| ادغام تهی | `a ?? b`| `b`اگر`a`تهی باشد |
| `?.`| دسترسی بدون آگاهی | `a?.b`| اگر`a`تهی باشد |
| `!`| ادعای پوچ | `a!`| پرتاب می کند در صورت تهی |
| `..``?..` | آبشار | `obj..a..b`| عملیات زنجیره ای روی همان شی |
| `??=`| تخصیص بی اطلاع | `a ??= b`| اختصاص اگر null |
| `as`| نوع بازیگران | `obj as String`| پرتاب در شکست |
| `is``is!` | نوع تست | `obj is String`| |
| `=>`| تابع پیکان | `() => expr`| |
### نماد آبشاری
```dart
...
## خلاصه
نحو دارت تمیز، سازگار و برای بهره وری طراحی شده است. ایمنی پوچ صدا خطاهای مرجع تهی را در زمان کامپایل حذف می کند. Async/wait و استریم ها الگوهای طبیعی را برای برنامه نویسی ناهمزمان ارائه می دهند. کلاس‌ها، میکسین‌ها و کلاس‌های مهر و موم شده، ترکیب شیء انعطاف‌پذیر را امکان‌پذیر می‌کنند. تطبیق الگوی دارت 3، رکوردها و اصلاح کننده‌های کلاس، بیانگر بودن را اضافه می‌کنند. در حالی که هدف اصلی Dart Flutter است، نحو آن برای هر دامنه برنامه ای مناسب است - سمت سرور (با Dart Frog یا Shelf)، ابزارهای CLI، و برنامه های کاربردی وب (کامپایل شده در جاوا اسکریپت).