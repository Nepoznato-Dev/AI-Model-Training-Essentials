...
-->
# دارت - اشتباهات رایج و ضد الگوها
این سند رایج ترین اشتباهات، تله ها و ضد الگوها در دارت را با اصلاحات فهرست می کند.
---

## 1. استفاده نادرست از ایمنی پوچ
```dart
// ❌ WRONG — force unwrap
...
## 3. عدم استفاده از سازندگان `const`
```dart
...
## 5. عدم استفاده از Spread و Collection-if
```dart
...
## خلاصه
ایمنی پوچ Dart قدرتمند است — آن را با`!`شکست ندهید. از سازنده های`const`برای نمونه های متعارف استفاده کنید، همیشه`hashCode`را با`==`لغو کنید، از عملگرهای collection-if و spread برای ایجاد فهرست مختصر استفاده کنید، و منطق تجاری را از ویجت ها استخراج کنید. دارت کدهای تمیز، بی خطر، و کدهای صحیح را به ارمغان می آورد.