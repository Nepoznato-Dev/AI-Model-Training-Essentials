...
---
# مقایسه بین زبانی - مدیریت خطا
## بررسی اجمالی مدل های مدیریت خطا
| مدل | زبان ها | مکانیسم |
|-------|-----------|-----------|
| **استثناها (تلاش/گرفتن)** | Java، C++، C#، Python، Ruby، PHP، JavaScript، TypeScript، Kotlin، Swift، Dart، Scala، VB، دلفی | `try { } catch (e) { }`|
| **انواع نتایج/گزینه** | Rust، Haskell، OCaml، Scala (همچنین) | `Result<T, E>`,`Maybe a`|
| **مقادیر بازگشتی چندگانه** | برو، لوا | `value, err := f()`|
| **کدهای خطا** | سی، فرترن | کد خطا را برگردانید، به صورت دستی بررسی کنید |
| **خطاهای تطبیق الگو** | اکسیر، ارلنگ | `{:ok, value}`/`{:error, reason}`|
| **شرایط (قابل راه اندازی مجدد)** | لب معمولی | `restart-case`,`handler-bind`|
| **بدون رسیدگی به خطا** | Scratch, SQL (پایه) | N/A |
| **قراردادها** | آدا، ایفل |  شرایط`Pre`,`Post`|
## مدیریت خطا مبتنی بر استثنا
```python
# Python
try:
    result = 10 / 0
...
## خلاصه: پارادایم های مدیریت خطا
| پارادایم | رویکرد | جوانب مثبت | معایب |
|----------|----------|------|------|
| **استثناء** | پرتاب/گرفتن | مسیر شاد پاک، ردپای پشته | می توان فراموش کرد، کنترل سوء استفاده از جریان |
| **انواع نتایج** | بازگشت`Ok`/`Err`| صریح، نمی توان خطاها را نادیده گرفت | اپراتور Verbose،`?`مورد نیاز است |
| **کدهای خطا** | بازگشت وضعیت int | ساده، سازگار با C | نادیده گرفتن آسان، بدون زمینه |
| **بازگشت های متعدد** | `value, err := f()`| صریح، اصطلاحی | می توانید فراموش کنید که`err`را بررسی کنید |
| **تطابق الگو** | `{:ok, v}`/`{:error, r}`| جامع، کاربردی | پرمخاطب بدون`with`|
| **شرایط** | کنترل کننده های قابل راه اندازی مجدد | قدرتمندترین، قابل بازیابی | پیچیده، به ندرت قابل درک |