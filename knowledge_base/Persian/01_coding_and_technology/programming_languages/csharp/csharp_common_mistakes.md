...
---
# سی شارپ - اشتباهات رایج و ضد الگوها
این سند رایج ترین اشتباهات، تله ها و ضد الگوها را در سی شارپ فهرست می کند. هر ورودی رویکرد نادرست را نشان می دهد، توضیح می دهد که چرا شکست می خورد، و راه حل صحیح را ارائه می دهد.
---

## 1. عدم دفع اشیاء `IDisposable`
```csharp
// ❌ WRONG — resource leak
...
## 2.`==`در مقابل`Equals()`برای رشته ها
```csharp
...
## 5. گرفتن متغیر حلقه (قبل از C# 5.0)
```csharp
...
## 7. ضد الگو: کلاس های خدا / مکان یاب خدمات
```csharp
...
## 10. عدم استفاده از رکورد برای انواع داده ها
```csharp
...
## 11. سوء استفاده از فیلتر استثنا
```csharp
...
## خلاصه
سی شارپ یک زبان خوب طراحی شده است اما دارای تله هایی است: دفع نکردن اشیاء `IDisposable`، روش های خالی غیر همگام، تخصیص های غیر ضروری LINQ و استثناهای مرجع تهی. سی شارپ مدرن (8+) ابزارهای قدرتمندی را ارائه می دهد: انواع مرجع تهی، تطبیق الگو، رکوردها، استفاده از اعلان ها و عبارات سوئیچ. اصول کلیدی: همیشه منابع را دور بریزید، هرگز از خالی غیر همگام استفاده نکنید (به جز کنترل کننده‌های رویداد)، از تزریق سازنده استفاده کنید، رکوردها را برای داده‌ها ترجیح دهید، و انواع مرجع ته‌پذیر را فعال کنید تا باگ‌های تهی را در زمان کامپایل پیدا کنند.