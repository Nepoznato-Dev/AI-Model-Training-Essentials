...
### حافظه GPU (VRAM)
بحرانی ترین منبعاندازه مدل در حافظه ≈ **پارامترها × بایت در هر پارامتر**.
...
- **از کتابخانه های آزمایش شده استفاده کنید**: برای اعتبارسنجی ایمیل، URL و تاریخ، از کتابخانه های استاندارد استفاده کنید (به عنوان مثال `email-validator` در پایتون، `validator.js` در Node).
...
- **شل فرار **: از ساخت دستورات پوسته از ورودی کاربر خودداری کنید.اگر اجتناب ناپذیر است، از `shlex.quote()` یا موارد مشابه استفاده کنید.
...
- فقط مبداهای خاص را مجاز کنید (هرگز `*` در تولید).
- هدر `Origin` را در سمت سرور تأیید کنید.

### اعتبار سنجی ورودی
- اعتبار تمام پارامترهای درخواست، از جمله هدر و بدنه.
- فیلدهای غیرمنتظره را رد کنید (`"strict": true` یا `additionalProperties: false` در طرحواره JSON).
...
- Harden SSH: غیرفعال کردن ورود به ریشه، استفاده از احراز هویت مبتنی بر کلید، تغییر پورت پیش فرض (اختیاری).
...
- گزارش‌ها را در یک سیستم متمرکز و غیرقابل دستکاری (مانند ELK Stack، Splunk، Datadog) ذخیره کنید.
