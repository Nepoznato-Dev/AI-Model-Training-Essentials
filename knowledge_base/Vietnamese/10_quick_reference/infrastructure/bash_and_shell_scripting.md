...
| Cờ | Hiệu ứng |
|------|--------|
| `set -e`| Thoát ngay nếu lệnh bị lỗi |
| `set -u`| Hãy coi các biến không được đặt là lỗi |
| `set -o pipefail`| Đường ống bị lỗi nếu bất kỳ lệnh nào trong đó bị lỗi |
| `set -x`| In từng lệnh trước khi thực hiện (chế độ gỡ lỗi) |
...
| `$@`| Tất cả các đối số vị trí (dưới dạng các từ riêng biệt) |
| `$*`| Tất cả các đối số vị trí (dưới dạng một chuỗi) |
| `$?`| Trạng thái thoát của lệnh cuối cùng (0 = thành công) |
| `$$`| PID của quy trình hiện tại |
| `$!`| PID của quá trình nền cuối cùng |
| `$_`| Đối số cuối cùng của lệnh trước |
---
...
| Kiểm tra | Ý nghĩa |
|------|----------|
| `[[ -f "$x" ]]`| Tệp tồn tại và là tệp thông thường |
| `[[ -d "$x" ]]`| Thư mục tồn tại |
| `[[ -e "$x" ]]`| Tệp/thư mục tồn tại (bất cứ thứ gì) |
| `[[ -r "$x" ]]`| Có thể đọc được |
| `[[ -w "$x" ]]`| Có thể ghi |
| `[[ -x "$x" ]]`| Có thể thực thi |
| `[[ -z "$x" ]]`| Chuỗi trống |
| `[[ -n "$x" ]]`| Chuỗi không trống |
| `[[ "$a" == "$b" ]]`| Chuỗi đẳng thức |
| `[[ "$a" != "$b" ]]`| Bất đẳng thức chuỗi |
| `[[ "$a" =~ regex ]]`| Trận đấu Regex |
| `[[ $a -eq $b ]]`| Bình đẳng số nguyên |
| `[[ $a -ne $b ]]`| Bất đẳng thức số nguyên |
| `[[ $a -gt $b ]]`| Lớn hơn |
| `[[ $a -lt $b ]]`| Ít hơn |
...
| Cú pháp | Ý nghĩa |
|--------|----------|
| `cmd > file`| Chuyển hướng thiết bị xuất chuẩn sang tệp (ghi đè) |
| `cmd >> file`| Chuyển hướng thiết bị xuất chuẩn sang tệp (nối thêm) |
| `cmd 2> errors.log`| Chuyển hướng stderr |
| `cmd &> all.log`| Chuyển hướng cả stdout và stderr |
| `cmd1 \| cmd2`| Thiết bị xuất chuẩn ống của cmd1 đến stdin của cmd2 |
| `cmd1 \|& cmd2`| Ống cả stdout và stderr |
| `cmd < file`| Chuyển hướng tập tin tới stdin |
| `cmd <<EOF ... EOF`| Tài liệu ở đây (đầu vào nhiều dòng) |
| `cmd <<< "string"`| Chuỗi đây (đầu vào một dòng) |
```bash
# Here-document
cat <<EOF
Name: $NAME
...
## Bản tóm tắt
Tập lệnh Bash tăng năng suất cho bất kỳ ai làm việc với máy tính. Không cần thiết phải ghi nhớ mọi lá cờ - biết điều gì có thể làm được và tìm tài liệu ở đâu là đủ. Bắt đầu với những điều cơ bản: biến, điều kiện, vòng lặp, đường ống. Sau đó thêm các công cụ xử lý văn bản (grep, sed, awk) nếu cần. Nên sử dụng`set -euo pipefail`để viết các tập lệnh mạnh mẽ.