...
-`DECIMAL(p,s)`- Số thập phân chính xác (độ chính xác, tỷ lệ)
-`FLOAT`- Dấu phẩy động gần đúng
-`DOUBLE`- Phao chính xác kép
### Chuỗi
-`CHAR(n)`- Chuỗi có độ dài cố định
-`VARCHAR(n)`- Chuỗi có độ dài thay đổi
-`TEXT`- Văn bản lớn
-`ENUM`- Các giá trị được liệt kê
### Ngày/Giờ
...
-`YEAR`- Giá trị năm
### Boolean
-`BOOLEAN`hoặc`BOOL`- Đúng/Sai
### Nhị phân
-`BLOB`- Đối tượng lớn nhị phân
-`BINARY`- Cố định nhị phân
-`VARBINARY`- Biến nhị phân
---
...
## Mẹo về hiệu suất
✅ **Làm:**
- Sử dụng chỉ mục trên các cột được truy vấn thường xuyên
- Chỉ chọn các cột cần thiết (tránh`SELECT *`)
- Sử dụng`EXPLAIN`để phân tích hiệu suất truy vấn
- Chuẩn hóa dữ liệu một cách thích hợp
- Sử dụng các câu lệnh đã chuẩn bị sẵn để ngăn chặn việc tiêm SQL
❌ **Không:**
- Sử dụng các hàm trên các cột được lập chỉ mục trong mệnh đề WHERE
- Tạo quá nhiều chỉ mục (ghi chậm)
- Sử dụng`SELECT DISTINCT`không cần thiết
- Bỏ qua kế hoạch thực hiện truy vấn
- Lưu trữ các giá trị được tính toán khi chúng có thể được tính toán
---
...
*Cập nhật lần cuối: Tháng 7 năm 2026 | Tiêu chuẩn SQL (tương thích với MySQL/PostgreSQL)*