...
- `alias ll='ls -la'` tạo một lối tắt — thêm lối tắt đó vào `~/.bashrc` hoặc `~/.zshrc`.
- Sử dụng `tmux` hoặc `screen` để duy trì các phiên khi bị ngắt kết nối khỏi máy chủ từ xa.
...
- **Container**: phiên bản đang chạy của một hình ảnh.
