...
---
# TypeScript — Các lỗi thường gặp và mẫu phản đối
Tài liệu này liệt kê các lỗi, bẫy và anti-pattern phổ biến nhất trong TypeScript. Mỗi mục hiển thị cách tiếp cận không chính xác, giải thích lý do tại sao nó thất bại và cung cấp giải pháp chính xác.
---
...
## 2. Gõ Assertions thay vì Type Guards
```typescript
...
## 3. Lạm dụng xác nhận không có giá trị
```typescript
...
## 4. Nhầm lẫn giữa giao diện và loại
```typescript
...
## 7. Những bất ngờ về cấu trúc khi gõ
```typescript
...
## 8. Lỗi loại không đồng bộ/chờ đợi
```typescript
...
## 9. Anti-Pattern: Kiểu lặp lại (Không dùng Generics)
```typescript
...
## 12. Không tận dụng các công đoàn bị phân biệt đối xử
```typescript
...
## Bản tóm tắt
Hệ thống kiểu chữ của TypeScript rất mạnh mẽ nhưng đòi hỏi tính kỷ luật. Tội lỗi chính đang chạm tới`any`- sử dụng`unknown`khi loại thực sự không xác định và thu hẹp với các loại bảo vệ. Tránh các xác nhận không null (`!`) bằng cách xử lý không xác định đúng cách. Sử dụng giao diện cho các hình dạng đối tượng, các loại cho các hiệp hội và các loại được tính toán. Các công đoàn bị phân biệt đối xử sẽ loại bỏ toàn bộ các loại lỗi. Kích hoạt`strict: true`ngay từ ngày đầu tiên — việc trang bị thêm tính nghiêm ngặt là điều khó khăn. Mục tiêu là làm cho các trạng thái bất hợp pháp không thể đại diện được ở cấp độ loại.