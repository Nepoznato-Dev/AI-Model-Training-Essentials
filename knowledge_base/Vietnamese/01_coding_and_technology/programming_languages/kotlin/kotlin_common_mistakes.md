...
# Kotlin — Những lỗi thường gặp và các mẫu phản đối
Tài liệu này liệt kê các lỗi, bẫy và mô hình phản đối phổ biến nhất trong Kotlin. Mỗi mục hiển thị cách tiếp cận không chính xác, giải thích lý do tại sao nó thất bại và cung cấp giải pháp chính xác.
---

## 1. Các loại nền tảng và NullPointerException
```kotlin
...
## 3.`data class`không có`equals`/`hashCode`phù hợp
```kotlin
...
## 4. Sử dụng không đúng các hàm phạm vi
```kotlin
...
## 5. Coroutines: Không xử lý việc hủy bỏ
```kotlin
...
## 6. Anti-Pattern: Kotlin kiểu Java
```kotlin
...
## 9. Không sử dụng`by lazy`để khởi tạo tốn kém
```kotlin
...
## 10. Sự nhầm lẫn giữa luồng và LiveData
```kotlin
...
## Bản tóm tắt
Tính đồng nhất của Kotlin có thể che giấu các vấn đề nhỏ: các loại nền tảng từ khả năng tương tác Java, sự cố `lateinit`, các lớp dữ liệu có thể thay đổi làm hỏng bộ sưu tập và hủy coroutine. Cách của Kotlin là: tận dụng sự an toàn null (không bao giờ sử dụng`!!`), sử dụng hàm phạm vi phù hợp (`let`cho các phép biến đổi,`apply`cho cấu hình,`also`cho các tác dụng phụ), sử dụng các lớp kín với`when`đầy đủ, xử lý việc hủy coroutine và viết Kotlin đặc trưng (không phải Java với cú pháp Kotlin). Trình biên dịch là hướng dẫn của bạn - nếu nó cảnh báo, hãy lắng nghe.