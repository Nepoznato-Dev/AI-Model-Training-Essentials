...
---
# Lua — Những lỗi thường gặp và các mẫu phản đối
Tài liệu này liệt kê các lỗi, bẫy và mô hình phản đối phổ biến nhất trong Lua kèm theo các chỉnh sửa.
---
...
## 2.`nil`vs`false`trong điều kiện
```lua
...
## 7. Không sử dụng Metatables cho OOP
```lua
...
## Bản tóm tắt
Sự tối giản của Lua tạo ra các bẫy: lập chỉ mục dựa trên 1, chỉ`nil`và`false`là giả, các bảng là tham chiếu (không phải giá trị), các biến là toàn cục theo mặc định và`pairs`không có thứ tự trong khi`ipairs`có thứ tự. Cách Lua là: luôn sử dụng`local`, sử dụng`ipairs`cho mảng và`pairs`cho từ điển, sử dụng`table.concat`để xây dựng chuỗi và sử dụng siêu dữ liệu cho OOP. Tính đơn giản của Lua chính là sức mạnh của nó — nhưng hãy tôn trọng các trường hợp khó khăn.