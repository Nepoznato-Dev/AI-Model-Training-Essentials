...
---
# Lua — Tham khảo cú pháp
Tài liệu này cung cấp tham chiếu cú ​​pháp có cấu trúc, toàn diện cho Lua (5.4). Nó bổ sung cho tham chiếu Lua chính bằng cách tập trung vào các mẫu cú pháp, bảng và siêu dữ liệu đầy đủ, coroutine cũng như triết lý thiết kế độc đáo của Lua.
---
...
|----------|------|----------|-------|
| `+``-``*``/``%``^` | Số học | `2 ^ 10`| `^`là lũy thừa |
| `//`| Phân tầng | `7 // 2`| Trả về 3 (Lua 5.3+) |
| `==``~=` | Bình đẳng | `a == b`| `~=`"không bằng" |
| `<``>``<=``>=` | So sánh | `a >= b`| |
| `and``or``not`| Hợp lý | `a and b`| Trả về toán hạng, không phải boolean |
| `..`| Nối | `"hello" .. " world"`| |
| `#`| Chiều dài | `#t`| Độ dài của chuỗi hoặc bảng |
| `&``\|``~``<<``>>`| Theo chiều bit | `a & b`| Lua 5.3+ (hoạt động số nguyên) |
### Toán tử logic — Trả về không phải Boolean
```lua
...
## Bảng - Cấu trúc dữ liệu phổ quát
```lua
...
## Bản tóm tắt
Cú pháp của Lua được cố tình tối giản — toàn bộ ngôn ngữ có thể được mô tả trong một trang duy nhất. Các bảng đóng vai trò là cấu trúc dữ liệu phổ quát (mảng, từ điển, đối tượng, mô-đun). Metatables cung cấp khả năng mở rộng mà không phức tạp về ngôn ngữ. Coroutine cho phép thực hiện đa nhiệm hợp tác và các mẫu vòng lặp tinh tế. Sức mạnh của ngôn ngữ này đến từ khả năng nhúng của nó: Lua được thiết kế để hoạt động bên trong các ứng dụng máy chủ, cung cấp khả năng tạo tập lệnh với chi phí tối thiểu. Đối với việc phát triển, cấu hình và hệ thống nhúng trò chơi, tính đơn giản của Lua là thế mạnh lớn nhất của nó — một ngôn ngữ nhỏ thực hiện chính xác những gì nó cần và không có gì hơn thế.