...
| `.`| Nối chuỗi | `$a . $b`| |
| `x`| Lặp lại chuỗi | `"ab" x 3`| `"ababab"`|
| `==``!=``<``>``<=``>=` | So sánh số | `$a == $b`| |
| `eq``ne``lt``gt``le``ge` | So sánh chuỗi | `$a eq $b`| |
| `<=>`| Tàu vũ trụ số | `$a <=> $b`| Trả về -1, 0, 1 |
| `cmp`| Tàu vũ trụ dây | `$a cmp $b`| |
| `&&``\|\|``!`| Hợp lý | `$a && $b`| |
| `and``or``not`| Hợp lý (độ chính xác thấp) | `$a and $b`| Tránh - sử dụng`&&`/`\|\|`|
| `=~``!~` | Trận đấu Regex | `$str =~ /pattern/`| |
| `..``...` | Phạm vi / flip-flop | `1..10`| |
| `?:`| Đệ tam | `$a ? $b : $c`| |
| `//`| Xác định-hoặc | `$a // $b`| `$b`nếu`$a`không thành công |
| `=>`| Dấu phẩy béo | `key => 'value'`| Tự động trích dẫn từ trần |
### Sigils — Các loại biến
```perl
...
## Perl hướng đối tượng (Moo/Moose)
```perl
# Modern OOP with Moo
package User;
use Moo;
...
## Bản tóm tắt
Cú pháp của Perl được xây dựng dựa trên ngữ cảnh, ký hiệu và nguyên tắc "Có nhiều cách để làm điều đó". Các biểu thức chính quy được tích hợp vào ngôn ngữ, giúp việc xử lý văn bản trở nên ngắn gọn chưa từng có. Tài liệu tham khảo cho phép cấu trúc dữ liệu phức tạp. CPAN cung cấp các mô-đun cho mọi thứ. Perl hiện đại (với`strict`,`warnings`và các biến từ vựng) sạch sẽ và có thể bảo trì. Di sản của Perl vẫn tồn tại trong các khuôn mẫu và công cụ mà mọi ngôn ngữ hiện đại đều áp dụng.