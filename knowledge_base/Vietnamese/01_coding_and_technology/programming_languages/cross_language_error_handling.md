...
|-------|--------------|----------|
| **Ngoại lệ (thử/bắt)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **Loại kết quả/tùy chọn** | Rust, Haskell, OCaml, Scala (cũng) | `Result<T, E>`,`Maybe a`|
| **Nhiều giá trị trả về** | Đi đi Lua | `value, err := f()`|
| **Mã lỗi** | C, Fortran | Trả về mã lỗi, kiểm tra thủ công |
| **Lỗi khớp mẫu** | Thuốc tiên, Erlang | `{:ok, value}`/`{:error, reason}`|
| **Điều kiện (có thể khởi động lại)** | Lisp thông thường | `restart-case`,`handler-bind`|
| **Không xử lý lỗi** | Scratch, SQL (cơ bản) | Không áp dụng |
| **Hợp đồng** | Ada, Eiffel |  Điều kiện`Pre`,`Post`|
...
## Tóm tắt: Mô hình xử lý lỗi
| Mô hình | Tiếp cận | Ưu điểm | Nhược điểm |
|----------|----------|------|------|
| **Ngoại lệ** | ném/bắt | Con đường hạnh phúc sạch sẽ, dấu vết ngăn xếp | Có thể bị lãng quên, kiểm soát lạm dụng dòng chảy |
| **Các loại kết quả** | Trở lại`Ok`/`Err`| Rõ ràng, không thể bỏ qua lỗi | Cần có toán tử dài dòng,`?`|
| **Mã lỗi** | Trả về trạng thái int | Đơn giản, tương thích với C | Dễ bỏ qua, không có ngữ cảnh |
| **Trả lại nhiều lần** | `value, err := f()`| Rõ ràng, thành ngữ | Có thể quên kiểm tra`err`|
| **Khớp mẫu** | `{:ok, v}`/`{:error, r}`| Đầy đủ, đầy đủ chức năng | Dài dòng không có`with`|
| **Điều kiện** | Trình xử lý có thể khởi động lại | Mạnh mẽ nhất, có thể phục hồi | Phức tạp, hiếm khi được hiểu |