...
-->
# OCaml — Những lỗi thường gặp và các mẫu chống đối
Tài liệu này liệt kê các lỗi, bẫy và mô hình chống phổ biến nhất trong OCaml cùng với các bản sửa lỗi.
---

## 1. Khớp mẫu không đầy đủ
```ocaml
(* ❌ WRONG — missing cases *)
...
## 4. Sử dụng`Option`không đúng cách
```ocaml
...
## 5. Trạng thái có thể thay đổi trong hàm đệ quy
```ocaml
...
## Bản tóm tắt
Bẫy OCaml: khớp mẫu không đầy đủ (cảnh báo trình biên dịch),`=`là sự bình đẳng về cấu trúc trong khi`==`là vật lý, cắt bớt phép chia số nguyên, sử dụng`Option`thay vì ngoại lệ và tránh trạng thái có thể thay đổi theo hướng đệ quy đuôi và gấp. Trình biên dịch của OCaml là đồng minh của bạn — hãy chú ý đến những cảnh báo của nó.