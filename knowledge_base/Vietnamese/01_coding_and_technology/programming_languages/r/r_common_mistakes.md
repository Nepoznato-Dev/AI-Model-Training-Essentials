...
## 1. Phát triển khung dữ liệu theo vòng lặp
```r
# ❌ WRONG — O(n²) growth
result <- data.frame()
for (i in 1:1000) {
...
## 3. Không Hiểu Về Tái Chế Vector
```r
...
## 5. Không sử dụng `stringsAsFactors = FALSE`
```r
...
## Bản tóm tắt
Bản chất tương tác của R tạo ra các bẫy: phát triển các khung dữ liệu theo vòng lặp (phân bổ trước hoặc vector hóa), tái chế vectơ bất ngờ, lan truyền`NA`và`T`/`F`có thể ghi đè. Cách R là: vector hóa các hoạt động, phân bổ trước khi lặp, xử lý`NA`một cách rõ ràng, sử dụng gọn gàng để thao tác dữ liệu và tránh`<<-`. R thưởng cho những người nghĩ về vectơ và sử dụng hệ sinh thái gói phong phú của nó.