...
-->
# R — Tham chiếu cú ​​pháp
Tài liệu này cung cấp tham chiếu cú ​​pháp có cấu trúc, toàn diện cho R (4.x). Nó bổ sung cho tham chiếu R chính bằng cách tập trung vào các mẫu cú pháp đầy đủ, hệ sinh thái gọn gàng, thao tác dữ liệu, mô hình thống kê và trực quan hóa.
---
...
| `+``-``*``/``^`| Số học | `2^10`| |
| `%%`| Mô đun | `7 %% 3`| Trả về 1 |
| `%/%`| Phép chia số nguyên | `7 %/% 3`| Trả về 2 |
| `==``!=` | Bình đẳng | `x == y`| Vector hóa |
| `<``>``<=``>=` | So sánh | `x >= y`| Vector hóa |
| `&``\|``!`| Hợp lý (được vector hóa) | `x & y`| Yếu tố khôn ngoan |
| `&&``\|\|` | Logic (vô hướng) | `x && y`| Chỉ phần tử đầu tiên |
| `%in%`| Thành viên | `x %in% y`| |
| `<-``=` | Bài tập | `x <- 10`| `<-`là thành ngữ |
| `->`| Giao quyền | `10 -> x`| Hiếm khi được sử dụng |
| `<<-`| Chuyển nhượng toàn cầu | `x <<- 10`| Chỉ định trong env gốc |
| `$`| Trích xuất phần tử | `df$column`| |
| `[``[[` | Lập chỉ mục | `x[1]`/`x[[1]]`| `[`trả về cùng loại;  Chiết xuất`[[`|
| `%>%`| Ống (magrittr) | `x %>% f()`| |
| `\|>`| Ống (đế R 4.1+) | `x \|> f()`| Ống bản địa |
...
## Bản tóm tắt
Cú pháp của R được thiết kế để tính toán thống kê và phân tích dữ liệu. Bản chất vector hóa của các hoạt động giúp loại bỏ vòng lặp cho hầu hết các nhiệm vụ. Hệ sinh thái gọn gàng (dplyr, ggplot2, gọn gàng) cung cấp một ngữ pháp nhất quán, dễ đọc để thao tác và trực quan hóa dữ liệu. Khả năng lập mô hình thống kê của R - từ các thử nghiệm t cơ bản đến các mô hình hiệu ứng hỗn hợp - có chiều sâu chưa từng có. Đối với khoa học dữ liệu, thống kê và nghiên cứu có thể tái tạo, R vẫn là một công cụ thiết yếu.