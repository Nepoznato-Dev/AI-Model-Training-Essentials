...
|------|--------|-------------|------|----|------|---|------|------|
| Số nguyên | `int`(tùy ý) | `number`| `i32`,`u64`| `int`| `int`,`long`| `int`| `int`| `int`|
| Phao | `float`| `number`| `f64`| `float64`| `double`| `double`| `double`| `double`|
| Chuỗi | `str`| `string`| `String`| `string`| `String`| `char*`| `string`| `string`|
| Boolean | `bool`| `boolean`| `bool`| `bool`| `boolean`| `bool`(C99) | `bool`| `bool`|
| Nhân vật | `str[0]`| `string[0]`| `char`| `rune`| `char`| `char`| `char`| `char`|
| Không | `None`| `null`| `Option<T>`| `nil`| `null`| `NULL`| `nullptr`| `null`|
## Mảng / Danh sách
```python
...
| rỉ sét | `let s: HashSet<i32> = [1, 2, 3].into()`|
| Đi | `s := map[int]bool{1: true, 2: true, 3: true}`(không có Bộ tích hợp) |
| Java | `Set<Integer> s = Set.of(1, 2, 3)`|
| C++ | `std::set<int> s = {1, 2, 3}`|
| C# | `var s = new HashSet<int> { 1, 2, 3 }`|
| Ruby | `s = Set[1, 2, 3]`|
| Nhanh | `var s: Set<Int> = [1, 2, 3]`|
| Kotlin | `val s = setOf(1, 2, 3)`|
| Scala | `val s = Set(1, 2, 3)`|
| Phi tiêu | `var s = {1, 2, 3}`|
| Julia | `s = Set([1, 2, 3])`|
| R | `s <- c(1, 2, 3)`(không có bộ gốc; sử dụng`%in%`) |
## Cấu trúc/Bản ghi/Lớp
```python
...
| Ngôn ngữ | Cú pháp |
|----------|--------|
| rỉ sét | `enum Color { Red, Green, Blue }`|
| Nhanh | `enum Color { case red, green, blue }`|
...
| C# | `enum Color { Red, Green, Blue }`|
| Đi | `const ( Red = iota; Green; Blue )`(không có enum gốc) |
| Phi tiêu | `enum Color { red, green, blue }`|
...
| Ruby | Không có enum gốc (sử dụng hằng số cố định hoặc`dry-enum`) |
## Không an toàn
| Ngôn ngữ | Xử lý Null |
|----------|--------------|
| Python |  `None`, không có sự an toàn về thời gian biên dịch |
| JavaScript | `null`/`undefined`, xâu chuỗi tùy chọn`?.`|
| rỉ sét | `Option<T>`— không có giá trị rỗng! Thực thi thời gian biên dịch |
| Đi | `nil`cho con trỏ, lát cắt, bản đồ, kênh |
| Java | `null`,`Optional<T>`(Java 8+),`@Nullable`|
| C | `NULL`(macro cho`((void*)0)`) |
| C++ | `nullptr`(C++11),`std::optional<T>`|
| C# |  `null`, các loại tham chiếu có thể vô hiệu (C# 8+),`Nullable<T>`|
| Nhanh | `Optional<T>`(`T?`),`if let`,`guard let`|
| Kotlin | `T?`(nullable), cuộc gọi an toàn `?.`,`?:`Elvis |
| TypeScript | `null`/`undefined`,`strictNullChecks`,`T \| null`|
| Haskell | `Maybe a`—`Just x`hoặc`Nothing`|
| Scala | `Option[T]`—`Some(x)`hoặc`None`|
| Phi tiêu | `T?`(nullable), an toàn null (Dart 2.12+) |
| Ruby | `nil`(mọi thứ đều là sự thật ngoại trừ`false`và`nil`) |
| Thuốc tiên | Không có giá trị rỗng - sử dụng`nil`hoặc khớp mẫu |
| Erlang | Không có giá trị rỗng - sử dụng`undefined`hoặc khớp mẫu |