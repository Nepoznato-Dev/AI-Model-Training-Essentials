...
## Toán tử & Biểu thức
| Danh mục | Nhà điều hành | Mô tả | Ví dụ |
|----------|----------|-------------|--------|
| **Số học** | `+`| Ngoài ra | `a + b`|
| | `-`| Phép trừ | `a - b`|
| | `*`| Phép nhân | `a * b`|
| | `/`| Phép chia (trả về Đôi) | `a / b`|
...
| **Hợp lý** | `AndAlso`| Ngắn mạch VÀ | `a AndAlso b`|
| | `OrElse`| Đoản mạch HOẶC | `a OrElse b`|
| | `Not`| KHÔNG logic | `Not a`|
| | `And`| Theo chiều bit VÀ | `a And b`|
| | `Or`| Theo bit HOẶC | `a Or b`|
| | `Xor`| XOR theo bit | `a Xor b`|
| **Chuỗi** | `&`| Nối | `"Hello" & " " & "World"`|
| | `+=`| Nối thêm | `s &= " more"`|
| **Bài tập** | `=`| Chỉ định | `x = 42`|
| **Không** | `Is`| Bình đẳng tham khảo | `obj Is Nothing`|
| | `IsNot`| Bất đẳng thức tham chiếu | `obj IsNot Nothing`|
| **Loại** | `TryCast`| Diễn viên an toàn (trả về Không có gì) | `TryCast(obj, String)`|
| | `CType`| Chuyển đổi | `CType(obj, Integer)`|
| | `DirectCast`| Ném trực tiếp (ném khi thất bại) | `DirectCast(obj, String)`|
---
...
## Bản tóm tắt
Cú pháp của Visual Basic rõ ràng và giống tiếng Anh, khiến nó trở thành một trong những ngôn ngữ lập trình dễ đọc nhất. VB.NET cung cấp quyền truy cập đầy đủ vào hệ sinh thái .NET bao gồm LINQ để truy vấn dữ liệu, async/await cho các hoạt động đồng thời, các khái niệm chung về an toàn kiểu và OOP toàn diện với các lớp, giao diện và tính kế thừa. Các từ khóa dài dòng và không phân biệt chữ hoa chữ thường của ngôn ngữ (End If, End Sub, End Class) giúp nâng cao khả năng đọc. Đối với các ứng dụng Windows dành cho doanh nghiệp, tự động hóa Office (VBA) và tạo mẫu nhanh trong .NET, VB.NET vẫn là một ngôn ngữ hiệu quả và có khả năng.