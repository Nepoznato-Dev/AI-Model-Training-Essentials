...
## 1. Đấu tranh với Người kiểm tra khoản vay — Nhân bản không cần thiết
```rust
...
## 3. Bỏ qua kết quả có`unwrap()`trong sản xuất
```rust
...
## 5. Anti-Pattern: Sử dụng`Rc`/`RefCell`ở mọi nơi
```rust
...
## 6. Không Hiểu Ngữ Nghĩa Di Chuyển
```rust
...
## 7. Chống mẫu:`to_string()`/`to_owned()`quá mức
```rust
...
## 9. Vô hiệu hóa Iterator — Thu thập một cách không cần thiết
```rust
...
## 10. Anti-Pattern: Enum không có phương thức
```rust
...
## 11. Không sử dụng`#[derive]`cho những đặc điểm chung
```rust
...
## 12. Mã không an toàn không có tài liệu
```rust
...
## Bản tóm tắt
Trình biên dịch của Rust rất nghiêm ngặt nhưng công bằng — hầu hết các lỗi đều dạy cho bạn điều gì đó về an toàn bộ nhớ. Sai lầm lớn nhất không phải là lỗi trình biên dịch mà là lỗi chống mẫu: sao chép không cần thiết để xoa dịu người kiểm tra mượn, sử dụng`unwrap()`trong sản xuất, tiếp cận`Rc<RefCell<T>>`trước khi thử quyền sở hữu và phân bổ chuỗi khi đủ tham chiếu. Tư duy của Rustacean là: hãy để trình biên dịch hướng dẫn bạn những cách trừu tượng hóa không tốn chi phí. Chỉ sao chép khi cần thiết, truyền lỗi với`?`, ưu tiên`&str`hơn`String`và ghi lại mọi khối `unsafe`.