...
---
# Rust — Các mẫu thành ngữ & các phương pháp hay nhất
Hướng dẫn này bao gồm các mẫu thành ngữ và các phương pháp hay nhất để viết mã Rust sạch sẽ, an toàn.
---
...
## Bản tóm tắt
Thành ngữ Rust nhấn mạnh: quyền sở hữu và mượn,`Result`/`Option`để xử lý lỗi, enum với các phương thức, đặc điểm để trừu tượng hóa, chuỗi vòng lặp, con trỏ thông minh cho trạng thái chia sẻ và trừu tượng hóa không tốn chi phí. Làm theo`rustfmt`để định dạng,`clippy`để tìm lỗi mã nguồn và Nguyên tắc API Rust. Cộng đồng Rust coi trọng sự an toàn, hiệu suất và tính chính xác — "nếu nó biên dịch, nó sẽ hoạt động." Ưu tiên`&str`hơn`String`trong các đối số hàm, sử dụng`?`để truyền lỗi và rút ra các đặc điểm chung với`#[derive(Debug, Clone, ...)]`.