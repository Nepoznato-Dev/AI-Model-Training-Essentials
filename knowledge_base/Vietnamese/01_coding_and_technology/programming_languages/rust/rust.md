...
|----------|----------|-------------------|
| **Đường cong học tập dốc** | Quyền sở hữu, vay mượn, thời gian sống không giống bất cứ điều gì trong các ngôn ngữ khác | Đầu tư thời gian vào "Cuốn sách rỉ sét"; các khái niệm nhấp vào thực hành |
| **Biên dịch chậm** | Thời gian biên dịch có thể lâu đối với các dự án lớn | Sử dụng`cargo check`để kiểm tra loại nhanh; giúp biên soạn tăng dần |
| **Xử lý lỗi dài dòng** |  Toán tử`Result<T, E>`và`?`yêu cầu xử lý rõ ràng | Sử dụng`anyhow`cho ứng dụng,`thiserror`cho thư viện |
| **Thị trường việc làm nhỏ hơn** | Ít công việc Rust hơn Java, Python hoặc JavaScript (nhưng tăng nhanh) | Hầu hết các vai trò của Rust đều thuộc về lập trình hệ thống, tiền điện tử hoặc cơ sở hạ tầng |
| **Hệ sinh thái chưa trưởng thành** | Ít thư viện hơn Python/Java/JS cho một số miền | Hệ sinh thái đang phát triển nhanh chóng; nhiều thùng chất lượng tuyệt vời |
---
...
Đây là sự đổi mới cốt lõi của Rust. Mỗi giá trị có chính xác một chủ sở hữu. Khi chủ sở hữu đi ra ngoài phạm vi, giá trị sẽ bị loại bỏ.
```rust
...
### Macro
Rust có hai loại macro: khai báo (`macro_rules!`) và thủ tục (dẫn xuất, thuộc tính, giống hàm).
```rust
...
### Kết hợp và hủy cấu trúc mẫu nâng cao
```rust
...
### Không đồng bộ/Đang chờ với Tokio
```rust
...
| Công cụ | Mục đích |
|------|----------|
| **hàng hóa** | Xây dựng hệ thống, trình quản lý gói, trình chạy thử, trình tạo tài liệu |
| **crates.io** | Đăng ký gói (150.000+ thùng) |
| **rỉ sét** | Trình định dạng mã |
| **cắt** | Nói dối với hàng trăm bài kiểm tra hữu ích |
| **toki** | Thời gian chạy không đồng bộ (tiêu chuẩn cho async Rust) |
| **serde** | Khung tuần tự hóa/giải tuần tự hóa |
| **actix-web / axum** | Khung web |
| **diesel / sqlx** | ORM cơ sở dữ liệu / trình tạo truy vấn |
---

## Khi nào nên sử dụng Rust
| Kịch bản | Tại sao rỉ sét | Thay thế tốt hơn |
|----------|----------|-------------------|
| Lập trình hệ thống | An toàn bộ nhớ + hiệu suất | C/C++ nếu bạn không cần đảm bảo an toàn |
| WebHội | Hỗ trợ WASM tốt nhất | -- |
| công cụ CLI | Nhanh, nhị phân đơn, UX tuyệt vời | Sử dụng CLI đơn giản hơn |
| Hệ thống nhúng | Không GC, truy cập phần cứng, an toàn | C để nhúng đơn giản hơn |
| Mã quan trọng về hiệu suất | Phù hợp với tốc độ C/C++ | -- |
| Cơ sở hạ tầng đám mây | Việc áp dụng ngày càng tăng (AWS, Cloudflare) | Đi để phát triển nhanh hơn |
| Phát triển ứng dụng chung | Đường cong học tập dốc làm chậm dev | Python, Go, Java |
...
## Bản tóm tắt
Rust là ngôn ngữ buộc bạn phải suy nghĩ về bộ nhớ, quyền sở hữu và tính đồng thời -- và thưởng cho bạn mã chính xác theo cách xây dựng. Quá trình học tập là có thật, nhưng kết quả thu được rất đáng kể: các chương trình nhanh như C nhưng không có lỗi con trỏ null, chạy đua dữ liệu và rò rỉ bộ nhớ. Rust không phải là ngôn ngữ năng suất có mục đích chung - nó là ngôn ngữ hệ thống khi cả tính chính xác và hiệu suất đều quan trọng. Việc áp dụng ngày càng tăng của nó trong công nghiệp (bao gồm cả nhân Linux và Android) cho thấy nó sẽ ngày càng trở nên quan trọng.