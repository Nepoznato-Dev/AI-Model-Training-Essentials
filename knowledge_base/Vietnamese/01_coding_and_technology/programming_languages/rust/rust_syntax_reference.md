...
-->
# Rust — Tham khảo cú pháp
Tài liệu này cung cấp tài liệu tham khảo cú pháp có cấu trúc, toàn diện cho Rust. Nó bổ sung cho tham chiếu Rust chính bằng cách tập trung vào các mẫu cú pháp đầy đủ, bảng toán tử và cơ chế nội bộ của hệ thống sở hữu, các đặc điểm và tính đồng thời.
---
...
|----------|------|----------|-------|
| `+`| Ngoài ra | `3 + 2`| Ngoài ra còn nối`String`thông qua`format!`|
| `-`| Phép trừ | `3 - 2`| |
| `*`| Phép nhân | `3 * 2`| |
| `/`| Phân chia | `7 / 2`| Phép chia số nguyên cho số nguyên:`3`|
| `%`| Phần còn lại | `7 % 2`| `1`|
| `-x`| Phủ định | `-5`| |
### Toán tử so sánh và logic
| Nhà điều hành | Tên | Ví dụ | Ghi chú |
|----------|------|----------|-------|
| `==`| Bằng | `x == y`| Yêu cầu`PartialEq`|
| `!=`| Không bằng | `x != y`| |
| `<`,`>`,`<=`,`>=`| Đặt hàng | `x >= y`| Yêu cầu`PartialOrd`|
...
| `\|`| HOẶC | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `!`| KHÔNG | `!0b1010u8`| Phần bù theo bit |
| `<<`| Dịch Chuyển Trái | `5 << 1`| `10`|
| `>>`| Shift Phải | `5 >> 1`| `2`|
### Độ ưu tiên của toán tử (cao nhất đến thấp nhất)
| Ưu tiên | Người vận hành | Ghi chú |
|----------||----------|-------|
| 1 (cao nhất) |  Đường dẫn`()``.` :: | |
| 2 | Cuộc gọi phương thức, truy cập trường | Tự động tắt |
| 3 | `-`(phủ định),`!`,`*`(deref) | |
| 4 | `as`(loại đúc) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
| 14 | `..``..=` | Phạm vi |
| 15 | `=``+=`, v.v. | Bài tập |
| 16 (thấp nhất) | `return``break``continue`| |
---
...
| Loại | Ngăn xếp/Đống | Kích thước | Khả năng thay đổi | Ghi chú |
|------|----------|------|----------|-------|
| Nguyên thủy (`i32`,`f64`,`bool`,`char`) | Ngăn xếp | Đã sửa | Không thể thay đổi theo mặc định | `Copy`|
| `[T; N]`(mảng) | Ngăn xếp | Đã sửa | Có thể thay đổi với`mut`| Kích thước cố định,`Copy`nếu T là`Copy`|
| `Vec<T>`| Đống | Năng động | Có thể thay đổi với`mut`| Có thể trồng được |
| `String`| Đống | Năng động | Có thể thay đổi với`mut`| UTF-8, có thể phát triển |
| `&[T]`(lát) | Stack (ptr béo) | Chế độ xem động | Phụ thuộc vào`&`hoặc`&mut`| Xem mượn |
| `HashMap<K, V>`| Đống | Năng động | Có thể thay đổi với`mut`| Tra cứu trung bình O(1) |
| `BTreeMap<K, V>`| Đống | Năng động | Có thể thay đổi với`mut`| Tra cứu O(log n), đặt hàng |
| `HashSet<T>`| Đống | Năng động | — | O(1) trung bình, không trùng lặp |
### Vectơ
```rust
// Creation
...
## Đặc điểm & Generic
### Định nghĩa và triển khai đặc điểm
```rust
// Define a trait
trait Summary {
...
### Đặc điểm thư viện tiêu chuẩn chung
```rust
...
## Bản tóm tắt
Cú pháp của Rust được xây dựng trên nền tảng đảm bảo an toàn được thực thi tại thời điểm biên dịch. Hệ thống quyền sở hữu, tính đa hình dựa trên đặc điểm, khớp mẫu và trừu tượng hóa không tốn phí tạo thành một thiết kế mạch lạc trong đó trình biên dịch là một đối tác nghiêm ngặt nhưng công bằng. Từ luồng điều khiển cơ bản đến các tính năng nâng cao như macro, mã không an toàn và con trỏ thông minh, mọi cấu trúc đều phục vụ mục tiêu lập trình hệ thống an toàn, hiệu quả. Hiểu được sự tương tác giữa các vòng đời, sự vay mượn và hệ thống loại là chìa khóa để thành thạo Rust.