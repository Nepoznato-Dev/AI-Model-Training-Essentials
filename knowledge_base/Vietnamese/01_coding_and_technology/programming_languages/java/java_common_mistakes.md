...
# Java — Các lỗi thường gặp và các mẫu phản đối
Tài liệu này liệt kê các lỗi, bẫy và phản mẫu phổ biến nhất trong Java. Mỗi mục hiển thị cách tiếp cận không chính xác, giải thích lý do tại sao nó thất bại và cung cấp giải pháp chính xác.
---

## 1.`equals()`so với`==`cho đối tượng
```java
...
## 2. Sửa đổi Bộ sưu tập trong quá trình lặp lại
```java
...
## 3. Không đóng tài nguyên (Try-with-Resources)
```java
...
## 4. Cạm bẫy của việc Autoboxing số nguyên
```java
...
## 7. Không xử lý ngoại lệ bị gián đoạn
```java
...
## 8. Trả về giá trị rỗng thay vì bộ sưu tập trống
```java
...
## 9. Đối tượng có thể thay đổi dưới dạng Khóa HashMap
```java
...
## 10. Anti-Pattern: Bắt`Exception`hoặc `Throwable`
```java
...
## 11. Không ghi đè`hashCode`khi ghi đè `equals`
```java
...
## 12. Điều kiện cuộc đua với khóa kiểm tra hai lần
```java
...
## Bản tóm tắt
Tính chi tiết của Java có thể che giấu các lỗi tinh vi:`==`so với `equals()`, sửa đổi đồng thời, rò rỉ tài nguyên, bẫy hiệu suất hộp thư tự động và hợp đồng bằng/hashCode. Cách tiếp cận Java hiện đại là: sử dụng`Objects.equals()`để so sánh an toàn null, luôn sử dụng tài nguyên dùng thử, trả về các bộ sưu tập trống thay vì null, nắm bắt các ngoại lệ cụ thể và tuân theo nguyên tắc trách nhiệm duy nhất. Các tính năng Java 8+ (luồng, Tùy chọn, tham chiếu phương thức) giảm bớt bản mẫu trong khi làm cho mã an toàn hơn và biểu cảm hơn.