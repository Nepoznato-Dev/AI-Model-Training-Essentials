...
# JavaScript — Các lỗi thường gặp và các mẫu phản kháng
Tài liệu này liệt kê các lỗi, bẫy và mẫu chống phổ biến nhất trong JavaScript. Mỗi mục hiển thị cách tiếp cận không chính xác, giải thích lý do tại sao nó thất bại và cung cấp giải pháp chính xác.
---

## 1. Nhầm lẫn liên kết `this`
```javascript
...
## 3. Các vấn đề về phạm vi và nâng `var`
```javascript
...
## 5. Không xử lý việc từ chối lời hứa
```javascript
...
## 6. Sửa đổi đối tượng trong quá trình lặp
```javascript
...
## 7. Anti-Pattern: Địa ngục gọi lại
```javascript
...
## 9. Các đối tượng giống mảng không phải là mảng
```javascript
...
## 10. Anti-Pattern: Thay đổi tham số hàm
```javascript
...
## 11. Không hiểu Event Loop và Microtask
```javascript
...
## 13. Quên dọn dẹp Event Listener
```javascript
// ❌ WRONG — memory leak
class Component {
  mount() {
...
## 14. Sao chép nông và Sao chép sâu
```javascript
...
## Bản tóm tắt
Tính linh hoạt của JavaScript đi kèm với các bẫy: liên kết`this`phụ thuộc vào trang web cuộc gọi,`==`thực hiện cưỡng chế, hứa hẹn yêu cầu xử lý lỗi và thứ tự vòng lặp sự kiện (tác vụ vi mô trước tác vụ macro) là không rõ ràng. Các quy tắc hiện đại rất đơn giản: sử dụng`const`/`let`không bao giờ`var`, thích`===`hơn`==`, luôn xử lý các lời từ chối, sử dụng các hàm mũi tên cho lệnh gọi lại, tránh đột biến và dọn dẹp trình xử lý sự kiện. Việc tuân theo các quy tắc này sẽ loại bỏ phần lớn các lỗi JavaScript.