...
| | `-`| Phép trừ | `A - B`|
| | `*`| Phép nhân | `A * B`|
| | `/`| Phân chia thực sự | `A / B`(luôn trả về Real) |
| | `div`| Phép chia số nguyên | `A div B`(trả về số nguyên) |
| | `mod`| Mô đun | `A mod B`|
| **So sánh** | `=`| Bằng | `A = B`|
| | `<>`| Không bằng | `A <> B`|
| | `<`| Ít hơn | `A < B`|
...
| **Hợp lý** | `and`| Hợp lý VÀ | `A and B`|
| | `or`| Hợp lý HOẶC | `A or B`|
| | `not`| KHÔNG logic | `not A`|
| | `xor`| Độc quyền HOẶC | `A xor B`|
| **Chuỗi** | `+`| Nối | `'Hello' + ' ' + 'World'`|
| **Đặt** | `in`| Thành viên | `X in [1, 2, 3]`|
| | `+`| Liên minh | `SetA + SetB`|
| | `-`| Sự khác biệt | `SetA - SetB`|
| | `*`| Giao lộ | `SetA * SetB`|
| **Bài tập** | `:=`| Gán giá trị | `X := 42`|
| **Địa chỉ** | `@`| Địa chỉ của | `P := @X`|
| | `^`| Dereference | `P^`|
---
...
## Bản tóm tắt
Delphi/Object Pascal kết hợp cú pháp rõ ràng, dễ đọc với các tính năng OOP mạnh mẽ. Các lớp, giao diện, khái quát và bản ghi cung cấp bộ công cụ đầy đủ để phát triển ứng dụng. Các khối bắt đầu/kết thúc rõ ràng của ngôn ngữ, các biến được nhập và mã định danh không phân biệt chữ hoa chữ thường làm cho ngôn ngữ này có thể tiếp cận được. Khung VCL/FMX mở rộng ngôn ngữ với các thư viện thành phần trực quan để phát triển ứng dụng nhanh chóng. Đối với các ứng dụng Windows và cơ sở dữ liệu ngoại vi dành cho doanh nghiệp, Delphi vẫn là một lựa chọn hiệu quả.