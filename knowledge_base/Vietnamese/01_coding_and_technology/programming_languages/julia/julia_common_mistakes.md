...
## 1. Gõ tính không ổn định trong hàm
```julia
...
## 2. Biến toàn cục trong mã quan trọng về hiệu suất
```julia
...
## 4. Không sử dụng nhiều công văn
```julia
...
## 7. Anti-Pattern: Không sử dụng Broadcasting
```julia
...
## Bản tóm tắt
Hiệu suất của Julia phụ thuộc vào độ ổn định của loại: tránh các biến toàn cục trong mã nóng, sử dụng nhiều công văn thay vì if/else trên các loại, phân bổ trước mảng, sử dụng phát sóng cho các hoạt động theo phần tử và chú thích các loại khi cần. Julia thưởng cho các nhà phát triển viết mã ổn định về loại và tận dụng nhiều công văn. Macro`@code_warntype`là người bạn tốt nhất của bạn để tìm kiếm sự không ổn định về kiểu.