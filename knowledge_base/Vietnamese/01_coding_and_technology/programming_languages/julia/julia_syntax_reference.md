...
# Julia — Tham khảo cú pháp
Tài liệu này cung cấp tài liệu tham khảo cú pháp có cấu trúc, toàn diện cho Julia (1.x). Nó bổ sung cho tài liệu tham khảo chính của Julia bằng cách tập trung vào các mẫu cú pháp đầy đủ, nhiều công văn, macro, siêu lập trình và tính toán khoa học.
---
...
|----------|------|----------|-------|
| `+``-``*``/``^`| Số học | `2^10`| |
| `÷`| Phép chia số nguyên | `7 ÷ 3`| Tương tự như`div(7, 3)`|
| `%``\` | Mô đun/dư lượng | `7 % 3`| |
| `==``!=` | Bình đẳng | `a == b`| Giá trị bình đẳng |
| `===`| Bản sắc | `a === b`| Cùng một đối tượng |
| `<``>``<=``>=` | So sánh | `a >= b`| Có thể kết nối:`1 < x < 10`|
| `&&``\|\|``!`| Hợp lý | `a && b`| Đoản mạch |
| `&``\|``⊻``~` | Theo chiều bit | `a & b`| `⊻`là XOR |
| `:`| Phạm vi | `1:10`| `start:step:stop`|
| `..`| Khoảng thời gian | `1..10`| Yêu cầu IntervalSets |
| `∈``∉``⊆`| Đặt thành viên | `x ∈ S`| `in(x, S)`|
| `∘`| Thành phần | `f ∘ g`| `(f ∘ g)(x) == f(g(x))`|
| `\|>`| Ống | `x \|> f`| `f(x)`|
| `=>`| Cặp | `a => b`| Đối với dicts |
| `...`| Tách | `f(args...)`| Mở rộng bộ sưu tập |
### So sánh theo chuỗi
```julia
...
## Bản tóm tắt
Cú pháp của Julia rõ ràng, toán học và biểu cảm. Nhiều công văn cho phép các hàm hoạt động khác nhau dựa trên tất cả các loại đối số, cho phép tổ chức mã tinh tế. Macro cung cấp khả năng lập trình siêu dữ liệu mạnh mẽ mà không có sự phức tạp như macro tiền xử lý kiểu C. Hệ thống kiểu — với các kiểu tham số, kiểu trừu tượng và kiểu kết hợp — cho phép cả tính linh hoạt và hiệu suất. Cú pháp lập chỉ mục, hiểu và khối do 1 dựa trên 1 của Julia làm cho việc tính toán toán học và khoa học trở nên tự nhiên. Ngôn ngữ này đạt được lời hứa "đi như Python, chạy như C" thông qua quá trình biên dịch JIT với suy luận kiểu.