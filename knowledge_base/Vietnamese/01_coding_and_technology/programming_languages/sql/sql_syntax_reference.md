...
---
# SQL — Tham chiếu cú ​​pháp
Tài liệu này cung cấp tham chiếu cú ​​pháp có cấu trúc, toàn diện cho SQL (ANSI SQL với các ghi chú về sự khác biệt giữa phương ngữ PostgreSQL/MySQL/SQLite). Nó bổ sung cho tham chiếu SQL chính bằng cách tập trung vào các mẫu truy vấn đầy đủ, các hàm cửa sổ, CTE và thiết kế cơ sở dữ liệu.
---

## Cú pháp truy vấn cốt lõi
### LỰA CHỌN
```sql
-- Basic query
...
## Bản tóm tắt
Cú pháp của SQL vẫn ổn định trong hơn 50 năm và không ngừng phát triển. Các truy vấn cốt lõi (CHỌN, THAM GIA, WHERE, GROUP BY) là tiêu chuẩn trên tất cả các cơ sở dữ liệu. Các hàm cửa sổ, CTE và hỗ trợ JSON thể hiện khả năng của SQL hiện đại. Việc sử dụng SQL hiệu quả mang lại lợi ích từ việc hiểu cách công cụ cơ sở dữ liệu xử lý các truy vấn — đọc kế hoạch thực hiện, hiểu chỉ mục và viết các hoạt động dựa trên tập hợp thay vì các vòng lặp thủ tục. Kỹ năng SQL mạnh mẽ có giá trị đối với bất kỳ ai làm việc với dữ liệu.