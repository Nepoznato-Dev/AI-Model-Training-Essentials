...
# Python — Các mẫu thành ngữ & các phương pháp hay nhất
Hướng dẫn này bao gồm các mẫu thành ngữ và các phương pháp hay nhất để viết mã Pythonic rõ ràng.
---
...
## Bản tóm tắt
Mã Pythonic tuân theo Zen của Python: dễ đọc, rõ ràng, đơn giản và phẳng. Các thành ngữ chính bao gồm: hiểu danh sách/dict/set, trình quản lý bối cảnh (`with`), xử lý lỗi EAFP,`enumerate`/`zip`để lặp lại,`dataclass`cho vùng chứa dữ liệu, gợi ý loại, giải nén và kiểm tra tính trung thực/giả. Cộng đồng Python coi trọng khả năng đọc và "cần có một cách rõ ràng để làm điều đó." Tuân theo PEP 8 để biết phong cách, sử dụng`ruff`để tìm lỗi mã nguồn và ưu tiên thư viện tiêu chuẩn hơn các giải pháp của bên thứ ba khi có thể.