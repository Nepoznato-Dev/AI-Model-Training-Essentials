...
---
# Python — Tham khảo cú pháp
Tài liệu này cung cấp tài liệu tham khảo cú pháp có cấu trúc, toàn diện cho Python. Nó bổ sung cho tham chiếu Python chính bằng cách tập trung vào các mẫu cú pháp đầy đủ, bảng toán tử và cơ chế bên trong.
---
...
|----------|------|---------|--------|-------|
| `+`| Ngoài ra | `3 + 2`| `5`| Ngoài ra còn nối chuỗi/danh sách |
| `-`| Phép trừ | `3 - 2`| `1`| Ngoài ra phủ định đơn nhất |
| `*`| Phép nhân | `3 * 2`| `6`| Ngoài ra lặp lại chuỗi/danh sách |
| `/`| Phân chia thực sự | `7 / 2`| `3.5`| Luôn trả về float |
| `//`| Phòng tầng | `7 // 2`| `3`| Vòng về phía âm vô cực |
| `%`| Mô đun | `7 % 2`| `1`| Dấu số khớp với số chia |
| `**`| lũy thừa | `2 ** 10`| `1024`| Đúng-kết hợp |
### Toán tử so sánh & Boolean
| Nhà điều hành | Tên | Ví dụ | Ghi chú |
|----------|------|----------|-------|
| `==`| Bằng | `x == y`| So sánh giá trị (không phải danh tính) |
| `!=`| Không bằng | `x != y`| |
| `<`,`>`,`<=`,`>=`| Đặt hàng | `x >= y`| Có thể kết nối:`a < b < c`|
| `is`| Bản sắc | `x is y`| Cùng một đối tượng trong bộ nhớ |
| `is not`| Không phải danh tính | `x is not None`| Được ưa thích hơn`!= None`|
| `in`| Thành viên | `x in collection`| O(1) cho bộ/dicts, O(n) cho danh sách |
| `and`| Hợp lý VÀ | `a and b`| Đoản mạch; trả về giá trị trung thực/giả cuối cùng |
| `or`| Hợp lý HOẶC | `a or b`| Đoản mạch; trả về giá trị trung thực/giả đầu tiên |
| `not`| KHÔNG logic | `not x`| |
### Toán tử bitwise
| Nhà điều hành | Tên | Ví dụ | Ghi chú |
|----------|------|----------|-------|
| `&`| VÀ | `5 & 3`| `1`|
...
| 8 | `^`| Trái |
| 9 | `\|`| Trái |
| 10 | `==`,`!=`,`<`,`>`,`<=`,`>=`,`is`,`in`| Có thể xâu chuỗi |
| 11 | `not`| Đúng |
| 12 | `and`| Trái |
| 13 (thấp nhất) | `or`| Trái |
### Bài tập & Bài tập tăng cường
```python
...
| Bộ sưu tập | Đã đặt hàng | Có thể thay đổi | Bản sao | Cú pháp | Tra cứu |
|----------||---------|---------|----------||--------|--------|
| `list`| Có | Có | Có | `[1, 2, 3]`| O(n) |
| `tuple`| Có | Không | Có | `(1, 2, 3)`| O(n) |
| `dict`| Có (3,7+) | Có | Phím: Không | `{"k": "v"}`| O(1) trung bình |
| `set`| Không | Có | Không | `{1, 2, 3}`| O(1) trung bình |
| `frozenset`| Không | Không | Không | `frozenset({1,2})`| O(1) trung bình |
### Danh sách
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
## Bản tóm tắt
Tham chiếu cú ​​pháp này bao gồm bộ tính năng hoàn chỉnh của Python — từ các toán tử cơ bản và luồng điều khiển cho đến siêu lập trình, bộ mô tả và hệ thống kiểu nâng cao. Cú pháp của Python được cố tình tối giản và dễ đọc nhưng độ sâu của nó rất đáng kể: khớp mẫu, trình tạo không đồng bộ, giao thức và siêu dữ liệu cung cấp các công cụ mạnh mẽ cho các ứng dụng phức tạp. Ngôn ngữ tiếp tục phát triển, với mỗi bản phát hành đều bổ sung thêm các tính năng giúp cải thiện tính biểu cảm trong khi vẫn duy trì khả năng tương thích ngược.