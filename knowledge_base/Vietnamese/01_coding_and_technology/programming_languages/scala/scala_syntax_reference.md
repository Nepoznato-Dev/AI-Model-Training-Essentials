...
---
# Scala — Tham khảo cú pháp
Tài liệu này cung cấp tài liệu tham khảo cú pháp có cấu trúc, toàn diện cho Scala (3.x có ghi chú về sự khác biệt của 2.x). Nó bổ sung cho tham chiếu Scala chính bằng cách tập trung vào các mẫu cú pháp đầy đủ, hệ thống kiểu, cấu trúc lập trình chức năng và thành ngữ Scala.
---
...
|----------|------|----------|-------|
| `+``-``*``/``%`| Số học | `a + b`| |
| `**`| Sức mạnh (BigDecimal) | `BigDecimal(2) ** 10`| Không được tích hợp sẵn cho Int |
| `==``!=` | Bình đẳng | `a == b`| Cuộc gọi`.equals()`— null-safe |
| `eq``ne` | Bình đẳng tham khảo | `a eq b`| Cùng một trường hợp |
| `<``>``<=``>=` | So sánh | `a >= b`| Yêu cầu`Ordered`|
| `&&``\|\|``!`| Hợp lý | `a && b`| Đoản mạch |
| `&``\|``^``~` | Theo chiều bit | `a & b`| |
| `<<``>>``>>>`| Thay đổi | `a << 2`| |
| `+:``:+``::``:::` | Hoạt động thu thập | `1 +: list`| Liên kết phải nếu kết thúc bằng`:`|
| `->`| Tạo bộ dữ liệu | `"key" -> "value"`| `("key", "value")`|
| `=>`| Chức năng/mũi tên | `x => x + 1`| Lambda hoặc ánh xạ kiểu |
| `<-`| Máy phát điện | `x <- List(1,2,3)`| Để hiểu |
### Quy tắc của người vận hành
```scala
...
## Bản tóm tắt
Cú pháp của Scala thống nhất lập trình hướng đối tượng và lập trình chức năng trong một hệ thống kiểu mạnh mẽ. Đối sánh mẫu, các lớp để hiểu và trường hợp cung cấp mô hình hóa dữ liệu biểu cảm. Các đặc điểm cho phép bố cục linh hoạt mà không có giới hạn về kế thừa đơn lẻ. Hệ thống kiểu — với các kiểu tổng quát, phương sai, giới hạn kiểu và kiểu mờ — phát hiện lỗi tại thời điểm biên dịch trong khi cho phép trừu tượng hóa với chi phí bằng 0. Scala 3 đơn giản hóa ngôn ngữ với các enum, phương thức mở rộng, các kiểu đã cho/sử dụng và các kiểu mờ, làm cho ngôn ngữ dễ tiếp cận hơn trong khi vẫn giữ được toàn bộ sức mạnh của nó.