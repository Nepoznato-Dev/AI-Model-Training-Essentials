...
---
# C++ — Các lỗi thường gặp và các mẫu phản kháng
Tài liệu này liệt kê các lỗi, bẫy và phản mẫu phổ biến nhất trong C++. Mỗi mục hiển thị cách tiếp cận không chính xác, giải thích lý do tại sao nó thất bại và cung cấp giải pháp chính xác. C++ hiện đại (C++11/14/17/20) cung cấp các công cụ để tránh hầu hết các cạm bẫy cổ điển.
---

## 1. Quy tắc vi phạm ba/năm
```cpp
...
## 2.`new`/`delete`thô thay vì Con trỏ thông minh
```cpp
...
## 6. Sử dụng`std::move`không đúng cách
```cpp
...
## 7. Anti-Pattern: Sử dụng`using namespace std;`trong Header
```cpp
...
## 8. Hành vi không xác định từ Signed Overflow
```cpp
...
## 10. Anti-Pattern: Quản lý tài nguyên thủ công (Pre-RAII)
```cpp
...
## 11. Mẫu phồng lên và thời gian biên dịch dài
```cpp
...
## 12. Không sử dụng`[[nodiscard]]`và `[[maybe_unused]]`
```cpp
...
## Bản tóm tắt
Sự phức tạp của C++ tạo ra nhiều cơ hội mắc lỗi: Quy tắc số 5, cắt đối tượng, vô hiệu hóa trình vòng lặp, tham chiếu lơ lửng và hành vi không xác định từ tràn đã ký. C++ hiện đại là liều thuốc giải độc: sử dụng con trỏ thông minh thay vì`new`/`delete`thô, RAII cho tất cả tài nguyên,`std::vector`và`std::string`thay vì mảng thô, di chuyển ngữ nghĩa để đạt hiệu quả và`[[nodiscard]]`để bắt các giá trị trả về bị bỏ qua. Nguyên tắc cốt lõi của C++ tồn tại là có lý do — hãy tuân theo chúng và sử dụng các công cụ như Clang-Tidy và công cụ khử trùng để nắm bắt những gì bạn bỏ lỡ.