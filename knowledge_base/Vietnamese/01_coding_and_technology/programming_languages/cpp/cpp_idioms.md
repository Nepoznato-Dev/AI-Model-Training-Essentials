...
## Bản tóm tắt
Các thành ngữ C++ hiện đại nhấn mạnh: RAII và con trỏ thông minh, ngữ nghĩa di chuyển,`auto`để loại trừ, phạm vi và thuật toán (C++20), các khái niệm về ràng buộc mẫu,`std::optional`/`std::expected`cho sự vắng mặt/lỗi, Quy tắc số 0 và`constexpr`cho tính toán thời gian biên dịch. Theo dõi`clang-format`để định dạng,`clang-tidy`để tìm lỗi mã nguồn và biên dịch bằng`-Wall -Wextra -Werror`. Cộng đồng C++ coi trọng sự trừu tượng không tốn phí và "không trả tiền cho những gì bạn không sử dụng".