...
## Bản tóm tắt
Cú pháp hợp ngữ được xác định bởi kiến ​​trúc bộ xử lý và trình biên dịch chương trình (NASM, GAS, MASM). x86-64 cung cấp 16 thanh ghi đa năng, nhiều chế độ địa chỉ và hàng trăm lệnh bao gồm chuyển động dữ liệu, số học, logic, luồng điều khiển, SIMD và lệnh gọi hệ thống. Cú pháp NASM được sử dụng ở đây là cú pháp phổ biến nhất để phát triển Linux x86-64. Việc thành thạo hợp ngữ đòi hỏi phải hiểu các thanh ghi, ngăn xếp, quy ước gọi và cách ánh xạ các cấu trúc cấp cao tới các lệnh máy.