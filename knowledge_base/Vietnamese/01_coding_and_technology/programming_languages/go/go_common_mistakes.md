...
---
# Đi — Những sai lầm thường gặp & những mô hình phản đối
Tài liệu này liệt kê các lỗi, bẫy và mô hình phản đối phổ biến nhất trong Go. Mỗi mục hiển thị cách tiếp cận không chính xác, giải thích lý do tại sao nó thất bại và cung cấp giải pháp chính xác.
---

## 1. Chụp biến vòng lặp trong Goroutine
```go
...
## 5. Không tham chiếu con trỏ trên các phương thức Struct
```go
...
## 6. Sử dụng`defer`trong vòng lặp
```go
...
## 7. Anti-Pattern: Trả về các biến được đặt tên sớm
```go
...
## 8. Thứ tự lặp bản đồ là ngẫu nhiên
```go
...
## 10. Anti-Pattern: Ô nhiễm giao diện
```go
...
## 11. Không sử dụng`sync.Once`để khởi tạo
```go
...
## Bản tóm tắt
Tính đơn giản của Go là điểm mạnh của nó, nhưng nó có những cái bẫy tinh vi: goroutine rò rỉ từ các kênh không có bộ đệm, ngữ nghĩa nối thêm không sửa đổi ban đầu, các cuộc gọi hoãn lại tích lũy trong các vòng lặp và lặp lại bản đồ được ngẫu nhiên hóa có chủ ý. Câu tục ngữ Go hướng dẫn cách: xử lý mọi lỗi, chấp nhận giao diện nhưng trả về cấu trúc, sử dụng bộ nhận con trỏ cho các loại mutex và ưu tiên các kiểu nguyên thủy đồng thời (`sync.Once`,`errgroup`, hủy ngữ cảnh) hơn là quản lý goroutine thủ công. Trước tiên hãy viết mã đơn giản - Hãy thưởng cho sự rõ ràng hơn là sự thông minh.