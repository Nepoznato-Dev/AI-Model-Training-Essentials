...
# Go — Tham khảo cú pháp
Tài liệu này cung cấp tài liệu tham khảo cú pháp có cấu trúc, toàn diện cho Go. Nó bổ sung cho tham chiếu Go chính bằng cách tập trung vào các mẫu cú pháp đầy đủ, bảng toán tử và cơ chế bên trong của goroutine, kênh và giao diện.
---
...
| `-`| Phép trừ | `3 - 2`| `1`| |
| `*`| Phép nhân | `3 * 2`| `6`| |
| `/`| Phân chia | `7 / 2`| `3`| Chia số nguyên cho int |
| `%`| Phần còn lại | `7 % 2`| `1`| Dấu hiệu khớp với cổ tức |
| `&`| Theo chiều bit VÀ | `5 & 3`| `1`| |
| `\|`| Theo bit HOẶC | `5 \| 3`| `7`| |
| `^`| XOR theo bit | `5 ^ 3`| `6`| Ngoài ra phần bù bitwise (đơn nguyên) |
| `&^`| Rõ ràng một chút (VÀ KHÔNG) | `5 &^ 3`| `4`| Duy nhất để đi |
| `<<`| Dịch Chuyển Trái | `5 << 1`| `10`| |
| `>>`| Shift Phải | `5 >> 1`| `2`| |
### Toán tử so sánh
| Nhà điều hành | Tên | Ví dụ | Ghi chú |
|----------|------|----------|-------|
| `==`| Bằng | `x == y`| Chỉ các loại có thể so sánh |
| `!=`| Không bằng | `x != y`| |
| `<`,`>`,`<=`,`>=`| Đặt hàng | `x >= y`| Các chuỗi được so sánh theo từ điển |
### Toán tử logic
| Nhà điều hành | Tên | Ví dụ | Ghi chú |
|----------|------|----------|-------|
...
## Bản tóm tắt
Cú pháp của Go được cố ý tối giản — không kế thừa, không nạp chồng phương thức, không ngoại lệ, không macro. Những gì còn lại là một tập hợp rõ ràng, trực giao các cấu trúc: cấu trúc và giao diện cho thành phần, goroutines và kênh cho hoạt động đồng thời cũng như xử lý lỗi rõ ràng để đảm bảo độ tin cậy. Sức mạnh của ngôn ngữ không đến từ cú pháp phức tạp mà đến từ sự tương tác tao nhã của những nguyên thủy đơn giản của nó. Generics (Go 1.18+) thêm tính linh hoạt của loại mà không làm mất đi tính rõ ràng đặc trưng của Go.