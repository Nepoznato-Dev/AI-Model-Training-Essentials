...
# Ruby — Tham khảo cú pháp
Tài liệu này cung cấp tài liệu tham khảo cú pháp có cấu trúc, toàn diện cho Ruby (3.x). Nó bổ sung cho tài liệu tham khảo chính của Ruby bằng cách tập trung vào các mẫu cú pháp, khối và phần đóng, siêu lập trình và thành ngữ Ruby đầy đủ.
---
...
| `==``!=``===``<=>` | Bình đẳng | `a == b`| `===`là phân biệt chữ hoa chữ thường; `<=>`là tàu vũ trụ |
| `eql?`| Giá trị bình đẳng | `a.eql?(b)`| Nghiêm ngặt hơn`==`(loại kiểm tra) |
| `equal?`| Bản sắc | `a.equal?(b)`| Cùng một đối tượng? |
| `<``>``<=``>=` | So sánh | `a >= b`| |
| `&&``\|\|``!`| Hợp lý | `a && b`| Đoản mạch; trả về giá trị được đánh giá cuối cùng |
| `and``or``not`| Hợp lý (ưu tiên thấp) | `a and b`| Tránh — sử dụng`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | Theo chiều bit | `a & b`| |
| `=~``!~` | Trận đấu Regex | `str =~ /pattern/`| Trả về chỉ số hoặc con số không |
| `..``...` | Phạm vi | `1..10`|  Bao gồm `..`; `...`độc quyền |
| `?.`| Điều hướng an toàn | `user&.name`| Trả về 0 nếu người dùng là 0 |
...
## Bản tóm tắt
Cú pháp của Ruby được thiết kế nhằm mang lại sự hài lòng cho nhà phát triển — nó đọc giống như tiếng Anh và mang lại tính biểu cảm. Sức mạnh của ngôn ngữ đến từ mô hình đối tượng của nó (mọi thứ đều là một đối tượng), các khối và bao đóng, khả năng siêu lập trình và mô-đun Enumerable cung cấp API xử lý bộ sưu tập phong phú. Ruby 3.x đã thêm tính năng khớp mẫu, chữ ký kiểu RBS và Ractor để chạy đồng thời, trong khi vẫn duy trì triết lý cốt lõi của ngôn ngữ: giúp các lập trình viên làm việc hiệu quả và vui vẻ.