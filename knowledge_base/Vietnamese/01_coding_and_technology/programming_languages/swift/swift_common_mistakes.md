...
# Swift — Những lỗi thường gặp và những mẫu phản đối
Tài liệu này liệt kê các lỗi, bẫy và anti-pattern phổ biến nhất trong Swift. Mỗi mục hiển thị cách tiếp cận không chính xác, giải thích lý do tại sao nó thất bại và cung cấp giải pháp chính xác.
---

## 1. Buộc mở các tùy chọn
```swift
// ❌ WRONG — crash if nil
...
## 3. Không sử dụng`enum`với các giá trị liên kết
```swift
...
## 4. Hiểu nhầm kiểu giá trị và kiểu tham chiếu
```swift
...
## 5. Anti-Pattern: Bộ điều khiển chế độ xem lớn
```swift
...
## 6. Không sử dụng`guard`để thoát sớm
```swift
...
## 9. Không sử dụng`lazy`cho tài sản đắt tiền
```swift
...
## Bản tóm tắt
Các tính năng an toàn của Swift ngăn chặn nhiều lỗi nhưng lại gây ra những cái bẫy riêng: buộc mở gói gây ra sự cố, giữ lại các chu trình ẩn trong các bao đóng và đại biểu cũng như ngữ nghĩa loại giá trị khiến các nhà phát triển đến từ các ngôn ngữ loại tham chiếu ngạc nhiên. Cách của Swift là: không bao giờ ép buộc mở (`!`), sử dụng`guard`để thoát sớm, sử dụng`weak`cho đại biểu và chụp đóng, đánh dấu mã UI bằng`@MainActor`và chia bộ điều khiển chế độ xem lớn thành mô hình và dịch vụ chế độ xem. Hệ thống kiểu của Swift là bạn của bạn — hãy hợp tác với nó chứ không phải chống lại nó.