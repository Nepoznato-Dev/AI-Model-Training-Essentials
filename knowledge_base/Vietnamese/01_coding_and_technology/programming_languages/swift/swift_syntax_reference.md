...
---
# Swift — Tham khảo cú pháp
Tài liệu này cung cấp tài liệu tham khảo cú pháp có cấu trúc, toàn diện cho Swift (5.9+). Nó bổ sung cho tham chiếu Swift chính bằng cách tập trung vào các mẫu cú pháp đầy đủ, các tùy chọn, lập trình hướng giao thức và tính tương tranh hiện đại.
---
...
|----------|------|----------|-------|
| `+``-``*``/``%`| Số học | `a + b`| |
| `..``...` | Phạm vi | `1...10`| `...`đã đóng cửa; `..<`nửa mở |
| `==``!=``<``>``<=``>=` | So sánh | `a == b`| Yêu cầu`Equatable`/`Comparable`|
| `===``!==` | Bản sắc | `a === b`| Cùng một tài liệu tham khảo (chỉ các lớp) |
| `&&``\|\|``!`| Hợp lý | `a && b`| Đoản mạch |
| `??`| Không liên kết | `a ?? b`| Trả về`b`nếu`a`bằng 0 |
| `?.`| Chuỗi tùy chọn | `a?.b`| Trả về 0 nếu`a`bằng 0 |
| `!`| Buộc mở khóa | `a!`| Sự cố nếu không - tránh |
| `&`| Tham số đầu vào | `swap(&a, &b)`| Đi qua tham khảo |
### Toán tử xử lý không
```swift
...
## Trình bao bọc thuộc tính & Trình tạo kết quả
```swift
// Property wrapper
@propertyWrapper
struct Trimmed {
...
## Bản tóm tắt
Cú pháp của Swift cân bằng giữa tính an toàn và tính biểu cảm. Các tùy chọn loại bỏ sự cố rỗng, các giao thức cho phép kết hợp linh hoạt mà không cần kế thừa và tính đồng thời hiện đại (async/await, Actor) giúp lập trình đồng thời an toàn có thể truy cập được. Ngôn ngữ tiếp tục phát triển với các tính năng như gói tham số, macro và quyền sở hữu — mỗi bổ sung giúp Swift an toàn hơn và mạnh mẽ hơn trong khi vẫn duy trì lời hứa cốt lõi: mã được xây dựng an toàn.