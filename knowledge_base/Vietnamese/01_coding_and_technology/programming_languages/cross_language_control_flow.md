...
---
# So sánh đa ngôn ngữ — Luồng điều khiển
## Câu lệnh If/Else
```python
# Python
if x > 0:
...
| rỉ sét | `for i in 0..10`|
| Đi | `for i := 0; i < 10; i++`|
| Java | `for (int i = 0; i < 10; i++)`|
| C/C++ | `for (int i = 0; i < 10; i++)`|
| Ruby | `10.times { \|i\| ... }`hoặc`(0..9).each { \|i\| ... }`|
| Nhanh | `for i in 0..<10`|
...
| R | `for (i in 1:10)`|
| PHP | `for ($i = 0; $i < 10; $i++)`|
| SQL | Không áp dụng (dựa trên tập hợp, không có vòng lặp trong SQL tiêu chuẩn) |
| Đánh | `for i in {0..9}; do ... done`|
| Lua | `for i = 0, 9 do ... end`|
| MATLAB | `for i = 1:10 ... end`|
| Prolog | Không áp dụng (đệ quy thay thế) |
| Nói ngọng | `(dotimes (i 10) ...)`|
| Ada | `for I in 0..9 loop ... end loop;`|
| COBOL | `PERFORM VARYING I FROM 0 BY 1 UNTIL I >= 10`|
| Erlang | Không áp dụng (đệ quy + danh sách thay thế) |
| OCaml | `for i = 0 to 9 do ... done`|
| Hội | Vòng lặp thủ công với CMP/JMP |
| Delphi | `for I := 0 to 9 do ... end;`|
| Cào | `repeat (10) ... end`|
| VB | `For i As Integer = 0 To 9 ... Next`|
| Scala | `for (i <- 0 until 10) ...`|
| Phi tiêu | `for (var i = 0; i < 10; i++)`|
...
| Nhanh | `switch x { case 1: ... default: ... }`|
| Kotlin | `when (x) { 1 -> ... else -> ... }`|
| OCaml | `match x with 1 -> ... \| _ -> ...`|
| Erlang | `case X of 1 -> ...; _ -> ... end`|
| Thuốc tiên | `case x do 1 -> ...; _ -> ... end`|
| Phi tiêu | `switch (x) { 1 => ...; _ => ... }`(Phi tiêu 3.0) |
| PHP | `match($x) { 1 => ..., default => ... }`(PHP 8.0) |
| C# | `x switch { 1 => ..., _ => ... }`(C# 8+) |
| TypeScript | Không khớp mẫu gốc (sử dụng if/else) |
| Python | `match x: case 1: ... case _: ...`(Python 3.10+) |
| Prolog | Được tích hợp vào khớp mệnh đề |
| Đi | `switch x { case 1: ... default: ... }`|