...
Ang dokumentong ito ay nagbibigay ng komprehensibo, structured na syntax na sanggunian para sa Dart (3.x). Kinukumpleto nito ang pangunahing sanggunian ng Dart sa pamamagitan ng pagtutuon sa mga kumpletong pattern ng syntax, null na kaligtasan, async programming, at mga pattern ng disenyo na nakatuon sa Flutter.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%``~/` | Arithmetic | `7 ~/ 3`|  Ang`~/`ay integer division |
| `==``!=` | Pagkakapantay-pantay | `a == b`| |
| `<``>``<=``>=` | Paghahambing | `a >= b`| |
| `&&``\|\|``!`| Lohikal | `a && b`| Short-circuit |
| `??`| Null coalescing | `a ?? b`| `b`kung ang`a`ay null |
| `?.`| Null-aware na access | `a?.b`| null kung ang`a`ay null |
| `!`| Null assertion | `a!`| Itinapon kung null |
| `..``?..` | Cascade | `obj..a..b`| Mga pagpapatakbo ng chain sa parehong bagay |
| `??=`| Null-aware assign | `a ??= b`| Italaga kung null |
| `as`| I-type ang cast | `obj as String`| Itinapon sa kabiguan |
| `is``is!` | Uri ng pagsubok | `obj is String`| |
| `=>`| Arrow function na | `() => expr`| |
### Cascade Notation
```dart
...
## Mga Paraan at Pattern ng Extension
```dart
// Extension methods
...
## Buod
Ang syntax ng Dart ay malinis, pare-pareho, at idinisenyo para sa pagiging produktibo. Ang sound null safety ay nag-aalis ng mga null reference error sa oras ng pag-compile. Nagbibigay ang Async/wait at stream ng mga natural na pattern para sa asynchronous na programming. Ang mga klase, mixin, at mga selyadong klase ay nagbibigay-daan sa flexible na komposisyon ng bagay. Ang pagtutugma ng pattern ng Dart 3, mga tala, at mga modifier ng klase ay nagdaragdag ng pagpapahayag. Bagama't ang pangunahing layunin ng Dart ay Flutter, ang syntax nito ay angkop para sa anumang domain ng aplikasyon — panig ng server (na may Dart Frog o Shelf), mga tool sa CLI, at mga web application (na-compile sa JavaScript).