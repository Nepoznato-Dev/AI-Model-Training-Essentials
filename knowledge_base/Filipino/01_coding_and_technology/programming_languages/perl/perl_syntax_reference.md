...
| `.`| Pagsasama-sama ng string | `$a . $b`| |
| `x`| Pag-uulit ng string | `"ab" x 3`| `"ababab"`|
| `==``!=``<``>``<=``>=` | Paghahambing ng numero | `$a == $b`| |
| `eq``ne``lt``gt``le``ge` | Paghahambing ng string | `$a eq $b`| |
| `<=>`| Numeric spaceship | `$a <=> $b`| Ibinabalik -1, 0, 1 |
| `cmp`| String spaceship | `$a cmp $b`| |
| `&&``\|\|``!`| Lohikal | `$a && $b`| |
| `and``or``not`| Lohikal (mababang prec.) | `$a and $b`| Iwasan — gamitin ang`&&`/`\|\|`|
| `=~``!~` | Regex match | `$str =~ /pattern/`| |
| `..``...` | Saklaw / flip-flop | `1..10`| |
| `?:`| Ternary | `$a ? $b : $c`| |
| `//`| Tinukoy-o | `$a // $b`| `$b`kung ang`$a`ay undef |
| `=>`| Matabang kuwit | `key => 'value'`| Auto-quotes bareword |
### Mga Sigil — Mga Uri ng Variable
```perl
...
## Mga Sanggunian at Istruktura ng Data
```perl
# Array reference
my $aref = [1, 2, 3];
push @$aref, 4;
...
## Buod
Ang syntax ng Perl ay binuo sa paligid ng konteksto, mga sigil, at ang prinsipyo ng "May Higit sa Isang Paraan Upang Gawin Ito." Ang mga regular na expression ay binuo sa wika, na ginagawang hindi mapapantayan ang pagpoproseso ng teksto sa pagiging maikli. Pinapagana ng mga sanggunian ang mga kumplikadong istruktura ng data. Nagbibigay ang CPAN ng mga module para sa lahat. Ang modernong Perl (na may`strict`,`warnings`, at mga lexical na variable) ay malinis at mapanatili. Ang legacy ni Perl ay nabubuhay sa mga pattern at tool na pinagtibay ng bawat modernong wika.