...
# Lisp at Clojure — Mga Karaniwang Pagkakamali at Anti-Pattern
Kino-catalog ng dokumentong ito ang mga pinakakaraniwang pagkakamali, traps, at anti-pattern sa Lisp at Clojure na may mga pagwawasto.
---
...
## 2. Hindi Pag-unawa sa`nil`vs`false`(Clojure)
```clojure
...
## 3. Pagbabago ng Mga Koleksyon (Clojure)
```clojure
...
## 4. Hindi Paggamit ng`recur`para sa Tail Recursion (Clojure)
```clojure
...
## Buod
Lisp/Clojure traps: pinipigilan ng quote ang pagsusuri (gumamit ng quasiquote na may`,`),`nil`at`false`ay parehong huwad ngunit naiiba, ang mga koleksyon ay hindi nababago (rebind o gumamit ng mga atom), gamitin ang`recur`para sa tail recursion sa macros, at Ang Lisp na paraan ay: yakapin ang immutability, unawain ang reader macro system, at magsulat ng hygienic macros.