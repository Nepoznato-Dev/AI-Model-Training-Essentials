...
|----------|------|---------|-------|
| `+``-``*``/``%`| Arithmetic | `a + b`| |
| `..``...` | Saklaw | `1...10`| `...`sarado; `..<`kalahating bukas |
| `==``!=``<``>``<=``>=` | Paghahambing | `a == b`| Nangangailangan ng`Equatable`/`Comparable`|
| `===``!==` | Pagkakakilanlan | `a === b`| Parehong sanggunian (mga klase lamang) |
| `&&``\|\|``!`| Lohikal | `a && b`| Short-circuit |
| `??`| Nil coalescing | `a ?? b`| Ibinabalik ang`b`kung ang`a`ay nil |
| `?.`| Opsyonal na pag-chain | `a?.b`| Nagbabalik ng nil kung ang`a`ay nil |
| `!`| Pilitin ang pag-unwrap | `a!`| Nag-crash kung wala — iwasan ang |
| `&`| Parameter ng inout | `swap(&a, &b)`| Dumaan sa sanggunian |
### Nil Handling Operator
```swift
...
## Mga Balot ng Ari-arian at Tagabuo ng Resulta
```swift
// Property wrapper
@propertyWrapper
struct Trimmed {
...
## Buod
Binabalanse ng syntax ng Swift ang kaligtasan sa pagpapahayag. Tinatanggal ng mga opsyonal ang mga null crash, pinapagana ng mga protocol ang flexible na komposisyon nang walang inheritance, at ginagawang accessible ng modernong concurrency (async/naghihintay, mga aktor) ang ligtas na kasabay na programming. Patuloy na umuunlad ang wika sa mga feature tulad ng mga parameter pack, macro, at pagmamay-ari — bawat karagdagan na ginagawang mas ligtas at mas malakas ang Swift habang pinapanatili ang pangunahing pangako nito: code na ligtas sa pamamagitan ng paggawa.