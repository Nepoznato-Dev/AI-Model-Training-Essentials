...
# C — Syntax Reference
Ang dokumentong ito ay nagbibigay ng komprehensibo, structured na syntax na reference para sa C (C11/C17 na may C23 na tala). Kinukumpleto nito ang pangunahing sanggunian ng C sa pamamagitan ng pagtuon sa mga kumpletong pattern ng syntax, mga talahanayan ng operator, mekanika ng pointer, at pamamahala ng memorya.
---
...
| `/`| Dibisyon | `7 / 2`| `3`| Integer truncation para sa mga uri ng int |
| `%`| Modulus | `7 % 2`| `1`| Para lamang sa mga uri ng integer |
| `++`| Pagtaas | `i++`/`++i`| | Post- at pre-increment |
| `--`| Pagbawas | `i--`/`--i`| | Post- at pre-decrement |
...
| `\|`| O | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `~`| Komplemento | `~0`| Lahat ng mga piraso ay binaligtad |
| `<<`| Kaliwa Shift | `1 << 3`| `8`|
| `>>`| Pakanan Shift | `8 >> 3`| `1`(implementasyon-tinukoy para sa nilagdaan) |
### Mga Operator ng Pointer at Address
| Operator | Pangalan | Halimbawa | Mga Tala |
|----------|------|---------|-------|
| `&`| Address-ng | `&x`| Ibinabalik ang pointer sa x |
| `*`| Dereference | `*ptr`| I-access ang halaga sa pointer |
| `->`| Access ng miyembro sa pamamagitan ng pointer | `p->x`| Katumbas ng`(*p).x`|
| `.`| Access sa miyembro | `s.field`| Direktang struct member access |
| `[]`| Subscript ng array | `arr[i]`| Katumbas ng`*(arr + i)`|
### Operator Precedence (pinakamataas hanggang pinakamababa)
| Pangunahin | Mga Operator | Pagkakaisa |
|------------|-----------|--------------|
| 1 (pinakamataas) | `()``[]``->``.` | Kaliwa pakanan |
| 2 | `!``~``+``-` (unary)`*``&``sizeof``(type)` | Kanan pakaliwa |
| 3 | `*``/``%`| Kaliwa pakanan |
| 4 | `+``-` | Kaliwa pakanan |
| 5 | `<<``>>` | Kaliwa pakanan |
| 6 | `<``<=``>``>=` | Kaliwa pakanan |
| 7 | `==``!=` | Kaliwa pakanan |
| 8 | `&`(bitwise AT) | Kaliwa pakanan |
| 9 | `^`| Kaliwa pakanan |
| 10 | `\|`| Kaliwa pakanan |
| 11 | `&&`| Kaliwa pakanan |
| 12 | `\|\|`| Kaliwa pakanan |
| 13 | `? :`(ternary) | Kanan pakaliwa |
| 14 | `=``+=``-=``*=``/=``%=``&=``\|=``^=`XQZ1XMARKER40 | Kanan pakaliwa |
| 15 (pinakamababa) | `,`(kuwit) | Kaliwa pakanan |
---
...
### Dynamic na Pamamahala ng Memory
```c
#include <stdlib.h>
#include <string.h>
...
## Karaniwang Mahahalagang Aklatan
```c
...
## Buod
Ang syntax ng C ay minimal at malapit sa hardware — mga pointer, manu-manong pamamahala ng memorya, at isang manipis na abstraction layer sa mga tagubilin ng makina. Ang kapangyarihan ng wika ay nagmumula sa pagiging simple na ito: halos walang mga nakatagong gastos, walang overhead ng runtime, at kumpletong kontrol sa layout ng memorya. Ang preprocessor, bagama't makapangyarihan, ay nangangailangan ng disiplina upang magamit nang ligtas. Nagdagdag ang Modern C (C11/C17/C23) ng mahahalagang feature sa kaligtasan tulad ng`_Static_assert`,`_Generic`,`nullptr`, at`typeof`, na ginagawang mas ligtas ang wika habang pinapanatili ang pangunahing pilosopiya ng pagtitiwala sa programmer.