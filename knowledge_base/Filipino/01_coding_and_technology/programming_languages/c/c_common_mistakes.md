...
## 1. Buffer Overflow na may`gets()`/ `strcpy()`
```c
...
## 4. Mga Uninitialized na Variable
```c
...
## 5.`scanf()`Nang Walang Mga Limitasyon sa Lapad
```c
...
## 9. Nakalilito ang`=`at`==`sa mga Kundisyon
```c
...
## 10. Hindi Pag-unawa sa Pointer Arithmetic
```c
...
## 12. Nawawala ang`break`sa `switch`
```c
...
## Buod
Ang mga panganib ng C ay kilala ngunit palaging naroroon: buffer overflows, nakabitin na mga pointer, integer overflow, format na pag-atake ng string, at memory leaks. Ang disiplina ay: palaging suriin ang mga return value, palaging libre kung ano ang iyong inilalaan, palaging gumamit ng mga bounded function (`snprintf`,`strncpy`,`fgets`), huwag gumamit ng`gets()`, simulan ang lahat ng mga variable, at gumamit ng mga tool tulad ng Valgrind at AddressSanitizer. Ang C ay nagbibigay ng gantimpala sa mga maingat na programmer ng walang kaparis na pagganap at kontrol — ngunit pinaparusahan ang kawalang-ingat na may hindi natukoy na pag-uugali at mga kahinaan sa seguridad.