...
|----------|------|---------|-------|
| `+``-``*``/` | Arithmetic | `x + y`| Num typeclass |
| `div``mod` | Integer division | `7 \`div\` 2`| Nagbabalik 3 |
| `^``**` | Kapangyarihan | `2 ^ 10`| `^`para sa integral; `**`para sa lumulutang |
| `==``/=` | Pagkakapantay-pantay | `x == y`| Eq typeclass |
| `<``>``<=``>=` | Paghahambing | `x >= y`| Ord typeclass |
| `&&``\|\|``not`| Lohikal | `a && b`| Short-circuit |
| `.`| Komposisyon ng function | `(f . g) x`| `f (g x)`|
| `$`| Application ng function | `f $ x + 1`| `f (x + 1)`— mababang precedence |
| `<>`| Monoid idugtong | `"hello" <> " world"`| Semigroup |
| `<$>`| Functor fmap | `(+1) <$> Just 5`| `fmap (+1) (Just 5)`|
| `<*>`| Applicative apply | `pure (+) <*> Just 3`| |
| `>>=`| Monad bind | `Just 5 >>= \x -> Just (x+1)`| |
| `>>`| Monad sequence | `putStrLn "hi" >> putStrLn "bye"`| |
| `=<<`| Bind (baligtad) | `f =<< mx`| Pareho sa`mx >>= f`|
| `++`| List concatenation | `[1,2] ++ [3,4]`| O(n) sa kaliwang listahan |
| `:`| Cons | `1 : [2, 3]`| O(1) |
| `!!`| Index | `[1,2,3] !! 1`| O(n) — bahagyang paggana |
| `\\`| Pagkakaiba sa listahan | `[1,2,3] \\ [2]`| `[1,3]`|
### Operator Precedence (pinili)
| Pangunahin | Mga Operator |
|------------|-----------|
| 7 (pinakamataas) | `*``/``div``mod` |
| 6 | `+``-` |
| 5 | `:``++``\\`|
| 4 | `==``/=``<``>``<=``>=` |
| 3 | `&&`|
...
## Buod
Ang syntax ng Haskell ay minimal ngunit napaka-expressive. Ang uri ng system — na may mga algebraic na uri ng data, uri ng klase, GADT, at uri ng pamilya — ay nagsisilbing parehong detalye at isang tool sa pag-verify. Ang pagtutugma ng pattern, tamad na pagsusuri, at mga monad ay nagbibigay ng mga eleganteng solusyon sa mga kumplikadong problema. Ang kadalisayan ng Haskell (walang mga side effect sa mga uri) ay ginagawang mas madaling pangangatwiran, pagsubok, at parallelize ang code. Habang ang curve ng pag-aaral ay matarik, ang impluwensya ni Haskell ay tumatagos sa mga modernong wika — mula sa mga opsyonal ni Swift hanggang sa mga selyadong klase ng Kotlin hanggang sa sistema ng uri ng Rust. Ang pag-master ng Haskell ay ginagawa kang mas mahusay na programmer sa bawat wika.