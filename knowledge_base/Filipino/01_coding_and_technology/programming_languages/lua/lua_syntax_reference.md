...
| `//`| Dibisyon sa sahig | `7 // 2`| Ibinabalik ang 3 (Lua 5.3+) |
| `==``~=` | Pagkakapantay-pantay | `a == b`|  Ang`~=`ay "hindi pantay" |
| `<``>``<=``>=` | Paghahambing | `a >= b`| |
| `and``or``not`| Lohikal | `a and b`| Nagbabalik ng mga operand, hindi mga boolean |
| `..`| Pagsasama-sama | `"hello" .. " world"`| |
| `#`| Haba | `#t`| Haba ng string o talahanayan |
| `&``\|``~``<<``>>`| Bitwise | `a & b`| Lua 5.3+ (integer ops) |
### Logical Operators — Non-Boolean Returns
```lua
...
### Precedence (pinakamataas hanggang pinakamababa)
...
## Mga Talahanayan — Ang Pangkalahatang Istruktura ng Data
```lua
...
## Mga Module at Paghawak ng Error
```lua
...
## Mga String Pattern (Ang Alternatibong Regex ni Lua)
```lua
...
## Buod
Ang syntax ni Lua ay sadyang minimal — ang buong wika ay maaaring ilarawan sa isang pahina. Ang mga talahanayan ay nagsisilbing unibersal na istraktura ng data (mga array, diksyunaryo, bagay, module). Ang mga metatable ay nagbibigay ng pagpapalawak nang walang kumplikadong wika. Ang mga Coroutine ay nagbibigay-daan sa cooperative multitasking at eleganteng iterator pattern. Ang kapangyarihan ng wika ay nagmumula sa pagkaka-embed nito: Ang Lua ay idinisenyo upang mabuhay sa loob ng mga host application, na nagbibigay ng mga kakayahan sa pag-script na may kaunting overhead. Para sa pagbuo ng laro, pagsasaayos, at mga naka-embed na system, ang pagiging simple ni Lua ay ang pinakadakilang lakas nito — isang maliit na wika na ginagawa kung ano mismo ang kailangan nito, at wala nang iba pa.