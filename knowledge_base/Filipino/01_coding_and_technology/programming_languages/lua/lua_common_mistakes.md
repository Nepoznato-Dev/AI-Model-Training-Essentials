...
## 2.`nil`vs`false`sa Mga Kundisyon
```lua
...
## 4. Mga Global Variable ayon sa Default
```lua
...
## 7. Hindi Paggamit ng Metatables para sa OOP
```lua
...
## Buod
Lumilikha ang minimalism ni Lua ng mga bitag: 1-based na pag-index, ang`nil`at`false`lamang ang mali, ang mga talahanayan ay mga sanggunian (hindi mga halaga), ang mga variable ay pandaigdigan bilang default, at ang`pairs`ay hindi nakaayos habang ang`ipairs`ay nakaayos. Ang paraan ng Lua ay: palaging gumamit ng`local`, gumamit ng`ipairs`para sa mga array at`pairs`para sa mga diksyunaryo, gumamit ng`table.concat`para sa pagbuo ng string, at gumamit ng mga metatable para sa OOP. Ang pagiging simple ni Lua ay ang kapangyarihan nito — ngunit igalang ang mga gilid na kaso.