...
---
# R — Mga Karaniwang Pagkakamali at Anti-Pattern
Kino-catalog ng dokumentong ito ang mga pinakakaraniwang pagkakamali, traps, at anti-pattern sa R ​​na may mga pagwawasto.
---

## 1. Lumalagong Data Frame sa Loops
```r
# ❌ WRONG — O(n²) growth
result <- data.frame()
for (i in 1:1000) {
...
## 3. Hindi Pag-unawa sa Vector Recycling
```r
...
## 4.`T`at`F`kumpara sa`TRUE`at `FALSE`
```r
...
## 5. Hindi Paggamit ng `stringsAsFactors = FALSE`
```r
...
## 7. Hindi Pangasiwaan ang Mga Halaga ng `NA`
```r
...
## 8. Global Assignment kasama ang `<<-`
```r
...
## Buod
Ang interactive na katangian ng R ay lumilikha ng mga bitag: lumalaki ang mga frame ng data sa mga loop (pre-allocate o vectorize), vector recycling surprises,`NA`propagation, at`T`/`F`na ma-overwritable. Ang paraan ng R ay: i-vector ang mga operasyon, i-pre-allocate kapag naglo-loop, tahasang pangasiwaan ang `NA`, gumamit ng tidyverse para sa pagmamanipula ng data, at iwasan ang`<<-`. Ginagantimpalaan ng R ang mga nag-iisip sa mga vectors at ginagamit ang mayamang ecosystem ng mga pakete.