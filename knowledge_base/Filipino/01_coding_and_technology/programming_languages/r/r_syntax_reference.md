...
|----------|------|---------|-------|
| `+``-``*``/``^`| Arithmetic | `2^10`| |
| `%%`| Modulus | `7 %% 3`| Ibinabalik ang 1 |
| `%/%`| Integer division | `7 %/% 3`| Ibinabalik ang 2 |
| `==``!=` | Pagkakapantay-pantay | `x == y`| Vectorized |
| `<``>``<=``>=` | Paghahambing | `x >= y`| Vectorized |
| `&``\|``!`| Lohikal (na-vector) | `x & y`| Element-wise |
| `&&``\|\|` | Lohikal (scalar) | `x && y`| Unang elemento lamang |
| `%in%`| Membership | `x %in% y`| |
| `<-``=` | Takdang-aralin | `x <- 10`|  Ang`<-`ay idyomatiko |
| `->`| Italaga sa kanan | `10 -> x`| Bihirang ginagamit |
| `<<-`| Global assign | `x <<- 10`| Italaga sa parent env |
| `$`| I-extract ang elemento | `df$column`| |
| `[``[[` | Pag-index | `x[1]`/`x[[1]]`|  Ang`[`ay nagbabalik ng parehong uri; `[[`extracts |
| `%>%`| Pipe (magrittr) | `x %>% f()`| |
| `\|>`| Pipe (base R 4.1+) | `x \|> f()`| Katutubong tubo |
...
## Visualization gamit ang ggplot2
```r
library(ggplot2)
...
## Buod
Ang syntax ng R ay idinisenyo para sa statistical computing at pagsusuri ng data. Ang vectorized na katangian ng mga operasyon ay nag-aalis ng mga loop para sa karamihan ng mga gawain. Ang tidyverse ecosystem (dplyr, ggplot2, tidyr) ay nagbibigay ng pare-pareho, nababasang grammar para sa pagmamanipula at visualization ng data. Ang mga kakayahan sa pagmomodelo ng istatistika ng R — mula sa mga pangunahing t-test hanggang sa mga mixed-effect na modelo — ay walang kaparis sa lalim. Para sa data science, statistics, at reproducible research, nananatiling mahalagang tool ang R.