...
# Cross-Language Comparison — Kontrolin ang Daloy
## Kung/Ibang Pahayag
```python
# Python
if x > 0:
...
| kalawang | `for i in 0..10`|
| Pumunta | `for i := 0; i < 10; i++`|
| Java | `for (int i = 0; i < 10; i++)`|
| C/C++ | `for (int i = 0; i < 10; i++)`|
| Ruby | `10.times { \|i\| ... }`o`(0..9).each { \|i\| ... }`|
| matulin | `for i in 0..<10`|
| Kotlin | `for (i in 0 until 10)`|
| Haskell | `mapM_ f [0..9]`|
| Perl | `for my $i (0..9)`|
| R | `for (i in 1:10)`|
| PHP | `for ($i = 0; $i < 10; $i++)`|
| SQL | N/A (set-based, walang mga loop sa karaniwang SQL) |
| Bash | `for i in {0..9}; do ... done`|
| Lua | `for i = 0, 9 do ... end`|
| MATLAB | `for i = 1:10 ... end`|
| Prolog | N/A (recursion sa halip) |
| Lisp | `(dotimes (i 10) ...)`|
| Ada | `for I in 0..9 loop ... end loop;`|
| COBOL | `PERFORM VARYING I FROM 0 BY 1 UNTIL I >= 10`|
| Erlang | N/A (recursion + mga listahan sa halip) |
| OCaml | `for i = 0 to 9 do ... done`|
| Assembly | Manu-manong loop na may CMP/JMP |
| Delphi | `for I := 0 to 9 do ... end;`|
| scratch | `repeat (10) ... end`|
...
| C# | `x switch { 1 => ..., _ => ... }`(C# 8+) |
| TypeScript | Walang katutubong pattern na tumutugma (gamitin kung/iba) |
| Python | `match x: case 1: ... case _: ...`(Python 3.10+) |
| Prolog | Built in clause matching |
| Pumunta | `switch x { case 1: ... default: ... }`|