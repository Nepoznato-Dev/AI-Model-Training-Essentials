...
## Bakit Mahalaga ang Fortran
- **Pagganap ng HPC**: Ang mga Fortran compiler ay gumagawa ng ilan sa pinakamabilis na magagamit na numerical code — kadalasang tumutugma o lumalampas sa C/C++ para sa mga operasyon ng array.
- **Legacy codebase**: Ang mga dekada ng siyentipikong code (mga modelo ng klima, simulation ng pisika) ay nakasulat sa Fortran.
- **Mga operasyon ng array**: Native multi-dimensional array support na may syntax na idinisenyo para sa mathematical computation.
- **Numerical stability**: Ang wika at mga compiler ay na-optimize para sa floating-point computation.
- **Coarrays**: Built-in na parallel programming support (Fortran 2008+).
- **Nagbabago pa rin**: Nagdaragdag ang Fortran 2023 ng mga modernong feature habang pinapanatili ang backward compatibility.
## Ang mga Trade-off
| Limitasyon | Mga Detalye | Karaniwang Workaround |
|-----------|---------|-------------------|
| **Niche community** | Maliit at dalubhasa — karamihan ay siyentipiko/HPC | Aktibong komunidad sa computational science |
| **Limitadong ecosystem** | Mas kaunting mga aklatan kaysa sa Python, Java, o C++ | Gumamit ng BLAS/LAPACK para sa numerical na gawain; C interoperability para sa iba pang mga pangangailangan |
| **Hindi pangkalahatang layunin** | Mahina para sa web, mobile, GUI, o mga system programming | Gamitin ang Fortran para sa pagkalkula; wrap sa Python/C para sa mga application |
| **Perception** | Madalas na nakikita bilang "hindi na ginagamit" sa kabila ng mga modernong kakayahan | Tumutok sa mga lakas nito: numerical at HPC |
| **Hiring** | Ilang mga developer ng Fortran ang pumapasok sa merkado | Mga kasalukuyang eksperto sa akademya at pambansang laboratoryo |
---

## Syntax Fundamentals (Modern Fortran)
```fortran
program hello
    implicit none
...
### Mga Hinango na Uri at Uri-Bound na Pamamaraan
```fortran
...
### Mga Generic na Pamamaraan at Overloading ng Operator
```fortran
module vector_ops
    implicit none
...
### Mga Module at Organisasyon ng Programa
```fortran
module constants
    implicit none
...
## Pagsubok
### Test Framework (fortran-test o pFUnit)
```fortran
...
### Pagsasama ng Python (sa pamamagitan ng f2py o shared library)
```fortran
...
### Pattern 1: Array Programming (Iwasan ang Mga Loop)
```fortran
...
### Pattern 2: Mga Parameterised Derived Uri
```fortran
...
### Pattern 3: Pattern ng Callback na may mga Procedure Pointer
```fortran
module integrator
    implicit none
    abstract interface
...
### Mga Flag ng Compiler Optimization
```bash
...
## Kailan Gamitin ang Fortran
| Sitwasyon | Bakit Fortran | Mas mahusay na Alternatibo |
|----------|-----------|-------------------|
| HPC / supercomputing | Na-optimize para sa numerical na pagganap | C++ (nang may pag-iingat), Julia |
| Siyentipikong simulation | Mga dekada ng validated code | Python (para sa prototyping), C++ |
| Mga modelo ng klima/panahon | Mga legacy na codebase; pagganap | — |
| Computational physics | Mga pagpapatakbo ng katutubong array | Python (NumPy), Julia |
| Financial modelling (HPC) | Pagganap para sa malakihang pagkalkula | C++, Python |
| Pangkalahatang pag-unlad ng application | Hindi angkop | Python, Java, Go |
| Pagbuo ng web | Hindi angkop | JavaScript, Python |
| Data science (interactive) | Hindi ang daloy ng trabaho | Python, R |
---

## Synthetic na Q&A
### Q1: Ano ang pagkakaiba sa pagitan ng Fortran 90 at modernong Fortran (2008+)?
**A:** Nagdagdag ang Modern Fortran ng maraming feature na ginagawa itong mas nagpapahayag:
```fortran
...
### Q2: Paano naiiba ang Fortran arrays sa C arrays?
**A:** Ang mga Fortran array ay mga first-class na object na may mga built-in na operasyon:
```fortran
...
### Q3: Paano ko makakamit ang pinakamataas na pagganap sa Fortran?
**S:** Mga pangunahing kasanayan:
- Gumamit ng tahasang`intent`para sa lahat ng dummy na argumento
- Gamitin ang`implicit none`kahit saan
- Mas gusto ang mga operasyon ng array kaysa sa mga loop
- Gumamit ng magkadikit na memory access pattern
- Gumamit ng mga flag ng compiler optimization:`-O3 -march=native -ffast-math`
- Profile na may`gprof`o mga tool na partikular sa compiler
- Gumamit ng`pure`at`elemental`para sa mga function na maaaring i-optimize ng compiler
### Q4: Paano ko i-interface ang Fortran sa C?
**A:** Gamitin ang`iso_c_binding`module:
```fortran
use iso_c_binding
...
### Q5: Anong build system ang dapat kong gamitin para sa mga proyekto ng Fortran?
**A:** Ang CMake ay may mahusay na suporta sa Fortran. Ang FPM (Fortran Package Manager) ay ang modernong katutubong opsyon:
```bash
...
### Problema 1: Paglutas ng PDE na may mga Finite Difference
**Hakbang 1: Unawain ang Problema**
Lutasin ang 1D heat equation: du/dt = alpha * d²u/dx²
**Hakbang 2: Tukuyin ang Diskarte**
I-discretize ang espasyo at oras gamit ang may hangganang pagkakaiba. Gumamit ng isang tahasang pamamaraan.
**Hakbang 3: Ipatupad**```fortran
...
**Hakbang 4: I-verify**
Suriin ang conservation, convergence sa grid refinement, at ihambing sa analytical solution.
### Problema 2: Matrix Diagonalization
**Hakbang 1: Unawain ang Problema**
Maghanap ng mga eigenvalue at eigenvector ng isang simetriko matrix.
**Hakbang 2: Tukuyin ang Diskarte**
Gamitin ang`dsyev`routine ng LAPACK sa pamamagitan ng interface ng Fortran.
**Hakbang 3: Ipatupad**```fortran
program diagonalize
    use lapack95
    implicit none
...
**Hakbang 4: I-verify**
Suriin na ang A*v = lambda*v para sa bawat eigenpair.
---

## Buod
Ang Fortran ay ang orihinal na pang-agham na programming language at nananatiling isang powerhouse sa high-performance computing. Ang Modern Fortran ay isang may kakayahang, umuusbong na wika na may mga pagpapatakbo ng native array, parallel programming support, at C interoperability. Bagama't maliit at dalubhasa ang komunidad nito, patuloy na pinapatakbo ng Fortran ang ilan sa mga pinaka-hinihingi na computational workload sa mundo. Para sa numerical computing sa sukat, nananatiling may kaugnayan ang Fortran.