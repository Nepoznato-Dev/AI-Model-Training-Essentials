...
|-------|-----------|-----------|
| **Exceptions (try/catch)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **Mga uri ng resulta/Pagpipilian** | kalawang, Haskell, OCaml, Scala (din) | `Result<T, E>`,`Maybe a`|
| **Maraming halaga ng pagbabalik** | Sige, Lua | `value, err := f()`|
| **Mga error code** | C, Fortran | Ibalik ang error code, suriin nang manu-mano |
| **Mga error sa pagtutugma ng pattern** | Elixir, Erlang | `{:ok, value}`/`{:error, reason}`|
| **Mga Kundisyon (maaaring i-restart)** | Karaniwang Lisp | `restart-case`,`handler-bind`|
| **Walang error sa paghawak** | Scratch, SQL (basic) | N/A |
| **Mga Kontrata** | Ada, Eiffel | `Pre`,`Post`kundisyon |
...
## Buod: Error sa Paghawak ng Paradigms
| Paradigm | Diskarte | Mga Pros | Cons |
|----------|----------|------|------|
| **Exceptions** | ihagis/huli | Malinis na masayang landas, stack traces | Maaaring makalimutan, kontrolin ang pag-abuso sa daloy |
| **Mga uri ng resulta** | Ibalik ang`Ok`/`Err`| Malinaw, hindi maaaring balewalain ang mga error | Verbose, kailangan ng operator ng`?`|
| **Mga error code** | Ibalik ang katayuan sa int | Simple, C-compatible | Madaling balewalain, walang konteksto |
| **Maramihang pagbabalik** | `value, err := f()`| Lantad, idyomatiko | Maaaring kalimutang suriin ang`err`|
| **Pagtutugma ng pattern** | `{:ok, v}`/`{:error, r}`| Lubusan, gumagana | Verbose nang walang`with`|
| **Mga Kundisyon** | Restartable handlers | Pinakamakapangyarihan, mababawi | Kumplikado, bihirang maunawaan |