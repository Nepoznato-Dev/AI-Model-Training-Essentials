...
### Mga Operator ng Arithmetic at Paghahambing
| Operator | Pangalan | Halimbawa | Mga Tala |
|----------|------|---------|-------|
| `+``-``*``/``%`| Arithmetic | `a + b`| Kapareho ng C |
| `++``--` | Pagtaas/Pagbawas | `++i`,`i++`| Mas gusto ang pre-increment |
| `==``!=``<``>``<=``>=` | Paghahambing | `a <=> b`|  Ang`<=>`ay three-way na paghahambing (C++20) |
| `&&``\|\|``!`| Lohikal | `a && b`| Short-circuit |
| `&``\|``^``~``<<``>>` | Bitwise | `a & b`| Kapareho ng C |
| `?:`| Ternary | `cond ? a : b`| |
### C++-Mga Tukoy na Operator
| Operator | Pangalan | Halimbawa | Mga Tala |
|----------|------|---------|-------|
| `::`| Resolusyon sa saklaw | `std::cout`| |
| `.*``->*` | Pointer-sa-miyembro | `obj.*pmf`| |
| `<=>`| Spaceship (C++20) | `a <=> b`| Ibinabalik ang`strong_ordering`|
| `co_await`| Coroutine suspend | `co_await expr`| C++20 |
| `co_yield`| Coroutine yield | `co_yield value`| C++20 |
| `co_return`| Coroutine return | `co_return value`| C++20 |
...
## Kontrol ng Daloy
### Mga Structured Bindings at Modernong Pattern
```cpp
...
## Mga Template at Konsepto
### Mga Pangunahing Kaalaman sa Template
```cpp
// Function template
template<typename T>
...
## Mga Lalagyan at Algorithm ng STL
### Pangkalahatang-ideya ng Container
| Lalagyan | Header | Nag-order | Lookup | Mga Tala |
|-----------|--------|---------|--------|-------|
| `vector`| `<vector>`| Oo | O(n) | Default na pagpipilian para sa mga sequence |
| `deque`| `<deque>`| Oo | O(n) | Mabilis na push/pop sa magkabilang dulo |
| `list`| `<list>`| Oo | O(n) | Dobleng naka-link na listahan |
| `array`| `<array>`| Oo | O(n) | Fixed-size, stack-allocated |
| `set`/`multiset`| `<set>`| Oo | O(log n) | Pula-itim na puno |
| `map`/`multimap`| `<map>`| Oo (sa pamamagitan ng susi) | O(log n) | Key-value, pula-itim na puno |
| `unordered_set`| `<unordered_set>`| Hindi | O(1) avg | Hash table |
| `unordered_map`| `<unordered_map>`| Hindi | O(1) avg | Hash table |
...
## Buod
Ang C++ syntax ay sumasaklaw mula sa mababang antas ng pagmamanipula ng pointer hanggang sa mataas na antas na template metaprogramming at coroutine-based async. Ang ebolusyon ng wika sa pamamagitan ng C++17/20/23 ay kapansin-pansing nagpabuti ng kaligtasan (mga matalinong pointer,`std::optional`, mga konsepto), pagpapahayag (structured bindings, ranges,`std::format`), at performance (mga konseptong nagpapagana ng zero-cost abstraction, coroutine para sa async na walang stack overhead). Nananatiling hindi nagbabago ang pangunahing pilosopiya: hindi ka dapat magbayad para sa hindi mo ginagamit, at dapat suportahan ng wika ang bawat antas ng abstraction mula sa bare metal hanggang sa mga wikang partikular sa domain.