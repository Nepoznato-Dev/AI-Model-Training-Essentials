...
Kino-catalog ng dokumentong ito ang mga pinakakaraniwang pagkakamali, traps, at anti-pattern sa C++. Ang bawat entry ay nagpapakita ng maling diskarte, nagpapaliwanag kung bakit ito nabigo, at nagbibigay ng tamang solusyon. Ang modernong C++ (C++11/14/17/20) ay nagbibigay ng mga tool upang maiwasan ang karamihan sa mga klasikong pitfalls.
---

## 1. Tuntunin ng Tatlo/Limang Paglabag
```cpp
...
## 2. Raw`new`/`delete`Sa halip na mga Smart Pointer
```cpp
...
## 6. Hindi Paggamit ng`std::move`nang Tama
```cpp
...
## 7. Anti-Pattern: Paggamit ng`using namespace std;`sa Mga Header
```cpp
...
## 8. Hindi Natukoy na Gawi mula sa Signed Overflow
```cpp
...
## 9. Hindi Pag-unawa sa Mga Kategorya ng Halaga
```cpp
...
## 10. Anti-Pattern: Manual Resource Management (Pre-RAII)
```cpp
...
## 11. Template Bloat at Long Compile Times
```cpp
...
## 12. Hindi Paggamit ng`[[nodiscard]]`at `[[maybe_unused]]`
```cpp
...
## Buod
Ang pagiging kumplikado ng C++ ay lumilikha ng maraming pagkakataon para sa mga pagkakamali: ang Rule of Five, paghiwa ng bagay, invalidation ng iterator, mga nakabitin na sanggunian, at hindi natukoy na pag-uugali mula sa nilagdaang overflow. Ang makabagong C++ ay ang antidote: gumamit ng mga matalinong pointer sa halip na raw`new`/`delete`, RAII para sa lahat ng mapagkukunan,`std::vector`at`std::string`sa halip na mga raw array, ilipat ang mga semantika para sa kahusayan, at`[[nodiscard]]`upang mahuli ang mga binalewalang halaga ng pagbabalik. Umiiral ang C++ Core Guidelines para sa isang dahilan — sundin ang mga ito, at gumamit ng mga tool tulad ng Clang-Tidy at mga sanitizer para mahuli kung ano ang hindi mo nakuha.