...
---
# C++ — Mga Idiomatic Pattern at Pinakamahuhusay na Kasanayan
Sinasaklaw ng gabay na ito ang mga idiomatic pattern at pinakamahuhusay na kagawian para sa pagsusulat ng malinis, modernong C++ (20/23) code.
---

## RAII at Smart Pointer
```cpp
...
## Buod
Binibigyang-diin ng mga modernong idyoma ng C++ ang: RAII at matalinong mga pointer, ilipat ang mga semantika,`auto`para sa pagbabawas ng uri, mga hanay at algorithm (C++20), mga konsepto para sa mga hadlang sa template,`std::optional`/`std::expected`para sa kawalan/mga error, Rule of Zero, at XQZMARKER3-X compile. Sundin ang`clang-format`para sa pag-format,`clang-tidy`para sa linting, at i-compile gamit ang`-Wall -Wextra -Werror`. Pinahahalagahan ng komunidad ng C++ ang mga abstraction na walang halaga at "huwag magbayad para sa hindi mo ginagamit."