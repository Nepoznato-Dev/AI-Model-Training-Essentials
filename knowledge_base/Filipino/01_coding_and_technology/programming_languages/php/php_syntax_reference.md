...
Ang dokumentong ito ay nagbibigay ng isang komprehensibo, structured syntax reference para sa PHP (8.x). Kinukumpleto nito ang pangunahing sanggunian sa PHP sa pamamagitan ng pagtutok sa mga kumpletong pattern ng syntax, mga modernong tampok ng PHP, programming na nakatuon sa object, at mga praktikal na idyoma.
---
...
| `==``!=``===``!==` | Pagkakapantay-pantay | `$a === $b`|  Ang uri ng`===`ay masyadong |
| `<=>`| Spaceship | `$a <=> $b`| Nagbabalik -1, 0, o 1 |
| `<``>``<=``>=` | Paghahambing | `$a >= $b`| |
| `&&``\|\|``!`| Lohikal | `$a && $b`| Short-circuit |
| `and``or``xor`| Lohikal (mababa ang precedence) | `$a and $b`| Iwasan — gamitin ang`&&`/`\|\|`|
| `??`| Null coalescing | `$a ?? 'default'`| Ibinabalik ang`'default'`kung ang`$a`ay null |
| `??=`| Null coalescing assign | `$a ??= 'default'`| Itinalaga lamang kung ang`$a`ay null |
| `?``:` | Ternary | `$a ? $b : $c`| |
| `.`| Pagsasama-sama | `$a . $b`| |
| `.=`| Pinagsasama-samang italaga | `$a .= $b`| |
| `=>`| Array assignment | `['key' => 'value']`| |
| `->`| Access sa miyembro | `$obj->method()`| |
| `?->`| Nullsafe operator | `$obj?->method()`| Ibinabalik ang null kung ang`$obj`ay null |
| `::`| Static na pag-access | `ClassName::method()`| |
| `instanceof`| I-type ang check | `$obj instanceof ClassName`| |
| `clone`| Pag-clone ng bagay | `$copy = clone $obj`| Ang malalim na kopya ay nangangailangan ng`__clone()`|
### Uri ng Juggling
```php
...
## Buod
Ang syntax ng PHP ay kapansin-pansing nagbago mula sa mga pinagmulan ng script nito tungo sa isang moderno, ligtas sa uri ng wika. Ipinakilala ng PHP 8.x ang mga uri ng unyon, mga uri ng intersection, enum, fiber, readonly na klase, attribute, at first-class na mga callable — binago ito mula sa isang maluwag na nai-type na wika ng script tungo sa isang sumusuporta sa mahigpit na software engineering. Ang lakas ng wika ay nananatiling nakatuon sa web nito: na may mga balangkas tulad ng Laravel at Symfony, isang napakalaking ecosystem, at malapit sa unibersal na suporta sa pagho-host, ang syntax ng PHP ay nagsisilbi sa milyun-milyong aplikasyon. Ang modernong PHP ay nagbibigay ng gantimpala sa mga developer na yakapin ang uri ng system nito, gumamit ng composer-based na autoloading, at magsulat ng malinis, object-oriented na code.