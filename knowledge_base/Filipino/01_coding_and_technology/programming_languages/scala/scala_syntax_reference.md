...
|----------|------|---------|-------|
| `+``-``*``/``%`| Arithmetic | `a + b`| |
| `**`| Power (BigDecimal) | `BigDecimal(2) ** 10`| Hindi built-in para sa Int |
| `==``!=` | Pagkakapantay-pantay | `a == b`| Tumatawag sa`.equals()`— null-safe |
| `eq``ne` | Pagkakapantay-pantay ng sanggunian | `a eq b`| Parehong instance |
| `<``>``<=``>=` | Paghahambing | `a >= b`| Nangangailangan ng`Ordered`|
| `&&``\|\|``!`| Lohikal | `a && b`| Short-circuit |
| `&``\|``^``~` | Bitwise | `a & b`| |
| `<<``>>``>>>`| Shift | `a << 2`| |
| `+:``:+``::``:::` | Collection ops | `1 +: list`| Right-associative kung nagtatapos sa`:`|
| `->`| Paglikha ng tuple | `"key" -> "value"`| `("key", "value")`|
| `=>`| Function/arrow | `x => x + 1`| Lambda o uri ng pagmamapa |
| `<-`| Generator | `x <- List(1,2,3)`| Para sa pag-unawa |
### Mga Panuntunan ng Operator
```scala
...
## Buod
Pinagsasama ng syntax ng Scala ang object-oriented at functional na programming sa ilalim ng isang malakas na uri ng sistema. Ang pagtutugma ng pattern, para sa pag-unawa, at mga klase ng kaso ay nagbibigay ng nagpapahayag na pagmomodelo ng data. Ang mga katangian ay nagbibigay-daan sa flexible na komposisyon nang walang mga limitasyon sa single-inheritance. Ang type system — na may mga generic, variance, type bounds, at opaque na mga uri — ay nakakakuha ng mga error sa oras ng pag-compile habang pinapagana ang zero-cost abstraction. Pinapasimple ng Scala 3 ang wika gamit ang mga enum, paraan ng extension, ibinigay/gamit, at mga opaque na uri, na ginagawang mas madaling lapitan ang wika habang pinapanatili ang buong kapangyarihan nito.