...
|----------|------|---------|-------|
| `+``-``*``/``^`| Arithmetic | `A * B`| Matrix multiply |
| `.*``./``.^`| Element-wise | `A .* B`| Array operation |
| `'`| Ilipat | `A'`| Conjugate transpose |
| `.'`| Non-conjugate transpose | `A.'`| |
| `\`| Kaliwang hatiin | `A \ b`| Lutasin ang Ax = b |
| `/`| Kanang hatiin | `b / A`| Lutasin ang xA = b |
| `==``~=` | Pagkakapantay-pantay | `A == B`| Element-wise |
| `<``>``<=``>=` | Paghahambing | `A > 0`| Element-wise |
| `&``\|``~`| Lohikal (element-wise) | `A & B`| |
| `&&``\|\|` | Short-circuit | `a && b`| Scalar lamang |
| `:`| Tutuldok/saklaw | `1:10`| `start:step:stop`|
| `;`| Row separator / suppress | `[1; 2; 3]`| |
| `,`| Column separator | `[1, 2, 3]`| |
---
...
## Buod
Ang syntax ng MATLAB ay matrix-first — bawat variable ay isang array, at ang mga operasyon ay na-optimize para sa linear algebra. Ang colon operator, logical indexing, at vectorized na mga operasyon ay nag-aalis ng mga loop para sa karamihan ng mga numerical na gawain. Ang sistema ng pag-plot ay gumagawa ng mga numero ng kalidad ng publikasyon na may kaunting code. Pinapalawak ng mga toolbox ang MATLAB sa bawat domain ng engineering. Habang ang Python ay nakapasok sa ilang teritoryo, ang MATLAB ay nananatiling pamantayan para sa engineering computation, Simulink-based modeling, at mabilis na prototyping.