...
|----------|------|---------|-------|
| `+`| Dagdag | `3 + 2`| Gayundin ang`String`concatenation sa pamamagitan ng`format!`|
| `-`| Pagbabawas | `3 - 2`| |
| `*`| Multiplikasyon | `3 * 2`| |
| `/`| Dibisyon | `7 / 2`| Integer division para sa mga integer:`3`|
| `%`| Natitira | `7 % 2`| `1`|
| `-x`| Negasyon | `-5`| |
### Paghahambing at Lohikal na Operator
| Operator | Pangalan | Halimbawa | Mga Tala |
|----------|------|---------|-------|
| `==`| Pantay | `x == y`| Nangangailangan ng`PartialEq`|
| `!=`| Hindi Katumbas | `x != y`| |
| `<`,`>`,`<=`,`>=`| Pag-order | `x >= y`| Nangangailangan ng`PartialOrd`|
| `&&`| Lohikal AT | `a && b`| Short-circuit |
| `\|\|`| Lohikal O | `a \|\| b`| Short-circuit |
| `!`| Lohikal HINDI | `!true`| |
...
| `\|`| O | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `!`| HINDI | `!0b1010u8`| Bitwise complement |
| `<<`| Kaliwa Shift | `5 << 1`| `10`|
| `>>`| Pakanan Shift | `5 >> 1`| `2`|
### Operator Precedence (pinakamataas hanggang pinakamababa)
| Pangunahin | Mga Operator | Mga Tala |
|------------|-----------|-------|
| 1 (pinakamataas) | `()`path`.`:: | |
| 2 | Mga tawag sa pamamaraan, pag-access sa field | Auto-deref |
| 3 | `-`(negation),`!`,`*`(deref) | |
| 4 | `as`(uri ng cast) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
| 13 | `\|\|`| |
| 14 | `..``..=` | Saklaw |
| 15 | `=``+=` atbp. | Takdang-aralin |
| 16 (pinakamababa) | `return``break``continue`| |
---

## Kontrol ng Daloy
### Mga Conditional Expression
```rust
// if / else if / else
...
### Pangkalahatang-ideya ng Mga Built-in na Uri
| Uri | Stack/Heap | Sukat | Pagbabago | Mga Tala |
|------|-----------|------|------------|-------|
| Primitives (`i32`,`f64`,`bool`,`char`) | Stack | Naayos | Hindi nababago bilang default | `Copy`|
| `[T; N]`(array) | Stack | Naayos | Nababago sa`mut`| Fixed-size,`Copy`kung ang T ay`Copy`|
| `Vec<T>`| Bunton | Dynamic | Nababago sa`mut`| Growable |
| `String`| Bunton | Dynamic | Nababago sa`mut`| UTF-8, maaaring lumaki |
| `&[T]`(hiwain) | Stack (fat ptr) | Dynamic na view | Depende sa`&`o`&mut`| Hiram na view |
| `HashMap<K, V>`| Bunton | Dynamic | Nababago sa`mut`| O(1) avg lookup |
| `BTreeMap<K, V>`| Bunton | Dynamic | Nababago sa`mut`| O(log n) lookup, inutusan |
| `HashSet<T>`| Bunton | Dynamic | — | O(1) avg, walang mga duplicate |
### Mga Vector
```rust
// Creation
...
### Mga Enum at Pagtutugma ng Pattern
```rust
// Basic enum
enum Direction {
    Up,
...
### Kahulugan at Pagpapatupad ng Trait
```rust
// Define a trait
trait Summary {
...
### Mga Karaniwang Katangian ng Aklatan
```rust
...
## Error sa Paghawak
### Mga Pattern ng Resulta at Pagpipilian
```rust
...
## Kasabay
### Mga Thread at Pagpasa ng Mensahe
```rust
use std::thread;
...
## Buod
Ang syntax ng Rust ay binuo sa pundasyon ng mga garantiyang pangkaligtasan na ipinapatupad sa oras ng pag-compile. Ang sistema ng pagmamay-ari, polymorphism na nakabatay sa katangian, pagtutugma ng pattern, at mga abstract na walang gastos ay bumubuo ng magkakaugnay na disenyo kung saan ang compiler ay isang mahigpit ngunit patas na kasosyo. Mula sa pangunahing daloy ng kontrol sa pamamagitan ng mga advanced na feature tulad ng mga macro, hindi ligtas na code, at mga matalinong pointer, ang bawat construct ay nagsisilbi sa layunin ng ligtas, mahusay na programming ng system. Ang pag-unawa sa interplay sa pagitan ng mga habambuhay, paghiram, at ang uri ng sistema ay ang susi sa pag-master ng Rust.