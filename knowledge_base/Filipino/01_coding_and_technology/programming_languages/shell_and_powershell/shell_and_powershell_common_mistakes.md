...
-->
# Shell at PowerShell — Mga Karaniwang Pagkakamali at Anti-Pattern
Kino-catalog ng dokumentong ito ang mga pinakakaraniwang pagkakamali, traps, at anti-pattern sa Shell/Bash at PowerShell na may mga pagwawasto.
---

## 1. Mga Hindi Naka-quote na Variable (Bash)
```bash
...
## 4. Hindi Sinusuri ang Mga Exit Code (Bash)
```bash
...
## 5. PowerShell: String Interpolation
```powershell
...
## 7. Saklaw ng Variable ng Subshell (Bash)
```bash
...
## 8. PowerShell: Pipeline Objects vs Strings
```powershell
...
## Buod
Shell scripting traps: palaging mag-quote ng mga variable sa Bash, gamitin ang`[[ ]]`sa`[ ]`, gamitin ang`set -euo pipefail`, tingnan ang mga exit code, at unawain ang subshell scoping. PowerShell traps: gumamit ng double quotes para sa interpolation, gumamit ng`-eq`hindi`==`, at tandaan na may mga katangian ang mga pipeline object. Parehong nagbibigay ng gantimpala sa maingat na pagsipi at tahasang paghawak ng error.