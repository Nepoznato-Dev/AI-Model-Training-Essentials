...
---
# Java — Mga Karaniwang Pagkakamali at Anti-Pattern
Kino-catalog ng dokumentong ito ang mga pinakakaraniwang pagkakamali, traps, at anti-pattern sa Java. Ang bawat entry ay nagpapakita ng maling diskarte, nagpapaliwanag kung bakit ito nabigo, at nagbibigay ng tamang solusyon.
---

## 1.`equals()`vs`==`para sa Mga Bagay
```java
...
## 2. Pagbabago ng Koleksyon sa Pag-ulit
```java
...
## 3. Hindi Pagsasara ng Mga Mapagkukunan (Try-with-Resources)
```java
...
## 6. String Concatenation sa Loops
```java
...
## 7. Hindi Pangasiwaan ang InterruptedException
```java
...
## 8. Pagbabalik ng Null Sa halip na Mga Walang Lamang Koleksyon
```java
...
## 9. Mga Nababagong Bagay bilang HashMap Keys
```java
...
## 10. Anti-Pattern: Paghuli sa`Exception`o `Throwable`
```java
...
## 11. Hindi Overriding`hashCode`Kapag Overriding `equals`
```java
...
## 12. Mga Kundisyon ng Lahi na may Double-Checked Locking
```java
...
## Buod
Maaaring itago ng verbosity ng Java ang mga banayad na bug:`==`vs`equals()`, sabay-sabay na pagbabago, paglabas ng mapagkukunan, mga bitag sa pagganap ng autoboxing, at ang katumbas/hashCode na kontrata. Ang modernong diskarte sa Java ay: gumamit ng`Objects.equals()`para sa null-safe na paghahambing, palaging gumamit ng try-with-resources, ibalik ang mga walang laman na koleksyon sa halip na null, kumuha ng mga partikular na eksepsiyon, at sundin ang iisang prinsipyo ng responsibilidad. Ang mga feature ng Java 8+ (mga stream, Opsyonal, mga sanggunian sa pamamaraan) ay nagpapababa ng boilerplate habang ginagawang mas ligtas at mas malinaw ang code.