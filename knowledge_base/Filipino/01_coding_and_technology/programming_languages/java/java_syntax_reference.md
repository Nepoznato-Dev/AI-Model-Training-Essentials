...
| `/`| Dibisyon | `7 / 2`| `3`| Integer division para sa`int`; `3.5`para sa`double`|
| `%`| Natitira | `7 % 2`| `1`| |
...
|----------|------|---------|-------|
| `==`| Pantay | `x == y`| Reference equality para sa mga object |
| `!=`| Hindi Katumbas | `x != y`| |
| `<`,`>`,`<=`,`>=`| Relational | `x >= y`| |
| `&&`| Lohikal AT | `a && b`| Short-circuit |
| `\|\|`| Lohikal O | `a \|\| b`| Short-circuit |
| `!`| Lohikal HINDI | `!true`| `false`|
| `instanceof`| I-type ang Check | `obj instanceof String`| Gayundin ang pagtutugma ng pattern (Java 16+) |
...
|------------|-----------|
| 1 (pinakamataas) | Postfix:`expr++``expr--` |
...
### Mga Lambdas at Functional na Interface
```java
// Lambda syntax
...
| Interface | Mga Pagpapatupad | Nag-order | Variant na Ligtas sa Thread |
|------------|----------------|---------|---------------------|
| `List`| `ArrayList`,`LinkedList`| Oo | `CopyOnWriteArrayList`|
| `Set`| `HashSet`,`TreeSet`,`LinkedHashSet`| Nag-iiba | `CopyOnWriteArraySet`|
| `Map`| `HashMap`,`TreeMap`,`LinkedHashMap`| Nag-iiba | `ConcurrentHashMap`|
| `Queue`| `ArrayDeque`,`PriorityQueue`| Nag-iiba | `ConcurrentLinkedQueue`|
| `Deque`| `ArrayDeque`| Parehong dulo | `ConcurrentLinkedDeque`|
### Mga listahan
```java
// Creation
...
## Buod
Ang syntax ng Java ay kapansin-pansing nagbago mula sa mga pinagmulan nito noong 1995. Nagtatampok ang modernong Java (21+) ng mga talaan, mga selyadong klase, pagtutugma ng pattern, mga virtual na thread, at mga template ng string — ginagawa itong mas maigsi at nagpapahayag kaysa dati. Ang pangunahing syntax ay nananatiling pamilyar (C-like, malakas na na-type, class-based), ngunit ang mga pagdaragdag ng lambdas, Stream API, at`var`ay makabuluhang nabawasan ang boilerplate. Ang lakas ng wika ay nakasalalay sa napakalaking ecosystem nito, pabalik na mga garantiya ng compatibility, at ang pag-optimize ng performance ng JVM sa pamamagitan ng JIT compilation.