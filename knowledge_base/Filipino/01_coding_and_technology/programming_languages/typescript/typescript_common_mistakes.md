...
-->
# TypeScript — Mga Karaniwang Pagkakamali at Anti-Pattern
Ang dokumentong ito ay nag-catalog ng mga pinakakaraniwang pagkakamali, traps, at anti-pattern sa TypeScript. Ang bawat entry ay nagpapakita ng maling diskarte, nagpapaliwanag kung bakit ito nabigo, at nagbibigay ng tamang solusyon.
---

## 1. Sobrang paggamit ng `any`
```typescript
...
## 2. I-type ang mga Assertion sa halip na Type Guards
```typescript
...
## 4. Mga Interface kumpara sa Mga Uri ng Pagkalito
```typescript
...
## 6. Hindi Gumagamit ng`strict`Mode
```json
...
## 7. Mga Structural Typing Sorpresa
```typescript
...
## 9. Anti-Pattern: Mga Umuulit na Uri (Hindi Gumagamit ng Generics)
```typescript
...
## 10. Labis na Opsyonal na Chaining
```typescript
...
## 11. Paggamit ng`namespace`(Legacy)
```typescript
...
## 12. Hindi Gumagamit ng mga Nadidiskriminang Unyon
```typescript
...
## Buod
Ang sistema ng uri ng TypeScript ay makapangyarihan ngunit nangangailangan ng disiplina. Ang kardinal na kasalanan ay umaabot sa`any`— gamitin ang`unknown`kapag ang uri ay talagang hindi kilala, at makitid na may mga uri ng bantay. Iwasan ang mga di-null assertion (`!`) sa pamamagitan ng paghawak ng hindi natukoy nang maayos. Gumamit ng mga interface para sa mga hugis ng bagay, mga uri para sa mga unyon at mga nakalkulang uri. Tinatanggal ng mga diskriminasyong unyon ang buong klase ng mga bug. I-enable ang`strict: true`mula sa unang araw — masakit ang pagsasaayos ng pagiging mahigpit. Ang layunin ay gawing hindi kinakatawan ang mga iligal na estado sa antas ng uri.