...
## Uri ng System Fundamentals
### Primitive at Built-in na Uri
```typescript
// Primitives
...
### Mga Interface at Uri ng Aliases
```typescript
...
## Generics
### Mga Generic na Function at Klase
```typescript
// Generic function
...
## Mga Uri ng Utility
### Mga Built-in na Uri ng Utility
```typescript
interface User {
  id: string;
  name: string;
...
## Mga Dekorador (TC39 Stage 3 / TypeScript 5.0+)
```typescript
// Class decorator
...
### Mga Literal na Uri ng Template
```typescript
...
## Buod
Ang type system ng TypeScript ay ang tampok na pagtukoy nito, at ito ay kapansin-pansing malalim. Mula sa mga pangunahing anotasyon hanggang sa mga generic, mga uri ng kondisyon, mga naka-mapa na uri, mga literal na uri ng template, at`infer`, ang uri ng system ay mahalagang isang functional na wika na tumatakbo sa oras ng pag-compile. Ang pag-unawa sa interplay sa pagitan ng`interface`vs`type`, generics vs mga uri ng utility, at structural vs nominal typing ay sumusuporta sa pagsulat ng TypeScript na parehong ligtas at ergonomic. Ang wika ay patuloy na nagbabago, sa bawat paglabas ay nagdaragdag ng higit na nagpapahayag na kapangyarihan habang pinapanatili ang ganap na backward compatibility sa JavaScript.