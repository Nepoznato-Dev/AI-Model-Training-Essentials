...
---
# JavaScript — Mga Karaniwang Pagkakamali at Anti-Pattern
Ang dokumentong ito ay katalogo ng mga pinakakaraniwang pagkakamali, traps, at anti-pattern sa JavaScript. Ang bawat entry ay nagpapakita ng maling diskarte, nagpapaliwanag kung bakit ito nabigo, at nagbibigay ng tamang solusyon.
---

## 1.`this`Nagbubuklod na Pagkalito
```javascript
...
## 3.`var`Mga Isyu sa Pagtaas at Saklaw
```javascript
...
## 5. Hindi Pangasiwaan ang mga Pagtanggi sa Pangako
```javascript
...
## 6. Pagbabago ng Mga Bagay sa Panahon ng Pag-ulit
```javascript
...
## 8. Hindi pagkakaunawaan `typeof null`
```javascript
...
## 9. Ang Mga Bagay na Parang Array ay Hindi Array
```javascript
...
## 10. Anti-Pattern: Mutating Function Parameter
```javascript
...
## 11. Hindi Pag-unawa sa Event Loop at Microtasks
```javascript
...
## 12. Anti-Pattern: Gamit ang `eval()`
```javascript
...
## 13. Nakakalimutang Linisin ang Mga Tagapakinig ng Kaganapan
```javascript
// ❌ WRONG — memory leak
class Component {
  mount() {
...
## Buod
Ang flexibility ng JavaScript ay may kasamang mga bitag: Ang`this`binding ay depende sa call site, ang`==`ay nagsasagawa ng pamimilit, ang mga pangako ay nangangailangan ng paghawak ng error, at ang event loop ordering (microtasks bago ang macrotasks) ay hindi halata. Ang mga modernong panuntunan ay simple: gamitin ang`const`/`let`hindi kailanman`var`, mas gusto ang`===`kaysa`==`, palaging pangasiwaan ang mga pagtanggi sa pangako, gumamit ng mga arrow function para sa mga callback, iwasan ang mutation, at linisin ang mga nakikinig sa kaganapan. Ang pagsunod sa mga panuntunang ito ay nag-aalis ng karamihan sa mga bug ng JavaScript.