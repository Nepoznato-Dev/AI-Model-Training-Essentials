...
Ang dokumentong ito ay nagbibigay ng komprehensibo, structured na syntax na sanggunian para sa JavaScript. Kinukumpleto nito ang pangunahing sanggunian ng JavaScript sa pamamagitan ng pagtutok sa mga kumpletong pattern ng syntax, mga talahanayan ng operator, at panloob na mekanika.
---
...
|----------|------|---------|--------|-------|
| `+`| Dagdag | `3 + 2`| `5`| Gayundin string concatenation |
| `-`| Pagbabawas | `3 - 2`| `1`| |
| `*`| Multiplikasyon | `3 * 2`| `6`| |
| `/`| Dibisyon | `7 / 2`| `3.5`| |
| `%`| Natitira | `7 % 2`| `1`| |
| `**`| Exponentiation | `2 ** 10`| `1024`| ES2016 |
| `++`| Pagtaas | `x++`/`++x`| | Post- at pre-increment |
| `--`| Pagbawas | `x--`/`--x`| | Post- at pre-decrement |
| `+x`,`-x`| Unary | `+"42"`→`42`| I-type ang pamimilit sa numero |
### Paghahambing at Pagkakapantay-pantay
| Operator | Pangalan | Halimbawa | Mga Tala |
|----------|------|---------|-------|
| `===`| Mahigpit na Pagkakapantay-pantay | `1 === 1`| Walang uri ng pamimilit — palaging mas gusto ito |
| `!==`| Mahigpit na Hindi pagkakapantay-pantay | `1 !== "1"`| `true`— iba't ibang uri |
| `==`| Abstract Equality | `1 == "1"`| Uri ng pamimilit — iwasan |
| `!=`| Abstract Inequality | `1 != "1"`| `false`— iwasan ang |
| `<`,`>`,`<=`,`>=`| Relational | `"b" > "a"`| Lexicographic para sa mga string |
### Logical at Nullish Operator
| Operator | Pangalan | Halimbawa | Mga Tala |
|----------|------|---------|-------|
| `&&`| Lohikal AT | `a && b`| Ibinabalik ang unang huwad o huling halaga |
| `\|\|`| Lohikal O | `a \|\| b`| Ibinabalik ang unang truthy o huling halaga |
| `!`| Lohikal HINDI | `!true`| `false`|
| `??`| Nullish Coalescing | `a ?? b`| Ibinabalik lamang ang`b`kung ang`a`ay`null`/`undefined`|
| `?.`| Opsyonal na Chaining | `obj?.prop?.method()`| Mga short-circuit sa`undefined`|
...
| `~`| HINDI | `~5`| `-6`|
| `<<`| Kaliwa Shift | `5 << 1`| `10`|
| `>>`| Pinirmahan ng Right Shift | `-5 >> 1`| `-3`|
| `>>>`| Unsigned Right Shift | `-1 >>> 0`| `4294967295`|
### Pagtatalaga at Pagsira
```javascript
// Basic assignment
let x = 10;
...
|------------|-----------|--------------|
| 1 (pinakamataas) | `()``[]``.``?.` | Kaliwa |
| 2 | `new`(walang args) | Kanan |
| 3 | `++``--` (postfix) | N/A |
| 4 | `!``~``+x``-x``typeof``void``delete``++``--`(prefix) | Kanan |
| 5 | `**`| Kanan |
| 6 | `*``/``%`| Kaliwa |
| 7 | `+``-` | Kaliwa |
| 8 | `<<``>>``>>>`| Kaliwa |
| 9 | `<``<=``>``>=``in``instanceof` | Kaliwa |
| 10 | `===``!==``==``!=` | Kaliwa |
| 11 | `&`| Kaliwa |
| 12 | `^`| Kaliwa |
| 13 | `\|`| Kaliwa |
...
| 16 | `??`| Kaliwa |
| 17 | `=``+=``-=`atbp. | Kanan |
| 18 (pinakamababa) | `=>`(arrow function) | Kanan |
---
...
## Mga Pag-andar
### Mga Deklarasyon at Ekspresyon ng Function
```javascript
...
| Koleksyon | Nag-order | Nababago | Mga Duplicate | Uri ng Key | Lookup |
|------------|---------|---------|------------|----------|--------|
| `Array`| Oo | Oo | Oo | Index (numero) | O(n) |
| `Object`| Oo (insertion) | Oo | Mga Susi: Hindi | String/Simbolo | O(1) avg |
| `Map`| Oo (insertion) | Oo | Mga Susi: Hindi | Anumang | O(1) avg |
| `Set`| Oo (insertion) | Oo | Hindi | — | O(1) avg |
| `WeakMap`| Hindi | Oo | Mga Susi: Hindi | Bagay lamang | O(1) avg |
| `WeakSet`| Hindi | Oo | Hindi | Bagay lamang | O(1) avg |
### Mga array
```javascript
// Creation
...
### WeakRef at FinalizationRegistry
```javascript
...
## Buod
Ang syntax ng JavaScript ay mapanlinlang na simple sa ibabaw ngunit naglalaman ng makabuluhang lalim. Ang wika ay umunlad mula sa isang simpleng tool sa pag-script tungo sa isang multi-paradigm na wika na may mga klase, module, generator, proxy, at isang sopistikadong modelo ng async. Ang pag-unawa sa precedence ng operator, ang event loop, mga prototype na chain, at ang mga nuances ng`this`binding ay naghihiwalay sa mga karampatang developer mula sa mga eksperto. Ang modernong JavaScript (ES2020+) na may opsyonal na chaining, nullish coalescing,`Promise.allSettled`, at top-level na paghihintay ay patuloy na binabawasan ang boilerplate habang pinapanatili ang backward compatibility.