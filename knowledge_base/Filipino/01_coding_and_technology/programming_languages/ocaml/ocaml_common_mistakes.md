...
## 4. Hindi Paggamit ng`Option`nang Tama
```ocaml
...
## 5. Mutable State sa Recursive Function
```ocaml
...
## Buod
OCaml traps: hindi kumpletong pagtutugma ng pattern (nagbabala ang compiler),`=`ay structural equality habang ang`==`ay pisikal, integer division truncates, gamitin ang`Option`sa halip na mga exception, at iwasan ang nababagong estado na pabor sa tail recursion at fold. Ang compiler ng OCaml ay ang iyong kaalyado — pakinggan ang mga babala nito.