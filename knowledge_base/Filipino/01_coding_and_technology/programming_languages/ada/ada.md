...
## Bakit Mahalaga si Ada
- **Mga sistemang kritikal sa kaligtasan**: Idinisenyo mula sa simula para sa mga system kung saan ang pagkabigo ay hindi isang opsyon.
- **Malakas na pagta-type**: Ang pinakamahigpit na uri ng sistema ng anumang pangunahing wika — nakakakuha ng mga error sa oras ng pag-compile na hindi nakuha ng ibang mga wika.
- **Built-in concurrency**: Ang tasking (kasabay na programming) ay bahagi ng wika, hindi isang library.
- **Pormal na pag-verify**: Sinusuportahan ang mga pormal na pamamaraan para sa pagpapatunay ng katumpakan ng programa.
- **Mga feature ng pagiging maaasahan**: Mga built-in na exception, kontrata (pre/postconditions), at runtime checks.
- **Modernong Ada**: Ang Ada 2012 at Ada 2022 ay nagdagdag ng mga modernong feature habang pinapanatili ang mga garantiya sa kaligtasan.
## Ang mga Trade-off
| Limitasyon | Mga Detalye | Karaniwang Workaround |
|-----------|---------|-------------------|
| **Niche community** | Maliit na developer base kumpara sa C, Java, o Python | Espesyalista ngunit may kaalaman sa komunidad |
| **Verbose syntax** | Higit pang verbose kaysa sa C o Python | Tanggapin bilang bahagi ng disenyong nakatuon sa kaligtasan |
| **Limitadong ecosystem** | Mas kaunting mga aklatan kaysa sa mga pangunahing wika | Sumulat ng pasadyang code; gamitin ang malawak na karaniwang aklatan |
| **Learning curve** | Kumplikadong wika na may maraming tampok sa kaligtasan | Magsimula sa subset ng "SPARK" ni Ada para sa gawaing kritikal sa kaligtasan |
| **Pagiging available ng compiler** | Mas kaunting mga pagpipilian sa compiler (GNAT ang pangunahing isa) | Ang GNAT ay libre at maayos na pinananatili |
---
...
### Mga Kontrata at Aspekto (Ada 2012)
```ada
...
### Mga Protektadong Bagay (Thread-Safe Shared Data)
```ada
...
## Interoperability
### C Interoperability (Mga Interface)
```ada
with Interfaces.C;
...
### Pattern 1: Tagamasid na may Mga Protektadong Bagay
```ada
...
### Pattern 2: State Machine na may Nadidiskriminang Uri
```ada
...
### Pattern 3: RAII na may Mga Kontroladong Uri
```ada
...
## Deployment
### SPARK para sa Safety-Critical Certification
```ada
...
## Kailan Gamitin ang Ada
| Sitwasyon | Bakit Ada | Mas mahusay na Alternatibo |
|----------|---------|-------------------|
| Mga sistemang kritikal sa kaligtasan | Idinisenyo para dito; suporta sa pormal na pagpapatunay | — |
| Aviation / aerospace | Mga sertipikadong compiler; Pagsunod sa DO-178C | — |
| Mga sistema ng pagtatanggol | pamana ng DoD; mga tampok ng seguridad | — |
| Riles / medikal na kagamitan | Mga kinakailangan sa mataas na integridad | — |
| Pangkalahatang pag-unlad ng application | Overkill para sa mga hindi kritikal na sistema | Python, Java, Go |
| Pagbuo ng web | Hindi angkop | JavaScript, Python |
| Data science / ML | Hindi ang ecosystem | Python, R |
---

## Synthetic na Q&A
### Q1: Paano pinipigilan ng uri ng system ni Ada ang mga bug sa oras ng pag-compile?
**A:** Ang uri ng sistema ni Ada ay kabilang sa pinakamahigpit sa anumang wika. Nakakakuha ito ng mga error na hindi nakuha ng ibang mga wika:
```ada
...
### Q2: Ano ang tasking model ni Ada at paano ito kumpara sa iba pang concurrency na modelo?
**A:** May built-in na concurrency si Ada sa mga protektadong bagay at gawain:
```ada
...
### Q3: Paano ko gagamitin ang mga generic sa Ada?
**A:** Ang mga generic ng Ada ay tahasan at ligtas sa uri:
```ada
generic
   type Element_Type is private;
...
### Q4: Ano ang dahilan kung bakit angkop ang Ada para sa mga sistemang kritikal sa kaligtasan?
**A:** Nagbibigay si Ada ng:
- SPARK subset para sa pormal na pag-verify (mathematical proof of correctness)
- Contract-based programming (pre/postconditions, type invariants)
- Walang implicit na paglalaan ng memorya sa SPARK
- Deterministic tasking at scheduling
- Ravenscar profile para sa high-integrity real-time system
- Kwalipikasyon ng Toolchain (DO-178C para sa avionics)
### Q5: Paano ako bubuo ng mga proyekto ng Ada?
**A:** Gamitin ang GPRBuild sa mga file ng proyekto ng GPR:
```bash
gprbuild -P my_project.gpr
gprclean -P my_project.gpr
```
...
### Problema 1: Pagpapatupad ng Type-Safe Queue
**Hakbang 1: Unawain ang Problema**
Gumawa ng bounded, thread-safe na queue na may pagsuri sa laki ng compile-time.
**Hakbang 2: Tukuyin ang Diskarte**
Gumamit ng protektadong bagay na may bounded buffer.
**Hakbang 3: Ipatupad**```ada
...
**Hakbang 4: I-verify**
Ginagarantiyahan ng protektadong bagay ang kapwa pagbubukod. Pinipigilan ng mga hadlang sa pagpasok ang overflow/underflow.
### Problema 2: Contract-Based Validation
**Hakbang 1: Unawain ang Problema**
Magpatupad ng square root function na may mga pormal na kontrata.
**Hakbang 2: Tukuyin ang Diskarte**
Gamitin ang mga kontrata ng Ada 2012 (pre/postconditions).
**Hakbang 3: Ipatupad**```ada
...
**Hakbang 4: I-verify**
Ang runtime checks (assertions) ay nakakakuha ng mga paglabag. Sa SPARK, ang mga ito ay nagiging patunay na obligasyon.
---

## Buod
Ang Ada ay isang wikang binuo para sa tama. Ang mahigpit nitong uri ng system, built-in na concurrency, at pormal na suporta sa pag-verify ay ginagawa itong pagpipilian para sa mga system kung saan ang pagkabigo ay hindi katanggap-tanggap. Bagama't maliit ang komunidad nito kumpara sa mga pangunahing wika, nananatiling mahalaga ang Ada sa aviation, defense, space, at iba pang mga domain na kritikal sa kaligtasan. Para sa mga application na ito, ang mahigpit na diskarte ni Ada sa software engineering ay hindi isang limitasyon — ito ang punto.