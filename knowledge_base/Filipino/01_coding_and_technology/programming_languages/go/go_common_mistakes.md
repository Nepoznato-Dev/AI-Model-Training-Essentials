...
# Go — Mga Karaniwang Pagkakamali at Anti-Pattern
Ang dokumentong ito ay katalogo ng mga pinakakaraniwang pagkakamali, traps, at anti-pattern sa Go. Ang bawat entry ay nagpapakita ng maling diskarte, nagpapaliwanag kung bakit ito nabigo, at nagbibigay ng tamang solusyon.
---

## 1. Loop Variable Capture sa Goroutines
```go
...
## 2. Pagbabalewala sa Error Returns
```go
...
## 5. Nil Pointer Dereference sa Struct Methods
```go
...
## 6. Paggamit ng`defer`sa isang Loop
```go
...
## 7. Anti-Pattern: Pagbabalik ng Pinangalanang Variable nang maaga
```go
...
## 8. Random ang Order ng Pag-ulit ng Mapa
```go
...
## 10. Anti-Pattern: Interface Polusyon
```go
...
## 11. Hindi Paggamit ng`sync.Once`para sa Initialization
```go
...
## 12. Pagkopya ng mga Uri ng `sync`
```go
...
## Buod
Ang pagiging simple ni Go ay ang lakas nito, ngunit mayroon itong banayad na mga bitag: mga pagtagas ng goroutine mula sa mga hindi na-buffer na channel, mga semantikong dugtungan ng slice na hindi nagbabago sa orihinal, mga ipinagpaliban na tawag na naipon sa mga loop, at pag-ulit ng mapa na sadyang randomized. Ang mga kawikaan ng Go ay gumagabay sa paraan: hawakan ang bawat error, tanggapin ang mga interface ngunit ibalik ang mga istruktura, gumamit ng mga pointer receiver para sa mga uri ng mutex, at mas gusto ang concurrency primitives (`sync.Once`,`errgroup`, pagkansela ng konteksto) kaysa sa manu-manong pamamahala ng goroutine. Sumulat muna ng simpleng code — Gantimpalaan ng Go ang kalinawan kaysa katalinuhan.