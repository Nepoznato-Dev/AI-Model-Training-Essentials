...
Ang dokumentong ito ay nag-catalog ng mga pinakakaraniwang pagkakamali, traps, at anti-pattern sa C#. Ang bawat entry ay nagpapakita ng maling diskarte, nagpapaliwanag kung bakit ito nabigo, at nagbibigay ng tamang solusyon.
---

## 1. Hindi Pagtatapon ng`IDisposable`Objects
```csharp
// ❌ WRONG — resource leak
...
## 4. LINQ`.ToList()`Hindi Kinakailangang Paglalaan
```csharp
...
## 5. Pagkuha ng Loop Variable (Pre-C# 5.0)
```csharp
...
## 6. Hindi Paggamit ng Pattern Matching
```csharp
...
## 7. Anti-Pattern: Mga Klase ng Diyos / Tagahanap ng Serbisyo
```csharp
...
## 10. Hindi Paggamit ng Mga Tala para sa Mga Uri ng Data
```csharp
...
## Buod
Ang C# ay isang wikang mahusay na idinisenyo ngunit may mga bitag: hindi nagtatapon ng mga bagay na `IDisposable`, mga pamamaraan ng async na walang bisa, mga hindi kinakailangang paglalaan ng LINQ, at mga null reference exception. Ang modernong C# (8+) ay nagbibigay ng makapangyarihang mga tool: nullable reference type, pattern matching, records, gamit ang mga deklarasyon, at switch expression. Ang mga pangunahing prinsipyo: palaging nagtatapon ng mga mapagkukunan, huwag gumamit ng async void (maliban sa mga tagapangasiwa ng kaganapan), gumamit ng constructor injection, mas gusto ang mga talaan para sa data, at paganahin ang mga nullable na uri ng sanggunian upang mahuli ang mga null na bug sa oras ng pag-compile.