...
## Mga Mapa / Mga Diksyunaryo / Hash Table
```python
# Python: dict
...
| kalawang | `let s: HashSet<i32> = [1, 2, 3].into()`|
| Pumunta | `s := map[int]bool{1: true, 2: true, 3: true}`(walang built-in na Set) |
| Java | `Set<Integer> s = Set.of(1, 2, 3)`|
| C++ | `std::set<int> s = {1, 2, 3}`|
| C# | `var s = new HashSet<int> { 1, 2, 3 }`|
| Ruby | `s = Set[1, 2, 3]`|
| matulin | `var s: Set<Int> = [1, 2, 3]`|
| Kotlin | `val s = setOf(1, 2, 3)`|
| Scala | `val s = Set(1, 2, 3)`|
| Dart | `var s = {1, 2, 3}`|
| Julia | `s = Set([1, 2, 3])`|
| R | `s <- c(1, 2, 3)`(walang katutubong hanay; gamitin ang`%in%`) |
## Mga Struct / Mga Tala / Mga Klase
```python
...
| Wika | Syntax |
|----------|--------|
| kalawang | `enum Color { Red, Green, Blue }`|
| matulin | `enum Color { case red, green, blue }`|
...
| C# | `enum Color { Red, Green, Blue }`|
| Pumunta | `const ( Red = iota; Green; Blue )`(walang katutubong enum) |
...
| Ruby | Walang katutubong enum (gumamit ng frozen constants o`dry-enum`) |
## Null Safety
| Wika | Null Handling |
|----------|--------------|
| Python | `None`, walang compile-time null safety |
| JavaScript | `null`/`undefined`, opsyonal na chaining`?.`|
| kalawang | `Option<T>`— walang null! Ipinatupad ang oras ng pag-compile |
| Pumunta | `nil`para sa mga pointer, hiwa, mapa, channel |
| Java | `null`,`Optional<T>`(Java 8+),`@Nullable`|
| C | `NULL`(macro para sa`((void*)0)`) |
| C++ | `nullptr`(C++11),`std::optional<T>`|
| C# | `null`, mga nullable na uri ng sanggunian (C# 8+),`Nullable<T>`|
| matulin | `Optional<T>`(`T?`),`if let`,`guard let`|
| Kotlin | `T?`(nullable),`?.`ligtas na tawag,`?:`Elvis |
...
| Ruby | `nil`(lahat ay totoo maliban sa`false`at`nil`) |
| Elixir | Walang null — gumamit ng`nil`o pattern match |
| Erlang | Walang null — gumamit ng`undefined`o pattern match |