...
-->
# Gabay sa Mabilis na Sanggunian ng SQL
Mahahalagang SQL command para sa mga pagpapatakbo ng database.
---

## Pangunahing Istraktura ng Query
```sql
...
## Pagkuha ng Data (PUMILI)
### Pangunahing Pagpili```sql
-- Select all columns
SELECT * FROM users;
...
## Mga Pag-andar ng Pagsasama-sama
```sql
-- Count rows
...
## Mga Pagpapatakbo ng Table
### GUMAWA ng Talahanayan```sql
CREATE TABLE users (
...
## Mga Common Table Expressions (CTEs)
```sql
-- Simple CTE
...
### Numeric
-`INT`- Integer
-`BIGINT`- Malaking integer
-`DECIMAL(p,s)`- Eksaktong decimal (katumpakan, sukat)
-`FLOAT`- Tinatayang lumulutang na punto
-`DOUBLE`- Dobleng precision float
### String
-`CHAR(n)`- Nakapirming haba ng string
-`VARCHAR(n)`- Variable ang haba ng string
-`TEXT`- Malaking text
-`ENUM`- Enumerated values
### Petsa/Oras
-`DATE`- Petsa (YYYY-MM-DD)
-`TIME`- Oras (HH:MM:SS)
-`DATETIME`- Petsa at oras
-`TIMESTAMP`- Unix timestamp
-`YEAR`- Halaga ng taon
### Boolean
-`BOOLEAN`o`BOOL`- Tama/Mali
### Binary
-`BLOB`- Binary malaking bagay
-`BINARY`- Nakapirming binary
-`VARBINARY`- Variable binary
---

## Mga Kapaki-pakinabang na Pag-andar
### Mga Pag-andar ng String```sql
...
## Mga Tip sa Pagganap
✅ **Gawin:**
- Gumamit ng mga index sa mga column na madalas itanong
- Piliin lamang ang mga kinakailangang column (iwasan ang`SELECT *`)
- Gamitin ang`EXPLAIN`upang suriin ang pagganap ng query
- I-normalize ang data nang naaangkop
- Gumamit ng mga inihandang pahayag upang maiwasan ang SQL injection
❌ **Huwag:**
- Gumamit ng mga function sa mga naka-index na column sa mga sugnay na WHERE
- Lumikha ng masyadong maraming index (mabagal ang pagsusulat)
- Gamitin ang`SELECT DISTINCT`nang hindi kinakailangan
- Huwag pansinin ang mga plano sa pagpapatupad ng query
- Mag-imbak ng mga nakalkulang halaga kapag maaari silang kalkulahin
---
...
*Huling na-update: Hulyo 2026 | SQL Standard (MySQL/PostgreSQL compatible)*