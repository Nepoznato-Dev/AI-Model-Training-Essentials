...
---
# Mwongozo wa Marejeleo wa Haraka wa SQL
Amri muhimu za SQL kwa shughuli za hifadhidata.
---

## Muundo wa Maswali ya Msingi
```sql
...
## Urejeshaji Data (CHAGUA)
### Chaguo la Msingi```sql
-- Select all columns
SELECT * FROM users;
...
##Inajiunga
### Kujiunga kwa Ndani```sql
SELECT u.name, o.order_date, o.total
FROM users u
INNER JOIN orders o ON u.id = o.user_id;
```

### Kushoto/Kulia Jiunge```sql
...
## Uendeshaji wa Jedwali
### TENGENEZA Jedwali```sql
CREATE TABLE users (
...
## Maneno ya Kawaida ya Jedwali (CTEs)
```sql
-- Simple CTE
...
### Nambari
`INT` - Nambari kamili
-`BIGINT`- Nambari kubwa
-`DECIMAL(p,s)`- Desimali kamili (usahihi, kiwango)
-`FLOAT`- Takriban sehemu ya kuelea
-`DOUBLE`- Kuelea kwa usahihi mara mbili
### Kamba
-`CHAR(n)`- Kamba ya urefu usiohamishika
-`VARCHAR(n)`- Mfuatano wa urefu unaobadilika
-`TEXT`- Maandishi makubwa
-`ENUM`- Thamani zilizohesabiwa
### Tarehe/Saa
`DATE` - Tarehe (YYYY-MM-DD)
-`TIME`- Saa (HH:MM:SS)
`DATETIME` - Tarehe na wakati
`TIMESTAMP` - Muhuri wa muda wa Unix
-`YEAR`- Thamani ya mwaka
### Boolean
`BOOLEAN` au`BOOL`- Kweli/Uongo
### Nambari
-`BLOB`- Kitu kikubwa cha binary
-`BINARY`- Fixed binary
-`VARBINARY`- Binari inayobadilika
---
...
✅ **Fanya:**
- Tumia faharasa kwenye safu wima zinazoulizwa mara kwa mara
- Chagua safu wima zinazohitajika tu (epuka `SELECT *`)
- Tumia`EXPLAIN`kuchanganua utendaji wa hoja
- Kurekebisha data ipasavyo
- Tumia taarifa zilizoandaliwa ili kuzuia sindano ya SQL
❌ **Usifanye:**
- Tumia vitendaji kwenye safu wima zilizoorodheshwa katika vifungu vya WAPI
- Unda faharisi nyingi (inaandika polepole)
- Tumia`SELECT DISTINCT`bila lazima
- Puuza mipango ya utekelezaji wa hoja
- Hifadhi maadili yaliyokokotwa wakati yanaweza kuhesabiwa
---
...
*Ilisasishwa mara ya mwisho: Julai 2026 | SQL Standard (MySQL/PostgreSQL patanifu)*