...
|----------|------|---------|-------|
| `+``-``*``/``^`| Hesabu | `2^10`| |
| `÷`| Mgawanyiko kamili | `7 ÷ 3`| Sawa na`div(7, 3)`|
| `%``\` | Modulus/salio | `7 % 3`| |
| `==``!=` | Usawa | `a == b`| Thamani usawa |
| `===`| Utambulisho | `a === b`| Kitu sawa |
| `<``>``<=``>=` | Kulinganisha | `a >= b`| Chainable:`1 < x < 10`|
| `&&``\|\|``!`| Mantiki | `a && b`| Mzunguko mfupi |
| `&``\|``⊻``~` | Bitwise | `a & b`| `⊻`ni XOR |
| `:`| Masafa | `1:10`| `start:step:stop`|
| `..`| Muda | `1..10`| Inahitaji IntervalSets |
| `∈``∉``⊆`| Weka uanachama | `x ∈ S`| `in(x, S)`|
| `∘`| Muundo | `f ∘ g`| `(f ∘ g)(x) == f(g(x))`|
| `\|>`| Bomba | `x \|> f`| `f(x)`|
| `=>`| Oa | `a => b`| Kwa maagizo |
| `...`| Splat | `f(args...)`| Panua mkusanyiko |
### Kulinganisha kwa Minyororo
```julia
...
## Muhtasari
Sintaksia ya Julia ni safi, ya hisabati, na ya kueleza. Utumaji nyingi huruhusu utendakazi kufanya kazi kwa njia tofauti kulingana na aina zote za hoja, kuwezesha mpangilio mzuri wa msimbo. Macros hutoa upangaji meta kwa nguvu bila ugumu wa macros ya awali ya mtindo wa C. Mfumo wa aina - wenye aina za vigezo, aina za dhahania, na aina za muungano - huwezesha kubadilika na utendakazi. Uorodheshaji wenye msingi 1 wa Julia, ufahamu, na sintaksia ya kuzuia hufanya iwe ya asili kwa kompyuta ya hisabati na kisayansi. Lugha inafanikisha "kutembea kama Python, kukimbia kama C" ahadi kupitia mkusanyiko wa JIT na uelekezaji wa aina.