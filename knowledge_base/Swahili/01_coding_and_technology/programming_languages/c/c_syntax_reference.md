...
# C - Rejeleo la Sintaksia
Hati hii inatoa marejeleo ya kina, yenye muundo wa sintaksia ya C (C11/C17 yenye madokezo ya C23). Inakamilisha marejeleo kuu ya C kwa kuzingatia mifumo kamili ya sintaksia, majedwali ya opereta, mechanics ya vielelezo, na usimamizi wa kumbukumbu.
---
...
| `*`| Kuzidisha | `3 * 2`| `6`| |
| `/`| Idara | `7 / 2`| `3`| Upunguzaji nambari kamili kwa aina za int |
| `%`| Moduli | `7 % 2`| `1`| Kwa aina kamili pekee |
| `++`| Ongezeko | `i++`/`++i`| | Post- na kabla ya nyongeza |
| `--`| Kupungua | `i--`/`--i`| | Punguzo la baada na la awali |
...
| `\|`| AU | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `~`| Kukamilisha | `~0`| Biti zote zimepinduliwa |
| `<<`| Shift ya Kushoto | `1 << 3`| `8`|
| `>>`| Shift ya kulia | `8 >> 3`| `1`(utekelezaji-umefafanuliwa kwa saini) |
### Kielekezi & Viendeshaji Anwani
| Opereta | Jina | Mfano | Vidokezo |
|----------|------|---------|-------|
| `&`| Anwani ya | `&x`| Hurejesha kielekezi kwa x |
| `*`| Marejeleo | `*ptr`| Thamani ya ufikiaji kwenye kielekezi |
| `->`| Idhini ya mwanachama kupitia pointer | `p->x`| Sawa na`(*p).x`|
| `.`| Idhini ya mwanachama | `s.field`| Ufikiaji wa moja kwa moja wa muundo |
| `[]`| Usajili wa safu | `arr[i]`| Sawa na`*(arr + i)`|
### Utangulizi wa Opereta (juu hadi chini kabisa)
| Utangulizi | Waendeshaji | Ushirika |
|----------------------------------------|
| 1 (ya juu zaidi) | `()``[]``->``.` | Kushoto kwenda kulia |
| 2 | `!``~``+``-` (unary)`*``&``sizeof``(type)` | Kulia kwenda kushoto |
| 3 | `*``/``%`| Kushoto kwenda kulia |
| 4 | `+``-` | Kushoto kwenda kulia |
| 5 | `<<``>>` | Kushoto kwenda kulia |
| 6 | `<``<=``>``>=` | Kushoto kwenda kulia |
| 7 | `==``!=` | Kushoto kwenda kulia |
| 8 | `&`(bitwise NA) | Kushoto kwenda kulia |
| 9 | `^`| Kushoto kwenda kulia |
| 10 | `\|`| Kushoto kwenda kulia |
| 11 | `&&`| Kushoto kwenda kulia |
| 12 | `\|\|`| Kushoto kwenda kulia |
| 13 | `? :`(ternary) | Kulia kwenda kushoto |
| 14 | `=``+=``-=``*=``/=``%=``&=``\|=` XQZMARKER3QZMARKX`>>=`| Kulia kwenda kushoto |
| 15 (chini kabisa) | `,`(koma) | Kushoto kwenda kulia |
---
...
#Viashiria na Kumbukumbu
### Mitambo ya Vielekezi
```c
int x = 42;
...
### Usimamizi wa Kumbukumbu Inayobadilika
```c
#include <stdlib.h>
#include <string.h>
...
## Muhtasari
Sintaksia ya C ni ndogo na iko karibu na maunzi - viashiria, usimamizi wa kumbukumbu kwa mikono, na safu nyembamba ya uondoaji juu ya maagizo ya mashine. Nguvu ya lugha inatokana na unyenyekevu huu: karibu hakuna gharama zilizofichwa, hakuna muda wa kukimbia, na udhibiti kamili wa mpangilio wa kumbukumbu. Kichakataji, ingawa kina nguvu, kinahitaji nidhamu ili kutumia kwa usalama. C ya kisasa (C11/C17/C23) imeongeza vipengele muhimu vya usalama kama vile`_Static_assert`,`_Generic`,`nullptr`, na`typeof`, na kufanya lugha kuwa salama zaidi huku ikidumisha falsafa yake kuu ya kumwamini mtayarishaji programu.