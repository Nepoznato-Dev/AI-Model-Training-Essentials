...
|----------|------|---------|-------|
| `+``-``*``/``^`| Hesabu | `A * B`| Matrix kuzidisha |
| `.*``./``.^`| Kipengele-busara | `A .* B`| Uendeshaji wa safu |
| `'`| Transpose | `A'`| Conjugate transpose |
| `.'`| Uhamisho usio wa kuunganisha | `A.'`| |
| `\`| Mgawanyiko wa kushoto | `A \ b`| Tatua Shoka = b |
| `/`| Mgawanyiko wa kulia | `b / A`| Tatua xA = b |
| `==``~=` | Usawa | `A == B`| Kipengele-busara |
| `<``>``<=``>=` | Kulinganisha | `A > 0`| Kipengele-busara |
| `&``\|``~`| Mantiki (kipengele-busara) | `A & B`| |
| `&&``\|\|` | Mzunguko mfupi | `a && b`| Scalar pekee |
| `:`| Safu/safu | `1:10`| `start:step:stop`|
| `;`| Kitenganisha safu / kukandamiza | `[1; 2; 3]`| |
| `,`| Kitenganishi cha safu wima | `[1, 2, 3]`| |
---
...
## Muhtasari
Sintaksia ya MATLAB ni ya matrix-kwanza - kila kigezo ni safu, na utendakazi huboreshwa kwa aljebra ya mstari. Opereta ya koloni, uwekaji faharasa wa kimantiki, na utendakazi wa kivekta huondoa vitanzi kwa kazi nyingi za nambari. Mfumo wa kupanga njama hutoa takwimu za ubora wa uchapishaji na msimbo mdogo. Sanduku za zana hupanua MATLAB katika kila kikoa cha uhandisi. Ingawa Python imeingilia baadhi ya maeneo, MATLAB inasalia kuwa kiwango cha ukokotoaji wa kihandisi, uundaji wa msingi wa Simulink, na uchapaji wa haraka wa protoksi.