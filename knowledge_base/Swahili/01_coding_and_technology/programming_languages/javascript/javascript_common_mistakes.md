...
---
# JavaScript - Makosa ya Kawaida & Miundo ya Kupambana
Hati hii inaorodhesha makosa ya kawaida, mitego, na mifumo ya kupingana katika JavaScript. Kila ingizo linaonyesha njia isiyo sahihi, inaelezea kwa nini inashindwa, na hutoa suluhisho sahihi.
---
...
## 3. Masuala ya Upandishaji na Upeo wa `var`
```javascript
...
## 5. Kutoshughulikia Kukataliwa kwa Ahadi
```javascript
...
## 6. Kurekebisha Vitu Wakati wa Kurudia
```javascript
...
## 9. Vipengee Vinavyofanana na Mpangilio Sio Mkusanyiko
```javascript
...
## 10. Anti-Pattern: Mutating Function Parameters
```javascript
...
## 11. Kutoelewa Kitanzi cha Tukio na Shughuli Ndogo
```javascript
...
## 12. Anti-Pattern: Kwa kutumia `eval()`
```javascript
...
## 13. Kusahau Kusafisha Wasikilizaji wa Tukio
```javascript
// ❌ WRONG — memory leak
class Component {
  mount() {
...
## Muhtasari
Unyumbufu wa JavaScript unakuja na mitego: Kufunga kwa`this`kunategemea tovuti ya simu,`==`hufanya shuruti, ahadi zinahitaji kushughulikia makosa, na uagizaji wa kitanzi cha tukio (shughuli ndogo kabla ya makrotasks kuu) sio dhahiri. Sheria za kisasa ni rahisi: tumia`const`/`let`kamwe`var`, pendelea`===`kuliko`==`, shughulikia kukataliwa kwa ahadi kila wakati, tumia vitendaji vya mshale kwa kurudi nyuma, epuka mabadiliko, na safisha wasikilizaji wa tukio. Kufuatia sheria hizi huondoa hitilafu nyingi za JavaScript.