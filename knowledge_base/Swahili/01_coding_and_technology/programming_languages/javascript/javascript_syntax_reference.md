...
---
# JavaScript - Rejea ya Sintaksia
Hati hii inatoa marejeleo ya kina, yenye muundo wa sintaksia ya JavaScript. Inakamilisha rejeleo kuu la JavaScript kwa kuzingatia mifumo kamili ya sintaksia, majedwali ya waendeshaji na ufundi wa ndani.
---

## Waendeshaji & Vielezi
### Waendeshaji Hesabu
| Opereta | Jina | Mfano | Matokeo | Vidokezo |
|----------|------|------------------|--------|
| `+`| Nyongeza | `3 + 2`| `5`| Pia muunganisho wa kamba |
| `-`| Utoaji | `3 - 2`| `1`| |
| `*`| Kuzidisha | `3 * 2`| `6`| |
| `/`| Idara | `7 / 2`| `3.5`| |
| `%`| Salio | `7 % 2`| `1`| |
| `**`| Ufafanuzi | `2 ** 10`| `1024`| ES2016 |
| `++`| Ongezeko | `x++`/`++x`| | Post- na kabla ya nyongeza |
| `--`| Kupungua | `x--`/`--x`| | Punguzo la baada na la awali |
| `+x`,`-x`| Unaria | `+"42"`→`42`| Andika shurutisho kwa nambari |
### Ulinganisho na Usawa
| Opereta | Jina | Mfano | Vidokezo |
|----------|------|---------|-------|
| `===`| Usawa Mkali | `1 === 1`| Hakuna aina ya kulazimishwa - pendelea hii kila wakati |
| `!==`| Kutokuwa na Usawa Mkali | `1 !== "1"`| `true`— aina tofauti |
| `==`| Usawa wa Kikemikali | `1 == "1"`| Aina ya kulazimisha — epuka |
| `!=`| Kikemikali Kutokuwa na Usawa | `1 != "1"`| `false`- epuka |
| `<`,`>`,`<=`,`>=`| Mahusiano | `"b" > "a"`| Leksikografia ya tungo |
### Viendeshaji Mantiki na Batili
| Opereta | Jina | Mfano | Vidokezo |
|----------|------|---------|-------|
| `&&`| Mantiki NA | `a && b`| Hurejesha thamani ya kwanza ya uwongo au ya mwisho |
| `\|\|`| Mantiki AU | `a \|\| b`| Hurejesha thamani ya kwanza ya ukweli au ya mwisho |
| `!`| Mantiki SI | `!true`| `false`|
| `??`| Nullish Coalescing | `a ?? b`| Hurejesha`b`ikiwa tu`a`ni`null`/`undefined`|
| `?.`| Chain ya Hiari | `obj?.prop?.method()`| Mzunguko mfupi hadi`undefined`|
### Viendeshaji Bitwise
| Opereta | Jina | Mfano | Vidokezo |
|----------|------|---------|-------|
| `&`| NA | `5 & 3`| `1`|
...
| `>>`| Shift ya Kulia iliyosainiwa | `-5 >> 1`| `-3`|
| `>>>`| Shift ya Kulia Haijatiwa saini | `-1 >>> 0`| `4294967295`|
### Kazi na Uharibifu
```javascript
// Basic assignment
let x = 10;
...
| Utangulizi | Waendeshaji | Ushirika |
|----------------------------------------|
| 1 (ya juu zaidi) | `()``[]``.``?.` | Kushoto |
| 2 | `new`(bila args) | Sawa |
| 3 | `++``--` (postfix) | N/A |
| 4 | `!``~``+x``-x``typeof``void``delete``++``--`| Sawa |
| 5 | `**`| Sawa |
| 6 | `*``/``%`| Kushoto |
| 7 | `+``-` | Kushoto |
| 8 | `<<``>>``>>>`| Kushoto |
| 9 | `<``<=``>``>=``in``instanceof` | Kushoto |
| 10 | `===``!==``==``!=` | Kushoto |
| 11 | `&`| Kushoto |
| 12 | `^`| Kushoto |
| 13 | `\|`| Kushoto |
...
| 16 | `??`| Kushoto |
| 17 | `=``+=``-=`nk. Sawa |
| 18 (chini kabisa) | `=>`(kazi ya mshale) | Sawa |
---
...
## Miundo ya Data
### Muhtasari wa Mikusanyiko Iliyojumuishwa
| Mkusanyiko | Imeagizwa | Inaweza kubadilika | Nakala | Aina ya Ufunguo | Tafuta |
|--------------------------------|--------------------------------|
| `Array`| Ndiyo | Ndiyo | Ndiyo | Kielezo (nambari) | O(n) |
| `Object`| Ndiyo (kuingizwa) | Ndiyo | Funguo: Hapana | Kamba/Alama | O(1) wastani |
| `Map`| Ndiyo (kuingizwa) | Ndiyo | Funguo: Hapana | Yoyote | O(1) wastani |
| `Set`| Ndiyo (kuingizwa) | Ndiyo | Hapana | - | O(1) wastani |
| `WeakMap`| Hapana | Ndiyo | Funguo: Hapana | Kitu pekee | O(1) wastani |
| `WeakSet`| Hapana | Ndiyo | Hapana | Kitu pekee | O(1) wastani |
### Safu
```javascript
// Creation
...
## Muhtasari
Sintaksia ya JavaScript ni rahisi kiudanganyifu kwenye uso lakini ina kina kikubwa. Lugha imebadilika kutoka kwa zana rahisi ya uandishi hadi lugha ya dhana nyingi yenye madarasa, moduli, jenereta, proksi, na muundo wa hali ya juu wa usawazishaji. Kuelewa utangulizi wa opereta, kitanzi cha tukio, minyororo ya mfano, na nuances ya ufungaji wa`this`hutenganisha wasanidi programu wenye uwezo kutoka kwa wataalamu. JavaScript ya Kisasa (ES2020+) iliyo na mnyororo wa hiari, kuunganisha batili,`Promise.allSettled`, na kusubiri kwa kiwango cha juu inaendelea kupunguza bodi ya boiler huku ikidumisha uoanifu wa kurudi nyuma.