...
## 1. Kupambana na Kikagua Kukopa — Kuunganisha Kusio lazima
```rust
...
## 2. Kamba dhidi ya &str Kuchanganyikiwa
```rust
...
## 3. Kupuuza Matokeo yenye`unwrap()`katika Uzalishaji
```rust
...
## 4. Makosa ya Ufafanuzi wa Maisha
```rust
...
## 5. Anti-Pattern: Kutumia`Rc`/`RefCell`Kila mahali
```rust
...
## 7. Anti-Pattern: Kupita Kiasi`to_string()`/ `to_owned()`
```rust
...
## 9. Ubatilishaji wa Kiigizaji — Kukusanya Isivyo Lazima
```rust
...
## 10. Anti-Pattern: Enum Bila Mbinu
```rust
...
## 11. Kutotumia`#[derive]`kwa Sifa za Kawaida
```rust
...
## 12. Msimbo Usio Salama Bila Hati
```rust
...
## Muhtasari
Mkusanyaji wa Rust ni mkali lakini sawa - makosa mengi hufundisha kitu kuhusu usalama wa kumbukumbu. Makosa makubwa zaidi si makosa ya mkusanyaji bali ni vielelezo vinavyopingana: uunganishaji usio wa lazima ili kutuliza kikagua kuazima, kwa kutumia`unwrap()`katika uzalishaji, kufikia`Rc<RefCell<T>>`kabla ya kujaribu umiliki, na kutenga mifuatano wakati marejeleo yanapotosha. Mtazamo wa Rustacean ni: wacha mkusanyaji akuongoze kuelekea vifupisho vya bei sifuri. Funga tu inapobidi, sambaza makosa kwa`?`, pendelea`&str`kuliko`String`, na uweke kumbukumbu kila kizuizi cha `unsafe`.