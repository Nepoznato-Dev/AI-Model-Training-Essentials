...
|----------|------|---------|-------|
| `+``-``*``/``%`| Hesabu | `a + b`| |
| `..``...` | Masafa | `1...10`| `...`imefungwa; `..<`nusu wazi |
| `==``!=``<``>``<=``>=` | Kulinganisha | `a == b`| Inahitaji`Equatable`/`Comparable`|
| `===``!==` | Utambulisho | `a === b`| Rejeleo sawa (madarasa pekee) |
| `&&``\|\|``!`| Mantiki | `a && b`| Mzunguko mfupi |
| `??`| Nil coalescing | `a ?? b`| Hurejesha`b`ikiwa`a`haipo |
| `?.`| Mnyororo wa hiari | `a?.b`| Hurejesha nil ikiwa`a`haipo |
| `!`| Lazimisha kufunua | `a!`| Mivurugiko ikiwa hakuna — epuka |
| `&`| Kigezo cha Inout | `swap(&a, &b)`| Pitia kwa kumbukumbu |
...
## Vifungashio vya Mali & Wajenzi wa Matokeo
```swift
// Property wrapper
@propertyWrapper
struct Trimmed {
...
## Muhtasari
Sintaksia ya Swift inasawazisha usalama na usemi. Hiari huondoa hitilafu batili, itifaki huwezesha utungaji unaonyumbulika bila urithi, na upatanifu wa kisasa (async/ait, watendaji) hufanya programu salama kwa wakati mmoja kufikiwa. Lugha inaendelea kubadilika ikiwa na vipengele kama vifurushi vya vigezo, makro na umiliki - kila nyongeza hufanya Swift kuwa salama na yenye nguvu zaidi huku ikidumisha ahadi yake ya msingi: msimbo ambao ni salama kwa ujenzi.