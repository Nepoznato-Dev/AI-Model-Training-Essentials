...
-->
# Mwepesi - Makosa ya Kawaida & Miundo ya Kupambana
Hati hii inaorodhesha makosa ya kawaida, mitego, na mifumo ya kupinga katika Swift. Kila ingizo linaonyesha njia isiyo sahihi, inaelezea kwa nini inashindwa, na hutoa suluhisho sahihi.
---

## 1. Lazimisha Chaguo za Kufungua
```swift
// ❌ WRONG — crash if nil
...
## 2. Hifadhi Mizunguko yenye Mizunguko
```swift
...
## 3. Kutotumia`enum`yenye Maadili Yanayohusishwa
```swift
...
## 4. Kutoelewa Aina za Thamani dhidi ya Aina za Marejeleo
```swift
...
## 5. Anti-Pattern: Kidhibiti Kikubwa cha Mwonekano
```swift
...
## 6. Kutotumia`guard`kwa Kutoka Mapema
```swift
...
## 8. Mkanganyiko wa Kielezo cha Kamba
```swift
...
## 9. Kutotumia`lazy`kwa Mali Ghali
```swift
...
## Muhtasari
Vipengele vya usalama vya Swift huzuia hitilafu nyingi lakini huanzisha mitego yao wenyewe: kufunua kwa nguvu husababisha kuacha kufanya kazi, kubakiza mizunguko kujificha katika kufungwa na wajumbe, na wasanidi wa semantiki za aina ya thamani hushangaza kutoka kwa lugha za aina za marejeleo. Njia ya Mwepesi ni: usilazimishe kufungua (`!`), tumia`guard`kwa kuondoka mapema, tumia`weak`kwa wajumbe na picha za kufungwa, weka alama kwenye msimbo wa UI kwa`@MainActor`, na uvunje vidhibiti vikubwa vya kutazama katika miundo na huduma za kutazamwa. Mfumo wa aina ya Swift ni rafiki yako - fanya kazi nao, sio dhidi yake.