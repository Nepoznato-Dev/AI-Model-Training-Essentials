...
---
# TypeScript - Makosa ya Kawaida & Miundo ya Kupambana
Hati hii inaorodhesha makosa ya kawaida, mitego, na mifumo ya kupingana katika TypeScript. Kila ingizo linaonyesha njia isiyo sahihi, inaelezea kwa nini inashindwa, na hutoa suluhisho sahihi.
---

## 1. Kutumia kupita kiasi `any`
```typescript
...
## 2. Aina Madai Badala ya Walinzi Aina
```typescript
...
## 3. Unyanyasaji wa Madai Yasio Batili
```typescript
...
## 4. Violesura dhidi ya Aina za Kuchanganyikiwa
```typescript
...
## 9. Anti-Pattern: Aina Zinazorudiwa (Kutotumia Jenerali)
```typescript
...
## 10. Minyororo ya Hiari ya Kupindukia
```typescript
...
## 11. Kwa kutumia`namespace`(Legacy)
```typescript
...
## 12. Kutotumia Vyama Vilivyobaguliwa
```typescript
...
## Muhtasari
Mfumo wa aina ya TypeScript una nguvu lakini unahitaji nidhamu. Dhambi kuu inafikia`any`- tumia`unknown`wakati aina haijulikani kabisa, na punguza kwa walinzi wa aina. Epuka madai yasiyo ya ubatili (`!`) kwa kushughulikia bila kufafanuliwa ipasavyo. Tumia violesura vya maumbo ya vitu, aina za miungano na aina zilizokokotwa. Vyama vya wafanyakazi vilivyobaguliwa huondoa aina zote za mende. Washa`strict: true`kutoka siku ya kwanza - ukali wa kuweka upya ni chungu. Lengo ni kufanya majimbo haramu kutowakilishwa katika kiwango cha aina.