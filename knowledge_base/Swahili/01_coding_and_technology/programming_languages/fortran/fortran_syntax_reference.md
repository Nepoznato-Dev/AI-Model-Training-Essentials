...
| `==``/=` | Usawa | `a == b`| `/=`"si sawa" |
| `<``>``<=``>=` | Kulinganisha | `a >= b`| |
| `.and.``.or.``.not.`| Mantiki | `a .and. b`| |
| `.eqv.``.neqv.` | Usawa wa kimantiki | `a .eqv. b`| |
| `//`| Muunganisho wa kamba | `'hello' // ' world'`| |
---
...
## Muhtasari
Sintaksia ya kisasa ya Fortran ni safi, imeundwa, na imeboreshwa kwa ukokotoaji wa nambari. Uendeshaji wa safu huondoa vitanzi kwa kazi nyingi za hisabati. Modules hutoa encapsulation na usimamizi wa nafasi ya majina. Vipengele vya OOP (aina zinazotokana, urithi, polymorphism) huwezesha upangaji wa msimbo safi. Coarrays hutoa programu ya asili sambamba bila maktaba za nje. Nguvu za Fortran - utendakazi, utendakazi wa safu, na usahihi wa nambari - husalia bila kulinganishwa kwa kompyuta ya kisayansi. Kwa HPC, fizikia ya hesabu, na uigaji wa uhandisi, Fortran inaendelea kuwa nguvu.