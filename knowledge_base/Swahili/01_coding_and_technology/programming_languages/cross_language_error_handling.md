...
---
# Ulinganisho wa Lugha Mtambuka - Kushughulikia Hitilafu
## Muhtasari wa Kushughulikia Miundo
| Mfano | Lugha | Utaratibu |
|-------|-----------|------------|
| **Vighairi (jaribu/kamata)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **Aina za matokeo/Chaguo** | Rust, Haskell, OCaml, Scala (pia) | `Result<T, E>`,`Maybe a`|
| **Thamani nyingi za kurejesha** | Nenda, Lua | `value, err := f()`|
| **Misimbo ya hitilafu** | C, Fortran | Rudisha msimbo wa hitilafu, angalia mwenyewe |
| **Hitilafu za kulinganisha muundo** | Elixir, Erlang | `{:ok, value}`/`{:error, reason}`|
| **Masharti (yanayoweza kuanzishwa upya)** | Lisp ya Kawaida | `restart-case`,`handler-bind`|
| **Hakuna kushughulikia makosa** | Scratch, SQL (msingi) | N/A |
| **Mikataba** | Ada, Eiffel | `Pre`,`Post`masharti |
## Kushughulikia Hitilafu Kwa Msingi wa Isipokuwa
```python
# Python
try:
    result = 10 / 0
...
## Muhtasari: Hitilafu Kushughulikia Vielelezo
| Dhana | Mbinu | Faida | Hasara |
|----------|----------|------|------|
| **Vighairi** | tupa/kamata | Safisha njia ya furaha, ufuatiliaji wa safu | Inaweza kusahaulika, kudhibiti matumizi mabaya ya mtiririko |
| **Aina za matokeo** | Rudisha`Ok`/`Err`| Dhahiri, haiwezi kupuuza makosa | Verbose, opereta wa`?`anahitajika |
| **Misimbo ya hitilafu** | Rudisha hali ya int | Rahisi, Inayolingana na C | Rahisi kupuuza, hakuna muktadha |
| **Rejesha nyingi** | `value, err := f()`| Wazi, nahau | Unaweza kusahau kuangalia`err`|
| **Muundo unaolingana** | `{:ok, v}`/`{:error, r}`| Inakamilika, inafanya kazi | Verbose bila`with`|
| **Masharti** | Vishikilizi vinavyoweza kuwashwa upya | Nguvu zaidi, inayoweza kurejeshwa | Ngumu, haieleweki sana |