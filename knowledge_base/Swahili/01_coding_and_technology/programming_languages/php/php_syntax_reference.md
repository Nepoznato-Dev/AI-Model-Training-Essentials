...
---
# PHP - Rejeleo la Sintaksia
Hati hii inatoa marejeleo ya kina, yaliyopangwa ya sintaksia ya PHP (8.x). Inakamilisha rejeleo kuu la PHP kwa kuzingatia mifumo kamili ya sintaksia, vipengele vya kisasa vya PHP, upangaji unaolenga kitu, na nahau za vitendo.
---
...
| `+``-``*``/``%`| Hesabu | `$a + $b`| |
| `**`| Ufafanuzi | `2 ** 10`| `1024`|
| `==``!=``===``!==` | Usawa | `$a === $b`| `===`huangalia aina pia |
| `<=>`| Nafasi | `$a <=> $b`| Hurejesha -1, 0, au 1 |
| `<``>``<=``>=` | Kulinganisha | `$a >= $b`| |
| `&&``\|\|``!`| Mantiki | `$a && $b`| Mzunguko mfupi |
| `and``or``xor`| Mantiki (utangulizi wa chini) | `$a and $b`| Epuka - tumia`&&`/`\|\|`|
| `??`| Null coalescing | `$a ?? 'default'`| Hurejesha`'default'`ikiwa`$a`ni batili |
| `??=`| Null coalescing hawawajui | `$a ??= 'default'`| Inakabidhi ikiwa`$a`ni batili |
| `?``:` | Ternary | `$a ? $b : $c`| |
| `.`| Kuunganisha | `$a . $b`| |
| `.=`| Kuunganisha kukabidhi | `$a .= $b`| |
| `=>`| Mgawo wa safu | `['key' => 'value']`| |
| `->`| Idhini ya mwanachama | `$obj->method()`| |
| `?->`| Opereta Nullsafe | `$obj?->method()`| Hurejesha batili ikiwa`$obj`ni batili |
| `::`| Ufikiaji tuli | `ClassName::method()`| |
| `instanceof`| Andika hundi | `$obj instanceof ClassName`| |
| `clone`| Uundaji wa kitu | `$copy = clone $obj`| Nakala ya kina inahitaji`__clone()`|
### Aina ya Mauzauza
```php
...
## Nafasi za Majina & Upakiaji Kiotomatiki
```php
...
## Muhtasari
Sintaksia ya PHP imebadilika sana kutoka asili yake ya uandishi hadi lugha ya kisasa, salama ya aina. PHP 8.x ilianzisha aina za miungano, aina za makutano, enum, nyuzi, madarasa ya kusoma pekee, sifa, na viitwavyo vya daraja la kwanza - kuibadilisha kutoka lugha ya uandishi iliyochapwa kwa urahisi hadi kuwa inayotumia uhandisi wa programu dhabiti. Nguvu ya lugha inasalia kuwa mwelekeo wake wa wavuti: na mifumo kama Laravel na Symfony, mfumo mkubwa wa ikolojia, na usaidizi wa karibu wa upangishaji, sintaksia ya PHP hutumikia mamilioni ya programu. PHP ya kisasa huwatuza wasanidi programu wanaokubali mfumo wa aina yake, hutumia upakiaji otomatiki unaotegemea mtunzi, na kuandika msimbo safi, unaolenga kitu.