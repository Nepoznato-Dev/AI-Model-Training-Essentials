...
-->
# Python - Rejea ya Sintaksia
Hati hii inatoa marejeleo ya kina, yaliyopangwa ya syntax ya Python. Inakamilisha rejeleo kuu la Python kwa kuzingatia mifumo kamili ya sintaksia, majedwali ya waendeshaji, na mechanics ya ndani.
---
...
|----------|------|------------------|--------|
| `+`| Nyongeza | `3 + 2`| `5`| Pia muunganisho wa kamba/orodha |
| `-`| Utoaji | `3 - 2`| `1`| Pia ukanushaji usio wa kawaida |
| `*`| Kuzidisha | `3 * 2`| `6`| Pia mfuatano/orodha marudio |
| `/`| Idara ya Kweli | `7 / 2`| `3.5`| Hurejesha kuelea kila wakati |
| `//`| Sehemu ya Sakafu | `7 // 2`| `3`| Mizunguko kuelekea infinity hasi |
| `%`| Moduli | `7 % 2`| `1`| Kigawanyaji cha mechi za ishara |
| `**`| Ufafanuzi | `2 ** 10`| `1024`| Mshiriki wa kulia |
### Ulinganisho na Viendeshaji vya Boolean
| Opereta | Jina | Mfano | Vidokezo |
|----------|------|---------|-------|
| `==`| Sawa | `x == y`| Ulinganisho wa thamani (sio utambulisho) |
| `!=`| Sio Sawa | `x != y`| |
| `<`,`>`,`<=`,`>=`| Kuagiza | `x >= y`| Chainable:`a < b < c`|
| `is`| Utambulisho | `x is y`| Kitu sawa kwenye kumbukumbu |
| `is not`| Sio Utambulisho | `x is not None`| Inapendekezwa zaidi ya`!= None`|
| `in`| Uanachama | `x in collection`| O(1) kwa seti/dict, O(n) kwa orodha |
| `and`| Mantiki NA | `a and b`| Mzunguko mfupi; inarudisha thamani ya mwisho ya ukweli/uongo |
| `or`| Mantiki AU | `a or b`| Mzunguko mfupi; inarudisha thamani ya kwanza ya ukweli/uongo |
| `not`| Mantiki SI | `not x`| |
### Viendeshaji Bitwise
| Opereta | Jina | Mfano | Vidokezo |
|----------|------|---------|-------|
| `&`| NA | `5 & 3`| `1`|
...
| 8 | `^`| Kushoto |
| 9 | `\|`| Kushoto |
| 10 | `==`,`!=`,`<`,`>`,`<=`,`>=`,`is`, X3XQZMARKER2 | Chainable |
| 11 | `not`| Sawa |
| 12 | `and`| Kushoto |
| 13 (chini kabisa) | `or`| Kushoto |
### Mgawo na Mgawo Ulioongezwa
```python
...
## Kazi & Kufungwa
### Ufafanuzi wa Kazi na Upigaji simu
```python
...
| Mkusanyiko | Imeagizwa | Inaweza kubadilika | Nakala | Sintaksia | Tafuta |
|--------------------------------|---------------------|--------|
| `list`| Ndiyo | Ndiyo | Ndiyo | `[1, 2, 3]`| O(n) |
| `tuple`| Ndiyo | Hapana | Ndiyo | `(1, 2, 3)`| O(n) |
| `dict`| Ndiyo (3.7+) | Ndiyo | Funguo: Hapana | `{"k": "v"}`| O(1) wastani |
| `set`| Hapana | Ndiyo | Hapana | `{1, 2, 3}`| O(1) wastani |
| `frozenset`| Hapana | Hapana | Hapana | `frozenset({1,2})`| O(1) wastani |
### Orodha
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
### Miundo ya Ushughulikiaji Isiyofaa
```python
...
## Muhtasari
Rejeleo hili la sintaksia linashughulikia seti kamili ya kipengele cha Python - kutoka kwa waendeshaji msingi na mtiririko wa udhibiti kupitia upangaji wa hali ya juu, vielezi, na mfumo wa aina. Sintaksia ya Python ni ndogo kimakusudi na inasomeka, lakini kina chake ni kikubwa: kulinganisha muundo, jenereta zisizolingana, itifaki, na metaclass hutoa zana zenye nguvu kwa programu changamano. Lugha inaendelea kubadilika, na kila toleo linaongeza vipengele vinavyoboresha uelezaji huku vikidumisha upatanifu wa nyuma.