...
---
# Ulinganisho wa Lugha Mtambuka - Mtiririko wa Udhibiti
## Kama/Vinginevyo Taarifa
```python
# Python
if x > 0:
    print("positive")
...
| Lugha | Sintaksia |
|----------|--------|
| Chatu | `for i in range(10):`|
| JavaScript | `for (let i = 0; i < 10; i++)`|
| Kutu | `for i in 0..10`|
| Nenda | `for i := 0; i < 10; i++`|
| Java | `for (int i = 0; i < 10; i++)`|
| C/C++ | `for (int i = 0; i < 10; i++)`|
| Ruby | `10.times { \|i\| ... }`au`(0..9).each { \|i\| ... }`|
| Mwepesi | `for i in 0..<10`|
| Kotlin | `for (i in 0 until 10)`|
| Haskell | `mapM_ f [0..9]`|
| Perl | `for my $i (0..9)`|
| R | `for (i in 1:10)`|
| PHP | `for ($i = 0; $i < 10; $i++)`|
| SQL | N/A (iliyowekwa, hakuna vitanzi katika SQL ya kawaida) |
| Bashi | `for i in {0..9}; do ... done`|
| Lua | `for i = 0, 9 do ... end`|
| MATLAB | `for i = 1:10 ... end`|
| Matangazo | N/A (recursion badala yake) |
| Lip | `(dotimes (i 10) ...)`|
| Ada | `for I in 0..9 loop ... end loop;`|
| COBOL | `PERFORM VARYING I FROM 0 BY 1 UNTIL I >= 10`|
| Elang | N/A (recursion + orodha badala yake) |
| OCaml | `for i = 0 to 9 do ... done`|
| Bunge | Kitanzi cha mwongozo na CMP/JMP |
| Delphi | `for I := 0 to 9 do ... end;`|
| Mwanga | `repeat (10) ... end`|
...
| Mwepesi | `switch x { case 1: ... default: ... }`|
| Kotlin | `when (x) { 1 -> ... else -> ... }`|
| OCaml | `match x with 1 -> ... \| _ -> ...`|
| Elang | `case X of 1 -> ...; _ -> ... end`|
| Elixir | `case x do 1 -> ...; _ -> ... end`|
| Dart | `switch (x) { 1 => ...; _ => ... }`(Dart 3.0) |
| PHP | `match($x) { 1 => ..., default => ... }`(PHP 8.0) |
| C# | `x switch { 1 => ..., _ => ... }`(C# 8+) |
| TypeScript | Hakuna mchoro asili unaolingana (tumia kama/vinginevyo) |
| Chatu | `match x: case 1: ... case _: ...`(Python 3.10+) |
| Matangazo | Imejumuishwa katika ulinganifu wa kifungu |
| Nenda | `switch x { case 1: ... default: ... }`|