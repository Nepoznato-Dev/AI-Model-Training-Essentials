...
|----------|------|---------|-------|
| `+``-``*``/``%`| Hesabu | `a + b`| `+`pia inaunganisha masharti |
| `++``--` | Ongezeko/Kupungua | `++i`| Pendelea ongezeko la awali |
| `==``!=` | Usawa | `a == b`| Inaweza kupita kiasi; rekodi hutumia usawa wa thamani |
| `<``>``<=``>=` | Mahusiano | `a >= b`| |
| `&&``\|\|``!`| Mantiki | `a && b`| Mzunguko mfupi |
| `&``\|``^``~` | Bitwise | `a & b`| |
| `<<``>>``>>>`| Shift | `a << 2`| `>>>`zamu ya kulia ambayo haijatiwa saini (C# 11) |
| `??`| Null coalescing | `a ?? b`| Hurejesha`b`ikiwa`a`ni batili |
| `?.`| Null masharti | `a?.B`| Hurejesha batili ikiwa`a`ni batili |
| `!`| Kusamehe bure | `a!.B`| Hukandamiza onyo lisiloweza kubatilishwa |
| `is`| Chapa muundo | `x is string s`| Ulinganishaji wa muundo |
| `as`| Waigizaji salama | `x as string`| Hurejesha null juu ya kutofaulu |
| `nameof`| Jina halisi | `nameof(x)`| `"x"`- wakati wa kukusanya |
| `typeof`| Andika kitu | `typeof(int)`| `System.Int32`|
| `sizeof`| Ukubwa kwa baiti | `sizeof(int)`| `4`(muktadha usio salama) |
### Utangulizi wa Opereta (juu hadi chini kabisa)
| Utangulizi | Waendeshaji |
|------------|-----------|
| 1 (ya juu zaidi) | Msingi:`()``.``?.``[]``()`(omba)`++``--` (postfix) XQZMARKER7XQER7XQER7XQZ8XZQZMARK9 |
| 2 | Unary:`+``-``!``~``++``--` (kiambishi awali)`(T)x``await` |
| 3 | Kuzidisha:`*``/``%`|
| 4 | Nyongeza:`+``-` |
| 5 | Shift:`<<``>>``>>>`|
| 6 | Uhusiano:`<``>``<=``>=``is``as` |
...
### Ulinganishaji wa Muundo (C# 8–12)
```csharp
// Switch expression
...
## LINQ
### Sintaksia ya Hoji dhidi ya Sintaksia ya Mbinu
```csharp
...
## Ufafanuzi wa Mfuatano na Uumbizaji
```csharp
...
## Muhtasari
C# ya kisasa ni lugha yenye dhana nyingi inayochanganya upangaji bora unaolenga kitu, utendakazi, na vipengele. Sintaksia yake imebadilika sana - kutoka kwa msimbo wa kitenzi-kama wa Java hadi misemo mifupi yenye kulinganisha muundo, rekodi, usemi wa mkusanyo, na waundaji msingi. LINQ hutoa API ya hoja sawa kwenye mikusanyiko yote. Async/ait hufanya upangaji usio na usawa kuwa wa asili. Mfumo wa aina husawazisha usalama na pragmatism kupitia aina za marejeleo zisizoweza kubatilishwa, jenetiki zenye vikwazo, na kulinganisha muundo. C# inaendelea kubadilika kwa haraka, huku kila toleo likiongeza vipengele ambavyo hupunguza bodi ya boiler huku vikidumisha uoanifu wa kurudi nyuma.