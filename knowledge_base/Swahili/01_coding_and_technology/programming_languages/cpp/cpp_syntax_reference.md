...
Hati hii inatoa marejeleo ya kina, yenye muundo wa sintaksia ya Kisasa C++ (C++17/20/23). Inakamilisha marejeleo kuu ya C++ kwa kuzingatia mifumo kamili ya sintaksia, majedwali ya waendeshaji, umekanika wa violezo na vipengele vya kisasa.
---

## Waendeshaji & Vielezi
### Hesabu na Viendeshaji Kulinganisha
| Opereta | Jina | Mfano | Vidokezo |
|----------|------|---------|-------|
| `+``-``*``/``%`| Hesabu | `a + b`| Sawa na C |
| `++``--` | Ongezeko/Kupungua | `++i`,`i++`| Pendelea ongezeko la awali |
| `==``!=``<``>``<=``>=` | Kulinganisha | `a <=> b`| `<=>`ni ulinganisho wa njia tatu (C++20) |
| `&&``\|\|``!`| Mantiki | `a && b`| Mzunguko mfupi |
| `&``\|``^``~``<<``>>` | Bitwise | `a & b`| Sawa na C |
| `?:`| Ternary | `cond ? a : b`| |
### C++-Viendeshaji Maalum
| Opereta | Jina | Mfano | Vidokezo |
|----------|------|---------|-------|
| `::`| Azimio la upeo | `std::cout`| |
| `.*``->*` | Kielekezi-kwa-mwanachama | `obj.*pmf`| |
| `<=>`| Usafiri wa anga (C++20) | `a <=> b`| Hurejesha`strong_ordering`|
| `co_await`| Coroutine kusitisha | `co_await expr`| C++20 |
| `co_yield`| Mazao ya Corroutine | `co_yield value`| C++20 |
| `co_return`| Kurudi kwa Corutine | `co_return value`| C++20 |
### Kupakia kwa Opereta
```cpp
class Vector2D {
    double x_, y_;
...
### Viunga Vilivyoundwa & Miundo ya Kisasa
```cpp
...
## Kazi & Lambdas
### Sintaksia ya Utendaji ya Kisasa
```cpp
...
## Viashiria Mahiri na Kumbukumbu
```cpp
...
## Vyombo vya STL & Kanuni
### Muhtasari wa Kontena
| Chombo | Kichwa | Imeagizwa | Tafuta | Vidokezo |
|-----------|------------------|--------|-------|
| `vector`| `<vector>`| Ndiyo | O(n) | Chaguo-msingi kwa mifuatano |
| `deque`| `<deque>`| Ndiyo | O(n) | Shinikiza / pop kwa haraka kwenye ncha zote mbili |
| `list`| `<list>`| Ndiyo | O(n) | Orodha iliyounganishwa mara mbili |
| `array`| `<array>`| Ndiyo | O(n) | Ukubwa usiobadilika, uliogawiwa kwa rafu |
| `set`/`multiset`| `<set>`| Ndiyo | O(logi n) | mti mwekundu-nyeusi |
| `map`/`multimap`| `<map>`| Ndiyo (kwa ufunguo) | O(logi n) | Thamani-msingi, mti mwekundu-mweusi |
| `unordered_set`| `<unordered_set>`| Hapana | O(1) wastani | Jedwali la hashi |
| `unordered_map`| `<unordered_map>`| Hapana | O(1) wastani | Jedwali la hashi |
### Algorithms Muhimu
```cpp
#include <algorithm>
#include <numeric>
...
## Muhtasari
Sintaksia ya C++ inaenea kutoka kwa upotoshaji wa viashiria vya kiwango cha chini hadi upangaji wa kiolezo cha hali ya juu na usawazishaji kulingana na kanuni. Mageuzi ya lugha kupitia C++17/20/23 yameboresha usalama kwa kiasi kikubwa (viashiria mahiri,`std::optional`, dhana), uwazi (viunga vilivyoundwa, safu,`std::format`), na utendakazi (dhana zinazowezesha vipunguzo vya gharama sifuri, bila usawazishaji wa safu ya juu). Falsafa ya msingi bado haijabadilika: hupaswi kulipia usichotumia, na lugha inapaswa kuunga mkono kila kiwango cha uondoaji kutoka kwa chuma tupu hadi lugha mahususi za kikoa.