...
-->
# Dart - Rejeleo la Sintaksia
Hati hii inatoa marejeleo ya kina, yenye muundo wa sintaksia ya Dart (3.x). Inakamilisha rejeleo kuu la Dart kwa kuzingatia mifumo kamili ya sintaksia, usalama tupu, upangaji wa programu zisizolingana na muundo wa muundo unaolenga Flutter.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%``~/` | Hesabu | `7 ~/ 3`| `~/`ni kitengo kamili |
| `==``!=` | Usawa | `a == b`| |
| `<``>``<=``>=` | Kulinganisha | `a >= b`| |
| `&&``\|\|``!`| Mantiki | `a && b`| Mzunguko mfupi |
| `??`| Null coalescing | `a ?? b`| `b`ikiwa`a`ni batili |
| `?.`| Ufikiaji usio na ufahamu | `a?.b`| null kama`a`ni null |
| `!`| Madai ya ubatili | `a!`| Inarusha kama null |
| `..``?..` | Kuteleza | `obj..a..b`| Operesheni za mnyororo kwenye kitu kimoja |
| `??=`| Null-aware kukabidhi | `a ??= b`| Hawilisha ikiwa null |
| `as`| Chapa kutupwa | `obj as String`| Hurusha kushindwa |
| `is``is!` | Chapa mtihani | `obj is String`| |
| `=>`| Kitendaji cha mshale | `() => expr`| |
### Nukuu ya Kuteleza
```dart
...
## Muhtasari
Sintaksia ya Dart ni safi, thabiti, na imeundwa kwa ajili ya tija. Usalama wa sauti batili huondoa makosa ya marejeleo matupu wakati wa kukusanya. Async/ngoja na mitiririko hutoa mifumo asilia ya upangaji usiolandanishi. Madarasa, michanganyiko, na madarasa yaliyofungwa huwezesha utungaji wa vitu vinavyonyumbulika. Ulinganishaji wa muundo wa Dart 3, rekodi, na virekebishaji vya darasa huongeza uwazi. Ingawa kusudi kuu la Dart ni Flutter, sintaksia yake inafaa kwa kikoa chochote cha programu - upande wa seva (iliyo na Dart Frog au Rafu), zana za CLI, na programu za wavuti (zilizokusanywa kwa JavaScript).