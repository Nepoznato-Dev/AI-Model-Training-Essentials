...
| `==``!=``===``<=>` | Usawa | `a == b`| `===`ni usawa wa kesi; `<=>`ni meli ya angani |
| `eql?`| Thamani usawa | `a.eql?(b)`| Mkali kuliko`==`(aina ya hundi) |
| `equal?`| Utambulisho | `a.equal?(b)`| Kitu sawa? |
| `<``>``<=``>=` | Kulinganisha | `a >= b`| |
| `&&``\|\|``!`| Mantiki | `a && b`| Mzunguko mfupi; inarejesha thamani iliyotathminiwa mwisho |
| `and``or``not`| Mantiki (utangulizi wa chini) | `a and b`| Epuka — tumia`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | Bitwise | `a & b`| |
| `=~``!~` | Mechi ya regex | `str =~ /pattern/`| Hurejesha faharasa au nil |
| `..``...` | Masafa | `1..10`| `..`pamoja; `...`kipekee |
| `?.`| Urambazaji salama | `user&.name`| Hurejesha nil ikiwa mtumiaji hana |
| `[]``[]=` | Ufikiaji wa kipengele | `arr[0]`| |
### Ukweli
```ruby
...
## Muhtasari
Sintaksia ya Ruby imeundwa kwa ajili ya furaha ya msanidi programu — inasomeka kama Kiingereza na hutuza kwa kujieleza. Nguvu ya lugha hutoka kwa muundo wake wa kitu (kila kitu ni kitu), vizuizi na kufungwa, uwezo wa kupanga metaprogramu, na moduli ya Kuhesabika ambayo hutoa API ya usindikaji wa mkusanyiko tajiri. Ruby 3.x ameongeza ulinganishaji wa muundo, saini za aina ya RBS, na Ractor kwa upatanishi, huku ikidumisha falsafa kuu ya lugha: wafanye watayarishaji programu wafanikiwe na wawe na furaha.