...
-->
# Lua - Makosa ya Kawaida & Miundo ya Kupinga
Hati hii inaorodhesha makosa ya kawaida, mitego, na mifumo ya kupingana katika Lua na masahihisho.
---

## 1. Kuorodhesha kwa Msingi 1
```lua
...
## 2.`nil`vs`false`katika Masharti
```lua
...
## 3. Semantiki za Marejeleo ya Jedwali
```lua
...
## 4. Vigeu vya Ulimwengu kwa Chaguomsingi
```lua
...
## Muhtasari
Ubunifu wa Lua huunda mitego: kuorodhesha kwa msingi 1,`nil`na`false`pekee ni za uwongo, majedwali ni marejeleo (sio maadili), vigeuzo ni vya kimataifa kwa chaguo-msingi, na`pairs`haijapangwa huku`ipairs`imeagizwa. Njia ya Lua ni: kila wakati tumia`local`, tumia`ipairs`kwa safu na`pairs`kwa kamusi, tumia`table.concat`kwa ujenzi wa kamba, na tumia metatable kwa OOP. Usahili wa Lua ni nguvu yake - lakini heshimu kesi za makali.