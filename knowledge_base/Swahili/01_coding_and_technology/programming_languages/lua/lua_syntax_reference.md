...
| `//`| Mgawanyiko wa sakafu | `7 // 2`| Inarudi 3 (Lua 5.3+) |
| `==``~=` | Usawa | `a == b`| `~=`"si sawa" |
| `<``>``<=``>=` | Kulinganisha | `a >= b`| |
| `and``or``not`| Mantiki | `a and b`| Hurejesha uendeshaji, si booleans |
| `..`| Kuunganisha | `"hello" .. " world"`| |
| `#`| Urefu | `#t`| Urefu wa kamba au jedwali |
| `&``\|``~``<<``>>`| Bitwise | `a & b`| Lua 5.3+ (ops kamili) |
### Viendeshaji Kimantiki — Urejeshaji Usio na Boolean
```lua
...
### Utangulizi (juu hadi chini kabisa)
...
## Majedwali — Muundo wa Data wa Jumla
```lua
...
## Moduli na Ushughulikiaji wa Hitilafu
```lua
...
## Muhtasari
Sintaksia ya Lua ni ndogo kimakusudi - lugha nzima inaweza kuelezewa katika ukurasa mmoja. Majedwali hutumika kama muundo wa data zima (safu, kamusi, vitu, moduli). Metatable hutoa upanuzi bila utata wa lugha. Coroutines huwezesha mifumo ya ushirika ya kufanya kazi nyingi na maridadi. Nguvu ya lugha inatokana na upachikaji wake: Lua imeundwa kuishi ndani ya programu-tumizi za seva pangishi, ikitoa uwezo wa uandishi na uendeshaji mdogo. Kwa maendeleo ya mchezo, usanidi, na mifumo iliyopachikwa, usahili wa Lua ndio nguvu yake kuu - lugha ndogo ambayo hufanya kile inachohitaji kufanya, na hakuna zaidi.