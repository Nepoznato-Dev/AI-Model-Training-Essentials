...
|------|--------|-----------|------|----|------|---|-----|-----|
| Nambari | `int`(kiholela) | `number`| `i32`,`u64`| `int`| `int`,`long`| `int`| `int`| `int`|
| Kuelea | `float`| `number`| `f64`| `float64`| `double`| `double`| `double`| `double`|
| Kamba | `str`| `string`| `String`| `string`| `String`| `char*`| `string`| `string`|
| Boolea | `bool`| `boolean`| `bool`| `bool`| `boolean`| `bool`(C99) | `bool`| `bool`|
| Tabia | `str[0]`| `string[0]`| `char`| `rune`| `char`| `char`| `char`| `char`|
...
## Ramani / Kamusi / Majedwali ya Hash
```python
# Python: dict
...
| Kutu | `let s: HashSet<i32> = [1, 2, 3].into()`|
| Nenda | `s := map[int]bool{1: true, 2: true, 3: true}`(hakuna Seti iliyojengwa ndani) |
| Java | `Set<Integer> s = Set.of(1, 2, 3)`|
| C++ | `std::set<int> s = {1, 2, 3}`|
| C# | `var s = new HashSet<int> { 1, 2, 3 }`|
| Ruby | `s = Set[1, 2, 3]`|
| Mwepesi | `var s: Set<Int> = [1, 2, 3]`|
| Kotlin | `val s = setOf(1, 2, 3)`|
| Scala | `val s = Set(1, 2, 3)`|
| Dart | `var s = {1, 2, 3}`|
| Julia | `s = Set([1, 2, 3])`|
| R | `s <- c(1, 2, 3)`(hakuna seti asili; tumia`%in%`) |
## Miundo / Rekodi / Madarasa
```python
...
| Lugha | Sintaksia |
|----------|--------|
| Kutu | `enum Color { Red, Green, Blue }`|
| Mwepesi | `enum Color { case red, green, blue }`|
...
| C# | `enum Color { Red, Green, Blue }`|
| Nenda | `const ( Red = iota; Green; Blue )`(hakuna enum asili) |
| Dart | `enum Color { red, green, blue }`|
| PHP | `enum Color: string { case Red = 'red'; }`(PHP 8.1) |
| Scala | `enum Color { case Red, Green, Blue }`(Scala 3) |
| Chatu | `from enum import Enum; class Color(Enum): RED = 1`|
| Ruby | Hakuna enum asili (tumia vidhibiti vilivyogandishwa au`dry-enum`) |
## Usalama tupu
| Lugha | Null Ushughulikiaji |
|----------|--------------|
| Chatu | `None`, hakuna usalama wa wakati wa kukusanya |
| JavaScript | `null`/`undefined`, minyororo ya hiari`?.`|
| Kutu | `Option<T>`- hakuna null! Muda wa mkusanyiko umetekelezwa |
| Nenda | `nil`kwa viashiria, vipande, ramani, chaneli |
| Java | `null`,`Optional<T>`(Java 8+),`@Nullable`|
| C | `NULL`(jumla ya`((void*)0)`) |
| C++ | `nullptr`(C++11),`std::optional<T>`|
| C# | `null`, aina za kumbukumbu zisizoweza kutumika (C# 8+),`Nullable<T>`|
| Mwepesi | `Optional<T>`(`T?`),`if let`,`guard let`|
| Kotlin | `T?`(haiwezekani),`?.`simu salama,`?:`Elvis |
| TypeScript | `null`/`undefined`,`strictNullChecks`,`T \| null`|
| Haskell | `Maybe a`—`Just x`au`Nothing`|
| Scala | `Option[T]`—`Some(x)`au`None`|
| Dart | `T?`(haiwezekani), usalama usiofaa (Dart 2.12+) |
| Ruby | `nil`(kila kitu ni cha ukweli isipokuwa`false`na`nil`) |
| Elixir | Hakuna null — tumia`nil`au muundo unaolingana |
| Elang | Hakuna null — tumia`undefined`au muundo unaolingana |