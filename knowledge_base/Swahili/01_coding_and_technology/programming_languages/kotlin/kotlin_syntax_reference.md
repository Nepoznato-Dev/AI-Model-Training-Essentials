...
|----------|------|---------|-------|
| `+``-``*``/``%`| Hesabu | `a + b`| |
| `..`| Masafa | `1..10`| Safu inayojumuisha |
| `until`| Masafa ya nusu wazi | `1 until 10`| `1..9`|
| `downTo`| Kiwango cha kushuka | `10 downTo 1`| |
| `==``!=` | Usawa wa kimuundo | `a == b`| Inapiga simu`.equals()`|
| `===``!==` | Usawa wa marejeleo | `a === b`| Mfano sawa |
| `<``>``<=``>=` | Kulinganisha | `a >= b`| Inahitaji`Comparable`|
| `&&``\|\|``!`| Mantiki | `a && b`| Mzunguko mfupi |
| `?:`| Opereta wa Elvis | `a ?: b`| Hurejesha`b`ikiwa`a`ni batili |
| `?.`| Simu salama | `a?.b`| Hurejesha batili ikiwa`a`ni batili |
| `!!`| Madai yasiyo batili | `a!!`| Hurusha NPE ikiwa null |
| `as`| Tuma | `a as String`| Hurusha ClassCastException |
| `as?`| Waigizaji salama | `a as? String`| Hurejesha null juu ya kutofaulu |
| `in`| Uanachama | `x in list`| |
| `!in`| Sio mwanachama | `x !in list`| |
| `is`| Andika hundi | `x is String`| Smart cast baada ya kuangalia |
| `!is`| Sio chapa | `x !is String`| |
### Kupakia kwa Opereta
```kotlin
...
## Muhtasari
Sintaksia ya Kotlin ni fupi, salama, na inashirikiana na Java. Usalama tupu huondoa darasa zima la mende kwa wakati wa kukusanya. Coroutines hutoa njia ya asili ya kuandika msimbo usiolingana. Madarasa yaliyofungwa, madarasa ya data na vitendaji vya viendelezi huwezesha uundaji wa kikoa unaoeleweka. Nguvu ya lugha inatokana na mwingiliano wa vipengele vyake - vitendaji vya upeo, DSL, madarasa ya ndani, na jenetiki zilizothibitishwa huchanganyika ili kutoa msimbo ambao ni salama na unaofurahisha kuandika.