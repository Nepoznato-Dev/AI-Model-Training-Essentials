...
|----------|------|---------|-------|
| `+``-``*``/``%`| Hesabu | `a + b`| |
| `**`| Nguvu (BigDecimal) | `BigDecimal(2) ** 10`| Haijajengwa ndani kwa Int |
| `==``!=` | Usawa | `a == b`| Inapigia simu`.equals()`— bila usalama |
| `eq``ne` | Usawa wa marejeleo | `a eq b`| Mfano sawa |
| `<``>``<=``>=` | Kulinganisha | `a >= b`| Inahitaji`Ordered`|
| `&&``\|\|``!`| Mantiki | `a && b`| Mzunguko mfupi |
| `&``\|``^``~` | Bitwise | `a & b`| |
| `<<``>>``>>>`| Shift | `a << 2`| |
| `+:``:+``::``:::` | Kazi za mkusanyiko | `1 +: list`| Ushirikiano wa kulia ukiisha na`:`|
| `->`| Ubunifu wa Tuple | `"key" -> "value"`| `("key", "value")`|
| `=>`| Kazi/mshale | `x => x + 1`| Lambda au chapa uchoraji ramani |
| `<-`| Jenereta | `x <- List(1,2,3)`| Kwa-ufahamu |
### Kanuni za Opereta
```scala
...
## Muhtasari
Sintaksia ya Scala huunganisha upangaji unaolenga kitu na utendakazi chini ya mfumo wa aina wenye nguvu. Ulinganishaji wa ruwaza, ufahamu, na aina za mifano hutoa muundo wa data unaoeleweka. Sifa huwezesha utunzi unaonyumbulika bila vikwazo vya urithi mmoja. Mfumo wa aina - wenye jenetiki, tofauti, mipaka ya aina, na aina zisizo wazi - hupata hitilafu wakati wa kukusanya huku kuwezesha uondoaji wa gharama sifuri. Scala 3 hurahisisha lugha kwa kutumia enums, mbinu za upanuzi, zinazotolewa/kutumia, na aina zisizo wazi, na kuifanya lugha iweze kufikiwa zaidi huku ikihifadhi uwezo wake kamili.