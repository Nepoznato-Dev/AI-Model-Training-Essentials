...
# Java - Makosa ya Kawaida & Miundo ya Kupambana
Hati hii inaorodhesha makosa ya kawaida, mitego, na mifumo ya kupingana katika Java. Kila ingizo linaonyesha njia isiyo sahihi, inaelezea kwa nini inashindwa, na hutoa suluhisho sahihi.
---

## 1.`equals()`dhidi ya`==`kwa Vitu
```java
...
## 2. Kurekebisha Mkusanyiko Wakati wa Kurudia
```java
...
## 3. Sio Kufunga Rasilimali (Jaribu-na-Rasilimali)
```java
...
## 4. Mitego ya Integer Autoboxing
```java
...
## 6. Muunganisho wa Kamba katika Vitanzi
```java
...
## 7. KutoshughulikiaIliyoingiliwaIla
```java
...
## 8. Kurudisha Batili Badala ya Mikusanyiko Tupu
```java
...
## 9. Vitu Vinavyoweza Kubadilishwa kama Vifunguo vya HashMap
```java
...
## 10. Anti-Pattern: Kukamata`Exception`au `Throwable`
```java
...
## 11. Sio Kupindua`hashCode`Wakati Unabatilisha `equals`
```java
...
## 12. Masharti ya Mbio kwa Kufunga Kwa Kukaguliwa Mara Mbili
```java
...
## Muhtasari
Utenzi wa Java unaweza kuficha hitilafu fiche:`==`dhidi ya`equals()`, urekebishaji wa wakati mmoja, uvujaji wa rasilimali, mitego ya utendaji wa ndondi otomatiki, na mkataba wa equals/hashCode. Mbinu ya kisasa ya Java ni: tumia`Objects.equals()`kwa ulinganisho usio salama, tumia kila wakati kujaribu-na-rasilimali, rudisha mikusanyo tupu badala ya kubatilisha, pata vighairi maalum, na ufuate kanuni ya uwajibikaji mmoja. Vipengele vya Java 8+ (mitiririko, Hiari, marejeleo ya mbinu) hupunguza sahani huku ikifanya msimbo kuwa salama na unaoeleweka zaidi.