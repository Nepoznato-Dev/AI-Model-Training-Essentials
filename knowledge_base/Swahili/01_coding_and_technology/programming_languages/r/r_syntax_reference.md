...
# R - Rejeleo la Sintaksia
Hati hii inatoa marejeleo ya kina, yenye muundo wa sintaksia ya R (4.x). Inakamilisha rejeleo kuu la R kwa kuangazia mifumo kamili ya sintaksia, mfumo safi wa mazingira, upotoshaji wa data, uundaji wa takwimu na taswira.
---
...
|----------|------|---------|-------|
| `+``-``*``/``^`| Hesabu | `2^10`| |
| `%%`| Moduli | `7 %% 3`| Inarudi 1 |
| `%/%`| Mgawanyiko kamili | `7 %/% 3`| Inarudi 2 |
| `==``!=` | Usawa | `x == y`| Vectorized |
| `<``>``<=``>=` | Kulinganisha | `x >= y`| Vectorized |
| `&``\|``!`| Ya kimantiki (iliyo na vekta) | `x & y`| Kipengele-busara |
| `&&``\|\|` | Mantiki (scalar) | `x && y`| Kipengele cha kwanza pekee |
| `%in%`| Uanachama | `x %in% y`| |
| `<-``=` | Kazi | `x <- 10`| `<-`ni nahau |
| `->`| Haki ya kukabidhi | `10 -> x`| Hutumika mara chache |
| `<<-`| Ugawaji wa kimataifa | `x <<- 10`| Weka env ya mzazi |
| `$`| Dondoo kipengele | `df$column`| |
| `[``[[` | Kuorodhesha | `x[1]`/`x[[1]]`| `[`inarudisha aina sawa;  Dondoo za`[[`|
| `%>%`| Bomba (magrittr) | `x %>% f()`| |
| `\|>`| Bomba (msingi R 4.1+) | `x \|> f()`| Bomba la asili |
### Operesheni Zilizowekwa Kivekta
```r
...
## Muhtasari
Sintaksia ya R imeundwa kwa ajili ya kompyuta ya takwimu na uchambuzi wa data. Hali ya utendakazi iliyowekewa vekta huondoa vitanzi kwa kazi nyingi. Mfumo safi wa ikolojia (dplyr, ggplot2, tidyr) hutoa sarufi thabiti, inayoweza kusomeka kwa upotoshaji na taswira ya data. Uwezo wa kielelezo wa takwimu wa R - kutoka majaribio ya msingi ya t hadi miundo ya athari mchanganyiko - haulinganishwi kwa kina. Kwa sayansi ya data, takwimu, na utafiti unaoweza kutolewa tena, R inasalia kuwa zana muhimu.