...
-->
# R - Makosa ya Kawaida & Miundo ya Kupinga
Hati hii inaorodhesha makosa ya kawaida, mitego, na mifumo ya kupingana katika R pamoja na masahihisho.
---

## 1. Kukuza Fremu za Data katika Mizunguko
```r
# ❌ WRONG — O(n²) growth
result <- data.frame()
for (i in 1:1000) {
...
## 3. Kutoelewa Usafishaji wa Vekta
```r
...
## 5. Kutotumia `stringsAsFactors = FALSE`
```r
...
## 7. Kutoshughulikia Maadili ya `NA`
```r
...
## Muhtasari
Asili ya mwingiliano ya R huunda mitego: kukuza fremu za data katika vitanzi (tenga mapema au weka vekta), maajabu ya kuchakata vekta, uenezi wa `NA`, na`T`/`F`inaweza kuandikwa tena. Njia ya R ni: vektari utendakazi, tenga mapema wakati wa kitanzi, shughulikia`NA`kwa uwazi, tumia mpangilio kwa upotoshaji wa data, na epuka`<<-`. R huwapa thawabu wale wanaofikiria katika vekta na kutumia mfumo wake tajiri wa ikolojia wa vifurushi.