...
|----------|------|---------|-------|
| `+``-``*``/` | Hesabu | `x + y`| Nambari ya aina |
| `div``mod` | Mgawanyiko kamili | `7 \`div\` 2`| Inarudi 3 |
| `^``**` | Nguvu | `2 ^ 10`| `^`kwa muhimu; `**`ya kuelea |
| `==``/=` | Usawa | `x == y`| Eq typeclass |
| `<``>``<=``>=` | Kulinganisha | `x >= y`| Aina ya aina |
| `&&``\|\|``not`| Mantiki | `a && b`| Mzunguko mfupi |
| `.`| Utungaji wa kazi | `(f . g) x`| `f (g x)`|
| `$`| Programu ya kazi | `f $ x + 1`| `f (x + 1)`- utangulizi wa chini |
| `<>`| Nyongeza ya Monoid | `"hello" <> " world"`| Semigroup |
| `<$>`| Kitendaji fmap | `(+1) <$> Just 5`| `fmap (+1) (Just 5)`|
| `<*>`| Ombi linatumika | `pure (+) <*> Just 3`| |
| `>>=`| Monad funga | `Just 5 >>= \x -> Just (x+1)`| |
| `>>`| Mlolongo wa Monad | `putStrLn "hi" >> putStrLn "bye"`| |
| `=<<`| Funga (imebadilishwa) | `f =<< mx`| Sawa na`mx >>= f`|
| `++`| Muunganisho wa orodha | `[1,2] ++ [3,4]`| O(n) katika orodha ya kushoto |
| `:`| Hasara | `1 : [2, 3]`| O(1) |
| `!!`| Kielezo | `[1,2,3] !! 1`| O(n) - kitendakazi cha sehemu |
| `\\`| Tofauti ya orodha | `[1,2,3] \\ [2]`| `[1,3]`|
### Utangulizi wa Opereta (umechaguliwa)
...
## Muhtasari
Sintaksia ya Haskell ni ndogo lakini inajieleza zaidi. Mfumo wa aina - wenye aina za data za aljebra, madarasa ya aina, GADTs, na aina za familia - hutumika kama ubainishaji na zana ya uthibitishaji. Kulinganisha muundo, tathmini ya uvivu, na monadi hutoa suluhisho maridadi kwa shida ngumu. Usafi wa Haskell (hakuna madhara katika aina) hurahisisha msimbo kufikiria, kujaribu, na kusawazisha. Ingawa mkondo wa kujifunza ni mwinuko, ushawishi wa Haskell unaenea katika lugha za kisasa - kutoka kwa hiari za Swift hadi madarasa yaliyofungwa ya Kotlin hadi mfumo wa aina ya Rust. Mastering Haskell hukufanya uwe mtayarishaji programu bora katika kila lugha.