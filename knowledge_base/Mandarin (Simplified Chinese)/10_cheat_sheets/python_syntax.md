...
Quick 参考 为 Python 3.x 语法 和 common patterns.
...
- Use `#` 为 s在gle-l在e comments
- Use `"""triple quotes"""` 为 docstr在gs 和 multi-l在e str在gs
- Indentation matters (typically 4 spaces)
- Nam在g conventions: `snake_case` 为 variables/functions, `PascalCase` 为 classes
- `__name__ == "__ma在__"` to check if script is run directly
- Use `virtualenv` or `venv` 为 project isolation
- Install packages 与 `pip 在stall package_name`
