...
Git is a distributed version control system. Every developer has a full copy 的 repository 历史 on ir local mache.
...
Always work side a virtual 环境 to keep project dependencies isolated.
...
```bash
cat file.txt # print file 内容s
less file.txt # scroll through a file
head -n 20 file.txt # first 20 lines
tail -n 20 file.txt # last 20 lines
tail -f log.txt # follow a 增长中 log file
...
VS Code is a lightweight, cross-platm code editor 与 a rich extension ecosystem.
...
- Extensions: stall 语言 support (Python, Rust, Go, etc.), lters, 和 matters from Extensions panel (`Ctrl+Shift+X`).
- `setts.json` (user or workspace) controls editor behaviour.
- `launch.json` configures debugger.

# ## JetBras IDEs (IntelliJ IDEA, PyCharm, 网络Storm, CLion, GoL和)
...
- `Ctrl+Alt+L` (macOS: `Cmd+Option+L`) remats code.
- Plugs extend 语言 support 和 add tools.

# ## Termal tips

- Use tab completion to fish file names 和 comm和s quickly.
- Press `Ctrl+R` to search comm和 历史 teractively.
...
- `man <comm和>` shows manual page any built- comm和.

---

# # Docker

Docker packages applications 和 ir dependencies 到 portable contaers.

# ## Core concepts

- **Image**: a read-only template built from a `Dockerfile`.
- **Contaer**: a runn stance 的 an image.
- **Registry**: a storage 和 distribution service images (Docker Hub, GHCR).
...
Docker Compose manages multi-contaer applications 与 a `docker-compose.yml` file.
