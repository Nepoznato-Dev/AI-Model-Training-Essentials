---
# Métadonnées
titre : « Problèmes de fiabilité du système »
description : « Défaillances du système et modèles de fiabilité »
...
#Classement
tags : [système, fiabilité, leçons des échecs]
...
## Fuites de mémoire
Les fuites de mémoire se produisent lorsque les programmes allouent de la mémoire mais ne parviennent pas à la libérer lorsqu'ils ne sont plus nécessaires, ce qui entraîne une croissance progressive de la consommation de mémoire.
### Écouteurs d'événements oubliés
**Mauvais exemple (JavaScript) :**```javascript
...
**Solution :** Utilisez des références faibles pour les références arrière```python
import weakref
...
Les conditions de concurrence se produisent lorsque le comportement du logiciel dépend du timing relatif des événements, tel que l'ordre d'exécution des threads.
### Vérifiez, puis agissez, condition de concurrence
**Mauvais exemple (Python) :**```python
...
### Condition de concurrence en lecture-modification-écriture
...
### Condition de course à l'initialisation paresseuse
**Mauvais exemple :**```python
...
**Prévention :** Acquérez toujours les verrous dans un ordre cohérent```python
# Both threads:
lock_a.acquire()
lock_b.acquire()
# ... critical section ...
...
**Solution :** Utilisez une planification équitable ou un vieillissement prioritaire
---

## Problèmes de performances
### Problème de requête N+1
**Mauvais exemple :**```python
# Fetch all users
...
1. **Utiliser RAII** : l'acquisition de ressources est un modèle d'initialisation
2. **Gestionnaires de contexte** : utilisez les instructions`with`pour les ressources
3. **Références faibles** : pour les caches et les références arrière
4. **Profilage** : profilage régulier de la mémoire en production
### Concurrence
1. **Réduire l'état partagé** : préférez la transmission de messages
2. **Utiliser des abstractions de haut niveau** : pools de threads, asynchrone/attendre
3. **Ordre de verrouillage** : évitez les blocages grâce à un ordre cohérent
4. **Tester simultanément** : utilisez des tests de stress et des détecteurs de course
### Performances
1. **Profil d'abord** : mesurez avant d'optimiser
2. **Choix de l'algorithme** : sélectionnez les structures de données appropriées
3. **Opérations par lots** : réduisez les allers-retours vers les bases de données/API
4. **Caching** : mettez en cache les calculs coûteux de manière appropriée
---

## Sujets connexes
- **Fulnérabilités de sécurité** : voir`security_vulnerabilities.md`pour les problèmes liés à la sécurité
- **Qualité du code** : voir`code_quality_issues.md`pour les problèmes de maintenabilité
- **Conception d'API** : voir`../07_api_system_design/api_system_design.md`pour les modèles d'architecture système
- **Échecs AI/LLM** : voir`ai_llm_failures.md`pour les problèmes de fiabilité spécifiques à l'IA
---

## Modèles supplémentaires de fiabilité du système
### Épuisement des ressources
**Qu'est-ce que c'est :** Épuisement des ressources système (descripteurs de fichiers, connexions, mémoire) via une allocation illimitée.
**Mauvais exemple :**```python
...
### Échecs en cascade
**Qu'est-ce que c'est :** La défaillance d'un composant déclenche des défaillances dans les composants dépendants.
**Mauvais exemple :**```python
...
**Que tester :**
1. **Échecs d'instance** : tuez des pods/VM aléatoires
2. **Problèmes de réseau** : ajoutez de la latence, supprimez des paquets
3. **Épuisement des ressources** : remplir le disque, épuiser la mémoire
4. **Échecs de dépendance** : simulation de pannes de service
5. **Décalage de l'horloge** : désynchronisez les horloges du serveur
**Exemple d'expérience de chaos :**```yaml
...
### Jours de match
Exercices réguliers pour tester la résilience du système :
1. **Calendrier** : Journées de match trimestrielles
2. **Portée** : environnement de type production
3. **Scénarios** : plusieurs échecs simultanés
4. **Mesures** : mesurez le temps de détection et le temps de récupération
5. **Apprentissages** : documenter et améliorer les runbooks
---
...
### Déchets de surapprovisionnement
**Mauvais exemple :**```markdown
...
### Prévention de l'épuisement professionnel sur appel
**Mauvaises pratiques :**
- Même personne d'astreinte chaque semaine
- Pas de chemin d'escalade
- Alertes pour les éléments non exploitables
- Des autopsies qui attribuent le blâme
...
2. Check for locks:
   ```sql
SELECT * FROM pg_locks WHERE accordé = false;   ```
