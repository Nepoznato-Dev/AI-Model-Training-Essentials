...
|--------------|------|---------|-------|
| `+``-``*``/``%`| Arithmétique | `a + b`| |
| `**`| Puissance (BigDecimal) | `BigDecimal(2) ** 10`| Non intégré pour Int |
| `==``!=` | Égalité | `a == b`| Appels`.equals()`— sécurité nulle |
| `eq``ne` | Égalité de référence | `a eq b`| Même exemple |
| `<``>``<=``>=` | Comparaison | `a >= b`| Nécessite`Ordered`|
| `&&``\|\|``!`| Logique | `a && b`| Court-circuit |
| `&``\|``^``~` | Au niveau du bit | `a & b`| |
| `<<``>>``>>>`| Changement | `a << 2`| |
| `+:``:+``::``:::` | Opérations de collecte | `1 +: list`| Associatif à droite si se termine par`:`|
| `->`| Création de tuples | `"key" -> "value"`| `("key", "value")`|
| `=>`| Fonction/flèche | `x => x + 1`| Lambda ou mappage de type |
| `<-`| Générateur | `x <- List(1,2,3)`| Pour-compréhension |
...
## Résumé
La syntaxe de Scala unifie la programmation orientée objet et fonctionnelle sous un système de types puissant. La correspondance de modèles, les compréhensions et les classes de cas fournissent une modélisation de données expressive. Les traits permettent une composition flexible sans limitations d’héritage unique. Le système de types – avec les génériques, la variance, les limites de type et les types opaques – détecte les erreurs au moment de la compilation tout en permettant des abstractions sans coût. Scala 3 simplifie le langage avec des énumérations, des méthodes d'extension, des types donnés/utilisant et opaques, rendant le langage plus accessible tout en conservant toute sa puissance.