...
# R — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour R (4.x). Il complète la référence principale de R en se concentrant sur les modèles de syntaxe exhaustifs, l'écosystème Tidyverse, la manipulation des données, la modélisation statistique et la visualisation.
---
...
|--------------|------|---------|-------|
| `+``-``*``/``^`| Arithmétique | `2^10`| |
| `%%`| Module | `7 %% 3`| Renvoie 1 |
| `%/%`| Division entière | `7 %/% 3`| Renvoie 2 |
| `==``!=` | Égalité | `x == y`| Vectorisé |
| `<``>``<=``>=` | Comparaison | `x >= y`| Vectorisé |
| `&``\|``!`| Logique (vectorisé) | `x & y`| Par élément |
| `&&``\|\|` | Logique (scalaire) | `x && y`| Premier élément uniquement |
| `%in%`| Adhésion | `x %in% y`| |
| `<-``=` | Affectation | `x <- 10`| `<-`est idiomatique |
| `->`| Attribuer le droit | `10 -> x`| Rarement utilisé |
| `<<-`| Affectation globale | `x <<- 10`| Attribuer dans l'environnement parent |
| `$`| Extraire l'élément | `df$column`| |
| `[``[[` | Indexation | `x[1]`/`x[[1]]`| `[`renvoie le même type ;  Extraits`[[`|
| `%>%`| Tuyau (magrittr) | `x %>% f()`| |
| `\|>`| Tuyau (base R 4.1+) | `x \|> f()`| Tuyau natif |
...
## Résumé
La syntaxe de R est conçue pour le calcul statistique et l'analyse de données. La nature vectorisée des opérations élimine les boucles pour la plupart des tâches. L'écosystème Tidyverse (dplyr, ggplot2, Tidyr) fournit une grammaire cohérente et lisible pour la manipulation et la visualisation des données. Les capacités de modélisation statistique de R (des tests t de base aux modèles à effets mixtes) sont inégalées en termes de profondeur. Pour la science des données, les statistiques et la recherche reproductible, R reste un outil essentiel.