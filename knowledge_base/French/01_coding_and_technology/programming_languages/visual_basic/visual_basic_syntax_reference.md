...
| **Logique** | `AndAlso`| Court-circuit ET | `a AndAlso b`|
| | `OrElse`| Court-circuit OU | `a OrElse b`|
| | `Not`| NON logique | `Not a`|
| | `And`| ET au niveau du bit | `a And b`|
| | `Or`| OU au niveau du bit | `a Or b`|
| | `Xor`| XOR au niveau du bit | `a Xor b`|
| **Chaîne** | `&`| Concaténation | `"Hello" & " " & "World"`|
| | `+=`| Ajouter | `s &= " more"`|
| **Affectation** | `=`| Attribuer | `x = 42`|
| **Nul** | `Is`| Égalité de référence | `obj Is Nothing`|
| | `IsNot`| Inégalité de référence | `obj IsNot Nothing`|
| **Tapez** | `TryCast`| Casting sécurisé (ne renvoie rien) | `TryCast(obj, String)`|
| | `CType`| Convertir | `CType(obj, Integer)`|
| | `DirectCast`| Casting direct (lancements en cas d'échec) | `DirectCast(obj, String)`|
---
...
## Résumé
La syntaxe de Visual Basic est claire et proche de l'anglais, ce qui en fait l'un des langages de programmation les plus lisibles. VB.NET offre un accès complet à l'écosystème .NET, notamment LINQ pour les requêtes de données, async/wait pour les opérations simultanées, des génériques pour la sécurité des types et une POO complète avec classes, interfaces et héritage. Le langage ne respecte pas la casse et les mots-clés verbeux (End If, End Sub, End Class) améliorent la lisibilité. Pour les applications Windows d'entreprise, la bureautique (VBA) et le prototypage rapide au sein de .NET, VB.NET reste un langage productif et performant.