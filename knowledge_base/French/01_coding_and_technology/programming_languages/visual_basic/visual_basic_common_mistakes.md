...
# Visual Basic – Erreurs courantes et anti-modèles
Ce document répertorie les erreurs, pièges et anti-modèles les plus courants dans Visual Basic (VB.NET) avec des corrections.
---

## 1. Ne pas disposer de ressources
```vb
' ❌ WRONG — resource leak
...
## 2.`=`vs`Is`pour la comparaison d'objets
```vb
...
## 3. Option strictement désactivée
```vb
...
## 4. Ne pas utiliser`Using`pour les connexions à la base de données
```vb
...
## 5. Anti-Pattern : logique métier dans les gestionnaires d'événements
```vb
...
## Résumé
Pièges VB.NET : utilisez toujours`Using`pour les ressources jetables, distinguez`=`(valeur) de`Is`(référence), activez`Option Strict On`, utilisez les blocs`Using`pour les connexions à la base de données et séparez la logique métier des gestionnaires d'événements de l'interface utilisateur. VB.NET moderne est un langage .NET puissant : utilisez correctement ses fonctionnalités.