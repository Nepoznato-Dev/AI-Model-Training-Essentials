...
|--------------|------|---------|--------|-------|
| `+`| Ajout | `3 + 2`| `5`| Également concaténation de chaînes/listes |
| `-`| Soustraction | `3 - 2`| `1`| Aussi négation unaire |
| `*`| Multiplications | `3 * 2`| `6`| Également répétition de chaîne/liste |
| `/`| Véritable division | `7 / 2`| `3.5`| Renvoie toujours float |
| `//`| Division d'étage | `7 // 2`| `3`| Arrondit vers l'infini négatif |
| `%`| Module | `7 % 2`| `1`| Le signe correspond au diviseur |
| `**`| Exponentiation | `2 ** 10`| `1024`| Association à droite |
### Comparaison et opérateurs booléens
| Opérateur | Nom | Exemple | Remarques |
|--------------|------|---------|-------|
| `==`| Égalité | `x == y`| Comparaison de valeur (pas d'identité) |
| `!=`| Pas égal | `x != y`| |
| `<`,`>`,`<=`,`>=`| Commande | `x >= y`| Chaînable :`a < b < c`|
| `is`| Identité | `x is y`| Même objet en mémoire |
| `is not`| Pas d'identité | `x is not None`| Préféré à`!= None`|
| `in`| Adhésion | `x in collection`| O(1) pour les ensembles/dicts, O(n) pour les listes |
| `and`| ET logique | `a and b`| Court-circuit ; renvoie la dernière valeur véridique/fausse |
| `or`| OU logique | `a or b`| Court-circuit ; renvoie la première valeur véridique/fausse |
...
| Collecte | Commandé | Mutable | Doublons | Syntaxe | Recherche |
|------------|---------|---------|------------|--------|--------|
| `list`| Oui | Oui | Oui | `[1, 2, 3]`| O(n) |
| `tuple`| Oui | Non | Oui | `(1, 2, 3)`| O(n) |
| `dict`| Oui (3,7+) | Oui | Clés : Non | `{"k": "v"}`| O(1) moyenne |
| `set`| Non | Oui | Non | `{1, 2, 3}`| O(1) moyenne |
| `frozenset`| Non | Non | Non | `frozenset({1,2})`| O(1) moyenne |
### Listes
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
## Résumé
Cette référence de syntaxe couvre l'ensemble complet des fonctionnalités de Python, depuis les opérateurs de base et le flux de contrôle jusqu'à la métaprogrammation avancée, les descripteurs et le système de types. La syntaxe de Python est délibérément minimale et lisible, mais sa profondeur est substantielle : la correspondance de modèles, les générateurs asynchrones, les protocoles et les métaclasses fournissent des outils puissants pour les applications complexes. Le langage continue d'évoluer, chaque version ajoutant des fonctionnalités qui améliorent l'expressivité tout en maintenant la compatibilité ascendante.