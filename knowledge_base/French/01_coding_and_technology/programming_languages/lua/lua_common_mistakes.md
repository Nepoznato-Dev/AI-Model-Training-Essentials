...
---
# Lua – Erreurs courantes et anti-modèles
Ce document répertorie les erreurs, pièges et anti-modèles les plus courants dans Lua avec des corrections.
---

## 1. Indexation basée sur 1
```lua
...
## 2.`nil`contre`false`dans les conditions
```lua
...
## 3. Sémantique de référence de table
```lua
...
## 4. Variables globales par défaut
```lua
...
## 7. Ne pas utiliser de métatables pour la POO
```lua
...
## Résumé
Le minimalisme de Lua crée des pièges : indexation basée sur 1, seuls`nil`et`false`sont faux, les tables sont des références (pas des valeurs), les variables sont globales par défaut et`pairs`n'est pas ordonné tandis que`ipairs`est ordonné. La méthode Lua est la suivante : utilisez toujours`local`, utilisez`ipairs`pour les tableaux et`pairs`pour les dictionnaires, utilisez`table.concat`pour la création de chaînes et utilisez des métatables pour la POO. La simplicité de Lua est sa puissance, mais respectez les cas extrêmes.