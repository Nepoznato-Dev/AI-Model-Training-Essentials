...
---
# Erlang & Elixir — Modèles idiomatiques et meilleures pratiques
Ce guide couvre les modèles idiomatiques pour Erlang et Elixir sur la VM BEAM.
---

## Expressions idiomatiques d'élixir
```elixir
# ✅ Pipe operator
result = data
...
## Résumé
Les idiomes Elixir mettent l'accent sur : l'opérateur de canal, la correspondance de modèles,`with`pour le chaînage d'erreurs et les spécifications de type `@spec`. Les idiomes Erlang mettent l'accent sur : la correspondance de modèles, la compréhension de listes, les rappels gen_server et la philosophie "let crash". Les deux valorisent l’immuabilité, la transmission de messages et la tolérance aux pannes.