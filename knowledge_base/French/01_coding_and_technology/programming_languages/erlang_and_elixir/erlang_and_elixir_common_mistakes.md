...
---
# Erlang & Elixir — Erreurs courantes et anti-modèles
Ce document répertorie les erreurs, pièges et anti-modèles les plus courants dans Erlang et Elixir avec des corrections.
---

## 1. Devoir unique (Erlang)
```erlang
...
## 2. Ne pas lier/surveiller les processus (Erlang)
```erlang
...
## 3. Appel GenServer sans délai d'attente (Elixir)
```elixir
...
## 4. Correspondance de modèle dans`receive`sans délai d'attente (Erlang)
```erlang
...
## 5. Ne pas utiliser`with`pour la correspondance de modèles imbriqués (Elixir)
```elixir
...
## 6. Anti-modèle : rappels de longue durée du GenServer
```elixir
...
## Résumé
Pièges Erlang/Elixir : les variables sont à affectation unique, les processus ont besoin de liens/moniteurs, les appels GenServer ont besoin de délais d'attente explicites,`receive`se bloque pour toujours sans délai d'attente et les rappels GenServer ne doivent pas bloquer. La méthode BEAM est la suivante : laissez-le planter (avec supervision), utilisez`with`pour une correspondance de modèle propre, gardez les rappels GenServer rapides et gérez toujours les échecs de processus.