...
## Registres et tailles de données
```nasm
...
## Macros et assemblage conditionnel
```nasm
; === NASM Macros ===
...
## Résumé
La syntaxe du langage assembleur est définie par l'architecture du processeur et l'assembleur (NASM, GAS, MASM). x86-64 fournit 16 registres à usage général, plusieurs modes d'adressage et des centaines d'instructions couvrant le mouvement des données, l'arithmétique, la logique, le flux de contrôle, SIMD et les appels système. La syntaxe NASM utilisée ici est la plus courante pour le développement Linux x86-64. La maîtrise de l'assemblage nécessite une compréhension des registres, de la pile, des conventions d'appel et de la façon dont les constructions de haut niveau sont mappées aux instructions machine.