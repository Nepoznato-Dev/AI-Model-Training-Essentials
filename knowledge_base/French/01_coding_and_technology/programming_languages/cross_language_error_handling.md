...
---
# Comparaison multilingue — Gestion des erreurs
## Présentation des modèles de gestion des erreurs
| Modèle | Langues | Mécanisme |
|-------|-----------|---------------|
| **Exceptions (essayer/cacher)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **Types de résultats/options** | Rust, Haskell, OCaml, Scala (également) | `Result<T, E>`,`Maybe a`|
| **Plusieurs valeurs de retour** | Allez, Lua | `value, err := f()`|
| **Codes d'erreur** | C, Fortran | Renvoyer le code d'erreur, vérifier manuellement |
| **Erreurs de correspondance de modèle** | Élixir, Erlang | `{:ok, value}`/`{:error, reason}`|
| **Conditions (redémarrable)** | Lisp commun | `restart-case`,`handler-bind`|
| **Aucune gestion des erreurs** | Scratch, SQL (de base) | N/A |
| **Contrats** | Ada, Eiffel |  Conditions`Pre`,`Post`|
## Gestion des erreurs basées sur les exceptions
```python
# Python
try:
    result = 10 / 0
...
## Résumé : Paradigmes de gestion des erreurs
| Paradigme | Approche | Avantages | Inconvénients |
|--------------|----------|------|------|
| **Exceptions** | lancer/attraper | Nettoyer le chemin heureux, empiler les traces | Peut être oublié, contrôler les abus de flux |
| **Types de résultats** | Retour`Ok`/`Err`| Explicite, ne peut pas ignorer les erreurs | Verbeux, opérateur`?`nécessaire |
| **Codes d'erreur** | Retourner le statut int | Simple, compatible C | Facile à ignorer, sans contexte |
| **Retours multiples** | `value, err := f()`| Explicite, idiomatique | Peut oublier de vérifier`err`|
| **Correspondance de modèles** | `{:ok, v}`/`{:error, r}`| exhaustif, fonctionnel | Verbeux sans`with`|
| **Conditions** | Gestionnaires redémarrables | Le plus puissant et récupérable | Complexe, rarement compris |