...
---
# Comparaison multilingue — Flux de contrôle
...
| Rouille | `for i in 0..10`|
| Aller | `for i := 0; i < 10; i++`|
| Java | `for (int i = 0; i < 10; i++)`|
| C/C++ | `for (int i = 0; i < 10; i++)`|
| Rubis | `10.times { \|i\| ... }`ou`(0..9).each { \|i\| ... }`|
| Rapide | `for i in 0..<10`|
| Kotlin | `for (i in 0 until 10)`|
| Haskell | `mapM_ f [0..9]`|
| Perl | `for my $i (0..9)`|
| R | `for (i in 1:10)`|
| PHP | `for ($i = 0; $i < 10; $i++)`|
| SQL | N/A (basé sur un ensemble, pas de boucles en SQL standard) |
| Frapper | `for i in {0..9}; do ... done`|
| Lua | `for i = 0, 9 do ... end`|
| MATLAB | `for i = 1:10 ... end`|
| Prologue | N/A (récursion à la place) |
| Lisp | `(dotimes (i 10) ...)`|
| Ada | `for I in 0..9 loop ... end loop;`|
| COBOL | `PERFORM VARYING I FROM 0 BY 1 UNTIL I >= 10`|
| Erlang | N/A (récursion + listes à la place) |
| OCaml | `for i = 0 to 9 do ... done`|
| Assemblée | Boucle manuelle avec CMP/JMP |
| Delphes | `for I := 0 to 9 do ... end;`|
| Gratter | `repeat (10) ... end`|
| VB | `For i As Integer = 0 To 9 ... Next`|
| Échelle | `for (i <- 0 until 10) ...`|
| Fléchette | `for (var i = 0; i < 10; i++)`|
| Julie | `for i in 0:9 ... end`|
| Tapuscrit | `for (let i = 0; i < 10; i++)`|
| C# | `for (int i = 0; i < 10; i++)`|
| Fortran | `do i = 1, 10 ... end do`|
## Correspondance de motifs / Changement
| Langue | Construire |
|--------------|---------------|
...
| Échelle | `x match { case 1 => ... }`|
| Rapide | `switch x { case 1: ... default: ... }`|
| Kotlin | `when (x) { 1 -> ... else -> ... }`|
| OCaml | `match x with 1 -> ... \| _ -> ...`|
| Erlang | `case X of 1 -> ...; _ -> ... end`|
| Élixir | `case x do 1 -> ...; _ -> ... end`|
| Fléchette | `switch (x) { 1 => ...; _ => ... }`(Dart 3.0) |
| PHP | `match($x) { 1 => ..., default => ... }`(PHP 8.0) |
| C# | `x switch { 1 => ..., _ => ... }`(C#8+) |
| Tapuscrit | Aucune correspondance de modèle natif (utiliser if/else) |
| Python | `match x: case 1: ... case _: ...`(Python 3.10+) |
| Prologue | Intégré à la correspondance de clause |
| Aller | `switch x { case 1: ... default: ... }`|