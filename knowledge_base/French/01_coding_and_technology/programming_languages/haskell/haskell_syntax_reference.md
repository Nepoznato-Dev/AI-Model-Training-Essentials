...
---
# Haskell — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour Haskell (GHC 9.x). Il complète la référence principale Haskell en se concentrant sur les modèles de syntaxe exhaustifs, le système de types, les monades, l'évaluation paresseuse et les idiomes de programmation fonctionnelle.
---
...
|--------------|------|---------|-------|
| `+``-``*``/` | Arithmétique | `x + y`| Nombre de classes de types |
| `div``mod` | Division entière | `7 \`div\` 2`| Renvoie 3 |
| `^``**` | Puissance | `2 ^ 10`| `^`pour intégral ; `**`pour flottant |
| `==``/=` | Égalité | `x == y`| Classe de type Eq |
| `<``>``<=``>=` | Comparaison | `x >= y`| Classe de type Ord |
| `&&``\|\|``not`| Logique | `a && b`| Court-circuit |
| `.`| Composition des fonctions | `(f . g) x`| `f (g x)`|
| `$`| Application de fonction | `f $ x + 1`| `f (x + 1)`— faible priorité |
| `<>`| Monoïde ajouter | `"hello" <> " world"`| Semigroupe |
| `<$>`| Foncteur fmap | `(+1) <$> Just 5`| `fmap (+1) (Just 5)`|
| `<*>`| Application applicative | `pure (+) <*> Just 3`| |
| `>>=`| Liaison monade | `Just 5 >>= \x -> Just (x+1)`| |
| `>>`| Séquence monade | `putStrLn "hi" >> putStrLn "bye"`| |
| `=<<`| Lier (inversé) | `f =<< mx`| Identique à`mx >>= f`|
| `++`| Concaténation de listes | `[1,2] ++ [3,4]`| O(n) dans la liste de gauche |
| `:`| Inconvénients | `1 : [2, 3]`| O(1) |
| `!!`| Indice | `[1,2,3] !! 1`| O(n) — fonction partielle |
| `\\`| Différence de liste | `[1,2,3] \\ [2]`| `[1,3]`|
### Priorité de l'opérateur (sélectionné)
| Priorité | Opérateurs |
|------------|-----------|
| 7 (le plus élevé) | `*``/``div``mod` |
| 6 | `+``-` |
| 5 | `:``++``\\`|
| 4 | `==``/=``<``>``<=``>=` |
| 3 | `&&`|
...
## Résumé
La syntaxe de Haskell est minimale mais extraordinairement expressive. Le système de types — avec les types de données algébriques, les classes de types, les GADT et les familles de types — sert à la fois de spécification et d'outil de vérification. La correspondance de modèles, l'évaluation paresseuse et les monades fournissent des solutions élégantes à des problèmes complexes. La pureté de Haskell (aucun effet secondaire dans les types) facilite le raisonnement, le test et le parallélisme du code. Bien que la courbe d'apprentissage soit abrupte, l'influence de Haskell imprègne les langages modernes – des options de Swift aux classes scellées de Kotlin en passant par le système de types de Rust. Maîtriser Haskell fait de vous un meilleur programmeur dans toutes les langues.