...
- **Fonctions pures** : aucun effet secondaire par défaut : les fonctions renvoient toujours la même sortie pour la même entrée.
- **Système de types** : l'un des langages les plus expressifs – détecte les bogues au moment de la compilation que d'autres langages ne peuvent pas.
- **Évaluation paresseuse** : les calculs sont différés jusqu'à ce qu'ils soient nécessaires — permet des structures de données infinies et une composition efficace.
- **Fondements mathématiques** : basés sur le calcul lambda et la théorie des catégories — les programmes sont plus proches des preuves mathématiques.
- **Influence** : les idées de Haskell ont façonné la plupart des langues modernes.
- **Concurrency** : la mémoire transactionnelle logicielle (STM) offre une programmation simultanée élégante.
## Les compromis
| Limitation | Détails | Solution de contournement typique |
|-----------|---------|-------------------|
| **Courbe d'apprentissage abrupte** | Monades, foncteurs, classes de types — très différents des langages impératifs | Investissez du temps ; les concepts sont transférables |
| **Surprises d'évaluation paresseuse** | Peut entraîner des problèmes inattendus d'utilisation de la mémoire et de performances | Utiliser une évaluation stricte (`!`) si nécessaire |
| **Écosystème plus petit** | Moins de bibliothèques que Python, Java ou JavaScript | Le piratage se développe ; de nombreux forfaits de qualité |
| **Marché du travail** | Niche – principalement des travaux de finance, de recherche et de compilateur | Croissance dans les communautés de programmation fonctionnelle |
| **Vitesse de compilation** | GHC peut être lent pour les grands projets | Utilisez GHCi pour le développement interactif |
---
...
### Familles de types — Fonctions au niveau du type
Les familles de types vous permettent de calculer des types au niveau du type, de la même manière que les fonctions calculent des valeurs au niveau de la valeur.
```haskell
...
### GADT — Types de données algébriques généralisées
Les GADT vous permettent de spécifier avec précision le type de retour de chaque constructeur, permettant ainsi des interpréteurs de type sécurisé et des DSL intégrés.
```haskell
...
### Transformateurs Monade
Les transformateurs Monad empilent des effets monadiques, vous permettant de combiner les E/S, l'état, la gestion des erreurs, etc.
```haskell
...
### Hiérarchie des foncteurs, des applicatifs et des monades
```haskell
...
### Évaluation paresseuse ou stricte
```haskell
...
STM fournit des transactions composables pour l'état partagé, comme les transactions de base de données mais pour la mémoire.
```haskell
...
### Opérations asynchrones simultanées
```haskell
...
### Structure du projet (Pile/Cabale)
```
my-haskell-project/
├── app/
...
### Configuration de la pile (stack.yaml)
```yaml
# stack.yaml
...
### Description du package (package.yaml pour hpack)
```yaml
# package.yaml
...
| Commande | Descriptif |
|---------|-------------|
| `stack new my-project`| Créer un nouveau projet à partir d'un modèle |
| `stack build`| Construire le projet |
| `stack ghci`| Démarrez REPL interactif avec le projet chargé |
| `stack test`| Exécuter la suite de tests |
| `stack bench`| Exécuter des benchmarks |
| `stack haddock`| Générer de la documentation |
| `stack exec my-app`| Exécutez l'exécutable |
| `stack clean`| Nettoyer les artefacts de construction |
| `stack update`| Mettre à jour l'index du package |
| `stack freeze`| Épingler les versions exactes des dépendances |
...
## Tests
### HSpec — Cadre de tests unitaires
HSpec est le framework de test le plus populaire, inspiré du RSpec de Ruby. Il fournit une syntaxe de style BDD.
```haskell
-- test/Spec.hs
import Test.Hspec
...
### Tests basés sur les propriétés avec QuickCheck
QuickCheck génère des cas de test aléatoires pour vérifier les propriétés de votre code.
```haskell
import Test.QuickCheck
...
| Commande | Descriptif |
|---------|-------------|
| `stack test`| Exécuter toutes les suites de tests |
| `stack test --fast`| Ignorer les optimisations pour des builds de tests plus rapides |
| `stack build --test --test-arguments "--color"`| Exécutez des tests avec une sortie colorée |
| `stack ghci --test`| Charger les modules de test dans REPL |

---

## Interopérabilité
### Interface de fonction étrangère C (FFI)
Haskell peut appeler des fonctions C directement et exposer les fonctions Haskell à C.
```haskell
...
### Python Interop (via inline-c ou HPy)
```haskell
...
### Final sans étiquette (DSL intégrés)
Le style final sans balise code les DSL à l'aide de classes de types, permettant de multiples interprétations.
```haskell
...
### Monades gratuites
Des monades libres séparent la description des effets de leur interprétation.
```haskell
...
### Modèle ReaderT pour l'architecture d'application
Le modèle ReaderT est l’approche Haskell moderne de l’injection de dépendances.
```haskell
{-# LANGUAGE OverloadedStrings #-}
...
|------|---------|---------|
| **Profileur GHC** | Profilage du temps et des allocations | `stack build --profile`puis`./app +RTS -p`|
| **ThreadScope** | Visualisez l'exécution parallèle | `./app +RTS -l`puis ouvrez`app.eventlog`|
| **ghc-événements** | Analyser les journaux d'événements | `ghc-events show app.eventlog`|
| **Critère** | Analyse comparative statistique | Utiliser le package`criterion`|
| **hp2jolie** | Visualiser les profils de tas | `./app +RTS -h`puis`hp2pretty app.hp`|
### Analyse comparative avec Criterion
```haskell
-- bench/Bench.hs
...
## Déploiement
### Création des binaires de version
```bash
...
## Quand utiliser Haskell
| Scénario | Pourquoi Haskell | Meilleure alternative |
|----------|-----------|-------------------|
| Vérification formelle | Le système de types permet des preuves | Agda, Coq |
| Développement de compilateur | Excellent pour la mise en œuvre du langage | OCaml, Rouille |
| Systèmes financiers | Exactitude à travers les types | Scala, Fa# |
| Apprentissage des concepts de PF | Le langage fonctionnel le plus pur | Scala (plus pratique), Elm |
...
## Résumé
Haskell est l'expression la plus pure de la programmation fonctionnelle dans un langage grand public. Son système de types est parmi les plus puissants et l'accent mis sur les fonctions pures produit un code plus facile à raisonner et à tester. Même si Haskell n’est pas largement utilisé dans l’industrie, ses idées ont profondément influencé la programmation moderne. Apprendre Haskell change votre façon de penser la programmation, même si vous ne l'utilisez jamais professionnellement.