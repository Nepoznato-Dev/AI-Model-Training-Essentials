...
---
# Perl — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour Perl (5.38+). Il complète la référence principale de Perl en se concentrant sur les modèles de syntaxe exhaustifs, les expressions régulières, les références et la puissance de traitement de texte de Perl.
---
...
| `x`| Répétition de chaînes | `"ab" x 3`| `"ababab"`|
| `==``!=``<``>``<=``>=` | Comparaison numérique | `$a == $b`| |
| `eq``ne``lt``gt``le``ge` | Comparaison de chaînes | `$a eq $b`| |
| `<=>`| Vaisseau spatial numérique | `$a <=> $b`| Renvoie -1, 0, 1 |
| `cmp`| Vaisseau spatial à cordes | `$a cmp $b`| |
| `&&``\|\|``!`| Logique | `$a && $b`| |
| `and``or``not`| Logique (faible précision) | `$a and $b`| À éviter : utilisez`&&`/`\|\|`|
| `=~``!~` | Correspondance d'expression régulière | `$str =~ /pattern/`| |
| `..``...` | Gamme / bascule | `1..10`| |
| `?:`| Ternaire | `$a ? $b : $c`| |
| `//`| Défini-ou | `$a // $b`| `$b`si`$a`est undef |
| `=>`| Grosse virgule | `key => 'value'`| Citations automatiques mots nus |
### Sceaux — Types de variables
```perl
...
## Références et structures de données
```perl
# Array reference
my $aref = [1, 2, 3];
push @$aref, 4;
...
## Résumé
La syntaxe de Perl est construite autour du contexte, des sceaux et du principe « Il existe plusieurs façons de le faire ». Les expressions régulières sont intégrées au langage, ce qui rend le traitement de texte d'une concision inégalée. Les références permettent des structures de données complexes. CPAN fournit des modules pour tout. Le Perl moderne (avec`strict`,`warnings`et variables lexicales) est propre et maintenable. L'héritage de Perl perdure dans les modèles et les outils que chaque langage moderne a adoptés.