...
---
# Go — Erreurs courantes et anti-modèles
Ce document répertorie les erreurs, pièges et anti-modèles les plus courants dans Go. Chaque entrée montre l'approche incorrecte, explique pourquoi elle échoue et fournit la solution correcte.
---

## 1. Capture de variables en boucle dans Goroutines
```go
...
## 2. Ignorer les retours d'erreur
```go
...
## 5. Déréférencement de pointeur nul sur les méthodes Struct
```go
...
## 6. Utiliser`defer`dans une boucle
```go
...
## 7. Anti-Pattern : renvoi prématuré de variables nommées
```go
...
## 8. L'ordre des itérations de la carte est aléatoire
```go
...
## 10. Anti-modèle : pollution de l'interface
```go
...
## 11. Ne pas utiliser`sync.Once`pour l'initialisation
```go
...
## Résumé
La simplicité de Go est sa force, mais il présente des pièges subtils : des fuites de goroutines à partir de canaux non tamponnés, une sémantique d'ajout de tranches qui ne modifie pas l'original, des appels différés s'accumulant dans des boucles et une itération de carte intentionnellement aléatoire. Les proverbes Go montrent la voie : gérer chaque erreur, accepter les interfaces mais renvoyer les structures, utiliser des récepteurs de pointeurs pour les types mutex et préférer les primitives de concurrence (`sync.Once`,`errgroup`, annulation de contexte) à la gestion manuelle des goroutines. Écrivez d’abord du code simple – Go récompense la clarté plutôt que l’intelligence.