...
# Rust — Modèles idiomatiques et meilleures pratiques
Ce guide couvre les modèles idiomatiques et les meilleures pratiques pour écrire du code Rust propre et sûr.
---
...
## Résumé
Les idiomes Rust mettent l'accent sur : la propriété et l'emprunt,`Result`/`Option`pour la gestion des erreurs, les énumérations avec les méthodes, les traits pour l'abstraction, le chaînage des itérateurs, les pointeurs intelligents pour l'état partagé et les abstractions sans coût. Suivez`rustfmt`pour le formatage,`clippy`pour le peluchage et les directives de l'API Rust. La communauté Rust valorise la sécurité, les performances et l'exactitude : « si ça compile, ça marche ». Préférez`&str`à`String`dans les arguments de fonction, utilisez`?`pour la propagation des erreurs et dérivez des traits communs avec`#[derive(Debug, Clone, ...)]`.