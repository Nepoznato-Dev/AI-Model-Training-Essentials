...
|--------------|------|---------|-------|
| `+`| Ajout | `3 + 2`| Également la concaténation`String`via`format!`|
| `-`| Soustraction | `3 - 2`| |
| `*`| Multiplications | `3 * 2`| |
| `/`| Division | `7 / 2`| Division entière pour les entiers :`3`|
| `%`| Reste | `7 % 2`| `1`|
| `-x`| Négation | `-5`| |
### Comparaison et opérateurs logiques
| Opérateur | Nom | Exemple | Remarques |
|--------------|------|---------|-------|
| `==`| Égalité | `x == y`| Nécessite`PartialEq`|
| `!=`| Pas égal | `x != y`| |
| `<`,`>`,`<=`,`>=`| Commande | `x >= y`| Nécessite`PartialOrd`|
...
| `\|`| OU | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `!`| PAS | `!0b1010u8`| Complément au niveau du bit |
| `<<`| Maj gauche | `5 << 1`| `10`|
| `>>`| Maj droite | `5 >> 1`| `2`|
### Priorité des opérateurs (de la plus élevée à la plus basse)
| Priorité | Opérateurs | Remarques |
|------------|-----------|-------|
| 1 (le plus élevé) |  Chemins`()``.` :: | |
| 2 | Appels de méthodes, accès aux champs | Déref automatique |
| 3 | `-`(négation),`!`,`*`(deref) | |
| 4 | `as`(type moulé) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
| 14 | `..``..=` | Gamme |
| 15 | `=``+=` etc. | Affectation |
| 16 (le plus bas) | `return``break``continue`| |
---
...
### Présentation des types intégrés
| Tapez | Pile/Tas | Taille | Mutabilité | Remarques |
|------|-----------|------|------------|-------|
| Primitives (`i32`,`f64`,`bool`,`char`) | Pile | Fixe | Immuable par défaut | `Copy`|
| `[T; N]`(tableau) | Pile | Fixe | Mutable avec`mut`| Taille fixe,`Copy`si T est`Copy`|
| `Vec<T>`| Tas | Dynamique | Mutable avec`mut`| Cultivable |
| `String`| Tas | Dynamique | Mutable avec`mut`| UTF-8, extensible |
| `&[T]`(tranche) | Pile (gros ptr) | Vue dynamique | Dépend de`&`ou`&mut`| Vue empruntée |
| `HashMap<K, V>`| Tas | Dynamique | Mutable avec`mut`| O(1) recherche moyenne |
| `BTreeMap<K, V>`| Tas | Dynamique | Mutable avec`mut`| Recherche O(log n), ordonnée |
| `HashSet<T>`| Tas | Dynamique | — | O(1) moy, pas de doublons |
### Vecteurs
```rust
// Creation
...
## Caractéristiques et génériques
...
### Caractéristiques communes de la bibliothèque standard
```rust
...
## Gestion des erreurs
### Modèles de résultats et d'options
```rust
...
## Concurrence
### Fils de discussion et transmission de messages
```rust
use std::thread;
...
## Résumé
La syntaxe de Rust repose sur une base de garanties de sécurité appliquées au moment de la compilation. Le système de propriété, le polymorphisme basé sur les traits, la correspondance de modèles et les abstractions à coût nul forment une conception cohérente dans laquelle le compilateur est un partenaire strict mais équitable. Depuis le flux de contrôle de base jusqu'aux fonctionnalités avancées telles que les macros, le code non sécurisé et les pointeurs intelligents, chaque construction sert l'objectif d'une programmation système sûre et efficace. Comprendre l'interaction entre les durées de vie, les emprunts et le système de types est la clé pour maîtriser Rust.