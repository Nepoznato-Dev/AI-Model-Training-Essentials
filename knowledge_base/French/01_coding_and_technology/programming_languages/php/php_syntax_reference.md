...
---
# PHP — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour PHP (8.x). Il complète la principale référence PHP en se concentrant sur des modèles de syntaxe exhaustifs, des fonctionnalités PHP modernes, une programmation orientée objet et des idiomes pratiques.
---
...
| `==``!=``===``!==` | Égalité | `$a === $b`| `===`vérifie également le type |
| `<=>`| Vaisseau spatial | `$a <=> $b`| Renvoie -1, 0 ou 1 |
| `<``>``<=``>=` | Comparaison | `$a >= $b`| |
| `&&``\|\|``!`| Logique | `$a && $b`| Court-circuit |
| `and``or``xor`| Logique (faible priorité) | `$a and $b`| À éviter : utilisez`&&`/`\|\|`|
| `??`| Coalescence nulle | `$a ?? 'default'`| Renvoie`'default'`si`$a`est nul |
| `??=`| Affectation de fusion nulle | `$a ??= 'default'`| Attribué uniquement si`$a`est nul |
| `?``:` | Ternaire | `$a ? $b : $c`| |
| `.`| Concaténation | `$a . $b`| |
| `.=`| Concaténation assigner | `$a .= $b`| |
| `=>`| Affectation de tableau | `['key' => 'value']`| |
| `->`| Accès membres | `$obj->method()`| |
| `?->`| Opérateur Nullsafe | `$obj?->method()`| Renvoie null si`$obj`est nul |
| `::`| Accès statique | `ClassName::method()`| |
| `instanceof`| Vérification du type | `$obj instanceof ClassName`| |
| `clone`| Clonage d'objets | `$copy = clone $obj`| La copie complète nécessite`__clone()`|
### Jonglerie de types
```php
...
## Résumé
La syntaxe de PHP a considérablement évolué depuis ses origines de script vers un langage moderne et sécurisé. PHP 8.x a introduit les types d'union, les types d'intersection, les énumérations, les fibres, les classes en lecture seule, les attributs et les callables de première classe, le transformant d'un langage de script faiblement typé en un langage prenant en charge une ingénierie logicielle rigoureuse. La force du langage reste son orientation Web : avec des frameworks comme Laravel et Symfony, un écosystème massif et un support d'hébergement quasi universel, la syntaxe de PHP sert des millions d'applications. Le PHP moderne récompense les développeurs qui adoptent son système de types, utilisent le chargement automatique basé sur le compositeur et écrivent du code propre et orienté objet.