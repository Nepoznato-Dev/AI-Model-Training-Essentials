...
---
# TypeScript - Erreurs courantes et anti-modèles
Ce document répertorie les erreurs, pièges et anti-modèles les plus courants dans TypeScript. Chaque entrée montre l'approche incorrecte, explique pourquoi elle échoue et fournit la solution correcte.
---
...
## 2. Assertions de type au lieu de gardes de type
```typescript
...
## 3. Abus d'assertions non nulles
```typescript
...
## 4. Confusion des interfaces et des types
```typescript
...
## 5. Énumération des anti-modèles
```typescript
...
## 6. Ne pas utiliser le mode `strict`
```json
...
## 7. Surprises de frappe structurelle
```typescript
...
## 9. Anti-modèle : types répétitifs (sans utilisation de génériques)
```typescript
...
## 10. Chaînage facultatif excessif
```typescript
...
## 11. Utilisation de`namespace`(héritage)
```typescript
...
## 12. Ne pas tirer parti des syndicats discriminés
```typescript
...
## Résumé
Le système de types de TypeScript est puissant mais nécessite de la discipline. Le péché cardinal est d'atteindre`any`— utilisez`unknown`lorsque le type est vraiment inconnu et restreint avec des gardes de type. Évitez les assertions non nulles (`!`) en gérant correctement undefined. Utilisez des interfaces pour les formes d'objets, des types pour les unions et des types calculés. Des syndicats discriminés éliminent des classes entières de microbes. Activez`strict: true`dès le premier jour : la rigueur de la mise à niveau est douloureuse. L’objectif est de rendre les États illégaux irreprésentables au niveau du type.