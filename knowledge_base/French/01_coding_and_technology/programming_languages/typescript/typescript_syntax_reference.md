...
---
# TypeScript — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour TypeScript. Il complète la référence principale de TypeScript en se concentrant sur le système de types, les génériques, les types utilitaires et la programmation avancée au niveau des types.
---

## Fondamentaux du système de saisie
### Types primitifs et intégrés
```typescript
// Primitives
...
### Types d'utilitaires personnalisés
```typescript
...
## Décorateurs (TC39 Stage 3 / TypeScript 5.0+)
```typescript
// Class decorator
...
### Assertions Const et rétrécissement
```typescript
...
## Résumé
Le système de types de TypeScript est sa caractéristique déterminante, et il est particulièrement profond. Des annotations de base aux génériques, aux types conditionnels, aux types mappés, aux types littéraux de modèles et à`infer`, le système de types est essentiellement un langage fonctionnel fonctionnant au moment de la compilation. Comprendre l'interaction entre`interface`et`type`, les types génériques et utilitaires et le typage structurel par rapport au typage nominal permet d'écrire du TypeScript à la fois sûr et ergonomique. Le langage continue d'évoluer, chaque version ajoutant plus de puissance expressive tout en conservant une compatibilité ascendante totale avec JavaScript.