...
# Kotlin — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour Kotlin (2.0+). Il complète la référence principale de Kotlin en se concentrant sur les modèles de syntaxe exhaustifs, la sécurité nulle, les coroutines et les idiomes Kotlin.
---
...
|--------------|------|---------|-------|
| `+``-``*``/``%`| Arithmétique | `a + b`| |
| `..`| Gamme | `1..10`| Gamme inclusive |
| `until`| Gamme semi-ouverte | `1 until 10`| `1..9`|
| `downTo`| Gamme décroissante | `10 downTo 1`| |
| `==``!=` | Égalité structurelle | `a == b`| Appels`.equals()`|
| `===``!==` | Égalité référentielle | `a === b`| Même exemple |
| `<``>``<=``>=` | Comparaison | `a >= b`| Nécessite`Comparable`|
| `&&``\|\|``!`| Logique | `a && b`| Court-circuit |
| `?:`| Opérateur Elvis | `a ?: b`| Renvoie`b`si`a`est nul |
| `?.`| Appel sécurisé | `a?.b`| Renvoie null si`a`est nul |
| `!!`| Assertion non nulle | `a!!`| Lance NPE si nul |
| `as`| Casting | `a as String`| Lève ClassCastException |
| `as?`| Casting sécurisé | `a as? String`| Renvoie null en cas d'échec |
| `in`| Adhésion | `x in list`| |
| `!in`| Non membre | `x !in list`| |
| `is`| Vérification du type | `x is String`| Casting intelligent après vérification |
| `!is`| Ne pas taper | `x !is String`| |
...
## Génériques et système de types
```kotlin
// Generic function
...
## Résumé
La syntaxe de Kotlin est concise, sûre et interopérable avec Java. La sécurité nulle élimine toute une classe de bogues au moment de la compilation. Les coroutines offrent un moyen naturel d'écrire du code asynchrone. Les classes scellées, les classes de données et les fonctions d'extension permettent une modélisation de domaine expressive. La puissance du langage vient de l'interaction de ses fonctionnalités : les fonctions de portée, les DSL, les classes en ligne et les génériques réifiés se combinent pour produire un code à la fois sûr et agréable à écrire.