...
# Dart — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour Dart (3.x). Il complète la référence principale de Dart en se concentrant sur les modèles de syntaxe exhaustifs, la sécurité nulle, la programmation asynchrone et les modèles de conception orientés Flutter.
---
...
|--------------|------|---------|-------|
| `+``-``*``/``%``~/` | Arithmétique | `7 ~/ 3`| `~/`est une division entière |
| `==``!=` | Égalité | `a == b`| |
| `<``>``<=``>=` | Comparaison | `a >= b`| |
| `&&``\|\|``!`| Logique | `a && b`| Court-circuit |
| `??`| Coalescence nulle | `a ?? b`| `b`si`a`est nul |
| `?.`| Accès nul | `a?.b`| null si`a`est nul |
| `!`| Assertion nulle | `a!`| Lance si nul |
| `..``?..` | Cascade | `obj..a..b`| Opérations en chaîne sur le même objet |
| `??=`| Affectation prenant en compte les valeurs nulles | `a ??= b`| Attribuer si nul |
| `as`| Type moulé | `obj as String`| Jette sur l'échec |
| `is``is!` | Essai de type | `obj is String`| |
| `=>`| Fonction flèche | `() => expr`| |
### Notation en cascade
```dart
...
## Méthodes et modèles d'extension
```dart
// Extension methods
...
## Résumé
La syntaxe de Dart est propre, cohérente et conçue pour la productivité. Une sécurité nulle saine élimine les erreurs de référence nulles au moment de la compilation. Async/wait et streams fournissent des modèles naturels pour la programmation asynchrone. Les classes, mixins et classes scellées permettent une composition flexible des objets. La correspondance de modèles, les enregistrements et les modificateurs de classe de Dart 3 ajoutent de l'expressivité. Bien que l'objectif principal de Dart soit Flutter, sa syntaxe est bien adaptée à n'importe quel domaine d'application : côté serveur (avec Dart Frog ou Shelf), outils CLI et applications Web (compilées en JavaScript).