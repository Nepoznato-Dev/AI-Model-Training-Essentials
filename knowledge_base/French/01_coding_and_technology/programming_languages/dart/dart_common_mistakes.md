...
## 1. Utilisation abusive de la sécurité nulle
```dart
// ❌ WRONG — force unwrap
...
## 2. Async/Await sans gestion des erreurs
```dart
...
## 3. Ne pas utiliser les constructeurs `const`
```dart
...
## 5. Ne pas utiliser Spread et Collection-if
```dart
...
## 6. Anti-modèle : Massive StatefulWidget
```dart
...
## Résumé
La sécurité nulle de Dart est puissante – ne la battez pas avec`!`. Utilisez les constructeurs`const`pour les instances canoniques, remplacez toujours`hashCode`par`==`, utilisez les opérateurs collection-if et spread pour la création de listes concises et extrayez la logique métier des widgets. Dart récompense un code propre, sans danger pour les valeurs nulles et correct en const.