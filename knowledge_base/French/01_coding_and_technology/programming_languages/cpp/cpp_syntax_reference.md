...
Ce document fournit une référence de syntaxe complète et structurée pour le C++ moderne (C++17/20/23). Il complète la référence principale C++ en se concentrant sur des modèles de syntaxe exhaustifs, des tables d'opérateurs, des mécanismes de modèles et des fonctionnalités modernes.
---
...
|--------------|------|---------|-------|
| `+``-``*``/``%`| Arithmétique | `a + b`| Identique à C |
| `++``--` | Incrémenter/Décrémenter | `++i`,`i++`| Préférer le pré-incrément |
| `==``!=``<``>``<=``>=` | Comparaison | `a <=> b`| `<=>`est une comparaison à trois voies (C++20) |
| `&&``\|\|``!`| Logique | `a && b`| Court-circuit |
| `&``\|``^``~``<<``>>` | Au niveau du bit | `a & b`| Identique à C |
| `?:`| Ternaire | `cond ? a : b`| |
### Opérateurs spécifiques au C++
| Opérateur | Nom | Exemple | Remarques |
|--------------|------|---------|-------|
| `::`| Résolution de la portée | `std::cout`| |
| `.*``->*` | Pointeur vers membre | `obj.*pmf`| |
| `<=>`| Vaisseau spatial (C++20) | `a <=> b`| Retours`strong_ordering`|
| `co_await`| Coroutine suspendre | `co_await expr`| C++20 |
| `co_yield`| Rendement de la coroutine | `co_yield value`| C++20 |
| `co_return`| Retour de coroutine | `co_return value`| C++20 |
...
## Flux de contrôle
### Reliures structurées et motifs modernes
```cpp
...
## Fonctions et Lambda
### Syntaxe des fonctions modernes
```cpp
...
| Conteneur | En-tête | Commandé | Recherche | Remarques |
|-----------|--------|---------|--------|-------|
| `vector`| `<vector>`| Oui | O(n) | Choix par défaut pour les séquences |
| `deque`| `<deque>`| Oui | O(n) | Push/pop rapide aux deux extrémités |
| `list`| `<list>`| Oui | O(n) | Liste doublement chaînée |
| `array`| `<array>`| Oui | O(n) | Taille fixe, alloué par pile |
| `set`/`multiset`| `<set>`| Oui | O(logn) | Arbre rouge-noir |
| `map`/`multimap`| `<map>`| Oui (par clé) | O(logn) | Valeur-clé, arbre rouge-noir |
| `unordered_set`| `<unordered_set>`| Non | O(1) moyenne | Table de hachage |
| `unordered_map`| `<unordered_map>`| Non | O(1) moyenne | Table de hachage |
### Algorithmes clés
```cpp
#include <algorithm>
#include <numeric>
...
## Résumé
La syntaxe C++ s'étend de la manipulation de pointeurs de bas niveau à la métaprogrammation de modèles de haut niveau et à l'asynchrone basée sur la coroutine. L'évolution du langage via C++ 17/20/23 a considérablement amélioré la sécurité (pointeurs intelligents,`std::optional`, concepts), l'expressivité (liaisons structurées, plages,`std::format`) et les performances (concepts permettant des abstractions sans coût, coroutines pour l'asynchrone sans surcharge de pile). La philosophie de base reste inchangée : vous ne devez pas payer pour ce que vous n'utilisez pas, et le langage doit prendre en charge tous les niveaux d'abstraction, du bare metal aux langages spécifiques à un domaine.