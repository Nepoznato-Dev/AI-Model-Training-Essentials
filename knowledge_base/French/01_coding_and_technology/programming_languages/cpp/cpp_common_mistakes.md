...
Ce document répertorie les erreurs, pièges et anti-modèles les plus courants en C++. Chaque entrée montre l'approche incorrecte, explique pourquoi elle échoue et fournit la solution correcte. Le C++ moderne (C++11/14/17/20) fournit des outils pour éviter la plupart des pièges classiques.
---

## 1. Règle des trois/cinq violations
```cpp
...
## 2.`new`/`delete`bruts au lieu de pointeurs intelligents
```cpp
...
## 6. Ne pas utiliser correctement `std::move`
```cpp
...
## 7. Anti-Pattern : Utilisation de`using namespace std;`dans les en-têtes
```cpp
...
## 8. Comportement non défini en cas de débordement signé
```cpp
...
## 9. Ne pas comprendre les catégories de valeur
```cpp
...
## 10. Anti-Pattern : gestion manuelle des ressources (pré-RAII)
```cpp
...
## 11. Gonflement des modèles et temps de compilation longs
```cpp
...
## 12. Ne pas utiliser`[[nodiscard]]`et `[[maybe_unused]]`
```cpp
...
## Résumé
La complexité du C++ crée de nombreuses opportunités d'erreurs : la règle de cinq, le découpage d'objets, l'invalidation d'itérateur, les références pendantes et le comportement indéfini dû au débordement de signatures. Le C++ moderne est l'antidote : utilisez des pointeurs intelligents au lieu de`new`/`delete`bruts, RAII pour toutes les ressources,`std::vector`et`std::string`au lieu de tableaux bruts, déplacez la sémantique pour plus d'efficacité et`[[nodiscard]]`pour capturer les valeurs de retour ignorées. Les directives de base C++ existent pour une raison : suivez-les et utilisez des outils comme Clang-Tidy et des désinfectants pour détecter ce que vous manquez.