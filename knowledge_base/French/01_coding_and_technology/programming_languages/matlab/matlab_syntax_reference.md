...
# MATLAB — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour MATLAB (R2024+). Il complète la référence principale MATLAB en se concentrant sur les modèles de syntaxe exhaustifs, les opérations matricielles, le traçage et les idiomes de calcul scientifique.
---
...
|--------------|------|---------|-------|
| `+``-``*``/``^`| Arithmétique | `A * B`| Multiplication matricielle |
| `.*``./``.^`| Par élément | `A .* B`| Opération de tableau |
| `'`| Transposer | `A'`| Conjugué transposer |
| `.'`| Transposition non conjuguée | `A.'`| |
| `\`| Division de gauche | `A \ b`| Résoudre Axe = b |
| `/`| Division à droite | `b / A`| Résoudre xA = b |
| `==``~=` | Égalité | `A == B`| Par élément |
| `<``>``<=``>=` | Comparaison | `A > 0`| Par élément |
| `&``\|``~`| Logique (par élément) | `A & B`| |
| `&&``\|\|` | Court-circuit | `a && b`| Scalaire uniquement |
| `:`| Côlon/intervalle | `1:10`| `start:step:stop`|
| `;`| Séparateur de lignes/suppression | `[1; 2; 3]`| |
| `,`| Séparateur de colonnes | `[1, 2, 3]`| |
---
...
## Résumé
La syntaxe de MATLAB est axée sur la matrice : chaque variable est un tableau et les opérations sont optimisées pour l'algèbre linéaire. L'opérateur deux-points, l'indexation logique et les opérations vectorisées éliminent les boucles pour la plupart des tâches numériques. Le système de traçage produit des chiffres de qualité publication avec un minimum de code. Les boîtes à outils étendent MATLAB à tous les domaines d'ingénierie. Même si Python a empiété sur certains territoires, MATLAB reste la norme en matière de calcul technique, de modélisation basée sur Simulink et de prototypage rapide.