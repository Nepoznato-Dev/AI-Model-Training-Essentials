...
# Ada — Erreurs courantes et anti-modèles
Ce document répertorie les erreurs, pièges et anti-modèles les plus courants dans Ada avec des corrections.
---

## 1. Ne pas utiliser `pragma Strict`
```ada
...
## 2. Erreurs de plage sur les sous-types
```ada
...
## 3. Ne pas utiliser de paramètres nommés pour la lisibilité
```ada
...
## 4. Oublier de gérer les exceptions
```ada
...
## Résumé
Le typage fort d'Ada évite de nombreux bogues mais nécessite de la discipline : utilisez`pragma Strict`, gérez`Constraint_Error`pour les sous-types, utilisez l'association de paramètres nommés pour plus de lisibilité, gérez toujours les exceptions et concevez soigneusement la communication des tâches pour éviter les blocages. Ada récompense une programmation minutieuse et soucieuse de la sécurité.