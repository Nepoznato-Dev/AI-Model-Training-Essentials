...
# Ada — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour Ada (2012/2022). Il complète la référence principale d'Ada en se concentrant sur les modèles de syntaxe exhaustifs, les tâches, les objets protégés, les génériques et la programmation critique pour la sécurité.
---
...
|--------------|------|---------|-------|
| `+``-``*``/``**`| Arithmétique | `A ** 2`| |
| `mod``rem` | Modulaire/reste | `A mod B`| `mod`toujours positif |
| `&`| Concaténation | `"hello" & " world"`| |
| `=``/=` | Égalité | `A = B`| |
| `<``>``<=``>=` | Comparaison | `A >= B`| |
| `and``or``not``xor` | Logique | `A and B`| |
| `and then``or else` | Court-circuit | `A and then B`| |
| `in``not in` | Adhésion à la gamme | `X in 1 .. 10`| |
| `:=`| Affectation | `X := 10`| |
---
...
## Résumé
La syntaxe d'Ada est verbeuse mais précise. Chaque construction est conçue pour plus de clarté et de sécurité. Les objets protégés éliminent les courses aux données au niveau linguistique. Les contrats (pré/postconditions, invariants) rendent les exigences d'exactitude explicites et vérifiables. Les génériques permettent une réutilisation sécurisée. Pour les systèmes critiques en matière de sécurité, la syntaxe d'Ada n'est pas un fardeau : c'est une garantie que le code fait ce qu'il dit.