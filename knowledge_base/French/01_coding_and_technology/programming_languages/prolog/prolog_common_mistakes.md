...
## 2. Ne pas utiliser correctement la coupe (`!`)
```prolog
...
## 4. Ne pas comprendre la vérification se produit
```prolog
...
## 5. Lister les opérations sans accumulateurs
```prolog
...
## Résumé
Le paradigme de programmation logique de Prolog crée des pièges uniques : confusion entre`=`(unification),`==`(égalité) et`is`(arithmétique) ; placement incorrect de la coupe ; récursivité infinie sans suivi des visites ; et le contrôle se produit. La méthode Prolog est la suivante : comprendre en profondeur l'unification, utiliser cut avec parcimonie et correctement, utiliser des accumulateurs pour une création efficace de listes et toujours envisager la résiliation.