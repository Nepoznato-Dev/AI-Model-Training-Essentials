...
---
# Delphi / Object Pascal — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour Delphi/Object Pascal. Il complète la référence principale Delphi en se concentrant sur des modèles de syntaxe exhaustifs, des fonctionnalités de POO, des génériques, des enregistrements et des idiomes de langage.
---

## Opérateurs et expressions
| Catégorie | Opérateur | Descriptif | Exemple |
|--------------|----------|-------------|---------|
| **Arithmétique** | `+`| Ajout | `A + B`|
| | `-`| Soustraction | `A - B`|
| | `*`| Multiplications | `A * B`|
| | `/`| Division réelle | `A / B`(renvoie toujours Real) |
| | `div`| Division entière | `A div B`(renvoie un entier) |
| | `mod`| Module | `A mod B`|
| **Comparaison** | `=`| Égalité | `A = B`|
| | `<>`| Pas égal | `A <> B`|
| | `<`| Moins de | `A < B`|
| | `>`| Supérieur à | `A > B`|
| | `<=`| Inférieur ou égal | `A <= B`|
| | `>=`| Supérieur ou égal | `A >= B`|
| **Logique** | `and`| ET logique | `A and B`|
| | `or`| OU logique | `A or B`|
| | `not`| NON logique | `not A`|
| | `xor`| OU exclusif | `A xor B`|
| **Chaîne** | `+`| Concaténation | `'Hello' + ' ' + 'World'`|
| **Ensemble** | `in`| Adhésion | `X in [1, 2, 3]`|
| | `+`| Syndicat | `SetA + SetB`|
| | `-`| Différence | `SetA - SetB`|
| | `*`| Intersection | `SetA * SetB`|
| **Affectation** | `:=`| Attribuer une valeur | `X := 42`|
| **Adresse** | `@`| Adresse de | `P := @X`|
| | `^`| Déréférencement | `P^`|
---
...
## Résumé
Delphi/Object Pascal combine une syntaxe claire et lisible avec de puissantes fonctionnalités POO. Les classes, les interfaces, les génériques et les enregistrements fournissent une boîte à outils complète pour le développement d'applications. Les blocs de début/fin explicites du langage, les variables typées et les identifiants sans distinction de casse le rendent accessible. Les frameworks VCL/FMX étendent le langage avec des bibliothèques de composants visuels pour un développement rapide d'applications. Pour les applications Windows d'entreprise et les frontaux de bases de données, Delphi reste un choix productif.