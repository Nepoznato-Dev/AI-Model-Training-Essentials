...
# Swift — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour Swift (5.9+). Il complète la référence principale de Swift en se concentrant sur les modèles de syntaxe exhaustifs, les options, la programmation orientée protocole et la concurrence moderne.
---
...
|--------------|------|---------|-------|
| `+``-``*``/``%`| Arithmétique | `a + b`| |
| `..``...` | Gamme | `1...10`| `...`fermé ; `..<`semi-ouvert |
| `==``!=``<``>``<=``>=` | Comparaison | `a == b`| Nécessite`Equatable`/`Comparable`|
| `===``!==` | Identité | `a === b`| Même référence (cours uniquement) |
| `&&``\|\|``!`| Logique | `a && b`| Court-circuit |
| `??`| Coalescence nulle | `a ?? b`| Renvoie`b`si`a`est nul |
| `?.`| Chaînage optionnel | `a?.b`| Renvoie nul si`a`est nul |
| `!`| Forcer le déballage | `a!`| Crashes si nul – à éviter |
| `&`| Paramètre d'entrée | `swap(&a, &b)`| Passer par référence |
### Opérateurs de manutention nuls
```swift
...
## Résumé
La syntaxe de Swift équilibre sécurité et expressivité. Les options éliminent les crashs nuls, les protocoles permettent une composition flexible sans héritage et la concurrence moderne (async/wait, acteurs) rend accessible la programmation simultanée sécurisée. Le langage continue d'évoluer avec des fonctionnalités telles que les packs de paramètres, les macros et la propriété – chaque ajout rendant Swift plus sûr et plus puissant tout en conservant sa promesse principale : un code sécurisé par construction.