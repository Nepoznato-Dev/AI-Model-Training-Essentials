...
# Fortran — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour Modern Fortran (2008/2018). Il complète la référence principale de Fortran en se concentrant sur les modèles de syntaxe exhaustifs, les opérations sur les tableaux, les modules, la POO et le calcul haute performance.
---
...
| `<``>``<=``>=` | Comparaison | `a >= b`| |
| `.and.``.or.``.not.`| Logique | `a .and. b`| |
| `.eqv.``.neqv.` | Equivalence logique | `a .eqv. b`| |
...
## Coarrays (programmation parallèle)
```fortran
...
## Résumé
La syntaxe de Modern Fortran est propre, structurée et optimisée pour le calcul numérique. Les opérations sur les tableaux éliminent les boucles pour la plupart des tâches mathématiques. Les modules assurent l'encapsulation et la gestion des espaces de noms. Les fonctionnalités POO (types dérivés, héritage, polymorphisme) permettent une organisation propre du code. Les Coarrays offrent une programmation parallèle native sans bibliothèques externes. Les atouts de Fortran (performances, opérations sur réseaux et précision numérique) restent inégalés pour le calcul scientifique. Pour le HPC, la physique computationnelle et la simulation technique, Fortran continue d'être une puissance.