...
---
# Ruby — Référence de syntaxe
Ce document fournit une référence de syntaxe complète et structurée pour Ruby (3.x). Il complète la référence principale de Ruby en se concentrant sur les modèles de syntaxe exhaustifs, les blocs et fermetures, la métaprogrammation et les idiomes Ruby.
---
...
| `==``!=``===``<=>` | Égalité | `a == b`| `===`est l'égalité de casse ; `<=>`est un vaisseau spatial |
| `eql?`| Égalité des valeurs | `a.eql?(b)`| Plus strict que`==`(type de contrôle) |
| `equal?`| Identité | `a.equal?(b)`| Même objet ? |
| `<``>``<=``>=` | Comparaison | `a >= b`| |
| `&&``\|\|``!`| Logique | `a && b`| Court-circuit ; renvoie la dernière valeur évaluée |
| `and``or``not`| Logique (faible priorité) | `a and b`| À éviter : utilisez`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | Au niveau du bit | `a & b`| |
| `=~``!~` | Correspondance d'expression régulière | `str =~ /pattern/`| Renvoie l'index ou zéro |
| `..``...` | Gamme | `1..10`| `..`inclus ; `...`exclusif |
| `?.`| Navigation sécurisée | `user&.name`| Renvoie nul si l'utilisateur est nul |
| `[]``[]=` | Accès aux éléments | `arr[0]`| |
### Vérité
```ruby
...
## Résumé
La syntaxe de Ruby est conçue pour le bonheur des développeurs : elle se lit comme l'anglais et récompense l'expressivité. La puissance du langage vient de son modèle objet (tout est un objet), de ses blocs et fermetures, de ses capacités de métaprogrammation et du module Enumerable qui fournit une riche API de traitement de collection. Ruby 3.x a ajouté la correspondance de modèles, les signatures de type RBS et Ractor pour la concurrence, tout en conservant la philosophie fondamentale du langage : rendre les programmeurs productifs et heureux.