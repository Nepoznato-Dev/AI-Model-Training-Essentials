...
-->
# Java – Erreurs courantes et anti-modèles
Ce document répertorie les erreurs, pièges et anti-modèles les plus courants en Java. Chaque entrée montre l'approche incorrecte, explique pourquoi elle échoue et fournit la solution correcte.
---

## 1.`equals()`vs`==`pour les objets
```java
...
## 2. Modifier une collection pendant l'itération
```java
...
## 3. Ne pas fermer les ressources (Essayer avec les ressources)
```java
...
## 4. Pièges de l'Autoboxing entier
```java
...
## 6. Concaténation de chaînes dans les boucles
```java
...
## 7. Ne pas gérer l'exception Interrupted
```java
...
## 8. Renvoyer des collections nulles au lieu de vides
```java
...
## 9. Objets mutables en tant que clés HashMap
```java
...
## 10. Anti-modèle : attraper`Exception`ou `Throwable`
```java
...
## 11. Ne pas remplacer`hashCode`lors du remplacement de `equals`
```java
...
## 12. Conditions de course avec verrouillage revérifié
```java
...
## Résumé
La verbosité de Java peut cacher des bugs subtils :`==`vs`equals()`, modifications simultanées, fuites de ressources, pièges de performances de boxing automatique et contrat égal/hashCode. L'approche Java moderne est la suivante : utilisez`Objects.equals()`pour une comparaison sans risque, utilisez toujours try-with-resources, renvoyez des collections vides au lieu de null, interceptez des exceptions spécifiques et suivez le principe de responsabilité unique. Les fonctionnalités de Java 8+ (flux, facultatifs, références de méthodes) réduisent le passe-partout tout en rendant le code plus sûr et plus expressif.