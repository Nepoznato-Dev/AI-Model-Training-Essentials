...
Le **retour** (récompense totale à prix réduit) est :
```
G = R₁ + γR₂ + γ²R₃ + ...
```
...
Sans RLHF, un modèle de langage est comme un étudiant qui a lu tous les livres mais ne sait pas comment se comporter dans une conversation. Il peut générer du texte, mais celui-ci peut être inutile, toxique ou complètement passer à côté de l'essentiel. RLHF enseigne au modèle *ce que veulent les humains* – pas seulement à quoi ressemble le texte.
