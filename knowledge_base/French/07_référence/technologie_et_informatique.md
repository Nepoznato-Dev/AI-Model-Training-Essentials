...
Version control Systèmes track changes to code over time, allowing developers to collaborate et revert to previous states. Git is le/la most widely used version control system. A repository (repo) is a collection de files et their historique. A commit is a saved snapshot de changes. A branch is an independent line de Développement. A pull request is a proposal to merge changes from one branch into another.
