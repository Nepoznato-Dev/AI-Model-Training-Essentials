---
# Métadonnées
title : "Guide de référence rapide SQL"
description: "Référence de requête SQL"
...
Commandes SQL essentielles pour les opérations de base de données.
---
...
-`DOUBLE`- Flotteur double précision
### Chaîne
...
- Sélectionnez uniquement les colonnes nécessaires (évitez`SELECT *`)
...
❌ **À ne pas faire :**
- Utiliser des fonctions sur les colonnes indexées dans les clauses WHERE
- Créer trop d'index (ralentit les écritures)
- Utilisez`SELECT DISTINCT`inutilement
- Ignorer les plans d'exécution des requêtes
- Stocker les valeurs calculées lorsqu'elles peuvent être calculées
---
...
*Dernière mise à jour : juillet 2026 | Norme SQL (compatible MySQL/PostgreSQL)*