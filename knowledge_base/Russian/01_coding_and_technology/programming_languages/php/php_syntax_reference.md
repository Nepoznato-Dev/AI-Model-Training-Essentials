...
| `==``!=``===``!==` | Равенство | `$a === $b`| `===`также проверяет тип |
| `<=>`| Космический корабль | `$a <=> $b`| Возвращает -1, 0 или 1 |
| `<``>``<=``>=` | Сравнение | `$a >= $b`| |
| `&&``\|\|``!`| Логический | `$a && $b`| Короткое замыкание |
| `and``or``xor`| Логический (низкий приоритет) | `$a and $b`| Избегайте — используйте`&&`/`\|\|`|
| `??`| Нулевое объединение | `$a ?? 'default'`| Возвращает `'default'`, если`$a`имеет значение null |
| `??=`| Назначение объединения значений null | `$a ??= 'default'`| Назначает, только если`$a`имеет значение null |
| `?``:` | Тройная | `$a ? $b : $c`| |
| `.`| Конкатенация | `$a . $b`| |
| `.=`| Объединение присваивания | `$a .= $b`| |
| `=>`| Назначение массива | `['key' => 'value']`| |
| `->`| Доступ для участников | `$obj->method()`| |
| `?->`| Оператор Nullsafe | `$obj?->method()`| Возвращает значение NULL, если`$obj`имеет значение NULL |
| `::`| Статический доступ | `ClassName::method()`| |
| `instanceof`| Проверка типа | `$obj instanceof ClassName`| |
| `clone`| Клонирование объектов | `$copy = clone $obj`| Для глубокого копирования требуется`__clone()`|
### Тип Жонглирование
```php
...
## Краткое содержание
Синтаксис PHP претерпел значительные изменения от своего происхождения в виде сценариев и превратился в современный типобезопасный язык. В PHP 8.x появились типы объединения, типы пересечений, перечисления, волокна, классы только для чтения, атрибуты и первоклассные вызываемые объекты, превратив его из свободно типизированного языка сценариев в язык, поддерживающий строгую разработку программного обеспечения. Сильной стороной языка остается его веб-ориентация: благодаря таким платформам, как Laravel и Symfony, огромной экосистеме и почти универсальной поддержке хостинга, синтаксис PHP обслуживает миллионы приложений. Современный PHP вознаграждает разработчиков, которые используют его систему типов, используют автозагрузку на основе композитора и пишут чистый объектно-ориентированный код.