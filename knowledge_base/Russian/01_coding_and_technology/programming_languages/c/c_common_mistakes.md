...
# C — Распространенные ошибки и антипаттерны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в C. Каждая запись показывает неправильный подход, объясняет, почему он не работает, и предлагает правильное решение. C дает вам полный контроль, что означает полную ответственность за безопасность памяти, проверку границ и неопределенное поведение.
---

## 1. Переполнение буфера с помощью `gets()`/`strcpy()`
```c
...
## 5.`scanf()`без ограничений ширины
```c
...
## 10. Непонимание арифметики указателей
```c
...
## 11. Антипаттерн: магические числа
```c
...
## Краткое содержание
Опасности языка C хорошо известны, но они всегда присутствуют: переполнение буфера, висячие указатели, переполнение целых чисел, атаки на форматную строку и утечки памяти. Дисциплина такова: всегда проверять возвращаемые значения, всегда освобождать то, что вы выделяете, всегда использовать ограниченные функции (`snprintf`,`strncpy`,`fgets`), никогда не использовать`gets()`, инициализировать все переменные и использовать такие инструменты, как Valgrind и AddressSanitizer. C вознаграждает осторожных программистов непревзойденной производительностью и контролем, но наказывает за невнимательность неопределённым поведением и уязвимостями безопасности.