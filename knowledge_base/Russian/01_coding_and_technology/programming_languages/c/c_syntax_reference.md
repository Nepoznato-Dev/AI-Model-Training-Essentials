...
В этом документе представлен полный структурированный справочник по синтаксису языка C (C11/C17 с примечаниями к C23). Он дополняет основной справочник по C, уделяя особое внимание исчерпывающим шаблонам синтаксиса, таблицам операторов, механике указателей и управлению памятью.
---
...
| `*`| Умножение | `3 * 2`| `6`| |
| `/`| Отдел | `7 / 2`| `3`| Усечение целых чисел для типов int |
| `%`| Модуль | `7 % 2`| `1`| Только для целочисленных типов |
| `++`| Приращение | `i++`/`++i`| | Пост- и предварительный прирост |
| `--`| Декремент | `i--`/`--i`| | Пост- и предварительный декремент |
...
| `~`| Дополнить | `~0`| Все биты перевернуты |
| `<<`| Левый сдвиг | `1 << 3`| `8`|
| `>>`| Правый сдвиг | `8 >> 3`| `1`(определенная реализация для подписанных) |
### Операторы указателей и адресов
| Оператор | Имя | Пример | Заметки |
|----------|------|---------|-------|
| `&`| Адрес | `&x`| Возвращает указатель на x |
| `*`| Разыменование | `*ptr`| Доступ к значению по указателю |
| `->`| Доступ к членам через указатель | `p->x`| Эквивалент`(*p).x`|
| `.`| Доступ для участников | `s.field`| Прямой доступ к членам структуры |
| `[]`| Индекс массива | `arr[i]`| Эквивалент`*(arr + i)`|
### Приоритет оператора (от самого высокого к самому низкому)
| Приоритет | Операторы | Ассоциативность |
|------------|-----------|---------------|
| 1 (самый высокий) | `()``[]``->``.` | Слева направо |
| 2 | `!``~``+``-` (унарный)`*``&``sizeof``(type)` | Справа налево |
| 3 | `*``/``%`| Слева направо |
| 4 | `+``-` | Слева направо |
| 5 | `<<``>>` | Слева направо |
| 6 | `<``<=``>``>=` | Слева направо |
...
| 11 | `&&`| Слева направо |
| 12 | `\|\|`| Слева направо |
| 13 | `? :`(тройной) | Справа налево |
| 14 | `=``+=``-=``*=``/=``%=``&=``\|=``^=``<<=``>>=`| Справа налево |
| 15 (самый низкий) | `,`(запятая) | Слева направо |
---
...
## Краткое содержание
Синтаксис C минимален и близок к аппаратному обеспечению — указатели, ручное управление памятью и тонкий уровень абстракции над машинными инструкциями. Сила языка проистекает из этой простоты: почти нет скрытых затрат, нет накладных расходов во время выполнения и полный контроль над расположением памяти. Препроцессор, несмотря на свою мощь, требует дисциплины для безопасного использования. В современный C (C11/C17/C23) добавлены важные функции безопасности, такие как`_Static_assert`,`_Generic`,`nullptr`и`typeof`, что делает язык постепенно более безопасным, сохраняя при этом его основную философию доверия программисту.