...
## Почему Ruby важен
- **Счастье разработчика**: Ruby создан так, чтобы его было легко читать и доставлять удовольствие. «Ruby создан, чтобы делать программистов счастливыми» — Мац.
- **Выразительный синтаксис**: код читается как английский. Минимум пунктуации, естественная формулировка.
- **Ruby on Rails**: один из самых продуктивных когда-либо созданных веб-фреймворков. Поддерживает GitHub, Shopify, Basecamp, GitLab.
- **Метапрограммирование**: Ruby может изменяться во время выполнения — динамически определять методы, создавать предметно-ориентированные языки (DSL).
- **Шаблон блока/итератора**: блоки и итераторы Ruby делают обработку коллекций элегантной.
- **Все является объектом**:`3.times { puts "hello" }`— у целых чисел есть методы.
## Компромиссы
...
## Рубин на рельсах
Rails — это полнофункциональная веб-инфраструктура, которая следует архитектуре MVC (Model-View-Controller) и уделяет особое внимание:
- **Соглашение важнее конфигурации**: разумные значения по умолчанию — не нужно настраивать все подряд.
- **Не повторяйте себя (DRY)**: используйте генераторы, миграции и соглашения, чтобы свести к минимуму повторение.
- **Активная запись**: объекты базы данных являются объектами Ruby. `User.find(1)`извлекает пользователя.
- **Миграции**: изменения схемы базы данных представляют собой версионный код Ruby.
Rails поддерживает GitHub, Shopify, Stripe (ранняя версия), Basecamp, GitLab и многие стартапы.
---

## Расширенный синтаксис и шаблоны
### Утиный ввод и динамическая отправка
```ruby
...
### Сопоставление с образцом (Ruby 3.0+)
```ruby
...
### Fibers — облегченные сопрограммы
```ruby
...
### Async/Await с помощью Async Gem
```ruby
require "async"
...
### Gemfile — Управление зависимостями
```ruby
# Gemfile
...
## Когда использовать Ruby
| Сценарий | Почему Руби | Лучшая альтернатива |
|----------|---------|-------------------|
| Веб-приложения (Rails) | Быстрое развитие, продуктивная основа | Джанго (Python), Laravel (PHP), Next.js |
| Прототипирование | Очень быстро писать и повторять | Питон, JavaScript |
| Скрипты и автоматизация | Элегантный синтаксис, мощная обработка текста | Питон, оболочка |
| Инструменты DevOps (Chef, Puppet) | Созданная экосистема | Вперёд, Питон |
| Инструменты CLI | Возможно, но не идеально | Вперёд, Раст |
| Системы, критичные к производительности | Слишком медленно | C, C++, Rust, Go |
