...
# Ruby — распространенные ошибки и антипаттерны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в Ruby. Каждая запись показывает неправильный подход, объясняет, почему он не работает, и предлагает правильное решение.
---
...
## 2.`nil`против`false`Правдивость
```ruby
...
## 3. Интерполяция строк и конкатенация
```ruby
...
## 4. Не использовать блоки/перечислители идиоматически
```ruby
# ❌ WRONG — C-style loop
i = 0
...
## 5. Злоупотребление глобальными переменными
```ruby
...
## 6. Обновление базовых классов Monkey
```ruby
...
## 10. Не замораживать строковые литералы
```ruby
...
## Краткое содержание
Выразительность Ruby может привести к антишаблонам: изменению коллекций во время итерации, исправлению базовых классов, глобальным переменным и толстым моделям. Путь Ruby таков: использовать перечислители (`each`,`map`,`reject`), отдавать предпочтение символам и аргументам ключевых слов, замораживать строковые литералы, извлекать служебные объекты из моделей и использовать уточнения вместо обезьяньих патчей. Ruby вознаграждает разработчиков, которые используют блоки, утиный ввод и принцип наименьшего неожиданности.