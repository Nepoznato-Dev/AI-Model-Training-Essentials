...
| `==``!=``===``<=>` | Равенство | `a == b`| `===`— равенство регистра; `<=>`— космический корабль |
| `eql?`| Равенство ценностей | `a.eql?(b)`| Строже, чем`==`(тип проверки) |
| `equal?`| идентичность | `a.equal?(b)`| Тот же объект? |
| `<``>``<=``>=` | Сравнение | `a >= b`| |
| `&&``\|\|``!`| Логический | `a && b`| Короткое замыкание; возвращает последнее вычисленное значение |
| `and``or``not`| Логический (низкий приоритет) | `a and b`| Избегайте — используйте`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | Побитовое | `a & b`| |
| `=~``!~` | Соответствие регулярному выражению | `str =~ /pattern/`| Возвращает индекс или ноль |
| `..``...` | Диапазон | `1..10`| `..`включительно; `...`эксклюзив |
| `?.`| Безопасная навигация | `user&.name`| Возвращает ноль, если пользователь равен нулю |
...
## Краткое содержание
Синтаксис Ruby создан для удобства разработчиков — он читается как английский и поощряет выразительность. Мощь языка заключается в его объектной модели (все является объектом), блоках и замыканиях, возможностях метапрограммирования и модуле Enumerable, который предоставляет богатый API для обработки коллекций. В Ruby 3.x добавлено сопоставление шаблонов, сигнатуры типов RBS и Ractor для параллелизма, сохраняя при этом основную философию языка: сделать программистов продуктивными и счастливыми.