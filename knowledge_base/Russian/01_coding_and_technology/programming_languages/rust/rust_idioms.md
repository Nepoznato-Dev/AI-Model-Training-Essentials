...
-->
# Rust — идиоматические шаблоны и лучшие практики
В этом руководстве рассматриваются идиоматические шаблоны и лучшие практики написания чистого и безопасного кода на Rust.
---
...
## Краткое содержание
Идиомы Rust подчеркивают: владение и заимствование, `Result`/`Option` для обработки ошибок, перечисления с методами, типы для абстракции, цепочки итераторов, интеллектуальные указатели для общего состояния и абстракции с нулевой стоимостью. Следуйте`rustfmt`для форматирования,`clippy`для проверки и руководству по Rust API. Сообщество Rust ценит безопасность, производительность и корректность — «если оно компилируется, значит, оно работает». Предпочитайте`&str``String` в аргументах функции, используйте`?`для распространения ошибок и выведите общие признаки с помощью `#[derive(Debug, Clone, ...)]`.