...
|----------|------|---------|-------|
| `+`| Дополнение | `3 + 2`| Также объединение`String`через`format!`|
| `-`| Вычитание | `3 - 2`| |
| `*`| Умножение | `3 * 2`| |
| `/`| Отдел | `7 / 2`| Целочисленное деление целых чисел:`3`|
| `%`| Остаток | `7 % 2`| `1`|
| `-x`| Отрицание | `-5`| |
### Сравнение и логические операторы
| Оператор | Имя | Пример | Заметки |
|----------|------|---------|-------|
| `==`| Равный | `x == y`| Требуется`PartialEq`|
| `!=`| Не равно | `x != y`| |
| `<`,`>`,`<=`,`>=`| Заказ | `x >= y`| Требуется`PartialOrd`|
| `&&`| Логическое И | `a && b`| Короткое замыкание |
| `\|\|`| Логическое ИЛИ | `a \|\| b`| Короткое замыкание |
| `!`| Логическое НЕ | `!true`| |
### Побитовые операторы
| Оператор | Имя | Пример | Заметки |
|----------|------|---------|-------|
...
| `!`| НЕ | `!0b1010u8`| Побитовое дополнение |
| `<<`| Левый сдвиг | `5 << 1`| `10`|
| `>>`| Правый сдвиг | `5 >> 1`| `2`|
### Приоритет оператора (от самого высокого к самому низкому)
| Приоритет | Операторы | Заметки |
|------------|-----------|-------|
| 1 (самый высокий) | `()`пути`.`:: | |
| 2 | Вызовы методов, доступ к полям | Авторазрешение |
| 3 | `-`(отрицание),`!`,`*`(разыменование) | |
| 4 | `as`(приведение типа) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
| 7 | `<<``>>` | |
| 8 | `&`| |
...
| 14 | `..``..=` | Диапазон |
| 15 | `=``+=` и т. д. | Назначение |
| 16 (самый низкий) | `return``break``continue`| |
---
...
| Тип | Стек/куча | Размер | Мутабельность | Заметки |
|------|-----------|------|------------|-------|
| Примитивы (`i32`,`f64`,`bool`,`char`) | Стек | Исправлено | Неизменяемый по умолчанию | `Copy`|
| `[T; N]`(массив) | Стек | Исправлено | Изменяемый с помощью`mut`| Фиксированный размер, `Copy`, если T равно`Copy`|
| `Vec<T>`| Куча | Динамический | Изменяемый с помощью`mut`| Выращиваемый |
| `String`| Куча | Динамический | Изменяемый с помощью`mut`| UTF-8, расширяемый |
| `&[T]`(срез) | Стек (жирный PTR) | Динамический просмотр | Зависит от`&`или`&mut`| Заимствованный вид |
| `HashMap<K, V>`| Куча | Динамический | Изменяемый с помощью`mut`| O(1) средний поиск |
| `BTreeMap<K, V>`| Куча | Динамический | Изменяемый с помощью`mut`| Поиск O(log n), упорядоченный |
| `HashSet<T>`| Куча | Динамический | — | O(1) в среднем, дубликатов нет |
### Векторы
```rust
// Creation
...
### Общие особенности стандартной библиотеки
```rust
...
## Обработка ошибок
### Шаблоны результатов и опционов
```rust
...
## Краткое содержание
Синтаксис Rust построен на гарантиях безопасности, реализуемых во время компиляции. Система владения, полиморфизм на основе признаков, сопоставление с образцом и абстракции с нулевой стоимостью образуют последовательный дизайн, в котором компилятор является строгим, но справедливым партнером. От базового потока управления до расширенных функций, таких как макросы, небезопасный код и интеллектуальные указатели, каждая конструкция служит цели безопасного и эффективного системного программирования. Понимание взаимодействия между временем жизни, заимствованием и системой типов является ключом к освоению Rust.