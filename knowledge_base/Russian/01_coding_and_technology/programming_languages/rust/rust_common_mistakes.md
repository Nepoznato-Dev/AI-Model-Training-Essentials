...
-->
# Rust — распространенные ошибки и антипаттерны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в Rust. Каждая запись показывает неправильный подход, объясняет, почему он не работает, и предлагает правильное решение. Компилятор Rust обнаруживает множество ошибок, но понимание этих закономерностей ускорит ваше обучение.
---

## 1. Борьба с проверкой займов — ненужное клонирование
```rust
...
## 2. Путаница между строкой и &str
```rust
...
## 3. Игнорирование результатов с помощью`unwrap()`в рабочей среде
```rust
...
## 4. Ошибки в аннотациях на всю жизнь
```rust
...
## 5. Антипаттерн: повсеместное использование `Rc`/`RefCell`
```rust
...
## 6. Непонимание семантики перемещения
```rust
...
## 7. Антипаттерн: чрезмерное количество `to_string()`/`to_owned()`
```rust
...
## 9. Инвалидация итератора — сбор без необходимости
```rust
...
## 10. Антишаблон: перечисление без методов
```rust
...
## 11. Не использовать`#[derive]`для общих свойств
```rust
...
## 12. Небезопасный код без документации
```rust
...
## Краткое содержание
Компилятор Rust строгий, но справедливый — большинство ошибок учат вас чему-то о безопасности памяти. Самые большие ошибки — это не ошибки компилятора, а антишаблоны: ненужное клонирование для успокоения средства проверки заимствований, использование`unwrap()`в производстве, обращение к`Rc<RefCell<T>>`перед попыткой владения и выделение строк, когда ссылок достаточно. Мышление Rustacean таково: позвольте компилятору вести вас к абстракциям с нулевой стоимостью. Клонируйте только при необходимости, распространяйте ошибки с помощью `?`, отдавайте предпочтение`&str``String` и документируйте каждый блок `unsafe`.