...
Обобщенные шаблоны позволяют писать код, который работает с любым типом, сохраняя при этом полную типобезопасность. Черты определяют общее поведение.
```rust
