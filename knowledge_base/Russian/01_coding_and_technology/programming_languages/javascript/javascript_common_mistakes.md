...
-->
# JavaScript — распространенные ошибки и антишаблоны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в JavaScript. Каждая запись показывает неправильный подход, объясняет, почему он не работает, и предлагает правильное решение.
---

## 1. Путаница привязки `this`
```javascript
...
## 3.`var`Проблемы с подъемом и областью действия
```javascript
...
## 5. Не обрабатывать отказы от обещаний
```javascript
...
## 6. Изменение объектов во время итерации
```javascript
...
## 7. Антипаттерн: ад обратного вызова
```javascript
...
## 9. Объекты, подобные массивам, не являются массивами
```javascript
...
## 10. Антипаттерн: изменение параметров функции
```javascript
...
## 11. Непонимание цикла событий и микрозадач
```javascript
...
## 12. Антипаттерн: использование `eval()`
```javascript
...
## 13. Забываем очистить прослушиватели событий
```javascript
// ❌ WRONG — memory leak
class Component {
  mount() {
...
## 14. Мелкое копирование против глубокого копирования
```javascript
...
## Краткое содержание
Гибкость JavaScript сопряжена с ловушками: привязка`this`зависит от места вызова,`==`выполняет приведение, обещания требуют обработки ошибок, а порядок цикла событий (микрозадачи перед макрозадачами) неочевиден. Современные правила просты: никогда не используйте`const`/`let``var` , отдавайте предпочтение`===``==` , всегда обрабатывайте отклонения обещаний, используйте функции стрелок для обратных вызовов, избегайте мутаций и очищайте прослушиватели событий. Следование этим правилам устраняет большинство ошибок JavaScript.