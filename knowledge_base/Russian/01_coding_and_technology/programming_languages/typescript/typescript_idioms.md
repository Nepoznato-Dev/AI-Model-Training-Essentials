...
-->
# TypeScript — идиоматические шаблоны и лучшие практики
В этом руководстве рассматриваются идиоматические шаблоны и лучшие практики написания чистого, типобезопасного кода TypeScript.
---
...
## Краткое содержание
Идиомы TypeScript подчеркивают: строгую проверку типов, интерфейсы для форм объектов, распознаваемые объединения для состояния, защиту типов для сужения, универсальные шаблоны с ограничениями,`readonly`для неизменяемости и`as const`для литеральных типов. Следуйте ESLint с`typescript-eslint`, Prettier для форматирования и всегда включайте`strict: true`. Сообщество TypeScript ценит безопасность типов — если он компилируется, то, вероятно, работает. Предпочитайте`interface`для форм объектов,`type`для объединений и утилит и избегайте`any`любой ценой.