...
## Краткое содержание
Идиомы Lua подчеркивают: таблицы как универсальная структура данных, локальные переменные, замыкания для состояния, метатаблицы для ООП и индексация на основе 1. Следуйте luacheck для проверки и стилуса для форматирования. Lua ценит простоту и минимализм — язык намеренно мал, поэтому шаблоны возникают из композиции.