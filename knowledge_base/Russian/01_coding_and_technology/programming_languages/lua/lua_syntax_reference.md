...
# Lua — Справочник по синтаксису
Этот документ предоставляет исчерпывающий структурированный справочник по синтаксису Lua (5.4). Он дополняет основной справочник по Lua, уделяя особое внимание исчерпывающим синтаксическим шаблонам, таблицам и метатаблицам, сопрограммам и уникальной философии дизайна Lua.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%``^` | Арифметика | `2 ^ 10`| `^`— возведение в степень |
| `//`| Этажное разделение | `7 // 2`| Возвращает 3 (Lua 5.3+) |
| `==``~=` | Равенство | `a == b`| `~=`— «не равно» |
| `<``>``<=``>=` | Сравнение | `a >= b`| |
| `and``or``not`| Логический | `a and b`| Возвращает операнды, а не логические значения |
| `..`| Конкатенация | `"hello" .. " world"`| |
| `#`| Длина | `#t`| Длина строки или таблицы |
| `&``\|``~``<<``>>`| Побитовое | `a & b`| Lua 5.3+ (целочисленные операции) |
### Логические операторы — нелогические возвращаемые значения
```lua
...
### Приоритет (от самого высокого к самому низкому)
| Приоритет | Операторы |
|------------|-----------|
| 1 (самый высокий) | `^`|
| 2 | `not``#``-`(унарный) |
| 3 | `*``/``//``%` |
| 4 | `+``-` |
| 5 | `..`(правоассоциативный) |
| 6 | `<<``>>` |
| 7 | `&`|
| 8 | `~`(двоичное исключающее ИЛИ) |
| 9 | `\|`|
| 10 | `==``~=``<``>``<=``>=` |
| 11 | `and`|
| 12 (самый низкий) | `or`|
...
## Краткое содержание
Синтаксис Lua намеренно минимален — весь язык можно описать на одной странице. Таблицы служат универсальной структурой данных (массивы, словари, объекты, модули). Метатаблицы обеспечивают расширяемость без языковой сложности. Сопрограммы обеспечивают совместную многозадачность и элегантные шаблоны итераторов. Сила языка заключается в его встраиваемости: Lua предназначен для работы внутри хост-приложений, обеспечивая возможности написания сценариев с минимальными накладными расходами. Для разработки игр, настройки и встроенных систем простота Lua является его самой сильной стороной — небольшой язык, который делает именно то, что ему нужно, и не более того.