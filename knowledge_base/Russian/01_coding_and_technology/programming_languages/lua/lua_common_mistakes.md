...
-->
# Lua — распространенные ошибки и антипаттерны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в Lua с исправлениями.
---

## 1. Индексация на основе 1
```lua
...
## 4. Глобальные переменные по умолчанию
```lua
...
## 7. Не использовать метатаблицы для ООП
```lua
...
## Краткое содержание
Минимализм Lua создает ловушки: индексация на основе 1, только`nil`и`false`являются ложными, таблицы являются ссылками (а не значениями), переменные по умолчанию являются глобальными, а`pairs`неупорядочен, а`ipairs`упорядочен. Способ Lua: всегда используйте`local`, используйте`ipairs`для массивов и`pairs`для словарей, используйте`table.concat`для построения строк и используйте метатаблицы для ООП. Простота Lua — это его сила, но уважайте крайние случаи.