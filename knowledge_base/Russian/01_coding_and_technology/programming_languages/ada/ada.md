...
## Почему Ада важна
- **Системы, критически важные для безопасности**: разработаны с нуля для систем, где сбой невозможен.
- **Строгая типизация**: самая строгая система типов среди всех основных языков — выявляет ошибки во время компиляции, которые пропускают другие языки.
- **Встроенный параллелизм**: выполнение задач (параллельное программирование) является частью языка, а не библиотеки.
- **Формальная проверка**: поддерживает формальные методы проверки правильности программы.
- **Функции обеспечения надежности**: встроенные исключения, контракты (предварительные и постусловия) и проверки во время выполнения.
- **Современная Ada**: в Ada 2012 и Ada 2022 добавлены современные функции, сохранены гарантии безопасности.
## Компромиссы
| Ограничение | Подробности | Типичный обходной путь |
|-----------|---------|-------------------|
| **Нишевое сообщество** | Небольшая база разработчиков по сравнению с C, Java или Python | Специализированное, но знающее сообщество |
| **Подробный синтаксис** | Более подробный, чем C или Python | Принять как часть конструкции, ориентированной на безопасность |
| **Ограниченная экосистема** | Меньше библиотек, чем в основных языках | Написать собственный код; использовать обширную стандартную библиотеку |
| **Кривая обучения** | Сложный язык со множеством функций безопасности | Начните с подмножества Ada «SPARK» для критически важных с точки зрения безопасности работ |
| **Наличие компилятора** | Меньше опций компилятора (основной — GNAT) | GNAT бесплатен и поддерживается в хорошем состоянии |
---
...
### Тегированные типы и наследование (ООП)
```ada
-- Abstract base type
package Shapes is
...
### Дженерики (полиморфизм времени компиляции)
```ada
...
### Контракты и аспекты (Ада, 2012 г.)
```ada
...
### Защищенные объекты (потокобезопасные общие данные)
```ada
...
### Параллельные вычисления (Ада, 2022 г.)
```ada
...
### Схема 1: наблюдатель с защищенными объектами
```ada
...
### Паттерн 2: Конечный автомат с различаемыми типами
```ada
...
### Шаблон 3: RAII с контролируемыми типами
```ada
...
### Абстракции с нулевыми издержками
```ada
...
## Развертывание
### SPARK для сертификации критически важных систем безопасности
```ada
...
## Когда использовать Аду
| Сценарий | Почему Ада | Лучшая альтернатива |
|----------|---------|-------------------|
| Системы, критичные к безопасности | Предназначен для этого; официальная поддержка проверки | — |
| Авиация / аэрокосмическая промышленность | Сертифицированные составители; Соответствие DO-178C | — |
| Защитные системы | Наследие Министерства обороны; функции безопасности | — |
| Железнодорожное оборудование / медицинское оборудование | Требования высокой целостности | — |
...
## Краткое содержание
Ада — это язык, созданный для корректности. Его строгая система типов, встроенная поддержка параллелизма и формальная поддержка проверки делают его идеальным выбором для систем, где сбой недопустим. Несмотря на то, что сообщество Ada невелико по сравнению с основными языками, Ada по-прежнему играет важную роль в авиации, обороне, космосе и других областях, критически важных для безопасности. Для этих приложений строгий подход Ады к разработке программного обеспечения не является ограничением — в этом суть.