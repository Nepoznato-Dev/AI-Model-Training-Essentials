...
| **Непрямая регистрация** |  __ЗАЩИЩЕНО_3__ | Операнд находится по адресу в регистре |
...
| Концепция | Описание |
|---------|-------------|
| **Регистры** | Внутренняя память ЦП (EAX, EBX, ECX, EDX на x86; R0-R15 на ARM) |
| **Адресация памяти** | Доступ к оперативной памяти через адреса (`MOV EAX, [0x1000]`) |
...
| **Прерывания/системные вызовы** | Запрос сервисов у операционной системы |
...
Программы на ассемблере компилируются непосредственно в исполняемые файлы собственного машинного кода. Не требуется ни среды выполнения, ни виртуальной машины, ни интерпретатора. Развертывание так же просто, как копирование двоичного файла в целевую систему.
```bash
