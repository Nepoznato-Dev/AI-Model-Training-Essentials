...
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в C++. Каждая запись показывает неправильный подход, объясняет, почему он не работает, и предлагает правильное решение. Современный C++ (C++11/14/17/20) предоставляет инструменты, позволяющие избежать большинства классических ошибок.
---

## 1. Правило трех/пяти нарушений
```cpp
...
## 2. Необработанные `new`/`delete` вместо интеллектуальных указателей
```cpp
...
## 6. Неправильное использование `std::move`
```cpp
...
## 7. Антишаблон: использование`using namespace std;`в заголовках
```cpp
...
## 8. Неопределённое поведение из-за знакового переполнения
```cpp
...
## 9. Непонимание категорий ценностей
```cpp
...
## 10. Антишаблон: ручное управление ресурсами (Pre-RAII)
```cpp
...
## 11. Раздувание шаблонов и длительное время компиляции
```cpp
...
## 12. Не использовать`[[nodiscard]]`и `[[maybe_unused]]`
```cpp
...
## Краткое содержание
Сложность C++ создает множество возможностей для ошибок: правило пяти, разделение объектов, аннулирование итераторов, висячие ссылки и неопределенное поведение из-за знакового переполнения. Современный C++ является противоядием: используйте интеллектуальные указатели вместо необработанных`new`/`delete`, RAII для всех ресурсов,`std::vector`и`std::string`вместо необработанных массивов, перемещайте семантику для повышения эффективности и`[[nodiscard]]`для перехвата игнорируемых возвращаемых значений. Основные рекомендации C++ существуют не просто так — следуйте им и используйте такие инструменты, как Clang-Tidy и дезинфицирующие средства, чтобы уловить то, что вы упускаете.