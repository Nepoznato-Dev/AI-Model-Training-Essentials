...
В этом руководстве рассматриваются идиоматические шаблоны и лучшие практики написания чистого, современного кода на C++ (20/23).
---

## RAII и интеллектуальные указатели
```cpp
...
## Краткое содержание
Современные идиомы C++ подчеркивают: RAII и интеллектуальные указатели, семантику перемещения,`auto`для вывода типов, диапазонов и алгоритмов (C++20), концепции ограничений шаблона,`std::optional`/`std::expected`для отсутствия/ошибок, правило нуля и`constexpr`для вычислений во время компиляции. Следуйте`clang-format`для форматирования,`clang-tidy`для проверки и компиляции с помощью `-Wall -Wextra -Werror`. Сообщество C++ ценит абстракции с нулевой стоимостью и «не платите за то, чем не пользуетесь».