...
|----------|------|---------|-------|
| `+``-``*``/``%`| Арифметика | `a + b`| То же, что C |
| `++``--` | Увеличение/Уменьшение | `++i`,`i++`| Предпочитаю предварительное приращение |
| `==``!=``<``>``<=``>=` | Сравнение | `a <=> b`| `<=>`— трехстороннее сравнение (C++20) |
| `&&``\|\|``!`| Логический | `a && b`| Короткое замыкание |
| `&``\|``^``~``<<``>>` | Побитовое | `a & b`| То же, что C |
| `?:`| Тройная | `cond ? a : b`| |
### Операторы, специфичные для C++
| Оператор | Имя | Пример | Заметки |
|----------|------|---------|-------|
| `::`| Разрешение области | `std::cout`| |
| `.*``->*` | Указатель на член | `obj.*pmf`| |
| `<=>`| Космический корабль (C++20) | `a <=> b`| Возвращает`strong_ordering`|
| `co_await`| Приостановка сопрограммы | `co_await expr`| С++20 |
| `co_yield`| Выход сопрограммы | `co_yield value`| С++20 |
| `co_return`| Возврат сопрограммы | `co_return value`| С++20 |
...
## Поток управления
### Структурированные переплеты и современные узоры
```cpp
...
### Обзор контейнера
| Контейнер | Заголовок | Заказал | Поиск | Заметки |
|-----------|--------|---------|--------|-------|
| `vector`| `<vector>`| Да | О(п) | Выбор по умолчанию для последовательностей |
| `deque`| `<deque>`| Да | О(п) | Быстрое нажатие/выталкивание на обоих концах |
| `list`| `<list>`| Да | О(п) | Двусвязный список |
| `array`| `<array>`| Да | О(п) | Фиксированный размер, размещение в стеке |
| `set`/`multiset`| `<set>`| Да | O(журнал n) | Красно-черное дерево |
| `map`/`multimap`| `<map>`| Да (по ключу) | O(журнал n) | Ключ-значение, красно-черное дерево |
| `unordered_set`| `<unordered_set>`| Нет | O(1) среднее | Хэш-таблица |
| `unordered_map`| `<unordered_map>`| Нет | O(1) среднее | Хэш-таблица |
### Ключевые алгоритмы
```cpp
#include <algorithm>
#include <numeric>
...
## Краткое содержание
Синтаксис C++ простирается от низкоуровневого манипулирования указателями до высокоуровневого метапрограммирования шаблонов и асинхронной обработки на основе сопрограмм. Эволюция языка через C++17/20/23 значительно повысила безопасность (умные указатели, `std::optional`, концепции), выразительность (структурированные привязки, диапазоны, `std::format`) и производительность (концепции, обеспечивающие абстракции с нулевой стоимостью, сопрограммы для асинхронности без накладных расходов на стек). Основная философия остается неизменной: вы не должны платить за то, чем не пользуетесь, а язык должен поддерживать все уровни абстракции — от «голого железа» до языков, специфичных для предметной области.