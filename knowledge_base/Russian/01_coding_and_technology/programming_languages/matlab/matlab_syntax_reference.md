...
|----------|------|---------|-------|
| `+``-``*``/``^`| Арифметика | `A * B`| Матричное умножение |
| `.*``./``.^`| Поэлементно | `A .* B`| Операция с массивом |
| `'`| Транспонировать | `A'`| Сопряженное транспонирование |
| `.'`| Несопряженное транспонирование | `A.'`| |
| `\`| Левый разделитель | `A \ b`| Решите Ax = b |
| `/`| Правый разделитель | `b / A`| Решите xA = b |
| `==``~=` | Равенство | `A == B`| Поэлементно |
| `<``>``<=``>=` | Сравнение | `A > 0`| Поэлементно |
| `&``\|``~`| Логический (поэлементный) | `A & B`| |
| `&&``\|\|` | Короткое замыкание | `a && b`| Только скаляр |
| `:`| Двоеточие/диапазон | `1:10`| `start:step:stop`|
| `;`| Разделитель строк/подавление | `[1; 2; 3]`| |
| `,`| Разделитель столбцов | `[1, 2, 3]`| |
---
...
## Краткое содержание
Синтаксис MATLAB ориентирован на матрицу: каждая переменная представляет собой массив, а операции оптимизированы для линейной алгебры. Оператор двоеточия, логическое индексирование и векторизованные операции исключают циклы для большинства числовых задач. Система построения графиков создает рисунки публикационного качества с минимальным количеством кода. Наборы инструментов расширяют возможности MATLAB во всех инженерных областях. Хотя Python вторгся на некоторую территорию, MATLAB остается стандартом для инженерных вычислений, моделирования на основе Simulink и быстрого прототипирования.