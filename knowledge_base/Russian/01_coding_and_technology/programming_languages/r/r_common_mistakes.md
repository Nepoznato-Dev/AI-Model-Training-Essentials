...
# R — Распространенные ошибки и антипаттерны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в R с исправлениями.
---

## 1. Увеличение кадров данных в циклах
```r
# ❌ WRONG — O(n²) growth
result <- data.frame()
for (i in 1:1000) {
...
## 3. Непонимание переработки векторов
```r
...
## 5. Не использовать `stringsAsFactors = FALSE`
```r
...
## 6. Семья`apply`против Tidyverse
```r
...
## 8. Глобальное присвоение с помощью `<<-`
```r
...
## Краткое содержание
Интерактивная природа R создает ловушки: рост кадров данных в циклах (предварительное выделение или векторизация), сюрпризы при переработке векторов, распространение`NA`и возможность перезаписи `T`/`F`. Способ R: векторизация операций, предварительное выделение памяти при цикле, явная обработка `NA`, использование tidyverse для манипулирования данными и избегание `<<-`. R вознаграждает тех, кто мыслит векторами и использует богатую экосистему пакетов.