...
# Swift — распространенные ошибки и антишаблоны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в Swift. Каждая запись показывает неправильный подход, объясняет, почему он не работает, и предлагает правильное решение.
---

## 1. Принудительное разворачивание опций
```swift
// ❌ WRONG — crash if nil
...
## 2. Сохранение циклов с замыканиями
```swift
...
## 3. Не использовать`enum`со связанными значениями
```swift
...
## 4. Непонимание типов значений и ссылочных типов
```swift
...
## 5. Антипаттерн: контроллер Massive View
```swift
...
## 6. Не использовать`guard`для раннего выхода
```swift
...
## 9. Не использовать`lazy`для дорогих свойств
```swift
...
## 10. Потокобезопасность с помощью `@MainActor`
```swift
...
## Краткое содержание
Функции безопасности Swift предотвращают множество ошибок, но создают свои собственные ловушки: принудительное развертывание приводит к сбоям, циклы сохранения скрываются в замыканиях и делегатах, а семантика типов значений удивляет разработчиков, работающих на языках ссылочного типа. Способ Swift таков: никогда не выполнять принудительное развертывание (`!`), использовать`guard`для ранних выходов, использовать`weak`для делегатов и захватов замыканий, отмечать код пользовательского интерфейса с помощью`@MainActor`и разбивать массивные контроллеры представлений на модели и сервисы представлений. Система типов Swift — ваш друг — работайте с ней, а не против нее.