...
-->
# Swift — Справочник по синтаксису
В этом документе представлен полный структурированный справочник по синтаксису Swift (5.9+). Он дополняет основной справочник по Swift, уделяя особое внимание исчерпывающим синтаксическим шаблонам, опциям, протокольно-ориентированному программированию и современному параллелизму.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Арифметика | `a + b`| |
| `..``...` | Диапазон | `1...10`| `...`закрыт; `..<`полуоткрытый |
| `==``!=``<``>``<=``>=` | Сравнение | `a == b`| Требуется`Equatable`/`Comparable`|
| `===``!==` | идентичность | `a === b`| Та же ссылка (только классы) |
| `&&``\|\|``!`| Логический | `a && b`| Короткое замыкание |
| `??`| Ноль объединения | `a ?? b`| Возвращает `b`, если`a`равен нулю |
| `?.`| Дополнительное связывание | `a?.b`| Возвращает ноль, если`a`равен нулю |
| `!`| Принудительное развертывание | `a!`| Сбой, если ноль — избегать |
| `&`| Входной параметр | `swap(&a, &b)`| Перейти по ссылке |
### Операторы обработки нуля
```swift
...
## Краткое содержание
Синтаксис Swift сочетает в себе безопасность и выразительность. Опциональные возможности устраняют нулевые сбои, протоколы обеспечивают гибкую композицию без наследования, а современный параллелизм (async/await, актеры) делает доступным безопасное параллельное программирование. Язык продолжает развиваться благодаря таким функциям, как пакеты параметров, макросы и владение — каждое дополнение делает Swift более безопасным и мощным, сохраняя при этом его основное обещание: код, безопасный по своей конструкции.