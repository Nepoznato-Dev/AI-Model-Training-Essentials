...
---
# Дарт — распространенные ошибки и антипаттерны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в Dart с исправлениями.
---

## 1. Неправильное использование нулевой безопасности
```dart
// ❌ WRONG — force unwrap
...
## 2. Async/Await без обработки ошибок
```dart
...
## 3. Не использовать конструкторы `const`
```dart
...
## 5. Не использовать Spread и Collection-if
```dart
...
## 6. Анти-шаблон: массивный виджет с состоянием
```dart
...
## Краткое содержание
Нулевая безопасность Dart мощная — не победите ее с помощью`!`. Используйте конструкторы`const`для канонических экземпляров, всегда переопределяйте`hashCode`с помощью`==`, используйте операторы сбора-если и расширения для краткого построения списка и извлекайте бизнес-логику из виджетов. Dart вознаграждает чистый, нулевой безопасный и константно-корректный код.