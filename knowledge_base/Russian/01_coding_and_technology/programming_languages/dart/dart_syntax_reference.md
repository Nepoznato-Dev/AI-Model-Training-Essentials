...
|----------|------|---------|-------|
| `+``-``*``/``%``~/` | Арифметика | `7 ~/ 3`| `~/`— целочисленное деление |
| `==``!=` | Равенство | `a == b`| |
| `<``>``<=``>=` | Сравнение | `a >= b`| |
| `&&``\|\|``!`| Логический | `a && b`| Короткое замыкание |
| `??`| Нулевое объединение | `a ?? b`|  `b`, если`a`имеет значение NULL |
| `?.`| Доступ с нулевым значением | `a?.b`| null, если`a`имеет значение null |
| `!`| Пустое утверждение | `a!`| Выдает, если ноль |
| `..``?..` | Каскад | `obj..a..b`| Цепочка операций над одним и тем же объектом |
| `??=`| Назначение с учетом нуля | `a ??= b`| Назначить, если ноль |
| `as`| Тип литья | `obj as String`| Броски при неудаче |
| `is``is!` | Типовое испытание | `obj is String`| |
| `=>`| Стрелочная функция | `() => expr`| |
### Каскадная нотация
```dart
...
## Краткое содержание
Синтаксис Dart понятен, последователен и создан для повышения производительности. Надежная нулевая безопасность устраняет ошибки нулевых ссылок во время компиляции. Async/await и потоки предоставляют естественные шаблоны для асинхронного программирования. Классы, примеси и запечатанные классы обеспечивают гибкую композицию объектов. Сопоставление шаблонов, записи и модификаторы классов в Dart 3 добавляют выразительности. Хотя основной целью Dart является Flutter, его синтаксис хорошо подходит для любой области приложений — серверных (с Dart Frog или Shelf), инструментов CLI и веб-приложений (скомпилированных в JavaScript).