...
# Delphi/Object Pascal — распространенные ошибки и антипаттерны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в Delphi/Object Pascal с исправлениями.
---

## 1. Неосвобождение объектов (утечки памяти)
```pascal
...
## 3. Индексирование строк (на основе 1)
```pascal
...
## 4. Не использовать`inherited`в конструкторах
```pascal
...
## 5. Антипаттерн: бизнес-логика в формах
```pascal
...
## Краткое содержание
Ловушки Delphi: всегда освобождайте объекты с помощью try/finally, отличайте`=`(сравнение) от`:=`(присвоение), строки индексируются с 1, вызывайте`inherited`в конструкторах и отделяйте бизнес-логику от пользовательского интерфейса. Платформа Delphi VCL/FMX обеспечивает четкое разделение задач.