...
---
# Perl — Справочник по синтаксису
Этот документ представляет собой полный структурированный справочник по синтаксису Perl (5.38+). Он дополняет основной справочник Perl, уделяя особое внимание исчерпывающим синтаксическим шаблонам, регулярным выражениям, ссылкам и возможностям Perl по обработке текста.
---
...
| `x`| Повторение строки | `"ab" x 3`| `"ababab"`|
| `==``!=``<``>``<=``>=` | Числовое сравнение | `$a == $b`| |
| `eq``ne``lt``gt``le``ge` | Сравнение строк | `$a eq $b`| |
| `<=>`| Числовой космический корабль | `$a <=> $b`| Возвращает -1, 0, 1 |
| `cmp`| Струнный космический корабль | `$a cmp $b`| |
| `&&``\|\|``!`| Логический | `$a && $b`| |
| `and``or``not`| Логический (низкая точность) | `$a and $b`| Избегайте — используйте`&&`/`\|\|`|
| `=~``!~` | Соответствие регулярному выражению | `$str =~ /pattern/`| |
| `..``...` | Диапазон/триггер | `1..10`| |
| `?:`| Тройная | `$a ? $b : $c`| |
| `//`| Определено-или | `$a // $b`|  `$b`, если`$a`не определен |
| `=>`| Жирная запятая | `key => 'value'`| Авто-цитирование по голому слову |
### Сигилы — типы переменных
```perl
...
## Объектно-ориентированный Perl (Moo/Moose)
```perl
# Modern OOP with Moo
package User;
use Moo;
...
## Краткое содержание
Синтаксис Perl построен на контексте, символах и принципе «есть более чем один способ сделать это». Регулярные выражения встроены в язык, что делает обработку текста непревзойденной по лаконичности. Ссылки позволяют создавать сложные структуры данных. CPAN предоставляет модули для всего. Современный Perl (с `strict`,`warnings`и лексическими переменными) чист и удобен в сопровождении. Наследие Perl продолжает жить в шаблонах и инструментах, принятых в каждом современном языке.