...
- **Удобочитаемость**: синтаксис, подобный английскому, делает бизнес-логику понятной непрограммистам.
...
| **77** | Отдельный элементарный элемент (без подпунктов) | `77 WS-COUNTER PIC 9(5).`|
...
Тетради — это механизм повторного использования кода COBOL, аналогичный`#include`в C. Они хранятся как отдельные члены и вставляются во время компиляции.
```cobol
...
| `-fixed`| Фиксированный формат (традиционные столбцы 1–80) | `cobc -fixed prog.cbl`|
...
### Вопрос 1. Почему COBOL до сих пор используется в банковском деле спустя 60 с лишним лет?
**О:** COBOL обрабатывает примерно 70–80 % банковских транзакций. Причины:
- Массивные кодовые базы (миллионы строк), которые работают корректно.
- Исключительная надежность — эти системы проверены на производстве десятилетиями.
- Стоимость и риск миграции перевешивают затраты на обслуживание.
- Многословный английский синтаксис COBOL самодокументируется.
- Десятичная арифметика встроена в язык (без ошибок округления с плавающей запятой)
### Вопрос 2: Как COBOL обрабатывает десятичную арифметику без ошибок с плавающей запятой?
**A:** В COBOL есть собственные десятичные типы с фиксированной точностью:
```cobol
...
`V` — это подразумеваемая десятичная точка. COBOL никогда не использует двоичные числа с плавающей запятой для денег.
### Вопрос 3. Какова структура программы на языке COBOL?
**A:** Каждая программа COBOL состоит из четырех разделов:
```cobol
       IDENTIFICATION DIVISION.
           PROGRAM-ID. HELLO.
       ENVIRONMENT DIVISION.
...
### Вопрос 4: Как читать и обрабатывать последовательные файлы в COBOL?
**О:** COBOL превосходно справляется с обработкой файлов:
```cobol
...
### Вопрос 5: Какие инструменты доступны для современной разработки на COBOL?
**О:** Расширения GnuCOBOL (с открытым исходным кодом), IBM Enterprise COBOL, Micro Focus и VS Code предоставляют современные среды разработки. Создайте с помощью `cobc -x program.cob`.
---

## Решение проблем с цепочкой мыслей
### Проблема 1. Создание отчета о клиенте
**Шаг 1. Поймите проблему**
Прочитайте записи о клиентах, подсчитайте итоговые данные и создайте форматированный отчет.
**Шаг 2. Определите подход**
Используйте возможности COBOL по обработке файлов и написанию отчетов.
**Шаг 3. Реализация**```cobol
       IDENTIFICATION DIVISION.
...
**Шаг 4. Проверка**
Перекрестная проверка итоговых данных с исходными данными. Тестируйте с пограничными случаями (пустой файл, нулевые балансы).
### Проблема 2: пакетная обработка с перерывами в управлении
**Шаг 1. Поймите проблему**
Обработка транзакций, сгруппированных по отделам, печать промежуточных итогов.
**Шаг 2. Определите подход**
Использовать логику прерывания управления — обнаруживать изменение группового ключа.
**Шаг 3. Реализация**```cobol
...
**Шаг 4. Проверка**
Убедитесь, что итоговая сумма последней группы напечатана. Убедитесь, что общий итог равен сумме итогов отдела.
---

## Краткое содержание
COBOL — это наследие первых десятилетий компьютерной эры, которое до сих пор активно используется, поскольку замена в больших масштабах невозможна. Мировые банковские и правительственные системы зависят от программ COBOL, которые надежно работают на протяжении десятилетий. Хотя сегодня COBOL обычно не выбирают для нового проекта, этот язык остается важным для поддержания инфраструктуры, поддерживающей глобальные финансы. Нехватка разработчиков COBOL делает эту нишу прибыльной.