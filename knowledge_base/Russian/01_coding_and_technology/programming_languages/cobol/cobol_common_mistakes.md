...
## 1. Не использовать 88-уровневые имена условий
```cobol
* ❌ WRONG — magic numbers
...
## 3. Не инициализировать переменные
```cobol
...
## 4. ВЫПОЛНЕНИЕ без явных ограничений
```cobol
...
## 5. Не использовать современные функции COBOL
```cobol
...
## Краткое содержание
Подробный синтаксис COBOL скрывает ловушки: магические числа без 88-уровневых имен условий, числовое переполнение из-за недостаточного размера предложений PIC, неинициализированные переменные и неограниченные циклы PERFORM. Современный COBOL (2002+) добавляет FUNCTION TRIM, встроенное PERFORM и другие улучшения. Способ COBOL: используйте имена условий 88-го уровня, проверяйте ON SIZE ERROR, инициализируйте с помощью VALUE и связывайте все циклы.