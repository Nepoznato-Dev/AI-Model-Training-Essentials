...
---
# SQL — история версий и эволюция
## Временная шкала
| Версия | Год | Ключевая тема |
|---------|------|-----------|
| ПРОДОЛЖЕНИЕ | 1974 | Оригинальный исследовательский язык IBM (Чемберлин и Бойс) |
| SQL-86 | 1986 | **Первый стандарт ANSI** (SQL-86) |
| SQL-89 | 1989 | Незначительная доработка (ограничения целостности) |
| SQL-92 | 1992 | **Основные**:`JOIN`, подзапросы,`CASE`,`COALESCE`|
| SQL:1999 | 1999 | **Регулярные выражения**, рекурсивные запросы, триггеры, BLOB-объекты |
| SQL:2003 | 2003 | **Оконные функции**, столбцы XML,`GENERATED`|
| SQL:2006 | 2006 | Поддержка XML,`MERGE`|
| SQL:2008 | 2008 |  Триггеры `INSTEAD OF`,`TRUNCATE`,`ORDER BY`в представлениях |
| SQL:2011 | 2011 | **Временные данные** (`AS OF`,`FOR SYSTEM_TIME`),`FETCH FIRST`|
| SQL:2016 | 2016 | **Поддержка JSON**, распознавание шаблонов строк |
| SQL:2019 | 2019 | **Полиморфные табличные функции**,`LISTAGG`|
| SQL:2023 | 2023 | **`JSON_TABLE`**, операции `SET`, улучшения массива |
## Основные вехи
### ПРОДОЛЖЕНИЕ и ранний SQL (1974–1986)
- **1974**: Дональд Чемберлин и Рэймонд Бойс создают ПРОДОЛЖЕНИЕ в IBM Research.
- **Цель**: обработка запросов для System R (реляционная база данных).
- Переименовано в SQL (язык структурированных запросов) из-за конфликта товарных знаков.
- **1986**: Первый стандарт ANSI (SQL-86).
- **1987**: ISO принимает SQL-87.
### SQL-92 — Фонд (1992)
- **Самый важный стандарт** — от него произошел весь современный SQL.
- `INNER JOIN`, `LEFT JOIN`,`RIGHT JOIN`
- Подзапросы (вложенные `SELECT`)
- Выражение `CASE`
-`COALESCE`,`NULLIF`
- Ограничения `UNIQUE`, `CHECK`
- Определение схемы (`CREATE SCHEMA`)
### SQL:1999 — Начало современного SQL (1999)
- Регулярные выражения (`LIKE`,`SIMILAR TO`)
- Рекурсивные запросы (`WITH RECURSIVE`)
- Триггеры
- BLOB/CLOB (большие двоичные/символьные объекты)
- Пользовательские типы (UDT)
-`ORDER BY`в подзапросах
### SQL:2003 — Революция в аналитике (2003 г.)
- **Оконные функции**: `ROW_NUMBER()`, `RANK()`, `DENSE_RANK()`, `LAG()`, `LEAD()`,`SUM() OVER()`
- Тип данных XML и функции
-`GENERATED ALWAYS AS IDENTITY`
-`SAVEPOINT`(контроль транзакций)
- Хэш-функции
### SQL:2011 — Временные данные (2011)
- **Временные таблицы**: `FOR SYSTEM_TIME AS OF`, `VERSIONING`. 
-`FETCH FIRST n ROWS ONLY`(стандартный `LIMIT`)
- Нумерация страниц`OFFSET`/ `FETCH`
### SQL:2016–2023 — JSON и другие возможности (2016 – настоящее время)
- **2016**: тип данных JSON, `JSON_VALUE`, `JSON_QUERY`, `JSON_EXISTS`. 
- **2019**: Полиморфные табличные функции, `LISTAGG`. 
- **2023**:`JSON_TABLE`(реляционное представление JSON), операции `SET`, улучшения массива.
## Эволюция синтаксиса
```sql
