...
|----------|------|---------|-------|
| `+``-``*``/``^`| Арифметика | `2^10`| |
| `÷`| Целочисленное деление | `7 ÷ 3`| То же, что`div(7, 3)`|
| `%``\` | Модуль/остаток | `7 % 3`| |
| `==``!=` | Равенство | `a == b`| Равенство ценностей |
| `===`| идентичность | `a === b`| Тот же объект |
| `<``>``<=``>=` | Сравнение | `a >= b`| Цепочка:`1 < x < 10`|
| `&&``\|\|``!`| Логический | `a && b`| Короткое замыкание |
| `&``\|``⊻``~` | Побитовое | `a & b`| `⊻`— это исключающее ИЛИ |
| `:`| Диапазон | `1:10`| `start:step:stop`|
| `..`| Интервал | `1..10`| Требуются наборы интервалов |
| `∈``∉``⊆`| Установить членство | `x ∈ S`| `in(x, S)`|
| `∘`| Состав | `f ∘ g`| `(f ∘ g)(x) == f(g(x))`|
| `\|>`| Труба | `x \|> f`| `f(x)`|
| `=>`| Пара | `a => b`| Для диктов |
| `...`| Сплат | `f(args...)`| Развернуть коллекцию |
### Связанные сравнения
```julia
...
## Функции и множественная отправка
```julia
# Basic function
function add(x, y)
    x + y
...
## Краткое содержание
Синтаксис Джулии чистый, математический и выразительный. Множественная диспетчеризация позволяет функциям вести себя по-разному в зависимости от типов аргументов, обеспечивая элегантную организацию кода. Макросы обеспечивают мощное метапрограммирование без сложностей макросов препроцессора в стиле C. Система типов с параметрическими типами, абстрактными типами и типами объединения обеспечивает как гибкость, так и производительность. Индексирование, понимание и синтаксис do-block Джулии делают его естественным для математических и научных вычислений. Язык выполняет свое обещание «идти как Python, работать как C» посредством JIT-компиляции с выводом типа.