...
---
# OCaml — распространенные ошибки и антишаблоны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в OCaml с исправлениями.
---

## 1. Неполное сопоставление с образцом
```ocaml
(* ❌ WRONG — missing cases *)
...
## 4. Неправильное использование `Option`
```ocaml
...
## 5. Изменяемое состояние в рекурсивных функциях
```ocaml
...
## Краткое содержание
Ловушки OCaml: неисчерпывающее сопоставление шаблонов (компилятор предупреждает),`=`— структурное равенство, а`==`— физическое, усечение целочисленного деления, использование`Option`вместо исключений и избегание изменяемого состояния в пользу хвостовой рекурсии и сверток. Компилятор OCaml — ваш союзник — прислушивайтесь к его предупреждениям.