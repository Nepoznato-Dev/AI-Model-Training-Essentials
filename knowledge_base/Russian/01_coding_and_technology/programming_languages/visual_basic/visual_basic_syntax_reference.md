...
| **Логично** | `AndAlso`| Короткое замыкание И | `a AndAlso b`|
| | `OrElse`| Короткое замыкание ИЛИ | `a OrElse b`|
| | `Not`| Логическое НЕ | `Not a`|
| | `And`| Побитовое И | `a And b`|
| | `Or`| Побитовое ИЛИ | `a Or b`|
| | `Xor`| Побитовое исключающее ИЛИ | `a Xor b`|
| **Строка** | `&`| Конкатенация | `"Hello" & " " & "World"`|
| | `+=`| Добавить | `s &= " more"`|
| **Задание** | `=`| Назначить | `x = 42`|
| **Ноль** | `Is`| Справочное равенство | `obj Is Nothing`|
| | `IsNot`| Эталонное неравенство | `obj IsNot Nothing`|
| **Тип** | `TryCast`| Безопасное приведение (ничего не возвращает) | `TryCast(obj, String)`|
| | `CType`| Конвертировать | `CType(obj, Integer)`|
| | `DirectCast`| Прямое применение (броски при неудаче) | `DirectCast(obj, String)`|
---
...
## Краткое содержание
Синтаксис Visual Basic понятен и похож на английский, что делает его одним из самых читаемых языков программирования. VB.NET обеспечивает полный доступ к экосистеме .NET, включая LINQ для запросов данных, асинхронность/ожидание для параллельных операций, универсальные шаблоны для безопасности типов и комплексное ООП с классами, интерфейсами и наследованием. Нечувствительность к регистру и подробные ключевые слова (End If, End Sub, End Class) повышают читаемость. Для корпоративных приложений Windows, автоматизации Office (VBA) и быстрого прототипирования в .NET VB.NET остается продуктивным и функциональным языком.