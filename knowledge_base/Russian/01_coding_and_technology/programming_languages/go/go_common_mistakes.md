...
## 1. Захват переменных цикла в горутинах
```go
...
## 2. Игнорирование возвратов ошибок
```go
...
## 3. Ошибка при добавлении фрагмента
```go
...
## 5. Разыменование нулевого указателя в методах структуры
```go
...
## 7. Антипаттерн: преждевременный возврат именованных переменных
```go
...
## 8. Порядок итерации карты случайный
```go
...
## 10. Антипаттерн: загрязнение интерфейса
```go
...
## 11. Не использовать`sync.Once`для инициализации
```go
...
## Краткое содержание
В простоте Go есть его сила, но у него есть тонкие ловушки: утечки горутин из небуферизованных каналов, семантика добавления срезов, которая не изменяет исходные данные, отложенные вызовы, накапливающиеся в циклах, и итерация карты, которая намеренно рандомизирована. Пословицы Go указывают путь: обрабатывать каждую ошибку, принимать интерфейсы, но возвращать структуры, использовать приемники указателей для типов мьютексов и предпочитать примитивы параллелизма (`sync.Once`,`errgroup`, отмена контекста) ручному управлению горутинами. Сначала напишите простой код — Go ценит ясность больше, чем сообразительность.