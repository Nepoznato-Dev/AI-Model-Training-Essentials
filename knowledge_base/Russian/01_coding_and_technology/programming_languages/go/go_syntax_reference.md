...
| `-`| Вычитание | `3 - 2`| `1`| |
| `*`| Умножение | `3 * 2`| `6`| |
| `/`| Отдел | `7 / 2`| `3`| Целочисленное деление для целых |
| `%`| Остаток | `7 % 2`| `1`| Знак соответствует дивидендам |
| `&`| Побитовое И | `5 & 3`| `1`| |
| `\|`| Побитовое ИЛИ | `5 \| 3`| `7`| |
| `^`| Побитовое исключающее ИЛИ | `5 ^ 3`| `6`| Также побитовое дополнение (унарное) |
| `&^`| Бит ясно (И НЕ) | `5 &^ 3`| `4`| Уникально, чтобы пойти |
| `<<`| Левый сдвиг | `5 << 1`| `10`| |
| `>>`| Правый сдвиг | `5 >> 1`| `2`| |
### Операторы сравнения
| Оператор | Имя | Пример | Заметки |
|----------|------|---------|-------|
| `==`| Равный | `x == y`| Только сопоставимые типы |
| `!=`| Не равно | `x != y`| |
| `<`,`>`,`<=`,`>=`| Заказ | `x >= y`| Строки сравниваются лексикографически |
...
## Краткое содержание
Синтаксис Go намеренно минимален — никакого наследования, никакой перегрузки методов, никаких исключений и макросов. Остается чистый, ортогональный набор конструкций: структуры и интерфейсы для композиции, горутины и каналы для параллелизма и явная обработка ошибок для надежности. Сила языка заключается не в сложном синтаксисе, а в элегантном взаимодействии его простых примитивов. Обобщения (Go 1.18+) добавляют гибкость типов, не жертвуя характерной для Go ясностью.