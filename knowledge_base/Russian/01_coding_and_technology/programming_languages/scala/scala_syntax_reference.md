...
|----------|------|---------|-------|
| `+``-``*``/``%`| Арифметика | `a + b`| |
| `**`| Мощность (BigDecimal) | `BigDecimal(2) ** 10`| Не встроено для Int |
| `==``!=` | Равенство | `a == b`| Вызывает`.equals()`— нулевая безопасность |
| `eq``ne` | Справочное равенство | `a eq b`| Тот же экземпляр |
| `<``>``<=``>=` | Сравнение | `a >= b`| Требуется`Ordered`|
| `&&``\|\|``!`| Логический | `a && b`| Короткое замыкание |
| `&``\|``^``~` | Побитовое | `a & b`| |
| `<<``>>``>>>`| Сдвиг | `a << 2`| |
| `+:``:+``::``:::` | Сбор операций | `1 +: list`| Правоассоциативный, если заканчивается на`:`|
| `->`| Создание кортежа | `"key" -> "value"`| `("key", "value")`|
| `=>`| Функция/стрелка | `x => x + 1`| Лямбда или сопоставление типов |
| `<-`| Генератор | `x <- List(1,2,3)`| Для понимания |
### Правила оператора
```scala
...
## Краткое содержание
Синтаксис Scala объединяет объектно-ориентированное и функциональное программирование в рамках мощной системы типов. Сопоставление с образцом, for-comprehension и case-классы обеспечивают выразительное моделирование данных. Признаки обеспечивают гибкую композицию без ограничений единственного наследования. Система типов — с обобщениями, вариациями, границами типов и непрозрачными типами — улавливает ошибки во время компиляции, обеспечивая при этом абстракции с нулевой стоимостью. Scala 3 упрощает язык с помощью перечислений, методов расширения, данных/использования и непрозрачных типов, делая язык более доступным, сохраняя при этом его полную мощь.