...
# Scala — распространенные ошибки и антишаблоны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в Scala с исправлениями.
---
...
## 2. Не использовать сопоставление с образцом для закрытых типов
```scala
// ❌ WRONG — if/else chain
...
## 3. Производительность коллекций
```scala
...
## 4. Ад обратного вызова в будущем
```scala
...
## 5. Злоупотребление неявными конверсиями
```scala
...
## 6. Неправильная обработка `Option`
```scala
...
## 8. Стирание типов с помощью дженериков
```scala
...
## Краткое содержание
Возможности Scala связаны с ответственностью: отдавайте предпочтение`val``var` , используйте сопоставление с образцом для запечатанных типов, правильно обрабатывайте`Option`(никогда не вызывайте`.get`), используйте for-comprehensions вместо вложенных Futures и избегайте null. Scala 3 упрощает многие из этих шаблонов с помощью перечислений, методов расширения и типов объединения. Ключевой принцип: если компилятор может это проверить, пусть компилятор это проверит.