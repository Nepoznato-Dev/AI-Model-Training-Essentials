...
---
# Межъязыковое сравнение — обработка ошибок
## Обзор моделей обработки ошибок
| Модель | Языки | Механизм |
|-------|-----------|-----------|
| **Исключения (попробовать/переловить)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **Типы результатов/опций** | Rust, Haskell, OCaml, Scala (также) | `Result<T, E>`,`Maybe a`|
| **Несколько возвращаемых значений** | Вперёд, Луа | `value, err := f()`|
| **Коды ошибок** | С, Фортран | Возвращает код ошибки, проверьте вручную |
| **Ошибки сопоставления шаблонов** | Эликсир, Эрланг | `{:ok, value}`/`{:error, reason}`|
| **Условия (перезапускаемые)** | Общий Лисп | `restart-case`,`handler-bind`|
| **Без обработки ошибок** | Скретч, SQL (базовый) | Н/Д |
| **Контракты** | Ада, Эйфель |  Условия`Pre`,`Post`|
...
## Резюме: парадигмы обработки ошибок
| Парадигма | Подход | Плюсы | Минусы |
|----------|----------|------|------|
| **Исключения** | бросить/поймать | Чистый счастливый путь, следы стека | Можно забыть, злоупотребление потоком контроля |
| **Типы результатов** | Возврат`Ok`/`Err`| Явные, нельзя игнорировать ошибки | Подробно, требуется оператор`?`|
| **Коды ошибок** | Вернуть int статус | Простой, C-совместимый | Легко игнорировать, нет контекста |
| **Множественный возврат** | `value, err := f()`| Явный, идиоматический | Можете забыть проверить`err`|
| **Сопоставление с образцом** | `{:ok, v}`/`{:error, r}`| Исчерпывающий, функциональный | Подробно без`with`|
| **Условия** | Перезапускаемые обработчики | Самый мощный, восстанавливаемый | Сложный, редко понимаемый |