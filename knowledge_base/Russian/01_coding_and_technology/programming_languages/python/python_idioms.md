...
## Понимание списков и генераторы
```python
# ❌ Not idiomatic
squares = []
for x in range(10):
...
## Питонические условные выражения
```python
# ❌ Not idiomatic
if len(items) > 0:
    process(items)
...
## Краткое содержание
Pythonic-код следует дзен Python: читабельный, явный, простой и плоский. Ключевые идиомы включают в себя: понимание списка/диктанта/набора, менеджеры контекста (`with`), обработку ошибок EAFP,`enumerate`/`zip`для итерации,`dataclass`для контейнеров данных, подсказки типов, распаковку и проверки истинности/ложности. Сообщество Python ценит читабельность, и «должен быть один очевидный способ сделать это». Следуйте PEP 8 для стиля, используйте`ruff`для проверки и, когда это возможно, отдавайте предпочтение стандартной библиотеке сторонним решениям.