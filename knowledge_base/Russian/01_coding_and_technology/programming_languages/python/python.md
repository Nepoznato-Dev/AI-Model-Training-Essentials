...
| **GIL (глобальная блокировка интерпретатора)** | Предотвращает настоящий многопоточный параллелизм при работе с загрузкой ЦП | Используйте `multiprocessing`,`asyncio`или очереди задач, такие как Celery |
...
### Стандартная библиотека («Батареи в комплекте»)
...
|  __ЗАЩИЩЕНО_3__ | Кодирование/декодирование JSON | `json.loads(response_text)`|
...
`asyncio` Python обеспечивает одновременный ввод-вывод без потоков, что важно для веб-скраперов, серверов чата и клиентов API:
```python
import asyncio
import aiohttp
...
| ФастAPI | Современный, асинхронный, управляемый типами | API и микросервисы (на данный момент самые быстрорастущие) |
...
|---------|------|-----------------|
| 3.10 | 2021 | Сопоставление структурных шаблонов (`match/case`), улучшенные сообщения об ошибках |
...
| 3.13 | 2024 | Экспериментальный режим со свободной резьбой (без GIL), улучшенный REPL |
