...
# Python — распространенные ошибки и антипаттерны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в Python. Каждая запись показывает неправильный подход, объясняет, почему он не работает, и предлагает правильное решение. Понимание этих ловушек поможет вам писать более надежный код на Python.
---
...
**Почему это происходит:** Аргументы по умолчанию оцениваются один раз во время определения функции, а не при каждом вызове. Изменяемое значение по умолчанию (list, dict, set) является общим для всех вызовов.
---
...
## 3. Позднее связывание в замыканиях
```python
...
## 4. Использование`type()`вместо `isinstance()`
```python
...
## 5. Не использовать менеджеры контекста для ресурсов
```python
...
## 6. Оператор`is`для сравнения значений
```python
...
## 8. Непонимание правил области действия Python (LEGB)
```python
...
## 10. Анти-шаблон: использование`except:`без исключения
```python
...
## 11. Возврат изменяемых ссылок во внутреннее состояние
```python
...
## 12. Не использовать`setdefault`или `collections.defaultdict`
```python
...
## 14. Непонимание истощения генератора
```python
...
## 15. Антипаттерн: объекты-боги/перегруженные классы
```python
...
## Краткое содержание
Простота Python обманчива — изменяемые значения по умолчанию, позднее закрытие привязок, правила области действия и управление ресурсами — все это имеет тонкое поведение, которое заманивает разработчиков в ловушку. Ключевые принципы, которые следует запомнить: значения по умолчанию оцениваются один раз,`is`проверяет идентичность, а не равенство, всегда использовать контекстные менеджеры для ресурсов, перехватывать определенные исключения и отдавать предпочтение композиции над объектами-богами. Написание кода на Python означает понимание этих ловушек и использование функций языка (контекстные менеджеры, генераторы, модуль `collections`) по назначению.