...
# Kotlin — распространенные ошибки и антишаблоны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в Kotlin. Каждая запись показывает неправильный подход, объясняет, почему он не работает, и предлагает правильное решение.
---

## 1. Типы платформ и исключение NullPointerException
```kotlin
...
## 2. Неправильное использование `lateinit`
```kotlin
...
## 3.`data class`без правильного`equals`/ `hashCode`
```kotlin
...
## 4. Неправильное использование функций области видимости
```kotlin
...
## 5. Сопрограммы: не обрабатывают отмену
```kotlin
...
## 6. Анти-паттерн: Kotlin в стиле Java
```kotlin
...
## 7. Исчерпывание запечатанных классов
```kotlin
...
## 8. Затенение функции расширения
```kotlin
...
## 9. Не использовать`by lazy`для дорогостоящей инициализации
```kotlin
...
## 10. Путаница между Flow и LiveData
```kotlin
...
## Краткое содержание
Лаконичность Kotlin может скрыть тонкие проблемы: типы платформ из взаимодействия Java, сбои `lateinit`, изменяемые классы данных, нарушающие коллекции, и отмену сопрограмм. Путь Kotlin: использовать нулевую безопасность (никогда не используйте `!!`), использовать правильную функцию области видимости (`let` для преобразований,`apply`для конфигурации,`also`для побочных эффектов), использовать запечатанные классы с исчерпывающим `when`, обрабатывать отмену сопрограмм и писать идиоматический Kotlin (а ​​не Java с синтаксисом Kotlin). Компилятор — ваш проводник — если он предупреждает, прислушайтесь.