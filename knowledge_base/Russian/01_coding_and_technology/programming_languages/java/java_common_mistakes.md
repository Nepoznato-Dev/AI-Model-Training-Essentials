...
-->
# Java — распространенные ошибки и антишаблоны
В этом документе перечислены наиболее распространенные ошибки, ловушки и антишаблоны в Java. Каждая запись показывает неправильный подход, объясняет, почему он не работает, и предлагает правильное решение.
---

## 1.`equals()`против`==`для объектов
```java
...
## 3. Не закрытие ресурсов (попробуйте с ресурсами)
```java
...
## 4. Ловушки целочисленного автобокса
```java
...
## 7. Не обрабатывать InterruptedException
```java
...
## 8. Возврат пустых коллекций вместо пустых
```java
...
## 9. Изменяемые объекты как ключи HashMap
```java
...
## 10. Антипаттерн: перехват`Exception`или `Throwable`
```java
...
## 11. Не переопределять`hashCode`при переопределении `equals`
```java
...
## 12. Условия гонки с двойной проверкой блокировки
```java
...
## Краткое содержание
Многословие Java может скрыть незначительные ошибки:`==`против `equals()`, параллельная модификация, утечки ресурсов, ловушки производительности автобокса и контракт Equals/hashCode. Современный подход Java таков: используйте`Objects.equals()`для сравнения с нулевым значением, всегда используйте попытку с ресурсами, возвращайте пустые коллекции вместо нулевых, перехватывайте определенные исключения и следуйте принципу единой ответственности. Функции Java 8+ (потоки, опционально, ссылки на методы) сокращают количество шаблонов, делая код более безопасным и выразительным.