...
-->
# Java — Справочник по синтаксису
Этот документ предоставляет исчерпывающий структурированный справочник по синтаксису Java. Он дополняет основной справочник по Java, уделяя особое внимание исчерпывающим синтаксическим шаблонам, таблицам операторов и внутренней механике JVM, ООП и параллелизма.
---
...
| `-`| Вычитание | `3 - 2`| `1`| |
| `*`| Умножение | `3 * 2`| `6`| |
| `/`| Отдел | `7 / 2`| `3`| Целочисленное деление для`int`; `3.5`для`double`|
| `%`| Остаток | `7 % 2`| `1`| |
...
|----------|------|---------|-------|
| `==`| Равный | `x == y`| Ссылочное равенство для объектов |
| `!=`| Не равно | `x != y`| |
| `<`,`>`,`<=`,`>=`| Реляционный | `x >= y`| |
| `&&`| Логическое И | `a && b`| Короткое замыкание |
| `\|\|`| Логическое ИЛИ | `a \|\| b`| Короткое замыкание |
| `!`| Логическое НЕ | `!true`| `false`|
| `instanceof`| Проверка типа | `obj instanceof String`| Также сопоставление с образцом (Java 16+) |
### Побитовые операторы
| Оператор | Имя | Пример | Заметки |
|----------|------|---------|-------|
| `&`| И | `5 & 3`| `1`|
...
| Приоритет | Операторы |
|------------|-----------|
| 1 (самый высокий) | Постфикс:`expr++``expr--` |
| 2 | Унарный:`++expr``--expr``+expr``-expr``~``!` |
| 3 | В ролях:`(Type)expr`|
| 4 | Мультипликативный:`*``/``%`|
| 5 | Добавка:`+``-` |
| 6 | Сдвиг:`<<``>>``>>>`|
| 7 | Относительное:`<``>``<=``>=``instanceof`|
| 8 | Равенство:`==``!=` |
| 9 | Побитовое И:`&`|
...
## Структуры данных
### Платформа коллекций
| Интерфейс | Реализации | Заказал | Потокобезопасный вариант |
|-----------|----------------|---------|---------------------|
| `List`| `ArrayList`,`LinkedList`| Да | `CopyOnWriteArrayList`|
| `Set`| `HashSet`,`TreeSet`,`LinkedHashSet`| Варьируется | `CopyOnWriteArraySet`|
| `Map`| `HashMap`,`TreeMap`,`LinkedHashMap`| Варьируется | `ConcurrentHashMap`|
| `Queue`| `ArrayDeque`,`PriorityQueue`| Варьируется | `ConcurrentLinkedQueue`|
| `Deque`| `ArrayDeque`| Оба конца | `ConcurrentLinkedDeque`|
### Списки
```java
// Creation
...
## Краткое содержание
Синтаксис Java претерпел значительные изменения с момента своего появления в 1995 году. Современная Java (21+) включает записи, запечатанные классы, сопоставление с образцом, виртуальные потоки и шаблоны строк, что делает ее более лаконичной и выразительной, чем когда-либо. Основной синтаксис остается знакомым (подобный C, строго типизированный, основанный на классах), но добавление лямбда-выражений, Stream API и`var`значительно сократило шаблонный код. Сила языка заключается в его обширной экосистеме, гарантиях обратной совместимости и оптимизации производительности JVM посредством JIT-компиляции.