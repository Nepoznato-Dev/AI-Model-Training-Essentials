...
Промпт-инжиниринг — это практика проектирования, доработки и оптимизации входных промптов, чтобы получать от языковой модели наилучший возможный результат. Это одновременно искусство и наука, а также основной интерфейс управления поведением LLM без дообучения.
...
**С контекстом:**
> "Ты старший Python-разработчик. Напиши функцию для сортировки списка словарей по заданному ключу. Используй аннотации типов и обработай пограничные случаи. Аудитория — начинающие разработчики."
...
Большинство API для LLM поддерживают многоходовую структуру:
...
**Примеры:**
- "Ты разработчик ядра Linux, который объясняет управление памятью недавнему выпускнику."
- "Ты дружелюбный нутрициолог, который даёт клиенту общие рекомендации."
- "Ты циничный технокритик, который обозревает новый гаджет."
...
- **Top-p** (сэмплирование по ядру распределения): Отсекает хвост вероятностной массы на определённом накопленном пороге. 0.9 означает, что модель выбирает токены из верхних 90% наиболее вероятных вариантов. Обычно настраивают либо `temperature`, либо `top-p`, но не оба одновременно.
...
Добавь строку документации (docstring).
...
### Классификация
Классифицируй следующий отзыв клиента как [положительный, нейтральный, отрицательный].
...
- **Измеряйте успех** с помощью человеческой оценки или автоматических метрик (например, точного совпадения, BLEU, пользовательских оценок).
- **Ведите реестр промптов** (простой текстовый файл или таблицу) с самим промптом, версией и наблюдаемым качеством.
