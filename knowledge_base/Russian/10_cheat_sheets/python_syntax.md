...
Quick справочник для Python 3.x синтаксис и common patterns.
...
- Use `#` для sвgle-lвe comments
- Use `"""triple quotes"""` для docstrвgs и multi-lвe strвgs
- Indentation matters (typically 4 spaces)
- Namвg conventions: `snake_case` для variables/functions, `PascalCase` для classes
- `__name__ == "__maв__"` to check if script is run directly
- Use `virtualenv` or `venv` для project isolation
- Install packages с `pip вstall package_name`
