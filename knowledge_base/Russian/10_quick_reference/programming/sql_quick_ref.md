...
-`FLOAT`- Приблизительное значение с плавающей запятой
-`DOUBLE`— плавающее число двойной точности
### Строка
...
-`YEAR`— Значение года
### Логическое значение
-`BOOLEAN`или`BOOL`— правда/ложь
### Бинарный
-`BLOB`— Большой двоичный объект
-`BINARY`— фиксированный двоичный файл
-`VARBINARY`— двоичная переменная
---
...
- Выберите только необходимые столбцы (избегайте `SELECT *`).
- Используйте`EXPLAIN`для анализа производительности запросов.
- Нормализовать данные соответствующим образом
- Используйте подготовленные операторы, чтобы предотвратить SQL-инъекцию.
❌ **Не следует:**
— Используйте функции для индексированных столбцов в предложениях WHERE.
- Создать слишком много индексов (замедляет запись)
- Используйте`SELECT DISTINCT`без необходимости.
- Игнорировать планы выполнения запросов.
- Сохраняйте вычисленные значения, когда их можно вычислить.
---
...
*Последнее обновление: июль 2026 г. | Стандарт SQL (совместим с MySQL/PostgreSQL)*