...
## Краткое содержание
Сценарии Bash повышают эффективность каждого, кто работает с компьютерами. Вам не нужно запоминать каждый флаг — вам нужно знать, что возможно и где это посмотреть. Начните с основ: переменных, условий, циклов, каналов. Затем добавьте инструменты обработки текста (grep, sed, awk) по мере необходимости. И всегда используйте`set -euo pipefail`— в будущем вы будете благодарны.