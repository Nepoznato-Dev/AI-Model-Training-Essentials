...
A practical リファレンス に developers sysadms — core concepts, protocols, comms, troubleshoot.

---

# # The OSI Model (7 Layers)

A conceptual framework に underst ネットワーク コミュニケーション.

| Layer | Name | Function | Example protocols |
|-------|------|----------|-------------------|
| 7 | Application | End-user services | HTTP, HTTPS, FTP, SMTP, DNS, SSH |
| 6 | Presentation | データ にmatt, encryption, compression | TLS, JPEG, ASCII |
| 5 | Session | Connection 管理 | NetBIOS, RPC |
| 4 | Transport | End-to-end delivery, error correction, flow control | TCP, UDP |
| 3 | ネットワーク | Rout, address | IP, ICMP, OSPF, BGP |
| 2 | データ Lk | Fram, error detection, MAC addresses | Ernet, Wi-Fi, PPP |
| 1 | Physical | Raw bit transmission | Ernet cables, fiber optics, radio waves |

In practice, **TCP/IP model** (4 layers: Lk, Internet, Transport, Application) is more commonly used に ternet.
...
- 合計: ~4.3 billion addresses (but exhausted practice).

# ## IPv6
- 128-bit address, written hex: `2001:0db8:85a3:0000:0000:8a2e:0370:7334`
- 合計: 2¹²⁸ addresses (practically fite).

# ## Private IP Ranges (RFC 1918)
These are not routable on ternet; used side local ネットワークs:
- `10.0.0.0/8` (10.0.0.0 – 10.255.255.255)
- `172.16.0.0/12` (172.16.0.0 – 172.31.255.255)
- `192.168.0.0/16` (192.168.0.0 – 192.168.255.255)

# ## CIDR Notation
`192.168.1.0/24` means first 24 bits are ネットワーク prefix; last 8 bits are hosts. It 含むs addresses `192.168.1.0` to `192.168.1.255`.
...
| **NS** | Nameserver に doma |
| **SRV** | Service record (e.g., に SIP) |
...
Firewall (Linux ip表/nf表)
bash
sudo ufw status # Ubuntu: simple firewall
sudo ip表 -L -n # List rules
Network Statistics
bash
...
AWS ELB, Azure Load Balancer, GCP Cloud Load Balancing (クラウド)
...
Is the firewall blocking the port? (Check ip表/ufw or クラウド security groups).
...
```markdown
# DevOps System Admistration

A practical ガイド to manag servers, automat operations, mata reliable frastructure.
...
2 different media types (e.g., SSD + tape, or local + クラウド).

1 copy off-site (e.g., クラウド or remote data centre).
...
Use secrets (via Docker secrets or 環境 with care).
...
Deploy: Update staging/production 環境.
...
Ansible
Agentless configuration management using YA機械学習.
...
Use variables and secrets (AWS_SECRET_ACCESS_KEY via 環境, not hardcoded).
