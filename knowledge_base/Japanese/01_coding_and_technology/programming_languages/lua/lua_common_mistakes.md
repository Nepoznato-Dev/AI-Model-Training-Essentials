...
-->
# Lua — よくある間違いとアンチパターン
このドキュメントでは、Lua で最も一般的な間違い、罠、およびアンチパターンを修正とともにカタログ化します。
---
...
＃＃ まとめ
Lua のミニマリズムは罠を生み出します。1 ベースのインデックス付け、`nil` と`false`のみが false、テーブルは参照 (値ではない)、変数はデフォルトでグローバル、`pairs` は順序付けされていないが、`ipairs` は順序付けされています。 Lua の方法は、常に`local`を使用し、配列には`ipairs`を使用し、辞書には`pairs`を使用し、文字列の構築には`table.concat`を使用し、OOP にはメタテーブルを使用します。 Lua のシンプルさはその強みですが、エッジケースを尊重してください。