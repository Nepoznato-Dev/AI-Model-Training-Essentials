...
# Lua — 構文リファレンス
このドキュメントは、Lua (5.4) の包括的で構造化された構文リファレンスを提供します。これは、網羅的な構文パターン、テーブルとメタテーブル、コルーチン、および Lua の独自の設計哲学に焦点を当て、メインの Lua リファレンスを補完します。
---
...
|----------|------|----------|----------|
| `+``-``*``/``%``^` |算数 | `2 ^ 10`| `^`はべき乗 |
| `//`|フロア区分 | `7 // 2`| 3 を返します (Lua 5.3 以降) |
| `==`|`~=`|平等 | `a == b`| `~=`は「等しくありません」 |
| `<``>``<=``>=` |比較 | `a >= b`| |
| `and``or``not`|論理 | `a and b`|ブール値ではなくオペランドを返します |
| `..`|連結 | `"hello" .. " world"`| |
| `#`|長さ | `#t`|文字列またはテーブルの長さ |
| `&``\|``~``<<``>>`|ビットごと | `a & b`| Lua 5.3+ (整数演算) |
### 論理演算子 — 非ブール値の戻り値
```lua
...
＃＃ まとめ
Lua の構文は意図的に最小限になっており、言語全体を 1 つのページで説明できます。テーブルは、ユニバーサル データ構造 (配列、辞書、オブジェクト、モジュール) として機能します。メタテーブルは、言語を複雑にすることなく拡張性を提供します。コルーチンにより、協調的なマルチタスクとエレガントなイテレータ パターンが可能になります。この言語の能力はその埋め込みやすさから生まれます。Lua はホスト アプリケーション内で動作するように設計されており、最小限のオーバーヘッドでスクリプト機能を提供します。ゲーム開発、構成、組み込みシステムにとって、Lua のシンプルさは最大の強みです。必要なことを正確に実行する、それ以上のことは何もしない小さな言語です。