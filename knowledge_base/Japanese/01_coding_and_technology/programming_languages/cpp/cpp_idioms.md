...
このガイドでは、クリーンで最新の C++ (20/23) コードを作成するための慣用的なパターンとベスト プラクティスについて説明します。
---
...
＃＃ まとめ
最新の C++ のイディオムでは、RAII とスマート ポインター、移動セマンティクス、型推定の `auto`、範囲とアルゴリズム (C++20)、テンプレート制約の概念、不在/エラーの`std::optional`/ `std::expected`、ルール オブ ゼロ、およびコンパイル時の計算の`constexpr`が強調されています。フォーマットには`clang-format`に従い、lint には`clang-tidy`に従い、`-Wall -Wextra -Werror`でコンパイルします。 C++ コミュニティはゼロコストの抽象化を重視し、「使用しないものにはお金を払いません」。