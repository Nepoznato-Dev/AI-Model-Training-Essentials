...
|----------|------|----------|----------|
| `+``-``*``/``%`|算数 | `a + b`| C と同じ |
| `++``--` |インクリメント/デクリメント | `++i`、`i++`|事前インクリメントを優先する |
| `==``!=``<``>``<=``>=` |比較 | `a <=> b`| `<=>`は 3 者間比較 (C++20) |
| `&&``\|\|``!`|論理 | `a && b`|短絡 |
| `&``\|``^``~``<<``>>` |ビットごと | `a & b`| C と同じ |
| `?:`| 3進法 |  XQZマーカー30XQZ | |
### C++ 固有の演算子
...
| `::`|スコープの解像度 | `std::cout`| |
| `.*``->*` |メンバーへのポインター | `obj.*pmf`| |
| `<=>`|宇宙船 (C++20) | `a <=> b`|`strong_ordering`| を返します。
| `co_await`|コルーチンのサスペンド | `co_await expr`| C++20 |
| `co_yield`|コルーチンの収量 | `co_yield value`| C++20 |
| `co_return`|コルーチンの戻り値 | `co_return value`| C++20 |
### 演算子のオーバーロード
```cpp
class Vector2D {
    double x_, y_;
...
## STL コンテナとアルゴリズム
### コンテナの概要
|コンテナ |ヘッダー |注文済み |検索 |メモ |
|----------|----------|----------|----------|----------|
| `vector`| `<vector>`|はい | O(n) |シーケンスのデフォルトの選択 |
| `deque`| `<deque>`|はい | O(n) |両端での高速プッシュ/ポップ |
| `list`| `<list>`|はい | O(n) |二重リンクリスト |
| `array`| `<array>`|はい | O(n) |固定サイズ、スタック割り当て |
| `set`/`multiset`| `<set>`|はい | O(log n) |赤黒の木 | 写真 赤黒の木
| `map`/`multimap`| `<map>`|はい (キーによる) | O(log n) | Key-Value、赤黒ツリー |
| `unordered_set`| `<unordered_set>`|いいえ | O(1) 平均 |ハッシュテーブル |
| `unordered_map`| `<unordered_map>`|いいえ | O(1) 平均 |ハッシュテーブル |
### 主要なアルゴリズム
```cpp
#include <algorithm>
#include <numeric>
...
＃＃ まとめ
C++ 構文は、低レベルのポインター操作から高レベルのテンプレート メタプログラミング、コルーチン ベースの非同期まで多岐にわたります。 C++17/20/23 による言語の進化により、安全性 (スマート ポインター、`std::optional`、概念)、表現力 (構造化バインディング、範囲、`std::format`)、およびパフォーマンス (ゼロコストの抽象化を可能にする概念、スタック オーバーヘッドなしの非同期用のコルーチン) が劇的に向上しました。基本的な哲学は変わりません。使用しないものにはお金を払うべきではなく、言語はベアメタル言語からドメイン固有の言語まで、あらゆるレベルの抽象化をサポートする必要があります。