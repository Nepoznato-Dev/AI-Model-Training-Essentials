...
|----------|------|----------|----------|
| `+``-``*``/``%`|算数 | `a + b`| `+`も文字列を連結します。
| `++`|`--`|インクリメント/デクリメント | `++i`|事前インクリメントを優先する |
| `==`|`!=`|平等 | `a == b`|オーバーライド可能。レコードは値の等価性を使用します。
| `<``>``<=``>=` |リレーショナル | `a >= b`| |
| `&&``\|\|``!`|論理 | `a && b`|短絡 |
| `&``\|``^``~` |ビットごと | `a & b`| |
| `<<``>>``>>>`|シフト |  XQZマーカー30XQZ | `>>>`符号なし右シフト (C# 11) |
| `??`|ヌル合体 | `a ?? b`|`a`が null の場合は`b`を返します。
| `?.`| Null 条件付き | `a?.B`|`a`が null の場合は null を返します。
| `!`|ヌル寛容 |  XQZマーカー40XQZ | Null 許容の警告を抑制します |
| `is`|タイプパターン | `x is string s`|パターンマッチング |
| `as`|安全なキャスト | `x as string`|失敗した場合は null を返します。
| `nameof`|名前リテラル | `nameof(x)`| `"x"`— コンパイル時 |
| `typeof`|型オブジェクト | `typeof(int)`|  XQZマーカー50XQZ |
| `sizeof`|バイト単位のサイズ | `sizeof(int)`| `4`(安全でないコンテキスト) |
### 演算子の優先順位 (最高から最低)
|優先順位 |オペレーター |
|-----------|----------|
| 1 (最高) |プライマリ:`()``.``?.``[]``()`(呼び出し)`++``--` (後置)`new``typeof``sizeof`|
| 2 |単項:`+``-``!``~``++``--` (プレフィックス)`(T)x``await` |
| 3 |乗算:`*``/``%`|
| 4 |添加剤:`+``-` |
| 5 |シフト:`<<``>>``>>>`|
| 6 |関係:`<``>``<=``>=``is``as` |
...
＃＃ まとめ
最新の C# は、オブジェクト指向、関数型、コンポーネント指向のプログラミングの長所を組み合わせたマルチパラダイム言語です。その構文は、Java のような冗長なコードから、パターン マッチング、レコード、コレクション式、プライマリ コンストラクターを備えた簡潔な式まで、劇的に進化しました。 LINQ は、すべてのコレクションにわたって均一なクエリ API を提供します。 Async/await により、非同期プログラミングが自然になります。型システムは、null 許容参照型、制約付きジェネリック、パターン マッチングを通じて、安全性と実用性のバランスをとります。 C# は急速に進化し続けており、リリースごとに下位互換性を維持しながらボイラープレートを削減する機能が追加されています。