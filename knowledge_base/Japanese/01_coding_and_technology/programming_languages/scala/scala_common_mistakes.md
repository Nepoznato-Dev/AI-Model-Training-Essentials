...
# Scala — よくある間違いとアンチパターン
このドキュメントでは、Scala で最も一般的な間違い、罠、およびアンチパターンを修正とともにカタログ化します。
---

## 1.`val`の代わりに`var`を使用する
```scala
...
＃＃ まとめ
Scala の能力には責任が伴います。`var`よりも`val`を優先し、シールされた型にパターン マッチングを使用し、`Option`を適切に処理し (決して`.get`を呼び出さない)、ネストされた Future の代わりに for 内包表記を使用し、null を避けます。 Scala 3 では、列挙型、拡張メソッド、共用体型を使用して、これらのパターンの多くを簡素化しています。重要な原則は、コンパイラがチェックできる場合は、コンパイラにチェックさせます。