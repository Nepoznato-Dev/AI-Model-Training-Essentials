...
|----------|------|----------|----------|
| `+``-``*``/``%`|算数 | `a + b`| |
| `**`|パワー (BigDecimal) | `BigDecimal(2) ** 10`| Int には組み込まれていません |
| `==`|`!=`|平等 | `a == b`|`.equals()`を呼び出す — null セーフ |
| `eq`|`ne`|参照の等価性 | `a eq b`|同じインスタンス |
| `<``>``<=``>=` |比較 | `a >= b`|`Ordered`が必要 |
| `&&``\|\|``!`|論理 | `a && b`|短絡 |
| `&``\|``^``~` |ビットごと | `a & b`| |
| `<<``>>``>>>`|シフト | `a << 2`| |
| `+:``:+``::``:::` |コレクション作戦 | `1 +: list`|`:`で終わる場合は右結合 |
|  XQZマーカー40XQZ |タプルの作成 | `"key" -> "value"`| `("key", "value")`|
| `=>`|機能/矢印 | `x => x + 1`|ラムダまたは型マッピング |
| `<-`|ジェネレーター | `x <- List(1,2,3)`|理解のために |
### 演算子のルール
```scala
...
＃＃ まとめ
Scala の構文は、強力な型システムの下でオブジェクト指向プログラミングと関数型プログラミングを統合します。パターン マッチング、for 内包表記、および case クラスは、表現力豊かなデータ モデリングを提供します。特性により、単一継承の制限なしで柔軟な構成が可能になります。ジェネリックス、バリアンス、型境界、および不透明型を備えた型システムは、ゼロコストの抽象化を有効にしながら、コンパイル時にエラーを捕捉します。 Scala 3 は、列挙型、拡張メソッド、与えられた/使用する型、および不透明な型を使用して言語を簡素化し、その言語の能力を最大限に維持しながら、言語をより親しみやすくしています。