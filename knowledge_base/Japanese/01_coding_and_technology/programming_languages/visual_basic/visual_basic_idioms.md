...
---
# Visual Basic — 慣用的なパターンとベスト プラクティス
このガイドでは、クリーンな Visual Basic (.NET) コードを記述するための慣用的なパターンについて説明します。
---
...
＃＃ まとめ
Visual Basic のイディオムは、タイプ セーフティのための `Option Strict On`、リソース管理のための`Using`ブロック、文字列補間、コレクションのための LINQ、全体の `Async/Await`、特定の例外処理、パターン マッチングや null 条件演算子のような最新の機能を強調しています。 VB は読みやすさを重視します。「コードはよく整理された仕様のように読めるべきです」。