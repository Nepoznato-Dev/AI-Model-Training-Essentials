...
|バージョン |年 |主要テーマ |
|----------|------|----------|
|フローマティック | 1955年 |グレース・ホッパーのビジネス言語の先駆者 |
| COBOL-60 | 1959年 | **最初の COBOL** (CODASYL 委員会) |
| COBOL-61 | 1961年 |レポート ライター、`PERFORM` の改善 |
| COBOL-68 | 1968年 |最初の ANSI 規格 (X3.1-1968) |
| COBOL-74 | 1974年 | `IF`/`ELSE`、`EVALUATE`(スイッチ)、一般化 |
| COBOL-85 | 1985年 | **構造化プログラミング**:`END-IF`、`END-PERFORM`、スコープ ターミネータ |
| COBOL2002 | 2002年 | **OOP**: クラス、メソッド、継承、`FUNCTION` |
| COBOL 2014 | 2014年 | **XML**、`JSON` (部分)、`BOOLEAN` タイプ |
| COBOL 2023 | 2023年 | **ネイティブ JSON**、UTF-8、`LIST` コレクション |
## 主要なマイルストーン
### COBOL の誕生 (1959)
- **1959**: CODASYL (データ システム言語会議) が COBOL を作成
- **グレース・ホッパーの影響**: 「ビジネス言語は英語のようにあるべきである」
- **目標**: ポータブルなビジネス言語 — あらゆるコンピュータで実行可能
- COBOL-60: 最初のバージョン — ファイル処理、レポート、算術演算
### COBOL 68 ～ 74: 標準化 (1968 ～ 1974 年)
- **COBOL-68**: 最初の ANSI 標準
- **COBOL-74**:`EVALUATE`(switch ステートメント)、構造化`IF`/`ELSE`
- COBOL が世界中で主流のビジネス言語になる
### COBOL 85: 構造化 COBOL (1985)
- **スコープ ターミネータ**:`END-IF`、`END-PERFORM`、`END-READ`
-`EVALUATE`/`WHEN`(スイッチ)
-`PERFORM`/`END-PERFORM`(インラインループ)
- `SECTION`の改善
- これは、ほとんどの COBOL コードが書かれているバージョンです
### COBOL 2002: オブジェクト指向 COBOL (2002)
- **クラスとメソッド** —`CLASS-ID`、`METHOD-ID`
- **継承** —`INHERITS`
- **インターフェイス** —`IMPLEMENTS`
-`FUNCTION`キーワード
・`BOOLEAN`型（一部）
- 入れ子になったプログラム
### COBOL 2014–2023: 最新の COBOL (2014–現在)
- **2014**: XML サポート、部分的な JSON、`BOOLEAN` タイプ
- **2023**: **ネイティブ JSON** (PARSE JSON、GENERATE JSON)、UTF-8、`LIST` コレクション
- 現代に向けて進化を続けるCOBOL
## 構文の進化
```cobol
