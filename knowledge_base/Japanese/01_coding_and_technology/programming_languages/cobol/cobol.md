...
＃＃ まとめ
COBOL は、コンピューティングの初期の時代の名残であり、死ぬことを拒否します。なぜなら、そうする余裕がないからです。世界の銀行システムと政府システムは、何十年にもわたって確実に実行されてきた COBOL プログラムに依存しています。現在、新しいプロジェクトに COBOL を選択する人はいないでしょうが、この言語は世界金融を支えるインフラストラクチャを維持するために依然として非常に重要です。 COBOL 開発者が不足しているため、COBOL は驚くほど儲かるニッチ市場となっています。