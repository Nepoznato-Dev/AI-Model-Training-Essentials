...
＃＃ まとめ
COBOL の構文は設計上冗長であり、すべてのステートメントが英語のように読めます。 PIC 句を使用したデータ記述はフィールド形式を正確に定義します。 4 部門構造により、懸念事項の分離が強制されます。ファイル処理は言語に組み込まれています。ビジネス データ処理では、COBOL の 10 進数演算とレコード指向 I/O は依然として比類のないものです。 COBOL を理解するということは、エンタープライズ コンピューティングの基礎を理解することを意味します。