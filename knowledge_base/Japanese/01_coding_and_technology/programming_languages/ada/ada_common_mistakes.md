...
---
# Ada — よくある間違いとアンチパターン
このドキュメントでは、Ada で最も一般的な間違い、罠、アンチパターンを修正とともにカタログ化します。
---

## 1.`pragma Strict`を使用しない
```ada
...
＃＃ まとめ
Ada の強力な型指定は多くのバグを防ぎますが、規律が必要です。`pragma Strict`を使用し、サブタイプには`Constraint_Error`を処理し、読みやすさのために名前付きパラメーターの関連付けを使用し、常に例外を処理し、デッドロックを回避するためにタスク通信を慎重に設計します。 Ada は、慎重で安全性を意識したプログラミングを表彰します。