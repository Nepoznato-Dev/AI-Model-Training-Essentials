...
| `==`|`/=`|平等 | `a == b`| `/=`は「等しくありません」 |
| `<``>``<=``>=` |比較 | `a >= b`| |
| `.and.``.or.``.not.`|論理 | `a .and. b`| |
| `.eqv.`|`.neqv.`|論理的等価性 | `a .eqv. b`| |
| `//`|文字列の連結 | `'hello' // ' world'`| |
---
...
＃＃ まとめ
最新の Fortran の構文はクリーンで構造化されており、数値計算用に最適化されています。配列演算により、ほとんどの数学的タスクのループが排除されます。モジュールはカプセル化と名前空間管理を提供します。 OOP 機能 (派生型、継承、ポリモーフィズム) により、クリーンなコード編成が可能になります。 Coarray は、外部ライブラリを使用せずにネイティブな並列プログラミングを提供します。 Fortran の強みであるパフォーマンス、配列演算、数値精度は、科学技術計算においては依然として比類のないものです。 HPC、計算物理学、エンジニアリング シミュレーションにとって、Fortran は引き続き有力な存在です。