...
| `+``-``*``/``%``**` |算数 | `2 ** 10`| `**`はべき乗 |
| `.`|文字列の連結 | `$a . $b`| |
| `x`|文字列の繰り返し | `"ab" x 3`| `"ababab"`|
| `==``!=``<``>``<=``>=` |数値比較 | `$a == $b`| |
| `eq``ne``lt``gt``le``ge` |文字列の比較 | `$a eq $b`| |
| `<=>`|数字の宇宙船 | 写真 `$a <=> $b`|戻り値 -1、0、1 |
| `cmp`|文字列宇宙船 | 写真  XQZマーカー30XQZ | |
| `&&``\|\|``!`|論理 | `$a && $b`| |
| `and``or``not`|論理 (精度が低い) | `$a and $b`|避けてください —`&&`/`\|\|`を使用してください |
| `=~`|`!~`|正規表現一致 | `$str =~ /pattern/`| |
| `..`|`...`|レンジ / フリップフロップ | `1..10`| |
| `?:`| 3進法 | `$a ? $b : $c`| |
| `//`|定義済みまたは |  XQZマーカー50XQZ | `$a`が undef の場合は`$b`|
| `=>`|太いコンマ | `key => 'value'`|自動引用符
### シギル — 変数の型
```perl
...
＃＃ まとめ
Perl の構文は、コンテキスト、シジル、および「方法は 1 つだけではない」という原則を中心に構築されています。正規表現が言語に組み込まれているため、テキスト処理の簡潔さが比類のないものになります。参照により、複雑なデータ構造が可能になります。 CPAN はあらゆるものに対応するモジュールを提供します。最新の Perl (`strict`、`warnings`、および字句変数を含む) はクリーンで保守しやすいです。 Perl の遺産は、現代のあらゆる言語が採用しているパターンとツールの中に生き続けています。