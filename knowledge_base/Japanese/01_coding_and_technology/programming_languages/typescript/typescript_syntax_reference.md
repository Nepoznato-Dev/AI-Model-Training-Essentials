...
## デコレータ (TC39 ステージ 3 / TypeScript 5.0+)
```typescript
// Class decorator
...
＃＃ まとめ
TypeScript の型システムはその特徴的な機能であり、特に奥が深いです。基本的なアノテーションからジェネリックス、条件型、マップされた型、テンプレート リテラル型、および`infer`に至るまで、型システムは本質的にはコンパイル時に動作する関数型言語です。`interface`と`type`、ジェネリック型とユーティリティ型、構造型型と名目型型の間の相互作用を理解することで、安全で人間工学に基づいた TypeScript の作成がサポートされます。この言語は進化し続けており、JavaScript との完全な下位互換性を維持しながら、リリースごとに表現力がさらに追加されています。