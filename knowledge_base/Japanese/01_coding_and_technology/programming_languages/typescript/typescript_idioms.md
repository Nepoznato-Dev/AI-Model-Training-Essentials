...
＃＃ まとめ
TypeScript のイディオムは、厳密な型チェック、オブジェクト形状のインターフェイス、状態の判別共用体、絞り込みの型ガード、制約のあるジェネリックス、不変性の `readonly`、リテラル型の`as const`を強調します。 ESLint に従って`typescript-eslint`、書式設定のために Prettier を使用し、常に`strict: true`を有効にします。 TypeScript コミュニティはタイプ セーフティを重視しています。コンパイルできれば、おそらく機能します。オブジェクト シェイプには`interface`を、ユニオンとユーティリティには`type`を優先し、`any` は絶対に避けてください。