...
＃＃ まとめ
Prolog のロジック プログラミング パラダイムは、混乱を招く`=`(統一)、`==` (等価)、および`is`(算術) という独特のトラップを作成します。間違ったカット位置。訪問追跡なしの無限再帰。そしてチェックが発生します。 Prolog の方法は、統合を深く理解し、カットを控えめかつ正確に使用し、効率的なリスト構築のためにアキュムレータを使用し、常に終了を考慮することです。