...
| `-`|減算 | `3 - 2`| `1`|単項否定も |
| `*`|乗算 | `3 * 2`| `6`| |
| `/`|部門 | `7 / 2`| `3`| int 型の整数の切り捨て |
| `%`|弾性率 | `7 % 2`| `1`|整数型のみ |
| `++`|インクリメント | `i++`/`++i`| |インクリメント後およびインクリメント前 |
| `--`|デクリメント | `i--`/`--i`| |デクリメント後およびデクリメント前 |
### 比較演算子と論理演算子
|オペレーター |名前 |例 |メモ |
|----------|------|----------|----------|
| `==`|等しい | `x == y`| |
...
| `\|`|または | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `~`|補足 | `~0`|すべてのビットが反転されました |
| `<<`|左シフト | `1 << 3`| `8`|
| `>>`|右シフト | `8 >> 3`| `1`(署名付きの実装定義) |
### ポインタ演算子とアドレス演算子
|オペレーター |名前 |例 |メモ |
|----------|------|----------|----------|
| `&`| | の住所 `&x`| x へのポインタを返します |
| `*`|逆参照 | `*ptr`|ポインタの値にアクセス |
| `->`|ポインターによるメンバーアクセス | `p->x`|`(*p).x`| と同等
| `.`|メンバーアクセス | `s.field`|構造体メンバーへの直接アクセス |
| `[]`|配列の添字 | `arr[i]`|`*(arr + i)`| と同等
### 演算子の優先順位 (最高から最低)
|優先順位 |オペレーター |結合性 |
|-----------|-----------|---------------|
| 1 (最高) | `()``[]``->``.` |左から右へ |
| 2 | `!``~``+``-` (単項)`*``&``sizeof``(type)` |右から左へ |
| 3 | `*``/``%`|左から右へ |
| 4 | `+`|`-`|左から右へ |
| 5 | `<<`|`>>`|左から右へ |
| 6 | `<``<=``>``>=` |左から右へ |
...
＃＃ まとめ
C の構文は最小限でハードウェアに近く、ポインター、手動メモリ管理、機械語命令上の薄い抽象化レイヤーです。この言語の威力は、このシンプルさから生まれます。隠れたコストがほとんどなく、実行時のオーバーヘッドもなく、メモリ レイアウトを完全に制御できます。プリプロセッサは強力ですが、安全に使用するには規律が必要です。 Modern C (C11/C17/C23) には、`_Static_assert`、`_Generic`、`nullptr`、`typeof`などの重要な安全機能が追加されており、プログラマを信頼するというその中心的な哲学を維持しながら、言語の安全性が段階的に向上しています。