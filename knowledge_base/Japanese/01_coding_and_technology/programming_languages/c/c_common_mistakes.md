...
＃＃ まとめ
C の危険性はよく知られていますが、常に存在します。バッファ オーバーフロー、ダングリング ポインタ、整数オーバーフロー、フォーマット文字列攻撃、メモリ リークなどです。原則は、戻り値を常にチェックすること、割り当てたものを常に解放すること、常に有界関数 (`snprintf`、`strncpy`、`fgets`) を使用すること、`gets()`を決して使用しないこと、すべての変数を初期化すること、および Valgrind や AddressSanitizer などのツールを使用することです。 C は、注意深いプログラマに比類のないパフォーマンスと制御を与えますが、不注意には未定義の動作やセキュリティの脆弱性を与えます。