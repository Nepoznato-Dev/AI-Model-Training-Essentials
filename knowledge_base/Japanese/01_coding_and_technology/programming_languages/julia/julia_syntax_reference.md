...
|----------|------|----------|----------|
| `+``-``*``/``^`|算数 | `2^10`| |
| `÷`|整数の除算 | `7 ÷ 3`|`div(7, 3)`と同じ |
| `%``\` |モジュラス/剰余 | `7 % 3`| |
| `==`|`!=`|平等 | `a == b`|値の等しい |
| `===`|アイデンティティ | `a === b`|同じオブジェクト |
| `<``>``<=``>=` |比較 | `a >= b`|チェーン可能:`1 < x < 10`|
| `&&``\|\|``!`|論理 | `a && b`|短絡 |
| `&``\|``⊻``~` |ビットごと | `a & b`| `⊻`は XOR |
| `:`|範囲 | `1:10`| `start:step:stop`|
| `..`|間隔 | `1..10`| IntervalSet が必要です |
| `∈``∉``⊆`|メンバーシップを設定する | `x ∈ S`| `in(x, S)`|
| `∘`|構成 | `f ∘ g`| `(f ∘ g)(x) == f(g(x))`|
| `\|>`|パイプ | `x \|> f`| `f(x)`|
| `=>`|ペア |  XQZマーカー50XQZ |辞書の場合 |
| `...`|スプラット | `f(args...)`|コレクションを展開する |
### 連鎖比較
```julia
...
＃＃ まとめ
Julia の構文はクリーンで、数学的で、表現力豊かです。複数のディスパッチを使用すると、すべての引数の型に基づいて関数が異なる動作を行うことができるため、エレガントなコード編成が可能になります。マクロは、C スタイルのプリプロセッサ マクロのような複雑さを必要とせずに、強力なメタプログラミングを提供します。パラメトリック型、抽象型、および共用体型を備えた型システムにより、柔軟性とパフォーマンスの両方が可能になります。 Julia の 1 ベースのインデックス付け、内包表記、および do ブロック構文により、数学的および科学的コンピューティングが自然になります。この言語は、型推論を使用した JIT コンパイルを通じて、「Python のように歩き、C のように実行する」という約束を達成します。