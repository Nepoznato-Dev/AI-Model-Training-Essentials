...
＃＃ まとめ
Julia のイディオムは、型の安定性、複数のディスパッチ、`.`によるブロードキャスト、内包表記、具象型によるパフォーマンスを重視します。 Julia スタイル ガイドに従い、書式設定には JuliaFormatter、型推論分析には JET を使用します。 Julia の価値観は、「デフォルトで高速化する」ことです。つまり、クリーンなコードを記述し、JIT コンパイラーに最適化してもらいます。