...
＃＃ まとめ
Erlang と Elixir は、軽量プロセス、メッセージ パッシング、フォールト トレラント監視ツリーなどの BEAM VM のスーパーパワーを共有します。 Erlang の構文は最小限で機能的です。 Elixir は、パイプ、マクロ、プロトコルを使用した最新の構文を追加します。 OTP の動作 (GenServer、Supervisor、Application) は、信頼性の高いシステムを構築するための実証済みのパターンを提供します。リアルタイムの分散型フォールト トレラント アプリケーションでは、Erlang/Elixir は比類のないものです。