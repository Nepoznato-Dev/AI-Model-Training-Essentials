...
---
# Erlang と Elixir — よくある間違いとアンチパターン
このドキュメントでは、Erlang と Elixir で最も一般的な間違い、罠、アンチパターンを修正とともにカタログ化します。
---
...
## 3. タイムアウトなしの GenServer 呼び出し (Elixir)
```elixir
...
## 4. タイムアウトなしの`receive`でのパターン マッチ (Erlang)
```erlang
...
## 5. ネストされたパターンマッチングに`with`を使用しない (Elixir)
```elixir
...
## 6. アンチパターン: 長時間実行される GenServer コールバック
```elixir
...
＃＃ まとめ
Erlang/Elixir トラップ: 変数は単一割り当てであり、プロセスにはリンク/モニターが必要で、GenServer 呼び出しには明示的なタイムアウトが必要で、`receive` はタイムアウトなしで永久にブロックされ、GenServer コールバックはブロックすべきではありません。 BEAM の方法は、(監視付きで) クラッシュさせ、クリーンなパターン マッチングに`with`を使用し、GenServer コールバックを高速に保ち、常にプロセス障害を処理することです。