...
＃＃ まとめ
Elixir のイディオムは、パイプ演算子、パターン マッチング、エラー チェーン用の `with`、および`@spec`型仕様を強調します。 Erlang のイディオムでは、パターン マッチング、リスト内包表記、gen_server コールバック、および「クラッシュさせる」哲学が強調されています。どちらも不変性、メッセージパッシング、フォールトトレランスを重視しています。