...
|オペレーター |名前 |例 |メモ |
|----------|------|----------|----------|
| `=`|割り当て | `x=10`|`=`| の周りにスペースを入れないでください
| `$var`|変数の展開 | `echo $HOME`| |
| `${var}`|ブレース拡張 | `${HOME}/docs`|曖昧さ回避 |
| `$(cmd)`|コマンド置換 | `$(date +%Y)`|バッククォートよりも優先される |
| `$((expr))`|算数 | `$((2 + 3))`| |
| `${var:-default}`|デフォルト値 | `${PORT:-8080}`|未設定または空の場合 |
| `${var:=default}`|デフォルトを割り当てる | `${count:=0}`| |
| `${#var}`|文字列の長さ | `${#name}`| |
| `${var%%pattern}`|最長の接尾辞を削除 | `${file%%.*}`|拡張子を削除 |
| `${var##pattern}`|最長のプレフィックスを削除 | `${path##*/}`|ファイル名を取得 |
| `"`|`"`|二重引用符 | `"$var"`|拡張を許可します |
| `'`|`'`|一重引用符 | `'$var'`|リテラル文字列 |
### テストオペレーター
```bash
...
＃＃ まとめ
Bash と PowerShell は、シェル スクリプトの 2 つのパラダイムを表します。 Bash はテキストをパイプします。すべてのコマンドは文字列を変換します。 PowerShell はオブジェクトをパイプします。すべてのコマンドで、プロパティとメソッドを含む構造化データが生成されます。 Bash は Linux/macOS と DevOps を支配しています。 PowerShell は Windows 管理にとって不可欠であり、ますますクロスプラットフォーム化が進んでいます。どちらも最新の技術スタックにおいて不可欠なツールです。シェル スクリプトは、システムを接続し、ワークフローを自動化し、物事を完了させるための接着剤です。