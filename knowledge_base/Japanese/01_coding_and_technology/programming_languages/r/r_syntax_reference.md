...
-->
# R — 構文リファレンス
このドキュメントは、R (4.x) の包括的で構造化された構文リファレンスを提供します。これは、網羅的な構文パターン、Tidyverse エコシステム、データ操作、統計モデリング、および視覚化に焦点を当てることで、メインの R リファレンスを補完します。
---
...
| `%in%`|メンバーシップ | `x %in% y`| |
| `<-`|`=`|割り当て | `x <- 10`| `<-`は慣用句です |
| `->`|右割り当て | `10 -> x`|めったに使用されません |
| `<<-`|グローバル割り当て | `x <<- 10`|親環境で割り当てる |
| `$`|要素の抽出 | `df$column`| |
| `[`|`[[`|インデックス作成 | `x[1]`/`x[[1]]`| `[`は同じ型を返します。 `[[`抽出 |
| `%>%`|パイプ (マグリットル) | `x %>% f()`| |
| `\|>`|パイプ (ベース R 4.1+) | `x \|> f()`|ネイティブパイプ |
### ベクトル化された操作
```r
...
＃＃ まとめ
R の構文は、統計計算とデータ分析用に設計されています。操作のベクトル化の性質により、ほとんどのタスクでループが排除されます。 tinyverse エコシステム (dplyr、ggplot2、tidyr) は、データ操作と視覚化のための一貫した読みやすい文法を提供します。基本的な t 検定から混合効果モデルに至るまで、R の統計モデリング機能は、その深さにおいて他に類を見ません。データ サイエンス、統計、再現可能な研究にとって、R は依然として不可欠なツールです。