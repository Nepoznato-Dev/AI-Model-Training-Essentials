...
## 5.`stringsAsFactors = FALSE`を使用しない
```r
...
＃＃ まとめ
R のインタラクティブな性質は、ループ内でのデータ フレームの増大 (事前割り当てまたはベクトル化)、ベクトルのリサイクルの予期せぬ事態、`NA` の伝播、`T` /`F`の上書き可能性などの罠を生み出します。 R の方法は、操作をベクトル化し、ループ時に事前に割り当て、`NA`を明示的に処理し、データ操作に Tidyverse を使用し、`<<-`を回避することです。 R は、ベクトルで考え、パッケージの豊富なエコシステムを使用する人に報酬を与えます。