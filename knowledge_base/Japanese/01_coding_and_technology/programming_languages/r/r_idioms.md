...
＃＃ まとめ
R イディオムは、ベクトル化 (ループの回避)、tidyverse パイプ (`|>`)、代入用の `<-`、snake_case の命名、および`purrr`を使用した関数型プログラミングを強調します。 Tidyverse スタイル ガイドに従い、リンティングには lintr を、フォーマットには styler を使用します。 R 値はベクトル化された操作と読み取り可能なデータ変換用のパイプです。