...
---
# Rust — 構文リファレンス
このドキュメントは、Rust の包括的で構造化された構文リファレンスを提供します。これは、網羅的な構文パターン、演算子テーブル、所有権システム、特性、同時実行性の内部機構に焦点を当て、主要な Rust リファレンスを補完します。
---
...
|オペレーター |名前 |例 |メモ |
|----------|------|----------|----------|
| `+`|追加 | `3 + 2`|また、`format!` を介した`String`連結も可能です。
| `-`|減算 | `3 - 2`| |
| `*`|乗算 | `3 * 2`| |
| `/`|部門 | `7 / 2`|整数の整数除算:`3`|
| `%`|残り | `7 % 2`| `1`|
...
|オペレーター |名前 |例 |メモ |
|----------|------|----------|----------|
| `==`|等しい | `x == y`|`PartialEq`が必要 |
| `!=`|等しくない | `x != y`| |
| `<`、`>`、`<=`、`>=`|注文 | `x >= y`|`PartialOrd`が必要 |
| `&&`|論理積 | `a && b`|短絡 |
| `\|\|`|論理和 | `a \|\| b`|短絡 |
| `!`|論理否定 | `!true`| |
### ビット演算子
...
|-----------|-----------|----------|
| 1 (最高) | `()`パス`.`:: | |
| 2 |メソッド呼び出し、フィールドアクセス |自動デリファレンス |
| 3 | `-`(否定)、`!`、`*`(参照解除) | |
| 4 | `as`(型キャスト) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
## データ構造
### 組み込み型の概要
|タイプ |スタック/ヒープ |サイズ |可変性 |メモ |
|------|-----------|------|-----------|------|
|プリミティブ (`i32`、`f64`、`bool`、`char`)スタック |修正済み |デフォルトでは不変 | `Copy`|
| `[T; N]`(配列) |スタック |修正済み |`mut`で変更可能 |固定サイズ、T が`Copy`の場合は`Copy`|
| `Vec<T>`|ヒープ |ダイナミック |`mut`で変更可能 |成長可能 |
| `String`|ヒープ |ダイナミック |`mut`で変更可能 | UTF-8、拡張可能 |
| `&[T]`(スライス) |スタック (ファット PTR) |ダイナミックビュー |`&`または`&mut`に依存 |借景 |
| `HashMap<K, V>`|ヒープ |ダイナミック |`mut`で変更可能 | O(1) 平均ルックアップ |
| `BTreeMap<K, V>`|ヒープ |ダイナミック |`mut`で変更可能 | O(log n) ルックアップ、順序付け |
| `HashSet<T>`|ヒープ |ダイナミック | — |平均 O(1)、重複なし |
### ベクトル
```rust
// Creation
...
＃＃ まとめ
Rust の構文は、コンパイル時に適用される安全性保証の基盤に基づいて構築されています。所有権システム、特性ベースのポリモーフィズム、パターン マッチング、およびゼロコストの抽象化により、コンパイラーが厳密かつ公平なパートナーとなる一貫した設計が形成されます。基本的な制御フローからマクロ、安全でないコード、スマート ポインターなどの高度な機能に至るまで、すべての構成要素が安全で効率的なシステム プログラミングの目標を達成します。ライフタイム、借用、型システムの間の相互作用を理解することが、Rust をマスターする鍵となります。