...
-->
# Kotlin — 慣用的なパターンとベスト プラクティス
このガイドでは、クリーンで慣用的な Kotlin コードを記述するための慣用的なパターンとベスト プラクティスについて説明します。
---
...
＃＃ まとめ
Kotlin のイディオムは、null 安全性、データ クラス、シールされた型、スコープ関数、拡張関数、コルーチン、式指向のコードを強調します。 Kotlin コーディング規約に従い、フォーマットには ktlint を、静的分析には detekt を使用します。 Kotlin は簡潔さと表現力を重視します。定型文の記述を減らし、標準ライブラリを採用し、デフォルトで`val`を使用します。