...
---
# Delphi / Object Pascal — 慣用的なパターンとベスト プラクティス
このガイドでは、クリーンな Delphi コードと Object Pascal コードを記述するための慣用的なパターンについて説明します。
---
...
＃＃ まとめ
Delphi のイディオムでは、決定論的なリソースのクリーンアップには `try/finally`、文字列構築には `Format`、疎結合のインターフェイス、データ キャリアのレコード、特定の例外処理、ジェネリックや匿名メソッドなどの最新の機能が強調されます。 Delphi は、「作成し、使用し、最後に解放する」という明示的なリソース管理を重視します。