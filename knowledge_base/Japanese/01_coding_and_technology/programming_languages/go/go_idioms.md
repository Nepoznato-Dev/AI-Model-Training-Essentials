...
# Go — 慣用的なパターンとベスト プラクティス
このガイドでは、クリーンで慣用的な Go コードを記述するための慣用的なパターンとベスト プラクティスについて説明します。
---
...
＃＃ まとめ
Go のイディオムは、単純さ、明示的なエラー処理、小さなインターフェイス、継承よりも合成、コンテキストを伴うゴルーチン、および「インターフェイスを受け入れ、構造体を返す」ことを強調します。フォーマットについては `gofmt`、lint については `golangci-lint`、および公式の Go コード レビュー コメント ガイドに従ってください。 Go コミュニティは、読みやすさ、シンプルさ、そして「明白な方法」を重視しています。巧妙なコードを避ける — Go コードは退屈で予測可能であるべきです。