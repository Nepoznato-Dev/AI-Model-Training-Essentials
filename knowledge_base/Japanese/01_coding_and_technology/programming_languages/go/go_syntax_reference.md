...
---
# Go — 構文リファレンス
このドキュメントは、Go の包括的で構造化された構文リファレンスを提供します。網羅的な構文パターン、演算子テーブル、ゴルーチン、チャネル、インターフェイスの内部機構に焦点を当て、メインの Go リファレンスを補完します。
---
...
| `*`|乗算 | `3 * 2`| `6`| |
| `/`|部門 | `7 / 2`| `3`| int の整数除算 |
| `%`|残り | `7 % 2`| `1`|符号は配当と一致します。
| `&`|ビット単位の AND | `5 & 3`| `1`| |
| `\|`|ビットごとの OR | `5 \| 3`| `7`| |
| `^`|ビットごとの XOR | `5 ^ 3`| `6`|また、ビット単位の補数 (単項) |
| `&^`|ビットクリア (AND NOT) | `5 &^ 3`| `4`| Go ならでは |
| `<<`|左シフト | `5 << 1`| `10`| |
|  XQZマーカー30XQZ |右シフト | `5 >> 1`| `2`| |
### 比較演算子
|オペレーター |名前 |例 |メモ |
|----------|------|----------|----------|
| `==`|等しい | `x == y`|比較可能なタイプのみ |
...
＃＃ まとめ
Go の構文は意図的に最小限になっており、継承、メソッドのオーバーロード、例外、マクロはありません。残っているのは、クリーンで直交した構成要素のセットです。つまり、構成のための構造体とインターフェイス、同時実行のためのゴルーチンとチャネル、そして信頼性のための明示的なエラー処理です。この言語の力は、複雑な構文からではなく、単純なプリミティブの洗練された相互作用から生まれます。ジェネリック (Go 1.18+) は、Go の特徴的な明瞭さを犠牲にすることなく型の柔軟性を追加します。