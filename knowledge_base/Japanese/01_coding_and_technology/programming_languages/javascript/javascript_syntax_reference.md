...
-->
# JavaScript — 構文リファレンス
このドキュメントは、JavaScript の包括的で構造化された構文リファレンスを提供します。網羅的な構文パターン、演算子テーブル、内部機構に焦点を当て、メインの JavaScript リファレンスを補完します。
---
...
| `/`|部門 | `7 / 2`| `3.5`| |
| `%`|残り | `7 % 2`| `1`| |
| `**`|べき乗 | `2 ** 10`| `1024`| ES2016 |
| `++`|インクリメント | `x++`/`++x`| |インクリメント後およびインクリメント前 |
| `--`|デクリメント | `x--`/`--x`| |デクリメント後およびデクリメント前 |
|  `+x`、`-x` |単項 | `+"42"`→`42`|数値への型強制 |
### 比較と等価性
|オペレーター |名前 |例 |メモ |
|----------|------|----------|----------|
| `===`|厳密な平等 | `1 === 1`|型の強制はありません - 常にこれを優先します |
| `!==`|厳密な不平等 | `1 !== "1"`| `true`— さまざまなタイプ |
| `==`|抽象的な平等 | `1 == "1"`|型強制 — 回避 |
| `!=`|抽象的な不平等 | `1 != "1"`| `false`— 避ける |
| `<`、`>`、`<=`、`>=`|リレーショナル | `"b" > "a"`|文字列の辞書編集 |
### 論理演算子とヌル演算子
|オペレーター |名前 |例 |メモ |
|----------|------|----------|----------|
| `&&`|論理積 | `a && b`|最初の false または最後の値を返します |
| `\|\|`|論理和 | `a \|\| b`|最初の真実の値または最後の値を返します |
| `!`|論理否定 | `!true`| `false`|
| `??`|ヌルリッシュ合体 | `a ?? b`|`a`が`null`/`undefined`である場合にのみ、`b` を返します。
| `?.`|オプションのチェーン | `obj?.prop?.method()`| `undefined`への短絡 |
### ビット演算子
|オペレーター |名前 |例 |メモ |
|----------|------|----------|----------|
| `&`|そして | `5 & 3`| `1`|
...
| 2 | `new`(引数なし) |右 |
| 3 | `++``--` (後置) |該当なし |
| 4 | `!``~``+x``-x``typeof``void``delete``++``--`(プレフィックス) |右 |
| 5 | `**`|右 |
| 6 | `*``/``%`|左 |
| 7 | `+`|`-`|左 |
| 8 | `<<``>>``>>>`|左 |
...
## データ構造
### 組み込みコレクションの概要
|コレクション |注文済み |可変 |重複 |キーの種類 |検索 |
|-----------|-----------|----------|------------|----------|----------|
| `Array`|はい |はい |はい |インデックス (数値) | O(n) |
| `Object`|はい (挿入) |はい |キー: いいえ |文字列/記号 | O(1) 平均 |
| `Map`|はい (挿入) |はい |キー: いいえ |任意 | O(1) 平均 |
| `Set`|はい (挿入) |はい |いいえ | — | O(1) 平均 |
| `WeakMap`|いいえ |はい |キー: いいえ |オブジェクトのみ | O(1) 平均 |
| `WeakSet`|いいえ |はい |いいえ |オブジェクトのみ | O(1) 平均 |
### 配列
```javascript
// Creation
...
＃＃ まとめ
JavaScript の構文は、一見単純に見えますが、非常に奥深いものです。この言語は、単純なスクリプト ツールから、クラス、モジュール、ジェネレーター、プロキシ、および洗練された非同期モデルを備えたマルチパラダイム言語に進化しました。演算子の優先順位、イベント ループ、プロトタイプ チェーン、`this` バインディングの微妙な違いを理解することで、有能な開発者と専門家を区別できます。オプションのチェーン、ヌル結合、`Promise.allSettled`、およびトップレベルの await を備えた最新の JavaScript (ES2020+) は、下位互換性を維持しながらボイラープレートを削減し続けます。