...
|----------|------|----------|----------|
| `+``-``*``/``%`|算数 | `a + b`| |
| `..``...` |範囲 | `1...10`| `...`は閉じられました。 `..<`ハーフオープン |
| `==``!=``<``>``<=``>=` |比較 | `a == b`|`Equatable`/`Comparable`が必要 |
| `===`|`!==`|アイデンティティ | `a === b`|同じ参照 (クラスのみ) |
| `&&``\|\|``!`|論理 | `a && b`|短絡 |
| `??`|ニル合体 | `a ?? b`|`a`が nil の場合は`b`を返します。
| `?.`|オプションのチェーン | `a?.b`|`a`が nil の場合は nil を返します。
| `!`|強制的にアンラップ | `a!`| nil の場合はクラッシュします — 回避 |
| `&`|入出力パラメータ | `swap(&a, &b)`|参照渡し |
### Nil 処理演算子
```swift
...
＃＃ まとめ
Swift の構文は、安全性と表現力のバランスが取れています。オプションにより null クラッシュが排除され、プロトコルにより継承なしの柔軟な構成が可能になり、最新の同時実行 (async/await、アクター) により安全な同時プログラミングが可能になります。この言語は、パラメーター パック、マクロ、所有権などの機能を備えて進化し続けています。追加されるたびに、Swift の安全性と強力性が向上し、その中核となる約束であるコードの構築が安全であることが維持されます。