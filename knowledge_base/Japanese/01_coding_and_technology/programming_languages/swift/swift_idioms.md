...
---
# Swift — 慣用的なパターンとベスト プラクティス
このガイドでは、クリーンな Swift コードを記述するための慣用的なパターンとベスト プラクティスについて説明します。
---
...
＃＃ まとめ
Swift のイディオムでは、安全性 (オプション、ガード)、値の型 (構造体)、プロトコル指向の設計、パターン マッチング、非同期/待機、不変性 (デフォルトでは `let`) が強調されます。 Swift API 設計ガイドラインに従い、コード品質には SwiftLint を、フォーマットには SwiftFormat を使用します。 Swift は安全性と表現力を重視しています。「コンパイルできれば、それはおそらく正しいでしょう。」