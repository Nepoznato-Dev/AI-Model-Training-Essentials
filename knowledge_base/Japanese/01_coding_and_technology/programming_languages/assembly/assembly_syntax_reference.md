...
＃＃ まとめ
アセンブリ言語の構文は、プロセッサ アーキテクチャとアセンブラ (NASM、GAS、MASM) によって定義されます。 x86-64 は、16 個の汎用レジスタ、複数のアドレッシング モード、およびデータ移動、算術、ロジック、制御フロー、SIMD、システム コールにわたる数百の命令を提供します。ここで使用される NASM 構文は、Linux x86-64 開発で最も一般的なものです。アセンブリをマスターするには、レジスタ、スタック、呼び出し規約、および高レベルの構造が機械語命令にどのようにマップされるかを理解する必要があります。