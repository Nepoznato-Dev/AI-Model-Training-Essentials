...
＃＃ まとめ
アセンブリ トラップ: 呼び出し先が保存したレジスタを保持し、16 バイトのスタック アライメントを維持し、文字列に null ターミネータを含め、オペランドのサイズを明示し、常にループ終了条件を提供します。アセンブリ プログラミングでは、ハードウェアを最低レベルで理解する必要があります。すべての命令はレジスタ、メモリ、フラグに影響を与えます。