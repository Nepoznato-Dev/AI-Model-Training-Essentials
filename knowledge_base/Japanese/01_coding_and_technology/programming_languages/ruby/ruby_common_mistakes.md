...
-->
# Ruby — よくある間違いとアンチパターン
このドキュメントには、Ruby で最も一般的な間違い、罠、およびアンチパターンがカタログ化されています。各エントリは、間違ったアプローチを示し、それが失敗する理由を説明し、正しい解決策を提供します。
---
...
＃＃ まとめ
Ruby の表現力は、反復中のコレクションの変更、コア クラス、グローバル変数、ファット モデルのモンキー パッチなど、アンチパターンにつながる可能性があります。 Ruby の方法は次のとおりです。列挙子 (`each`、`map`、`reject`) を使用し、シンボルとキーワード引数を優先し、文字列リテラルを固定し、モデルからサービス オブジェクトを抽出し、モンキー パッチの代わりに改良を使用します。 Ruby は、ブロック、ダックタイピング、最小驚きの原則を採用する開発者に報酬を与えます。