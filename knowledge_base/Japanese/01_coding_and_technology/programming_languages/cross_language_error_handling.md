...
|モデル |言語 |メカニズム |
|------|-----------|-----------|
| **例外 (トライ/キャッチ)** | Java、C++、C#、Python、Ruby、PHP、JavaScript、TypeScript、Kotlin、Swift、Dart、Scala、VB、Delphi | `try { } catch (e) { }`|
| **結果/オプションのタイプ** | Rust、Haskell、OCaml、Scala (も) |  `Result<T, E>`、`Maybe a` |
| **複数の戻り値** |行きなさい、ルア | `value, err := f()`|
| **エラーコード** | C、フォートラン |エラー コードを返します。手動で確認してください。
| **パターン マッチング エラー** | Elixir、Erlang | `{:ok, value}`/`{:error, reason}`|
| **条件 (再起動可能)** |共通Lisp |  `restart-case`、`handler-bind` |
| **エラー処理なし** |スクラッチ、SQL (基本) |該当なし |
| **契約** |エイダ、エッフェル | `Pre`、`Post`条件 |
## 例外ベースのエラー処理
```python
# Python
try:
...
|パラダイム |アプローチ |長所 |短所 |
|----------|----------|------|------|
| **例外** |スロー/キャッチ |クリーンなハッピー パス、スタック トレース |忘れられる可能性がある、制御フローの乱用 |
| **結果の種類** |`Ok`/`Err`を返す |明示的、エラーを無視できません |詳細、`?` 演算子が必要 |
| **エラーコード** | int ステータスを返す |シンプル、C 互換 |無視しやすい、文脈がない |
| **複数の返品** | `value, err := f()`|明示的、慣用的 |`err`をチェックするのを忘れても構いません |
| **パターン マッチング** | `{:ok, v}`/`{:error, r}`|徹底した機能性 |`with`を使用しない冗長 |
| **条件** |再起動可能なハンドラー |最も強力で回復可能 |複雑でほとんど理解されない |