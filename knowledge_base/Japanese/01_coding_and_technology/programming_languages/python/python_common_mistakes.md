...
**この問題が発生する理由:** デフォルトの引数は、呼び出しごとではなく、関数定義時に 1 回評価されます。変更可能なデフォルト (list、dict、set) はすべての呼び出しで共有されます。
---
...
## 8. Python のスコープ ルール (LEGB) を理解していない
```python
...
## 12.`setdefault`または`collections.defaultdict`を使用しない
```python
...
## 15. アンチパターン: 神オブジェクト/オーバーロードされたクラス
```python
...
＃＃ まとめ
Python の単純さは欺瞞的です。変更可能なデフォルト、遅延バインディング クロージャ、スコープ ルール、およびリソース管理にはすべて、開発者を罠にはめる微妙な動作があります。覚えておくべき重要な原則: デフォルトは 1 回評価され、`is` は同一性ではなく同一性をチェックし、リソースには常にコンテキスト マネージャーを使用し、特定の例外をキャッチし、神オブジェクトよりも合成を優先します。 Python コードを書くということは、これらの落とし穴を理解し、言語の機能 (コンテキスト マネージャー、内包表記、ジェネレーター、`collections` モジュール) を意図したとおりに使用することを意味します。