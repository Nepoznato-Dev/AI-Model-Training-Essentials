...
# Python — 構文リファレンス
このドキュメントは、Python の包括的で構造化された構文リファレンスを提供します。網羅的な構文パターン、演算子テーブル、内部機構に焦点を当て、メインの Python リファレンスを補完します。
---
...
|オペレーター |名前 |例 |結果 |メモ |
|----------|------|----------|----------|----------|
| `+`|追加 | `3 + 2`| `5`|文字列/リストの連結も |
| `-`|減算 | `3 - 2`| `1`|単項否定も |
| `*`|乗算 | `3 * 2`| `6`|また、文字列/リストの繰り返し |
| `/`|真のディビジョン | `7 / 2`| `3.5`|常に float | を返します。
| `//`|フロア部門 | `7 // 2`| `3`|負の無限大に向かって丸めます |
| `%`|弾性率 | `7 % 2`| `1`|符号は除数と一致します |
| `**`|べき乗 | `2 ** 10`| `1024`|右連想 |
### 比較演算子とブール演算子
|オペレーター |名前 |例 |メモ |
|----------|------|----------|----------|
| `==`|等しい | `x == y`|値の比較 (同一性ではない) |
| `!=`|等しくない | `x != y`| |
| `<`、`>`、`<=`、`>=`|注文 | `x >= y`|チェーン可能:`a < b < c`|
| `is`|アイデンティティ | `x is y`|メモリ内の同じオブジェクト |
| `is not`|アイデンティティではない | `x is not None`|`!= None`よりも優先 |
| `in`|メンバーシップ | `x in collection`|セット/辞書の場合は O(1)、リストの場合は O(n) |
| `and`|論理積 | `a and b`|短絡;最後の真/偽値を返します |
| `or`|論理和 | `a or b`|短絡;最初の真/偽値を返します |
| `not`|論理否定 | `not x`| |
### ビット演算子
|オペレーター |名前 |例 |メモ |
|----------|------|----------|----------|
...
## データ構造
### 組み込みコレクションの概要
|コレクション |注文済み |可変 |重複 |構文 |検索 |
|-----------|-----------|-----------|------------|----------|----------|
| `list`|はい |はい |はい | `[1, 2, 3]`| O(n) |
| `tuple`|はい |いいえ |はい | `(1, 2, 3)`| O(n) |
| `dict`|はい (3.7 以降) |はい |キー: いいえ | `{"k": "v"}`| O(1) 平均 |
| `set`|いいえ |はい |いいえ | `{1, 2, 3}`| O(1) 平均 |
| `frozenset`|いいえ |いいえ |いいえ | `frozenset({1,2})`| O(1) 平均 |
### リスト
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
＃＃ まとめ
この構文リファレンスでは、基本的な演算子や制御フローから高度なメタプログラミング、記述子、型システムに至るまで、Python の完全な機能セットをカバーしています。 Python の構文は意図的に最小限に抑えられ、読みやすくなっていますが、その奥深さはかなりのものです。パターン マッチング、非同期ジェネレーター、プロトコル、メタクラスは、複雑なアプリケーションに強力なツールを提供します。この言語は進化し続けており、リリースごとに下位互換性を維持しながら表現力を向上させる機能が追加されています。