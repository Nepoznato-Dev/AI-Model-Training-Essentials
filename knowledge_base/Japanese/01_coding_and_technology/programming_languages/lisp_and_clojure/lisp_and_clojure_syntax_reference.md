...
＃＃ まとめ
Clojure の構文は一律に括弧で囲まれており、すべての形式はリストです。この同形性により、コードをデータとして変換する強力なマクロが可能になります。永続的なデータ構造は、効率性を高めるための構造共有による不変性を提供します。スレッド マクロは、読み取り可能なデータ パイプラインを作成します。同時実行プリミティブ (アトム、参照、エージェント、チャネル) は状態の変更を安全に処理します。 Lisp/Clojure が現代のプログラミングに与えた影響は非常に大きく、ガベージ コレクション、REPL、マクロ、関数型プログラミングはすべて Lisp に遡ります。