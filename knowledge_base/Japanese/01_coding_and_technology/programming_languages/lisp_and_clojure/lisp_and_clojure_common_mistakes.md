...
＃＃ まとめ
Lisp/Clojure トラップ: 引用符は評価を防ぎます (`,`で準引用符を使用します)、`nil`と`false`は両方とも偽ですが区別されます、コレクションは不変です (再バインドまたはアトムを使用します)、末尾再帰には`recur`を使用し、変数キャプチャを避けるためにマクロで`gensym`を使用します。 Lisp のやり方は、不変性を受け入れ、リーダー マクロ システムを理解し、衛生的なマクロを書くことです。