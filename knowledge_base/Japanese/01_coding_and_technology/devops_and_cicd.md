...
主な実践方法: スリム/アルパインベースイメージを使用し、非ルートとして実行し、レイヤーキャッシュを活用し、`.dockerignore`を使用し、イメージの脆弱性をスキャンし (`trivy`、`docker scan`)、リソース制限を設定します。
### Docker Compose
...
目標はインシデントを解決することだけではなく、同じインシデントが二度と起こらないようにすることです。