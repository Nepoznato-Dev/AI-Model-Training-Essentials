...
A practical ガイド to runn large 言語 models entirely on-device — hardware considerations, ference eng, memory optimisation, system design に edge デプロイ.

---

# # Why Run 人工知能 Locally?

- **Privacy**: No データ leaves device.
- **Cost**: No API fees per token.
- **Latency**: Predictable, ネットワーク-free ference.
- **Offle availability**: Works out ternet.
...
- For GPU ference, system RAM matters に load model へ memory 前に fload to VRAM.

# ## Storage
- Quantised model weights take up a few GB (e.g., 4-bit 7B ≈ 4 GB on disk). Ensure at least 20–50 GB free に multiple models.

# ## CPU
- For prompt process (prefill) CPU-fload, a modern multi-core CPU helps.
- Apple M-series chips have excellent perにmance に 大規模言語モデル due to unified memory Neural Enge.
...
| Format | Bits | 説明 | Typical use |
|--------|------|-------------|-------------|
| **GGUF** | 4–8 | llama.cpp にmat, optimised に CPU/GPU hybrid | Best に local ference |
| **GPTQ** | 4–8 | GPU-only, efficient on CUDA | Best に NVIDIA GPUs |
| **AWQ** | 4 | Activation-aware, GPU-only | Good に batch ference on GPUs |
| **ONNX** | variable | Stardised, cross-platにm | Production serv |
...
- **Q4_K_M** (4-bit): smallest, acceptable quality に most tasks.
- **IQ4_XS** / **IQ3_XS**: Improved quantisation better perplexity at 4/3 bits.

**Rule thumb:** Use Q4_K_M に a good balance quality size. If you have extra VRAM, use Q5 or Q6.
...
- Written C++.
- Supports GGUF にmat.
- Optimised に CPU GPU (via CUDA, Metal, OpenCL).
...
Built-in local server with Open人工知能-compatible API.
...
Hybrid Edge-Cloud
Local model handles common queries; fallback to a クラウド model for complex questions. This gives the best of both worlds — speed/private for most, capability for edge cases.
...
Mobile Deployment
Android: Use llama.cpp via JNI bindings or 機械学習 Kit.
...
Quality gap: Small local models (3.8B–7B) generally underperform large クラウド models (GPT-4, Claude 3.5) on complex reasoning.
...
A practical ガイド to secur applications, frastructure, データ — from 開発 to production.
...
7. **Identification Auntication Failures**: Weak passwords, session mis管理.
8. **Stware データ Integrity Failures**: Supply cha attacks, unsigned updates.
9. **セキュリティ Logg Monitor Failures**: No detection breaches.
10. **Server-Side Request Forgery (SSRF)**: Abuse server to make requests to ternal システム.
...
# ## Validation Rules
- **Whitelist > Blacklist**: Defe allowed patterns (e.g., regex に email) rar than block known bad patterns.
- **Length limits**: Enにce maximum lengths to prevent buffer overflows DoS.
...
# ## Output Encod
- **HT機械学習 encod**: Encode `<`, `>`, `&`, `"`, `'` to prevent XSS.
- **SQL parameterisation**: Never concatenate user put へ SQL queries. Use parameterised queries (prepared statements) or an ORM.
...
# ## Password 管理
- **Hash**: Store passwords a strong, slow hash algorithm: **Argon2id** (preferred), **bcrypt**, **scrypt**, or **PBKDF2**.
- **Salt**: Add a unique per-user salt.
- **Mimum length**: Enにce at least 12–16 characters.
- **MFA (Multi-Factor Auntication)**: Require a second factor (TOTP, SMS, hardware key) に sensitive operations.
- **Rate limit**: Prevent brute-にce attempts on log endpots (e.g., 5 attempts per 5 mutes per IP/user).

# ## Session 管理
- Use secure, HTTP-only, SameSite cookies に session tokens.
- Set appropriate expiration times.
- Invalidate sessions on logout on password change.
- Avoid expos session IDs URLs.

# ## OAuth2 / OIDC
- Use well-established libraries (e.g., Authlib, PyJWT, Passport.js, Spr セキュリティ).
- Validate ID tokens thoroughly (sig自然, issuer, 読者, expiration).
- Use state parameters to prevent CSRF.
- Keep client secrets confidential.

# ## JWT (JSON ウェブ Tokens)
- **Sign**: Use RS256 or ES256 (asymmetric) に better セキュリティ; HS256 (symmetric) is acceptable if shared secrets are managed well.
- **Validate**: Always verify sig自然, issuer (`iss`), 読者 (`aud`), expiration (`exp`).
- **Keep short expiration**: 15–60 mutes に access tokens; use refresh tokens に longer sessions.
...
- Return `429 Too Many Requests` a `Retry-After` header.
...
# ## HTTPS / TLS
- Enにce HTTPS production.
- Use HSTS (HTTP Strict Transport セキュリティ) to にce browsers to use HTTPS.
...
- Use 環境 variables or secret 管理 tools.

# ## Tools
- **HashiCorp Vault**: Enterprise-grade, dynamic secrets.
- **AWS Secrets Manager / Azure Key Vault / GCP Secret Manager**: Cloud-native.
- **SOPS**: Encrypt secrets files commit m ( KMS or GPG).
- **Docker secrets**: For Swarm mode; Kubernetes secrets (base64-encoded, but use care; consider external Secrets Store CSI driver).
...
- Set up automated pull requests に mor/patch updates.
- Review changelogs に break changes.
...
- Prefer ficial registries trust only 検証済み publishers.
...
- Use セキュリティ groups (AWS) or NSGs (Azure) に fe-graed control.

# ## OS Harden
- Apply セキュリティ updates regularly (`sudo apt upgrade`, `yum update`).
- Disable unnecessary services default accounts.
- Use fail2ban to block brute-にce attempts on SSH.
- Harden SSH: disable root log, use key-based auth, change default port (optional).

# ## ネットワーク Segmentation
- Place データbases caches private subnets no ternet access.
- Use a DMZ に public-fac services.
- Apply prciple least privilege to ネットワーク access.

# ## Secrets Infrastructure
- Never store secrets CI/CD 環境 variables unless encrypted.
- Use クラウド provider's IAM roles に EC2/VM stances stead long-lived keys.
...
# ## What to Log
- Auntication イベント (success/failure).
...
- API requests responses (redact sensitive データ).

# ## What Not to Log
- Passwords, secrets, tokens, PII (Personal Identifiable 情報) unless hashed/redacted.
- Full credit card numbers.

# ## Alert
- Set up alerts に:
 - Multiple failed logs (potential brute にce).
 - Unusual access patterns (e.g., from new locations, at odd hours).
 - New adm accounts created.
 - High error rates or latency spikes.
- Use a SIEM (セキュリティ 情報 Event 管理) に 上級 correlation.

# ## Log Retention
- Reta logs に at least 30–90 days depend on regulatory requirements.
- Store logs a centralised, tamper-evident system (e.g., ELK Stack, Splunk, データdog).
...
3. **Secure cod stards**: Enにce via lters code review checklists.
4. **SAST** (Static Application セキュリティ Test): Scan source code に vulnerabilities (SonarQube, CodeQL).
5. **DAST** (Dynamic Application セキュリティ Test): Scan runn applications (OWASP ZAP, Burp Suite).
...
9. **Incident response plan**: Have a clear plan に when a breach is detected.
...
2. **Isolate** affected システム (disconnect from ネットワーク if needed).
3. **Preserve evidence**: Capture logs, memory dumps, disk images.
4. **Identify** scope: which システム, which データ.
5. **Rotate** all compromised credentials secrets.
6. **Patch** vulnerability.
7. **Notify** affected users regulatory bodies if required ( 法的 timeframes).
