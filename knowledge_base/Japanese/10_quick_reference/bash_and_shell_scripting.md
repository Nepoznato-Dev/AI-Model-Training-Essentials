...
| `cmd >> file`|標準出力をファイルにリダイレクト (追加) |
| `cmd 2> errors.log`|標準エラー出力をリダイレクト |
| `cmd &> all.log`| stdout と stderr の両方をリダイレクトする |
...
＃＃ まとめ
Bash スクリプトは、コンピュータを扱うすべての人にとって力を倍増させるものです。すべてのフラグを暗記する必要はありません。何が可能で、どこで調べればよいのかを知っておく必要があります。変数、条件、ループ、パイプなどの基本から始めます。次に、必要に応じてテキスト処理ツール (grep、sed、awk) を重ねます。そして常に`set -euo pipefail`を使用してください。今後もよろしくお願いいたします。