...
- 必要な列のみを選択します (`SELECT *`は避けてください)
-`EXPLAIN`を使用してクエリのパフォーマンスを分析する
- データを適切に正規化する
- SQL インジェクションを防ぐためにプリペアド ステートメントを使用する
❌ **してはいけないこと:**
- WHERE 句のインデックス付き列で関数を使用する
- 作成するインデックスが多すぎる (書き込みが遅くなる)
- 不必要に`SELECT DISTINCT`を使用する
- クエリ実行プランを無視する
- 計算可能な場合は計算値を保存します
---
...
*最終更新日: 2026 年 7 月 | SQL 標準 (MySQL/PostgreSQL 互換)*