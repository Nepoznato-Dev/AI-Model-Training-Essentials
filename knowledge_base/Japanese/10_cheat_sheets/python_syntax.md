...
Quick リファレンス に Python 3.x 構文 common patterns.
...
- Use `#` に sle-le comments
- Use `"""triple quotes"""` に docstrs multi-le strs
- Indentation matters (typically 4 spaces)
- Nam conventions: `snake_case` に variables/functions, `PascalCase` に classes
- `__name__ == "__ma__"` to check if script is run directly
- Use `virtualenv` or `venv` に project isolation
- Install packages `pip stall package_name`
