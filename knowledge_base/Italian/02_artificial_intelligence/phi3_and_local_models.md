...
2. **Proximal Policy Optimization (PPO)** rispetto a un modello di ricompensa addestrato sui dati delle preferenze umane
...
Phi-3-mini si comporta molto bene in relazione al conteggio dei parametri:
...
L’esperienza Phi illustra diverse lezioni importanti:### 1. La distribuzione dei dati di formazione è la cosa più importante
...
**Local-first è fattibile**: il successo di Phi-3-mini dimostra che un'intelligenza artificiale completamente locale può eguagliare modelli basati su cloud per molte attività. Ciò convalida l'architettura di Potato.ai che prevede l'esecuzione interamente sul dispositivo senza chiamate API esterne.
