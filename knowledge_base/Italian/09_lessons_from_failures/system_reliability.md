...
Questo documento consolida i problemi di affidabilità tra cui perdite di memoria, condizioni di competizione, errori di programmazione simultanea ed errori di progettazione del sistema.
---

## Perdite di memoria
Le perdite di memoria si verificano quando i programmi allocano memoria ma non riescono a rilasciarla quando non sono più necessarie, provocando un aumento graduale del consumo di memoria.
### Ascoltatori di eventi dimenticati
**Esempio errato (JavaScript):**```javascript
...
**Soluzione:** utilizzare riferimenti deboli per i riferimenti all'indietro```python
import weakref
...
## Condizioni di gara
Le race conditions si verificano quando il comportamento del software dipende dalla tempistica relativa degli eventi, come l'ordine di esecuzione dei thread.
### Controlla la condizione di gara e poi agisci
**Cattivo esempio (Python):**```python
...
### Condizione di competizione di lettura-modifica-scrittura
...
### Race Condition di inizializzazione pigra
**Cattivo esempio:**```python
...
**Prevenzione:** acquisisci sempre i lucchetti in un ordine coerente```python
# Both threads:
lock_a.acquire()
lock_b.acquire()
# ... critical section ...
...
**Soluzione:** utilizzare una pianificazione equa o un invecchiamento prioritario
---
...
1. **Utilizza RAII**: modello di acquisizione risorse è inizializzazione
2. **Gestori di contesto**: utilizza le istruzioni`with`per le risorse
3. **Riferimenti deboli**: per cache e riferimenti a ritroso
4. **Profilazione**: profilazione regolare della memoria in produzione
### Concorrenza
1. **Riduci al minimo lo stato condiviso**: preferisci il passaggio dei messaggi
2. **Utilizza astrazioni di alto livello**: pool di thread, asincrono/attendono
3. **Blocca l'ordine**: previene i deadlock con un ordine coerente
4. **Test simultaneo**: utilizza stress test e rilevatori di gare
### Prestazioni
1. **Prima il profilo**: misura prima di ottimizzare
2. **Scelta dell'algoritmo**: selezionare le strutture dati appropriate
3. **Operazioni batch**: riduzione dei viaggi di andata e ritorno verso database/API
4. **Caching**: memorizza nella cache i calcoli costosi in modo appropriato
---

## Argomenti correlati
- **Vulnerabilità della sicurezza**: vedere`security_vulnerabilities.md`per problemi relativi alla sicurezza
- **Qualità del codice**: vedere`code_quality_issues.md`per problemi di manutenibilità
- **Progettazione API**: vedere`../07_api_system_design/api_system_design.md`per i modelli di architettura del sistema
- **Errori AI/LLM**: vedere`ai_llm_failures.md`per problemi di affidabilità specifici dell'IA
---

## Ulteriori modelli di affidabilità del sistema
### Esaurimento delle risorse
**Che cos'è:** Esaurimento delle risorse di sistema (handle di file, connessioni, memoria) attraverso un'allocazione illimitata.
**Cattivo esempio:**```python
...
### Guasti a cascata
**Che cos'è:** Il guasto in un componente provoca guasti nei componenti dipendenti.
**Cattivo esempio:**```python
...
### Problemi di disallineamento dell'orologio
**Cattivo esempio:**```python
...
### Scenari di fallimento dei test
**Cosa testare:**
1. **Errori delle istanze**: uccidi pod/VM casuali
2. **Problemi di rete**: aggiungi latenza, elimina pacchetti
3. **Esaurimento risorse**: riempire il disco, esaurire la memoria
4. **Errori delle dipendenze**: simulazioni di interruzioni del servizio
5. **Disallineamento dell'orologio**: desincronizza gli orologi del server
**Esempio di esperimento sul caos:**```yaml
...
### Giorni di gioco
Esercizi regolari per testare la resilienza del sistema:
1. **Programma**: giornate di gioco trimestrali
2. **Ambito**: ambiente di tipo produttivo
3. **Scenari**: più guasti simultanei
4. **Metriche**: misura il tempo di rilevamento e il tempo di recupero
5. **Apprendimenti**: documentare e migliorare i runbook
---
...
### Fornitura eccessiva di rifiuti
**Cattivo esempio:**```markdown
...
### Prevenzione del burnout durante le chiamate
**Cattive pratiche:**
- Stessa persona di guardia ogni settimana
- Nessun percorso di escalation
- Avvisi per elementi non utilizzabili
- Autopsie che assegnano la colpa
**Buone pratiche:**```markdown
On-Call Rotation:
...
**Buon manuale operativo:**```markdown
## Database Slow Query
...
SELEZIONA * DA pg_stat_activity 
   DOVE stato = 'attivo' 
   ORDINA PER query_start;   ```

2. Check for locks:
   ```sql
SELECT * FROM pg_locks WHERE concesso = false;   ```
