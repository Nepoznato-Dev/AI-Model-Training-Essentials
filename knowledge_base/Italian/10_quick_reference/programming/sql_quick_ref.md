...
# Guida di riferimento rapido SQL
Comandi SQL essenziali per le operazioni del database.
---
...
## Recupero dati (SELEZIONE)
### Selezione di base```sql
-- Select all columns
SELECT * FROM users;
...
-`DECIMAL(p,s)`- Decimale esatto (precisione, scala)
-`FLOAT`- Virgola mobile approssimativa
-`DOUBLE`- Galleggiante doppia precisione
### Stringa
-`CHAR(n)`- Stringa di lunghezza fissa
-`VARCHAR(n)`- Stringa di lunghezza variabile
-`TEXT`- Testo grande
-`ENUM`- Valori enumerati
### Data/Ora
...
-`YEAR`- Valore anno
### Booleano
-`BOOLEAN`o`BOOL`- Vero/Falso
### Binario
-`BLOB`- Oggetto binario di grandi dimensioni
-`BINARY`- Binario fisso
-`VARBINARY`- Binario variabile
---
...
✅ **Fai:**
- Utilizzare gli indici sulle colonne interrogate frequentemente
- Seleziona solo le colonne necessarie (evita`SELECT *`)
- Utilizza`EXPLAIN`per analizzare le prestazioni delle query
- Normalizzare i dati in modo appropriato
- Utilizzare istruzioni preparate per impedire l'iniezione SQL
❌ **Non farlo:**
- Utilizzare le funzioni sulle colonne indicizzate nelle clausole WHERE
- Crea troppi indici (rallenta le scritture)
- Utilizzare`SELECT DISTINCT`inutilmente
- Ignorare i piani di esecuzione delle query
- Memorizza i valori calcolati quando possono essere calcolati
---
...
*Ultimo aggiornamento: luglio 2026 | SQL standard (compatibile con MySQL/PostgreSQL)*