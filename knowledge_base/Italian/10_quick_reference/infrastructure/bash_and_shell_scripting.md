...
| `set -x`| Stampa ogni comando prima dell'esecuzione (modalità debug) |
...
## Riepilogo
Lo scripting Bash aumenta la produttività di chiunque lavori con i computer. Non è necessario memorizzare ogni flag: è sufficiente sapere cosa è possibile fare e dove trovare la documentazione. Inizia dalle basi: variabili, condizionali, loop, pipe. Quindi aggiungi strumenti di elaborazione del testo (grep, sed, awk) secondo necessità. Si consiglia di utilizzare`set -euo pipefail`per scrivere script robusti.