...
3. Spingere il ramo:`git push origin feature/new-thing`.
...
-`settings.json`(utente o area di lavoro) controlla il comportamento dell'editor.
...
- Premi`Ctrl+R`per cercare la cronologia dei comandi in modo interattivo.
-`alias ll='ls -la'`crea un collegamento: aggiungilo a`~/.bashrc`o`~/.zshrc`.
- Utilizza`tmux`o`screen`per mantenere attive le sessioni quando sei disconnesso da un server remoto.
-`man <command>`mostra la pagina man per qualsiasi comando integrato.
---

##Docker
Docker assembla le applicazioni e le relative dipendenze in contenitori portatili.
### Concetti fondamentali
- **Immagine**: un modello di sola lettura creato da un file`Dockerfile`.
