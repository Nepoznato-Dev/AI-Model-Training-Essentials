...
| | `-`| Sottrazione | `A - B`|
| | `*`| Moltiplicazione | `A * B`|
| | `/`| Divisione reale | `A / B`(restituisce sempre Reale) |
| | `div`| Divisione intera | `A div B`(restituisce intero) |
| | `mod`| Modulo | `A mod B`|
...
| **Logico** | `and`| AND logico | `A and B`|
| | `or`| OR logico | `A or B`|
| | `not`| NON logico | `not A`|
| | `xor`| OR esclusivo | `A xor B`|
| **Stringa** | `+`| Concatenazione | `'Hello' + ' ' + 'World'`|
| **Imposta** | `in`| Appartenenza | `X in [1, 2, 3]`|
| | `+`| Unione | `SetA + SetB`|
| | `-`| Differenza | `SetA - SetB`|
| | `*`| Intersezione | `SetA * SetB`|
| **Compito** | `:=`| Assegna valore | `X := 42`|
| **Indirizzo** | `@`| Indirizzo di | `P := @X`|
| | `^`| Dereferenziazione | `P^`|
---
...
## Riepilogo
Delphi/Object Pascal combina una sintassi pulita e leggibile con potenti funzionalità OOP. Classi, interfacce, generici e record forniscono un kit di strumenti completo per lo sviluppo di applicazioni. I blocchi espliciti di inizio/fine del linguaggio, le variabili tipizzate e gli identificatori senza distinzione tra maiuscole e minuscole lo rendono accessibile. I framework VCL/FMX estendono il linguaggio con librerie di componenti visivi per un rapido sviluppo delle applicazioni. Per le applicazioni Windows aziendali e i front-end di database, Delphi rimane una scelta produttiva.