...
# Linguaggio Assembly: riferimento alla sintassi
Questo documento fornisce un riferimento completo e strutturato alla sintassi per x86-64 Assembly (sintassi NASM). Completa il riferimento principale dell'Assembly concentrandosi su categorie esaustive di istruzioni, modalità di indirizzamento, convenzioni di stack frame e modelli di programmazione del sistema.
---

## Registri e dimensioni dei dati
```nasm
...
## Macro e assemblaggio condizionale
```nasm
; === NASM Macros ===
...
## Riepilogo
La sintassi del linguaggio assembly è definita dall'architettura del processore e dall'assemblatore (NASM, GAS, MASM). x86-64 fornisce 16 registri generici, modalità di indirizzamento multiple e centinaia di istruzioni che spaziano dal movimento dei dati, all'aritmetica, alla logica, al flusso di controllo, al SIMD e alle chiamate di sistema. La sintassi NASM utilizzata qui è la più comune per lo sviluppo Linux x86-64. Per padroneggiare l'assemblaggio è necessario comprendere i registri, lo stack, le convenzioni di chiamata e il modo in cui i costrutti di alto livello si associano alle istruzioni della macchina.