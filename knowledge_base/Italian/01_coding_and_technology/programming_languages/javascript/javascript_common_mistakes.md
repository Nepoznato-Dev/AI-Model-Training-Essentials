...
-->
# JavaScript: errori comuni e anti-pattern
Questo documento cataloga gli errori, le trappole e gli anti-pattern più comuni in JavaScript. Ogni voce mostra l'approccio errato, spiega perché fallisce e fornisce la soluzione corretta.
---

## 1.`this`Confusione di rilegatura
```javascript
...
## 3.`var`Problemi di sollevamento e ambito
```javascript
...
## 5. Non gestire le promesse rifiutate
```javascript
...
## 6. Modifica degli oggetti durante l'iterazione
```javascript
...
## 9. Gli oggetti simili ad array non sono array
```javascript
...
## 10. Anti-Pattern: parametri funzionali mutanti
```javascript
...
## 11. Non comprendere il ciclo di eventi e i microtask
```javascript
...
## 12. Anti-pattern: utilizzo di `eval()`
```javascript
...
## 13. Dimenticare di pulire gli ascoltatori di eventi
```javascript
// ❌ WRONG — memory leak
class Component {
  mount() {
...
## 14. Copia superficiale e copia approfondita
```javascript
...
## Riepilogo
La flessibilità di JavaScript presenta delle trappole: l'associazione`this`dipende dal sito di chiamata,`==`esegue la coercizione, le promesse richiedono la gestione degli errori e l'ordine del ciclo di eventi (microattività prima delle macroattività) non è ovvio. Le regole moderne sono semplici: usa`const`/`let`mai`var`, preferisci`===`su`==`, gestisci sempre i rifiuti delle promesse, usa le funzioni freccia per i callback, evita la mutazione e ripulisci i listener di eventi. Seguendo queste regole si elimina la maggior parte dei bug JavaScript.