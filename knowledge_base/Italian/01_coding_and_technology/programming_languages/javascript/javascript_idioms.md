...
---
# JavaScript: modelli idiomatici e migliori pratiche
Questa guida illustra i modelli idiomatici e le migliori pratiche per scrivere codice JavaScript (ES2024+) pulito e moderno.
---

## Dichiarazioni moderne
```javascript
// ❌ Avoid var
var name = "Alice";
...
## Riepilogo
I moderni idiomi JavaScript enfatizzano:`const`/`let`su`var`, destrutturazione, funzioni freccia, valori letterali template, async/await, operatori spread/rest, metodi array (`map`/`filter`/`reduce`), moduli ES, concatenamento opzionale (`?.`), nullish coalescente (`??`) e campi di classe con membri privati. Segui ESLint per la qualità del codice, Prettier per la formattazione e preferisci i modelli funzionali rispetto allo stato modificabile. La comunità JavaScript valorizza la concisione, la componibilità e le moderne funzionalità ES2024+.