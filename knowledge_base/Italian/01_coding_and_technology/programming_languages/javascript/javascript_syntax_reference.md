...
# JavaScript: riferimento alla sintassi
Questo documento fornisce un riferimento completo e strutturato alla sintassi per JavaScript. Completa il riferimento JavaScript principale concentrandosi su modelli di sintassi esaustivi, tabelle degli operatori e meccanica interna.
---
...
|----------|------|---------|--------|-------|
| `+`| Aggiunta | `3 + 2`| `5`| Anche la concatenazione di stringhe |
| `-`| Sottrazione | `3 - 2`| `1`| |
| `*`| Moltiplicazione | `3 * 2`| `6`| |
| `/`| Divisione | `7 / 2`| `3.5`| |
| `%`| Resto | `7 % 2`| `1`| |
| `**`| Esponenziazione | `2 ** 10`| `1024`| ES2016 |
| `++`| Incremento | `x++`/`++x`| | Post e pre-incremento |
| `--`| Decremento | `x--`/`--x`| | Post e pre decremento |
| `+x`,`-x`| Unario | `+"42"`→`42`| Digitare coercizione al numero |
### Confronto e uguaglianza
| Operatore | Nome | Esempio | Note |
|----------|------|---------|-------|
| `===`| Uguaglianza rigorosa | `1 === 1`| Nessuna coercizione di tipo: preferisci sempre questo |
| `!==`| Disuguaglianza rigorosa | `1 !== "1"`| `true`— tipi diversi |
| `==`| Uguaglianza astratta | `1 == "1"`| Digitare coercizione — evitare |
| `!=`| Disuguaglianza astratta | `1 != "1"`| `false`— evita |
|  `<`, `>`, `<=`,`>=`| Relazionale | `"b" > "a"`| Lessicografico per archi |
### Operatori logici e nulli
| Operatore | Nome | Esempio | Note |
|----------|------|---------|-------|
| `&&`| AND logico | `a && b`| Restituisce il primo falso o l'ultimo valore |
| `\|\|`| OR logico | `a \|\| b`| Restituisce la prima verità o l'ultimo valore |
| `!`| NON logico | `!true`| `false`|
| `??`| Coalescenza nulla | `a ?? b`| Restituisce`b`solo se`a`è`null`/`undefined`|
| `?.`| Concatenamento opzionale | `obj?.prop?.method()`| Cortocircuiti verso`undefined`|
### Operatori bit a bit
| Operatore | Nome | Esempio | Note |
|----------|------|---------|-------|
| `&`| E | `5 & 3`| `1`|
...
| `>>`| Firmato Maiusc a destra | `-5 >> 1`| `-3`|
| `>>>`| Maiusc destro senza segno | `-1 >>> 0`| `4294967295`|
### Assegnazione e destrutturazione
```javascript
// Basic assignment
let x = 10;
...
|------------|------------|-------|
| 1 (più alto) | `()``[]``.``?.` | Sinistra |
| 2| `new`(senza argomenti) | Giusto |
| 3| `++``--` (postfisso) | N/D |
| 4| `!``~``+x``-x``typeof``void``delete``++``--`(prefisso) | Giusto |
| 5| `**`| Giusto |
| 6| `*``/``%`| Sinistra |
| 7| `+``-` | Sinistra |
| 8| `<<``>>``>>>`| Sinistra |
| 9| `<``<=``>``>=``in``instanceof` | Sinistra |
| 10| `===``!==``==``!=` | Sinistra |
| 11| `&`| Sinistra |
| 12| `^`| Sinistra |
| 13| `\|`| Sinistra |
...
| 16| `??`| Sinistra |
| 17| `=``+=``-=`ecc. | Giusto |
| 18 (il più basso) | `=>`(funzione freccia) | Giusto |
---
...
## Funzioni
### Dichiarazioni ed espressioni di funzioni
```javascript
...
## Strutture dati
### Panoramica delle raccolte integrate
| Collezione | Ordinato | Mutevole | Duplicati | Tipo chiave | Ricerca |
|------------|---------|---------|------------|----------|--------|
| `Array`| Sì | Sì | Sì | Indice (numero) | O(n) |
| `Object`| Sì (inserimento) | Sì | Chiavi: No | Stringa/Simbolo | O(1) media |
| `Map`| Sì (inserimento) | Sì | Chiavi: No | Qualsiasi | O(1) media |
| `Set`| Sì (inserimento) | Sì | No | — | O(1) media |
| `WeakMap`| No | Sì | Chiavi: No | Solo oggetto | O(1) media |
| `WeakSet`| No | Sì | No | Solo oggetto | O(1) media |
### Array
```javascript
// Creation
...
## Riepilogo
La sintassi di JavaScript è apparentemente semplice in superficie ma contiene una profondità significativa. Il linguaggio si è evoluto da un semplice strumento di scripting in un linguaggio multi-paradigma con classi, moduli, generatori, proxy e un sofisticato modello asincrono. Comprendere la precedenza degli operatori, il ciclo degli eventi, le catene di prototipi e le sfumature dell'associazione`this`separa gli sviluppatori competenti da quelli esperti. JavaScript moderno (ES2020+) con concatenamento opzionale, coalescenza nulla,`Promise.allSettled`e wait di livello superiore continua a ridurre il boilerplate mantenendo la compatibilità con le versioni precedenti.