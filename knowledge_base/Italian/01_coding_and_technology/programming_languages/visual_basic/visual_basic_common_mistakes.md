...
---
# Visual Basic: errori comuni e anti-pattern
Questo documento cataloga gli errori, i trap e gli anti-pattern più comuni in Visual Basic (VB.NET) con le correzioni.
---

## 1. Non smaltire risorse
```vb
' ❌ WRONG — resource leak
...
## 2.`=`vs`Is`per il confronto degli oggetti
```vb
...
## 3. Opzione rigorosamente disattivata
```vb
...
## 4. Non utilizzare`Using`per le connessioni al database
```vb
...
## 5. Anti-pattern: logica di business nei gestori di eventi
```vb
...
## Riepilogo
Trap VB.NET: utilizzare sempre`Using`per risorse usa e getta, distinguere`=`(valore) da`Is`(riferimento), abilitare`Option Strict On`, utilizzare blocchi`Using`per le connessioni al database e separare la logica aziendale dai gestori eventi dell'interfaccia utente. Il moderno VB.NET è un potente linguaggio .NET: utilizza correttamente le sue funzionalità.