...
-->
# Visual Basic: riferimento alla sintassi
Questo documento fornisce un riferimento completo e strutturato alla sintassi per Visual Basic (VB.NET). Completa il riferimento principale di Visual Basic concentrandosi su modelli di sintassi esaustivi, funzionalità OOP, LINQ, async/await e idiomi di integrazione .NET.
---

## Operatori ed espressioni
| Categoria | Operatore | Descrizione | Esempio |
|----------|----------|-------------|---------|
| **Aritmetica** | `+`| Aggiunta | `a + b`|
| | `-`| Sottrazione | `a - b`|
| | `*`| Moltiplicazione | `a * b`|
| | `/`| Divisione (restituisce Doppio) | `a / b`|
| | `\`| Divisione intera | `a \ b`|
| | `Mod`| Modulo | `a Mod b`|
| | `^`| Esponenziazione | `a ^ b`|
| **Confronto** | `=`| Uguale | `a = b`|
| | `<>`| Non uguali | `a <> b`|
| | `<`| Meno di | `a < b`|
| | `>`| Maggiore di | `a > b`|
| | `<=`| Minore o uguale | `a <= b`|
| | `>=`| Maggiore o uguale | `a >= b`|
| **Logico** | `AndAlso`| Cortocircuito AND | `a AndAlso b`|
| | `OrElse`| Cortocircuito OR | `a OrElse b`|
| | `Not`| NON logico | `Not a`|
| | `And`| Bit per bit AND | `a And b`|
| | `Or`| OR bit a bit | `a Or b`|
| | `Xor`| XOR bit a bit | `a Xor b`|
| **Stringa** | `&`| Concatenazione | `"Hello" & " " & "World"`|
| | `+=`| Aggiungi | `s &= " more"`|
| **Compito** | `=`| Assegna | `x = 42`|
| **Nullo** | `Is`| Uguaglianza di riferimento | `obj Is Nothing`|
| | `IsNot`| Disuguaglianza di riferimento | `obj IsNot Nothing`|
| **Tipo** | `TryCast`| Cast sicuro (non restituisce nulla) | `TryCast(obj, String)`|
| | `CType`| Converti | `CType(obj, Integer)`|
| | `DirectCast`| Lancio diretto (lancia in caso di fallimento) | `DirectCast(obj, String)`|
---
...
## Riepilogo
La sintassi di Visual Basic è pulita e simile all'inglese, rendendolo uno dei linguaggi di programmazione più leggibili. VB.NET fornisce accesso completo all'ecosistema .NET, incluso LINQ per l'interrogazione dei dati, async/await per operazioni simultanee, generici per l'indipendenza dai tipi e OOP completo con classi, interfacce ed ereditarietà. La distinzione tra maiuscole e minuscole e le parole chiave dettagliate (End If, End Sub, End Class) migliorano la leggibilità. Per le applicazioni Windows aziendali, l'automazione degli uffici (VBA) e la prototipazione rapida all'interno di .NET, VB.NET rimane un linguaggio produttivo e capace.