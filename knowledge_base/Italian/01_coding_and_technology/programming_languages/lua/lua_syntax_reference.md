...
---
# Lua — Riferimento alla sintassi
Questo documento fornisce un riferimento alla sintassi completo e strutturato per Lua (5.4). Completa il riferimento principale di Lua concentrandosi su modelli di sintassi esaustivi, tabelle e metatabelle, coroutine e sulla filosofia di design unica di Lua.
---
...
| `//`| Divisione del pavimento | `7 // 2`| Restituisce 3 (Lua 5.3+) |
| `==``~=` | Uguaglianza | `a == b`| `~=`è "non uguale" |
| `<``>``<=``>=` | Confronto | `a >= b`| |
| `and``or``not`| Logico | `a and b`| Restituisce operandi, non booleani |
| `..`| Concatenazione | `"hello" .. " world"`| |
| `#`| Lunghezza | `#t`| Lunghezza della stringa o della tabella |
| `&``\|``~``<<``>>`| Bit per bit | `a & b`| Lua 5.3+ (operazioni intere) |
### Operatori logici: risultati non booleani
```lua
...
## Tabelle: la struttura universale dei dati
```lua
...
## Moduli e gestione degli errori
```lua
...
## Riepilogo
La sintassi di Lua è volutamente minimale: l'intero linguaggio può essere descritto in un'unica pagina. Le tabelle fungono da struttura dati universale (array, dizionari, oggetti, moduli). Le metatabelle forniscono estensibilità senza complessità del linguaggio. Le coroutine consentono il multitasking cooperativo e modelli iteratori eleganti. La potenza del linguaggio deriva dalla sua integrabilità: Lua è progettato per vivere all'interno delle applicazioni host, fornendo funzionalità di scripting con un sovraccarico minimo. Per lo sviluppo di giochi, la configurazione e i sistemi embedded, la semplicità di Lua è il suo più grande punto di forza: un piccolo linguaggio che fa esattamente ciò di cui ha bisogno e niente di più.