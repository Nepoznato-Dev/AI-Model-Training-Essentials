...
-->
# Lua: errori comuni e anti-modelli
Questo documento cataloga gli errori, le trappole e gli anti-pattern più comuni in Lua con le relative correzioni.
---

## 1. Indicizzazione su base 1
```lua
...
## 2.`nil`contro`false`in Condizioni
```lua
...
## 3. Semantica di riferimento delle tabelle
```lua
...
## 4. Variabili globali per impostazione predefinita
```lua
...
## 7. Non utilizzare metatabelle per OOP
```lua
...
## Riepilogo
Il minimalismo di Lua crea trappole: indicizzazione basata su 1, solo`nil`e`false`sono falsi, le tabelle sono riferimenti (non valori), le variabili sono globali per impostazione predefinita e`pairs`non è ordinato mentre`ipairs`è ordinato. Il metodo Lua è: usa sempre`local`, usa`ipairs`per gli array e`pairs`per i dizionari, usa`table.concat`per la creazione di stringhe e usa le metatabelle per OOP. La semplicità di Lua è il suo potere, ma rispetta i casi limite.