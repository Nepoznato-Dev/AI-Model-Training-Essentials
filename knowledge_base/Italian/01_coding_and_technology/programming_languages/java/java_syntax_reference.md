...
|----------|------|---------|--------|-------|
| `+`| Aggiunta | `3 + 2`| `5`| Anche concatenazione di stringhe |
| `-`| Sottrazione | `3 - 2`| `1`| |
| `*`| Moltiplicazione | `3 * 2`| `6`| |
| `/`| Divisione | `7 / 2`| `3`| Divisione intera per`int`; `3.5`per`double`|
| `%`| Resto | `7 % 2`| `1`| |
...
|----------|------|---------|-------|
| `==`| Uguale | `x == y`| Uguaglianza di riferimento per oggetti |
| `!=`| Non uguali | `x != y`| |
|  `<`, `>`, `<=`,`>=`| Relazionale | `x >= y`| |
| `&&`| AND logico | `a && b`| Cortocircuito |
| `\|\|`| OR logico | `a \|\| b`| Cortocircuito |
| `!`| NON logico | `!true`| `false`|
| `instanceof`| Digitare Controlla | `obj instanceof String`| Anche la corrispondenza dei modelli (Java 16+) |
### Operatori bit a bit
| Operatore | Nome | Esempio | Note |
|----------|------|---------|-------|
| `&`| E | `5 & 3`| `1`|
...
|------------|------------|
| 1 (più alto) | Suffisso:`expr++``expr--` |
| 2| Unario:`++expr``--expr``+expr``-expr``~``!` |
| 3| Cast:`(Type)expr`|
| 4| Moltiplicativo:`*``/``%`|
| 5| Additivo:`+``-` |
| 6| Shift:`<<``>>``>>>`|
| 7| Relazionale:`<``>``<=``>=``instanceof`|
| 8| Uguaglianza:`==``!=` |
| 9| AND bit a bit:`&`|
| 10| XOR bit per bit:`^`|
| 11| OR bit a bit:`\|`|
...
| Interfaccia | Implementazioni | Ordinato | Variante thread-safe |
|-----------|----------------|---------|----------------------|
| `List`| `ArrayList`,`LinkedList`| Sì | `CopyOnWriteArrayList`|
...
| `Deque`| `ArrayDeque`| Entrambe le estremità | `ConcurrentLinkedDeque`|
### Elenchi
```java
// Creation
...
## Riepilogo
La sintassi di Java si è evoluta notevolmente rispetto alle sue origini nel 1995. Modern Java (21+) include record, classi sigillate, corrispondenza di modelli, thread virtuali e modelli di stringhe, rendendolo più conciso ed espressivo che mai. La sintassi principale rimane familiare (simile a C, fortemente tipizzata, basata su classi), ma le aggiunte di lambda, API Stream e`var`hanno ridotto significativamente il boilerplate. La forza del linguaggio risiede nel suo enorme ecosistema, nelle garanzie di compatibilità con le versioni precedenti e nell'ottimizzazione delle prestazioni della JVM attraverso la compilazione JIT.