...
# Confronto tra lingue diverse: flusso di controllo
## Istruzioni If/Else
```python
# Python
if x > 0:
...
| Lingua | Sintassi |
|----------|--------|
| Pitone | `for i in range(10):`|
| JavaScript | `for (let i = 0; i < 10; i++)`|
| Ruggine | `for i in 0..10`|
| Vai | `for i := 0; i < 10; i++`|
| Giava | `for (int i = 0; i < 10; i++)`|
| C/C++ | `for (int i = 0; i < 10; i++)`|
| Rubino | `10.times { \|i\| ... }`o`(0..9).each { \|i\| ... }`|
| Veloce | `for i in 0..<10`|
| Kotlin | `for (i in 0 until 10)`|
| Haskell | `mapM_ f [0..9]`|
| Perl | `for my $i (0..9)`|
| R | `for (i in 1:10)`|
| PHP | `for ($i = 0; $i < 10; $i++)`|
| SQL | N/D (basato su set, nessun loop nell'SQL standard) |
| Bash | `for i in {0..9}; do ... done`|
| Lua | `for i = 0, 9 do ... end`|
| MATLAB | `for i = 1:10 ... end`|
| Prologo | N/A (ricorsione invece) |
| Lisp | `(dotimes (i 10) ...)`|
| Ada | `for I in 0..9 loop ... end loop;`|
| COBOL | `PERFORM VARYING I FROM 0 BY 1 UNTIL I >= 10`|
| Erlang | N/A (ricorsione + liste invece) |
| OCaml | `for i = 0 to 9 do ... done`|
| Assemblea | Loop manuale con CMP/JMP |
| Delfi | `for I := 0 to 9 do ... end;`|
| Gratta | `repeat (10) ... end`|
| V.B. | `For i As Integer = 0 To 9 ... Next`|
| Scala | `for (i <- 0 until 10) ...`|
| Dardo | `for (var i = 0; i < 10; i++)`|
| Giulia | `for i in 0:9 ... end`|
| Dattiloscritto | `for (let i = 0; i < 10; i++)`|
| C# | `for (int i = 0; i < 10; i++)`|
| Fortran | `do i = 1, 10 ... end do`|
## Corrispondenza/cambia modello
| Lingua | Costruisci |
|----------|-----------|
| C| `switch (x) { case 1: ... break; }`|
| Giava | `switch (x) { case 1 -> ... }`(Java 14+) |
| Ruggine | `match x { 1 => ..., _ => ... }`|
| Haskell | `case x of 1 -> ...; _ -> ...`|
| Scala | `x match { case 1 => ... }`|
| Veloce | `switch x { case 1: ... default: ... }`|
| Kotlin | `when (x) { 1 -> ... else -> ... }`|
| OCaml | `match x with 1 -> ... \| _ -> ...`|
| Erlang | `case X of 1 -> ...; _ -> ... end`|
| Elisir | `case x do 1 -> ...; _ -> ... end`|
| Dardo | `switch (x) { 1 => ...; _ => ... }`(Dardo 3.0) |
| PHP | `match($x) { 1 => ..., default => ... }`(PHP 8.0) |
| C# | `x switch { 1 => ..., _ => ... }`(C# 8+) |
| Dattiloscritto | Nessuna corrispondenza di pattern nativa (usa if/else) |
| Pitone | `match x: case 1: ... case _: ...`(Python 3.10+) |
| Prologo | Costruito nella corrispondenza delle clausole |
| Vai | `switch x { case 1: ... default: ... }`|