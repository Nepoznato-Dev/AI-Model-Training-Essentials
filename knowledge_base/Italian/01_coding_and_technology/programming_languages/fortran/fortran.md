...
## Perché Fortran è importante
- **Prestazioni HPC**: i compilatori Fortran producono alcuni dei codici numerici più veloci disponibili, spesso corrispondenti o superiori a C/C++ per le operazioni sugli array.
- **Base di codici legacy**: decenni di codice scientifico (modelli climatici, simulazioni fisiche) sono scritti in Fortran.
- **Operazioni sugli array**: supporto nativo di array multidimensionali con sintassi progettata per il calcolo matematico.
- **Stabilità numerica**: il linguaggio e i compilatori sono ottimizzati per il calcolo in virgola mobile.
- **Coarray**: supporto integrato per la programmazione parallela (Fortran 2008+).
- **Ancora in evoluzione**: Fortran 2023 aggiunge funzionalità moderne mantenendo la compatibilità con le versioni precedenti.
## I compromessi
| Limitazione | Dettagli | Soluzione tipica |
|-----------|---------|-------------|
| **Comunità di nicchia** | Piccolo e specializzato, per lo più scientifico/HPC | Comunità attiva nella scienza computazionale |
| **Ecosistema limitato** | Meno librerie rispetto a Python, Java o C++ | Utilizzare BLAS/LAPACK per lavori numerici; C interoperabilità per altre esigenze |
| **Non generico** | Scarso per la programmazione web, mobile, GUI o di sistema | Utilizzare Fortran per il calcolo; wrap in Python/C per le applicazioni |
| **Percezione** | Spesso visto come "obsoleto" nonostante le moderne funzionalità | Focus sui suoi punti di forza: numerici e HPC |
| **Assunzione** | Pochi sviluppatori Fortran entrano nel mercato | Esperti esistenti nel mondo accademico e nei laboratori nazionali |
---

## Fondamenti di sintassi (Fortran moderno)
```fortran
program hello
    implicit none
...
### Tipi derivati ​​e procedure legate al tipo
```fortran
...
### Procedure generiche e sovraccarico degli operatori
```fortran
module vector_ops
    implicit none
...
### Moduli e organizzazione del programma
```fortran
module constants
    implicit none
...
### Coarray (parallelismo integrato in Fortran 2008)
```fortran
...
## Interoperabilità
### Interoperabilità C (ISO_C_BINDING)
```fortran
module c_interface
...
### Integrazione Python (tramite f2py o libreria condivisa)
```fortran
...
### Modello 1: programmazione di array (evitare loop)
```fortran
...
### Modello 2: tipi derivati ​​parametrizzati
```fortran
...
### Modello 3: modello di callback con puntatori a procedura
```fortran
module integrator
    implicit none
    abstract interface
...
## Quando utilizzare Fortran
| Scenario | Perché Fortran | Alternativa migliore |
|----------|-----------|-------------|
| HPC/supercalcolo | Ottimizzato per le prestazioni numeriche | C++ (con attenzione), Julia |
| Simulazioni scientifiche | Decenni di codice convalidato | Python (per la prototipazione), C++ |
| Modelli climatologici | Basi di codici legacy; prestazioni | — |
| Fisica computazionale | Operazioni su array nativi | Python (NumPy), Julia |
| Modellazione finanziaria (HPC) | Prestazioni per calcoli su larga scala | C++, Python |
...
## Riepilogo
Fortran è il linguaggio di programmazione scientifica originale e rimane una potenza nel calcolo ad alte prestazioni. Il Fortran moderno è un linguaggio capace e in evoluzione con operazioni di array native, supporto per la programmazione parallela e interoperabilità C. Sebbene la sua comunità sia piccola e specializzata, Fortran continua a eseguire alcuni dei carichi di lavoro computazionali più impegnativi al mondo. Per il calcolo numerico su larga scala, Fortran rimane rilevante.