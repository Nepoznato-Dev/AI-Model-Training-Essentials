...
| `<``>``<=``>=` | Confronto | `a >= b`| |
| `.and.``.or.``.not.`| Logico | `a .and. b`| |
| `.eqv.``.neqv.` | Equivalenza logica | `a .eqv. b`| |
| `//`| Concatenazione di stringhe | `'hello' // ' world'`| |
---
...
## Coarray (programmazione parallela)
```fortran
...
## Riepilogo
La sintassi del Fortran moderno è pulita, strutturata e ottimizzata per il calcolo numerico. Le operazioni sugli array eliminano i cicli per la maggior parte delle attività matematiche. I moduli forniscono l'incapsulamento e la gestione dello spazio dei nomi. Le funzionalità OOP (tipi derivati, ereditarietà, polimorfismo) consentono un'organizzazione pulita del codice. I coarray offrono programmazione parallela nativa senza librerie esterne. I punti di forza di Fortran (prestazioni, operazioni di array e precisione numerica) rimangono ineguagliati per il calcolo scientifico. Per l’HPC, la fisica computazionale e la simulazione ingegneristica, Fortran continua a essere una centrale elettrica.