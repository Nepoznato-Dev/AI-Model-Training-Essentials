...
Questo documento fornisce un riferimento sintattico completo e strutturato per C (C11/C17 con note C23). Completa il riferimento principale del C concentrandosi su modelli di sintassi esaustivi, tabelle degli operatori, meccanica dei puntatori e gestione della memoria.
---
...
| `/`| Divisione | `7 / 2`| `3`| Troncamento intero per tipi int |
| `%`| Modulo | `7 % 2`| `1`| Solo per i tipi interi |
| `++`| Incremento | `i++`/`++i`| | Post e pre-incremento |
| `--`| Decremento | `i--`/`--i`| | Post e pre decremento |
...
| `\|`| OPPURE | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `~`| Complemento | `~0`| Tutti i bit sono girati |
| `<<`| Maiusc sinistro | `1 << 3`| `8`|
| `>>`| Maiusc destro | `8 >> 3`| `1`(definito dall'implementazione per firmato) |
### Operatori puntatori e indirizzi
| Operatore | Nome | Esempio | Note |
|----------|------|---------|-------|
| `&`| Indirizzo di | `&x`| Restituisce il puntatore a x |
| `*`| Dereferenziazione | `*ptr`| Valore di accesso al puntatore |
| `->`| Accesso ai membri tramite puntatore | `p->x`| Equivalente a`(*p).x`|
| `.`| Accesso membri | `s.field`| Accesso diretto ai membri della struttura |
| `[]`| Pedice dell'array | `arr[i]`| Equivalente a`*(arr + i)`|
### Precedenza degli operatori (dal più alto al più basso)
| Precedenza | Operatori | Associatività |
|------------|------------|-------|
| 1 (più alto) | `()``[]``->``.` | Da sinistra a destra |
| 2| `!``~``+``-` (unario)`*``&``sizeof``(type)` | Da destra a sinistra |
...
| 13| `? :`(ternario) | Da destra a sinistra |
| 14| `=``+=``-=``*=``/=``%=``&=``\|=``^=``<<=``>>=`| Da destra a sinistra |
| 15 (il più basso) | `,`(virgola) | Da sinistra a destra |
---
...
## Elementi essenziali della libreria standard
```c
...
## Riepilogo
La sintassi del C è minima e vicina all'hardware: puntatori, gestione manuale della memoria e un sottile strato di astrazione sulle istruzioni della macchina. La potenza del linguaggio deriva da questa semplicità: non ci sono quasi costi nascosti, nessun sovraccarico di runtime e controllo completo sul layout della memoria. Il preprocessore, sebbene potente, richiede disciplina per essere utilizzato in sicurezza. Il C moderno (C11/C17/C23) ha aggiunto importanti funzionalità di sicurezza come`_Static_assert`,`_Generic`,`nullptr`e`typeof`, rendendo il linguaggio sempre più sicuro pur mantenendo la sua filosofia fondamentale di fiducia nel programmatore.