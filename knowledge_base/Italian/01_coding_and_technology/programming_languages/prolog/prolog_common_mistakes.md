...
-->
# Prolog: errori comuni e anti-pattern
Questo documento cataloga gli errori, le trappole e gli anti-pattern più comuni in Prolog con le relative correzioni.
---
...
## 2. Non utilizzare correttamente il taglio (`!`).
```prolog
...
## 4. Si verifica una mancata comprensione
```prolog
...
## 5. Elenca le operazioni senza accumulatori
```prolog
...
## Riepilogo
Il paradigma di programmazione logica di Prolog crea trappole uniche: confondere`=`(unificazione),`==`(uguaglianza) e`is`(aritmetica); posizionamento errato del taglio; ricorsione infinita senza tracciamento delle visite; e il controllo avviene. Il metodo Prolog è: comprendere a fondo l'unificazione, utilizzare il taglio con parsimonia e correttamente, utilizzare gli accumulatori per un'efficiente creazione di elenchi e considerare sempre la terminazione.