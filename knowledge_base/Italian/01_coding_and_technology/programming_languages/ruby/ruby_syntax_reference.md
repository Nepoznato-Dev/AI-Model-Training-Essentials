...
| `==``!=``===``<=>` | Uguaglianza | `a == b`| `===`è l'uguaglianza tra maiuscole e minuscole; `<=>`è un'astronave |
| `eql?`| Uguaglianza di valore | `a.eql?(b)`| Più severo di`==`(tipo controlli) |
| `equal?`| Identità | `a.equal?(b)`| Stesso oggetto? |
| `<``>``<=``>=` | Confronto | `a >= b`| |
| `&&``\|\|``!`| Logico | `a && b`| Cortocircuito; restituisce l'ultimo valore valutato |
| `and``or``not`| Logico (precedenza bassa) | `a and b`| Evita: usa`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | Bit per bit | `a & b`| |
| `=~``!~` | Corrispondenza regex | `str =~ /pattern/`| Restituisce indice o zero |
| `..``...` | Gamma | `1..10`| `..`compreso; `...`esclusivo |
| `?.`| Navigazione sicura | `user&.name`| Restituisce nil se l'utente è nil |
| `[]``[]=` | Accesso agli elementi | `arr[0]`| |
### Verità
```ruby
...
## Riepilogo
La sintassi di Ruby è progettata per la felicità degli sviluppatori: si legge come l'inglese e premia l'espressività. La potenza del linguaggio deriva dal suo modello a oggetti (tutto è un oggetto), blocchi e chiusure, capacità di metaprogrammazione e il modulo Enumerable che fornisce una ricca API per l'elaborazione delle raccolte. Ruby 3.x ha aggiunto la corrispondenza dei modelli, le firme dei tipi RBS e Ractor per la concorrenza, pur mantenendo la filosofia principale del linguaggio: rendere i programmatori produttivi e felici.