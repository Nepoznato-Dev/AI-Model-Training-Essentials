...
|----------|------|---------|-------|
| `+``-``*``/``^`| Aritmetica | `2^10`| |
| `%%`| Modulo | `7 %% 3`| Restituisce 1 |
| `%/%`| Divisione intera | `7 %/% 3`| Restituisce 2 |
| `==``!=` | Uguaglianza | `x == y`| Vettorializzato |
| `<``>``<=``>=` | Confronto | `x >= y`| Vettorializzato |
| `&``\|``!`| Logico (vettorizzato) | `x & y`| Dal punto di vista degli elementi |
| `&&``\|\|` | Logico (scalare) | `x && y`| Solo primo elemento |
| `%in%`| Appartenenza | `x %in% y`| |
| `<-``=` | Compito | `x <- 10`| `<-`è idiomatico |
| `->`| Assegna diritto | `10 -> x`| Usato raramente |
| `<<-`| Assegnazione globale | `x <<- 10`| Assegna nell'ambiente genitore |
| `$`| Estrai elemento | `df$column`| |
| `[``[[` | Indicizzazione | `x[1]`/`x[[1]]`| `[`restituisce lo stesso tipo; `[[`estrae |
| `%>%`| Tubo (magrittr) | `x %>% f()`| |
...
## Riepilogo
La sintassi di R è progettata per il calcolo statistico e l'analisi dei dati. La natura vettorizzata delle operazioni elimina i loop per la maggior parte delle attività. L'ecosistema tidyverse (dplyr, ggplot2, tidyr) fornisce una grammatica coerente e leggibile per la manipolazione e la visualizzazione dei dati. Le capacità di modellazione statistica di R, dai test t di base ai modelli a effetti misti, non hanno eguali in termini di profondità. Per la scienza dei dati, la statistica e la ricerca riproducibile, R rimane uno strumento essenziale.