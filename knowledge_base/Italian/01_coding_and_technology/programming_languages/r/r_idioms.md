...
# R: modelli idiomatici e migliori pratiche
Questa guida illustra i modelli idiomatici e le procedure consigliate per scrivere codice R pulito e idiomatico.
---
...
## Riepilogo
Gli idiomi R enfatizzano: vettorizzazione (evitare loop), tidyverse pipe (`|>`),`<-`per l'assegnazione, denominazione snake_case e programmazione funzionale con`purrr`. Segui la Guida allo stile di Tidyverse, usa lintr per la linting e lo styler per la formattazione. R valorizza le operazioni vettorizzate e la pipe per le trasformazioni di dati leggibili.