...
|----------|------|---------|-------|
| `+``-``*``/` | Aritmetica | `x + y`| Num classe di tipo |
| `div``mod` | Divisione intera | `7 \`div\` 2`| Restituisce 3 |
| `^``**` | Potenza | `2 ^ 10`| `^`per integrale; `**`per flottante |
| `==``/=` | Uguaglianza | `x == y`| Classe tipo Eq |
| `<``>``<=``>=` | Confronto | `x >= y`| Ord typeclass |
| `&&``\|\|``not`| Logico | `a && b`| Cortocircuito |
| `.`| Composizione delle funzioni | `(f . g) x`| `f (g x)`|
| `$`| Applicazione della funzione | `f $ x + 1`| `f (x + 1)`— precedenza bassa |
| `<>`| Il monoide aggiunge | `"hello" <> " world"`| Semigruppo |
| `<$>`| Funtore fmap | `(+1) <$> Just 5`| `fmap (+1) (Just 5)`|
| `<*>`| Applicativo applicare | `pure (+) <*> Just 3`| |
| `>>=`| Monade vincola | `Just 5 >>= \x -> Just (x+1)`| |
| `>>`| Sequenza monade | `putStrLn "hi" >> putStrLn "bye"`| |
| `=<<`| Rilegatura (invertita) | `f =<< mx`| Uguale a`mx >>= f`|
| `++`| Concatenazione di elenchi | `[1,2] ++ [3,4]`| O(n) nell'elenco di sinistra |
| `:`| Contro | `1 : [2, 3]`| O(1) |
| `!!`| Indice | `[1,2,3] !! 1`| O(n) — funzione parziale |
| `\\`| Differenza elenco | `[1,2,3] \\ [2]`| `[1,3]`|
### Precedenza operatore (selezionato)
...
## Riepilogo
La sintassi di Haskell è minima ma straordinariamente espressiva. Il sistema di tipi, con tipi di dati algebrici, classi di tipi, GADT e famiglie di tipi, funge sia da specifica che da strumento di verifica. Il pattern match, la valutazione pigra e le monadi forniscono soluzioni eleganti a problemi complessi. La purezza di Haskell (nessun effetto collaterale nei tipi) rende più facile ragionare, testare e parallelizzare il codice. Sebbene la curva di apprendimento sia ripida, l'influenza di Haskell permea i linguaggi moderni: dagli optional di Swift alle classi sigillate di Kotlin al sistema di tipi di Rust. Padroneggiare Haskell ti rende un programmatore migliore in ogni lingua.