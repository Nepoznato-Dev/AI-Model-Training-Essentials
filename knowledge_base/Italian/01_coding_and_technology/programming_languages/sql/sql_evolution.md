...
# SQL: cronologia ed evoluzione delle versioni
## Cronologia
| Versione | Anno | Tema chiave |
|---------|------|-----------|
| SEGUITO | 1974 | Linguaggio di ricerca originale IBM (Chamberlin & Boyce) |
| SQL-86 | 1986 | **Primo standard ANSI** (SQL-86) |
| SQL-89 | 1989 | Revisione minore (vincoli di integrità) |
| SQL-92 | 1992 | **Principale**:`JOIN`, sottoquery,`CASE`,`COALESCE`|
| SQL:1999 | 1999 | **Espressioni regolari**, query ricorsive, trigger, BLOB |
| SQL:2003 | 2003| **Funzioni finestra**, XML, colonne`GENERATED`|
| SQL:2006 | 2006| Supporto XML,`MERGE`|
| SQL:2008 | 2008| `INSTEAD OF`attiva,`TRUNCATE`,`ORDER BY`nelle visualizzazioni |
| SQL:2011 | 2011 | **Dati temporali** (`AS OF`,`FOR SYSTEM_TIME`),`FETCH FIRST`|
| SQL:2016 | 2016| **Supporto JSON**, riconoscimento del modello di riga |
| SQL:2019 | 2019 | **Funzioni tabella polimorfica**,`LISTAGG`|
| SQL:2023 | 2023 | **`JSON_TABLE`**, operazioni `SET`, miglioramenti dell'array |
## Traguardi importanti
### SEQUEL e primo SQL (1974–1986)
- **1974**: Donald Chamberlin e Raymond Boyce creano SEQUEL presso IBM Research
- **Obiettivo**: manipolazione delle query per System R (database relazionale)
- Rinominato SQL (Structured Query Language) a causa di un conflitto di marchi
- **1986**: Primo standard ANSI (SQL-86)
- **1987**: l'ISO adotta SQL-87
### SQL-92 — La Fondazione (1992)
- **Lo standard più importante**: tutto l'SQL moderno discende da questo
-`INNER JOIN`, `LEFT JOIN`,`RIGHT JOIN`
- Sottoquery (`SELECT` nidificate)
- Espressione `CASE`
-`COALESCE`,`NULLIF`
- Vincoli`UNIQUE`, `CHECK`
- Definizione dello schema (`CREATE SCHEMA`)
### SQL:1999 - Inizia l'SQL moderno (1999)
- Espressioni regolari (`LIKE`,`SIMILAR TO`)
- Query ricorsive (`WITH RECURSIVE`)
- Trigger
- BLOB/CLOB (oggetti di grandi dimensioni binari/caratteri)
- Tipi definiti dall'utente (UDT)
-`ORDER BY`nelle sottoquery
### SQL:2003 — Rivoluzione dell'analisi (2003)
- **Funzioni finestra**:`ROW_NUMBER()`,`RANK()`,`DENSE_RANK()`,`LAG()`,`LEAD()`,`SUM() OVER()`
- Tipo di dati e funzioni XML
-`GENERATED ALWAYS AS IDENTITY` 
-`SAVEPOINT`(controllo delle transazioni)
- Funzioni hash
### SQL:2011 — Dati temporali (2011)
- **Tabelle temporali**:`FOR SYSTEM_TIME AS OF`,`VERSIONING`
-`FETCH FIRST n ROWS ONLY` (standard `LIMIT`)
- Impaginazione`OFFSET`/ `FETCH`
### SQL:2016–2023 — JSON e oltre (2016–presente)
- **2016**: tipo di dati JSON,`JSON_VALUE`,`JSON_QUERY`,`JSON_EXISTS`
- **2019**: funzioni della tabella polimorfica,`LISTAGG`
- **2023**:`JSON_TABLE`(vista relazionale di JSON), operazioni `SET`, miglioramenti dell'array
...
## Evoluzione del dialetto maggiore
```
