...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmetica | `a + b`| |
| `..``...` | Gamma | `1...10`| `...`chiuso; `..<`semiaperto |
| `==``!=``<``>``<=``>=` | Confronto | `a == b`| Richiede`Equatable`/`Comparable`|
| `===``!==` | Identità | `a === b`| Stesso riferimento (solo classi) |
| `&&``\|\|``!`| Logico | `a && b`| Cortocircuito |
| `??`| Zero coalescente | `a ?? b`| Restituisce`b`se`a`è zero |
| `?.`| Concatenamento opzionale | `a?.b`| Restituisce nil se`a`è nil |
| `!`| Forza lo scarto | `a!`| Si blocca se pari a zero: evita |
| `&`| Parametro Inout | `swap(&a, &b)`| Passa per riferimento |
### Zero Operatori Handling
```swift
...
## Wrapper di proprietà e costruttori di risultati
```swift
// Property wrapper
@propertyWrapper
struct Trimmed {
...
## Riepilogo
La sintassi di Swift bilancia la sicurezza con l'espressività. Gli optional eliminano gli arresti anomali nulli, i protocolli consentono una composizione flessibile senza ereditarietà e la concorrenza moderna (asincrona/attesa, attori) rende accessibile la programmazione simultanea sicura. Il linguaggio continua ad evolversi con funzionalità come pacchetti di parametri, macro e proprietà: ogni aggiunta rende Swift più sicuro e potente pur mantenendo la sua promessa principale: codice sicuro per costruzione.