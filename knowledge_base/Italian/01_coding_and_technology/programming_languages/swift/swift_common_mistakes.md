...
---
# Swift: errori comuni e anti-pattern
Questo documento cataloga gli errori, le trappole e gli anti-pattern più comuni in Swift. Ogni voce mostra l'approccio errato, spiega perché fallisce e fornisce la soluzione corretta.
---

## 1. Forza gli optional di scarto
```swift
// ❌ WRONG — crash if nil
...
##2. Conservare i Cicli con Chiusure
```swift
...
## 3. Non utilizzare`enum`con valori associati
```swift
...
## 4. Incomprensione tra tipi di valore e tipi di riferimento
```swift
...
## 5. Anti-Pattern: controller di visualizzazione massiccio
```swift
...
## 6. Non utilizzare`guard`per l'uscita anticipata
```swift
...
## 7. Dimenticare`weak`nei delegati
```swift
...
## 8. Confusione nell'indice delle stringhe
```swift
...
## 9. Non utilizzare`lazy`per proprietà costose
```swift
...
## 10. Sicurezza del thread con `@MainActor`
```swift
...
## Riepilogo
Le funzionalità di sicurezza di Swift prevengono molti bug ma introducono le proprie trappole: forzare l'unwrapping provoca arresti anomali, i cicli di conservazione si nascondono nelle chiusure e nei delegati e la semantica del tipo valore sorprende gli sviluppatori provenienti da linguaggi di tipo riferimento. Il metodo Swift è: non forzare mai l'unwrap (`!`), utilizzare`guard`per le uscite anticipate, utilizzare`weak`per delegati e acquisizioni di chiusura, contrassegnare il codice dell'interfaccia utente con`@MainActor`e suddividere controller di visualizzazione di grandi dimensioni in modelli e servizi di visualizzazione. Il sistema di tipi di Swift è tuo amico: lavora con esso, non contro di esso.