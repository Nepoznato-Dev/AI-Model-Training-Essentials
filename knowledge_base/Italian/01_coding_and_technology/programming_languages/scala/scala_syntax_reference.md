...
---
# Scala — Riferimento alla sintassi
Questo documento fornisce un riferimento sintattico completo e strutturato per Scala (3.x con note sulle differenze 2.x). Completa il riferimento principale di Scala concentrandosi su modelli di sintassi esaustivi, sul sistema dei tipi, sui costrutti di programmazione funzionale e sugli idiomi di Scala.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmetica | `a + b`| |
| `**`| Potenza (BigDecimal) | `BigDecimal(2) ** 10`| Non integrato per Int |
| `==``!=` | Uguaglianza | `a == b`| Chiama`.equals()`— null-safe |
| `eq``ne` | Uguaglianza di riferimento | `a eq b`| Stessa istanza |
| `<``>``<=``>=` | Confronto | `a >= b`| Richiede`Ordered`|
| `&&``\|\|``!`| Logico | `a && b`| Cortocircuito |
| `&``\|``^``~` | Bit per bit | `a & b`| |
| `<<``>>``>>>`| Maiusc | `a << 2`| |
| `+:``:+``::``:::` | Operazioni di raccolta | `1 +: list`| Associativo a destra se termina con`:`|
| `->`| Creazione tuple | `"key" -> "value"`| `("key", "value")`|
| `=>`| Funzione/freccia | `x => x + 1`| Lambda o tipo mapping |
| `<-`| Generatore | `x <- List(1,2,3)`| Per-comprensione |
...
## Riepilogo
La sintassi di Scala unifica la programmazione orientata agli oggetti e quella funzionale in un potente sistema di tipi. La corrispondenza dei modelli, le comprensioni e le classi caso forniscono una modellazione dei dati espressiva. I tratti consentono una composizione flessibile senza limitazioni di ereditarietà singola. Il sistema di tipi, con generici, varianza, limiti di tipo e tipi opachi, rileva gli errori in fase di compilazione consentendo astrazioni a costo zero. Scala 3 semplifica il linguaggio con enumerazioni, metodi di estensione, give/using e tipi opachi, rendendo il linguaggio più accessibile pur mantenendo tutta la sua potenza.