...
## 1. Utilizzo di`var`invece di `val`
```scala
...
## 2. Non utilizzare la corrispondenza dei modelli per i tipi sigillati
```scala
// ❌ WRONG — if/else chain
...
## 3. Performance delle collezioni
```scala
...
## 4. Inferno di richiamata futura
```scala
...
## 5. Abuso di conversioni implicite
```scala
...
## 6. Non gestire correttamente `Option`
```scala
...
## Riepilogo
Il potere di Scala deriva dalla responsabilità: preferisci`val`a`var`, usa il pattern match per i tipi sigillati, gestisci`Option`correttamente (non chiamare mai`.get`), usa for-comprehensions invece di Futures nidificati ed evita null. Scala 3 semplifica molti di questi modelli con enumerazioni, metodi di estensione e tipi di unione. Il principio chiave: se il compilatore può controllarlo, lascia che sia il compilatore a controllarlo.