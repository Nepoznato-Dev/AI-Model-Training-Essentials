...
## Perché Ada è importante
- **Sistemi critici per la sicurezza**: progettati da zero per i sistemi in cui il guasto non è un'opzione.
- **Tipizzazione forte**: il sistema di tipi più rigido di qualsiasi linguaggio tradizionale: rileva gli errori in fase di compilazione che altri linguaggi non rilevano.
- **Concorrenza incorporata**: il tasking (programmazione simultanea) fa parte del linguaggio, non di una libreria.
- **Verifica formale**: supporta metodi formali per dimostrare la correttezza del programma.
- **Caratteristiche di affidabilità**: eccezioni integrate, contratti (pre/postcondizioni) e controlli di runtime.
- **Ada moderna**: Ada 2012 e Ada 2022 hanno aggiunto funzionalità moderne pur mantenendo le garanzie di sicurezza.
## I compromessi
| Limitazione | Dettagli | Soluzione tipica |
|-----------|---------|-------------|
| **Comunità di nicchia** | Piccola base di sviluppatori rispetto a C, Java o Python | Comunità specializzata ma competente |
| **Sintassi dettagliata** | Più dettagliato di C o Python | Accettare come parte della progettazione orientata alla sicurezza |
| **Ecosistema limitato** | Meno biblioteche rispetto alle lingue tradizionali | Scrivi codice personalizzato; utilizzare l'ampia libreria standard |
| **Curva di apprendimento** | Linguaggio complesso con molte caratteristiche di sicurezza | Inizia con il sottoinsieme "SPARK" di Ada per lavori critici per la sicurezza |
| **Disponibilità del compilatore** | Meno opzioni del compilatore (GNAT è la principale) | GNAT è gratuito e ben mantenuto |
---
...
### Tipi ed ereditarietà contrassegnati (OOP)
```ada
-- Abstract base type
package Shapes is
...
### Generici (polimorfismo in fase di compilazione)
```ada
...
### Oggetti protetti (dati condivisi thread-safe)
```ada
...
### Programmazione in lingue miste
```ada
...
### Schema 1: Osservatore con oggetti protetti
```ada
...
### Modello 2: Macchina a stati con tipologie discriminate
```ada
...
### Modello 3: RAII con tipi controllati
```ada
...
### Astrazioni senza spese generali
```ada
...
## Distribuzione
### SPARK per la certificazione critica per la sicurezza
```ada
...
## Quando usare Ada
| Scenario | Perché Ada | Alternativa migliore |
|----------|---------|-------------|
| Sistemi critici per la sicurezza | Progettato per questo; supporto alla verifica formale | — |
| Aviazione/aerospaziale | Compilatori certificati; Conformità DO-178C | — |
| Sistemi di difesa | Patrimonio del Dipartimento della Difesa; caratteristiche di sicurezza | — |
| Ferroviario/dispositivi medici | Requisiti di elevata integrità | — |
| Sviluppo di applicazioni generali | Eccessivo per sistemi non critici | Python, Java, Vai |
| Sviluppo web | Non adatto | JavaScript, Python |
| Scienza dei dati/ML | Non l'ecosistema | Pitone, R |
---

## Domande e risposte sintetiche
### Q1: In che modo il sistema di tipi di Ada previene i bug in fase di compilazione?
**R:** Il sistema di tipi di Ada è tra i più severi di qualsiasi linguaggio. Rileva errori che altri linguaggi non rilevano:
```ada
...
### D2: Qual è il modello di task di Ada e come si confronta con altri modelli di concorrenza?
**R:** Ada ha una concorrenza integrata con oggetti e attività protetti:
```ada
...
### D3: Come utilizzo i generici in Ada?
**R:** I generici di Ada sono espliciti e indipendenti dai tipi:
```ada
generic
   type Element_Type is private;
...
### D4: Cosa rende Ada adatto a sistemi critici per la sicurezza?
**R:** Ada fornisce:
- Sottoinsieme SPARK per la verifica formale (prova matematica di correttezza)
- Programmazione basata su contratti (pre/postcondizioni, invarianti di tipo)
- Nessuna allocazione di memoria implicita in SPARK
- Tasking e scheduling deterministici
- Profilo Ravenscar per sistemi real-time ad alta integrità
- Qualificazione della toolchain (DO-178C per l'avionica)
### Q5: Come posso creare progetti Ada?
**R:** Utilizza GPRBuild con file di progetto GPR:
```bash
gprbuild -P my_project.gpr
gprclean -P my_project.gpr
```
...
### Problema 1: implementazione di una coda indipendente dai tipi
**Passaggio 1: comprendere il problema**
Crea una coda limitata e thread-safe con controllo delle dimensioni in fase di compilazione.
**Passaggio 2: identificare l'approccio**
Utilizzare un oggetto protetto con un buffer limitato.
**Passaggio 3: implementazione**```ada
...
L'oggetto protetto garantisce la mutua esclusione. Le barriere all'ingresso impediscono il trabocco/sottopieno.
### Problema 2: convalida basata su contratto
**Passaggio 1: comprendere il problema**
Implementare una funzione di radice quadrata con contratti formali.
**Passaggio 2: identificare l'approccio**
Utilizza contratti Ada 2012 (pre/postcondizioni).
**Passaggio 3: implementazione**```ada
...
**Passaggio 4: verifica**
I controlli di runtime (asserzioni) rilevano le violazioni. In SPARK, questi diventano obblighi di prova.
---

## Riepilogo
Ada è un linguaggio costruito per la correttezza. Il suo sistema di tipi rigoroso, la concorrenza integrata e il supporto per la verifica formale lo rendono la scelta ideale per i sistemi in cui il fallimento non è accettabile. Sebbene la sua comunità sia piccola rispetto alle lingue tradizionali, Ada rimane essenziale nell’aviazione, nella difesa, nello spazio e in altri settori critici per la sicurezza. Per queste applicazioni, l'approccio rigoroso di Ada all'ingegneria del software non è un limite: è il punto.