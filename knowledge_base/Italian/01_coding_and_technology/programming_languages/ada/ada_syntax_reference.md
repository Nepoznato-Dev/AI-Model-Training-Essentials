...
|----------|------|---------|-------|
| `+``-``*``/``**`| Aritmetica | `A ** 2`| |
| `mod``rem` | Modulare/Resto | `A mod B`| `mod`sempre positivo |
| `&`| Concatenazione | `"hello" & " world"`| |
| `=``/=` | Uguaglianza | `A = B`| |
| `<``>``<=``>=` | Confronto | `A >= B`| |
| `and``or``not``xor` | Logico | `A and B`| |
| `and then``or else` | Cortocircuito | `A and then B`| |
| `in``not in` | Appartenenza alla gamma | `X in 1 .. 10`| |
| `:=`| Compito | `X := 10`| |
---
...
## Riepilogo
La sintassi di Ada è prolissa ma precisa. Ogni costrutto è progettato per chiarezza e sicurezza. Gli oggetti protetti eliminano le gare di dati a livello linguistico. I contratti (pre/postcondizioni, invarianti) rendono espliciti e verificabili i requisiti di correttezza. I generici forniscono un riutilizzo indipendente dai tipi. Per i sistemi critici per la sicurezza, la sintassi di Ada non è un peso: è una garanzia che il codice faccia quello che dice.