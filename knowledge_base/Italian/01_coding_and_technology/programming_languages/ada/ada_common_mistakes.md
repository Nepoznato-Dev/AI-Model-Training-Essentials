...
-->
# Ada: errori comuni e anti-modelli
Questo documento cataloga gli errori, le trappole e gli anti-pattern più comuni in Ada con le relative correzioni.
---

## 1. Non utilizzare `pragma Strict`
```ada
...
## 2. Errori di intervallo sui sottotipi
```ada
...
## 3. Non utilizzare parametri con nome per la leggibilità
```ada
...
## 4. Dimenticare di gestire le eccezioni
```ada
...
## Riepilogo
La forte digitazione di Ada previene molti bug ma richiede disciplina: usa`pragma Strict`, gestisci`Constraint_Error`per i sottotipi, usa l'associazione di parametri con nome per la leggibilità, gestisci sempre le eccezioni e progetta attentamente la comunicazione delle attività per evitare situazioni di stallo. Ada premia una programmazione attenta e attenta alla sicurezza.