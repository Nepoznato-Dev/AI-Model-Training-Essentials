...
Questo documento cataloga gli errori, le trappole e gli anti-pattern più comuni in C#. Ogni voce mostra l'approccio errato, spiega perché fallisce e fornisce la soluzione corretta.
---

## 1. Non smaltire gli oggetti `IDisposable`
```csharp
// ❌ WRONG — resource leak
...
## 4. LINQ`.ToList()`Allocazione non necessaria
```csharp
...
## 5. Acquisizione della variabile del loop (precedente a C# 5.0)
```csharp
...
## 6. Non utilizzare la corrispondenza dei modelli
```csharp
...
## 7. Anti-Pattern: Classi di Dio / Localizzatore di servizi
```csharp
...
## 8. Eccezioni per riferimenti nulli
```csharp
...
## 10. Non utilizzare i record per i tipi di dati
```csharp
...
## Riepilogo
C# è un linguaggio ben progettato ma presenta trappole: mancata eliminazione di oggetti `IDisposable`, metodi void asincroni, allocazioni LINQ non necessarie ed eccezioni di riferimento null. Il C# moderno (8+) fornisce strumenti potenti: tipi di riferimento nullable, corrispondenza di modelli, record, dichiarazioni using ed espressioni switch. I principi chiave: eliminare sempre le risorse, non utilizzare mai async void (eccetto i gestori di eventi), utilizzare l'iniezione del costruttore, preferire i record per i dati e abilitare i tipi di riferimento nullable per rilevare bug null in fase di compilazione.