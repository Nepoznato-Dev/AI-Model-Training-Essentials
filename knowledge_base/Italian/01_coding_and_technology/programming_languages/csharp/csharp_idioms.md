...
## IDisposable e gestione delle risorse
```csharp
...
## Riepilogo
Gli idiomi C# moderni enfatizzano: record per dati, corrispondenza di modelli, sicurezza null (tipi di riferimento nullable), LINQ per raccolte, async/await con CancellationToken, inserimento di dipendenze, costruttori primari, espressioni di raccolta e dichiarazioni `using`. Segui le convenzioni di codifica C#, utilizza`dotnet format`per la formattazione e gli analizzatori Roslyn per la qualità del codice. Il C# moderno (12+) è conciso ed espressivo: abbraccia record, corrispondenza di modelli e costruttori primari.