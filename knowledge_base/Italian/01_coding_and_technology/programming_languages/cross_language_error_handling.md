...
---
# Confronto tra lingue diverse: gestione degli errori
## Panoramica dei modelli di gestione degli errori
| Modello | Lingue | Meccanismo |
|-------|-----------|-----------|
| **Eccezioni (try/catch)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **Tipi di risultati/opzioni** | Rust, Haskell, OCaml, Scala (anche) | `Result<T, E>`,`Maybe a`|
| **Valori restituiti multipli** | Vai, Lua | `value, err := f()`|
| **Codici di errore** | C, Fortran | Restituisci il codice di errore, controlla manualmente |
| **Errori di corrispondenza del modello** | Elisir, Erlang | `{:ok, value}`/`{:error, reason}`|
| **Condizioni (riavviabile)** | Lisp comune | `restart-case`,`handler-bind`|
| **Nessuna gestione degli errori** | Scratch, SQL (base) | N/D |
| **Contratti** | Ada, Eiffel | `Pre`,`Post`condizioni |
...
## Sommario: paradigmi di gestione degli errori
| Paradigma | Avvicinamento | Pro | Contro |
|----------|----------|------|------|
| **Eccezioni** | lanciare/prendere | Pulisci il percorso felice, impila le tracce | Può essere dimenticato, controllare l'abuso del flusso |
| **Tipi di risultati** | Ritorna`Ok`/`Err`| Esplicito, non è possibile ignorare gli errori | Verboso, è necessario l'operatore`?`|
| **Codici di errore** | Restituisce lo stato int | Semplice, compatibile con C | Facile da ignorare, senza contesto |
| **Resi multipli** | `value, err := f()`| Esplicito, idiomatico | Puoi dimenticare di controllare`err`|
| **Corrispondenza modello** | `{:ok, v}`/`{:error, r}`| Esauriente, funzionale | Verboso senza`with`|
| **Condizioni** | Gestori riavviabili | Più potente, recuperabile | Complesso, raramente compreso |