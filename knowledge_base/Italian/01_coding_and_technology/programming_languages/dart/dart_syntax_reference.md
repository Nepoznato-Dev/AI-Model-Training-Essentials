...
---
# Dart: riferimento alla sintassi
Questo documento fornisce un riferimento completo e strutturato alla sintassi per Dart (3.x). Completa il riferimento principale di Dart concentrandosi su modelli di sintassi esaustivi, sicurezza nulla, programmazione asincrona e modelli di progettazione orientati a Flutter.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%``~/` | Aritmetica | `7 ~/ 3`| `~/`è la divisione intera |
| `==``!=` | Uguaglianza | `a == b`| |
| `<``>``<=``>=` | Confronto | `a >= b`| |
| `&&``\|\|``!`| Logico | `a && b`| Cortocircuito |
| `??`| Coalescenza nulla | `a ?? b`| `b`se`a`è nullo |
| `?.`| Accesso con riconoscimento dei valori Null | `a?.b`| null se`a`è null |
| `!`| Affermazione nulla | `a!`| Genera se null |
| `..``?..` | Cascata | `obj..a..b`| Operazioni a catena sullo stesso oggetto |
| `??=`| Assegnazione in grado di riconoscere null | `a ??= b`| Assegna se null |
| `as`| Digitare cast | `obj as String`| Lancia in caso di fallimento |
| `is``is!` | Prova di tipo | `obj is String`| |
| `=>`| Funzione freccia | `() => expr`| |
### Notazione a cascata
```dart
...
## Metodi e modelli di estensione
```dart
// Extension methods
...
## Riepilogo
La sintassi di Dart è pulita, coerente e progettata per la produttività. La sicura sicurezza nulla elimina gli errori di riferimento nulli in fase di compilazione. Async/await e i flussi forniscono modelli naturali per la programmazione asincrona. Classi, mixin e classi sigillate consentono la composizione flessibile degli oggetti. La corrispondenza dei modelli, i record e i modificatori di classe di Dart 3 aggiungono espressività. Sebbene lo scopo principale di Dart sia Flutter, la sua sintassi è adatta a qualsiasi dominio applicativo: lato server (con Dart Frog o Shelf), strumenti CLI e applicazioni Web (compilate in JavaScript).