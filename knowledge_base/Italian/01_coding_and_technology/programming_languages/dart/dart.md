...
#Dardo
Dart è un linguaggio di programmazione ottimizzato per il client sviluppato da Google, rilasciato per la prima volta nel 2013. Sebbene Dart fosse inizialmente posizionato come potenziale sostituto di JavaScript per i browser Web, ha trovato il suo scopo principale come linguaggio dietro **Flutter**: il toolkit dell'interfaccia utente multipiattaforma di Google per la creazione di applicazioni mobili, Web, desktop e integrate da un'unica base di codice.
Dart combina le migliori caratteristiche dei linguaggi moderni: è orientato agli oggetti, ha una digitazione opzionale (sicurezza nulla a partire da Dart 3), supporta la programmazione asincrona con`async`/`await`e si compila sia in codice macchina nativo (per dispositivi mobili/desktop) che JavaScript (per web).
---

## Perché le freccette sono importanti
- **Flutter**: il linguaggio principale di Flutter, uno dei framework multipiattaforma in più rapida crescita.
- **Multipiattaforma**: base di codice unica per iOS, Android, Web, Windows, macOS, Linux e dispositivi incorporati.
- **Produttivo**: ricarica rapida, ricca libreria di widget e sintassi espressiva velocizzano lo sviluppo dell'interfaccia utente.
- **Sicurezza nulla**: la sicurezza nulla in fase di compilazione elimina gli errori di riferimento nulli.
- **Prestazioni**: compila in codice ARM nativo per dispositivi mobili; non è necessario alcun ponte.
- **Ecosistema in crescita**: l'ecosistema dei pacchetti Flutter si sta espandendo rapidamente.
## I compromessi
...
### Sicurezza nulla: approfondimento
```dart
...
```
### Annotazioni e generazione di codice
```dart
...
### Test dei widget con flutter_test
```dart
...
## Interoperabilità
### FFI (interfaccia funzione estera)
```dart
...
### Interoperabilità JavaScript (Dart Web)
```dart
...
### Modello 1: modello di repository
```dart
...
### Modello 2: gestione dello stato del provider
```dart
...
### Modello 3: BLoC (componente della logica aziendale)
```dart
...
| Modalità | Quando utilizzato | Avvio | Durata | Caso d'uso |
|------|-----------|---------|---------|----------|
| **JIT** (Just-In-Time) | Sviluppo, modalità debug | Veloce | Leggermente più lento | Ricaricamento a caldo durante lo sviluppo |
| **AOT** (in anticipo) | Rilascia build | Leggermente più lento | Più veloce | Produzione di app mobili/desktop |
| **JS** | Obiettivi Web | Dipende | Varia | Applicazioni web svolazzanti |
| **WASM** | Web (sperimentale) | Dipende | Veloce | Obiettivo web futuro |
```dart
...
## Distribuzione
### Obiettivi di distribuzione Flutter
| Piattaforma | Comando Costruisci | Uscita |
|----------|--------------|--------|
| **Android** | `flutter build apk`/`flutter build appbundle`| APK/AAB per Play Store |
| **iOS** | `flutter build ipa`| IPA per App Store |
| **Web** | `flutter build web`| HTML/JS/CSS statico |
| **Finestre** | `flutter build windows`| MSIX o exe autonomo |
| **macOS** | `flutter build macos`| Pacchetto .app |
| **Linux** | `flutter build linux`| Binario + asset |
```bash
# Build commands
...
## Quando utilizzare Dart
| Scenario | Perché Dart (Flutter) | Alternativa migliore |
|----------|-------------|-------------|
...
## Riepilogo
Lo scopo di Dart nella vita è Flutter. Come linguaggio autonomo, è competente ma insignificante. Essendo il motore dietro Flutter, consente agli sviluppatori di creare applicazioni belle e ad alte prestazioni per tutte le principali piattaforme da un'unica base di codice. Se stai creando applicazioni mobili o desktop multipiattaforma, Dart + Flutter è una delle migliori opzioni disponibili. Per tutto il resto sono più adatte altre lingue.