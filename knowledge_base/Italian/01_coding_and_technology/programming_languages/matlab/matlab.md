...
## Perché MATLAB è importante
- **Operazioni su matrici**: il tipo di dati fondamentale è la matrice. L’algebra lineare è naturale ed efficiente.
- **Ecosistema toolbox**: centinaia di toolbox specializzati per l'elaborazione dei segnali, i sistemi di controllo, l'apprendimento automatico, le comunicazioni e altro ancora.
- **Simulink**: ambiente visivo con diagramma a blocchi per la modellazione e la simulazione di sistemi dinamici.
- **Standard di settore**: competenza richiesta in molti ruoli ingegneristici: aerospaziale, automobilistico, telecomunicazioni, difesa.
- **Prototipazione rapida**: sviluppo rapido e test degli algoritmi prima della distribuzione in C/C++ o sistemi embedded.
- **Formazione**: lo strumento didattico standard per metodi numerici, algebra lineare e corsi di ingegneria.
## I compromessi
...
| **Controllo della versione** |  I file`.m`sono testo ma Simulink`.mdl`/`.slx`sono binari | Utilizza gli strumenti di confronto integrati di MATLAB |
---
...
### Gestire classi e programmazione orientata agli oggetti
```matlab
...
### Funzioni dinamiche e maniglie di funzione
```matlab
% Anonymous functions
square = @(x) x.^2;
result = square(5);  % 25
...
### Metaprogrammazione con inputname e varargin
```matlab
...
### spmd (programma singolo, dati multipli)
```matlab
% Open parallel pool
...
## Test
### Quadro di test unitario MATLAB
```matlab
% File: test_solver.m
...
### Modello 1: vettorizzazione su cicli
```matlab
% BAD — loop-based
n = 1000000;
result = zeros(n, 1);
...
### Modello 3: modello della struttura di configurazione
```matlab
...
## Quando utilizzare MATLAB
| Scenario | Perché MATLAB | Alternativa migliore |
|----------|-----------|-------------|
| Prototipazione ingegneristica | Standard del settore; Integrazione Simulink | Python (NumPy/SciPy) per contesti non ingegneristici |
| Elaborazione di segnali/immagini | Cassette degli strumenti estese | Python (scipy.signal, OpenCV) |
| Progettazione di sistemi di controllo | Simulink non ha eguali | — |
| Algebra lineare | Sintassi della matrice naturale | Python (NumPy), Julia |
| Ricerca accademica | Standard in molti campi dell'ingegneria | Pitone, R |
...
## Riepilogo
MATLAB è lo strumento standard per il calcolo ingegneristico e la prototipazione scientifica. Il suo linguaggio orientato a matrice, i numerosi toolbox e l'ambiente Simulink lo rendono indispensabile in molte discipline ingegneristiche. Sebbene Python abbia invaso parte del territorio di MATLAB (soprattutto nella scienza dei dati), MATLAB rimane lo strumento preferito per i sistemi di controllo, l'elaborazione dei segnali e la formazione ingegneristica. Per la distribuzione in produzione, il codice viene generalmente tradotto da MATLAB a C/C++ o Python.
---

## Matrice avanzata e calcolo numerico
### Matrici sparse
```matlab
