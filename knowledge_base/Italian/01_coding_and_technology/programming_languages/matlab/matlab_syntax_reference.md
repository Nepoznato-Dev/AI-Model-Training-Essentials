...
|----------|------|---------|-------|
| `+``-``*``/``^`| Aritmetica | `A * B`| Moltiplicazione della matrice |
| `.*``./``.^`| Dal punto di vista degli elementi | `A .* B`| Operazione sugli array |
| `'`| Trasporre | `A'`| Trasposizione coniugata |
| `.'`| Trasposizione non coniugata | `A.'`| |
| `\`| Divisione sinistra | `A \ b`| Risolvi Ax = b |
| `/`| Dividi a destra | `b / A`| Risolvi xA = b |
| `==``~=` | Uguaglianza | `A == B`| Dal punto di vista degli elementi |
| `<``>``<=``>=` | Confronto | `A > 0`| Dal punto di vista degli elementi |
| `&``\|``~`| Logico (a livello di elementi) | `A & B`| |
| `&&``\|\|` | Cortocircuito | `a && b`| Solo scalare |
| `:`| Due punti/intervallo | `1:10`| `start:step:stop`|
| `;`| Separatore di riga/soppressione | `[1; 2; 3]`| |
| `,`| Separatore di colonne | `[1, 2, 3]`| |
---
...
## Riepilogo
La sintassi di MATLAB è matrice-prima: ogni variabile è un array e le operazioni sono ottimizzate per l'algebra lineare. L'operatore due punti, l'indicizzazione logica e le operazioni vettorizzate eliminano i loop per la maggior parte delle attività numeriche. Il sistema di plottaggio produce figure di qualità da pubblicazione con un codice minimo. I toolbox estendono MATLAB a ogni ambito dell'ingegneria. Sebbene Python abbia invaso alcuni territori, MATLAB rimane lo standard per il calcolo ingegneristico, la modellazione basata su Simulink e la prototipazione rapida.