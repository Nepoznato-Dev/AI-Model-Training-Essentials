...
# Vai: riferimento alla sintassi
Questo documento fornisce un riferimento completo e strutturato alla sintassi per Go. Completa il riferimento Go principale concentrandosi su modelli di sintassi esaustivi, tabelle degli operatori e meccanismi interni di goroutine, canali e interfacce.
---
...
| `/`| Divisione | `7 / 2`| `3`| Divisione intera per int |
| `%`| Resto | `7 % 2`| `1`| Il segno corrisponde al dividendo |
| `&`| Bit per bit AND | `5 & 3`| `1`| |
| `\|`| OR bit a bit | `5 \| 3`| `7`| |
| `^`| XOR bit a bit | `5 ^ 3`| `6`| Anche complemento bit a bit (unario) |
| `&^`| Un po' chiaro (E NON) | `5 &^ 3`| `4`| Unico da visitare |
| `<<`| Maiusc sinistro | `5 << 1`| `10`| |
| `>>`| Maiusc destro | `5 >> 1`| `2`| |
### Operatori di confronto
| Operatore | Nome | Esempio | Note |
|----------|------|---------|-------|
| `==`| Uguale | `x == y`| Solo tipi comparabili |
| `!=`| Non uguali | `x != y`| |
|  `<`, `>`, `<=`,`>=`| Ordinare | `x >= y`| Stringhe confrontate lessicograficamente |
### Operatori logici
| Operatore | Nome | Esempio | Note |
|----------|------|---------|-------|
...
## Riepilogo
La sintassi di Go è intenzionalmente minima: nessuna ereditarietà, nessun sovraccarico del metodo, nessuna eccezione, nessuna macro. Ciò che rimane è un insieme di costrutti pulito e ortogonale: strutture e interfacce per la composizione, goroutine e canali per la concorrenza e gestione esplicita degli errori per l'affidabilità. Il potere del linguaggio non deriva dalla sintassi complessa ma dall'elegante interazione dei suoi semplici elementi primitivi. I generici (Go 1.18+) aggiungono flessibilità al tipo senza sacrificare la chiarezza caratteristica di Go.