...
## 1. Acquisizione delle variabili in loop nelle goroutine
```go
...
## 2. Ignorare gli errori restituiti
```go
...
## 5. Dereferenza del puntatore Nil sui metodi struct
```go
...
## 6. Utilizzo di`defer`in un ciclo
```go
...
## 7. Anti-Pattern: restituzione prematura di variabili con nome
```go
...
## 8. L'ordine di iterazione della mappa è casuale
```go
...
## 10. Anti-Pattern: inquinamento dell'interfaccia
```go
...
## 11. Non utilizzare`sync.Once`per l'inizializzazione
```go
...
## Riepilogo
La semplicità di Go è la sua forza, ma presenta trappole sottili: perdite di goroutine da canali senza buffer, semantica di aggiunta di slice che non modifica l'originale, chiamate differite che si accumulano in loop e iterazione della mappa intenzionalmente randomizzata. I proverbi Go indicano la strada: gestire ogni errore, accettare interfacce ma restituire strutture, utilizzare ricevitori di puntatori per tipi mutex e preferire primitive di concorrenza (`sync.Once`,`errgroup`, cancellazione del contesto) rispetto alla gestione manuale della goroutine. Scrivi prima un codice semplice: Go premia la chiarezza rispetto all'intelligenza.