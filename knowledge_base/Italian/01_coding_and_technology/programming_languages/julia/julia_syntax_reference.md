...
|----------|------|---------|-------|
| `+``-``*``/``^`| Aritmetica | `2^10`| |
| `÷`| Divisione intera | `7 ÷ 3`| Uguale a`div(7, 3)`|
| `%``\` | Modulo/resto | `7 % 3`| |
| `==``!=` | Uguaglianza | `a == b`| Uguaglianza di valore |
| `===`| Identità | `a === b`| Stesso oggetto |
| `<``>``<=``>=` | Confronto | `a >= b`| Concatenabile:`1 < x < 10`|
| `&&``\|\|``!`| Logico | `a && b`| Cortocircuito |
| `&``\|``⊻``~` | Bit per bit | `a & b`| `⊻`è XOR |
| `:`| Gamma | `1:10`| `start:step:stop`|
| `..`| Intervallo | `1..10`| Richiede IntervalSet |
| `∈``∉``⊆`| Imposta appartenenza | `x ∈ S`| `in(x, S)`|
| `∘`| Composizione | `f ∘ g`| `(f ∘ g)(x) == f(g(x))`|
| `\|>`| Tubo | `x \|> f`| `f(x)`|
| `=>`| Coppia | `a => b`| Per ditti |
| `...`| Splat | `f(args...)`| Espandi collezione |
...
## Gestione degli errori e moduli
```julia
# try / catch / finally
try
...
## Riepilogo
La sintassi di Julia è pulita, matematica ed espressiva. L'invio multiplo consente alle funzioni di comportarsi diversamente in base a tutti i tipi di argomenti, consentendo un'elegante organizzazione del codice. Le macro forniscono una potente metaprogrammazione senza la complessità delle macro del preprocessore in stile C. Il sistema di tipi, con tipi parametrici, tipi astratti e tipi di unione, consente flessibilità e prestazioni. L'indicizzazione basata su 1, le comprensioni e la sintassi do-block di Julia lo rendono naturale per l'elaborazione matematica e scientifica. Il linguaggio mantiene la promessa "cammina come Python, esegui come C" attraverso la compilazione JIT con inferenza del tipo.