...
# Rust: modelli idiomatici e migliori pratiche
Questa guida copre i modelli idiomatici e le migliori pratiche per scrivere codice Rust pulito e sicuro.
---
...
## Riepilogo
Gli idiomi Rust enfatizzano: proprietà e prestito,`Result`/`Option`per la gestione degli errori, enumerazioni con metodi, tratti per l'astrazione, concatenamento di iteratori, puntatori intelligenti per lo stato condiviso e astrazioni a costo zero. Segui`rustfmt`per la formattazione,`clippy`per la lanugine e le linee guida dell'API Rust. La community di Rust valorizza la sicurezza, le prestazioni e la correttezza: "se si compila, funziona". Preferire`&str`rispetto a`String`negli argomenti delle funzioni, utilizzare`?`per la propagazione degli errori e ricavare tratti comuni con`#[derive(Debug, Clone, ...)]`.