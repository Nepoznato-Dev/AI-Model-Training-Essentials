...
## Perché la ruggine è importante
...
Questa è l'innovazione principale di Rust. Ogni valore ha esattamente un proprietario. Quando il proprietario esce dall'ambito, il valore viene eliminato.
```rust
...
### Strutture, enumerazioni e corrispondenza di modelli
```rust
struct Point {
    x: f64,
    y: f64,
...
### Macro
Rust ha due tipi di macro: dichiarativa (`macro_rules!`) e procedurale (deriva, attributo, simile a una funzione).
```rust
...
### Corrispondenza e destrutturazione di modelli avanzati
```rust
...
### Modello di thread e sincronizzazione
Il sistema di proprietà di Rust previene le corse dei dati in fase di compilazione. I tratti`Send`e`Sync`rafforzano la sicurezza del thread.
```rust
use std::thread;
...
### Discussioni con ambito (Rust 1.63+)
```rust
use std::thread;
...
### Modello Newtype (sicurezza del tipo)
```rust
struct Meters(f64);
struct Seconds(f64);
...
### Modello di repository con tratti
```rust
trait Repository<T> {
...
| Strumento | Scopo |
|------|---------|
| **carico** | Sistema di creazione, gestore pacchetti, test runner, generatore di documenti |
| **crates.io** | Registro dei pacchetti (oltre 150.000 casse) |
| **ruggine** | Formattatore di codice |
| **clippy** | Linter con centinaia di controlli utili |
| **Tokio** | Runtime asincrono (lo standard per Rust asincrono) |
| **serde** | Framework di serializzazione/deserializzazione |
| **actix-web/axum** | Framework Web |
| **diesel/sqlx** | ORM del database/costruttori di query |
---

## Quando usare Rust
| Scenario | Perché ruggine | Alternativa migliore |
|----------|---------|-------------|
| Programmazione dei sistemi | Sicurezza della memoria + prestazioni | C/C++ se non hai bisogno di garanzie di sicurezza |
| WebAssembly | Supporto WASM migliore della categoria | -- |
| Strumenti CLI | Veloce, binario singolo, ottima UX | Scegli CLI più semplici |
| Sistemi integrati | Nessun GC, accesso all'hardware, sicurezza | C per incorporato più semplice |
| Codice critico per le prestazioni | Corrisponde alla velocità C/C++ | -- |
| Infrastruttura cloud | Adozione in crescita (AWS, Cloudflare) | Scegli uno sviluppo più rapido |
| Sviluppo di applicazioni generali | La ripida curva di apprendimento rallenta lo sviluppo | Python, Go, Java |
...
## Riepilogo
Rust è un linguaggio che ti costringe a pensare alla memoria, alla proprietà e alla concorrenza e ti premia con un codice corretto per costruzione. La curva di apprendimento è reale, ma il profitto è significativo: programmi veloci come C ma esenti da bug di puntatori nulli, corse di dati e perdite di memoria. Rust non è un linguaggio di produttività generico: è un linguaggio di sistema per quando contano sia la correttezza che le prestazioni. La sua crescente adozione nell’industria (inclusi il kernel Linux e Android) suggerisce che sarà sempre più importante.