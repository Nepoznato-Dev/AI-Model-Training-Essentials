...
# Ruggine: errori comuni e anti-modelli
Questo documento cataloga gli errori, le trappole e gli anti-pattern più comuni in Rust. Ogni voce mostra l'approccio errato, spiega perché fallisce e fornisce la soluzione corretta. Il compilatore di Rust rileva molti errori, ma comprendere questi modelli accelererà la tua curva di apprendimento.
---

## 1. Combattere il controllo dei prestiti: clonazione non necessaria
```rust
...
## 2. Confusione tra stringhe e &str
```rust
...
## 3. Ignorare i risultati con`unwrap()`in produzione
```rust
...
## 4. Errori di annotazione a vita
```rust
...
## 5. Anti-Pattern: utilizzo di`Rc`/`RefCell`ovunque
```rust
...
## 6. Non comprendere la semantica dei movimenti
```rust
...
## 7. Anti-Pattern:`to_string()`/`to_owned()`eccessivi
```rust
...
## 9. Invalidazione dell'iteratore: raccolta inutilmente
```rust
...
## 10. Anti-Pattern: Enum senza metodi
```rust
...
## 11. Non utilizzare`#[derive]`per i tratti comuni
```rust
...
## 12. Codice non sicuro senza documentazione
```rust
...
## Riepilogo
Il compilatore di Rust è rigoroso ma corretto: la maggior parte degli errori insegna qualcosa sulla sicurezza della memoria. Gli errori più grandi non sono errori del compilatore ma anti-pattern: clonazione non necessaria per placare il controllo del prestito, utilizzo di`unwrap()`in produzione, ricerca di`Rc<RefCell<T>>`prima di provare la proprietà e allocazione di stringhe quando i riferimenti sono sufficienti. La mentalità Rustacea è: lascia che il compilatore ti guidi verso astrazioni a costo zero. Clona solo quando necessario, propaga gli errori con`?`, preferisci`&str`a`String`e documenta ogni blocco `unsafe`.