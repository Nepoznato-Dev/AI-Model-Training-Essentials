...
---
# Kotlin: errori comuni e anti-modelli
Questo documento cataloga gli errori, le trappole e gli anti-pattern più comuni in Kotlin. Ogni voce mostra l'approccio errato, spiega perché fallisce e fornisce la soluzione corretta.
---

## 1. Tipi di piattaforma e NullPointerException
```kotlin
...
## 3.`data class`Senza`equals`/`hashCode`corretti
```kotlin
...
## 4. Non utilizzare correttamente le funzioni dell'oscilloscopio
```kotlin
...
## 5. Coroutine: non gestire la cancellazione
```kotlin
...
## 6. Anti-Pattern: Kotlin in stile Java
```kotlin
...
## 7. Esaustività della Classe Sigillata
```kotlin
...
## 8. Funzione di estensione Shadowing
```kotlin
...
## 9. Non utilizzare`by lazy`per un'inizializzazione costosa
```kotlin
...
## 10. Confusione tra flusso e LiveData
```kotlin
...
## Riepilogo
La concisione di Kotlin può nascondere problemi sottili: tipi di piattaforma dall'interoperabilità Java, arresti anomali di `lateinit`, classi di dati modificabili che interrompono le raccolte e cancellazione della coroutine. Il modo in cui Kotlin è: abbracciare la sicurezza nulla (non utilizzare mai`!!`), utilizzare la funzione di ambito corretta (`let`per le trasformazioni,`apply`per la configurazione,`also`per gli effetti collaterali), utilizzare classi sigillate con`when`esaustivo, gestire la cancellazione della coroutine e scrivere Kotlin idiomatico (non Java con la sintassi Kotlin). Il compilatore è la tua guida: se avvisa, ascolta.