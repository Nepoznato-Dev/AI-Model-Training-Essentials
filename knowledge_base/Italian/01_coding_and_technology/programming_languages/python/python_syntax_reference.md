...
-->
# Python — Riferimento alla sintassi
Questo documento fornisce un riferimento completo e strutturato alla sintassi per Python. Completa il riferimento principale di Python concentrandosi su modelli di sintassi esaustivi, tabelle degli operatori e meccanica interna.
---
...
|----------|------|---------|--------|-------|
| `+`| Aggiunta | `3 + 2`| `5`| Anche la concatenazione di stringhe/liste |
| `-`| Sottrazione | `3 - 2`| `1`| Anche negazione unaria |
| `*`| Moltiplicazione | `3 * 2`| `6`| Anche ripetizione di stringhe/liste |
| `/`| Vera Divisione | `7 / 2`| `3.5`| Restituisce sempre float |
| `//`| Divisione Piano | `7 // 2`| `3`| Arrotonda verso l'infinito negativo |
| `%`| Modulo | `7 % 2`| `1`| Il segno corrisponde al divisore |
| `**`| Esponenziazione | `2 ** 10`| `1024`| Associativo destro |
### Confronto e operatori booleani
| Operatore | Nome | Esempio | Note |
|----------|------|---------|-------|
| `==`| Uguale | `x == y`| Confronto di valori (non identità) |
| `!=`| Non uguali | `x != y`| |
|  `<`, `>`, `<=`,`>=`| Ordinare | `x >= y`| Concatenabile:`a < b < c`|
| `is`| Identità | `x is y`| Stesso oggetto in memoria |
| `is not`| Non Identità | `x is not None`| Preferito rispetto a`!= None`|
| `in`| Appartenenza | `x in collection`| O(1) per insiemi/dettati, O(n) per liste |
| `and`| AND logico | `a and b`| Cortocircuito; restituisce l'ultimo valore vero/falso |
| `or`| OR logico | `a or b`| Cortocircuito; restituisce il primo valore vero/falso |
| `not`| NON logico | `not x`| |
### Operatori bit a bit
| Operatore | Nome | Esempio | Note |
|----------|------|---------|-------|
| `&`| E | `5 & 3`| `1`|
...
| 8| `^`| Sinistra |
| 9| `\|`| Sinistra |
| 10|  `==`, `!=`, `<`, `>`, `<=`, `>=`, `is`,`in`| Incatenabile |
| 11| `not`| Giusto |
| 12| `and`| Sinistra |
| 13 (più basso) | `or`| Sinistra |
### Assegnazione e assegnazione aumentata
```python
...
| Collezione | Ordinato | Mutevole | Duplicati | Sintassi | Ricerca |
|------------|---------|---------|------------|--------|--------|
| `list`| Sì | Sì | Sì | `[1, 2, 3]`| O(n) |
| `tuple`| Sì | No | Sì | `(1, 2, 3)`| O(n) |
| `dict`| Sì (3.7+) | Sì | Chiavi: No | `{"k": "v"}`| O(1) media |
| `set`| No | Sì | No | `{1, 2, 3}`| O(1) media |
| `frozenset`| No | No | No | `frozenset({1,2})`| O(1) media |
### Elenchi
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
## Riepilogo
Questo riferimento alla sintassi copre l'insieme completo di funzionalità di Python: dagli operatori di base e il flusso di controllo attraverso la metaprogrammazione avanzata, i descrittori e il sistema di tipi. La sintassi di Python è volutamente minima e leggibile, ma la sua profondità è sostanziale: corrispondenza di modelli, generatori asincroni, protocolli e metaclassi forniscono strumenti potenti per applicazioni complesse. Il linguaggio continua ad evolversi, con ogni versione che aggiunge funzionalità che migliorano l'espressività mantenendo la compatibilità con le versioni precedenti.