...
# Python: modelli idiomatici e migliori pratiche
Questa guida copre i modelli idiomatici e le migliori pratiche per scrivere codice Python pulito.
---
...
## Elenco comprensioni e generatori
```python
# ❌ Not idiomatic
squares = []
for x in range(10):
...
## Riepilogo
Il codice Python segue lo Zen di Python: leggibile, esplicito, semplice e piatto. Gli idiomi chiave includono: comprensioni list/dict/set, gestori di contesto (`with`), gestione degli errori EAFP,`enumerate`/`zip`per l'iterazione,`dataclass`per contenitori di dati, suggerimenti sul tipo, disimballaggio e controlli di verità/falsità. La comunità Python valorizza la leggibilità e "dovrebbe esserci un modo ovvio per farlo". Segui PEP 8 per lo stile, usa`ruff`per l'effetto lanugine e, quando possibile, preferisci la libreria standard rispetto a soluzioni di terze parti.