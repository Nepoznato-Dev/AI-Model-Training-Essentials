...
---
# Cross-Language Comparison — Error Handling

## Error Handling Models Overview
...
| **Result/Option types** | Rust, Haskell, OCaml, Scala (also) | `Result<T, E>`, `Maybe a` |
| **Multiple return values** | Go, Lua | `value, err := f()` |
| **Error codes** | C, Fortran | Return error code, check manually |
| **Pattern matching errors** | Elixir, Erlang | `{:ok, value}` / `{:error, reason}` |
| **Conditions (restartable)** | Common Lisp | `restart-case`, `handler-bind` |
| **No error handling** | Scratch, SQL (basic) | N/A |
| **Contracts** | Ada, Eiffel | `Pre`, `Post` conditions |
...
## Summary: Error Handling Paradigms

| Paradigm | Approach | Pros | Cons |
|----------|----------|------|------|
| **Exceptions** | throw/catch | Clean happy path, stack traces | Can be forgotten, control flow abuse |
| **Result types** | Return `Ok`/`Err` | Explicit, can't ignore errors | Verbose, `?` operator needed |
| **Error codes** | Return int status | Simple, C-compatible | Easy to ignore, no context |
| **Multiple returns** | `value, err := f()` | Explicit, idiomatic | Can forget to check `err` |
| **Pattern matching** | `{:ok, v}` / `{:error, r}` | Exhaustive, functional | Verbose without `with` |
| **Conditions** | Restartable handlers | Most powerful, recoverable | Complex, rarely understood |
