...
This document provides a comprehensive, structured syntax reference for Ruby (3.x). It complements the main Ruby reference by focusing on exhaustive syntax patterns, blocks and closures, metaprogramming, and Ruby idioms.
...
| `==` `!=` `===` `<=>` | Equality | `a == b` | `===` is case equality; `<=>` is spaceship |
| `eql?` | Value equality | `a.eql?(b)` | Stricter than `==` (checks type) |
| `equal?` | Identity | `a.equal?(b)` | Same object? |
| `<` `>` `<=` `>=` | Comparison | `a >= b` | |
| `&&` `\|\|` `!` | Logical | `a && b` | Short-circuit; returns last evaluated value |
| `and` `or` `not` | Logical (low precedence) | `a and b` | Avoid — use `&&`/`\|\|`/`!` |
| `&` `\|` `^` `~` `<<` `>>` | Bitwise | `a & b` | |
| `=~` `!~` | Regex match | `str =~ /pattern/` | Returns index or nil |
| `..` `...` | Range | `1..10` | `..` inclusive; `...` exclusive |
| `?.` | Safe navigation | `user&.name` | Returns nil if user is nil |
...
Ruby's syntax is designed for developer happiness — it reads like English and rewards expressiveness. The language's power comes from its object model (everything is an object), blocks and closures, metaprogramming capabilities, and the Enumerable module that provides a rich collection processing API. Ruby 3.x has added pattern matching, RBS type signatures, and Ractor for concurrency, while maintaining the language's core philosophy: make programmers productive and happy.
