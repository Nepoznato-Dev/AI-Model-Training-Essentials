...
This document catalogs the most common mistakes, traps, and anti-patterns in C. Each entry shows the incorrect approach, explains why it fails, and provides the correct solution. C gives you full control — which means full responsibility for memory safety, bounds checking, and undefined behavior.

---

## 1. Buffer Overflow with `gets()` / `strcpy()`
...
## 10. Not Understanding Pointer Arithmetic
...
C's dangers are well-known but ever-present: buffer overflows, dangling pointers, integer overflow, format string attacks, and memory leaks. The discipline is: always check return values, always free what you allocate, always use bounded functions (`snprintf`, `strncpy`, `fgets`), never use `gets()`, initialize all variables, and use tools like Valgrind and AddressSanitizer. C rewards careful programmers with unmatched performance and control — but punishes carelessness with undefined behavior and security vulnerabilities.
