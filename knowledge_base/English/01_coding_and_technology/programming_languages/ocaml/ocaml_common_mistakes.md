...
-->
# OCaml — Common Mistakes & Anti-Patterns

This document catalogs the most common mistakes, traps, and anti-patterns in OCaml with corrections.
...
## 5. Mutable State in Recursive Functions
...
OCaml traps: non-exhaustive pattern matching (compiler warns), `=` is structural equality while `==` is physical, integer division truncates, use `Option` instead of exceptions, and avoid mutable state in favor of tail recursion and folds. OCaml's compiler is your ally — heed its warnings.
