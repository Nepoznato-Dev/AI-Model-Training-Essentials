...
This document provides a comprehensive, structured syntax reference for Rust. It complements the main Rust reference by focusing on exhaustive syntax patterns, operator tables, and internal mechanics of the ownership system, traits, and concurrency.
...
|----------|------|---------|-------|
| `+` | Addition | `3 + 2` | Also `String` concatenation via `format!` |
| `-` | Subtraction | `3 - 2` | |
| `*` | Multiplication | `3 * 2` | |
| `/` | Division | `7 / 2` | Integer division for integers: `3` |
| `%` | Remainder | `7 % 2` | `1` |
| `-x` | Negation | `-5` | |
...
| Operator | Name | Example | Notes |
|----------|------|---------|-------|
| `==` | Equal | `x == y` | Requires `PartialEq` |
| `!=` | Not Equal | `x != y` | |
| `<`, `>`, `<=`, `>=` | Ordering | `x >= y` | Requires `PartialOrd` |
...
| `\|` | OR | `5 \| 3` | `7` |
| `^` | XOR | `5 ^ 3` | `6` |
| `!` | NOT | `!0b1010u8` | Bitwise complement |
| `<<` | Left Shift | `5 << 1` | `10` |
| `>>` | Right Shift | `5 >> 1` | `2` |
...
|------------|-----------|-------|
| 1 (highest) | `()` paths `.` :: | |
| 2 | Method calls, field access | Auto-deref |
| 3 | `-` (negation), `!`, `*` (deref) | |
| 4 | `as` (type cast) | |
| 5 | `*` `/` `%` | |
| 6 | `+` `-` | |
...
| 14 | `..` `..=` | Range |
| 15 | `=` `+=` etc. | Assignment |
| 16 (lowest) | `return` `break` `continue` | |
...
| Type | Stack/Heap | Size | Mutability | Notes |
|------|-----------|------|------------|-------|
| Primitives (`i32`, `f64`, `bool`, `char`) | Stack | Fixed | Immutable by default | `Copy` |
| `[T; N]` (array) | Stack | Fixed | Mutable with `mut` | Fixed-size, `Copy` if T is `Copy` |
| `Vec<T>` | Heap | Dynamic | Mutable with `mut` | Growable |
| `String` | Heap | Dynamic | Mutable with `mut` | UTF-8, growable |
| `&[T]` (slice) | Stack (fat ptr) | Dynamic view | Depends on `&` or `&mut` | Borrowed view |
| `HashMap<K, V>` | Heap | Dynamic | Mutable with `mut` | O(1) avg lookup |
| `BTreeMap<K, V>` | Heap | Dynamic | Mutable with `mut` | O(log n) lookup, ordered |
| `HashSet<T>` | Heap | Dynamic | — | O(1) avg, no duplicates |
...
Rust's syntax is built on a foundation of safety guarantees enforced at compile time. The ownership system, trait-based polymorphism, pattern matching, and zero-cost abstractions form a coherent design where the compiler is a strict but fair partner. From basic control flow through advanced features like macros, unsafe code, and smart pointers, every construct serves the goal of safe, efficient systems programming. Understanding the interplay between lifetimes, borrowing, and the type system is the key to mastering Rust.
