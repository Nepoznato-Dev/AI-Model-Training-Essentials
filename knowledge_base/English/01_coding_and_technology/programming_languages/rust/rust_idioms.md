...
-->
# Rust — Idiomatic Patterns & Best Practices

This guide covers idiomatic patterns and best practices for writing clean, safe Rust code.
...
Rust idioms emphasize: ownership and borrowing, `Result`/`Option` for error handling, enums with methods, traits for abstraction, iterator chaining, smart pointers for shared state, and zero-cost abstractions. Follow `rustfmt` for formatting, `clippy` for linting, and the Rust API Guidelines. The Rust community values safety, performance, and correctness — "if it compiles, it works." Prefer `&str` over `String` in function arguments, use `?` for error propagation, and derive common traits with `#[derive(Debug, Clone, ...)]`.
