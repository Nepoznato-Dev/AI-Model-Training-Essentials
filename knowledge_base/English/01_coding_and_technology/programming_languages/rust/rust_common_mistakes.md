...
## 1. Fighting the Borrow Checker — Unnecessary Cloning
...
## 3. Ignoring Results with `unwrap()` in Production
...
## 4. Lifetime Annotation Mistakes
...
## 5. Anti-Pattern: Using `Rc`/`RefCell` Everywhere
...
## 6. Not Understanding Move Semantics
...
## 7. Anti-Pattern: Excessive `to_string()` / `to_owned()`
...
## 9. Iterator Invalidation — Collecting Unnecessarily
...
## 11. Not Using `#[derive]` for Common Traits
...
## 12. Unsafe Code Without Documentation
...
Rust's compiler is strict but fair — most errors teach you something about memory safety. The biggest mistakes aren't compiler errors but anti-patterns: unnecessary cloning to appease the borrow checker, using `unwrap()` in production, reaching for `Rc<RefCell<T>>` before trying ownership, and allocating strings when references suffice. The Rustacean mindset is: let the compiler guide you toward zero-cost abstractions. Clone only when necessary, propagate errors with `?`, prefer `&str` over `String`, and document every `unsafe` block.
