...
This document provides a comprehensive, structured syntax reference for Lua (5.4). It complements the main Lua reference by focusing on exhaustive syntax patterns, tables and metatables, coroutines, and Lua's unique design philosophy.
...
| `//` | Floor division | `7 // 2` | Returns 3 (Lua 5.3+) |
| `==` `~=` | Equality | `a == b` | `~=` is "not equal" |
| `<` `>` `<=` `>=` | Comparison | `a >= b` | |
| `and` `or` `not` | Logical | `a and b` | Returns operands, not booleans |
| `..` | Concatenation | `"hello" .. " world"` | |
| `#` | Length | `#t` | Length of string or table |
...
Lua's syntax is deliberately minimal — the entire language can be described in a single page. Tables serve as the universal data structure (arrays, dictionaries, objects, modules). Metatables provide extensibility without language complexity. Coroutines enable cooperative multitasking and elegant iterator patterns. The language's power comes from its embeddability: Lua is designed to live inside host applications, providing scripting capabilities with minimal overhead. For game development, configuration, and embedded systems, Lua's simplicity is its greatest strength — a small language that does exactly what it needs to, and nothing more.
