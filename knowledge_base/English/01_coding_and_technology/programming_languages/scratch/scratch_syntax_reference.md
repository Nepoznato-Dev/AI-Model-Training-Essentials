...
This document provides a comprehensive, structured syntax reference for Scratch 3.0. It complements the main Scratch reference by focusing on exhaustive block categories, event-driven patterns, sprite management, cloning, and creative coding techniques.

---

## Block Categories Overview

| Category | Colour | Block Types | Purpose |
|----------|--------|-------------|---------|
| **Motion** | Blue | Stack | Move sprites, change position, direction |
| **Looks** | Purple | Stack | Change appearance, say/think, show/hide |
| **Sound** | Pink | Stack | Play sounds, change volume/pitch |
| **Events** | Yellow | Hat | Start scripts, broadcast/receive messages |
| **Control** | Gold | Stack | Loops, conditionals, wait, stop |
| **Sensing** | Light Blue | Reporter | Detect collisions, input, timer, answers |
| **Operators** | Green | Reporter | Math, text, comparison, logic |
| **Variables** | Orange | Stack/Reporter | Store data, list operations |
| **My Blocks** | Dark Red | Stack | Custom block definitions (functions) |
...
Scratch's block-based syntax eliminates typing errors and makes programming concepts tangible. Every script starts with a hat block (event), then stacks command blocks (motion, looks, sound, control, sensing, operators, variables) and uses reporter blocks (round/oval shapes) for values. Custom blocks enable modular programming. Clones provide object-like behavior. Lists store collections. The pen extension enables creative drawing. For young learners, Scratch provides a complete introduction to programming logic, event-driven design, and creative expression through code.
