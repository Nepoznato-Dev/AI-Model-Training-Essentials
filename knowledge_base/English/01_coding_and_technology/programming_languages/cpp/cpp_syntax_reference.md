...
|----------|------|---------|-------|
| `+` `-` `*` `/` `%` | Arithmetic | `a + b` | Same as C |
| `++` `--` | Increment/Decrement | `++i`, `i++` | Prefer pre-increment |
| `==` `!=` `<` `>` `<=` `>=` | Comparison | `a <=> b` | `<=>` is three-way comparison (C++20) |
| `&&` `\|\|` `!` | Logical | `a && b` | Short-circuit |
| `&` `\|` `^` `~` `<<` `>>` | Bitwise | `a & b` | Same as C |
| `?:` | Ternary | `cond ? a : b` | |
...
| `<=>` | Spaceship (C++20) | `a <=> b` | Returns `strong_ordering` |
...
### Structured Bindings & Modern Patterns
...
| Container | Header | Ordered | Lookup | Notes |
|-----------|--------|---------|--------|-------|
| `vector` | `<vector>` | Yes | O(n) | Default choice for sequences |
| `deque` | `<deque>` | Yes | O(n) | Fast push/pop at both ends |
| `list` | `<list>` | Yes | O(n) | Doubly linked list |
| `array` | `<array>` | Yes | O(n) | Fixed-size, stack-allocated |
| `set` / `multiset` | `<set>` | Yes | O(log n) | Red-black tree |
| `map` / `multimap` | `<map>` | Yes (by key) | O(log n) | Key-value, red-black tree |
| `unordered_set` | `<unordered_set>` | No | O(1) avg | Hash table |
| `unordered_map` | `<unordered_map>` | No | O(1) avg | Hash table |
...
C++ syntax spans from low-level pointer manipulation to high-level template metaprogramming and coroutine-based async. The language's evolution through C++17/20/23 has dramatically improved safety (smart pointers, `std::optional`, concepts), expressiveness (structured bindings, ranges, `std::format`), and performance (concepts enabling zero-cost abstractions, coroutines for async without stack overhead). The core philosophy remains unchanged: you should not pay for what you don't use, and the language should support every level of abstraction from bare metal to domain-specific languages.
