...
This document catalogs the most common mistakes, traps, and anti-patterns in C++. Each entry shows the incorrect approach, explains why it fails, and provides the correct solution. Modern C++ (C++11/14/17/20) provides tools to avoid most classic pitfalls.

---

## 1. Rule of Three/Five Violations
...
## 2. Raw `new`/`delete` Instead of Smart Pointers
...
## 6. Not Using `std::move` Correctly
...
## 7. Anti-Pattern: Using `using namespace std;` in Headers
...
## 8. Undefined Behavior from Signed Overflow
...
## 9. Not Understanding Value Categories
...
## 11. Template Bloat and Long Compile Times
...
## 12. Not Using `[[nodiscard]]` and `[[maybe_unused]]`
...
C++'s complexity creates many opportunities for mistakes: the Rule of Five, object slicing, iterator invalidation, dangling references, and undefined behavior from signed overflow. Modern C++ is the antidote: use smart pointers instead of raw `new`/`delete`, RAII for all resources, `std::vector` and `std::string` instead of raw arrays, move semantics for efficiency, and `[[nodiscard]]` to catch ignored return values. The C++ Core Guidelines exist for a reason — follow them, and use tools like Clang-Tidy and sanitizers to catch what you miss.
