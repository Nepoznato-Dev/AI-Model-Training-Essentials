...
---
# MATLAB — Common Mistakes & Anti-Patterns

This document catalogs the most common mistakes, traps, and anti-patterns in MATLAB with corrections.
...
## 4. Not Understanding `find` vs Logical Indexing
...
## 6. Not Using `fprintf` for Output
...
MATLAB's matrix-centric design creates traps: `*` is matrix multiplication (use `.*` for element-wise), growing arrays in loops is slow (pre-allocate or vectorize), `i`/`j` are imaginary units (don't overwrite), and logical indexing is faster than `find`. The MATLAB way is: vectorize everything, pre-allocate, use logical indexing, and never shadow built-in functions.
