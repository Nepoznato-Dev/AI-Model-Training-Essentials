...
|----------|------|---------|-------|
| `+` `-` `*` `/` `**` | Arithmetic | `A ** 2` | |
| `mod` `rem` | Modular/remainder | `A mod B` | `mod` always positive |
| `&` | Concatenation | `"hello" & " world"` | |
| `=` `/=` | Equality | `A = B` | |
| `<` `>` `<=` `>=` | Comparison | `A >= B` | |
| `and` `or` `not` `xor` | Logical | `A and B` | |
| `and then` `or else` | Short-circuit | `A and then B` | |
| `in` `not in` | Range membership | `X in 1 .. 10` | |
| `:=` | Assignment | `X := 10` | |
...
Ada's syntax is verbose but precise. Every construct is designed for clarity and safety. Protected objects eliminate data races at the language level. Contracts (pre/postconditions, invariants) make correctness requirements explicit and checkable. Generics provide type-safe reuse. For safety-critical systems, Ada's syntax is not a burden — it is a guarantee that the code does what it says.
