...
# Go — Common Mistakes & Anti-Patterns

This document catalogs the most common mistakes, traps, and anti-patterns in Go. Each entry shows the incorrect approach, explains why it fails, and provides the correct solution.
...
## 5. Nil Pointer Dereference on Struct Methods
...
## 7. Anti-Pattern: Returning Named Variables Prematurely
...
## 8. Map Iteration Order Is Random
...
## 11. Not Using `sync.Once` for Initialization
...
Go's simplicity is its strength, but it has subtle traps: goroutine leaks from unbuffered channels, slice append semantics that don't modify the original, deferred calls accumulating in loops, and map iteration that's intentionally randomized. The Go proverbs guide the way: handle every error, accept interfaces but return structs, use pointer receivers for mutex types, and prefer concurrency primitives (`sync.Once`, `errgroup`, context cancellation) over manual goroutine management. Write simple code first — Go rewards clarity over cleverness.
