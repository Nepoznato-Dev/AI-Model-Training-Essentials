...
This document provides a comprehensive, structured syntax reference for Go. It complements the main Go reference by focusing on exhaustive syntax patterns, operator tables, and internal mechanics of goroutines, channels, and interfaces.
...
| `/` | Division | `7 / 2` | `3` | Integer division for ints |
| `%` | Remainder | `7 % 2` | `1` | Sign matches dividend |
| `&` | Bitwise AND | `5 & 3` | `1` | |
| `\|` | Bitwise OR | `5 \| 3` | `7` | |
| `^` | Bitwise XOR | `5 ^ 3` | `6` | Also bitwise complement (unary) |
| `&^` | Bit clear (AND NOT) | `5 &^ 3` | `4` | Unique to Go |
| `<<` | Left Shift | `5 << 1` | `10` | |
| `>>` | Right Shift | `5 >> 1` | `2` | |
...
|----------|------|---------|-------|
| `==` | Equal | `x == y` | Comparable types only |
| `!=` | Not Equal | `x != y` | |
| `<`, `>`, `<=`, `>=` | Ordering | `x >= y` | Strings compared lexicographically |
...
Go's syntax is intentionally minimal — no inheritance, no method overloading, no exceptions, no macros. What remains is a clean, orthogonal set of constructs: structs and interfaces for composition, goroutines and channels for concurrency, and explicit error handling for reliability. The language's power comes not from complex syntax but from the elegant interaction of its simple primitives. Generics (Go 1.18+) add type flexibility without sacrificing Go's characteristic clarity.
