...
# Fortran — Version History & Evolution

## Timeline

| Version | Year | Key Theme |
|---------|------|-----------|
| Fortran I   | 1957 | **First high-level language** (John Backus, IBM) |
| Fortran II  | 1958 | Subroutines, functions |
| Fortran IV  | 1962 | `DATA`, `EQUIVALENCE`, `COMMON` |
| FORTRAN 66  | 1966 | **First ANSI standard** (X3.4-1966) |
...
| Fortran 95  | 1997 | `FORALL`, `WHERE`, pure/elemental procedures |
| Fortran 2003 | 2004 | **OOP**: classes, inheritance, polymorphism, `PROCEDURE` pointers, `IEEE` arithmetic |
| Fortran 2008 | 2010 | **Coarrays** (parallel programming), `SUBMODULE`, `DO CONCURRENT` |
| Fortran 2018 | 2018 | **Enhanced coarrays**, `ASSOCIATE`, `TYPE IS` improvements |
| Fortran 2023 | 2024 | **`BLOCK`**, `ALLOCATE` improvements, `SELECT RANK`, unsigned integers |

## Major Milestones

### Fortran I–IV: The Birth of High-Level Programming (1957–1965)
- **1957**: John Backus and team at IBM create Fortran (Formula Translation)
- **The first widely used high-level programming language**
- **Fortran I**: Introduced `DO` loops, `IF`, `GO TO`, arithmetic expressions
- **Fortran II (1958)**: Subroutines and functions (separate compilation)
- **Fortran IV (1962)**: `DATA`, `EQUIVALENCE`, `COMMON` blocks
- Fixed-form source: columns 1-6 for labels, 7-72 for code

### FORTRAN 66 & 77: Standardization (1966–1990)
- **FORTRAN 66**: First ANSI standard — portable Fortran
- **FORTRAN 77 (1977)**: The classic
  - Structured programming: `IF`/`THEN`/`ELSE`/`ENDIF`
  - `CHARACTER` type (string handling)
  - List-directed I/O (`*` format)
  - `PARAMETER` (named constants)
  - `ENTRY` (multiple entry points)
  - Still widely used in scientific computing

### Fortran 90: The Modern Revolution (1991)
- **Free-form source** — no more column restrictions
- **Modules** — encapsulation, `USE`
- **Dynamic arrays** — `ALLOCATABLE`, `ALLOCATE`
- **Array operations** — whole-array syntax `a = b + c`
- `SELECT CASE` — structured branching
- `IMPLICIT NONE` — require variable declarations
...
- **Fortran 95**: `FORALL`, `WHERE`, pure/elemental procedures
- **Fortran 2003**: **Full OOP**
  - Classes (derived types with type-bound procedures)
  - Inheritance (`EXTENDS`)
...
### Fortran 2008–2023: Parallel & Modern (2010–present)
- **Fortran 2008**: **Coarrays** — parallel programming built into the language
  - `DO CONCURRENT` — parallel loop construct
  - `SUBMODULE` — modular programming
  - `CONTIGUOUS` attribute
- **Fortran 2018**: Enhanced coarrays, `ASSOCIATE` improvements, teams
- **Fortran 2023**: `BLOCK` construct improvements, `ALLOCATE` enhancements, `SELECT RANK`, unsigned integers
