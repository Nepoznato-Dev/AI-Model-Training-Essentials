...
|----------|------|---------|-------|
| `+` `-` `*` `/` `%` `~/` | Arithmetic | `7 ~/ 3` | `~/` is integer division |
| `==` `!=` | Equality | `a == b` | |
| `<` `>` `<=` `>=` | Comparison | `a >= b` | |
| `&&` `\|\|` `!` | Logical | `a && b` | Short-circuit |
| `??` | Null coalescing | `a ?? b` | `b` if `a` is null |
| `?.` | Null-aware access | `a?.b` | null if `a` is null |
| `!` | Null assertion | `a!` | Throws if null |
| `..` `?..` | Cascade | `obj..a..b` | Chain operations on same object |
| `??=` | Null-aware assign | `a ??= b` | Assign if null |
| `as` | Type cast | `obj as String` | Throws on failure |
| `is` `is!` | Type test | `obj is String` | |
| `=>` | Arrow function | `() => expr` | |
...
Dart's syntax is clean, consistent, and designed for productivity. Sound null safety eliminates null reference errors at compile time. Async/await and streams provide natural patterns for asynchronous programming. Classes, mixins, and sealed classes enable flexible object composition. Dart 3's pattern matching, records, and class modifiers add expressiveness. While Dart's primary purpose is Flutter, its syntax is well-suited for any application domain — server-side (with Dart Frog or Shelf), CLI tools, and web applications (compiled to JavaScript).
