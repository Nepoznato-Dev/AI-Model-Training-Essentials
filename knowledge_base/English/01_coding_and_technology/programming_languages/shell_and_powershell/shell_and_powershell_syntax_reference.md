...
-->
# Shell & PowerShell — Syntax Reference

This document provides a comprehensive, structured syntax reference for Bash and PowerShell. It complements the main Shell reference by focusing on exhaustive scripting patterns, pipelines, process management, and automation idioms for both shells.

---

## Bash — Operators & Variables

### Core Operators

| Operator | Name | Example | Notes |
|----------|------|---------|-------|
| `=` | Assignment | `x=10` | No spaces around `=` |
| `$var` | Variable expansion | `echo $HOME` | |
| `${var}` | Braced expansion | `${HOME}/docs` | Disambiguates |
| `$(cmd)` | Command substitution | `$(date +%Y)` | Preferred over backticks |
| `$((expr))` | Arithmetic | `$((2 + 3))` | |
| `${var:-default}` | Default value | `${PORT:-8080}` | If unset or empty |
| `${var:=default}` | Assign default | `${count:=0}` | |
| `${#var}` | String length | `${#name}` | |
| `${var%%pattern}` | Remove longest suffix | `${file%%.*}` | Remove extension |
| `${var##pattern}` | Remove longest prefix | `${path##*/}` | Get filename |
| `"` `"` | Double quotes | `"$var"` | Allows expansion |
| `'` `'` | Single quotes | `'$var'` | Literal string |
...
## PowerShell — Functions & Advanced Patterns
...
Bash and PowerShell represent two paradigms of shell scripting. Bash pipes text — every command transforms strings. PowerShell pipes objects — every command produces structured data with properties and methods. Bash dominates Linux/macOS and DevOps. PowerShell is essential for Windows administration and increasingly cross-platform. Both are essential tools in a modern tech stack. Shell scripts are the glue that connects systems, automates workflows, and gets things done.
