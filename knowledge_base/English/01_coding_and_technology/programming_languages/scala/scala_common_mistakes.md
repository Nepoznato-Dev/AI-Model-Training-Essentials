...
-->
# Scala — Common Mistakes & Anti-Patterns

This document catalogs the most common mistakes, traps, and anti-patterns in Scala with corrections.

---

## 1. Using `var` Instead of `val`
...
## 2. Not Using Pattern Matching for Sealed Types
...
## 6. Not Handling `Option` Properly
...
Scala's power comes with responsibility: prefer `val` over `var`, use pattern matching for sealed types, handle `Option` properly (never call `.get`), use for-comprehensions instead of nested Futures, and avoid null. Scala 3 simplifies many of these patterns with enums, extension methods, and union types. The key principle: if the compiler can check it, let the compiler check it.
