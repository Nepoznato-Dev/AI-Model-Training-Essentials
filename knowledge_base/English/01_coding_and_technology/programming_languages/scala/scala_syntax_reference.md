...
This document provides a comprehensive, structured syntax reference for Scala (3.x with notes on 2.x differences). It complements the main Scala reference by focusing on exhaustive syntax patterns, the type system, functional programming constructs, and Scala idioms.
...
|----------|------|---------|-------|
| `+` `-` `*` `/` `%` | Arithmetic | `a + b` | |
| `**` | Power (BigDecimal) | `BigDecimal(2) ** 10` | Not built-in for Int |
| `==` `!=` | Equality | `a == b` | Calls `.equals()` — null-safe |
| `eq` `ne` | Reference equality | `a eq b` | Same instance |
| `<` `>` `<=` `>=` | Comparison | `a >= b` | Requires `Ordered` |
| `&&` `\|\|` `!` | Logical | `a && b` | Short-circuit |
| `&` `\|` `^` `~` | Bitwise | `a & b` | |
| `<<` `>>` `>>>` | Shift | `a << 2` | |
| `+:` `:+` `::` `:::` | Collection ops | `1 +: list` | Right-associative if ends with `:` |
| `->` | Tuple creation | `"key" -> "value"` | `("key", "value")` |
| `=>` | Function/arrow | `x => x + 1` | Lambda or type mapping |
| `<-` | Generator | `x <- List(1,2,3)` | For-comprehension |
...
Scala's syntax unifies object-oriented and functional programming under a powerful type system. Pattern matching, for-comprehensions, and case classes provide expressive data modeling. Traits enable flexible composition without single-inheritance limitations. The type system — with generics, variance, type bounds, and opaque types — catches errors at compile time while enabling zero-cost abstractions. Scala 3 simplifies the language with enums, extension methods, given/using, and opaque types, making the language more approachable while retaining its full power.
