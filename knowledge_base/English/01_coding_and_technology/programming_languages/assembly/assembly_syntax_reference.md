...
This document provides a comprehensive, structured syntax reference for x86-64 Assembly (NASM syntax). It complements the main Assembly reference by focusing on exhaustive instruction categories, addressing modes, stack frame conventions, and system programming patterns.
...
Assembly language syntax is defined by the processor architecture and the assembler (NASM, GAS, MASM). x86-64 provides 16 general-purpose registers, multiple addressing modes, and hundreds of instructions spanning data movement, arithmetic, logic, control flow, SIMD, and system calls. The NASM syntax used here is the most common for Linux x86-64 development. Mastering assembly requires understanding registers, the stack, calling conventions, and how high-level constructs map to machine instructions.
