...
| `==` `!=` `===` `!==` | Equality | `$a === $b` | `===` checks type too |
| `<=>` | Spaceship | `$a <=> $b` | Returns -1, 0, or 1 |
| `<` `>` `<=` `>=` | Comparison | `$a >= $b` | |
| `&&` `\|\|` `!` | Logical | `$a && $b` | Short-circuit |
| `and` `or` `xor` | Logical (low precedence) | `$a and $b` | Avoid — use `&&`/`\|\|` |
| `??` | Null coalescing | `$a ?? 'default'` | Returns `'default'` if `$a` is null |
| `??=` | Null coalescing assign | `$a ??= 'default'` | Assigns only if `$a` is null |
| `?` `:` | Ternary | `$a ? $b : $c` | |
| `.` | Concatenation | `$a . $b` | |
| `.=` | Concatenating assign | `$a .= $b` | |
| `=>` | Array assignment | `['key' => 'value']` | |
| `->` | Member access | `$obj->method()` | |
| `?->` | Nullsafe operator | `$obj?->method()` | Returns null if `$obj` is null |
| `::` | Static access | `ClassName::method()` | |
| `instanceof` | Type check | `$obj instanceof ClassName` | |
| `clone` | Object cloning | `$copy = clone $obj` | Deep copy requires `__clone()` |
...
PHP's syntax has evolved dramatically from its scripting origins into a modern, type-safe language. PHP 8.x introduced union types, intersection types, enums, fibers, readonly classes, attributes, and first-class callables — transforming it from a loosely typed scripting language into one that supports rigorous software engineering. The language's strength remains its web focus: with frameworks like Laravel and Symfony, a massive ecosystem, and near-universal hosting support, PHP's syntax serves millions of applications. Modern PHP rewards developers who embrace its type system, use composer-based autoloading, and write clean, object-oriented code.
