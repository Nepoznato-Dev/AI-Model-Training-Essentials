...
This document provides a comprehensive, structured syntax reference for Haskell (GHC 9.x). It complements the main Haskell reference by focusing on exhaustive syntax patterns, the type system, monads, lazy evaluation, and functional programming idioms.
...
| `div` `mod` | Integer division | `7 \`div\` 2` | Returns 3 |
| `^` `**` | Power | `2 ^ 10` | `^` for integral; `**` for floating |
| `==` `/=` | Equality | `x == y` | Eq typeclass |
| `<` `>` `<=` `>=` | Comparison | `x >= y` | Ord typeclass |
| `&&` `\|\|` `not` | Logical | `a && b` | Short-circuit |
| `.` | Function composition | `(f . g) x` | `f (g x)` |
| `$` | Function application | `f $ x + 1` | `f (x + 1)` — low precedence |
| `<>` | Monoid append | `"hello" <> " world"` | Semigroup |
...
| `=<<` | Bind (reversed) | `f =<< mx` | Same as `mx >>= f` |
| `++` | List concatenation | `[1,2] ++ [3,4]` | O(n) in left list |
| `:` | Cons | `1 : [2, 3]` | O(1) |
| `!!` | Index | `[1,2,3] !! 1` | O(n) — partial function |
| `\\` | List difference | `[1,2,3] \\ [2]` | `[1,3]` |
...
Haskell's syntax is minimal yet extraordinarily expressive. The type system — with algebraic data types, type classes, GADTs, and type families — serves as both a specification and a verification tool. Pattern matching, lazy evaluation, and monads provide elegant solutions to complex problems. Haskell's purity (no side effects in types) makes code easier to reason about, test, and parallelize. While the learning curve is steep, Haskell's influence permeates modern languages — from Swift's optionals to Kotlin's sealed classes to Rust's type system. Mastering Haskell makes you a better programmer in every language.
