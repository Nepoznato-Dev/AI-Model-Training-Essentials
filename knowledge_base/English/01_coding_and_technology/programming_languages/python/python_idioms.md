...
---
# Python — Idiomatic Patterns & Best Practices

This guide covers idiomatic patterns and best practices for writing clean, Pythonic code.
...
Pythonic code follows the Zen of Python: readable, explicit, simple, and flat. Key idioms include: list/dict/set comprehensions, context managers (`with`), EAFP error handling, `enumerate`/`zip` for iteration, `dataclass` for data containers, type hints, unpacking, and truthy/falsy checks. The Python community values readability and "there should be one obvious way to do it." Follow PEP 8 for style, use `ruff` for linting, and prefer the standard library over third-party solutions when possible.
