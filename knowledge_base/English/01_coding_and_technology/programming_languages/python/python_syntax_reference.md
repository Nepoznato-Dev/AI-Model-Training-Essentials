...
This document provides a comprehensive, structured syntax reference for Python. It complements the main Python reference by focusing on exhaustive syntax patterns, operator tables, and internal mechanics.
...
|----------|------|---------|-------|
| `==` | Equal | `x == y` | Value comparison (not identity) |
| `!=` | Not Equal | `x != y` | |
| `<`, `>`, `<=`, `>=` | Ordering | `x >= y` | Chainable: `a < b < c` |
| `is` | Identity | `x is y` | Same object in memory |
| `is not` | Not Identity | `x is not None` | Preferred over `!= None` |
| `in` | Membership | `x in collection` | O(1) for sets/dicts, O(n) for lists |
| `and` | Logical AND | `a and b` | Short-circuit; returns last truthy/falsy value |
| `or` | Logical OR | `a or b` | Short-circuit; returns first truthy/falsy value |
...
| Collection | Ordered | Mutable | Duplicates | Syntax | Lookup |
|------------|---------|---------|------------|--------|--------|
| `list` | Yes | Yes | Yes | `[1, 2, 3]` | O(n) |
| `tuple` | Yes | No | Yes | `(1, 2, 3)` | O(n) |
| `dict` | Yes (3.7+) | Yes | Keys: No | `{"k": "v"}` | O(1) avg |
| `set` | No | Yes | No | `{1, 2, 3}` | O(1) avg |
| `frozenset` | No | No | No | `frozenset({1,2})` | O(1) avg |
