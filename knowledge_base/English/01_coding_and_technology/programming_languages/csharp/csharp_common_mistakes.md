...
This document catalogs the most common mistakes, traps, and anti-patterns in C#. Each entry shows the incorrect approach, explains why it fails, and provides the correct solution.
...
## 4. LINQ `.ToList()` Unnecessary Allocation
...
## 5. Capturing Loop Variable (Pre-C# 5.0)
...
## 10. Not Using Records for Data Types
...
C# is a well-designed language but has traps: not disposing `IDisposable` objects, async void methods, unnecessary LINQ allocations, and null reference exceptions. Modern C# (8+) provides powerful tools: nullable reference types, pattern matching, records, using declarations, and switch expressions. The key principles: always dispose resources, never use async void (except event handlers), use constructor injection, prefer records for data, and enable nullable reference types to catch null bugs at compile time.
