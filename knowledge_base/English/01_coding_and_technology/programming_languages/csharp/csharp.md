...
### Q1: What is the difference between `class` and `record` in C#?
**A:** A `class` is a reference type with mutable properties by default — two variables can reference the same object. A `record` (C# 9+) is a reference type with value-based equality — two records with the same data are considered equal. Records have init-only properties, a built-in `ToString`, and support `with` expressions for non-destructive mutation. Use records for data carriers (DTOs, value objects); use classes for behavior-rich entities with identity.
...
### Q2: How do async/await and `Task` work internally?
**A:** `async/await` is syntactic sugar over a state machine generated by the compiler. When you `await` a `Task`, the method is split at the await point: everything before is executed synchronously, then the remainder is registered as a continuation. The thread is freed to do other work. `Task<T>` represents a future value. `ValueTask<T>` is a struct alternative for hot paths that avoids heap allocation when the result is already available.
...
### Q3: What are extension methods, and when should I use them?
**A:** Extension methods add methods to existing types without modifying them. They are static methods in a static class, with the `this` keyword on the first parameter. They enable a fluent, chainable API. Use them to add utility methods to types you don't own (like `string` or `IEnumerable<T>`). Avoid overusing them — they can make code hard to discover.
...
### Q4: How does pattern matching work in modern C#?
**A:** C# has progressively added more powerful pattern matching. Switch expressions (C# 8), type patterns, property patterns, relational patterns, and list patterns (C# 11) allow concise, expressive conditional logic. Pattern matching replaces long if/else chains and is exhaustive-checked by the compiler.
...
### Q5: What is dependency injection in .NET, and how do I use it?
**A:** .NET has built-in DI support via `Microsoft.Extensions.DependencyInjection`. You register services with their lifetimes (Singleton, Scoped, Transient), and the container injects them via constructor parameters. Singleton: one instance for the app. Scoped: one per HTTP request. Transient: new instance every time.
...
### Problem 1: Build a Generic Repository with Caching

**Problem Statement:** Implement a generic repository pattern with a decorator that adds caching. The repository should support CRUD operations, and the caching decorator should cache reads and invalidate on writes.

**Step 1 — Understand the Problem:**
We need: (1) a generic `IRepository<T>` interface, (2) a concrete implementation (e.g., in-memory), (3) a caching decorator that wraps any repository, (4) cache invalidation on write operations. The decorator pattern keeps caching orthogonal to the data access logic.

**Step 2 — Identify the Approach:**
- Define `IRepository<T>` with `Get`, `GetAll`, `Add`, `Update`, `Delete`.
- Create `CachingRepository<T>` that wraps `IRepository<T>` and uses `IMemoryCache`.
- Cache key: `typeof(T).Name:{id}`.
- On write operations, invalidate the cache entry.
...
- Separation of concerns: caching is a decorator, not mixed into the repository.
- DI registration: `services.Decorate<IRepository<User>, CachingRepository<User>>()` (using Scrutor).
- Production: use `IDistributedCache` (Redis) for multi-server scenarios, and add cache-aside patterns with `CacheStampede` protection.

### Problem 2: Implement a Middleware Pipeline

**Problem Statement:** Build a middleware pipeline similar to ASP.NET Core's request pipeline. Each middleware can process the request, call the next middleware, and process the response.

**Step 1 — Understand the Problem:**
We need: (1) a `RequestDelegate` type representing the pipeline, (2) middleware that wraps the next delegate, (3) a builder API for composing middleware. This is the Chain of Responsibility pattern implemented with delegates.

**Step 2 — Identify the Approach:**
- `RequestDelegate` is `Func<Context, RequestDelegate, Task>`.
- Each middleware receives the context and a `next` function.
- `Use` adds middleware; `Build` composes them into a single delegate.
...
**Step 4 — Verify and Optimize:**
- Middleware order matters: first added = outermost (executed first on request, last on response).
- Terminal middleware (no `next` call) short-circuits the pipeline.
- Production: ASP.NET Core's pipeline is exactly this pattern, optimized with compiled expression trees for zero allocation.
