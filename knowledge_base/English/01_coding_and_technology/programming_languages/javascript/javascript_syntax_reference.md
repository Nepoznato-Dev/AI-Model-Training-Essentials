...
This document provides a comprehensive, structured syntax reference for JavaScript. It complements the main JavaScript reference by focusing on exhaustive syntax patterns, operator tables, and internal mechanics.
...
| `+x`, `-x` | Unary | `+"42"` → `42` | Type coercion to number |

### Comparison & Equality

| Operator | Name | Example | Notes |
|----------|------|---------|-------|
| `===` | Strict Equality | `1 === 1` | No type coercion — always prefer this |
| `!==` | Strict Inequality | `1 !== "1"` | `true` — different types |
| `==` | Abstract Equality | `1 == "1"` | Type coercion — avoid |
| `!=` | Abstract Inequality | `1 != "1"` | `false` — avoid |
| `<`, `>`, `<=`, `>=` | Relational | `"b" > "a"` | Lexicographic for strings |

### Logical & Nullish Operators

| Operator | Name | Example | Notes |
|----------|------|---------|-------|
| `&&` | Logical AND | `a && b` | Returns first falsy or last value |
| `\|\|` | Logical OR | `a \|\| b` | Returns first truthy or last value |
| `!` | Logical NOT | `!true` | `false` |
| `??` | Nullish Coalescing | `a ?? b` | Returns `b` only if `a` is `null`/`undefined` |
| `?.` | Optional Chaining | `obj?.prop?.method()` | Short-circuits to `undefined` |
...
|------------|-----------|---------------|
| 1 (highest) | `()` `[]` `.` `?.` | Left |
| 2 | `new` (without args) | Right |
| 3 | `++` `--` (postfix) | N/A |
| 4 | `!` `~` `+x` `-x` `typeof` `void` `delete` `++` `--` (prefix) | Right |
| 5 | `**` | Right |
| 6 | `*` `/` `%` | Left |
| 7 | `+` `-` | Left |
| 8 | `<<` `>>` `>>>` | Left |
| 9 | `<` `<=` `>` `>=` `in` `instanceof` | Left |
| 10 | `===` `!==` `==` `!=` | Left |
| 11 | `&` | Left |
| 12 | `^` | Left |
| 13 | `\|` | Left |
...
| 16 | `??` | Left |
| 17 | `=` `+=` `-=` etc. | Right |
| 18 (lowest) | `=>` (arrow function) | Right |
...
### Built-in Collections Overview

| Collection | Ordered | Mutable | Duplicates | Key Type | Lookup |
|------------|---------|---------|------------|----------|--------|
| `Array` | Yes | Yes | Yes | Index (number) | O(n) |
| `Object` | Yes (insertion) | Yes | Keys: No | String/Symbol | O(1) avg |
| `Map` | Yes (insertion) | Yes | Keys: No | Any | O(1) avg |
| `Set` | Yes (insertion) | Yes | No | — | O(1) avg |
| `WeakMap` | No | Yes | Keys: No | Object only | O(1) avg |
| `WeakSet` | No | Yes | No | Object only | O(1) avg |
...
JavaScript's syntax is deceptively simple on the surface but contains significant depth. The language has evolved from a simple scripting tool into a multi-paradigm language with classes, modules, generators, proxies, and a sophisticated async model. Understanding operator precedence, the event loop, prototype chains, and the nuances of `this` binding separates competent developers from expert ones. Modern JavaScript (ES2020+) with optional chaining, nullish coalescing, `Promise.allSettled`, and top-level await continues to reduce boilerplate while maintaining backward compatibility.
