...
-->
# JavaScript — Common Mistakes & Anti-Patterns

This document catalogs the most common mistakes, traps, and anti-patterns in JavaScript. Each entry shows the incorrect approach, explains why it fails, and provides the correct solution.
...
## 3. `var` Hoisting and Scope Issues
...
## 6. Modifying Objects During Iteration
...
## 8. Misunderstanding `typeof null`
...
## 9. Array-Like Objects Are Not Arrays
...
## 11. Not Understanding Event Loop and Microtasks
...
## 13. Forgetting to Clean Up Event Listeners
...
JavaScript's flexibility comes with traps: `this` binding depends on call site, `==` performs coercion, promises require error handling, and the event loop ordering (microtasks before macrotasks) is non-obvious. The modern rules are simple: use `const`/`let` never `var`, prefer `===` over `==`, always handle promise rejections, use arrow functions for callbacks, avoid mutation, and clean up event listeners. Following these rules eliminates the majority of JavaScript bugs.
