...
This document provides a comprehensive, structured syntax reference for TypeScript. It complements the main TypeScript reference by focusing on the type system, generics, utility types, and advanced type-level programming.
...
TypeScript's type system is its defining feature, and it is notably deep. From basic annotations through generics, conditional types, mapped types, template literal types, and `infer`, the type system is essentially a functional language operating at compile time. Understanding the interplay between `interface` vs `type`, generics vs utility types, and structural vs nominal typing supports writing TypeScript that is both safe and ergonomic. The language continues to evolve, with each release adding more expressive power while maintaining full backward compatibility with JavaScript.
