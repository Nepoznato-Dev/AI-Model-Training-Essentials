...
-->
# TypeScript — Common Mistakes & Anti-Patterns

This document catalogs the most common mistakes, traps, and anti-patterns in TypeScript. Each entry shows the incorrect approach, explains why it fails, and provides the correct solution.
...
## 4. Interfaces vs Types Confusion
...
## 9. Anti-Pattern: Repeating Types (Not Using Generics)
...
## 10. Excessive Optional Chaining
...
## 12. Not Leveraging Discriminated Unions
...
TypeScript's type system is powerful but requires discipline. The cardinal sin is reaching for `any` — use `unknown` when the type is truly unknown, and narrow with type guards. Avoid non-null assertions (`!`) by handling undefined properly. Use interfaces for object shapes, types for unions and computed types. Discriminated unions eliminate entire classes of bugs. Enable `strict: true` from day one — retrofitting strictness is painful. The goal is to make illegal states unrepresentable at the type level.
