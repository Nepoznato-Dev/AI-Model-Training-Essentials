...
-->
# TypeScript — Idiomatic Patterns & Best Practices

This guide covers idiomatic patterns and best practices for writing clean, type-safe TypeScript code.
...
TypeScript idioms emphasize: strict type checking, interfaces for object shapes, discriminated unions for state, type guards for narrowing, generics with constraints, `readonly` for immutability, and `as const` for literal types. Follow ESLint with `typescript-eslint`, Prettier for formatting, and always enable `strict: true`. The TypeScript community values type safety — if it compiles, it probably works. Prefer `interface` for object shapes, `type` for unions and utilities, and avoid `any` at all costs.
