...
This document provides a comprehensive, structured syntax reference for Kotlin (2.0+). It complements the main Kotlin reference by focusing on exhaustive syntax patterns, null safety, coroutines, and Kotlin idioms.
...
| `==` `!=` | Structural equality | `a == b` | Calls `.equals()` |
| `===` `!==` | Referential equality | `a === b` | Same instance |
| `<` `>` `<=` `>=` | Comparison | `a >= b` | Requires `Comparable` |
| `&&` `\|\|` `!` | Logical | `a && b` | Short-circuit |
| `?:` | Elvis operator | `a ?: b` | Returns `b` if `a` is null |
| `?.` | Safe call | `a?.b` | Returns null if `a` is null |
| `!!` | Non-null assertion | `a!!` | Throws NPE if null |
| `as` | Cast | `a as String` | Throws ClassCastException |
| `as?` | Safe cast | `a as? String` | Returns null on failure |
| `in` | Membership | `x in list` | |
| `!in` | Not member | `x !in list` | |
| `is` | Type check | `x is String` | Smart cast after check |
| `!is` | Not type | `x !is String` | |
...
Kotlin's syntax is concise, safe, and interoperable with Java. Null safety eliminates an entire class of bugs at compile time. Coroutines provide a natural way to write asynchronous code. Sealed classes, data classes, and extension functions enable expressive domain modeling. The language's power comes from the interplay of its features — scope functions, DSLs, inline classes, and reified generics combine to produce code that is both safe and enjoyable to write.
