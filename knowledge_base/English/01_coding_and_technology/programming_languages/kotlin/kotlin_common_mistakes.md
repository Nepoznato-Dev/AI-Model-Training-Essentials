...
---
# Kotlin — Common Mistakes & Anti-Patterns

This document catalogs the most common mistakes, traps, and anti-patterns in Kotlin. Each entry shows the incorrect approach, explains why it fails, and provides the correct solution.

---

## 1. Platform Types and NullPointerException
...
## 3. `data class` Without Proper `equals`/`hashCode`
...
## 4. Not Using Scope Functions Correctly
...
## 5. Coroutines: Not Handling Cancellation
...
## 9. Not Using `by lazy` for Expensive Initialization
...
Kotlin's conciseness can hide subtle issues: platform types from Java interop, `lateinit` crashes, mutable data classes breaking collections, and coroutine cancellation. The Kotlin way is: embrace null safety (never use `!!`), use the right scope function (`let` for transformations, `apply` for configuration, `also` for side effects), use sealed classes with exhaustive `when`, handle coroutine cancellation, and write idiomatic Kotlin (not Java with Kotlin syntax). The compiler is your guide — if it warns, listen.
