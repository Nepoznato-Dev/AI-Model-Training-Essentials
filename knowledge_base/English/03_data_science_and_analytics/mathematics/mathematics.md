...
Rates of change. The derivative f'(x) tells you how fast f is changing at any point.

| Function f(x) | Derivative f'(x) | Intuition |
|---|---|---|
| xⁿ | n·xⁿ⁻¹ | Power rule |
| eˣ | eˣ | The only function equal to its own derivative |
...
|------|---------|----------|
| **Chain Rule** | (f∘g)' = f'(g(x)) · g'(x) | Nested functions — backpropagation in neural networks |
| **Product Rule** | (fg)' = f'g + fg' | Multiplying two functions of x |
| **Quotient Rule** | (f/g)' = (f'g − fg') / g² | Dividing two functions of x |

### Integral Calculus

Accumulation. The integral represents the area under a curve. If derivatives answer "how fast is it changing?", integrals answer "how much has accumulated?"

The **fundamental theorem of calculus** connects both: differentiation and integration are inverse operations.

| Integral | Result | Use Case |
|----------|--------|----------|
| ∫ xⁿ dx | xⁿ⁺¹/(n+1) + C | Area under polynomial curves |
| ∫ eˣ dx | eˣ + C | Total accumulated growth |
...
|-----------------|---------|----------|
| **Addition** | a + b = [a₁+b₁, a₂+b₂, ...] | Combining feature vectors |
| **Scalar multiplication** | c·a = [c·a₁, c·a₂, ...] | Scaling features |
| **Dot product** | a·b = Σ aᵢbᵢ | Similarity, projections |
| **Norm (magnitude)** | ||a|| = √(Σ aᵢ²) | Vector length |
| **Cross product** | a × b (3D only) | Perpendicular vector, area |
...
| Transpose | Flips rows and columns | Gradient computation |
...
Not all of these topics are needed immediately. However, as one goes deeper in any technical field, these foundations become increasingly relevant. Each branch becomes clearer once the problem it was designed to solve is understood.
