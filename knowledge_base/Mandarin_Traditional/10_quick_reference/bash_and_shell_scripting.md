...
|語法 |意義|
|--------|---------|
|`cmd > file`|將標準輸出重定向到檔案（覆蓋） |
|`cmd >> file`|將標準輸出重定向到檔案（追加） |
|`cmd 2> errors.log`|重定向標準錯誤 |
|`cmd &> all.log`|重定向 stdout 和 stderr |
|`cmd1 \| cmd2`|將 cmd1 的 stdout 透過管道傳輸到 cmd2 的 stdin |
|`cmd1 \|& cmd2`|透過管道傳輸 stdout 和 stderr |
|`cmd < file`|將檔案重定向到標準輸入 |
|`cmd <<EOF ... EOF`| Here-document（多行輸入）|
|`cmd <<< "string"`| Here-string（單行輸入）|
```bash
# Here-document
cat <<EOF
Name: $NAME
...
＃＃ 概括
Bash 腳本對於任何使用電腦的人來說都是一個力量倍增器。您不需要記住每個標誌 - 您需要知道什麼是可能的以及在哪裡找到它。從基礎知識開始：變數、條件、循環、管道。然後根據需要使用文字處理工具（grep、sed、awk）。並且始終使用`set -euo pipefail`— 將來您會感激不已。