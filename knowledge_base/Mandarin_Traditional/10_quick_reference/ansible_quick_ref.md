...
# Ansible 與設定管理
Ansible 是一種無代理程式設定管理和自動化工具。它使用 SSH（或 Windows 的 WinRM）連接到伺服器並執行 YAML playbook 中定義的任務。與需要在每台機器上安裝代理的工具不同，Ansible 是基於推送的——您從控制節點運行命令。它用於伺服器配置、應用程式部署、組態管理和臨時任務執行。
---
...
＃＃ 概括
Ansible 透過透過 SSH 執行的 YAML playbook 自動執行伺服器設定和應用程式部署。工作流程是：定義清單→寫劇本→執行`ansible-playbook`。關鍵概念包括模組（工作單元）、角色（可重複使用集合）、處理程序（觸發的任務）和變數（動態值）。常用模組涵蓋套件管理、檔案操作、服務控制、使用者管理等。申請前請務必使用檢查模式；將機密儲存在 Ansible Vault 中；確保任務是冪等的；並在生產環境中運行之前進行本地測試。