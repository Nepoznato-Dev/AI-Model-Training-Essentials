...
- **免費和開源**：無許可費用；由全球社區積極維護。
## 權衡
|限制|詳情 |典型解決方法|
|------------|---------|--------------------|
| **效能** |預設單執行緒；對於大型資料集來說速度很慢使用`data.table`、並行包或 Rcpp 進行 C++ 整合 |
| **記憶體使用情況** |將整個資料集載入到 RAM 中 |使用`data.table::fread`，箭頭包進行核外處理 |
| **不是通用語言** |對於 Web 開發、系統程式設計或應用程式來說很尷尬 |使用 Python、Go 或 JavaScript 執行非統計任務 |
| **語法不一致** | Base R 有一些怪癖；不同的套件使用不同的約定|使用 tidyverse 保持一致性 |
| **就業市場** |主要是學術/研究角色 |資料科學角色越來越喜歡 Python |
---
...
|學術研究|多領域標準| — |
|生物資訊學| Bioconductor 擁有 2,000 多個專業套件 |用於生產管道的 Python |
...
＃＃ 概括
R 是統計學語言。對於數據分析、視覺化和統計建模，它在深度和廣度上仍然無與倫比。 tidyverse 使語言現代化，R Markdown/Quarto 讓可重複的研究變得簡單。雖然 Python 在資料科學領域普遍取得了進展，但 R 仍然是專家進行嚴格統計工作的工具。對於任何從事定量研究的人來說，學習 R 是必不可少的。
---
