...
---
# R — 慣用模式與最佳實踐
本指南涵蓋了編寫簡潔、慣用的 R 程式碼的慣用模式和最佳實踐。
---
...
＃＃ 概括
R 習語強調：向量化（避免循環）、tidyverse 管道 (`|>`)、用於賦值的 `<-`、snake_case 命名以及使用`purrr`進行函數式程式設計。遵循 Tidyverse 樣式指南，使用 lintr 進行 linting，使用 styler 進行格式化。 R 值向量化操作和用於可讀資料轉換的管道。