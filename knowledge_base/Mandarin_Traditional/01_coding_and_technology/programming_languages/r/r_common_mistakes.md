...
---
# R — 常見錯誤與反模式
本文檔列出了 R 中最常見的錯誤、陷阱和反模式，並進行了修正。
---
...
＃＃ 概括
R 的交互性质会造成陷阱：循环中不断增长的数据帧（预分配或向量化）、向量回收意外、`NA` 传播以及`T`/`F`可重写。 R 方式是：向量化操作，循环时预分配，显式处理`NA`，使用 tidyverse 进行数据操作，并避免`<<-`。 R 獎勵那些用向量思考並使用其豐富的包生態系統的人。