...
|右 |`for (i in 1:10)`|
| PHP |`for ($i = 0; $i < 10; $i++)`|
| SQL | N/A（基於集合，標準 SQL 中無循環）|
|猛擊 |`for i in {0..9}; do ... done`|
|盧阿 |`for i = 0, 9 do ... end`|
| MATLAB |`for i = 1:10 ... end`|
|序言| N/A（改為遞迴）|
|口齒不清 |`(dotimes (i 10) ...)`|
|艾達 |`for I in 0..9 loop ... end loop;`|
|科博爾 |`PERFORM VARYING I FROM 0 BY 1 UNTIL I >= 10`|
|二郎 | N/A（遞歸+列表代替）|
| OCaml |`for i = 0 to 9 do ... done`|
|組裝|使用 CMP/JMP 進行手動循環 |
|德爾福|`for I := 0 to 9 do ... end;`|
|刮刮|`repeat (10) ... end`|
| VB |`For i As Integer = 0 To 9 ... Next`|
|斯卡拉 |`for (i <- 0 until 10) ...`|
|飛鏢 |`for (var i = 0; i < 10; i++)`|
|茱莉亞 |`for i in 0:9 ... end`|
...
|長生不老藥 |`case x do 1 -> ...; _ -> ... end`|
|飛鏢 | `switch (x) { 1 => ...; _ => ... }`（Dart 3.0）|
| PHP |`match($x) { 1 => ..., default => ... }`(PHP 8.0) |
| C# |`x switch { 1 => ..., _ => ... }`(C# 8+) |
|打字稿 |沒有本機模式符合（使用 if/else）|
