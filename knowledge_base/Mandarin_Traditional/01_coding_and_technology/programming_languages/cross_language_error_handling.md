...
|型號|語言 |機制|
|--------|---------|------------|
| **異常（嘗試/捕獲）** | Java、C++、C#、Python、Ruby、PHP、JavaScript、TypeScript、Kotlin、Swift、Dart、Scala、VB、Delphi |`try { } catch (e) { }`|
| **結果/選項類型** | Rust、Haskell、OCaml、Scala（也）| `Result<T, E>`、`Maybe a` |
| **多個回傳值** |走吧，盧阿 |`value, err := f()`|
| **錯誤代碼** | C、Fortran |傳回錯誤碼，手動檢查 |
| **模式匹配錯誤** | Elixir、Erlang |`{:ok, value}`/`{:error, reason}`|
| **條件（可重新啟動）** |通用 Lisp | `restart-case`、`handler-bind` |
| **無錯誤處理** | Scratch、SQL（基礎）|不適用 |
| **合約** |艾達，艾菲爾 | `Pre`、`Post` 條件 |
## 基於例外的錯誤處理
```python
# Python
try:
...
|範式|方法|優點 |缺點 |
|----------|----------|------|-----|
| **例外** |投擲/接住|乾淨的快樂路徑，堆疊痕跡|可被遺忘、控制流濫用|
| **結果類型** |傳回`Ok`/`Err`|明確的，不能忽略錯誤 |需要詳細的`?`運算子 |
| **錯誤代碼** |返回 int 狀態 |簡單，C 相容 |容易被忽視，沒有上下文|
| **多次回報** |`value, err := f()`|明確、慣用 |可以忘記檢查`err` |
| **模式匹配** |`{:ok, v}`/`{:error, r}`|詳盡、實用 |沒有`with`的詳細資訊 |
| **條件** |可重新啟動的處理程序 |最強大，可恢復|複雜，很少被理解 |