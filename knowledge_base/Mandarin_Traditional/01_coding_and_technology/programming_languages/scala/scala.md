...
# 斯卡拉
Scala（可擴展語言）是一種靜態類型、編譯型程式語言，結合了物件導向和函數式程式設計範例。 Scala 由 Martin Odersky 創建並於 2004 年首次發布，它在 JVM 上運行（也稱為 JavaScript 和 Scala Native 的 Scala.js）。它旨在解決 Java 的冗長問題，同時保持完整的 Java 互通性。
Scala 是 Apache Spark（大數據處理框架）背後的語言，廣泛應用於資料工程、分散式系統和後端服務。 Twitter（現在的 X）、LinkedIn、Netflix 和衛報等公司都使用 Scala。
---

## 為什麼 Scala 很重要
- **函數式 + OOP 混合**：在單一語言中結合了兩個範例的優點。
- **簡潔**：比 Java 簡潔得多 — 模式匹配、案例類別、類型推論。
- **Apache Spark**：基於 Spark 的大數據處理的主要語言。
- **類型系統**：進階功能，如隱式、類型類別和代數資料類型。
- **JVM相容性**：使用所有Java函式庫；在同一個 JVM 上運行。
- **Akka / Pekko**：用於建構並發分散式系統的流行框架。
## 權衡
|限制|詳情 |典型解決方法|
|------------|---------|--------------------|
| **複雜性** |該語言有很多特性；可能很難掌握|從「好的部分」開始；避免過於聰明的程式碼 |
| **編譯時間** |比 Java 慢，尤其是複雜型別 |使用增量編譯（Bloop、sbt）|
| **學習曲線** |比 Java 或 Python 更陡峭 |投入時間；回報是顯著的|
| **就業市場較小** |比 Java 或 Python 更少的角色 |擅長資料工程與後端角色 |
| **風格不一致** |不同的團隊寫出截然不同的Scala |遵循社區風格指南 |
---
...
## 進階語法和模式
### 類型類別（透過隱式）
Scala 2 使用隱式參數來編碼型別類別。 Scala 3 引進了原生`given`/`using`語法。
```scala
...
|`sbt console`|使用類別路徑上的專案啟動 REPL |
|`sbt clean`|乾淨的編譯輸出 |
|`sbt assembly`|建立 fat JAR（使用 sbt-assemble 插件）|
|`sbt scalafmt`|使用 Scalafmt 格式化程式碼 |
|`sbt scalafmtCheck`|檢查程式碼格式 |
|`sbt ~compile`|持續編譯（更改時重新編譯）|
### 程式碼格式 (.scalafmt.conf)
```
# .scalafmt.conf
version = 3.7.14
...
|工具|目的|用途 |
|------|---------|--------|
| **JMH** |微基準測試|`sbt-jmh`插件 |
| **VisualVM** | JVM 分析與監控 |`jvisualvm`指令 |
| **非同步分析器** |低開銷 CPU/記憶體分析 |附加到正在執行的 JVM |
| **你的套件** |商業分析儀| IDE整合|
| **sbt-scoverage** |程式碼覆蓋率|`sbt coverage test coverageReport`|
### 使用 JMH 進行基準測試
```scala
...
＃＃ 概括
Scala 是一種功能強大的語言，它將函數式程式設計引入 JVM。它是 Apache Spark 的語言，也是資料工程、分散式系統和後端服務的強大選擇。學習曲線是真實的，但回報是一種既具有表現力又具有高性能的語言。對於已經投資 JVM 生態系統的團隊，Scala 提供了更簡潔、更強大的 Java 替代方案。