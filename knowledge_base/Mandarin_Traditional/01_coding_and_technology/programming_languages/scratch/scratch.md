...
## 為什麼 Scratch 很重要
- **最佳程式設計入門**：完全消除文法障礙。概念是透過視覺操縱來教導的。
- **計算思維**：教導分解、模式識別、抽象和演算法設計。
- **創造力驅動**：孩子們創造遊戲、動畫、故事和音樂——學習程式設計是製作他們關心的東西的副產品。
- **全球覆蓋**：在世界各地的學校中使用。有 70 多種語言版本。免費且基於瀏覽器。
- **社群**：Scratch 線上社群教授共享、混音和協作學習。
## 權衡
|限制|詳情 |典型解決方法|
|------------|---------|--------------------|
...
|问题 |原因 |解决方案 |
|--------|--------|----------|
|精靈沒有回應 |沒有事件帽子塊|新增「當綠旗被點擊」或其他事件 |
...
## 綜合問答
**問題1：Scratch真的是一種程式語言嗎？ **
A1：是的，Scratch 是一種真正的程式語言，但它是可視化的而不是基於文字的。它支援所有基本程式設計概念：變數、循環、條件、函數（自訂區塊）、列表和事件驅動程式設計。不同之處在於您拖放區塊而不是鍵入程式碼。這消除了語法錯誤，並使年輕學習者可以輕鬆編程。
**問題2：如何在Scratch中建立自訂函數（自訂區塊）？ **
A2：進入「我的區塊」類別，點擊「建立區塊」。為其命名，根據需要添加參數，然後透過在其下方添加區塊來定義其行為。自訂區塊可以接受輸入（數字、字串、布林值）並且可以呼叫其他自訂區塊。這使得模組化程式設計和程式碼重用成為可能。
**問題3：在Scratch中處理複雜遊戲邏輯的最佳方式是什麼？ **
A3：使用自訂區塊來組織邏輯，廣播訊息以在精靈之間進行事件協調，並使用清單來儲存遊戲狀態（分數、等級、庫存）。對於複雜的人工智慧，請使用具有追蹤當前狀態的變數的有限狀態機。為多個敵人克隆精靈，並使用「當我作為克隆開始時」來賦予每個敵人獨立的行為。
**問題4：如何在Scratch中的精靈之間共享資料？ **
A4：使用全域變數（在沒有「僅用於此精靈」的情況下建立）來共享數據，例如得分或遊戲狀態。使用廣播訊息來觸發跨精靈的事件。對於更複雜的通信，請使用清單作為共用資料結構。每個精靈都可以讀取和修改全域變數和列表，從而實現協調。
**Q5：Scratch中有哪些進階技巧？ **
A5：使用筆塊進行繪畫和創造視覺效果。為類似 3D 的圖形實現光線投射。將雲端變數用於多人遊戲（需要 Scratcher 狀態）。使用隨機數和列表創建程式生成。使用帶有參數的自訂區塊以實現可重複使用演算法。嘗試互動式專案的視訊感測和聲音處理。
---

## 思路
### 問題 1：創建平台遊戲
**第 1 步：了解問題**
我們需要創建一個平台遊戲，角色可以向左/向右移動、跳躍、避開障礙物和收集物品。
**第 2 步：確定方法**
- 使用「下落」變數的重力模擬
- 使用顏色或精靈觸控偵測地面/碰撞
- 將關卡資料儲存在清單中
- 使用自訂區塊進行跳躍和移動邏輯
**第 3 步：實施解決方案**```scratch
...
**第4步：驗證與最佳化**
在不同平台上測試跳躍。調整重力和跳躍高度以獲得良好的遊戲感覺。添加跑步和跳躍的動畫。使用廣播訊息實施檢查點。
---

### 問題 2：創建具有分數追蹤功能的問答遊戲
**第 1 步：了解問題**
建立一個提問、檢查答案並追蹤玩家得分的問答遊戲。
**第 2 步：確定方法**
- 將問題和答案儲存在並行清單中
- 使用問題計數器來追蹤進度
- 使用「詢問並等待」區塊輸入
...
**第4步：驗證與最佳化**
使用包括邊緣情況在內的各種答案進行測試。添加錯誤答案的回饋。實施重試選項。添加正確/錯誤答案的聲音效果和視覺回饋。
---
...
**第4步：驗證與最佳化**
調整美觀樹木的樹枝長度閾值和角度範圍。使用顏色變化在樹枝尖端添加葉子。實施不同的樹樣式。將繪圖另存為影像。
---
