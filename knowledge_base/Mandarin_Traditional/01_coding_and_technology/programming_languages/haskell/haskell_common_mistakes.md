...
# Haskell — 常見錯誤與反模式
本文檔列出了 Haskell 中最常見的錯誤、陷阱和反模式，並進行了修正。
---
...
＃＃ 概括
Haskell 的純粹性和惰性造成了獨特的陷阱：惰性求值造成的空間洩漏、運行時崩潰的部分函數、String/Text/ByteString 之間的混淆以及過度使用 IO。 Haskell 方法是：對累加器使用嚴格評估（`foldl'`、 BangPatterns ），避免部分函數（`head`、`read`、`!!`），使用`Maybe`/ `Either`QZ：SIOZIOZIOZ 並保留文字。編譯器的類型系統是你最好的盟友——讓它引導你走向完整的純函數。