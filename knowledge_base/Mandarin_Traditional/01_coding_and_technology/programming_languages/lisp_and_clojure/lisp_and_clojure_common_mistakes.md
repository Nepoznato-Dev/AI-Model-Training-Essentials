...
＃＃ 概括
Lisp/Clojure 陷阱：引用阻止求值（使用`,`的準引用），`nil` 和`false`都是假的但不同，集合是不可變的（重新綁定或使用原子），使用`recur`進行尾遞歸，並在巨集中使用 XQZMARKERLQQZ 進行尾遞歸，並在每個系列中使用 XQZMARKERQZXZ 進行尾遞歸，並在每個系列中使用 XQZMARKER。 Lisp 的方式是：擁抱不變性、理解讀者巨集系統、編寫衛生的巨集。