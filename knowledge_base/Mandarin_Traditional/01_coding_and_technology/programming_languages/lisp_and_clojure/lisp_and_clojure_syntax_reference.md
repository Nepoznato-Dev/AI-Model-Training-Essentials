...
＃＃ 概括
Clojure 的語法統一帶括號－每種形式都是一個清單。這種同像性使得強大的巨集能夠將程式碼轉換為資料。持久性資料結構提供不變性和結構共享以提高效率。線程宏創建可讀的資料管道。並發原語（原子、引用、代理、通道）安全地處理狀態變更。 Lisp/Clojure 對現代程式設計的影響是巨大的——垃圾收集、REPL、巨集和函數式程式設計都可以追溯到 Lisp。