...
-->
# Lua — 常見錯誤與反模式
本文檔列出了 Lua 中最常見的錯誤、陷阱和反模式，並進行了修正。
---
...
＃＃ 概括
Lua 的極簡主義造成了陷阱：基於 1 的索引，只有`nil`和`false`是假的，表是引用（不是值），變數預設是全局的，`pairs` 是無序的，而`ipairs`是有序的。 Lua 方式是：一律使用`local`，對陣列使用`ipairs`，對字典使用`pairs`，對字串建構使用`table.concat`，對 OOP 使用元表。 Lua 的簡單性就是它的力量——但要尊重邊緣情況。