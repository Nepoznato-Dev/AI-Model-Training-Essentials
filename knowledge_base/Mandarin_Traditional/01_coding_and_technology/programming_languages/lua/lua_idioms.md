...
-->
# Lua — 慣用模式與最佳實踐
本指南涵蓋了編寫乾淨、慣用的 Lua 程式碼的慣用模式和最佳實踐。
---
...
＃＃ 概括
Lua 慣用語強調：表格作為通用資料結構、局部變數、狀態閉包、OOP 元表以及基於 1 的索引。依照 luacheck 進行 linting，依照 stylua 進行格式化。 Lua 重視簡單性和極簡主義——語言故意很小，因此模式是從組合中顯現出來的。