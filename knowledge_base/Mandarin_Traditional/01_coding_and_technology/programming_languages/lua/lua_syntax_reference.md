...
---
# Lua — 語法參考
本文檔為 Lua (5.4) 提供了全面、結構化的語法參考。它透過關注詳盡的語法模式、表和元表、協程以及 Lua 獨特的設計理念來補充主要的 Lua 參考資料。
---
...
|----------|------|---------|--------|
|`+``-``*``/``%``^` |算術|`2 ^ 10` |`^`是求冪 |
|`//`|樓層劃分|`7 // 2`|返回 3 (Lua 5.3+) |
|`==``~=` |平等|`a == b`|`~=`是「不等於」|
|`<``>``<=``>=` |比較|`a >= b`| |
|`and``or``not`|邏輯 |`a and b`|傳回運算元，而非布林值 |
|`..`|連線 |`"hello" .. " world"`| |
|`#`|長度 |`#t`|字串或表格的長度 |
|`&``\|``~``<<``>>`|位元|`a & b`| Lua 5.3+（整數運算）|
### 邏輯運算子 — 非布林返回
...
＃＃ 概括
Lua 的語法刻意極簡——整個語言可以在一個頁面中描述。表作為通用資料結構（數組、字典、物件、模組）。元表提供了可擴展性，但沒有語言複雜性。協程支援協作式多工處理和優雅的迭代器模式。該語言的強大之處在於其可嵌入性：Lua 被設計為駐留在主機應用程式中，以最小的開銷提供腳本功能。對於遊戲開發、配置和嵌入式系統，Lua 的簡單性是其最大的優勢——一種小語言，可以準確地完成所需的任務，僅此而已。