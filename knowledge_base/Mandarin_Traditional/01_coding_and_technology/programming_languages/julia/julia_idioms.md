...
# Julia — 慣用模式與最佳實踐
本指南涵蓋了編寫乾淨、慣用的 Julia 程式碼的慣用模式和最佳實踐。
---
...
＃＃ 概括
Julia 習慣用語強調：類型穩定性、多重調度、使用`.`進行廣播、理解以及透過具體類型進行效能。遵循 Julia 風格指南，使用 JuliaFormatter 進行格式化，使用 JET 進行類型推斷分析。 Julia 價值觀「預設速度很快」——編寫乾淨的程式碼並讓 JIT 編譯器進行最佳化。