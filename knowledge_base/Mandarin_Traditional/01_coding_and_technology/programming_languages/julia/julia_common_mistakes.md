...
# Julia — 常見錯誤與反模式
本文檔列出了 Julia 中最常見的錯誤、陷阱和反模式，並進行了修正。
---
...
＃＃ 概括
Julia 的效能取決於類型穩定性：避免熱代碼中的全域變數、在類型上使用多重分派而不是 if/else、預先分配陣列、使用廣播進行逐元素操作以及在需要時對類型進行註解。 Julia 獎勵編寫類型穩定程式碼並利用多重調度的開發人員。`@code_warntype`巨集是尋找類型不穩定的最佳朋友。