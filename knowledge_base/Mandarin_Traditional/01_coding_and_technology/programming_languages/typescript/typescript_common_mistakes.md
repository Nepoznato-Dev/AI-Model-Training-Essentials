...
-->
# TypeScript — 常見錯誤與反模式
本文檔列出了 TypeScript 中最常見的錯誤、陷阱和反模式。每個條目都會顯示錯誤的方法，解釋其失敗的原因，並提供正確的解決方案。
---
...
＃＃ 概括
TypeScript 的類型系統很強大，但需要紀律。主要的罪過是使用`any`——當類型確實未知時使用 `unknown`，並且使用類型保護來縮小範圍。透過正確處理未定義來避免非空斷言（`!`）。使用物件形狀的介面、聯合的類型和計算類型。受歧視的聯合消除了整個類別的錯誤。從第一天起就啟用`strict: true`— 改造的嚴格性是痛苦的。目標是使非法狀態在類型層級上無法代表。