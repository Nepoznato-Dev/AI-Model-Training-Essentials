...
# Python — 慣用模式與最佳實踐
本指南涵蓋了編寫乾淨的 Python 程式碼的慣用模式和最佳實踐。
---
...
＃＃ 概括
Pythonic 程式碼遵循 Python 之禪：可讀、明確、簡單、扁平。關鍵習慣用法包括：清單/字典/集合理解、上下文管理器 (`with`)、EAFP 錯誤處理、用於迭代的`enumerate`/ `zip`、用於資料容器的 `dataclass`、類型提示、解包和真值/假值檢查。 Python 社群重視可讀性，並且「應該有一個明顯的方法來做到這一點」。遵循 PEP 8 的樣式，使用`ruff`進行 linting，並儘可能選擇標準庫而不是第三方解決方案。