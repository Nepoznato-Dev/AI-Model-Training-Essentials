...
# Python — 常見錯誤與反模式
本文檔列出了 Python 中最常見的錯誤、陷阱和反模式。每個條目都會顯示錯誤的方法，解釋其失敗的原因，並提供正確的解決方案。理解這些陷阱將幫助您編寫更健壯的 Python 程式碼。
---
...
**為什麼會發生：** 預設參數在函數定義時計算一次，而不是在每次呼叫時計算。可變的預設值（列表、字典、集合）在所有呼叫之間共用。
---
...
＃＃ 概括
Python 的簡單性是具有欺騙性的——可變的預設值、後期綁定閉包、範圍規則和資源管理都有一些微妙的行為讓開發人員陷入困境。要記住的關鍵原則：預設值評估一次，`is` 檢查身份而不是相等，始終使用資源上下文管理器，捕獲特定異常，並支援組合而不是上帝物件。編寫 Pythonic 程式碼意味著理解這些陷阱並按預期使用該語言的功能（上下文管理器、推導式、生成器、`collections` 模組）。