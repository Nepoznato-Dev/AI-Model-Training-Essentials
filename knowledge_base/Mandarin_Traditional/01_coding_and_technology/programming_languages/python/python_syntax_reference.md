...
-->
# Python — 語法參考
本文檔提供了全面、結構化的 Python 語法參考。它透過關注詳盡的語法模式、運算符表和內部機制來補充主要的 Python 參考。
---
...
|`-`|減法|`3 - 2`|`1`|也是一元否定 |
|`*`|乘法|`3 * 2`|`6`|還有字串/列表重複 |
|`/`|真正的分割 |`7 / 2`|`3.5`|總是回傳浮點數 |
|`//`|地板事業部|`7 // 2`|`3`|朝負無窮方向捨去 |
|`%`|模數|`7 % 2`|`1`|符號匹配除數|
|`**`|求冪|`2 ** 10`|`1024`|右結|
### 比較和布林運算符
|操作員|名稱 |範例|筆記|
...
|`!=`|不等於|`x != y`| |
| `<`、`>`、`<=`、`>=` |訂購 |`x >= y`|可連結：`a < b < c` |
|`is`|身分|`x is y`|記憶體中的相同物件|
|`is not`|不是身分|`x is not None`|優於`!= None`|
|`in`|會員資格 |`x in collection`|對於集合/字典，O(1)；對於列表，O(n) |
|`and`|邏輯與|`a and b`|短路；傳回最後一個真值/假值 |
|`or`|邏輯或 |`a or b`|短路；傳回第一個真/假值 |
|`not`|邏輯非 |`not x`| |
### 位元運算符
|操作員|名稱 |範例|筆記|
|----------|------|---------|--------|
...
|收藏|訂購 |可變 |重複 |文法 |查找 |
|------------|---------|---------|------------|--------|--------|
|`list`|是的 |是的 |是的 |`[1, 2, 3]`| O(n) |
|`tuple`|是的 |沒有 |是的 |`(1, 2, 3)`| O(n) |
|`dict`|是（3.7+）| 是的 |按鍵：否 |`{"k": "v"}`| O(1) 平均 |
|`set`|沒有 |是的 |沒有 |`{1, 2, 3}`| O(1) 平均 |
|`frozenset`|沒有 |沒有 |沒有 |`frozenset({1,2})`| O(1) 平均 |
### 列表
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
＃＃ 概括
此語法參考涵蓋了 Python 的完整功能集 - 從基本運算符和控制流到高級元編程、描述符和類型系統。 Python 的語法刻意簡潔且可讀，但其深度卻很豐富：模式匹配、非同步生成器、協定和元類為複雜應用程式提供了強大的工具。該語言不斷發展，每個版本都添加了提高表達能力同時保持向後相容性的功能。