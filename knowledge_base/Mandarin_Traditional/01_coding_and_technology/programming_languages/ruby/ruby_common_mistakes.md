...
# Ruby — 常見錯誤與反模式
本文檔列出了 Ruby 中最常見的錯誤、陷阱和反模式。每個條目都會顯示錯誤的方法，解釋其失敗的原因，並提供正確的解決方案。
---
...
＃＃ 概括
Ruby 的表現力可能會導致反模式：在迭代期間修改集合、猴子修補核心類別、全域變數和胖模型。 Ruby 方式是：使用枚舉器（`each`、`map`、`reject`）、首選符號和關鍵字參數、凍結字串文字、從模型中提取服務物件以及使用細化而不是猴子補丁。 Ruby 獎勵那些擁抱塊、鴨子類型和最小意外原則的開發者。