...
# Kotlin — 慣用模式與最佳實踐
本指南涵蓋了編寫乾淨、慣用的 Kotlin 程式碼的慣用模式和最佳實踐。
---
...
＃＃ 概括
Kotlin 習慣用法強調：空安全性、資料類別、密封類型、作用域函數、擴展函數、協程和以表達式為導向的程式碼。遵循 Kotlin 編碼約定，使用 ktlint 進行格式化，使用 detekt 進行靜態分析。 Kotlin 重視簡潔性和表現力——編寫更少的樣板文件，採用標準庫，並預設使用 `val`。