...
---
# Kotlin — 常見錯誤與反模式
本文檔列出了 Kotlin 中最常見的錯誤、陷阱和反模式。每個條目都會顯示錯誤的方法，解釋其失敗的原因，並提供正確的解決方案。
---

## 1. 平台類型和 NullPointerException
```kotlin
...
## 3.`data class`沒有適當的`equals`/ `hashCode`
```kotlin
...
＃＃ 概括
Kotlin 的簡潔性可以隱藏微妙的問題：Java 互通的平台類型、`lateinit` 崩潰、可變資料類破壞集合以及協程取消。 Kotlin 的方法是：擁抱空安全（永遠不要使用`!!`），使用正確的作用域函數（`let`用於轉換，`apply`用於配置，`also`的副作用來處理，`apply` Java）。編譯器是你的嚮導－如果它發出警告，請聽。