...
＃＃ 概括
組合語言語法由處理器體系結構和組譯器（NASM、GAS、MASM）定義。 x86-64 提供 16 個通用暫存器、多種尋址模式以及數百條涵蓋資料移動、算術、邏輯、控制流程、SIMD 和系統呼叫的指令。這裡使用的 NASM 語法是 Linux x86-64 開發中最常見的語法。掌握組譯需要了解暫存器、堆疊、呼叫約定以及高階結構如何對應到機器指令。