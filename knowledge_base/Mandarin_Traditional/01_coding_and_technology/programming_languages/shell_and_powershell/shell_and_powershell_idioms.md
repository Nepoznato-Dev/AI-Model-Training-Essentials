...
＃＃ 概括
Shell 習慣用法強調：`set -euo pipefail`、引用變數、`[[ ]]`over`[ ]`、參數擴展和清理陷阱。 PowerShell 習慣用法強調：CmdletBinding、管線處理、splatting 和結構化錯誤處理。遵循適用於 Bash 的 ShellCheck 和適用於 PowerShell 的 PSScriptAnalyzer。