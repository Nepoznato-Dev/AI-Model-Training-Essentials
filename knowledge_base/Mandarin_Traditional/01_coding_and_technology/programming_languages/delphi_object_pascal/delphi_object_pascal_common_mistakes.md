...
---
# Delphi / Object Pascal — 常見錯誤與反模式
本文檔列出了 Delphi/Object Pascal 中最常見的錯誤、陷阱和反模式，並進行了修正。
---
...
＃＃ 概括
Delphi 陷阱：始終使用 try/finally 釋放對象，區分 `=`（比較）和 `:=`（賦值），字串索引為 1，在構造函數中呼叫 `inherited`，並將業務邏輯與 UI 分開。 Delphi 的 VCL/FMX 框架獎勵乾淨的關注點分離。