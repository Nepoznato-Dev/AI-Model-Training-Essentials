...
# Rust — 語法參考
本文檔為 Rust 提供了全面、結構化的語法參考。它透過關注詳盡的語法模式、運算符表以及所有權系統、特徵和並發的內部機制來補充主要的 Rust 參考。
---
...
|類型 |堆疊/堆|尺寸|可變性 |筆記|
|------|------------|------|------------|--------|
|基元（`i32`、`f64`、`bool`、`char`） |堆疊|固定|預設不可變 |`Copy`|
| `[T; N]`（陣列）|堆疊|固定|與`mut`可變 |固定大小，若 T 是 `Copy`，則`Copy`|
|`Vec<T>`|堆|動態 |與`mut`可變 |可成長|
|`String`|堆|動態 |與`mut`可變 | UTF-8，可增長 |
| `&[T]`（切片）|堆疊（胖指標）|動態視圖|取決於`&`或`&mut`|借用觀點|
|`HashMap<K, V>`|堆|動態 |與`mut`可變 | O(1) 平均查找 |
|`BTreeMap<K, V>`|堆|動態 |與`mut`可變 | O(log n) 查找，有序 |
|`HashSet<T>`|堆|動態 | — | O(1) 平均，無重複 |
### 向量
```rust
// Creation
...
＃＃ 概括
Rust 的語法建立在編譯時強制執行的安全保證的基礎上。所有權系統、基於特徵的多態性、模式匹配和零成本抽象形成了一個連貫的設計，其中編譯器是一個嚴格但公平的合作夥伴。從基本控制流程到巨集、不安全程式碼和智慧指標等進階功能，每個構造都服務於安全、高效的系統程式設計的目標。理解生命週期、借用和類型系統之間的互動是掌握 Rust 的關鍵。