...
---
# Rust — 慣用模式與最佳實踐
本指南涵蓋了編寫乾淨、安全的 Rust 程式碼的慣用模式和最佳實踐。
---
...
＃＃ 概括
Rust 習慣用法強調：所有權和借用、用於錯誤處理的`Result`/ `Option`、帶有方法的枚舉、抽象特徵、迭代器鏈、用於共享狀態的智慧指標和零成本抽象。依照`rustfmt`進行格式化，請依照`clippy`進行 linting，以及 Rust API 指南。 Rust 社群重視安全性、效能和正確性——「如果它能編譯，它就能運作。」在函數參數中優先使用`&str`而不是 `String`，使用`?`進行錯誤傳播，並使用`#[derive(Debug, Clone, ...)]`衍生共同特徵。