...
-->
# COBOL — 語法參考
本文檔提供了 COBOL (2014+) 的全面、結構化語法參考。它透過關注詳盡的語法模式、資料描述、文件處理和業務邏輯來補充主要的 COBOL 參考。
---
...
＃＃ 概括
COBOL 的語法在設計上很冗長——每個語句讀起來都像英語。帶有 PIC 子句的資料描述精確定義了欄位格式。四部門結構強制關注點分離。文件處理內建於該語言中。對於業務資料處理，COBOL 的十進制算術和以記錄為導向的 I/O 仍然無與倫比。了解 COBOL 意味著了解企業運算的基礎。