...
# Swift — 慣用模式與最佳實踐
本指南涵蓋了編寫乾淨、Swift 程式碼的慣用模式和最佳實踐。
---
...
＃＃ 概括
Swift 習慣用法強調：安全性（可選、防護）、值類型（結構）、協定導向的設計、模式匹配、非同步/等待和不變性（預設為 `let`）。遵循 Swift API 設計指南，使用 SwiftLint 保證程式碼質量，使用 SwiftFormat 進行格式化。 Swift 重視安全性和表現力——“如果它能編譯，它可能是正確的。”