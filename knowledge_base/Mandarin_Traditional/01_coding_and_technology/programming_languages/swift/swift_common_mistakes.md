...
# Swift — 常見錯誤與反模式
本文檔列出了 Swift 中最常見的錯誤、陷阱和反模式。每個條目都會顯示錯誤的方法，解釋其失敗的原因，並提供正確的解決方案。
---
...
＃＃ 概括
Swift 的安全功能可以防止許多錯誤，但也引入了自己的陷阱：強制展開會導致崩潰，保留循環隱藏在閉包和委託中，而值類型語義則讓來自引用類型語言的開發人員感到驚訝。 Swift 的方法是：永遠不要強制展開 (`!`)，使用`guard`進行提前退出，使用`weak`進行委託和閉包捕獲，使用`@MainActor`標記 UI 程式碼，並將大量視圖控制器分解為視圖模型和服務。 Swift 的類型系統是你的朋友——使用它，而不是反對它。