...
-->
# C++ — 慣用模式與最佳實踐
本指南涵蓋了編寫簡潔、現代 C++ (20/23) 程式碼的慣用模式和最佳實踐。
---
...
＃＃ 概括
現代 C++ 習慣用法強調：RAII 和智慧指標、移動語意、用於類型推導的 `auto`、範圍和演算法 (C++20)、模板限制的概念、用於缺失/錯誤的`std::optional`/ `std::expected`、零規則和用於編譯時計算的 XQZKER3XQZ。依照`clang-format`進行格式化，依照`clang-tidy`進行 linting，並使用`-Wall -Wextra -Werror`進行編譯。 C++ 社群重視零成本抽象，並且「不要為不使用的東西付費」。