...
-->
# C++ — 語法參考
本文檔提供了現代 C++ (C++17/20/23) 的全面、結構化語法參考。它透過關注詳盡的語法模式、運算符表、模板機制和現代功能來補充主要的 C++ 參考。
---
...
|`::`|範圍解析 |`std::cout`| |
|`.*``->*` |指向成員的指標 |`obj.*pmf`| |
|`<=>`|太空船 (C++20) |`a <=> b`|返回`strong_ordering`|
|`co_await`|協程暫停 |`co_await expr`| C++20 | C++20
|`co_yield`|協程產量 |`co_yield value`| C++20 | C++20
|`co_return`|協程回傳|`co_return value`| C++20 | C++20
### 運算子重載
```cpp
class Vector2D {
    double x_, y_;
...
|貨櫃|標題 |已訂購 |查找 |筆記|
|------------|--------|---------|--------|--------|
|`vector`|`<vector>`|是的 | O(n) |序列的預設選擇 |
|`deque`|`<deque>`|是的 | O(n) |兩端快速推/彈出 |
|`list`|`<list>`|是的 | O(n) |雙向鍊錶|
|`array`|`<array>`|是的 | O(n) |固定大小、堆疊分配 |
|`set`/`multiset`|`<set>`|是的 | O(log n) |紅黑樹|
|`map`/`multimap`|`<map>`|是（按鍵）| O(log n) |鍵值、紅黑樹|
|`unordered_set`|`<unordered_set>`|沒有 | O(1) 平均 |哈希表|
|`unordered_map`|`<unordered_map>`|沒有 | O(1) 平均 |哈希表|
### 關鍵演算法
```cpp
#include <algorithm>
#include <numeric>
...
＃＃ 概括
C++ 語法涵蓋從低階指標操作到高階模板元程式設計和基於協程的非同步。該語言透過 C++17/20/23 的演變極大地提高了安全性（智慧指標、`std::optional`、概念）、表現力（結構化綁定、範圍、`std::format`）和效能（實現零成本抽象的概念、無需棧堆疊開銷的非同步協程）。核心理念保持不變：您不應該為不使用的東西付費，並且該語言應該支援從裸機到特定於領域的語言的每個抽象層級。