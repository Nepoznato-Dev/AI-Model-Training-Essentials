...
-->
# Erlang 和 Elixir — 文法參考
本文檔為 Erlang 和 Elixir 提供了全面、結構化的語法參考。它透過關注模式匹配、流程管理、OTP 行為和容錯系統模式來補充主要參考。
---
...
＃＃ 概括
Erlang 和 Elixir 共享 BEAM VM 的超能力：輕量級進程、訊息傳遞和容錯監督樹。 Erlang 的語法簡潔且實用。 Elixir 新增了帶有管道、巨集和協定的現代語法。 OTP 行為（GenServer、Supervisor、應用程式）為建立可靠的系統提供了經過實戰檢驗的模式。對於即時、分散式、容錯應用程序，Erlang/Elixir 仍然是無與倫比的。