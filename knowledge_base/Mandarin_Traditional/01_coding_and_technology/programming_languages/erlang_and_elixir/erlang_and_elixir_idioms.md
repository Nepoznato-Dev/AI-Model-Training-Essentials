...
-->
# Erlang 和 Elixir — 慣用模式和最佳實踐
本指南涵蓋了 Erlang 和 Elixir 在 BEAM VM 上的慣用模式。
---
...
＃＃ 概括
Elixir 習語強調：管道運算子、模式匹配、用於錯誤連結的`with`以及`@spec`類型規範。 Erlang 習慣用語強調：模式匹配、清單推導式、gen_server 回呼和「讓它崩潰」哲學。兩者都重視不變性、訊息傳遞和容錯性。