...
＃＃ 概括
現代 C# 習慣用法強調：資料記錄、模式比對、空安全（可為空引用型別）、集合 LINQ、使用 CancellationToken 進行非同步/等待、相依性注入、主建構子、集合表達式和`using`宣告。遵循 C# 編碼約定，使用`dotnet format`進行格式化，並使用 Roslyn 分析器確保程式碼品質。現代 C# (12+) 簡潔且富有表現力 — 包含記錄、模式匹配和主建構函數。