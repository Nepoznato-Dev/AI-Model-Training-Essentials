...
|------|--------|------------|-----|----|-----|---|-----|-----|
整數| `int`（任意）|`number`| `i32`、`u64` |`int`| `int`、`long` |9XMARKER7XMARQZXJZXMXMARQZQZXJ3XMARXSTSR9XSMARSSS7XSR9XMARSTSSoSSS;;
浮動|`float`|`number`|`f64`|`float64`|`double`|`double`|`double`|QQZMARKER17XQZ |
|字串|`str`|`string`|`String`|`string`|`String`|`char*`|`string`|`string` |
|布爾 |`bool`|`boolean`|`bool`|`bool`|`boolean`|`bool`(C99) |`bool`|`bool`(C99) |`bool`|
|人物 |`char`|`rune`|`char`|`char`|`char`|QQZ
|空 |`Option<T>`|`nil`|`null`|`NULL`|`nullptr`|QQZMARKER49XQZ |
## 數組/列表
```python
...
|鐵鏽|`let s: HashSet<i32> = [1, 2, 3].into()`|
|去 | `s := map[int]bool{1: true, 2: true, 3: true}`（無內建套裝）|
...
| C# |`enum Color { Red, Green, Blue }`|
|去 | `const ( Red = iota; Green; Blue )`（無本機枚舉）|
|飛鏢 |`enum Color { red, green, blue }`|
...
|紅寶石 |無本機枚舉（使用凍結常數或`dry-enum`）|
## 空安全
|語言 |空處理 |
|----------|--------------|
|蟒蛇 |`None`，無編譯時空安全 |
| JavaScript |`null`/`undefined`，可選連結`?.`|
|鐵鏽|`Option<T>`— 不為空！編譯時強制 |
|去 |`nil`用於指標、切片、映射、通道 |
|爪哇 | `null`、`Optional<T>`（Java 8+）、`@Nullable` |
| C | `NULL`（`((void*)0)` 的巨集）|
| C++ |`nullptr`(C++11)、`std::optional<T>` |
| C# |`null`、可為 null 的參考型別 (C# 8+)、`Nullable<T>` |
|斯威夫特 |`Optional<T>`(`T?`),`if let`,`guard let`|
|科特林 | `T?`（可為空）、`?.` 安全呼叫、`?:` Elvis |
|打字稿 |`null`/`undefined`、`strictNullChecks`、`T \| null`|
|哈斯克爾 |`Maybe a`—`Just x`或`Nothing`|
|斯卡拉 |`Option[T]`—`Some(x)`或`None`|
|飛鏢 | `T?`（可空），空安全（Dart 2.12+）|
|紅寶石 | `nil`（除了`false`和`nil`之外，一切都是真的）|
|長生不老藥 |無 null — 使用`nil`或模式匹配 |
|二郎 |無 null — 使用`undefined`或模式匹配 |