...
## 為什麼 Dart 很重要
- **Flutter**：Flutter 的主要語言 - 成長最快的跨平台框架之一。
- **跨平台**：適用於 iOS、Android、Web、Windows、macOS、Linux 和嵌入式裝置的單一程式碼庫。
- **高效**：熱重載、豐富的小部件庫和富有表現力的語法使 UI 開發變得快速。
- **健全的空安全性**：編譯時空安全性消除了空引用錯誤。
- **效能**：編譯為行動裝置的本機 ARM 程式碼；不需要橋樑。
- **不斷增長的生態系統**：Flutter 的包生態系統正在迅速擴張。
## 權衡
|限制|詳情 |典型解決方法|
|------------|---------|--------------------|
| **以顫振為中心** | Dart 使用最多的是 Flutter；限制在它之外|用於顫振；用於非 UI 工作的其他語言 |
| **較小的生態系統** |比 React Native 或本機平台更少的套件 |成長迅速；原生 API 的平台管道 |
| **網路效能** | Dart 編譯為 WASM 仍在成熟 |使用 CanvasKit 渲染器獲得一致的效能 |
| **就業市場** | Flutter 角色存在，但少於原生移動 |跨平台開發人員的需求不斷增長|
| **不適用於後端** |可能（伺服器端 Dart）但不是用例 |使用 Go、Node.js、Python 作為後端 |
---
...
| **網頁** |`flutter build web`|靜態 HTML/JS/CSS |
| **Windows** |`flutter build windows`| MSIX 或獨立 exe |
| **macOS** |`flutter build macos`| .app 捆綁包 |
| **Linux** |`flutter build linux`|二元+資產 |
```bash
# Build commands
