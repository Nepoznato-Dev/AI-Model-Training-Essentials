...
# Dart — 語法參考
本文檔為 Dart (3.x) 提供了全面、結構化的語法參考。它透過關注詳盡的語法模式、空安全、非同步程式設計和麵向 Flutter 的設計模式來補充主要的 Dart 參考。
---
...
|`&&``\|\|``!`|邏輯 |`a && b`|短路|
|`??`|空合併 |`a ?? b`| 若`a`為空，則`b`|
|`?.`|空感知存取 |`a?.b`|若`a`為空，則為空 |
|`!`|空斷言 |`a!`|如果為空則拋出 |
|`..``?..` |級聯|`obj..a..b`|相同物件的鍊式操作 |
|`??=`|空感知分配 |`a ??= b`|如果為空則賦值 |
|`as`|型別轉換 |`obj as String`|失敗時拋出 |
|`is``is!` |型式試驗|`obj is String`| |
|`=>`|箭頭功能|`() => expr`| |
### 級聯表示法
```dart
...
＃＃ 概括
Dart 的語法乾淨、一致，並且專為提高生產力而設計。健全的空安全性消除了編譯時的空引用錯誤。 Async/await 和串流為非同步程式設計提供了自然模式。類別、mixins 和密封類別支援靈活的物件組合。 Dart 3 的模式匹配、記錄和類別修飾符增加了表現力。雖然 Dart 的主要用途是 Flutter，但它的語法非常適合任何應用程式網域 - 伺服器端（使用 Dart Frog 或 Shelf）、CLI 工具和 Web 應用程式（編譯為 JavaScript）。