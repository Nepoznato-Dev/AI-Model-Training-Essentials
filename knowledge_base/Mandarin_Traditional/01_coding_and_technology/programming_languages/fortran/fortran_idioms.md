...
-->
# Fortran — 慣用模式與最佳實踐
本指南涵蓋了編寫乾淨、現代的 Fortran (2018+) 程式碼的慣用模式和最佳實踐。
---
...
＃＃ 概括
現代 Fortran 習慣用法強調：`implicit none`、陣列操作（避免循環）、假定形狀陣列、具有類型綁定過程的衍生型別以及所有參數的`intent`。按照 fprettify 進行格式化。具有 coarrays 和 DO CONCURRENT 的現代 Fortran (2018+) 本身支援並行計算。