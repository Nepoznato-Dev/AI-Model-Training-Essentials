...
# Fortran — 文法參考
本文檔提供了 Modern Fortran (2008/2018) 的全面、結構化語法參考。它透過關注詳盡的語法模式、數組操作、模組、OOP 和高效能運算來補充主要的 Fortran 參考。
---
...
|`==``/=` |平等|`a == b`|`/=`是「不等於」|
|`<``>``<=``>=` |比較|`a >= b`| |
|`.and.``.or.``.not.`|邏輯 |`a .and. b`| |
|`.eqv.``.neqv.` |邏輯等價|`a .eqv. b`| |
|`//`|字串連線 |`'hello' // ' world'`| |
---
...
＃＃ 概括
現代 Fortran 的語法乾淨、結構化，並且針對數值計算進行了最佳化。數組運算消除了大多數數學任務的循環。模組提供封裝和命名空間管理。 OOP 功能（衍生型別、繼承、多態性）支援乾淨的程式碼組織。 Coarray 提供原生並行編程，無需外部程式庫。 Fortran 的優勢——效能、陣列運算和數值精度——對於科學計算來說仍然是無與倫比的。對於 HPC、計算物理和工程模擬來說，Fortran 仍然是一個強大的工具。