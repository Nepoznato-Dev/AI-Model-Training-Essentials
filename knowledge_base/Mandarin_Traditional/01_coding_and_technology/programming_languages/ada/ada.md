...
## 為什麼艾達很重要
- **安全關鍵系統**：從頭開始設計，專為不允許故障的系統而設計。
- **強類型**：任何主流語言中最嚴格的類型系統 - 在編譯時捕獲其他語言錯過的錯誤。
- **內建並發**：任務（並發程式設計）是語言的一部分，而不是函式庫。
- **形式化驗證**：支持形式化方法證明程序正確性。
- **可靠性功能**：內建異常、契約（前置/後置條件）和運行時檢查。
- **現代 Ada**：Ada 2012 和 Ada 2022 在保持安全保證的同時添加了現代功能。
## 權衡
|限制|詳情 |典型解決方法|
|------------|---------|--------------------|
| **利基社群** |與 C、Java 或 Python 相比，開發人員基礎較小 |專業但知識淵博的社群 |
| **詳細語法** |比 C 或 Python 更詳細 |接受作為安全導向設計的一部分|
| **有限的生態系統** |比主流語言更少的函式庫 |編寫自訂程式碼；使用廣泛的標準函式庫|
| **學習曲線** |具有許多安全功能的複雜語言 |從 Ada 的「SPARK」子集開始進行安全關鍵工作 |
| **編譯器可用性** |較少的編譯器選項（GNAT 是主要選項）| GNAT 是免費且維護良好的 |
---
