...
---
# Go — 慣用模式與最佳實踐
本指南涵蓋了編寫乾淨、慣用的 Go 程式碼的慣用模式和最佳實踐。
---
...
＃＃ 概括
Go 慣用語強調：簡單性、顯式錯誤處理、小接口、繼承之上的組合、具有上下文的 goroutine 以及「接受接口，返回結構」。按照`gofmt`進行格式化，請依照`golangci-lint`進行 linting，以及官方的 Go 程式碼審查註解指南。 Go 社群重視可讀性、簡單性和「顯而易見的方法」。避免聰明的程式碼——Go 程式碼應該是無聊且可預測的。