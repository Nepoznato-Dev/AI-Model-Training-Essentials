...
＃＃ 概括
Go 的語法有意做到極簡－沒有繼承、沒有方法重載、沒有異常、沒有巨集。剩下的是一組乾淨、正交的構造：用於組合的結構和介面、用於並發的 goroutine 和通道，以及用於可靠性的明確錯誤處理。該語言的力量不是來自複雜的語法，而是來自其簡單原語的優雅互動。泛型（Go 1.18+）增加了類型靈活性，同時又不犧牲 Go 特有的清晰度。