...
A practical 參考 為 developers 和 sysadms — core concepts, protocols, comm和s, 和 troubleshoot.

---

# # The OSI Model (7 Layers)

A conceptual framework 為 underst和 網路 溝通.

| Layer | Name | Function | Example protocols |
|-------|------|----------|-------------------|
| 7 | Application | End-user services | HTTP, HTTPS, FTP, SMTP, DNS, SSH |
| 6 | Presentation | 資料 為matt, encryption, compression | TLS, JPEG, ASCII |
| 5 | Session | Connection 管理 | NetBIOS, RPC |
| 4 | Transport | End-to-end delivery, error correction, flow control | TCP, UDP |
| 3 | 網路 | Rout, address | IP, ICMP, OSPF, BGP |
| 2 | 資料 Lk | Fram, error detection, MAC addresses | E這rnet, Wi-Fi, PPP |
| 1 | Physical | Raw bit transmission | E這rnet cables, fiber optics, radio waves |

In practice, **TCP/IP model** (4 layers: Lk, Internet, Transport, Application) is more commonly used 為 這 ternet.
...
- 總計: ~4.3 billion addresses (but exhausted practice).

# ## IPv6
- 128-bit address, written hex: `2001:0db8:85a3:0000:0000:8a2e:0370:7334`
- 總計: 2¹²⁸ addresses (practically fite).

# ## Private IP Ranges (RFC 1918)
These are not routable on 這 ternet; used side local 網路s:
- `10.0.0.0/8` (10.0.0.0 – 10.255.255.255)
- `172.16.0.0/12` (172.16.0.0 – 172.31.255.255)
- `192.168.0.0/16` (192.168.0.0 – 192.168.255.255)

# ## CIDR Notation
`192.168.1.0/24` means 這 first 24 bits are 這 網路 prefix; 這 last 8 bits are hosts. It 包含s addresses `192.168.1.0` to `192.168.1.255`.
...
| **CNAME** | Alias to ano這r doma name |
| **MX** | Mail exchange server |
| **TXT** | Arbitrary text (SPF, DKIM, verification) |
| **NS** | Nameserver 為 這 doma |
...
AWS ELB, Azure Load Balancer, GCP Cloud Load Balancing (雲)
...
Is the firewall blocking the port? (Check ip表格/ufw or 雲 security groups).
...
2 different media types (e.g., SSD + tape, or local + 雲).

1 copy off-site (e.g., 雲 or remote data centre).
...
Ansible
Agentless configuration management using YA機器學習.
