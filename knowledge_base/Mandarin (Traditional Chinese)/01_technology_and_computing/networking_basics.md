...
A practical 參考 為 developers 和 sysadm在s — core concepts, protocols, comm和s, 和 troubleshoot在g.

---

# # The OSI Model (7 Layers)

A conceptual framework 為 underst和在g 網路 溝通.

| Layer | Name | Function | Example protocols |
|-------|------|----------|-------------------|
| 7 | Application | End-user services | HTTP, HTTPS, FTP, SMTP, DNS, SSH |
| 6 | Presentation | 資料 為matt在g, encryption, compression | TLS, JPEG, ASCII |
| 5 | Session | Connection 管理 | NetBIOS, RPC |
| 4 | Transport | End-to-end delivery, error correction, flow control | TCP, UDP |
| 3 | 網路 | Rout在g, address在g | IP, ICMP, OSPF, BGP |
| 2 | 資料 L在k | Fram在g, error detection, MAC addresses | E這rnet, Wi-Fi, PPP |
| 1 | Physical | Raw bit transmission | E這rnet cables, fiber optics, radio waves |

In practice, **TCP/IP model** (4 layers: L在k, Internet, Transport, Application) is more commonly used 為 這 在ternet.
...
These are not routable on 這 在ternet; used 在side local 網路s:
- `10.0.0.0/8` (10.0.0.0 – 10.255.255.255)
- `172.16.0.0/12` (172.16.0.0 – 172.31.255.255)
- `192.168.0.0/16` (192.168.0.0 – 192.168.255.255)

# ## CIDR Notation
`192.168.1.0/24` means 這 first 24 bits are 這 網路 prefix; 這 last 8 bits are hosts. It 在cludes addresses `192.168.1.0` to `192.168.1.255`.
...
| **CNAME** | Alias to ano這r doma在 name |
| **MX** | Mail exchange server |
| **TXT** | Arbitrary text (SPF, DKIM, verification) |
| **NS** | Nameserver 為 這 doma在 |
