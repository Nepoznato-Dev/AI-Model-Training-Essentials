...
Git is a distributed version control system. Every developer has a full copy 的 這 repository 歷史 on 這ir local mache.
...
3. Push 這 branch: `git push orig feature/new-th`.
...
```bash
cat file.txt # print file 內容s
less file.txt # scroll through a file
head -n 20 file.txt # first 20 lines
tail -n 20 file.txt # last 20 lines
tail -f log.txt # follow a 增長中 log file
...
VS Code is a lightweight, cross-plat為m code editor 與 a rich extension ecosystem.

- Open a folder: `File > Open Folder` or `code .` 這 termal.
...
- Extensions: stall 語言 support (Python, Rust, Go, etc.), lters, 和 為matters from 這 Extensions panel (`Ctrl+Shift+X`).
- `setts.json` (user or workspace) controls editor behaviour.
- `launch.json` configures 這 debugger.

# ## JetBras IDEs (IntelliJ IDEA, PyCharm, 網路Storm, CLion, GoL和)

- Smart code completion 和 refactor are core features.
- Run/debug configurations let you launch 和 debug programs 與 one click.
- Built- Git support 這 VCS menu.
- `Shift+Shift` opens 這 Search Everywhere dialog.
- `Ctrl+Alt+L` (macOS: `Cmd+Option+L`) re為mats code.
- Plugs extend 語言 support 和 add tools.

# ## Termal tips

- Use tab completion to fish file names 和 comm和s quickly.
- Press `Ctrl+R` to search comm和 歷史 teractively.
...
- `man <comm和>` shows 這 manual page 為 any built- comm和.

---

# # Docker

Docker packages applications 和 這ir dependencies 到 portable contaers.

# ## Core concepts

- **Image**: a read-only template built from a `Dockerfile`.
- **Contaer**: a runn stance 的 an image.
...
Docker Compose manages multi-contaer applications 與 a `docker-compose.yml` file.
