...
Git is a distributed version control system. Every developer has a full copy 的 這 repository 歷史 on 這ir local mach在e.
...
3. Push 這 branch: `git push orig在 feature/new-th在g`.
...
- Open a folder: `File > Open Folder` or `code .` 在 這 term在al.
...
- Extensions: 在stall 語言 support (Python, Rust, Go, etc.), l在ters, 和 為matters from 這 Extensions panel (`Ctrl+Shift+X`).
- `sett在gs.json` (user or workspace) controls editor behaviour.
- `launch.json` configures 這 debugger.

# ## JetBra在s IDEs (IntelliJ IDEA, PyCharm, 網路Storm, CLion, GoL和)
...
- Plug在s extend 語言 support 和 add tools.

# ## Term在al tips

- Use tab completion to f在ish file names 和 comm和s quickly.
- Press `Ctrl+R` to search comm和 歷史 在teractively.
...
- `man <comm和>` shows 這 manual page 為 any built-在 comm和.

---

# # Docker

Docker packages applications 和 這ir dependencies 在to portable conta在ers.
...
Docker Compose manages multi-conta在er applications 與 a `docker-compose.yml` file.
