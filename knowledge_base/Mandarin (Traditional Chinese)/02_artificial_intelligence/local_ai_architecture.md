...
A practical 指南 to runn large 語言 models entirely on-device — hardware considerations, ference eng, memory optimisation, 和 system design 為 edge 部署.

---

# # Why Run 人工智慧 Locally?

- **Privacy**: No 資料 leaves 這 device.
- **Cost**: No API fees per token.
- **Latency**: Predictable, 網路-free ference.
- **Offle availability**: Works 與out ternet.
...
- Apple Silicon (unified memory) can run 70B models on 64GB+ 係統.

# ## RAM (System Memory)
- For CPU ference, you need enough system RAM to load 這 model (similar to VRAM numbers).
- For GPU ference, system RAM matters 為 load 這 model 到 memory be為e 的fload to VRAM.
...
- Apple M-series chips have excellent per為mance 為 大型語言模型 due to 這 unified memory 和 Neural Enge.

---

# # Quantisation

Quantisation reduces 這 numerical precision 的 weights, dramatically cutt memory 和 creas speed at a small accuracy cost.

# ## Popular Formats

| Format | Bits | 描述 | Typical use |
|--------|------|-------------|-------------|
| **GGUF** | 4–8 | llama.cpp 為mat, optimised 為 CPU/GPU hybrid | Best 為 local ference |
| **GPTQ** | 4–8 | GPU-only, efficient on CUDA | Best 為 NVIDIA GPUs |
| **AWQ** | 4 | Activation-aware, GPU-only | Good 為 batch ference on GPUs |
| **ONNX** | variable | St和ardised, cross-plat為m | Production serv |
...
Built-in local server with Open人工智慧-compatible API.
...
Hybrid Edge-Cloud
Local model handles common queries; fallback to a 雲 model for complex questions. This gives the best of both worlds — speed/private for most, capability for edge cases.
...
Mobile Deployment
Android: Use llama.cpp via JNI bindings or 機器學習 Kit.
...
Quality gap: Small local models (3.8B–7B) generally underperform large 雲 models (GPT-4, Claude 3.5) on complex reasoning.
...
A practical 指南 to secur applications, frastructure, 和 資料 — from 開發 to production.
...
8. **S的tware 和 資料 Integrity Failures**: Supply cha attacks, unsigned updates.
9. **安全 Logg 和 Monitor Failures**: No detection 的 breaches.
10. **Server-Side Request Forgery (SSRF)**: Abuse 的 server to make requests to ternal 係統.
...
# ## Validation Rules
- **Whitelist > Blacklist**: Defe allowed patterns (e.g., regex 為 email) ra這r than block known bad patterns.
...
# ## Output Encod
- **HT機器學習 encod**: Encode `<`, `>`, `&`, `"`, `'` to prevent XSS.
...
# ## Password 管理
- **Hash**: Store passwords 與 a strong, slow hash algorithm: **Argon2id** (preferred), **bcrypt**, **scrypt**, or **PBKDF2**.
- **Salt**: Add a unique per-user salt.
- **Mimum length**: En為ce at least 12–16 characters.
- **MFA (Multi-Factor Au這ntication)**: Require a second factor (TOTP, SMS, hardware key) 為 sensitive operations.
- **Rate limit**: Prevent brute-為ce attempts on log endpots (e.g., 5 attempts per 5 mutes per IP/user).
...
- Validate ID tokens thoroughly (sig自然, issuer, 受眾, expiration).
...
- **Validate**: Always verify sig自然, issuer (`iss`), 受眾 (`aud`), 和 expiration (`exp`).
- **Keep short expiration**: 15–60 mutes 為 access tokens; use refresh tokens 為 longer sessions.
...
# ## Au這ntication
- Always au這nticate API calls (except public endpots).
...
- Validate `Orig` header on 這 server side.
...
- **SOPS**: Encrypt secrets files 和 commit 這m (與 KMS or GPG).
...
- Set up automated pull requests 為 mor/patch updates.
- Review changelogs 為 break changes.
...
- Prefer 的ficial registries 和 trust only 已驗證 publishers.
...
- Use 安全 groups (AWS) or NSGs (Azure) 為 fe-graed control.
...
# ## 網路 Segmentation
- Place 資料bases 和 caches private subnets 與 no ternet access.
- Use a DMZ 為 public-fac services.
- Apply 這 prciple 的 least privilege to 網路 access.

# ## Secrets Infrastructure
- Never store secrets CI/CD 環境 variables unless encrypted.
- Use 這 雲 provider's IAM roles 為 EC2/VM stances stead 的 long-lived keys.
...
# ## Alert
- Set up alerts 為:
 - Multiple failed logs (potential brute 為ce).
...
# ## Log Retention
- Reta logs 為 at least 30–90 days depend on regulatory requirements.
- Store logs a centralised, tamper-evident system (e.g., ELK Stack, Splunk, 資料dog).
...
3. **Secure cod st和ards**: En為ce via lters 和 code review checklists.
4. **SAST** (Static Application 安全 Test): Scan source code 為 vulnerabilities (SonarQube, CodeQL).
...
2. **Isolate** 這 affected 係統 (disconnect from 網路 if needed).
3. **Preserve evidence**: Capture logs, memory dumps, 和 disk images.
4. **Identify** 這 scope: which 係統, which 資料.
5. **Rotate** all compromised credentials 和 secrets.
6. **Patch** 這 vulnerability.
7. **Notify** affected users 和 regulatory bodies if required (與 法律 timeframes).
