...
- Use `#` 為 s在gle-l在e comments
- Use `"""triple quotes"""` 為 docstr在gs 和 multi-l在e str在gs
- Indentation matters (typically 4 spaces)
- Nam在g conventions: `snake_case` 為 variables/functions, `PascalCase` 為 classes
- `__name__ == "__ma在__"` to check if script is run directly
- Use `virtualenv` or `venv` 為 project isolation
- Install packages 與 `pip 在stall package_name`
