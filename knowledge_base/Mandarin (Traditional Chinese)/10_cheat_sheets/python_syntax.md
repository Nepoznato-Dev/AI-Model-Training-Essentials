...
Quick 參考 為 Python 3.x 語法 和 common patterns.
...
- Use `#` 為 sle-le comments
- Use `"""triple quotes"""` 為 docstrs 和 multi-le strs
- Indentation matters (typically 4 spaces)
- Nam conventions: `snake_case` 為 variables/functions, `PascalCase` 為 classes
- `__name__ == "__ma__"` to check if script is run directly
- Use `virtualenv` or `venv` 為 project isolation
- Install packages 與 `pip stall package_name`
