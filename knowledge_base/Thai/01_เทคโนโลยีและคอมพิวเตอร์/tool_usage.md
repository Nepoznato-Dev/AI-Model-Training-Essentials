...
- `settings.json` (ผู้ใช้หรือพื้นที่ทำงาน) ควบคุมพฤติกรรมของตัวแก้ไข
...
- `Ctrl+Alt+L` (macOS: `Cmd+Option+L`) ฟอร์แมตโค้ดใหม่
...
- `alias ll='ls -la'` สร้างทางลัด — เพิ่มไปที่ `~/.bashrc` หรือ `~/.zshrc`
