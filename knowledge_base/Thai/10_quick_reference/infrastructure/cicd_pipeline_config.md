...
| คำสำคัญ | คำอธิบาย |
|---------|-------------|
| `stages`| กำหนดขั้นตอนไปป์ไลน์และลำดับ |
| `stage`| มอบหมายงานให้กับสเตจ |
| `script`| คำสั่งที่จะดำเนินการ |
| `before_script`| คำสั่งทำงานก่อนสคริปต์หลัก |
| `after_script`| คำสั่งทำงานหลังจากสคริปต์หลัก (แม้จะล้มเหลว) |
| `only / except`| ควบคุมเวลาที่งานรัน (สาขา, แท็ก) |
| `rules`| เวอร์ชันที่ยืดหยุ่นมากขึ้นเท่านั้น/ยกเว้น |
| `variables`| กำหนดตัวแปร CI/CD |
| `cache`| ไฟล์แคชระหว่างไปป์ไลน์รัน |
| `artifacts`| ไฟล์ที่จะส่งผ่านระหว่างงาน |
| `environment`| สภาพแวดล้อมการปรับใช้ |
| `when`| ควบคุมการปฏิบัติงาน (on_success, on_failure, manual, เสมอ) |
| `needs`| ระบุการขึ้นต่อกันของงาน (โหมด DAG) |
| `extends`| สืบทอดการกำหนดค่าจากงานอื่น |
| `include`| นำเข้าไฟล์ YAML ภายนอก |
...
| **การทดสอบแบบขนาน** | เรียกใช้ชุดทดสอบแบบขนานเพื่อลดเวลาไปป์ไลน์ |
...
| **ชวา (กราเดิล)** | `~/.gradle/caches`| แคชพร้อมคีย์จาก`build.gradle`hash |
