...
# Swift - การอ้างอิงไวยากรณ์
เอกสารนี้ให้การอ้างอิงไวยากรณ์ที่มีโครงสร้างและครอบคลุมสำหรับ Swift (5.9+) มันช่วยเสริมการอ้างอิง Swift หลักโดยมุ่งเน้นไปที่รูปแบบไวยากรณ์ที่ละเอียดถี่ถ้วน ตัวเลือกเสริม การเขียนโปรแกรมเชิงโปรโตคอล และการทำงานพร้อมกันสมัยใหม่
---
...
|----------|-|---------|-------|
| `+``-``*``/``%`| เลขคณิต | `a + b`| |
| `..``...` | พิสัย | `1...10`| `...`ปิด; `..<`ครึ่งเปิด |
| `==``!=``<``>``<=``>=` | การเปรียบเทียบ | `a == b`| ต้องใช้`Equatable`/`Comparable`|
| `===``!==` | ตัวตน | `a === b`| การอ้างอิงเดียวกัน (คลาสเท่านั้น) |
| `&&``\|\|``!`| ตรรกะ | `a && b`| ลัดวงจร |
| `??`| ไม่มีการรวมตัว | `a ?? b`| ส่งกลับ`b`ถ้า`a`เป็น nil |
| `?.`| การผูกมัดเสริม | `a?.b`| ส่งคืน nil หาก`a`เป็น nil |
| `!`| บังคับแกะ | `a!`| ขัดข้องหากไม่มี — หลีกเลี่ยง |
| `&`| พารามิเตอร์ Inout | `swap(&a, &b)`| ผ่านการอ้างอิง |
### ไม่มีผู้ดำเนินการจัดการ
```swift
...
## สรุป
ไวยากรณ์ของ Swift สร้างความสมดุลระหว่างความปลอดภัยกับความหมาย ตัวเลือกช่วยขจัดข้อขัดข้องที่เป็นโมฆะ โปรโตคอลเปิดใช้งานองค์ประกอบที่ยืดหยุ่นโดยไม่มีการสืบทอด และการทำงานพร้อมกันสมัยใหม่ (async/await,acter) ทำให้สามารถเข้าถึงการเขียนโปรแกรมพร้อมกันที่ปลอดภัยได้ ภาษามีการพัฒนาอย่างต่อเนื่องด้วยคุณสมบัติต่างๆ เช่น ชุดพารามิเตอร์ มาโคร และความเป็นเจ้าของ ซึ่งแต่ละส่วนเพิ่มเติมทำให้ Swift ปลอดภัยและมีประสิทธิภาพยิ่งขึ้น ขณะเดียวกันก็รักษาคำมั่นสัญญาหลักไว้ นั่นคือโค้ดที่ปลอดภัยจากการสร้าง