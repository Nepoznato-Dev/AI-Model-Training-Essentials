...
เอกสารนี้รวบรวมข้อผิดพลาด กับดัก และรูปแบบการต่อต้านที่พบบ่อยที่สุดใน C++ แต่ละรายการจะแสดงแนวทางที่ไม่ถูกต้อง อธิบายว่าทำไมจึงล้มเหลว และให้แนวทางแก้ไขที่ถูกต้อง Modern C++ (C++11/14/17/20) มีเครื่องมือเพื่อหลีกเลี่ยงข้อผิดพลาดแบบคลาสสิกส่วนใหญ่
---

## 1. กฎสามหรือห้าการละเมิด
```cpp
...
## 2. Raw`new`/`delete`แทนที่จะเป็นตัวชี้อัจฉริยะ
```cpp
...
## 6. ไม่ได้ใช้`std::move`อย่างถูกต้อง
```cpp
...
## 7. การต่อต้านรูปแบบ: การใช้`using namespace std;`ในส่วนหัว
```cpp
...
## 8. พฤติกรรมที่ไม่ได้กำหนดจาก Signed Overflow
```cpp
...
## 10. การต่อต้านรูปแบบ: การจัดการทรัพยากรด้วยตนเอง (Pre-RAII)
```cpp
...
## 11. เทมเพลตขยายและเวลาคอมไพล์แบบยาว
```cpp
...
## 12. ไม่ได้ใช้`[[nodiscard]]`และ `[[maybe_unused]]`
```cpp
...
## สรุป
ความซับซ้อนของ C++ ทำให้เกิดข้อผิดพลาดมากมาย: กฎข้อที่ห้า, การแบ่งส่วนวัตถุ, ตัววนซ้ำใช้ไม่ได้, การอ้างอิงที่ค้างอยู่ และพฤติกรรมที่ไม่ได้กำหนดไว้จากการโอเวอร์โฟลว์ที่เซ็นชื่อ Modern C++ เป็นยาแก้พิษ: ใช้พอยน์เตอร์อัจฉริยะแทน raw`new`/`delete`, RAII สำหรับทรัพยากรทั้งหมด,`std::vector`และ`std::string`แทนอาร์เรย์ดิบ, ย้ายซีแมนทิกส์เพื่อประสิทธิภาพ และ`[[nodiscard]]`เพื่อจับค่าที่ส่งคืนที่ถูกละเว้น หลักเกณฑ์หลักของ C++ มีเหตุผล — ปฏิบัติตามและใช้เครื่องมือ เช่น Clang-Tidy และน้ำยาฆ่าเชื้อเพื่อตรวจจับสิ่งที่คุณพลาด