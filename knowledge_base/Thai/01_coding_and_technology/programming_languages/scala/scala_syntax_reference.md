...
|----------|-|---------|-------|
| `+``-``*``/``%`| เลขคณิต | `a + b`| |
| `**`| ยกกำลัง (ทศนิยมใหญ่) | `BigDecimal(2) ** 10`| ไม่มีในตัวสำหรับ Int |
| `==``!=` | ความเท่าเทียมกัน | `a == b`| เรียก`.equals()`— null-safe |
| `eq``ne` | ความเท่าเทียมกันอ้างอิง | `a eq b`| อินสแตนซ์เดียวกัน |
| `<``>``<=``>=` | การเปรียบเทียบ | `a >= b`| ต้องใช้`Ordered`|
| `&&``\|\|``!`| ตรรกะ | `a && b`| ลัดวงจร |
| `&``\|``^``~` | ระดับบิต | `a & b`| |
| `<<``>>``>>>`| กะ | `a << 2`| |
| `+:``:+``::``:::` | ปฏิบัติการคอลเลกชัน | `1 +: list`| การเชื่อมโยงขวาหากลงท้ายด้วย`:`|
| `->`| การสร้างทูเพิล | `"key" -> "value"`| `("key", "value")`|
| `=>`| ฟังก์ชัน/ลูกศร | `x => x + 1`| แลมบ์ดาหรือการแมปประเภท |
| `<-`| เครื่องกำเนิดไฟฟ้า | `x <- List(1,2,3)`| เพื่อความเข้าใจ |
### กฎของผู้ปฏิบัติงาน
```scala
...
## สรุป
ไวยากรณ์ของ Scala รวมการเขียนโปรแกรมเชิงวัตถุและเชิงฟังก์ชันเข้าด้วยกันภายใต้ระบบประเภทที่ทรงพลัง การจับคู่รูปแบบ เพื่อความเข้าใจ และคลาสเคสให้การสร้างแบบจำลองข้อมูลที่ชัดเจน ลักษณะช่วยให้สามารถจัดองค์ประกอบได้อย่างยืดหยุ่นโดยไม่มีข้อจำกัดในการสืบทอดเดี่ยว ระบบประเภท — พร้อมด้วยข้อมูลทั่วไป ความแปรปรวน ขอบเขตประเภท และประเภททึบ — จับข้อผิดพลาดในเวลาคอมไพล์ ในขณะที่เปิดใช้งานนามธรรมที่ไม่มีค่าใช้จ่าย Scala 3 ทำให้ภาษาง่ายขึ้นด้วยการแจกแจง วิธีการขยาย การให้/การใช้ และประเภททึบ ทำให้ภาษาเข้าถึงได้ง่ายขึ้นในขณะที่ยังคงประสิทธิภาพเต็มที่