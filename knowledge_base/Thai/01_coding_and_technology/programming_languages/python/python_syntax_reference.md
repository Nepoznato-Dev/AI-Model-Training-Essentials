...
# Python - การอ้างอิงไวยากรณ์
เอกสารนี้ให้การอ้างอิงไวยากรณ์ที่มีโครงสร้างและครอบคลุมสำหรับ Python มันช่วยเสริมการอ้างอิง Python หลักโดยเน้นไปที่รูปแบบไวยากรณ์ที่ละเอียดถี่ถ้วน ตารางตัวดำเนินการ และกลไกภายใน
---
...
|----------|-|---------|--------|-------|
| `+`| นอกจากนี้ | `3 + 2`| `5`| การต่อสตริง/รายการด้วย |
| `-`| การลบ | `3 - 2`| `1`| การปฏิเสธเอกพจน์ด้วย |
| `*`| การคูณ | `3 * 2`| `6`| การทำซ้ำสตริง / รายการด้วย |
| `/`| ทรูดิชั่น | `7 / 2`| `3.5`| ส่งคืน float | เสมอ
| `//`| กองพื้น | `7 // 2`| `3`| ปัดเศษไปทางลบอนันต์ |
| `%`| โมดูลัส | `7 % 2`| `1`| เครื่องหมายตรงกับตัวหาร |
| `**`| การยกกำลัง | `2 ** 10`| `1024`| ขวาเชื่อมโยง |
### การเปรียบเทียบและตัวดำเนินการบูลีน
| ตัวดำเนินการ | ชื่อ | ตัวอย่าง | หมายเหตุ |
|----------|-|---------|-------|
| `==`| เท่ากับ | `x == y`| การเปรียบเทียบค่า (ไม่ใช่ตัวตน) |
| `!=`| ไม่เท่ากัน | `x != y`| |
| `<`,`>`,`<=`,`>=`| การสั่งซื้อ | `x >= y`| เชื่อมต่อได้:`a < b < c`|
| `is`| ตัวตน | `x is y`| วัตถุเดียวกันในหน่วยความจำ |
| `is not`| ไม่ใช่ตัวตน | `x is not None`| เป็นที่นิยมมากกว่า`!= None`|
| `in`| สมาชิก | `x in collection`| O(1) สำหรับชุด/คำสั่ง, O(n) สำหรับรายการ |
| `and`| ตรรกะและ | `a and b`| ลัดวงจร; ส่งกลับค่าความจริง/เท็จล่าสุด |
| `or`| ตรรกะหรือ | `a or b`| ลัดวงจร; ส่งกลับค่าความจริง/เท็จแรก |
| `not`| ตรรกะไม่ใช่ | `not x`| |
...
| `\|`| หรือ | `5 \| 3`| `7`|
| `^`| แฮคเกอร์ | `5 ^ 3`| `6`|
| `~`| ไม่ | `~5`| `-6`(ส่วนเสริมสองตัว) |
| `<<`| เลื่อนซ้าย | `5 << 1`| `10`|
| `>>`| เลื่อนไปทางขวา | `5 >> 1`| `2`|
### ลำดับความสำคัญของตัวดำเนินการ (สูงสุดไปต่ำสุด)
| ลำดับความสำคัญ | ตัวดำเนินการ | สมาคม |
|------------|-----------|---------------|
| 1 (สูงสุด) | `()`| การจัดกลุ่ม |
| 2 | `**`| ขวา |
| 3 | `~`,`+x`,`-x`| อูนารี |
| 4 | `*`,`/`,`//`,`%`| ซ้าย |
| 5 | `+`,`-`| ซ้าย |
...
| 12 | `and`| ซ้าย |
| 13 (ต่ำสุด) | `or`| ซ้าย |
### การมอบหมายและการมอบหมายเพิ่มเติม
```python
...
## ฟังก์ชั่นและการปิด
### คำจำกัดความของฟังก์ชันและการโทร
```python
...
## โครงสร้างข้อมูล
### ภาพรวมคอลเลกชันในตัว
| คอลเลกชัน | สั่งซื้อ | ไม่แน่นอน | ซ้ำกัน | ไวยากรณ์ | ค้นหา |
|------------|---------|---------|------------|--------|--------|
| `list`| ใช่ | ใช่ | ใช่ | `[1, 2, 3]`| โอ(n) |
| `tuple`| ใช่ | ไม่ | ใช่ | `(1, 2, 3)`| โอ(n) |
| `dict`| ใช่ (3.7+) | ใช่ | คีย์: ไม่ | `{"k": "v"}`| O(1) เฉลี่ย |
| `set`| ไม่ | ใช่ | ไม่ | `{1, 2, 3}`| O(1) เฉลี่ย |
| `frozenset`| ไม่ | ไม่ | ไม่ | `frozenset({1,2})`| O(1) เฉลี่ย |
### รายการ
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
## สรุป
การอ้างอิงไวยากรณ์นี้ครอบคลุมชุดคุณลักษณะทั้งหมดของ Python ตั้งแต่ตัวดำเนินการพื้นฐานและโฟลว์การควบคุมไปจนถึงการเขียนโปรแกรมเมตาขั้นสูง ตัวอธิบาย และระบบประเภท ไวยากรณ์ของ Python นั้นจงใจให้น้อยที่สุดและสามารถอ่านได้ แต่ความลึกของมันนั้นมีความสำคัญ: การจับคู่รูปแบบ ตัวสร้างอะซิงก์ โปรโตคอล และเมตาคลาส มอบเครื่องมือที่ทรงพลังสำหรับแอปพลิเคชันที่ซับซ้อน ภาษามีการพัฒนาอย่างต่อเนื่อง โดยแต่ละรุ่นจะเพิ่มฟีเจอร์ที่ปรับปรุงความหมายในขณะที่ยังคงความเข้ากันได้แบบย้อนหลัง