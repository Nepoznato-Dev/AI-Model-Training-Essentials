...
---
# Python — ข้อผิดพลาดทั่วไปและการต่อต้านรูปแบบ
เอกสารนี้รวบรวมข้อผิดพลาด กับดัก และรูปแบบการต่อต้านที่พบบ่อยที่สุดใน Python แต่ละรายการจะแสดงแนวทางที่ไม่ถูกต้อง อธิบายว่าทำไมจึงล้มเหลว และให้แนวทางแก้ไขที่ถูกต้อง การทำความเข้าใจข้อผิดพลาดเหล่านี้จะช่วยให้คุณเขียนโค้ด Pythonic ที่แข็งแกร่งยิ่งขึ้นได้
---

## 1. อาร์กิวเมนต์เริ่มต้นที่ไม่แน่นอน
```python
...
**เหตุใดจึงเกิดขึ้น:** อาร์กิวเมนต์เริ่มต้นจะได้รับการประเมินหนึ่งครั้งในเวลาการกำหนดฟังก์ชัน ไม่ใช่ในการโทรแต่ละครั้ง ค่าเริ่มต้นที่ไม่แน่นอน (รายการ, dict, ชุด) จะถูกแชร์ในการร้องขอทั้งหมด
---

## 2. การแก้ไขรายการขณะวนซ้ำ
```python
...
## 5. ไม่ใช้ตัวจัดการบริบทสำหรับทรัพยากร
```python
...
## 6 ตัวดำเนินการ`is`สำหรับการเปรียบเทียบค่า
```python
...
## 8. ไม่เข้าใจกฎขอบเขตของ Python (LEGB)
```python
...
## 10. การต่อต้านรูปแบบ: การใช้`except:`Bare ยกเว้น
```python
...
## 11. การส่งคืนการอ้างอิงที่ไม่แน่นอนไปยังสถานะภายใน
```python
...
## 12. ไม่ได้ใช้`setdefault`หรือ `collections.defaultdict`
```python
...
## 14. ไม่เข้าใจความอ่อนล้าของเครื่องปั่นไฟ
```python
...
## 15. การต่อต้านรูปแบบ: วัตถุพระเจ้า / คลาสที่โอเวอร์โหลด
```python
...
## สรุป
ความเรียบง่ายของ Python นั้นหลอกลวง ค่าเริ่มต้นที่ไม่แน่นอน การปิดการเชื่อมโยงล่าช้า กฎขอบเขต และการจัดการทรัพยากร ล้วนมีพฤติกรรมที่ละเอียดอ่อนที่ดักจับนักพัฒนา หลักการสำคัญที่ต้องจำ: ค่าเริ่มต้นจะได้รับการประเมินเพียงครั้งเดียว`is`ตรวจสอบข้อมูลประจำตัวที่ไม่เท่าเทียมกัน ใช้ตัวจัดการบริบทสำหรับทรัพยากรเสมอ จับข้อยกเว้นเฉพาะ และสนับสนุนการจัดองค์ประกอบเหนือวัตถุพระเจ้า การเขียนโค้ด Pythonic หมายถึงการทำความเข้าใจข้อผิดพลาดเหล่านี้และการใช้คุณลักษณะของภาษา (ตัวจัดการบริบท ความเข้าใจ ตัวสร้าง โมดูล `collections`) ตามที่ตั้งใจไว้