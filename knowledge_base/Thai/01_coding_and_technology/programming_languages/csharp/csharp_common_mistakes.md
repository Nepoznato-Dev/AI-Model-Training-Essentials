...
เอกสารนี้รวบรวมข้อผิดพลาด กับดัก และรูปแบบการต่อต้านที่พบบ่อยที่สุดใน C# แต่ละรายการจะแสดงแนวทางที่ไม่ถูกต้อง อธิบายว่าทำไมจึงล้มเหลว และให้แนวทางแก้ไขที่ถูกต้อง
---

## 1. ไม่ทิ้งวัตถุ `IDisposable`
```csharp
// ❌ WRONG — resource leak
...
## 4. LINQ`.ToList()`การจัดสรรที่ไม่จำเป็น
```csharp
...
## 7. รูปแบบการต่อต้าน: คลาสพระเจ้า / ตัวระบุตำแหน่งบริการ
```csharp
...
## 8. ข้อยกเว้นการอ้างอิงที่เป็นโมฆะ
```csharp
...
## 10. ไม่ใช้บันทึกสำหรับประเภทข้อมูล
```csharp
...
## 11. การใช้ตัวกรองข้อยกเว้นในทางที่ผิด
```csharp
...
## สรุป
C# เป็นภาษาที่ได้รับการออกแบบมาอย่างดี แต่มีกับดัก: ไม่ทิ้งวัตถุ `IDisposable`, วิธี async void, การจัดสรร LINQ ที่ไม่จำเป็น และข้อยกเว้นการอ้างอิงที่เป็นโมฆะ Modern C# (8+) มีเครื่องมืออันทรงพลัง: ประเภทการอ้างอิงที่เป็นโมฆะ, การจับคู่รูปแบบ, บันทึก, การใช้การประกาศ และนิพจน์สวิตช์ หลักการสำคัญ: กำจัดทรัพยากรเสมอ ห้ามใช้ async void (ยกเว้นตัวจัดการเหตุการณ์) ใช้ Constructor Injection ชอบบันทึกข้อมูล และเปิดใช้งานประเภทการอ้างอิงที่เป็น Nullable เพื่อตรวจจับจุดบกพร่องที่เป็น Null ณ เวลารวบรวม