...
-->
# TypeScript — ข้อผิดพลาดทั่วไปและการต่อต้านรูปแบบ
เอกสารนี้รวบรวมข้อผิดพลาด กับดัก และรูปแบบการต่อต้านที่พบบ่อยที่สุดใน TypeScript แต่ละรายการจะแสดงแนวทางที่ไม่ถูกต้อง อธิบายว่าทำไมจึงล้มเหลว และให้แนวทางแก้ไขที่ถูกต้อง
---
...
## 2. พิมพ์การยืนยันแทน Type Guards
```typescript
...
## 3. การละเมิดการยืนยันที่ไม่ใช่โมฆะ
```typescript
...
## 4. อินเทอร์เฟซและความสับสนประเภท
```typescript
...
## 7 ความประหลาดใจในการพิมพ์โครงสร้าง
```typescript
...
## 8. ข้อผิดพลาดประเภท Async/Await
```typescript
...
## 9. Anti-Pattern: ประเภทการทำซ้ำ (ไม่ใช้ Generics)
```typescript
...
## 10. การผูกมัดเพิ่มเติมที่มากเกินไป
```typescript
...
## 12. ไม่ใช้ประโยชน์จากสหภาพที่เลือกปฏิบัติ
```typescript
...
## สรุป
ระบบประเภทของ TypeScript มีประสิทธิภาพแต่ต้องมีระเบียบวินัย บาปสำคัญกำลังเข้าถึง`any`— ใช้`unknown`เมื่อไม่ทราบประเภทอย่างแท้จริง และจำกัดให้แคบลงด้วย Type Guard หลีกเลี่ยงการยืนยันที่ไม่ใช่ค่าว่าง (`!`) โดยการจัดการ undefinition อย่างถูกต้อง ใช้อินเทอร์เฟซสำหรับรูปร่างของวัตถุ ประเภทสำหรับสหภาพ และประเภทที่คำนวณ สหภาพแรงงานที่ถูกเลือกปฏิบัติจะกำจัดข้อบกพร่องทั้งหมด เปิดใช้งาน`strict: true`ตั้งแต่วันแรก — ความเข้มงวดในการปรับปรุงใหม่เป็นเรื่องที่เจ็บปวด เป้าหมายคือการทำให้รัฐที่ผิดกฎหมายไม่สามารถเป็นตัวแทนได้ในระดับประเภท