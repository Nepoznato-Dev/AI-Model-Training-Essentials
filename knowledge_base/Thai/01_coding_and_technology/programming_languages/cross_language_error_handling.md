...
| รุ่น | ภาษา | กลไก |
|-------|-----------|-----------|
| **ข้อยกเว้น (ลอง/จับ)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **ประเภทผลลัพธ์/ตัวเลือก** | Rust, Haskell, OCaml, Scala (เช่น) | `Result<T, E>`,`Maybe a`|
| **ค่าส่งคืนหลายค่า** | ไปเถอะ ลัวะ | `value, err := f()`|
| **รหัสข้อผิดพลาด** | C, ฟอร์ทราน | คืนรหัสข้อผิดพลาด ตรวจสอบด้วยตนเอง |
| **ข้อผิดพลาดในการจับคู่รูปแบบ** | น้ำอมฤต, เออร์แลง | `{:ok, value}`/`{:error, reason}`|
| **เงื่อนไข (รีสตาร์ทได้)** | เสียงกระเพื่อมทั่วไป | `restart-case`,`handler-bind`|
| **ไม่มีการจัดการข้อผิดพลาด** | รอยขีดข่วน, SQL (พื้นฐาน) | ไม่มี |
| **สัญญา** | เอด้า, ไอเฟล | `Pre`,`Post`เงื่อนไข |
## การจัดการข้อผิดพลาดตามข้อยกเว้น
```python
# Python
try:
    result = 10 / 0
...
## สรุป: กระบวนทัศน์การจัดการข้อผิดพลาด
| กระบวนทัศน์ | วิธีการ | ข้อดี | ข้อเสีย |
|----------|----------|-------|-|
| **ข้อยกเว้น** | โยน/จับ | ทำความสะอาดเส้นทางแห่งความสุข ร่องรอยสแต็ก | สามารถลืมได้ ควบคุมการละเมิดการไหล |
| **ประเภทผลลัพธ์** | กลับ`Ok`/`Err`| ชัดเจน ไม่สามารถเพิกเฉยต่อข้อผิดพลาด | Verbose ต้องการตัวดำเนินการ`?`|
| **รหัสข้อผิดพลาด** | กลับสถานะ int | เรียบง่าย รองรับ C | เพิกเฉยได้ง่าย ไม่มีบริบท |
| **คืนสินค้าได้หลายครั้ง** | `value, err := f()`| ชัดเจน สำนวน | ลืมตรวจสอบ`err`| ได้เลย
| **ลายเข้ากัน** | `{:ok, v}`/`{:error, r}`| ครบถ้วนสมบูรณ์ | ใช้งานได้จริง | รายละเอียดที่ไม่มี`with`|
| **เงื่อนไข** | ตัวจัดการที่รีสตาร์ทได้ | ทรงพลังที่สุดที่สามารถกู้คืนได้ | ซับซ้อน ไม่ค่อยเข้าใจ |