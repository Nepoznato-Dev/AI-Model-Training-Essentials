...
-->
# Kotlin — ข้อผิดพลาดทั่วไปและการต่อต้านรูปแบบ
เอกสารนี้รวบรวมข้อผิดพลาด กับดัก และรูปแบบการต่อต้านที่พบบ่อยที่สุดใน Kotlin แต่ละรายการจะแสดงแนวทางที่ไม่ถูกต้อง อธิบายว่าทำไมจึงล้มเหลว และให้แนวทางแก้ไขที่ถูกต้อง
---

## 1. ประเภทแพลตฟอร์มและ NullPointerException
```kotlin
...
## 3.`data class`โดยไม่มี`equals`/`hashCode`ที่เหมาะสม
```kotlin
...
## 4. ไม่ใช้ฟังก์ชันขอบเขตอย่างถูกต้อง
```kotlin
...
## 5. Coroutines: ไม่จัดการกับการยกเลิก
```kotlin
...
## 6. ต่อต้านรูปแบบ: Kotlin สไตล์ Java
```kotlin
...
## 9. ไม่ใช้`by lazy`สำหรับการเริ่มต้นที่มีราคาแพง
```kotlin
...
## 10. การไหลเทียบกับความสับสนของ LiveData
```kotlin
...
## สรุป
ความกระชับของ Kotlin สามารถซ่อนปัญหาเล็กๆ น้อยๆ ได้ เช่น ประเภทแพลตฟอร์มจากการทำงานร่วมกันของ Java,`lateinit`ขัดข้อง, คลาสข้อมูลที่เปลี่ยนแปลงไม่ได้ซึ่งทำลายคอลเลกชัน และการยกเลิกโครูทีน วิธี Kotlin คือ: ยอมรับความปลอดภัยแบบ null (อย่าใช้`!!`), ใช้ฟังก์ชันขอบเขตที่ถูกต้อง (`let`สำหรับการแปลง,`apply`สำหรับการกำหนดค่า,`also`สำหรับผลข้างเคียง), ใช้คลาสที่ปิดผนึกด้วย`when`ครบถ้วนสมบูรณ์, จัดการการยกเลิก coroutine และเขียน Kotlin สำนวน (ไม่ใช่ Java ที่มีไวยากรณ์ Kotlin) คอมไพเลอร์คือแนวทางของคุณ — หากคำเตือนก็จงฟัง