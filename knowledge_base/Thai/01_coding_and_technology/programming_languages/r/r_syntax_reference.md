...
---
# R - การอ้างอิงไวยากรณ์
เอกสารนี้ให้การอ้างอิงไวยากรณ์ที่มีโครงสร้างและครอบคลุมสำหรับ R (4.x) มันเสริมการอ้างอิง R หลักโดยมุ่งเน้นไปที่รูปแบบไวยากรณ์ที่ละเอียดถี่ถ้วน ระบบนิเวศที่เป็นระเบียบ การจัดการข้อมูล การสร้างแบบจำลองทางสถิติ และการแสดงภาพ
---
...
|----------|-|---------|-------|
| `+``-``*``/``^`| เลขคณิต | `2^10`| |
| `%%`| โมดูลัส | `7 %% 3`| ส่งคืน 1 |
| `%/%`| การหารจำนวนเต็ม | `7 %/% 3`| ส่งคืน 2 |
| `==``!=` | ความเท่าเทียมกัน | `x == y`| เวกเตอร์ |
| `<``>``<=``>=` | การเปรียบเทียบ | `x >= y`| เวกเตอร์ |
| `&``\|``!`| ตรรกะ (เวกเตอร์) | `x & y`| องค์ประกอบที่ชาญฉลาด |
| `&&``\|\|` | ตรรกะ (สเกลาร์) | `x && y`| องค์ประกอบแรกเท่านั้น |
| `%in%`| สมาชิก | `x %in% y`| |
| `<-``=` | การมอบหมายงาน | `x <- 10`| `<-`เป็นสำนวน |
| `->`| กำหนดขวา | `10 -> x`| ไม่ค่อยได้ใช้ |
| `<<-`| มอบหมายทั่วโลก | `x <<- 10`| กำหนดใน parent env |
| `$`| แยกองค์ประกอบ | `df$column`| |
| `[``[[` | การจัดทำดัชนี | `x[1]`/`x[[1]]`| `[`ส่งคืนประเภทเดียวกัน `[[`สารสกัด |
| `%>%`| ท่อ (magrittr) | `x %>% f()`| |
| `\|>`| ท่อ (ฐาน R 4.1+) | `x \|> f()`| ไปป์พื้นเมือง |
...
## สรุป
ไวยากรณ์ของ R ได้รับการออกแบบมาเพื่อการคำนวณทางสถิติและการวิเคราะห์ข้อมูล ลักษณะการดำเนินการแบบเวกเตอร์ช่วยลดการวนซ้ำสำหรับงานส่วนใหญ่ ระบบนิเวศที่เป็นระเบียบเรียบร้อย (dplyr, ggplot2, tidyr) มอบไวยากรณ์ที่สอดคล้องกันและอ่านง่ายสำหรับการจัดการข้อมูลและการแสดงภาพ ความสามารถในการสร้างแบบจำลองทางสถิติของ R ตั้งแต่การทดสอบทีพื้นฐานไปจนถึงโมเดลเอฟเฟกต์แบบผสมนั้นไม่มีที่เปรียบในเชิงลึก สำหรับวิทยาศาสตร์ข้อมูล สถิติ และการวิจัยที่สามารถทำซ้ำได้ R ยังคงเป็นเครื่องมือสำคัญ