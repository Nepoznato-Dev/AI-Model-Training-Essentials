...
# Rust - การอ้างอิงไวยากรณ์
เอกสารนี้ให้การอ้างอิงไวยากรณ์ที่มีโครงสร้างและครอบคลุมสำหรับ Rust มันเสริมการอ้างอิง Rust หลักโดยมุ่งเน้นไปที่รูปแบบไวยากรณ์ที่ละเอียดถี่ถ้วน ตารางตัวดำเนินการ และกลไกภายในของระบบความเป็นเจ้าของ ลักษณะ และการทำงานพร้อมกัน
---
...
|----------|-|---------|-------|
| `+`| นอกจากนี้ | `3 + 2`| นอกจากนี้`String`เชื่อมต่อผ่าน`format!`|
| `-`| การลบ | `3 - 2`| |
| `*`| การคูณ | `3 * 2`| |
| `/`| กอง | `7 / 2`| การหารจำนวนเต็มสำหรับจำนวนเต็ม:`3`|
| `%`| ส่วนที่เหลือ | `7 % 2`| `1`|
| `-x`| การปฏิเสธ | `-5`| |
### การเปรียบเทียบและตัวดำเนินการเชิงตรรกะ
| ตัวดำเนินการ | ชื่อ | ตัวอย่าง | หมายเหตุ |
|----------|-|---------|-------|
| `==`| เท่ากับ | `x == y`| ต้องใช้`PartialEq`|
| `!=`| ไม่เท่ากัน | `x != y`| |
| `<`,`>`,`<=`,`>=`| การสั่งซื้อ | `x >= y`| ต้องใช้`PartialOrd`|
...
| `\|`| หรือ | `5 \| 3`| `7`|
| `^`| แฮคเกอร์ | `5 ^ 3`| `6`|
| `!`| ไม่ | `!0b1010u8`| ส่วนเสริม Bitwise |
| `<<`| เลื่อนซ้าย | `5 << 1`| `10`|
| `>>`| เลื่อนไปทางขวา | `5 >> 1`| `2`|
### ลำดับความสำคัญของตัวดำเนินการ (สูงสุดไปต่ำสุด)
| ลำดับความสำคัญ | ตัวดำเนินการ | หมายเหตุ |
|------------|-----------|-------|
| 1 (สูงสุด) |  เส้นทาง`()``.` :: | |
| 2 | การเรียกเมธอด การเข้าถึงฟิลด์ | ยกเลิกอัตโนมัติ |
| 3 | `-`(ปฏิเสธ),`!`,`*`(ยกเลิก) | |
| 4 | `as`(แบบหล่อ) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
| 13 | `\|\|`| |
| 14 | `..``..=` | พิสัย |
| 15 | `=``+=` ฯลฯ | การมอบหมายงาน |
| 16 (ต่ำสุด) | `return``break``continue`| |
---
...
## โครงสร้างข้อมูล
### ภาพรวมประเภทในตัว
| พิมพ์ | สแต็ก/ฮีป | ขนาด | ความไม่แน่นอน | หมายเหตุ |
|------|-----------|-|-------------|-------|
| พื้นฐาน (`i32`,`f64`,`bool`,`char`) | สแต็ค | แก้ไข | ไม่เปลี่ยนรูปตามค่าเริ่มต้น | `Copy`|
| `[T; N]`(อาร์เรย์) | สแต็ค | แก้ไข | ไม่แน่นอนด้วย`mut`| ขนาดคงที่`Copy`ถ้า T คือ`Copy`|
| `Vec<T>`| ฮีป | ไดนามิก | ไม่แน่นอนด้วย`mut`| ปลูกได้ |
| `String`| ฮีป | ไดนามิก | ไม่แน่นอนด้วย`mut`| UTF-8, เติบโตได้ |
| `&[T]`(สไลซ์) | สแต็ค (ไขมัน ptr) | มุมมองแบบไดนามิก | ขึ้นอยู่กับ`&`หรือ`&mut`| มุมมองที่ยืมมา |
| `HashMap<K, V>`| ฮีป | ไดนามิก | ไม่แน่นอนด้วย`mut`| O(1) การค้นหาเฉลี่ย |
| `BTreeMap<K, V>`| ฮีป | ไดนามิก | ไม่แน่นอนด้วย`mut`| ค้นหา O (log n) สั่งซื้อ |
| `HashSet<T>`| ฮีป | ไดนามิก | — | O(1) เฉลี่ย, ไม่ซ้ำกัน |
### เวกเตอร์
```rust
// Creation
...
### คำจำกัดความลักษณะและการนำไปใช้
```rust
// Define a trait
trait Summary {
...
## สรุป
ไวยากรณ์ของ Rust ถูกสร้างขึ้นบนพื้นฐานของการรับประกันความปลอดภัยที่บังคับใช้ในเวลารวบรวม ระบบความเป็นเจ้าของ ความหลากหลายตามคุณลักษณะ การจับคู่รูปแบบ และนามธรรมที่ไม่มีต้นทุน ก่อให้เกิดการออกแบบที่สอดคล้องกัน โดยที่คอมไพเลอร์เป็นพันธมิตรที่เข้มงวดแต่ยุติธรรม ตั้งแต่โฟลว์การควบคุมพื้นฐานไปจนถึงฟีเจอร์ขั้นสูง เช่น มาโคร โค้ดที่ไม่ปลอดภัย และตัวชี้อัจฉริยะ ทุกโครงสร้างตอบสนองเป้าหมายของการเขียนโปรแกรมระบบที่ปลอดภัยและมีประสิทธิภาพ การทำความเข้าใจความสัมพันธ์ระหว่างช่วงอายุ การยืม และระบบประเภทเป็นกุญแจสำคัญในการฝึกฝน Rust