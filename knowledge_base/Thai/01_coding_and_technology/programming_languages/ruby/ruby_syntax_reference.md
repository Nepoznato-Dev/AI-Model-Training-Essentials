...
-->
# Ruby - การอ้างอิงไวยากรณ์
เอกสารนี้ให้การอ้างอิงไวยากรณ์ที่มีโครงสร้างและครอบคลุมสำหรับ Ruby (3.x) มันเสริมการอ้างอิง Ruby หลักโดยมุ่งเน้นไปที่รูปแบบไวยากรณ์ที่ละเอียดถี่ถ้วน บล็อกและการปิด การเขียนโปรแกรมเมตา และสำนวน Ruby
---
...
|----------|-|---------|-------|
| `+``-``*``/``%``**` | เลขคณิต | `2 ** 10`| `**`เป็นการยกกำลัง |
| `==``!=``===``<=>` | ความเท่าเทียมกัน | `a == b`| `===`คือความเท่าเทียมกันของขนาดตัวพิมพ์ `<=>`คือ ยานอวกาศ |
| `eql?`| ความเท่าเทียมกันของมูลค่า | `a.eql?(b)`| เข้มงวดกว่า`==`(ประเภทเช็ค) |
| `equal?`| ตัวตน | `a.equal?(b)`| วัตถุเดียวกัน? |
| `<``>``<=``>=` | การเปรียบเทียบ | `a >= b`| |
| `&&``\|\|``!`| ตรรกะ | `a && b`| ลัดวงจร; ส่งกลับค่าประเมินล่าสุด |
| `and``or``not`| ตรรกะ (ลำดับความสำคัญต่ำ) | `a and b`| หลีกเลี่ยง — ใช้`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | ระดับบิต | `a & b`| |
| `=~``!~` | การจับคู่ Regex | `str =~ /pattern/`| ส่งกลับดัชนีหรือไม่มี |
| `..``...` | พิสัย | `1..10`|  รวม `..`; `...`พิเศษ |
| `?.`| การนำทางอย่างปลอดภัย | `user&.name`| ส่งคืน n หากผู้ใช้เป็น nil |
| `[]``[]=` | การเข้าถึงองค์ประกอบ | `arr[0]`| |
###ความจริง
```ruby
...
## สรุป
ไวยากรณ์ของ Ruby ได้รับการออกแบบมาเพื่อความสุขของนักพัฒนา — อ่านได้เหมือนภาษาอังกฤษและให้รางวัลกับการแสดงออก พลังของภาษามาจากโมเดลออบเจ็กต์ (ทุกอย่างเป็นออบเจ็กต์) การบล็อกและการปิด ความสามารถในการเขียนโปรแกรมเมตา และโมดูล Enumerable ที่ให้ API การประมวลผลคอลเลกชันที่หลากหลาย Ruby 3.x ได้เพิ่มการจับคู่รูปแบบ ลายเซ็นประเภท RBS และ Ractor สำหรับการทำงานพร้อมกัน ในขณะที่ยังคงรักษาปรัชญาหลักของภาษา: ทำให้โปรแกรมเมอร์มีประสิทธิผลและมีความสุข