...
เอกสารนี้ให้การอ้างอิงไวยากรณ์ที่มีโครงสร้างและครอบคลุมสำหรับ C (C11/C17 พร้อมหมายเหตุ C23) ช่วยเสริมการอ้างอิง C หลักโดยมุ่งเน้นไปที่รูปแบบไวยากรณ์ที่ละเอียดถี่ถ้วน ตารางตัวดำเนินการ กลไกของตัวชี้ และการจัดการหน่วยความจำ
---
...
| `*`| การคูณ | `3 * 2`| `6`| |
| `/`| กอง | `7 / 2`| `3`| การตัดทอนจำนวนเต็มสำหรับประเภท int |
| `%`| โมดูลัส | `7 % 2`| `1`| สำหรับประเภทจำนวนเต็มเท่านั้น |
| `++`| เพิ่มขึ้น | `i++`/`++i`| | หลังและก่อนเพิ่ม |
| `--`| ลดลง | `i--`/`--i`| | หลังและก่อนการลดลง |
...
| `\|`| หรือ | `5 \| 3`| `7`|
| `^`| แฮคเกอร์ | `5 ^ 3`| `6`|
| `~`| ส่วนประกอบ | `~0`| บิตทั้งหมดพลิก |
| `<<`| เลื่อนซ้าย | `1 << 3`| `8`|
| `>>`| เลื่อนไปทางขวา | `8 >> 3`| `1`(กำหนดการใช้งานสำหรับการลงนาม) |
### ตัวดำเนินการตัวชี้และที่อยู่
| ตัวดำเนินการ | ชื่อ | ตัวอย่าง | หมายเหตุ |
|----------|-|---------|-------|
| `&`| ที่อยู่ของ | `&x`| ส่งกลับตัวชี้ไปที่ x |
| `*`| ขอแสดงความนับถือ | `*ptr`| เข้าถึงค่าที่ตัวชี้ |
| `->`| การเข้าถึงของสมาชิกผ่านตัวชี้ | `p->x`| เทียบเท่ากับ`(*p).x`|
| `.`| การเข้าถึงของสมาชิก | `s.field`| การเข้าถึงสมาชิก struct โดยตรง |
| `[]`| ตัวห้อยอาร์เรย์ | `arr[i]`| เทียบเท่ากับ`*(arr + i)`|
### ลำดับความสำคัญของตัวดำเนินการ (สูงสุดไปต่ำสุด)
| ลำดับความสำคัญ | ตัวดำเนินการ | สมาคม |
|------------|-----------|---------------|
| 1 (สูงสุด) | `()``[]``->``.` | จากซ้ายไปขวา |
| 2 | `!``~``+``-` (unary)`*``&``sizeof``(type)` | ขวาไปซ้าย |
| 3 | `*``/``%`| จากซ้ายไปขวา |
| 4 | `+``-` | จากซ้ายไปขวา |
| 5 | `<<``>>` | จากซ้ายไปขวา |
| 6 | `<``<=``>``>=` | จากซ้ายไปขวา |
| 7 | `==``!=` | จากซ้ายไปขวา |
| 8 | `&`(ระดับบิต AND) | จากซ้ายไปขวา |
| 9 | `^`| จากซ้ายไปขวา |
| 10 | `\|`| จากซ้ายไปขวา |
| 11 | `&&`| จากซ้ายไปขวา |
| 12 | `\|\|`| จากซ้ายไปขวา |
...
## โครงสร้าง สหภาพแรงงาน และ Enums
```c
// Struct
struct Point {
    double x;
...
## สิ่งจำเป็นสำหรับห้องสมุดมาตรฐาน
```c
...
## สรุป
ไวยากรณ์ของ C นั้นมีน้อยมากและใกล้เคียงกับฮาร์ดแวร์ เช่น พอยน์เตอร์ การจัดการหน่วยความจำแบบแมนนวล และเลเยอร์นามธรรมบางๆ บนคำสั่งของเครื่อง พลังของภาษามาจากความเรียบง่ายนี้: เกือบจะไม่มีค่าใช้จ่ายแอบแฝง ไม่มีค่าใช้จ่ายรันไทม์ และการควบคุมเค้าโครงหน่วยความจำได้อย่างสมบูรณ์ แม้ว่าตัวประมวลผลล่วงหน้าจะทรงพลัง แต่ก็ต้องมีวินัยในการใช้งานอย่างปลอดภัย Modern C (C11/C17/C23) ได้เพิ่มคุณสมบัติด้านความปลอดภัยที่สำคัญ เช่น`_Static_assert`,`_Generic`,`nullptr`และ`typeof`ทำให้ภาษาปลอดภัยยิ่งขึ้นในขณะที่ยังคงรักษาปรัชญาหลักของการไว้วางใจโปรแกรมเมอร์