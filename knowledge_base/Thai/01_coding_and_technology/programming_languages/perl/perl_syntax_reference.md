...
| `.`| การต่อสตริง | `$a . $b`| |
| `x`| การทำซ้ำสตริง | `"ab" x 3`| `"ababab"`|
| `==``!=``<``>``<=``>=` | การเปรียบเทียบตัวเลข | `$a == $b`| |
| `eq``ne``lt``gt``le``ge` | การเปรียบเทียบสตริง | `$a eq $b`| |
| `<=>`| ยานอวกาศตัวเลข | `$a <=> $b`| ส่งกลับ -1, 0, 1 |
| `cmp`| ยานอวกาศสตริง | `$a cmp $b`| |
| `&&``\|\|``!`| ตรรกะ | `$a && $b`| |
| `and``or``not`| ตรรกะ (ความแม่นยำต่ำ) | `$a and $b`| หลีกเลี่ยง — ใช้`&&`/`\|\|`|
| `=~``!~` | การจับคู่ Regex | `$str =~ /pattern/`| |
| `..``...` | พิสัย / ฟลิปฟล็อป | `1..10`| |
| `?:`| เทอร์นารี | `$a ? $b : $c`| |
| `//`| กำหนด-หรือ | `$a // $b`| `$b`ถ้า`$a`เป็น undef |
| `=>`| ลูกน้ำอ้วน | `key => 'value'`| เสนอราคาอัตโนมัติ Bareword |
### Sigils — ประเภทตัวแปร
```perl
...
## สรุป
ไวยากรณ์ของ Perl สร้างขึ้นตามบริบท สัญลักษณ์ และหลักการของ "There's More Than One Way To Do It" นิพจน์ทั่วไปถูกสร้างขึ้นในภาษา ทำให้การประมวลผลข้อความมีความกระชับไม่ตรงกัน การอ้างอิงเปิดใช้งานโครงสร้างข้อมูลที่ซับซ้อน CPAN จัดเตรียมโมดูลสำหรับทุกสิ่ง Perl สมัยใหม่ (พร้อม`strict`,`warnings`และตัวแปรคำศัพท์) นั้นสะอาดและบำรุงรักษาได้ มรดกของ Perl ยังคงอยู่ในรูปแบบและเครื่องมือที่ภาษาสมัยใหม่ทุกภาษานำมาใช้