...
---
# Haskell - การอ้างอิงไวยากรณ์
เอกสารนี้ให้การอ้างอิงไวยากรณ์ที่มีโครงสร้างและครอบคลุมสำหรับ Haskell (GHC 9.x) มันช่วยเสริมการอ้างอิงหลักของ Haskell โดยมุ่งเน้นไปที่รูปแบบไวยากรณ์ที่ละเอียดถี่ถ้วน ระบบประเภท monads การประเมินแบบขี้เกียจ และสำนวนการเขียนโปรแกรมเชิงฟังก์ชัน
---
...
|----------|-|---------|-------|
| `+``-``*``/` | เลขคณิต | `x + y`| หมายเลขประเภทคลาส |
| `div``mod` | การหารจำนวนเต็ม | `7 \`div\` 2`| ส่งคืน 3 |
| `^``**` | พลังงาน | `2 ^ 10`| `^`สำหรับอินทิกรัล; `**`สำหรับการลอยตัว |
| `==``/=` | ความเท่าเทียมกัน | `x == y`| คลาสประเภท Eq |
| `<``>``<=``>=` | การเปรียบเทียบ | `x >= y`| ลำดับประเภทคลาส |
| `&&``\|\|``not`| ตรรกะ | `a && b`| ลัดวงจร |
| `.`| องค์ประกอบของฟังก์ชัน | `(f . g) x`| `f (g x)`|
| `$`| แอปพลิเคชั่นฟังก์ชัน | `f $ x + 1`| `f (x + 1)`— ลำดับความสำคัญต่ำ |
| `<>`| Monoid ผนวก | `"hello" <> " world"`| กึ่งกรุ๊ป |
| `<$>`| ฟังก์ชัน fmap | `(+1) <$> Just 5`| `fmap (+1) (Just 5)`|
| `<*>`| การสมัคร สมัคร | `pure (+) <*> Just 3`| |
| `>>=`| โมนาดผูก | `Just 5 >>= \x -> Just (x+1)`| |
| `>>`| ลำดับโมนาด | `putStrLn "hi" >> putStrLn "bye"`| |
| `=<<`| ผูก (กลับด้าน) | `f =<< mx`| เหมือนกับ`mx >>= f`|
| `++`| การต่อรายการ | `[1,2] ++ [3,4]`| O(n) ในรายการด้านซ้าย |
| `:`| ข้อเสีย | `1 : [2, 3]`| โอ(1) |
| `!!`| ดัชนี | `[1,2,3] !! 1`| O(n) — ฟังก์ชันบางส่วน |
| `\\`| รายการความแตกต่าง | `[1,2,3] \\ [2]`| `[1,3]`|
### ลำดับความสำคัญของผู้ให้บริการ (เลือก)
...
## สรุป
ไวยากรณ์ของ Haskell นั้นเรียบง่ายแต่แสดงออกได้อย่างพิเศษ ระบบประเภทซึ่งมีประเภทข้อมูลพีชคณิต คลาสประเภท GADT และตระกูลประเภท ทำหน้าที่เป็นทั้งข้อกำหนดและเครื่องมือตรวจสอบ การจับคู่รูปแบบ การประเมินแบบ Lazy และ Monads มอบวิธีแก้ปัญหาที่ซับซ้อนให้กับปัญหาที่ซับซ้อน ความบริสุทธิ์ของ Haskell (ไม่มีผลข้างเคียงในประเภท) ทำให้โค้ดง่ายต่อการให้เหตุผล ทดสอบ และขนานกัน แม้ว่าช่วงการเรียนรู้จะสูงชัน แต่อิทธิพลของ Haskell ก็แผ่ซ่านไปทั่วภาษาสมัยใหม่ ตั้งแต่ตัวเลือกของ Swift ไปจนถึงคลาสที่ปิดสนิทของ Kotlin ไปจนถึงระบบประเภทของ Rust Mastering Haskell ทำให้คุณเป็นโปรแกรมเมอร์ที่ดีขึ้นในทุกภาษา