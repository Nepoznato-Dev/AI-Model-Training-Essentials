...
---
# Java — ข้อผิดพลาดทั่วไปและการต่อต้านรูปแบบ
เอกสารนี้รวบรวมข้อผิดพลาด กับดัก และรูปแบบการต่อต้านที่พบบ่อยที่สุดใน Java แต่ละรายการจะแสดงแนวทางที่ไม่ถูกต้อง อธิบายว่าทำไมจึงล้มเหลว และให้แนวทางแก้ไขที่ถูกต้อง
---

## 1.`equals()`กับ`==`สำหรับวัตถุ
```java
...
## 2. การแก้ไขคอลเลกชันระหว่างการวนซ้ำ
```java
...
## 3. ไม่ปิดทรัพยากร (ลองใช้ทรัพยากร)
```java
...
## 4. ข้อผิดพลาดของออโต้บ็อกซ์จำนวนเต็ม
```java
...
## 7. ไม่จัดการ InterruptedException
```java
...
## 8. การคืนค่า Null แทนที่จะเป็นคอลเลกชั่นที่ว่างเปล่า
```java
...
## 9. วัตถุที่ไม่แน่นอนเป็นคีย์ HashMap
```java
...
## 10. การต่อต้านรูปแบบ: การจับ`Exception`หรือ `Throwable`
```java
...
## 11. ไม่เอาชนะ`hashCode`เมื่อเอาชนะ `equals`
```java
...
## 12. สภาพการแข่งขันพร้อมระบบล็อคแบบตรวจสอบซ้ำ
```java
...
## สรุป
การใช้คำฟุ่มเฟือยของ Java สามารถซ่อนข้อบกพร่องเล็กๆ น้อยๆ ได้:`==`กับ`equals()`, การแก้ไขที่เกิดขึ้นพร้อมกัน, การรั่วไหลของทรัพยากร, กับดักประสิทธิภาพการทำงานแบบ autoboxing และสัญญาเท่ากับ/แฮชโค้ด วิธีการ Java สมัยใหม่คือ: ใช้`Objects.equals()`สำหรับการเปรียบเทียบที่ปลอดภัยกับ null, ใช้ try-with-resources เสมอ, ส่งคืนคอลเลกชันเปล่าแทนที่จะเป็น null, จับข้อยกเว้นเฉพาะ และปฏิบัติตามหลักการความรับผิดชอบเดียว คุณสมบัติ Java 8+ (สตรีม ตัวเลือกเสริม การอ้างอิงเมธอด) ช่วยลดการสำเร็จรูปในขณะที่ทำให้โค้ดปลอดภัยและแสดงออกได้มากขึ้น