...
| `/`| กอง | `7 / 2`| `3.5`| |
| `%`| ส่วนที่เหลือ | `7 % 2`| `1`| |
| `**`| การยกกำลัง | `2 ** 10`| `1024`| อีเอส2016 |
| `++`| เพิ่มขึ้น | `x++`/`++x`| | หลังและก่อนเพิ่ม |
| `--`| ลดลง | `x--`/`--x`| | หลังและก่อนการลดลง |
| `+x`,`-x`| อูนารี | `+"42"`→`42`| พิมพ์บังคับเป็นตัวเลข |
### การเปรียบเทียบและความเท่าเทียมกัน
| ตัวดำเนินการ | ชื่อ | ตัวอย่าง | หมายเหตุ |
|----------|-|---------|-------|
| `===`| ความเท่าเทียมกันที่เข้มงวด | `1 === 1`| ไม่มีการบังคับพิมพ์ — ชอบสิ่งนี้เสมอ |
| `!==`| ความไม่เท่าเทียมกันที่เข้มงวด | `1 !== "1"`| `true`— ประเภทต่างๆ |
| `==`| ความเท่าเทียมกันเชิงนามธรรม | `1 == "1"`| พิมพ์การบังคับ — หลีกเลี่ยง |
| `!=`| ความไม่เท่าเทียมกันเชิงนามธรรม | `1 != "1"`| `false`— หลีกเลี่ยง |
| `<`,`>`,`<=`,`>=`| เชิงสัมพันธ์ | `"b" > "a"`| พจนานุกรมสำหรับสตริง |
### ตัวดำเนินการเชิงตรรกะและโมฆะ
| ตัวดำเนินการ | ชื่อ | ตัวอย่าง | หมายเหตุ |
|----------|-|---------|-------|
| `&&`| ตรรกะและ | `a && b`| ส่งคืนค่าเท็จหรือค่าสุดท้าย |
| `\|\|`| ตรรกะหรือ | `a \|\| b`| ส่งกลับค่าความจริงอันดับแรกหรือค่าสุดท้าย |
| `!`| ตรรกะไม่ใช่ | `!true`| `false`|
| `??`| การรวมตัวกันเป็นโมฆะ | `a ?? b`| ส่งกลับ`b`เฉพาะในกรณีที่`a`คือ`null`/`undefined`|
| `?.`| การผูกมัดเสริม | `obj?.prop?.method()`| ลัดวงจรไปที่`undefined`|
...
### การมอบหมายและการทำลายโครงสร้าง
```javascript
// Basic assignment
let x = 10;
...
|------------|-----------|---------------|
| 1 (สูงสุด) | `()``[]``.``?.` | ซ้าย |
| 2 | `new`(ไม่มี args) | ขวา |
| 3 | `++``--` (โพสต์ฟิกซ์) | ไม่มี |
| 4 | `!``~``+x``-x``typeof``void``delete``++``--`(คำนำหน้า) | ขวา |
| 5 | `**`| ขวา |
| 6 | `*``/``%`| ซ้าย |
| 7 | `+``-` | ซ้าย |
| 8 | `<<``>>``>>>`| ซ้าย |
| 9 | `<``<=``>``>=``in``instanceof` | ซ้าย |
| 10 | `===``!==``==``!=` | ซ้าย |
| 11 | `&`| ซ้าย |
| 12 | `^`| ซ้าย |
| 13 | `\|`| ซ้าย |
...
| คอลเลกชัน | สั่งซื้อ | ไม่แน่นอน | ซ้ำกัน | ประเภทคีย์ | ค้นหา |
|------------|---------|---------|------------|----------|--------|
| `Array`| ใช่ | ใช่ | ใช่ | ดัชนี (ตัวเลข) | โอ(n) |
| `Object`| ใช่ (แทรก) | ใช่ | คีย์: ไม่ | สตริง/สัญลักษณ์ | O(1) เฉลี่ย |
| `Map`| ใช่ (แทรก) | ใช่ | คีย์: ไม่ | ใดๆ | O(1) เฉลี่ย |
| `Set`| ใช่ (แทรก) | ใช่ | ไม่ | — | O(1) เฉลี่ย |
| `WeakMap`| ไม่ | ใช่ | คีย์: ไม่ | วัตถุเท่านั้น | O(1) เฉลี่ย |
| `WeakSet`| ไม่ | ใช่ | ไม่ | วัตถุเท่านั้น | O(1) เฉลี่ย |
### อาร์เรย์
```javascript
// Creation
...
## สรุป
ไวยากรณ์ของ JavaScript ดูเรียบง่ายแต่มีความลึกมาก ภาษาได้พัฒนาจากเครื่องมือสคริปต์ง่ายๆ เป็นภาษาที่มีหลายกระบวนทัศน์ พร้อมด้วยคลาส โมดูล ตัวสร้าง พรอกซี และโมเดลอะซิงก์ที่ซับซ้อน การทำความเข้าใจลำดับความสำคัญของตัวดำเนินการ ลูปเหตุการณ์ สายโซ่ต้นแบบ และความแตกต่างของการเชื่อมโยง`this`จะแยกนักพัฒนาที่มีความสามารถออกจากผู้เชี่ยวชาญ JavaScript สมัยใหม่ (ES2020+) พร้อมตัวเลือก chaining, nullish coalescing,`Promise.allSettled`และระดับบนสุด await ยังคงลดขนาดสำเร็จรูปในขณะที่ยังคงความเข้ากันได้แบบย้อนหลัง