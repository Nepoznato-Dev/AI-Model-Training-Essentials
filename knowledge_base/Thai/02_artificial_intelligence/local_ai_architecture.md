...
ทรัพยากรที่สำคัญที่สุดขนาดโมเดลในหน่วยความจำ γ **พารามิเตอร์ × ไบต์ต่อพารามิเตอร์**
...
### ที่เก็บข้อมูล
- น้ำหนักโมเดลเชิงปริมาณใช้เวลาไม่กี่ GB (เช่น 7B 4 บิต 4 บิตบนดิสก์)ตรวจสอบให้แน่ใจว่ามีพื้นที่ว่างอย่างน้อย 20–50 GB สำหรับหลายรุ่น
...
**หลักทั่วไป:** ใช้ Q4_K_M เพื่อความสมดุลระหว่างคุณภาพและขนาดหากคุณมี VRAM เพิ่มเติม ให้ใช้ Q5 หรือ Q6
...
- **การกำหนดพารามิเตอร์ SQL**: ไม่ต้องเชื่อมอินพุตของผู้ใช้เข้ากับคำสั่ง SQLใช้แบบสอบถามแบบมีพารามิเตอร์ (คำสั่งที่เตรียมไว้) หรือ ORM
- **การหลบหนีของเชลล์**: หลีกเลี่ยงการสร้างคำสั่งเชลล์จากอินพุตของผู้ใช้หากหลีกเลี่ยงไม่ได้ ให้ใช้ `shlex.quote()` หรือที่คล้ายกัน
...
### JWT (โทเค็นเว็บ JSON)
- **ลงชื่อ**: ใช้ RS256 หรือ ES256 (ไม่สมมาตร) เพื่อความปลอดภัยที่ดีขึ้นHS256 (สมมาตร) เป็นที่ยอมรับได้หากมีการจัดการความลับร่วมกันอย่างดี
- **ตรวจสอบ**: ตรวจสอบลายเซ็น ผู้ออก (`iss`) ผู้ชม (`aud`) และการหมดอายุ (`exp`) เสมอ
- **ให้หมดอายุสั้น**: 15–60 นาทีสำหรับโทเค็นการเข้าถึงใช้โทเค็นการรีเฟรชสำหรับเซสชันที่นานขึ้น
...
- ส่งคืน `429 Too Many Requests` ด้วยส่วนหัว `Retry-After`

### CORS (การแบ่งปันทรัพยากรข้ามแหล่งกำเนิด)
- อนุญาตเฉพาะต้นกำเนิดที่เฉพาะเจาะจง (ไม่เคย `*` ในการผลิต)
...
- ใช้ไฟล์ล็อคแพ็คเกจ (`package-lock.json`, `Cargo.lock`, `go.sum`) เพื่อให้แน่ใจว่าสามารถสร้างซ้ำได้
