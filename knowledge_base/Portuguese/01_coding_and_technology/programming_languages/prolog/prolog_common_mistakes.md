...
# Prolog – Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns no Prolog com correções.
---
...
## 2. Não usar o corte (`!`) corretamente
```prolog
...
## 4. Verificação de ocorrência de não compreensão
```prolog
...
## 5. Listar operações sem acumuladores
```prolog
...
## Resumo
O paradigma de programação lógica do Prolog cria armadilhas únicas: confundir`=`(unificação),`==`(igualdade) e`is`(aritmética); posicionamento incorreto do corte; recursão infinita sem rastreamento visitado; e a verificação ocorre. A maneira do Prolog é: entender profundamente a unificação, usar o corte com moderação e corretamente, usar acumuladores para construção eficiente de listas e sempre considerar o encerramento.