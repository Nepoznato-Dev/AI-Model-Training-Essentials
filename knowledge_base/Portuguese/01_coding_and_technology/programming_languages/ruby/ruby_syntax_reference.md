...
# Ruby — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para Ruby (3.x). Ele complementa a referência principal do Ruby, concentrando-se em padrões de sintaxe exaustivos, blocos e fechamentos, metaprogramação e expressões idiomáticas do Ruby.
---
...
| `==``!=``===``<=>` | Igualdade | `a == b`| `===`é igualdade de casos; `<=>`é uma nave espacial |
| `eql?`| Igualdade de valores | `a.eql?(b)`| Mais estrito que`==`(tipo de verificação) |
| `equal?`| Identidade | `a.equal?(b)`| Mesmo objeto? |
| `<``>``<=``>=` | Comparação | `a >= b`| |
| `&&``\|\|``!`| Lógico | `a && b`| Curto-circuito; retorna o último valor avaliado |
| `and``or``not`| Lógico (baixa precedência) | `a and b`| Evite - use`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | Bit a bit | `a & b`| |
| `=~``!~` | Correspondência Regex | `str =~ /pattern/`| Retorna índice ou nulo |
| `..``...` | Alcance | `1..10`| `..`inclusive;  Exclusivo`...`|
| `?.`| Navegação segura | `user&.name`| Retorna nulo se o usuário for nulo |
| `[]``[]=` | Acesso ao elemento | `arr[0]`| |
### Verdade
```ruby
...
## Resumo
A sintaxe do Ruby foi projetada para a felicidade do desenvolvedor — ela se parece com o inglês e recompensa a expressividade. O poder da linguagem vem de seu modelo de objeto (tudo é um objeto), blocos e fechamentos, recursos de metaprogramação e o módulo Enumerable que fornece uma rica API de processamento de coleção. Ruby 3.x adicionou correspondência de padrões, assinaturas de tipo RBS e Ractor para simultaneidade, enquanto mantém a filosofia central da linguagem: tornar os programadores produtivos e felizes.