...
---
# Ruby — Erros Comuns e Antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns em Ruby. Cada entrada mostra a abordagem incorreta, explica por que ela falha e fornece a solução correta.
---

## 1. Modificando uma coleção durante a iteração
```ruby
...
## 3. Interpolação de String vs Concatenação
```ruby
...
## 4. Não usar blocos/enumeradores de maneira idiomática
```ruby
# ❌ WRONG — C-style loop
i = 0
...
## 6. Classes principais de patch de macaco
```ruby
...
## 7. Não entendendo`return`em blocos
```ruby
...
## 9. Símbolo vs Confusão de Strings
```ruby
...
## 10. Não congelando literais de string
```ruby
...
## Resumo
A expressividade do Ruby pode levar a antipadrões: modificação de coleções durante a iteração, correção de classes principais, variáveis ​​globais e modelos gordos. A maneira Ruby é: usar enumeradores (`each`,`map`,`reject`), preferir símbolos e argumentos de palavras-chave, congelar literais de string, extrair objetos de serviço de modelos e usar refinamentos em vez de patches de macaco. Ruby recompensa os desenvolvedores que adotam blocos, digitação discreta e o princípio da menor surpresa.