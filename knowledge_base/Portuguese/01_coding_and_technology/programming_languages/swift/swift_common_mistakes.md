...
-->
# Swift – Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns no Swift. Cada entrada mostra a abordagem incorreta, explica por que ela falha e fornece a solução correta.
---

## 1. Forçar o desempacotamento de opcionais
```swift
// ❌ WRONG — crash if nil
...
## 2. Reter ciclos com fechamentos
```swift
...
## 3. Não usar`enum`com valores associados
```swift
...
## 4. Mal-entendido entre tipos de valor e tipos de referência
```swift
...
## 5. Antipadrão: Controlador de visualização massiva
```swift
...
## 6. Não usar`guard`para saída antecipada
```swift
...
## 7. Esquecendo`weak`em delegados
```swift
...
## 8. Confusão de índice de string
```swift
...
## 9. Não usar`lazy`para propriedades caras
```swift
...
## 10. Segurança de thread com `@MainActor`
```swift
...
## Resumo
Os recursos de segurança do Swift evitam muitos bugs, mas introduzem suas próprias armadilhas: forçar o desembrulhamento causa travamentos, reter ciclos ocultos em fechamentos e delegados, e a semântica do tipo de valor surpreende os desenvolvedores vindos de linguagens de tipo de referência. A maneira Swift é: nunca forçar o desembrulhamento (`!`), usar`guard`para saídas antecipadas, usar`weak`para delegados e capturas de fechamento, marcar o código da UI com`@MainActor`e dividir controladores de visualização massivos em modelos e serviços de visualização. O sistema de tipos do Swift é seu amigo – trabalhe com ele, não contra ele.