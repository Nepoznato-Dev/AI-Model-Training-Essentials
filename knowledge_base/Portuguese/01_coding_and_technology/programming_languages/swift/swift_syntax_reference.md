...
---
# Swift — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para Swift (5.9+). Ele complementa a referência principal do Swift, concentrando-se em padrões de sintaxe exaustivos, opcionais, programação orientada a protocolo e simultaneidade moderna.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmética | `a + b`| |
| `..``...` | Alcance | `1...10`| `...`fechado; `..<`semiaberto |
| `==``!=``<``>``<=``>=` | Comparação | `a == b`| Requer `Equatable`/`Comparable` |
| `===``!==` | Identidade | `a === b`| Mesma referência (apenas aulas) |
| `&&``\|\|``!`| Lógico | `a && b`| Curto-circuito |
| `??`| Nenhuma coalescência | `a ?? b`| Retorna`b`se`a`for nulo |
| `?.`| Encadeamento opcional | `a?.b`| Retorna nulo se`a`for nulo |
| `!`| Forçar desembrulhar | `a!`| Trava se for nulo — evite |
| `&`| Parâmetro de saída | `swap(&a, &b)`| Passe por referência |
### Operadores de Tratamento Nulo
```swift
...
## Resumo
A sintaxe do Swift equilibra segurança com expressividade. Os opcionais eliminam falhas nulas, os protocolos permitem a composição flexível sem herança e a simultaneidade moderna (assíncrona/espera, atores) torna acessível a programação simultânea segura. A linguagem continua a evoluir com recursos como pacotes de parâmetros, macros e propriedade — cada adição torna o Swift mais seguro e poderoso, ao mesmo tempo que mantém sua promessa principal: código que é seguro por construção.