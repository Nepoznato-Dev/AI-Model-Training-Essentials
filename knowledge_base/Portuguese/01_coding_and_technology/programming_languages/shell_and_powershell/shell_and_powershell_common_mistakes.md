...
-->
# Shell e PowerShell – erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns no Shell/Bash e no PowerShell com correções.
---

## 1. Variáveis ​​​​não citadas (Bash)
```bash
...
## 4. Não verificando códigos de saída (Bash)
```bash
...
## 5. PowerShell: interpolação de strings
```powershell
...
## 7. Escopo da variável subshell (Bash)
```bash
...
## 8. PowerShell: objetos de pipeline versus strings
```powershell
...
## Resumo
Armadilhas de script de shell: sempre cite variáveis ​​no Bash, use`[[ ]]`sobre`[ ]`, use`set -euo pipefail`, verifique os códigos de saída e entenda o escopo do subshell. Armadilhas do PowerShell: use aspas duplas para interpolação, use`-eq`e não`==`e lembre-se de que os objetos de pipeline têm propriedades. Ambos recompensam citações cuidadosas e tratamento explícito de erros.