...
## Resumo
As expressões idiomáticas do Kotlin enfatizam: segurança nula, classes de dados, tipos selados, funções de escopo, funções de extensão, corrotinas e código orientado a expressões. Siga as convenções de codificação Kotlin, use ktlint para formatação e detekt para análise estática. Kotlin valoriza concisão e expressividade – escreva menos clichês, adote a biblioteca padrão e use`val`por padrão.