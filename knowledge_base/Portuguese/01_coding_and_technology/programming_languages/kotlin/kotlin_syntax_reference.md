...
# Kotlin — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para Kotlin (2.0+). Ele complementa a referência principal do Kotlin, concentrando-se em padrões de sintaxe exaustivos, segurança nula, corrotinas e expressões idiomáticas do Kotlin.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmética | `a + b`| |
| `..`| Alcance | `1..10`| Gama inclusiva |
| `until`| Gama semiaberta | `1 until 10`| `1..9`|
| `downTo`| Faixa descendente | `10 downTo 1`| |
| `==``!=` | Igualdade estrutural | `a == b`| Chama`.equals()`|
| `===``!==` | Igualdade referencial | `a === b`| Mesma instância |
| `<``>``<=``>=` | Comparação | `a >= b`| Requer`Comparable`|
| `&&``\|\|``!`| Lógico | `a && b`| Curto-circuito |
| `?:`| Operador Elvis | `a ?: b`| Retorna`b`se`a`for nulo |
| `?.`| Chamada segura | `a?.b`| Retorna nulo se`a`for nulo |
| `!!`| Asserção não nula | `a!!`| Lança NPE se for nulo |
| `as`| Elenco | `a as String`| Lança ClassCastException |
| `as?`| Elenco seguro | `a as? String`| Retorna nulo em caso de falha |
| `in`| Associação | `x in list`| |
| `!in`| Não membro | `x !in list`| |
| `is`| Verificação de tipo | `x is String`| Elenco inteligente após verificação |
| `!is`| Não digite | `x !is String`| |
...
## Resumo
A sintaxe do Kotlin é concisa, segura e interoperável com Java. A segurança nula elimina uma classe inteira de bugs em tempo de compilação. As corrotinas fornecem uma maneira natural de escrever código assíncrono. Classes seladas, classes de dados e funções de extensão permitem modelagem de domínio expressiva. O poder da linguagem vem da interação de seus recursos – funções de escopo, DSLs, classes inline e genéricos reificados se combinam para produzir código que é ao mesmo tempo seguro e agradável de escrever.