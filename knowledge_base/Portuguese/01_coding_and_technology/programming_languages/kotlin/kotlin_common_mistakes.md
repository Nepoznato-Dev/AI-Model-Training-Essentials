...
-->
# Kotlin – Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns em Kotlin. Cada entrada mostra a abordagem incorreta, explica por que ela falha e fornece a solução correta.
---

## 1. Tipos de plataforma e NullPointerException
```kotlin
...
## 3.`data class`sem`equals`/`hashCode`adequado
```kotlin
...
## 4. Não usar funções de escopo corretamente
```kotlin
...
## 5. Corrotinas: não tratando de cancelamento
```kotlin
...
## 6. Antipadrão: Kotlin estilo Java
```kotlin
...
## 7. Exaustividade da aula selada
```kotlin
...
## 8. Sombreamento da função de extensão
```kotlin
...
## 9. Não usar`by lazy`para inicialização cara
```kotlin
...
## 10. Confusão entre fluxo e LiveData
```kotlin
...
## Resumo
A concisão do Kotlin pode ocultar problemas sutis: tipos de plataforma de interoperabilidade Java, falhas de `lateinit`, classes de dados mutáveis ​​que quebram coleções e cancelamento de corrotinas. A maneira Kotlin é: abraçar a segurança nula (nunca use`!!`), usar a função de escopo correta (`let`para transformações,`apply`para configuração,`also`para efeitos colaterais), usar classes seladas com`when`exaustivo, lidar com cancelamento de rotina e escrever Kotlin idiomático (não Java com sintaxe Kotlin). O compilador é o seu guia – se ele avisar, ouça.