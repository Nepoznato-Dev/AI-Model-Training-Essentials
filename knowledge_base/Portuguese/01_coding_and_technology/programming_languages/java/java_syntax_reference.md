...
-->
# Java — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para Java. Ele complementa a referência Java principal, concentrando-se em padrões de sintaxe exaustivos, tabelas de operadores e mecânica interna da JVM, OOP e simultaneidade.
---
...
| `-`| Subtração | `3 - 2`| `1`| |
| `*`| Multiplicação | `3 * 2`| `6`| |
| `/`| Divisão | `7 / 2`| `3`| Divisão inteira para`int`; `3.5`para`double`|
| `%`| Restante | `7 % 2`| `1`| |
...
|----------|------|---------|-------|
| `==`| Igual | `x == y`| Igualdade de referência para objetos |
| `!=`| Diferente | `x != y`| |
| `<`,`>`,`<=`,`>=`| Relacional | `x >= y`| |
| `&&`| E lógico | `a && b`| Curto-circuito |
| `\|\|`| OU lógico | `a \|\| b`| Curto-circuito |
| `!`| Lógico NÃO | `!true`| `false`|
| `instanceof`| Verificação de tipo | `obj instanceof String`| Também correspondência de padrões (Java 16+) |
...
|------------|-----------|
| 1 (mais alto) | Postfixo:`expr++``expr--` |
| 2 | Unário:`++expr``--expr``+expr``-expr``~``!` |
| 3 | Elenco:`(Type)expr`|
| 4 | Multiplicativo:`*``/``%`|
| 5 | Aditivo:`+``-` |
| 6 | Turno:`<<``>>``>>>`|
...
| Interface | Implementações | Encomendado | Variante Thread-Safe |
|-----------|----------------|---------|---------------------|
| `List`| `ArrayList`,`LinkedList`| Sim | `CopyOnWriteArrayList`|
| `Set`| `HashSet`,`TreeSet`,`LinkedHashSet`| Varia | `CopyOnWriteArraySet`|
| `Map`| `HashMap`,`TreeMap`,`LinkedHashMap`| Varia | `ConcurrentHashMap`|
| `Queue`| `ArrayDeque`,`PriorityQueue`| Varia | `ConcurrentLinkedQueue`|
| `Deque`| `ArrayDeque`| Ambas as extremidades | `ConcurrentLinkedDeque`|
### Listas
```java
// Creation
...
## Resumo
A sintaxe do Java evoluiu dramaticamente desde suas origens em 1995. O Java moderno (21+) apresenta registros, classes seladas, correspondência de padrões, threads virtuais e modelos de string — tornando-o mais conciso e expressivo do que nunca. A sintaxe principal permanece familiar (semelhante a C, fortemente tipada, baseada em classe), mas as adições de lambdas, a API Stream e`var`reduziram significativamente o padrão. A força da linguagem reside em seu enorme ecossistema, nas garantias de compatibilidade com versões anteriores e nas otimizações de desempenho da JVM por meio da compilação JIT.