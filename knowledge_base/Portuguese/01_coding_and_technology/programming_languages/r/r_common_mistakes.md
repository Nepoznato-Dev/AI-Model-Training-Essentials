...
-->
# R — Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns em R com correções.
---

## 1. Crescendo quadros de dados em loops
```r
# ❌ WRONG — O(n²) growth
result <- data.frame()
for (i in 1:1000) {
...
## 3. Não entendendo a reciclagem de vetores
```r
...
## 5. Não usando `stringsAsFactors = FALSE`
```r
...
## Resumo
A natureza interativa do R cria armadilhas: crescimento de quadros de dados em loops (pré-alocação ou vetorização), surpresas de reciclagem de vetores, propagação de`NA`e `T`/`F` sendo sobregraváveis. A maneira R é: vetorizar operações, pré-alocar durante o loop, manipular`NA`explicitamente, usar tidyverse para manipulação de dados e evitar`<<-`. R recompensa quem pensa em vetores e utiliza seu rico ecossistema de pacotes.