...
-->
# R — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para R (4.x). Ele complementa a referência principal do R, concentrando-se em padrões de sintaxe exaustivos, no ecossistema tidyverse, na manipulação de dados, na modelagem estatística e na visualização.
---
...
|----------|------|---------|-------|
| `+``-``*``/``^`| Aritmética | `2^10`| |
| `%%`| Módulo | `7 %% 3`| Retorna 1 |
| `%/%`| Divisão inteira | `7 %/% 3`| Retorna 2 |
| `==``!=` | Igualdade | `x == y`| Vetorizado |
| `<``>``<=``>=` | Comparação | `x >= y`| Vetorizado |
| `&``\|``!`| Lógico (vetorizado) | `x & y`| Em termos de elemento |
| `&&``\|\|` | Lógico (escalar) | `x && y`| Somente primeiro elemento |
| `%in%`| Associação | `x %in% y`| |
| `<-``=` | Atribuição | `x <- 10`| `<-`é idiomático |
| `->`| Atribuir à direita | `10 -> x`| Raramente usado |
| `<<-`| Atribuição global | `x <<- 10`| Atribuir no ambiente pai |
| `$`| Extrair elemento | `df$column`| |
| `[``[[` | Indexação | `x[1]`/`x[[1]]`| `[`retorna o mesmo tipo;  Extratos`[[`|
| `%>%`| Tubo (magritr) | `x %>% f()`| |
| `\|>`| Tubo (base R 4.1+) | `x \|> f()`| Tubo nativo |
...
## Resumo
A sintaxe do R é projetada para computação estatística e análise de dados. A natureza vetorizada das operações elimina loops para a maioria das tarefas. O ecossistema tidyverse (dplyr, ggplot2, tidyr) fornece uma gramática consistente e legível para manipulação e visualização de dados. Os recursos de modelagem estatística do R — desde testes t básicos até modelos de efeitos mistos — são incomparáveis ​​em profundidade. Para ciência de dados, estatística e pesquisa reproduzível, R continua sendo uma ferramenta essencial.