...
# SQL — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para SQL (ANSI SQL com notas sobre diferenças de dialeto PostgreSQL/MySQL/SQLite). Ele complementa a referência SQL principal, concentrando-se em padrões de consulta exaustivos, funções de janela, CTEs e design de banco de dados.
---

## Sintaxe da consulta principal
### SELECIONE
```sql
-- Basic query
...
## Resumo
A sintaxe do SQL permaneceu estável por mais de 50 anos, enquanto evolui continuamente. As consultas principais (SELECT, JOIN, WHERE, GROUP BY) são padrão em todos os bancos de dados. Funções de janela, CTEs e suporte JSON representam os recursos modernos do SQL. O uso eficaz do SQL se beneficia da compreensão de como o mecanismo de banco de dados processa consultas – lendo planos de execução, entendendo índices e escrevendo operações baseadas em conjuntos em vez de loops procedurais. Fortes habilidades em SQL são valiosas para qualquer pessoa que trabalhe com dados.