...
## 3. Off-by-One em operações de string
```asm
...
## 4. Não limpando os bits superiores
```asm
...
## 5. Loops infinitos da condição de saída ausente
```asm
...
## Resumo
Armadilhas de montagem: preservam os registros salvos pelo receptor, mantêm o alinhamento da pilha de 16 bytes, incluem terminadores nulos em strings, são explícitos sobre os tamanhos dos operandos e sempre fornecem condições de saída do loop. A programação assembly requer a compreensão do hardware no nível mais baixo – cada instrução tem consequências para registradores, memória e sinalizadores.