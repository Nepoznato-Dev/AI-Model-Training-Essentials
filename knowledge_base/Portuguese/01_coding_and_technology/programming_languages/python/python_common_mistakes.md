...
# Python — Erros Comuns e Antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns em Python. Cada entrada mostra a abordagem incorreta, explica por que ela falha e fornece a solução correta. Compreender essas armadilhas o ajudará a escrever um código Pythonic mais robusto.
---
...
**Por que isso acontece:** Os argumentos padrão são avaliados uma vez no momento da definição da função, não em cada chamada. Um padrão mutável (lista, ditado, conjunto) é compartilhado por todas as invocações.
---

## 2. Modificando uma lista durante a iteração
```python
...
## 3. Vinculação tardia em fechamentos
```python
...
## 4. Usando`type()`em vez de `isinstance()`
```python
...
## 5. Não usar gerenciadores de contexto para recursos
```python
...
## 6. O operador`is`para comparação de valores
```python
...
## 8. Não entendendo as regras de escopo do Python (LEGB)
```python
...
## 9. Concatenação de Strings em Loops
```python
...
## 10. Antipadrão: usando`except:`Bare Except
```python
...
## 11. Retornando referências mutáveis ​​ao estado interno
```python
...
## 12. Não usando`setdefault`ou `collections.defaultdict`
```python
...
## 14. Não entendendo o esgotamento do gerador
```python
...
## 15. Antipadrão: Objetos de Deus / Classes sobrecarregadas
```python
...
## Resumo
A simplicidade do Python é enganosa — padrões mutáveis, fechamentos tardios de vinculação, regras de escopo e gerenciamento de recursos têm comportamentos sutis que prendem os desenvolvedores. Os princípios-chave a serem lembrados: os padrões são avaliados uma vez,`is`verifica a identidade e não a igualdade, sempre usa gerenciadores de contexto para recursos, captura exceções específicas e favorece a composição em vez de objetos divinos. Escrever código Pythonic significa compreender essas armadilhas e usar os recursos da linguagem (gerenciadores de contexto, compreensões, geradores, módulo `collections`) conforme pretendido.