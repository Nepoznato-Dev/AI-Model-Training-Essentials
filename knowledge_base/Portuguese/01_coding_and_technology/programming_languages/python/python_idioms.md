...
## Resumo
O código Python segue o Zen do Python: legível, explícito, simples e plano. Os principais idiomas incluem: compreensões de lista/dict/conjunto, gerenciadores de contexto (`with`), tratamento de erros EAFP,`enumerate`/`zip`para iteração,`dataclass`para contêineres de dados, dicas de tipo, descompactação e verificações de verdade/falsidade. A comunidade Python valoriza a legibilidade e “deve haver uma maneira óbvia de fazer isso”. Siga o PEP 8 para estilo, use`ruff`para linting e prefira a biblioteca padrão em vez de soluções de terceiros, quando possível.