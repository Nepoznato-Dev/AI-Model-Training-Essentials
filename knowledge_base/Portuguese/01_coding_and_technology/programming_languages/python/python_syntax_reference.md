...
---
# Python — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para Python. Ele complementa a referência principal do Python, concentrando-se em padrões de sintaxe exaustivos, tabelas de operadores e mecânica interna.
---
...
|----------|------|--------|--------|-------|
| `+`| Adição | `3 + 2`| `5`| Também concatenação de string/lista |
| `-`| Subtração | `3 - 2`| `1`| Também negação unária |
| `*`| Multiplicação | `3 * 2`| `6`| Também repetição de string/lista |
| `/`| Divisão Verdadeira | `7 / 2`| `3.5`| Sempre retorna float |
| `//`| Divisão de Piso | `7 // 2`| `3`| Arredonda em direção ao infinito negativo |
| `%`| Módulo | `7 % 2`| `1`| Sinal corresponde ao divisor |
| `**`| Exponenciação | `2 ** 10`| `1024`| Associativo à direita |
### Comparação e operadores booleanos
| Operador | Nome | Exemplo | Notas |
|----------|------|---------|-------|
| `==`| Igual | `x == y`| Comparação de valores (não identidade) |
| `!=`| Diferente | `x != y`| |
| `<`,`>`,`<=`,`>=`| Pedidos | `x >= y`| Encadeável:`a < b < c`|
| `is`| Identidade | `x is y`| Mesmo objeto na memória |
| `is not`| Não é identidade | `x is not None`| Preferido em relação a`!= None`|
| `in`| Associação | `x in collection`| O(1) para conjuntos/dictos, O(n) para listas |
| `and`| E lógico | `a and b`| Curto-circuito; retorna o último valor verdadeiro/falso |
| `or`| OU lógico | `a or b`| Curto-circuito; retorna o primeiro valor verdadeiro/falso |
| `not`| Lógico NÃO | `not x`| |
...
| 8 | `^`| Esquerda |
| 9 | `\|`| Esquerda |
| 10 |  `==`, `!=`, `<`, `>`, `<=`, `>=`, `is`,`in`| Acorrentável |
| 11 | `not`| Certo |
| 12 | `and`| Esquerda |
| 13 (mais baixo) | `or`| Esquerda |
### Atribuição e Atribuição Aumentada
```python
...
## Estruturas de dados
### Visão geral das coleções integradas
| Coleção | Encomendado | Mutável | Duplicatas | Sintaxe | Pesquisa |
|--------|-----|---------|------------|--------|--------|
| `list`| Sim | Sim | Sim | `[1, 2, 3]`| Sobre(n) |
| `tuple`| Sim | Não | Sim | `(1, 2, 3)`| Sobre(n) |
| `dict`| Sim (3,7+) | Sim | Chaves: Não | `{"k": "v"}`| O(1) média |
| `set`| Não | Sim | Não | `{1, 2, 3}`| O(1) média |
| `frozenset`| Não | Não | Não | `frozenset({1,2})`| O(1) média |
### Listas
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
## Resumo
Esta referência de sintaxe cobre o conjunto completo de recursos do Python — desde operadores básicos e fluxo de controle até metaprogramação avançada, descritores e sistema de tipos. A sintaxe do Python é deliberadamente mínima e legível, mas sua profundidade é substancial: correspondência de padrões, geradores assíncronos, protocolos e metaclasses fornecem ferramentas poderosas para aplicações complexas. A linguagem continua a evoluir, com cada versão adicionando recursos que melhoram a expressividade enquanto mantêm a compatibilidade com versões anteriores.