...
-->
# Fortran — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para Modern Fortran (2008/2018). Ele complementa a referência principal do Fortran, concentrando-se em padrões de sintaxe exaustivos, operações de array, módulos, OOP e computação de alto desempenho.
---
...
| `.eqv.``.neqv.` | Equivalência lógica | `a .eqv. b`| |
| `//`| Concatenação de strings | `'hello' // ' world'`| |
---
...
## Resumo
A sintaxe do Fortran moderno é limpa, estruturada e otimizada para computação numérica. As operações de array eliminam loops para a maioria das tarefas matemáticas. Os módulos fornecem encapsulamento e gerenciamento de namespace. Os recursos OOP (tipos derivados, herança, polimorfismo) permitem uma organização limpa do código. Coarrays oferecem programação paralela nativa sem bibliotecas externas. Os pontos fortes do Fortran – desempenho, operações de array e precisão numérica – permanecem incomparáveis ​​na computação científica. Para HPC, física computacional e simulação de engenharia, Fortran continua a ser uma potência.