...
## Por que Fortran é importante
- **Desempenho de HPC**: os compiladores Fortran produzem alguns dos códigos numéricos mais rápidos disponíveis — geralmente igualando ou excedendo C/C++ para operações de array.
- **Bases de código legadas**: Décadas de código científico (modelos climáticos, simulações físicas) são escritos em Fortran.
- **Operações de array**: suporte nativo a array multidimensional com sintaxe projetada para computação matemática.
- **Estabilidade numérica**: A linguagem e os compiladores são otimizados para computação de ponto flutuante.
- **Coarrays**: Suporte integrado à programação paralela (Fortran 2008+).
- **Ainda em evolução**: Fortran 2023 adiciona recursos modernos enquanto mantém compatibilidade com versões anteriores.
## As compensações
| Limitação | Detalhes | Solução alternativa típica |
|-------|---------|-------------------|
| **Comunidade de nicho** | Pequenas e especializadas — maioritariamente científicas/HPC | Comunidade ativa em ciência da computação |
| **Ecossistema limitado** | Menos bibliotecas que Python, Java ou C++ | Utilize BLAS/LAPACK para trabalhos numéricos; Interoperabilidade C para outras necessidades |
| **Não é de uso geral** | Fraco para programação web, móvel, GUI ou sistemas | Use Fortran para computação; wrap em Python/C para aplicativos |
| **Percepção** | Muitas vezes visto como "obsoleto", apesar das capacidades modernas | Concentre-se nos seus pontos fortes: numérico e HPC |
| **Contratação** | Poucos desenvolvedores Fortran entrando no mercado | Especialistas existentes na academia e em laboratórios nacionais |
---

## Fundamentos de sintaxe (Fortran moderno)
```fortran
program hello
    implicit none
...
### Tipos derivados e procedimentos vinculados a tipos
```fortran
...
### Procedimentos Genéricos e Sobrecarga do Operador
```fortran
module vector_ops
    implicit none
...
### Módulos e Organização do Programa
```fortran
module constants
    implicit none
...
### Coarrays (paralelismo integrado do Fortran 2008)
```fortran
...
### Gerenciador de pacotes Fortran (fpm)
```toml
# fpm.toml
...
## Teste
### Estrutura de teste (fortran-test ou pFUnit)
```fortran
...
## Interoperabilidade
### Interoperabilidade C (ISO_C_BINDING)
```fortran
module c_interface
...
### Integração Python (via f2py ou biblioteca compartilhada)
```fortran
...
## Padrões de Projeto
### Padrão 1: Programação de Array (Evitar Loops)
```fortran
...
### Padrão 2: Tipos derivados parametrizados
```fortran
...
### Padrão 3: Padrão de retorno de chamada com ponteiros de procedimento
```fortran
module integrator
    implicit none
    abstract interface
...
### Sinalizadores de otimização do compilador
```bash
...
## Quando usar Fortran
| Cenário | Por que Fortran | Melhor Alternativa |
|----------|-----------|-------------------|
| HPC/supercomputação | Otimizado para desempenho numérico | C++ (com cuidado), Julia |
| Simulações científicas | Décadas de código validado | Python (para prototipagem), C++ |
| Modelos climáticos/meteorológicos | Bases de código legadas; desempenho | — |
| Física computacional | Operações de matriz nativa | Python (NumPy), Júlia |
| Modelagem financeira (HPC) | Desempenho para computação em larga escala | C++, Python |
| Desenvolvimento geral de aplicações | Não adequado | Python, Java, Go |
