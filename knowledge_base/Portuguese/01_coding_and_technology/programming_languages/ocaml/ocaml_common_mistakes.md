...
## 1. Correspondência de padrões não exaustiva
```ocaml
(* ❌ WRONG — missing cases *)
...
## 4. Não usar`Option`corretamente
```ocaml
...
## 5. Estado mutável em funções recursivas
```ocaml
...
## Resumo
Armadilhas OCaml: correspondência de padrões não exaustiva (aviso do compilador),`=`é igualdade estrutural enquanto`==`é físico, a divisão inteira trunca, usa`Option`em vez de exceções e evita estado mutável em favor de recursão e dobras de cauda. O compilador OCaml é seu aliado – preste atenção aos seus avisos.