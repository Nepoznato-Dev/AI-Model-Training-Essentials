...
-->
# Erlang & Elixir – Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns em Erlang e Elixir com correções.
---

## 1. Tarefa Única (Erlang)
```erlang
...
## 2. Não vincular/monitorar processos (Erlang)
```erlang
...
## 3. Chamada GenServer sem tempo limite (Elixir)
```elixir
...
## 4. Correspondência de padrão em`receive`sem tempo limite (Erlang)
```erlang
...
## 5. Não usar`with`para correspondência de padrões aninhados (Elixir)
```elixir
...
## 6. Antipadrão: retornos de chamada GenServer de longa duração
```elixir
...
## Resumo
Armadilhas Erlang/Elixir: as variáveis ​​são de atribuição única, os processos precisam de links/monitores, as chamadas do GenServer precisam de tempos limite explícitos,`receive`bloqueia para sempre sem tempo limite e os retornos de chamada do GenServer não devem ser bloqueados. A maneira do BEAM é: deixá-lo travar (com supervisão), usar`with`para correspondência de padrões limpa, manter os retornos de chamada do GenServer rápidos e sempre lidar com falhas de processo.