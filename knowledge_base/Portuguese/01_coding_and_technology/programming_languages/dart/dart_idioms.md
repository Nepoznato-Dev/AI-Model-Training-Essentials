...
## Resumo
As expressões idiomáticas do Dart enfatizam: segurança nula, construtores const, registros e correspondência de padrões (Dart 3), async/await e métodos de extensão. Siga o guia Effective Dart, use`dart format`para formatação e`dart analyze`para linting. A Dart valoriza a segurança e a produtividade — "escreva uma vez, execute em qualquer lugar" com o Flutter.