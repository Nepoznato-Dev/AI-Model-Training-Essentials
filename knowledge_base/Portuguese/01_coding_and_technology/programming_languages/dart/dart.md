...
---
#Dardo
Dart é uma linguagem de programação otimizada para cliente desenvolvida pelo Google, lançada pela primeira vez em 2013. Embora o Dart tenha sido inicialmente posicionado como um substituto potencial do JavaScript para navegadores da web, ele encontrou seu objetivo principal como a linguagem por trás do **Flutter** - o kit de ferramentas de interface de usuário de plataforma cruzada do Google para construir aplicativos móveis, da web, de desktop e incorporados a partir de uma única base de código.
O Dart combina os melhores recursos das linguagens modernas: é orientado a objetos, possui digitação opcional (segurança nula sonora desde o Dart 3), suporta programação assíncrona com`async`/`await`e compila para código de máquina nativo (para dispositivos móveis/desktop) e JavaScript (para web).
---

## Por que o Dart é importante
- **Flutter**: a linguagem principal do Flutter — uma das estruturas de plataforma cruzada de crescimento mais rápido.
- **Plataforma cruzada**: base de código única para iOS, Android, web, Windows, macOS, Linux e dispositivos incorporados.
- **Produtivo**: recarga dinâmica, biblioteca de widgets rica e sintaxe expressiva tornam o desenvolvimento da UI rápido.
- **Sound null safety**: A segurança nula em tempo de compilação elimina erros de referência nula.
- **Desempenho**: Compila para código ARM nativo para dispositivos móveis; nenhuma ponte necessária.
- **Ecossistema em crescimento**: o ecossistema de pacotes do Flutter está se expandindo rapidamente.
## As compensações
...
## Teste
### Testes unitários com pacote de teste
```dart
...
### Interoperabilidade JavaScript (Dart Web)
```dart
...
## Padrões de Projeto
### Padrão 1: Padrão de Repositório
```dart
...
### Padrão 2: Gerenciamento de estado do provedor
```dart
...
### Padrão 3: BLoC (componente de lógica de negócios)
```dart
...
### Compilação AOT vs JIT
| Modo | Quando usado | Inicialização | Tempo de execução | Caso de uso |
|------|-----------|---------|---------|----------|
| **JIT** (Just-In-Time) | Desenvolvimento, modo de depuração | Rápido | Um pouco mais lento | Recarga a quente durante o desenvolvimento |
| **AOT** (antecipado) | Liberar compilações | Um pouco mais lento | Mais rápido | Produção de aplicativos móveis/desktop |
| **JS** | Alvos da Web | Depende | Varia | Aplicativos web flutuantes |
| **WASM** | Web (experimental) | Depende | Rápido | Alvo futuro da web |
```dart
...
## Implantação
### Alvos de implantação do Flutter
| Plataforma | Comando de construção | Saída |
|----------|-------------|--------|
| **Android** | `flutter build apk`/`flutter build appbundle`| APK / AAB para Play Store |
| **iOS** | `flutter build ipa`| IPA para App Store |
| **Web** | `flutter build web`| HTML/JS/CSS estático |
| **Janelas** | `flutter build windows`| MSIX ou exe autônomo |
| **macOS** | `flutter build macos`| Pacote .app |
| **Linux** | `flutter build linux`| Binário + ativos |
```bash
# Build commands
...
## Quando usar o Dart
| Cenário | Por que Dart (Flutter) | Melhor Alternativa |
|----------|-------------------|-------------------|
| Aplicativos móveis multiplataforma | A vibração é excelente | React nativo, Swift/Kotlin nativo |
| Desktop multiplataforma | Flutter suporta | Elétron, C#, Avalonia |
| Aplicações Web | A web flutuante existe | React, Vue, Angular para aplicativos web mais ricos |
| UIs incorporadas | Flutter para incorporado | C, LVGL |
...
## Resumo
O propósito de vida de Dart é Flutter. Como linguagem independente, é competente, mas normal. Como mecanismo por trás do Flutter, ele permite que os desenvolvedores criem aplicativos bonitos e de alto desempenho para todas as principais plataformas a partir de uma única base de código. Se você estiver criando aplicativos multiplataforma para dispositivos móveis ou desktop, Dart + Flutter é uma das melhores opções disponíveis. Para todo o resto, outras línguas são mais apropriadas.