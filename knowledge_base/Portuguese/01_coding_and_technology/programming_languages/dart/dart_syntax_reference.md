...
|----------|------|---------|-------|
| `+``-``*``/``%``~/` | Aritmética | `7 ~/ 3`| `~/`é divisão inteira |
| `==``!=` | Igualdade | `a == b`| |
| `<``>``<=``>=` | Comparação | `a >= b`| |
| `&&``\|\|``!`| Lógico | `a && b`| Curto-circuito |
| `??`| Coalescência nula | `a ?? b`| `b`se`a`for nulo |
| `?.`| Acesso com reconhecimento nulo | `a?.b`| nulo se`a`for nulo |
| `!`| Asserção nula | `a!`| Lança se nulo |
| `..``?..` | Cascata | `obj..a..b`| Operações em cadeia no mesmo objeto |
| `??=`| Atribuição com reconhecimento nulo | `a ??= b`| Atribuir se nulo |
| `as`| Tipo elenco | `obj as String`| Lança em caso de falha |
| `is``is!` | Teste de tipo | `obj is String`| |
| `=>`| Função de seta | `() => expr`| |
### Notação em Cascata
```dart
...
## Resumo
A sintaxe do Dart é limpa, consistente e projetada para produtividade. A segurança nula sólida elimina erros de referência nula em tempo de compilação. Async/await e streams fornecem padrões naturais para programação assíncrona. Classes, mixins e classes seladas permitem uma composição flexível de objetos. A correspondência de padrões, registros e modificadores de classe do Dart 3 adicionam expressividade. Embora o objetivo principal do Dart seja Flutter, sua sintaxe é adequada para qualquer domínio de aplicativo - lado do servidor (com Dart Frog ou Shelf), ferramentas CLI e aplicativos da web (compilados em JavaScript).