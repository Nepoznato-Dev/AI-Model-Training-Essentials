...
# Haskell — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para Haskell (GHC 9.x). Ele complementa a referência principal de Haskell, concentrando-se em padrões de sintaxe exaustivos, sistema de tipos, mônadas, avaliação preguiçosa e idiomas de programação funcional.
---
...
|----------|------|---------|-------|
| `+``-``*``/` | Aritmética | `x + y`| Num tipo de classe |
| `div``mod` | Divisão inteira | `7 \`div\` 2`| Retorna 3 |
| `^``**` | Poder | `2 ^ 10`| `^`para integral; `**`para flutuante |
| `==``/=` | Igualdade | `x == y`| classe de tipo Eq |
| `<``>``<=``>=` | Comparação | `x >= y`| Classe de tipo de ordem |
| `&&``\|\|``not`| Lógico | `a && b`| Curto-circuito |
| `.`| Composição funcional | `(f . g) x`| `f (g x)`|
| `$`| Aplicação de função | `f $ x + 1`| `f (x + 1)`— baixa precedência |
| `<>`| Acrescentar monóide | `"hello" <> " world"`| Semigrupo |
| `<$>`| Funtor fmap | `(+1) <$> Just 5`| `fmap (+1) (Just 5)`|
| `<*>`| Aplicativo aplicar | `pure (+) <*> Just 3`| |
| `>>=`| Ligação de mônada | `Just 5 >>= \x -> Just (x+1)`| |
| `>>`| Sequência de mônadas | `putStrLn "hi" >> putStrLn "bye"`| |
| `=<<`| Vincular (invertido) | `f =<< mx`| Igual a`mx >>= f`|
| `++`| Concatenação de lista | `[1,2] ++ [3,4]`| O(n) na lista da esquerda |
| `:`| Contras | `1 : [2, 3]`| O(1) |
| `!!`| Índice | `[1,2,3] !! 1`| O(n) — função parcial |
| `\\`| Diferença de lista | `[1,2,3] \\ [2]`| `[1,3]`|
### Precedência do Operador (selecionado)
...
## Resumo
A sintaxe de Haskell é mínima, mas extraordinariamente expressiva. O sistema de tipos - com tipos de dados algébricos, classes de tipos, GADTs e famílias de tipos - serve tanto como especificação quanto como ferramenta de verificação. A correspondência de padrões, a avaliação preguiçosa e as mônadas fornecem soluções elegantes para problemas complexos. A pureza de Haskell (sem efeitos colaterais nos tipos) torna o código mais fácil de raciocinar, testar e paralelizar. Embora a curva de aprendizado seja íngreme, a influência de Haskell permeia as linguagens modernas - desde os opcionais do Swift até as classes seladas do Kotlin e o sistema de tipos do Rust. Dominar Haskell torna você um programador melhor em todas as linguagens.