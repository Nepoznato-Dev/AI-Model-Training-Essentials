...
-->
# Haskell — Padrões Idiomáticos e Melhores Práticas
Este guia cobre padrões idiomáticos e práticas recomendadas para escrever código Haskell idiomático e limpo.
---
...
## Resumo
As expressões idiomáticas de Haskell enfatizam: tipos fortes, correspondência de padrões, mônadas para efeitos, funções puras e avaliação preguiçosa (com rigor quando necessário). Siga o Haskell Style Guide, use`hlint`para sugestões e`fourmolu`para formatação. Haskell valoriza a correção e a composição - "tornar os estados ilegais irrepresentáveis".