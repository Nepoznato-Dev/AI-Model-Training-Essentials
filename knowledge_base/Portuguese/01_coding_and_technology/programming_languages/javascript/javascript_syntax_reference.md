...
|----------|------|--------|--------|-------|
| `+`| Adição | `3 + 2`| `5`| Também concatenação de strings |
| `-`| Subtração | `3 - 2`| `1`| |
| `*`| Multiplicação | `3 * 2`| `6`| |
| `/`| Divisão | `7 / 2`| `3.5`| |
| `%`| Restante | `7 % 2`| `1`| |
| `**`| Exponenciação | `2 ** 10`| `1024`| ES2016 |
| `++`| Incremento | `x++`/`++x`| | Pós e pré-incremento |
| `--`| Decremento | `x--`/`--x`| | Pós e pré-decremento |
| `+x`,`-x`| Unário | `+"42"`→`42`| Digite coerção para número |
### Comparação e Igualdade
| Operador | Nome | Exemplo | Notas |
|----------|------|---------|-------|
| `===`| Igualdade Estrita | `1 === 1`| Nenhum tipo de coerção — sempre prefira isso |
| `!==`| Desigualdade Estrita | `1 !== "1"`| `true`— tipos diferentes |
| `==`| Igualdade abstrata | `1 == "1"`| Digite coerção - evite |
| `!=`| Desigualdade abstrata | `1 != "1"`| `false`— evitar |
| `<`,`>`,`<=`,`>=`| Relacional | `"b" > "a"`| Lexicográfico para strings |
### Operadores Lógicos e Nulos
| Operador | Nome | Exemplo | Notas |
|----------|------|---------|-------|
| `&&`| E lógico | `a && b`| Retorna o primeiro valor falso ou o último valor |
| `\|\|`| OU lógico | `a \|\| b`| Retorna o primeiro valor verdadeiro ou o último valor |
| `!`| Lógico NÃO | `!true`| `false`|
| `??`| Coalescência Nula | `a ?? b`| Retorna`b`somente se`a`for`null`/`undefined`|
| `?.`| Encadeamento Opcional | `obj?.prop?.method()`| Curto-circuitos para`undefined`|
...
| `>>`| Mudança para a direita assinada | `-5 >> 1`| `-3`|
| `>>>`| Mudança para a direita não assinada | `-1 >>> 0`| `4294967295`|
...
|------------|-----------|---------------|
| 1 (mais alto) | `()``[]``.``?.` | Esquerda |
| 2 | `new`(sem argumentos) | Certo |
| 3 | `++``--` (postfixo) | N/A |
| 4 | `!``~``+x``-x``typeof``void``delete``++``--`(prefixo) | Certo |
| 5 | `**`| Certo |
| 6 | `*``/``%`| Esquerda |
| 7 | `+``-` | Esquerda |
| 8 | `<<``>>``>>>`| Esquerda |
| 9 | `<``<=``>``>=``in``instanceof` | Esquerda |
| 10 | `===``!==``==``!=` | Esquerda |
| 11 | `&`| Esquerda |
| 12 | `^`| Esquerda |
| 13 | `\|`| Esquerda |
...
| 16 | `??`| Esquerda |
| 17 | `=``+=``-=`etc. Certo |
| 18 (mais baixo) | `=>`(função de seta) | Certo |
---
...
## Funções
### Declarações e expressões de funções
```javascript
...
| Coleção | Encomendado | Mutável | Duplicatas | Tipo de chave | Pesquisa |
|------------|---------|---------|------------|----------|--------|
| `Array`| Sim | Sim | Sim | Índice (número) | Sobre(n) |
| `Object`| Sim (inserção) | Sim | Chaves: Não | String/Símbolo | O(1) média |
| `Map`| Sim (inserção) | Sim | Chaves: Não | Qualquer | O(1) média |
| `Set`| Sim (inserção) | Sim | Não | — | O(1) média |
| `WeakMap`| Não | Sim | Chaves: Não | Somente objeto | O(1) média |
| `WeakSet`| Não | Sim | Não | Somente objeto | O(1) média |
### Matrizes
```javascript
// Creation
...
### WeakRef e Registro de Finalização
```javascript
...
## Resumo
A sintaxe do JavaScript é aparentemente simples, mas contém uma profundidade significativa. A linguagem evoluiu de uma simples ferramenta de script para uma linguagem multiparadigma com classes, módulos, geradores, proxies e um modelo assíncrono sofisticado. Compreender a precedência do operador, o loop de eventos, as cadeias de protótipos e as nuances da ligação`this`separa os desenvolvedores competentes dos especialistas. JavaScript moderno (ES2020+) com encadeamento opcional, coalescência nula,`Promise.allSettled`e espera de nível superior continua a reduzir o padrão, mantendo a compatibilidade com versões anteriores.