...
# JavaScript – Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns em JavaScript. Cada entrada mostra a abordagem incorreta, explica por que ela falha e fornece a solução correta.
---

## 1. Confusão de ligação `this`
```javascript
...
## 3.`var`Problemas de içamento e escopo
```javascript
...
## 5. Não lidar com rejeições de promessas
```javascript
...
## 6. Modificando objetos durante a iteração
```javascript
...
## 7. Antipadrão: Inferno de retorno de chamada
```javascript
...
## 9. Objetos semelhantes a array não são arrays
```javascript
...
## 10. Antipadrão: parâmetros de função mutantes
```javascript
...
## 11. Não entendendo o loop de eventos e as microtarefas
```javascript
...
## 13. Esquecendo de limpar ouvintes de eventos
```javascript
// ❌ WRONG — memory leak
class Component {
  mount() {
...
## 14. Cópia superficial vs cópia profunda
```javascript
...
## Resumo
A flexibilidade do JavaScript vem com armadilhas: a ligação`this`depende do site de chamada,`==`executa coerção, as promessas exigem tratamento de erros e a ordem do loop de eventos (microtarefas antes de macrotarefas) não é óbvia. As regras modernas são simples: use`const`/`let`nunca`var`, prefira`===`em vez de`==`, sempre lide com rejeições de promessas, use funções de seta para retornos de chamada, evite mutação e limpe ouvintes de eventos. Seguir essas regras elimina a maioria dos bugs de JavaScript.