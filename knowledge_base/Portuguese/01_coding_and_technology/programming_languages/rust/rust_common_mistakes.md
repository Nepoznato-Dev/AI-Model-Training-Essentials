...
---
# Rust – Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns no Rust. Cada entrada mostra a abordagem incorreta, explica por que ela falha e fornece a solução correta. O compilador do Rust detecta muitos erros, mas compreender esses padrões acelerará sua curva de aprendizado.
---

## 1. Lutando contra o verificador de empréstimos - clonagem desnecessária
```rust
...
## 2. Confusão entre strings e &str
```rust
...
## 3. Ignorando resultados com`unwrap()`em produção
```rust
...
## 4. Erros de anotação vitalícios
```rust
...
## 5. Antipadrão: usando`Rc`/`RefCell`em todos os lugares
```rust
...
## 6. Não entendendo a semântica do movimento
```rust
...
## 7. Antipadrão:`to_string()`/`to_owned()`excessivo
```rust
...
## 9. Invalidação do Iterador – Coletando Desnecessariamente
```rust
...
## 10. Antipadrão: Enum sem métodos
```rust
...
## 11. Não usar`#[derive]`para características comuns
```rust
...
## 12. Código inseguro sem documentação
```rust
...
## Resumo
O compilador do Rust é rigoroso, mas justo – a maioria dos erros ensina algo sobre segurança de memória. Os maiores erros não são erros do compilador, mas antipadrões: clonagem desnecessária para apaziguar o verificador de empréstimo, uso de`unwrap()`na produção, busca por`Rc<RefCell<T>>`antes de tentar a propriedade e alocação de strings quando as referências forem suficientes. A mentalidade Rustaceana é: deixe o compilador guiá-lo em direção a abstrações de custo zero. Clone apenas quando necessário, propague erros com`?`, prefira`&str`a`String`e documente cada bloco `unsafe`.