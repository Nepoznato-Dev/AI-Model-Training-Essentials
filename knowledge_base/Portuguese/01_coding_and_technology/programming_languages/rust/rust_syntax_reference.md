...
---
# Rust — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para Rust. Ele complementa a referência principal do Rust, concentrando-se em padrões de sintaxe exaustivos, tabelas de operadores e mecânica interna do sistema de propriedade, características e simultaneidade.
---
...
|----------|------|---------|-------|
| `+`| Adição | `3 + 2`| Também concatenação`String`via`format!`|
| `-`| Subtração | `3 - 2`| |
| `*`| Multiplicação | `3 * 2`| |
| `/`| Divisão | `7 / 2`| Divisão inteira para inteiros:`3`|
| `%`| Restante | `7 % 2`| `1`|
| `-x`| Negação | `-5`| |
### Comparação e operadores lógicos
| Operador | Nome | Exemplo | Notas |
|----------|------|---------|-------|
| `==`| Igual | `x == y`| Requer`PartialEq`|
| `!=`| Diferente | `x != y`| |
| `<`,`>`,`<=`,`>=`| Pedidos | `x >= y`| Requer`PartialOrd`|
| `&&`| E lógico | `a && b`| Curto-circuito |
| `\|\|`| OU lógico | `a \|\| b`| Curto-circuito |
| `!`| Lógico NÃO | `!true`| |
...
| `\|`| OU | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `!`| NÃO | `!0b1010u8`| Complemento bit a bit |
| `<<`| Deslocamento para a esquerda | `5 << 1`| `10`|
| `>>`| Mudança para a direita | `5 >> 1`| `2`|
### Precedência do Operador (do maior para o menor)
| Precedência | Operadores | Notas |
|------------|-----------|-------|
| 1 (mais alto) |  Caminhos`()``.` :: | |
| 2 | Chamadas de métodos, acesso a campos | Deref automático |
| 3 | `-`(negação),`!`,`*`(deref) | |
| 4 | `as`(conversão de tipo) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
| 7 | `<<``>>` | |
| 8 | `&`| |
...
| 14 | `..``..=` | Alcance |
| 15 | `=``+=` etc. Atribuição |
| 16 (mais baixo) | `return``break``continue`| |
---
...
## Estruturas de dados
### Visão geral dos tipos integrados
| Tipo | Pilha/Pilha | Tamanho | Mutabilidade | Notas |
|------|-----------|------|-----------|-------|
| Primitivos (`i32`,`f64`,`bool`,`char`) | Pilha | Fixo | Imutável por padrão | `Copy`|
| `[T; N]`(matriz) | Pilha | Fixo | Mutável com`mut`| Tamanho fixo,`Copy`se T for`Copy`|
| `Vec<T>`| Pilha | Dinâmico | Mutável com`mut`| Crescível |
| `String`| Pilha | Dinâmico | Mutável com`mut`| UTF-8, cultivável |
| `&[T]`(fatia) | Pilha (gordura ptr) | Visão dinâmica | Depende de`&`ou`&mut`| Vista emprestada |
| `HashMap<K, V>`| Pilha | Dinâmico | Mutável com`mut`| O(1) pesquisa média |
| `BTreeMap<K, V>`| Pilha | Dinâmico | Mutável com`mut`| Pesquisa O (log n), ordenada |
| `HashSet<T>`| Pilha | Dinâmico | — | O(1) média, sem duplicatas |
### Vetores
```rust
// Creation
...
### Definição e implementação de características
```rust
// Define a trait
trait Summary {
...
### Características comuns da biblioteca padrão
```rust
...
## Tratamento de erros
### Padrões de resultados e opções
```rust
...
## Simultaneidade
### Tópicos e passagem de mensagens
```rust
use std::thread;
...
## Resumo
A sintaxe do Rust é construída sobre uma base de garantias de segurança aplicadas em tempo de compilação. O sistema de propriedade, o polimorfismo baseado em características, a correspondência de padrões e as abstrações de custo zero formam um design coerente onde o compilador é um parceiro estrito, mas justo. Desde o fluxo de controle básico até recursos avançados como macros, código inseguro e ponteiros inteligentes, cada construção atende ao objetivo de programação de sistemas segura e eficiente. Compreender a interação entre tempos de vida, empréstimos e o sistema de tipos é a chave para dominar o Rust.