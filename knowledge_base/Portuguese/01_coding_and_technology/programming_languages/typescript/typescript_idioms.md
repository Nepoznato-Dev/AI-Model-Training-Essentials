...
-->
# TypeScript – Padrões idiomáticos e práticas recomendadas
Este guia cobre padrões idiomáticos e práticas recomendadas para escrever código TypeScript limpo e seguro.
---
...
## Tipo Protetores e Estreitamento
```typescript
...
## Resumo
Os idiomas TypeScript enfatizam: verificação estrita de tipo, interfaces para formas de objetos, uniões discriminadas para estado, protetores de tipo para estreitamento, genéricos com restrições,`readonly`para imutabilidade e`as const`para tipos literais. Siga ESLint com`typescript-eslint`, Mais bonito para formatação e sempre habilite`strict: true`. A comunidade TypeScript valoriza a segurança de tipo – se compilar, provavelmente funcionará. Prefira`interface`para formatos de objetos,`type`para uniões e utilidades, e evite`any`a todo custo.