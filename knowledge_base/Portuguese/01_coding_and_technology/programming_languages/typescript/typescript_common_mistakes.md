...
---
# TypeScript – Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns no TypeScript. Cada entrada mostra a abordagem incorreta, explica por que ela falha e fornece a solução correta.
---
...
## 2. Asserções de tipo em vez de protetores de tipo
```typescript
...
## 4. Confusão de interfaces versus tipos
```typescript
...
## 7. Surpresas de digitação estrutural
```typescript
...
## 8. Erros de tipo assíncrono/espera
```typescript
...
## 9. Antipadrão: tipos repetidos (sem uso de genéricos)
```typescript
...
## 10. Encadeamento opcional excessivo
```typescript
...
## 12. Não aproveitar sindicatos discriminados
```typescript
...
## Resumo
O sistema de tipos do TypeScript é poderoso, mas requer disciplina. O pecado capital é alcançar`any`– use`unknown`quando o tipo for realmente desconhecido e restrinja com protetores de tipo. Evite asserções não nulas (`!`) manipulando indefinido corretamente. Use interfaces para formas de objetos, tipos para uniões e tipos computados. Os sindicatos discriminados eliminam classes inteiras de bugs. Habilite`strict: true`desde o primeiro dia – o rigor do retrofit é doloroso. O objetivo é tornar os estados ilegais irrepresentáveis ​​no nível do tipo.