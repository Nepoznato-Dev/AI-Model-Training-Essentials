...
-->
# Perl — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para Perl (5.38+). Ele complementa a referência principal do Perl, concentrando-se em padrões de sintaxe exaustivos, expressões regulares, referências e no poder de processamento de texto do Perl.
---
...
| `x`| Repetição de strings | `"ab" x 3`| `"ababab"`|
| `==``!=``<``>``<=``>=` | Comparação numérica | `$a == $b`| |
| `eq``ne``lt``gt``le``ge` | Comparação de strings | `$a eq $b`| |
| `<=>`| Nave espacial numérica | `$a <=> $b`| Retorna -1, 0, 1 |
| `cmp`| Nave espacial de corda | `$a cmp $b`| |
| `&&``\|\|``!`| Lógico | `$a && $b`| |
| `and``or``not`| Lógico (baixa prec.) | `$a and $b`| Evite - use`&&`/`\|\|`|
| `=~``!~` | Correspondência Regex | `$str =~ /pattern/`| |
| `..``...` | Alcance/flip-flop | `1..10`| |
| `?:`| Ternário | `$a ? $b : $c`| |
| `//`| Definido-ou | `$a // $b`| `$b`se`$a`estiver indefinido |
| `=>`| Vírgula gorda | `key => 'value'`| Citações automáticas de bareword |
### Sigilos — Tipos de Variáveis
```perl
...
## Perl Orientado a Objetos (Moo/Moose)
```perl
# Modern OOP with Moo
package User;
use Moo;
...
## Resumo
A sintaxe do Perl é construída em torno do contexto, dos sigilos e do princípio de "Há mais de uma maneira de fazer isso". Expressões regulares são incorporadas à linguagem, tornando o processamento de texto incomparável em termos de concisão. As referências permitem estruturas de dados complexas. CPAN fornece módulos para tudo. Perl moderno (com`strict`,`warnings`e variáveis ​​lexicais) é limpo e de fácil manutenção. O legado do Perl continua vivo nos padrões e ferramentas que todas as linguagens modernas adotaram.