...
Este documento fornece uma referência de sintaxe estruturada e abrangente para C (C11/C17 com notas C23). Ele complementa a referência principal do C, concentrando-se em padrões de sintaxe exaustivos, tabelas de operadores, mecânica de ponteiros e gerenciamento de memória.
---
...
| `*`| Multiplicação | `3 * 2`| `6`| |
| `/`| Divisão | `7 / 2`| `3`| Truncamento inteiro para tipos int |
| `%`| Módulo | `7 % 2`| `1`| Somente para tipos inteiros |
| `++`| Incremento | `i++`/`++i`| | Pós e pré-incremento |
| `--`| Decremento | `i--`/`--i`| | Pós e pré-decremento |
...
| `\|`| OU | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `~`| Complemento | `~0`| Todos os bits invertidos |
| `<<`| Deslocamento para a esquerda | `1 << 3`| `8`|
| `>>`| Mudança para a direita | `8 >> 3`| `1`(definido pela implementação para assinado) |
### Operadores de ponteiro e endereço
| Operador | Nome | Exemplo | Notas |
|----------|------|---------|-------|
| `&`| Endereço de | `&x`| Retorna o ponteiro para x |
| `*`| Desreferência | `*ptr`| Valor de acesso no ponteiro |
| `->`| Acesso de membros via ponteiro | `p->x`| Equivalente a`(*p).x`|
| `.`| Acesso de membro | `s.field`| Acesso direto aos membros da estrutura |
| `[]`| Subscrito da matriz | `arr[i]`| Equivalente a`*(arr + i)`|
### Precedência do Operador (do maior para o menor)
| Precedência | Operadores | Associatividade |
|------------|-----------|---------------|
| 1 (mais alto) | `()``[]``->``.` | Da esquerda para a direita |
| 2 | `!``~``+``-` (unário)`*``&``sizeof``(type)` | Da direita para a esquerda |
...
| 13 | `? :`(ternário) | Da direita para a esquerda |
| 14 | `=``+=``-=``*=``/=``%=``&=``\|=``^=``<<=``>>=`| Da direita para a esquerda |
| 15 (mais baixo) | `,`(vírgula) | Da esquerda para a direita |
---
...
## Fundamentos da Biblioteca Padrão
```c
...
## Resumo
A sintaxe do C é mínima e próxima do hardware – ponteiros, gerenciamento manual de memória e uma fina camada de abstração sobre instruções de máquina. O poder da linguagem vem dessa simplicidade: quase não há custos ocultos, nenhuma sobrecarga de tempo de execução e controle completo sobre o layout da memória. O pré-processador, embora poderoso, requer disciplina para ser usado com segurança. O C moderno (C11/C17/C23) adicionou recursos de segurança importantes como`_Static_assert`,`_Generic`,`nullptr`e`typeof`, tornando a linguagem cada vez mais segura, mantendo sua filosofia central de confiar no programador.