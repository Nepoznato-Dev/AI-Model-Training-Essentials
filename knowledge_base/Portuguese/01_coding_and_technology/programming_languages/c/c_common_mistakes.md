...
Este documento cataloga os erros, armadilhas e antipadrões mais comuns em C. Cada entrada mostra a abordagem incorreta, explica por que ela falha e fornece a solução correta. C oferece controle total – o que significa responsabilidade total pela segurança da memória, verificação de limites e comportamento indefinido.
---

## 1. Estouro de buffer com`gets()`/ `strcpy()`
```c
...
## 5.`scanf()`sem limites de largura
```c
...
## 8. Vulnerabilidades de string de formato
```c
...
## 10. Não entendendo aritmética de ponteiros
```c
...
## Resumo
Os perigos do C são bem conhecidos, mas sempre presentes: buffer overflows, ponteiros pendentes, overflow de inteiros, ataques de strings de formatação e vazamentos de memória. A disciplina é: sempre verificar os valores de retorno, sempre liberar o que você aloca, sempre usar funções limitadas (`snprintf`,`strncpy`,`fgets`), nunca usar`gets()`, inicializar todas as variáveis ​​e usar ferramentas como Valgrind e AddressSanitizer. C recompensa programadores cuidadosos com desempenho e controle incomparáveis ​​— mas pune o descuido com comportamento indefinido e vulnerabilidades de segurança.