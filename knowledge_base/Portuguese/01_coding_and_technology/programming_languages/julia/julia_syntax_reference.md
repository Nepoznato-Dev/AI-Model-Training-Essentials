...
---
# Julia — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para Julia (1.x). Ele complementa a referência principal de Julia, concentrando-se em padrões de sintaxe exaustivos, despacho múltiplo, macros, metaprogramação e computação científica.
---
...
|----------|------|---------|-------|
| `+``-``*``/``^`| Aritmética | `2^10`| |
| `÷`| Divisão inteira | `7 ÷ 3`| Igual a`div(7, 3)`|
| `%``\` | Módulo/resto | `7 % 3`| |
| `==``!=` | Igualdade | `a == b`| Igualdade de valores |
| `===`| Identidade | `a === b`| Mesmo objeto |
| `<``>``<=``>=` | Comparação | `a >= b`| Encadeável:`1 < x < 10`|
| `&&``\|\|``!`| Lógico | `a && b`| Curto-circuito |
| `&``\|``⊻``~` | Bit a bit | `a & b`| `⊻`é XOR |
| `:`| Alcance | `1:10`| `start:step:stop`|
| `..`| Intervalo | `1..10`| Requer IntervalSets |
| `∈``∉``⊆`| Definir associação | `x ∈ S`| `in(x, S)`|
| `∘`| Composição | `f ∘ g`| `(f ∘ g)(x) == f(g(x))`|
| `\|>`| Tubo | `x \|> f`| `f(x)`|
| `=>`| Par | `a => b`| Para ditados |
| `...`| Respingo | `f(args...)`| Expandir coleção |
...
## Resumo
A sintaxe de Julia é limpa, matemática e expressiva. O despacho múltiplo permite que as funções se comportem de maneira diferente com base em todos os tipos de argumentos, permitindo uma organização elegante do código. As macros fornecem metaprogramação poderosa sem a complexidade das macros de pré-processador estilo C. O sistema de tipos — com tipos paramétricos, tipos abstratos e tipos de união — permite flexibilidade e desempenho. A indexação, a compreensão e a sintaxe do-block baseadas em 1 de Julia tornam-no natural para a computação matemática e científica. A linguagem atinge sua promessa de "andar como Python, executar como C" por meio da compilação JIT com inferência de tipo.