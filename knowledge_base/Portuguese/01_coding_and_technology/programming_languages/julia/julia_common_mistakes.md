...
-->
# Julia – Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns em Julia com correções.
---

## 1. Digite Instabilidade em Funções
```julia
...
## 2. Variáveis ​​globais em código crítico de desempenho
```julia
...
## 7. Antipadrão: não usar transmissão
```julia
...
## Resumo
O desempenho de Julia depende da estabilidade de tipo: evite variáveis ​​globais em código ativo, use despacho múltiplo em vez de if/else em tipos, pré-aloque arrays, use transmissão para operações elemento a elemento e anote tipos quando necessário. Julia recompensa os desenvolvedores que escrevem código de tipo estável e aproveitam o envio múltiplo. A macro`@code_warntype`é sua melhor amiga para encontrar instabilidade de tipo.