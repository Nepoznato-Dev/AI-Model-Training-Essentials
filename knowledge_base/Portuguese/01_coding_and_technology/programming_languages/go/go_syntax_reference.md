...
# Go — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para Go. Ele complementa a referência principal do Go, concentrando-se em padrões de sintaxe exaustivos, tabelas de operadores e mecânica interna de goroutines, canais e interfaces.
---
...
| `-`| Subtração | `3 - 2`| `1`| |
| `*`| Multiplicação | `3 * 2`| `6`| |
| `/`| Divisão | `7 / 2`| `3`| Divisão inteira para ints |
| `%`| Restante | `7 % 2`| `1`| Sinal corresponde a dividendos |
| `&`| E bit a bit | `5 & 3`| `1`| |
| `\|`| OU bit a bit | `5 \| 3`| `7`| |
| `^`| XOR bit a bit | `5 ^ 3`| `6`| Também complemento bit a bit (unário) |
| `&^`| Pouco claro (E NÃO) | `5 &^ 3`| `4`| Único para ir |
| `<<`| Deslocamento para a esquerda | `5 << 1`| `10`| |
| `>>`| Mudança para a direita | `5 >> 1`| `2`| |
### Operadores de comparação
| Operador | Nome | Exemplo | Notas |
|----------|------|---------|-------|
| `==`| Igual | `x == y`| Apenas tipos comparáveis ​​|
| `!=`| Diferente | `x != y`| |
| `<`,`>`,`<=`,`>=`| Pedidos | `x >= y`| Strings comparadas lexicograficamente |
### Operadores Lógicos
| Operador | Nome | Exemplo | Notas |
|----------|------|---------|-------|
...
## Resumo
A sintaxe do Go é intencionalmente mínima — sem herança, sem sobrecarga de métodos, sem exceções, sem macros. O que resta é um conjunto limpo e ortogonal de construções: estruturas e interfaces para composição, goroutines e canais para simultaneidade e tratamento de erros explícito para confiabilidade. O poder da linguagem não vem da sintaxe complexa, mas da interação elegante de seus primitivos simples. Os genéricos (Go 1.18+) adicionam flexibilidade de tipo sem sacrificar a clareza característica do Go.