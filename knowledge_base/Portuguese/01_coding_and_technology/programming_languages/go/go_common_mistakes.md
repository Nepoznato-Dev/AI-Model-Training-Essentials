...
-->
# Go – Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns no Go. Cada entrada mostra a abordagem incorreta, explica por que ela falha e fornece a solução correta.
---

## 1. Captura de variável de loop em Goroutines
```go
...
## 5. Desreferência de ponteiro nulo em métodos estruturais
```go
...
## 7. Antipadrão: retornando variáveis ​​nomeadas prematuramente
```go
...
## 8. A ordem de iteração do mapa é aleatória
```go
...
## 10. Antipadrão: Poluição da Interface
```go
...
## 11. Não usando`sync.Once`para inicialização
```go
...
## Resumo
A simplicidade do Go é seu ponto forte, mas ele tem armadilhas sutis: vazamentos de goroutine de canais sem buffer, semântica de anexação de fatia que não modifica o original, chamadas adiadas acumuladas em loops e iteração de mapa que é intencionalmente aleatória. Os provérbios Go orientam o caminho: lidar com todos os erros, aceitar interfaces, mas retornar estruturas, usar receptores de ponteiro para tipos mutex e preferir primitivas de simultaneidade (`sync.Once`,`errgroup`, cancelamento de contexto) em vez do gerenciamento manual de goroutines. Escreva primeiro um código simples - Go recompensa a clareza em vez da inteligência.