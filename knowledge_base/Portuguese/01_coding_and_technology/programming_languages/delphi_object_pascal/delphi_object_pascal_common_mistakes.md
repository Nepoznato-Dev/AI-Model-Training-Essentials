...
-->
# Delphi/Object Pascal — Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns no Delphi/Object Pascal com correções.
---

## 1. Não liberar objetos (vazamentos de memória)
```pascal
...
## 3. Indexação de strings (baseado em 1)
```pascal
...
## 4. Não usar`inherited`em construtores
```pascal
...
## 5. Antipadrão: lógica de negócios em formulários
```pascal
...
## Resumo
Armadilhas Delphi: sempre libere objetos com try/finally, distinga`=`(comparação) de`:=`(atribuição), strings são indexadas em 1, chame`inherited`em construtores e separe a lógica de negócios da UI. A estrutura VCL/FMX da Delphi recompensa a separação clara de preocupações.