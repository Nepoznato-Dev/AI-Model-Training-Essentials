...
-->
# Lisp e Clojure – Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns em Lisp e Clojure com correções.
---

## 1. Confusão de citações
```lisp
...
## 2. Não entendendo`nil`vs`false`(Clojure)
```clojure
...
## 3. Modificando coleções (Clojure)
```clojure
...
## 4. Não usar`recur`para recursão de cauda (Clojure)
```clojure
...
## Resumo
Armadilhas Lisp/Clojure: a cotação impede a avaliação (use quasiquote com`,`),`nil`e`false`são falsas, mas distintas, as coleções são imutáveis ​​(revincular ou usar átomos), use`recur`para recursão final e use`gensym`em macros para evitar a captura de variáveis. O jeito do Lisp é: abraçar a imutabilidade, entender o sistema macro do leitor e escrever macros higiênicas.