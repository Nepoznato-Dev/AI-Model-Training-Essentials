...
## Por que Lisp/Clojure é importante
- **Homoiconicidade**: Código são dados — os programas podem manipular sua própria estrutura, permitindo macros poderosas.
- **Macros**: As macros Lisp operam no código como dados, permitindo estender a própria linguagem.
- **Programação funcional**: Lisp foi pioneiro em conceitos de FP ainda usados ​​hoje.
- **Clojure em JVM**: Lisp moderno com acesso completo à biblioteca Java, estruturas de dados imutáveis ​​e excelente simultaneidade.
- **Desenvolvimento orientado por REPL**: Desenvolvimento interativo com feedback imediato.
- **Simplicidade**: Clojure possui um design de linguagem pequeno e consistente — sem casos especiais.
## As compensações
| Limitação | Detalhes | Solução alternativa típica |
|-------|---------|-------------------|
| **Parênteses** | O uso intenso de`()`pode ser difícil de ler inicialmente | Use suporte IDE; aprenda a ver a estrutura |
| **Comunidade de nicho** | Mercado de trabalho pequeno em comparação com os idiomas convencionais | Comunidade ativa e apaixonada |
| **Tempo de inicialização do Clojure** | Baseado em JVM; inicialização lenta para CLIs | Use imagem nativa do GraalVM |
| **Dialetos Lisp** | Muitos Lisps incompatíveis (Common Lisp, Scheme, Emacs Lisp) | Escolha Clojure para trabalhos modernos |
| **Não é convencional** | Menos bibliotecas, frameworks e tutoriais | Aproveite o ecossistema Java (Clojure) |
---
...
### Átomos, referências, agentes e STM
```clojure
...
| Comando | Descrição |
|--------|-------------|
| `clj -M:dev`| Inicie o REPL com dependências de desenvolvimento |
| `clj -M:test`| Execute o conjunto de testes |
| `clj -M:run`| Execute o aplicativo |
| `clj -T:build uber`| Construir um super JAR |
| `lein new app my-app`| Criar projeto Leiningen |
| `lein test`| Executar testes (Leiningen) |
...
### test.check — Teste baseado em propriedades
```clojure
...
## Padrões de Projeto
### Sistema de componentes com montagem
```clojure
...
### Padrão de pipeline com rosqueamento
```clojure
...
|------|---------|-------|
| **Critério** | Benchmarking estatístico | `(bench (expr))`|
| **VisualVM** | Perfil JVM |  Comando`jvisualvm`|
| **clj-async-profil** | Perfil de CPU de baixa sobrecarga | `start`/`stop`/`serve`|
| **Tufte** | Perfil de tempo de execução | `(p :tag (expr))`|
...
## Quando usar Lisp/Clojure
| Cenário | Por que Clojure | Melhor Alternativa |
|----------|------------|-------------------|
| Back-ends da Web | Ring/Compojure são produtivos | Vá, Node.js para APIs mais simples |
| Processamento de dados | Excelente biblioteca de sequências | Python (Pandas), Scala (Spark) |
| Sistemas concorrentes | Dados imutáveis ​​+ STM | Vá, Erlang/Elixir |
| DSLs/extensão de linguagem | Macros são incomparáveis ​​| — |
| Desenvolvimento orientado por REPL | O melhor fluxo de trabalho interativo da categoria | — |
| Desenvolvimento geral de aplicações | Possível, mas nicho | Python, Java, Go |
| Aplicativos móveis | ClojureScript para aplicativos da web; não nativo | Rápido, Kotlin |
