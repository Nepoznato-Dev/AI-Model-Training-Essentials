...
Este documento cataloga os erros, armadilhas e antipadrões mais comuns em C#. Cada entrada mostra a abordagem incorreta, explica por que ela falha e fornece a solução correta.
---

## 1. Não descartando objetos `IDisposable`
```csharp
// ❌ WRONG — resource leak
...
## 3. Métodos assíncronos de vazio
```csharp
...
## 4. LINQ`.ToList()`Alocação desnecessária
```csharp
...
## 5. Capturando variável de loop (pré-C# 5.0)
```csharp
...
## 6. Não usar correspondência de padrões
```csharp
...
## 7. Antipadrão: Classes de Deus / Localizador de serviços
```csharp
...
## 10. Não usar registros para tipos de dados
```csharp
...
## Resumo
C# é uma linguagem bem projetada, mas possui armadilhas: não descartar objetos `IDisposable`, métodos assíncronos void, alocações LINQ desnecessárias e exceções de referência nula. O C# moderno (8+) fornece ferramentas poderosas: tipos de referência anuláveis, correspondência de padrões, registros, declarações de uso e expressões de comutação. Os princípios-chave: sempre descarte recursos, nunca use async void (exceto manipuladores de eventos), use injeção de construtor, prefira registros para dados e habilite tipos de referência anuláveis ​​para capturar bugs nulos em tempo de compilação.