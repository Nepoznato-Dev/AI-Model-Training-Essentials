...
Este documento fornece uma referência de sintaxe estruturada e abrangente para C++ moderno (C++ 17/20/23). Ele complementa a referência principal do C++, concentrando-se em padrões de sintaxe exaustivos, tabelas de operadores, mecânica de modelos e recursos modernos.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmética | `a + b`| O mesmo que C |
| `++``--` | Incremento/Decremento | `++i`,`i++`| Prefira pré-incremento |
| `==``!=``<``>``<=``>=` | Comparação | `a <=> b`| `<=>`é uma comparação de três vias (C++20) |
| `&&``\|\|``!`| Lógico | `a && b`| Curto-circuito |
| `&``\|``^``~``<<``>>` | Bit a bit | `a & b`| O mesmo que C |
| `?:`| Ternário | `cond ? a : b`| |
### Operadores específicos do C++
| Operador | Nome | Exemplo | Notas |
|----------|------|---------|-------|
| `::`| Resolução de escopo | `std::cout`| |
| `.*``->*` | Ponteiro para membro | `obj.*pmf`| |
| `<=>`| Nave espacial (C++20) | `a <=> b`| Retorna`strong_ordering`|
| `co_await`| Suspensão de corrotina | `co_await expr`| C++20 |
| `co_yield`| Rendimento da co-rotina | `co_yield value`| C++20 |
| `co_return`| Retorno de co-rotina | `co_return value`| C++20 |
...
## Fluxo de controle
### Ligações Estruturadas e Padrões Modernos
```cpp
...
## Ponteiros inteligentes e memória
```cpp
...
## Contêineres e algoritmos STL
### Visão geral do contêiner
| Recipiente | Cabeçalho | Encomendado | Pesquisa | Notas |
|-----------|--------|-----|--------|-------|
| `vector`| `<vector>`| Sim | Sobre(n) | Escolha padrão para sequências |
| `deque`| `<deque>`| Sim | Sobre(n) | Push/pop rápido em ambas as extremidades |
| `list`| `<list>`| Sim | Sobre(n) | Lista duplamente vinculada |
| `array`| `<array>`| Sim | Sobre(n) | Tamanho fixo, alocado em pilha |
| `set`/`multiset`| `<set>`| Sim | O(logn) | Árvore rubro-negra |
| `map`/`multimap`| `<map>`| Sim (por chave) | O(logn) | Valor-chave, árvore vermelho-preto |
| `unordered_set`| `<unordered_set>`| Não | O(1) média | Tabela hash |
| `unordered_map`| `<unordered_map>`| Não | O(1) média | Tabela hash |
### Algoritmos Chave
```cpp
#include <algorithm>
#include <numeric>
...
## Resumo
A sintaxe C++ abrange desde manipulação de ponteiro de baixo nível até metaprogramação de modelo de alto nível e assíncrono baseado em corrotina. A evolução da linguagem através do C++ 17/20/23 melhorou drasticamente a segurança (ponteiros inteligentes,`std::optional`, conceitos), expressividade (ligações estruturadas, intervalos,`std::format`) e desempenho (conceitos que permitem abstrações de custo zero, corrotinas para assíncrono sem sobrecarga de pilha). A filosofia central permanece inalterada: você não deve pagar pelo que não usa, e a linguagem deve suportar todos os níveis de abstração, desde bare metal até linguagens específicas de domínio.