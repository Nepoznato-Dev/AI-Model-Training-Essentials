...
-->
# Comparação entre idiomas - Fluxo de controle
## Declarações If/Else
```python
# Python
if x > 0:
...
| Idioma | Sintaxe |
|----------|--------|
| Pitão | `for i in range(10):`|
| JavaScript | `for (let i = 0; i < 10; i++)`|
| Ferrugem | `for i in 0..10`|
| Vá | `for i := 0; i < 10; i++`|
| Java | `for (int i = 0; i < 10; i++)`|
| C/C++ | `for (int i = 0; i < 10; i++)`|
| Rubi | `10.times { \|i\| ... }`ou`(0..9).each { \|i\| ... }`|
...
| C# | `x switch { 1 => ..., _ => ... }`(C# 8+) |
| Datilografado | Nenhuma correspondência de padrão nativo (use if/else) |
| Pitão | `match x: case 1: ... case _: ...`(Python 3.10+) |
| Prólogo | Correspondência de cláusula incorporada |
| Vá | `switch x { case 1: ... default: ... }`|