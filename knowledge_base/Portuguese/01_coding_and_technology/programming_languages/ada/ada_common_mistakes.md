...
---
# Ada – Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns no Ada com correções.
---

## 1. Não usando `pragma Strict`
```ada
...
## 2. Erros de intervalo em subtipos
```ada
...
## 3. Não usar parâmetros nomeados para legibilidade
```ada
...
## 4. Esquecendo de lidar com exceções
```ada
...
## Resumo
A digitação forte de Ada evita muitos bugs, mas requer disciplina: use`pragma Strict`, manipule`Constraint_Error`para subtipos, use associação de parâmetros nomeados para facilitar a leitura, sempre trate exceções e projete a comunicação da tarefa com cuidado para evitar impasses. Ada recompensa uma programação cuidadosa e preocupada com a segurança.