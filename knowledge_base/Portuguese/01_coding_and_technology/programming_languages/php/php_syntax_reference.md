...
-->
# PHP — Referência de sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para PHP (8.x). Ele complementa a referência principal do PHP, concentrando-se em padrões de sintaxe exaustivos, recursos modernos do PHP, programação orientada a objetos e expressões práticas.
---
...
| `==``!=``===``!==` | Igualdade | `$a === $b`| `===`verifica o tipo também |
| `<=>`| Nave espacial | `$a <=> $b`| Retorna -1, 0 ou 1 |
| `<``>``<=``>=` | Comparação | `$a >= $b`| |
| `&&``\|\|``!`| Lógico | `$a && $b`| Curto-circuito |
| `and``or``xor`| Lógico (baixa precedência) | `$a and $b`| Evite - use`&&`/`\|\|`|
| `??`| Coalescência nula | `$a ?? 'default'`| Retorna`'default'`se`$a`for nulo |
| `??=`| Atribuição de coalescência nula | `$a ??= 'default'`| Atribui apenas se`$a`for nulo |
| `?``:` | Ternário | `$a ? $b : $c`| |
| `.`| Concatenação | `$a . $b`| |
| `.=`| Concatenando atribuir | `$a .= $b`| |
| `=>`| Atribuição de matriz | `['key' => 'value']`| |
| `->`| Acesso de membro | `$obj->method()`| |
| `?->`| Operador Nullsafe | `$obj?->method()`| Retorna nulo se`$obj`for nulo |
| `::`| Acesso estático | `ClassName::method()`| |
| `instanceof`| Verificação de tipo | `$obj instanceof ClassName`| |
| `clone`| Clonagem de objetos | `$copy = clone $obj`| A cópia profunda requer`__clone()`|
...
## Matrizes e estruturas de dados
```php
// Indexed array
...
## Resumo
A sintaxe do PHP evoluiu dramaticamente desde suas origens de script até uma linguagem moderna e de tipo seguro. O PHP 8.x introduziu tipos de união, tipos de interseção, enums, fibras, classes somente leitura, atributos e callables de primeira classe — transformando-o de uma linguagem de script de tipo flexível em uma que suporta engenharia de software rigorosa. A força da linguagem continua sendo seu foco na web: com frameworks como Laravel e Symfony, um enorme ecossistema e suporte de hospedagem quase universal, a sintaxe do PHP atende milhões de aplicações. O PHP moderno recompensa os desenvolvedores que adotam seu sistema de tipos, usam o carregamento automático baseado em compositor e escrevem código limpo e orientado a objetos.