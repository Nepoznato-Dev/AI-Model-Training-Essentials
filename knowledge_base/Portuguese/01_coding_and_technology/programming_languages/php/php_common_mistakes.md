...
---
# PHP – Erros comuns e antipadrões
Este documento cataloga os erros, armadilhas e antipadrões mais comuns em PHP com correções.
---

## 1. Comparação vaga (`==`) Armadilhas
```php
...
## 3. Não usar declarações de tipo
```php
...
## 4. Variáveis ​​Variáveis ​​e Acesso Dinâmico
```php
...
## 6. Estado Global e Superglobais
```php
...
## 7. Confusão de array versus objeto
```php
...
## 8. Não usando o carregamento automático do Composer
```php
...
## 10. Antipadrão: lógica de negócios em visualizações
```php
...
## Resumo
A flexibilidade do PHP é tanto seu ponto forte quanto seu ponto fraco. As regras fundamentais: sempre use comparação estrita (`===`), use instruções preparadas para SQL, habilite`declare(strict_types=1)`, use o Composer para carregamento automático, gere novamente os IDs de sessão após a autenticação e mantenha a lógica de negócios fora das visualizações. O PHP moderno (8.x) com tipos estritos, tipos de união, expressões de correspondência e argumentos nomeados está muito longe do PHP 5 – adote os recursos modernos.