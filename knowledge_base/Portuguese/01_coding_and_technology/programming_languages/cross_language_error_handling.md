...
|-------|-----------|-----------|
| **Exceções (try/catch)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **Tipos de resultado/opção** | Rust, Haskell, OCaml, Scala (também) | `Result<T, E>`,`Maybe a`|
| **Vários valores de retorno** | Vai, Lua | `value, err := f()`|
| **Códigos de erro** | C, Fortran | Retornar código de erro, verificar manualmente |
| **Erros de correspondência de padrões** | Elixir, Erlang | `{:ok, value}`/`{:error, reason}`|
| **Condições (reiniciáveis)** | Lisp comum | `restart-case`,`handler-bind`|
| **Sem tratamento de erros** | Scratch, SQL (básico) | N/A |
| **Contratos** | Ada, Eiffel |  Condições`Pre`,`Post`|
...
## Resumo: Paradigmas de Tratamento de Erros
| Paradigma | Abordagem | Prós | Contras |
|----------|----------|------|------|
| **Exceções** | lançar/pegar | Caminho limpo e feliz, rastreamentos de pilha | Pode ser esquecido, controle o abuso de fluxo |
| **Tipos de resultados** | Retornar`Ok`/`Err`| Explícito, não pode ignorar erros | Verbose, operador`?`necessário |
| **Códigos de erro** | Retornar status interno | Simples, compatível com C | Fácil de ignorar, sem contexto |
| **Devoluções múltiplas** | `value, err := f()`| Explícito, idiomático | Pode esquecer de verificar`err`|
| **Correspondência de padrões** | `{:ok, v}`/`{:error, r}`| Exaustivo, funcional | Detalhado sem`with`|
| **Condições** | Manipuladores reinicializáveis ​​| Mais poderoso, recuperável | Complexo, raramente compreendido |