...
-->
# Scala — Referência de Sintaxe
Este documento fornece uma referência de sintaxe estruturada e abrangente para Scala (3.x com notas sobre as diferenças do 2.x). Ele complementa a referência principal do Scala, concentrando-se em padrões de sintaxe exaustivos, no sistema de tipos, nas construções de programação funcional e nas expressões idiomáticas do Scala.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmética | `a + b`| |
| `**`| Potência (GrandeDecimal) | `BigDecimal(2) ** 10`| Não integrado para Int |
| `==``!=` | Igualdade | `a == b`| Chama`.equals()`- seguro para nulos |
| `eq``ne` | Igualdade de referência | `a eq b`| Mesma instância |
| `<``>``<=``>=` | Comparação | `a >= b`| Requer`Ordered`|
| `&&``\|\|``!`| Lógico | `a && b`| Curto-circuito |
| `&``\|``^``~` | Bit a bit | `a & b`| |
| `<<``>>``>>>`| Mudança | `a << 2`| |
| `+:``:+``::``:::` | Operações de coleta | `1 +: list`| Associativo à direita se termina com`:`|
| `->`| Criação de tupla | `"key" -> "value"`| `("key", "value")`|
| `=>`| Função/seta | `x => x + 1`| Lambda ou mapeamento de tipo |
| `<-`| Gerador | `x <- List(1,2,3)`| Para compreensão |
### Regras do Operador
```scala
...
## Resumo
A sintaxe do Scala unifica a programação funcional e orientada a objetos em um poderoso sistema de tipos. Correspondência de padrões, compreensão for e classes de caso fornecem modelagem de dados expressiva. As características permitem uma composição flexível sem limitações de herança única. O sistema de tipos — com genéricos, variância, limites de tipo e tipos opacos — detecta erros em tempo de compilação enquanto permite abstrações de custo zero. Scala 3 simplifica a linguagem com enums, métodos de extensão, dados/uso e tipos opacos, tornando a linguagem mais acessível, mantendo todo o seu poder.