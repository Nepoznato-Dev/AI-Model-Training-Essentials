...
# Scala — Padrões Idiomáticos e Melhores Práticas
Este guia cobre padrões idiomáticos e práticas recomendadas para escrever código Scala 3 limpo e funcional.
---
...
## Resumo
As expressões idiomáticas do Scala enfatizam: imutabilidade (`val`), correspondência de padrões, tipos selados, opção/qualquer para segurança, compreensão for, coleções funcionais e classes de tipo (dadas/usadas no Scala 3). Siga o Guia de estilo Scala, use scalafmt para formatação e scalafix para linting. A sintaxe mais limpa, enumerações, tipos opacos e métodos de extensão do Scala 3 tornam a linguagem mais acessível, mantendo seu poder funcional.