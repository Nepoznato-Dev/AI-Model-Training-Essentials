...
Este documento consolida problemas de confiabilidade, incluindo vazamentos de memória, condições de corrida, erros de programação simultâneos e falhas de design do sistema.
---

## Vazamentos de memória
Vazamentos de memória ocorrem quando os programas alocam memória, mas não conseguem liberá-la quando não são mais necessários, causando um crescimento gradual do consumo de memória.
### Ouvintes de eventos esquecidos
**Exemplo incorreto (JavaScript):**```javascript
...
**Solução:** Use referências fracas para referências anteriores```python
import weakref
...
As condições de corrida ocorrem quando o comportamento do software depende do tempo relativo dos eventos, como a ordem de execução do thread.
### Verifique-Então-Aja Condição de Corrida
**Mau exemplo (Python):**```python
...
### Condição de corrida de leitura-modificação-gravação
**Mau exemplo:**```python
...
### Condição de corrida de inicialização lenta
**Mau exemplo:**```python
...
**Prevenção:** Sempre adquira bloqueios em uma ordem consistente```python
# Both threads:
lock_a.acquire()
lock_b.acquire()
# ... critical section ...
...
**Solução:** use agendamento justo ou vencimento prioritário
---
...
1. **Use RAII**: aquisição de recursos é padrão de inicialização
2. **Gerenciadores de contexto**: use instruções`with`para recursos
3. **Referências Fracas**: Para caches e referências anteriores
4. **Criação de perfil**: criação de perfil de memória regular em produção
### Simultaneidade
1. **Minimize o estado compartilhado**: prefira a passagem de mensagens
2. **Use abstrações de alto nível**: pools de threads, assíncronos/aguardam
3. **Bloquear pedidos**: evite impasses com pedidos consistentes
4. **Teste Simultaneamente**: Use testes de estresse e detectores de corrida
### Desempenho
1. **Perfil Primeiro**: Meça antes de otimizar
2. **Escolha do algoritmo**: Selecione estruturas de dados apropriadas
3. **Operações em lote**: reduza as viagens de ida e volta para bancos de dados/APIs
4. **Cache**: Armazene em cache cálculos caros de forma adequada
---

## Tópicos Relacionados
- **Vulnerabilidades de segurança**: consulte`security_vulnerabilities.md`para problemas relacionados à segurança
- **Qualidade do código**: consulte`code_quality_issues.md`para questões de manutenção
- **Design de API**: consulte`../07_api_system_design/api_system_design.md`para padrões de arquitetura do sistema
- **Falhas de AI/LLM**: Consulte`ai_llm_failures.md`para problemas de confiabilidade específicos de AI
---

## Padrões Adicionais de Confiabilidade do Sistema
### Esgotamento de recursos
**O que é:** Esgotar recursos do sistema (identificadores de arquivos, conexões, memória) por meio de alocação ilimitada.
**Mau exemplo:**```python
...
### Falhas em cascata
**O que é:** A falha em um componente desencadeia falhas nos componentes dependentes.
**Mau exemplo:**```python
...
### Verificações de integridade ausentes
**Mau exemplo:**```python
...
## Desafios do sistema distribuído
### Problemas de distorção do relógio
**Mau exemplo:**```python
...
### Manipulação de partição de rede
**Mau exemplo:**```python
...
**O que testar:**
1. **Falhas de instância**: eliminar pods/VMs aleatórios
2. **Problemas de rede**: Adicionar latência, descartar pacotes
3. **Esgotamento de recursos**: Preencher disco, esgotar memória
4. **Falhas de dependência**: interrupções simuladas do serviço
5. **Inclinação do relógio**: dessincronize os relógios do servidor
**Exemplo de experimento do caos:**```yaml
...
### Dias de Jogo
Exercícios regulares para testar a resiliência do sistema:
1. **Programação**: Dias de jogos trimestrais
2. **Escopo**: Ambiente semelhante ao de produção
3. **Cenários**: múltiplas falhas simultâneas
4. **Métricas**: Meça o tempo de detecção e o tempo de recuperação
5. **Aprendizagens**: documente e melhore runbooks
---
...
### Superprovisionamento de Resíduos
**Mau exemplo:**```markdown
...
### Prevenção de esgotamento durante o plantão
**Práticas inadequadas:**
- A mesma pessoa de plantão toda semana
- Sem caminho de escalação
- Alertas para itens não acionáveis
- Post-mortems que atribuem culpa
...
### Qualidade do runbook
**Runbook ruim:**```markdown
## Database Slow Query
...
1. Identify slow queries:
   ```sql
SELECIONE * DE pg_stat_activity 
   ONDE estado = 'ativo' 
   ORDER BY query_start;   ```

2. Check for locks:
   ```sql
SELECT * FROM pg_locks WHERE concedido = falso;   ```
