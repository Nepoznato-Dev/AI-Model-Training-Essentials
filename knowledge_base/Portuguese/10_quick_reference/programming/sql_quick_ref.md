...
Comandos SQL essenciais para operações de banco de dados.
---
...
### Sequência
-`CHAR(n)`- Sequência de comprimento fixo
-`VARCHAR(n)`- Sequência de comprimento variável
-`TEXT`- Texto grande
...
-`BINARY`- Binário fixo
-`VARBINARY`- Variável binária
---

## Funções úteis
### Funções de string```sql
...
- Normalize os dados adequadamente
- Use instruções preparadas para evitar injeção de SQL
❌ **Não:**
- Use funções em colunas indexadas em cláusulas WHERE
- Crie muitos índices (retarda a gravação)
- Use`SELECT DISTINCT`desnecessariamente
- Ignorar planos de execução de consulta
- Armazene valores computados quando eles puderem ser calculados
---
...
*Última atualização: julho de 2026 | Padrão SQL (compatível com MySQL/PostgreSQL)*