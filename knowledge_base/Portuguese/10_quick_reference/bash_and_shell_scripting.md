...
## Estrutura do script
Todo script Bash começa com uma linha **shebang**:
```bash
#!/usr/bin/env bash
...
| `set -u`| Tratar variáveis ​​não definidas como erros |
| `set -o pipefail`| O pipeline falha se algum comando nele falhar |
| `set -x`| Imprima cada comando antes de executar (modo de depuração) |
Execute um script:`chmod +x script.sh && ./script.sh`ou `bash script.sh`
---
...
| `$@`| Todos os argumentos posicionais (como palavras separadas) |
| `$*`| Todos os argumentos posicionais (como uma única string) |
...
| Teste | Significado |
|------|---------|
| `[[ -f "$x" ]]`| O arquivo existe e é um arquivo normal |
| `[[ -d "$x" ]]`| O diretório existe |
| `[[ -e "$x" ]]`| Arquivo/diretório existe (qualquer coisa) |
| `[[ -r "$x" ]]`| Legível |
| `[[ -w "$x" ]]`| Gravável |
| `[[ -x "$x" ]]`| Executável |
| `[[ -z "$x" ]]`| A string está vazia |
| `[[ -n "$x" ]]`| A string não está vazia |
| `[[ "$a" == "$b" ]]`| Igualdade de strings |
| `[[ "$a" != "$b" ]]`| Desigualdade de strings |
| `[[ "$a" =~ regex ]]`| Correspondência Regex |
| `[[ $a -eq $b ]]`| Igualdade inteira |
| `[[ $a -ne $b ]]`| Desigualdade inteira |
| `[[ $a -gt $b ]]`| Maior que |
| `[[ $a -lt $b ]]`| Menos que |
...
| Sintaxe | Significado |
|--------|---------|
| `cmd > file`| Redirecionar stdout para arquivo (sobrescrever) |
| `cmd >> file`| Redirecionar stdout para arquivo (anexar) |
| `cmd 2> errors.log`| Redirecionar stderr |
| `cmd &> all.log`| Redirecionar stdout e stderr |
| `cmd1 \| cmd2`| Canalize stdout de cmd1 para stdin de cmd2 |
| `cmd1 \|& cmd2`| Canalize stdout e stderr |
| `cmd < file`| Redirecionar arquivo para stdin |
| `cmd <<EOF ... EOF`| Documento aqui (entrada multilinha) |
| `cmd <<< "string"`| Here-string (entrada de linha única) |
```bash
# Here-document
cat <<EOF
Name: $NAME
...
## Resumo
O script Bash é um multiplicador de força para quem trabalha com computadores. Você não precisa memorizar todas as bandeiras – você precisa saber o que é possível e onde procurar. Comece com o básico: variáveis, condicionais, loops, pipes. Em seguida, aplique ferramentas de processamento de texto (grep, sed, awk) conforme necessário. E sempre use`set -euo pipefail`– no futuro você ficará grato.