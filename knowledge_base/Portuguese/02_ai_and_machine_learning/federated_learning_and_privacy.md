...
O aprendizado federado é uma técnica para treinar modelos de aprendizado de máquina em vários dispositivos ou organizações sem compartilhar os dados brutos. Em vez de enviar dados para um servidor central, cada dispositivo treina um modelo local e compartilha apenas as atualizações do modelo (gradientes ou pesos). O servidor central agrega essas atualizações para produzir um modelo global. Ele foi projetado pelo Google para treinar modelos de linguagem de teclado em telefones Android – e desde então se tornou uma técnica fundamental para IA que preserva a privacidade.
---
