...
Quick referência para Python 3.x sintaxe e common patterns.
...
- Use `#` para semgle-leme comments
- Use `"""triple quotes"""` para docstremgs e multi-leme stremgs
- Indentation matters (typically 4 spaces)
- Namemg conventions: `snake_case` para variables/functions, `PascalCase` para classes
- `__name__ == "__maem__"` to check if script is run directly
- Use `virtualenv` or `venv` para project isolation
- Install packages com `pip emstall package_name`
