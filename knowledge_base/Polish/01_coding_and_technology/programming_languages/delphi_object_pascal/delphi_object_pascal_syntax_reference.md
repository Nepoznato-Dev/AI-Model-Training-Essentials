...
# Delphi / Object Pascal — Informacje o składni
Ten dokument zawiera kompleksowe, uporządkowane omówienie składni Delphi/Object Pascal. Uzupełnia główne odniesienia do Delphi, koncentrując się na wyczerpujących wzorcach składni, funkcjach OOP, rodzajach, rekordach i idiomach językowych.
---
...
| | `-`| Odejmowanie | `A - B`|
| | `*`| Mnożenie | `A * B`|
| | `/`| Prawdziwy podział | `A / B`(zawsze zwraca wartość Real) |
| | `div`| Dzielenie całkowite | `A div B`(zwraca liczbę całkowitą) |
| | `mod`| Moduł | `A mod B`|
...
| **Logiczne** | `and`| Logiczne ORAZ | `A and B`|
| | `or`| Logiczne LUB | `A or B`|
| | `not`| Logiczne NIE | `not A`|
| | `xor`| Ekskluzywne LUB | `A xor B`|
| **Ciąg** | `+`| Konkatenacja | `'Hello' + ' ' + 'World'`|
| **Ustaw** | `in`| Członkostwo | `X in [1, 2, 3]`|
| | `+`| Unia | `SetA + SetB`|
| | `-`| Różnica | `SetA - SetB`|
| | `*`| Przecięcie | `SetA * SetB`|
| **Zadanie** | `:=`| Przypisz wartość | `X := 42`|
| **Adres** | `@`| Adres | `P := @X`|
| | `^`| Dereferencja | `P^`|
---
...
## Streszczenie
Delphi/Object Pascal łączy przejrzystą, czytelną składnię z potężnymi funkcjami OOP. Klasy, interfejsy, typy generyczne i rekordy zapewniają pełny zestaw narzędzi do tworzenia aplikacji. Wyraźne bloki początkowe/końcowe języka, zmienne wpisywane i identyfikatory wolne od wielkości liter sprawiają, że jest on przystępny. Frameworki VCL/FMX rozszerzają język o biblioteki komponentów wizualnych w celu szybkiego tworzenia aplikacji. W przypadku korporacyjnych aplikacji Windows i interfejsów baz danych Delphi pozostaje produktywnym wyborem.