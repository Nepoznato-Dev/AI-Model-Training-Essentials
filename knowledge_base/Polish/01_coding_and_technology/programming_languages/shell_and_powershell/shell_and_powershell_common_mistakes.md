...
---
# Powłoka i PowerShell — typowe błędy i anty-wzorce
Ten dokument kataloguje najczęstsze błędy, pułapki i antywzorce w powłokach Shell/Bash i PowerShell wraz z poprawkami.
---

## 1. Zmienne niecytowane (Bash)
```bash
...
## 4. Nie sprawdzanie kodów wyjścia (Bash)
```bash
...
## 5. PowerShell: interpolacja ciągów
```powershell
...
## 7. Zmienny zakres podpowłoki (Bash)
```bash
...
## 8. PowerShell: Obiekty potoku a ciągi znaków
```powershell
...
## Streszczenie
Pułapki skryptów powłoki: zawsze cytuj zmienne w Bash, używaj`[[ ]]`zamiast`[ ]`, używaj`set -euo pipefail`, sprawdzaj kody wyjścia i zrozum zakres podpowłoki. Pułapki programu PowerShell: do interpolacji używaj podwójnych cudzysłowów, używaj `-eq`, a nie`==`i pamiętaj, że obiekty potoku mają właściwości. Obydwa nagradzają ostrożne cytowanie i wyraźną obsługę błędów.