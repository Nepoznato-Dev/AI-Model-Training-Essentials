...
## Dlaczego Fortran ma znaczenie
- **Wydajność HPC**: Kompilatory Fortran tworzą jeden z najszybszych dostępnych kodów numerycznych — często dorównujący lub przewyższający C/C++ w przypadku operacji tablicowych.
- **Starsze bazy kodu**: Dziesięciolecia kodu naukowego (modele klimatyczne, symulacje fizyczne) są pisane w Fortranie.
- **Operacje na tablicach**: Natywna obsługa tablic wielowymiarowych ze składnią zaprojektowaną do obliczeń matematycznych.
- **Stabilność numeryczna**: Język i kompilatory są zoptymalizowane do obliczeń zmiennoprzecinkowych.
- **Coarrays**: Wbudowana obsługa programowania równoległego (Fortran 2008+).
- **Wciąż się rozwija**: Fortran 2023 dodaje nowoczesne funkcje, zachowując jednocześnie kompatybilność wsteczną.
## Kompromisy
...
## Podstawy składni (nowoczesny Fortran)
```fortran
program hello
    implicit none
...
### Typy pochodne i procedury powiązane z typami
```fortran
...
### Procedury ogólne i przeciążenie operatora
```fortran
module vector_ops
    implicit none
...
### Coarrays (wbudowana równoległość w Fortran 2008)
```fortran
...
### MPI (równoległość pamięci rozproszonej)
```fortran
program mpi_example
    use mpi
    implicit none
...
## Testowanie
### Struktura testowa (test fortran lub pFUnit)
```fortran
...
## Interoperacyjność
### Interoperacyjność C (ISO_C_BINDING)
```fortran
module c_interface
...
### Integracja z Pythonem (przez f2py lub bibliotekę współdzieloną)
```fortran
...
## Wzorce projektowe
### Wzorzec 1: Programowanie tablicowe (unikaj pętli)
```fortran
...
### Wzorzec 2: Sparametryzowane typy pochodne
```fortran
...
### Wzorzec 3: Wzorzec wywołania zwrotnego ze wskaźnikami procedur
```fortran
module integrator
    implicit none
    abstract interface
...
### Flagi optymalizacji kompilatora
```bash
...
| Scenariusz | Dlaczego Fortran | Lepsza alternatywa |
|---------|-----------|--------------------------------|
| HPC / superkomputery | Zoptymalizowany pod kątem wydajności numerycznej | C++ (z ostrożnością), Julia |
| Symulacje naukowe | Dekady sprawdzonego kodu | Python (do prototypowania), C++ |
| Modele klimatyczno-pogodowe | Starsze bazy kodów; wydajność | — |
| Fizyka obliczeniowa | Natywne operacje na tablicach | Python (NumPy), Julia |
| Modelowanie finansowe (HPC) | Wydajność w obliczeniach na dużą skalę | C++, Python |
...
## Streszczenie
Fortran jest oryginalnym językiem programowania naukowego i pozostaje potęgą w dziedzinie obliczeń o wysokiej wydajności. Nowoczesny Fortran to wydajny, rozwijający się język z natywnymi operacjami tablicowymi, obsługą programowania równoległego i interoperacyjnością języka C. Chociaż jego społeczność jest mała i wyspecjalizowana, Fortran w dalszym ciągu obsługuje jedne z najbardziej wymagających obciążeń obliczeniowych na świecie. W przypadku obliczeń numerycznych na dużą skalę Fortran pozostaje istotny.