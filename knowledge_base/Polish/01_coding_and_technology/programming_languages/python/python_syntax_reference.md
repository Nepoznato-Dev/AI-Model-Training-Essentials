...
|---------|------|---------|--------|-------|
| `+`| Dodatek | `3 + 2`| `5`| Również łączenie ciągów/list |
| `-`| Odejmowanie | `3 - 2`| `1`| Również jednoargumentowa negacja |
| `*`| Mnożenie | `3 * 2`| `6`| Również powtórzenie ciągu/listy |
| `/`| Prawdziwy podział | `7 / 2`| `3.5`| Zawsze zwraca float |
| `//`| Podział pięter | `7 // 2`| `3`| Zaokrągla w stronę ujemnej nieskończoności |
| `%`| Moduł | `7 % 2`| `1`| Znak dopasowania dzielnika |
| `**`| Potęgowanie | `2 ** 10`| `1024`| Prawicowe skojarzenie |
### Operatory porównania i logiczne
| Operator | Imię | Przykład | Notatki |
|---------|------|---------|-------|
| `==`| Równe | `x == y`| Porównanie wartości (nie tożsamość) |
| `!=`| Nierówne | `x != y`| |
| `<`,`>`,`<=`,`>=`| Zamawianie | `x >= y`| Możliwość połączenia łańcuchowego:`a < b < c`|
| `is`| Tożsamość | `x is y`| Ten sam obiekt w pamięci |
| `is not`| Nie tożsamość | `x is not None`| Preferowane zamiast`!= None`|
| `in`| Członkostwo | `x in collection`| O(1) dla zbiorów/dyktatów, O(n) dla list |
| `and`| Logiczne ORAZ | `a and b`| Zwarcie; zwraca ostatnią wartość typu prawda/fałsz |
| `or`| Logiczne LUB | `a or b`| Zwarcie; zwraca pierwszą wartość typu prawda/fałsz |
| `not`| Logiczne NIE | `not x`| |
### Operatory bitowe
| Operator | Imię | Przykład | Notatki |
|---------|------|---------|-------|
| `&`| ORAZ | `5 & 3`| `1`|
...
| 8 | `^`| Lewy |
| 9 | `\|`| Lewy |
| 10 | `==`,`!=`,`<`,`>`,`<=`,`>=`,`is`,`in`| Możliwość łączenia |
| 11 | `not`| Jasne |
| 12 | `and`| Lewy |
| 13 (najniższy) | `or`| Lewy |
### Przypisanie i przypisanie rozszerzone
```python
...
| Kolekcja | Zamówione | Zmienne | Duplikaty | Składnia | Wyszukaj |
|------------|--------|---------|------------|------------|--------|
| `list`| Tak | Tak | Tak | `[1, 2, 3]`| O(n) |
| `tuple`| Tak | Nie | Tak | `(1, 2, 3)`| O(n) |
| `dict`| Tak (3,7+) | Tak | Klawisze: Nie | `{"k": "v"}`| O(1) średnio |
| `set`| Nie | Tak | Nie | `{1, 2, 3}`| O(1) średnio |
| `frozenset`| Nie | Nie | Nie | `frozenset({1,2})`| O(1) średnio |
### Listy
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
## Streszczenie
Ten opis składni obejmuje pełny zestaw funkcji Pythona — od podstawowych operatorów i przepływu sterowania, poprzez zaawansowane metaprogramowanie, deskryptory i system typów. Składnia Pythona jest celowo minimalna i czytelna, ale jej głębia jest znaczna: dopasowywanie wzorców, generatory asynchroniczne, protokoły i metaklasy zapewniają potężne narzędzia do złożonych aplikacji. Język stale ewoluuje, z każdym wydaniem dodawane są funkcje poprawiające ekspresję przy jednoczesnym zachowaniu kompatybilności wstecznej.