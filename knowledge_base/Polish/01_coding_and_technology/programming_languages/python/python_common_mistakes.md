...
# Python — typowe błędy i anty-wzorce
Ten dokument kataloguje najczęstsze błędy, pułapki i antywzorce w Pythonie. Każdy wpis pokazuje nieprawidłowe podejście, wyjaśnia przyczynę niepowodzenia i podaje prawidłowe rozwiązanie. Zrozumienie tych pułapek pomoże Ci napisać solidniejszy kod w Pythonie.
---
...
**Dlaczego tak się dzieje:** Domyślne argumenty są oceniane raz w momencie definiowania funkcji, a nie przy każdym wywołaniu. Zmienna wartość domyślna (lista, dykt, zestaw) jest wspólna dla wszystkich wywołań.
---

## 2. Modyfikowanie listy podczas iteracji
```python
...
## 3. Późne wiązanie w zamknięciach
```python
...
## 4. Używanie`type()`zamiast `isinstance()`
```python
...
## 5. Nieużywanie menedżerów kontekstu dla zasobów
```python
...
## 6. Operator`is`do porównywania wartości
```python
...
## 8. Niezrozumienie reguł zakresu Pythona (LEGB)
```python
...
## 10. Anty-wzorzec: użycie nagiego`except:`z wyjątkiem
```python
...
## 11. Zwracanie zmiennych odwołań do stanu wewnętrznego
```python
...
## 12. Nieużywanie`setdefault`lub `collections.defaultdict`
```python
...
## 14. Niezrozumienie wyczerpania generatora
```python
...
## 15. Anty-wzorzec: Boskie Obiekty / Przeciążone Klasy
```python
...
## Streszczenie
Prostota Pythona jest zwodnicza — modyfikowalne wartości domyślne, późne zamykanie wiązań, reguły zakresu i zarządzanie zasobami mają subtelne zachowania, które zatrzymują programistów. Kluczowe zasady do zapamiętania: wartości domyślne są oceniane raz,`is`sprawdza tożsamość, a nie równość, zawsze używaj menedżerów kontekstu dla zasobów, wychwytuj określone wyjątki i faworyzuj kompozycję zamiast obiektów boskich. Pisanie kodu w Pythonie oznacza zrozumienie tych pułapek i wykorzystanie funkcji języka (menedżerów kontekstu, wyrażeń, generatorów, modułu `collections`) zgodnie z zamierzeniami.