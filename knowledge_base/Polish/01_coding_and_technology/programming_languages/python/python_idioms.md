...
## Streszczenie
Kod Pythona jest zgodny z Zen Pythona: czytelny, wyraźny, prosty i płaski. Kluczowe idiomy obejmują: rozumienie list/dykt/zestawów, menedżery kontekstu (`with`), obsługa błędów EAFP,`enumerate`/`zip`do iteracji,`dataclass`do kontenerów danych, wskazówki dotyczące typów, rozpakowywanie i sprawdzanie prawdziwości/fałszu. Społeczność Pythona ceni czytelność i „powinien istnieć jeden oczywisty sposób, aby to zrobić”. Aby uzyskać styl, postępuj zgodnie z PEP 8, użyj`ruff`do lintingu i, jeśli to możliwe, preferuj standardową bibliotekę zamiast rozwiązań innych firm.