...
|---------|------|---------|-------|
| `+``-``*``/``%`| Arytmetyka | `a + b`| |
| `..``...` | Zakres | `1...10`| `...`zamknięty; `..<`półotwarty |
| `==``!=``<``>``<=``>=` | Porównanie | `a == b`| Wymaga`Equatable`/`Comparable`|
| `===``!==` | Tożsamość | `a === b`| To samo odniesienie (tylko klasy) |
| `&&``\|\|``!`| Logiczne | `a && b`| Zwarcie |
| `??`| Nil łączy się | `a ?? b`| Zwraca `b`, jeśli`a`wynosi zero |
| `?.`| Opcjonalne łączenie | `a?.b`| Zwraca zero, jeśli`a`ma wartość nil |
| `!`| Wymuś rozpakowanie | `a!`| Awaria, jeśli zero — unikaj |
| `&`| Parametr wejściowy | `swap(&a, &b)`| Przejdź przez odniesienie |
### Brak operatorów obsługi
```swift
...
## Opakowywanie właściwości i narzędzia do tworzenia wyników
```swift
// Property wrapper
@propertyWrapper
struct Trimmed {
...
## Streszczenie
Składnia Swifta równoważy bezpieczeństwo z ekspresją. Opcje eliminują awarie zerowe, protokoły umożliwiają elastyczną kompozycję bez dziedziczenia, a nowoczesna współbieżność (asynchronizacja/oczekiwanie, aktorzy) umożliwia bezpieczne programowanie współbieżne. Język stale ewoluuje dzięki takim funkcjom, jak pakiety parametrów, makra i własność — każdy dodatek czyni Swift bezpieczniejszym i potężniejszym, zachowując jednocześnie jego podstawową obietnicę: kod, który jest bezpieczny w konstrukcji.