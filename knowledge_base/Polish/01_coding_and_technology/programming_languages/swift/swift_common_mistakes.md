...
-->
# Swift — typowe błędy i anty-wzorce
Ten dokument kataloguje najczęstsze błędy, pułapki i anty-wzorce w Swift. Każdy wpis pokazuje nieprawidłowe podejście, wyjaśnia przyczynę niepowodzenia i podaje prawidłowe rozwiązanie.
---

## 1. Wymuś opcje rozpakowywania
```swift
// ❌ WRONG — crash if nil
...
## 2. Zachowaj cykle z zamknięciami
```swift
...
## 3. Nieużywanie`enum`z powiązanymi wartościami
```swift
...
## 4. Nieporozumienie między typami wartościowymi a typami referencyjnymi
```swift
...
## 5. Anty-wzorzec: Kontroler widoku masywnego
```swift
...
## 6. Nieużywanie`guard`do wcześniejszego wyjścia
```swift
...
## 7. Zapominanie`weak`w Delegatach
```swift
...
## 8. Zamieszanie w indeksie ciągów
```swift
...
## 9. Nieużywanie`lazy`w przypadku drogich nieruchomości
```swift
...
## 10. Bezpieczeństwo wątków za pomocą `@MainActor`
```swift
...
## Streszczenie
Funkcje bezpieczeństwa Swifta zapobiegają wielu błędom, ale wprowadzają własne pułapki: wymuszenie rozpakowywania powoduje awarie, zachowanie cykli ukrytych w zamknięciach i delegatach, a semantyka typów wartości zaskakuje programistów pochodzących z języków referencyjnych. Szybki sposób to: nigdy nie wymuszaj rozpakowywania (`!`), używaj`guard`do wczesnych wyjść, używaj`weak`do delegatów i przechwytywania zamknięć, oznaczaj kod interfejsu użytkownika za pomocą`@MainActor`i dziel ogromne kontrolery widoku na modele widoków i usługi. System typów Swifta jest Twoim przyjacielem — pracuj z nim, a nie przeciwko niemu.