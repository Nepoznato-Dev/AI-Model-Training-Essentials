...
---
# Porównanie międzyjęzykowe — przepływ kontroli
## Instrukcje if/else
```python
# Python
if x > 0:
...
| Język | Składnia |
|---------|--------|
| Pythona | `for i in range(10):`|
| JavaScript | `for (let i = 0; i < 10; i++)`|
| Rdza | `for i in 0..10`|
| Idź | `for i := 0; i < 10; i++`|
| Jawa | `for (int i = 0; i < 10; i++)`|
| C/C++ | `for (int i = 0; i < 10; i++)`|
| Rubin | `10.times { \|i\| ... }`lub`(0..9).each { \|i\| ... }`|
| Szybki | `for i in 0..<10`|
| Kotlina | `for (i in 0 until 10)`|
| Haskell | `mapM_ f [0..9]`|
| Perł | `for my $i (0..9)`|
| R | `for (i in 1:10)`|
| PHP | `for ($i = 0; $i < 10; $i++)`|
| SQL | N/A (oparty na zestawie, bez pętli w standardowym SQL) |
| Uderz | `for i in {0..9}; do ... done`|
| Lua | `for i = 0, 9 do ... end`|
| MATLAB | `for i = 1:10 ... end`|
| Prolog | N/A (zamiast rekurencji) |
| Lisp | `(dotimes (i 10) ...)`|
| Ada | `for I in 0..9 loop ... end loop;`|
| COBOL | `PERFORM VARYING I FROM 0 BY 1 UNTIL I >= 10`|
| Erlang | N/A (zamiast tego rekurencja + listy) |
| OCaml | `for i = 0 to 9 do ... done`|
| Montaż | Pętla ręczna z CMP/JMP |
| Delfy | `for I := 0 to 9 do ... end;`|
| Zadrapanie | `repeat (10) ... end`|
...
## Dopasowanie wzorca / przełącznik
| Język | Konstruuj |
|---------|-----------|
| C | `switch (x) { case 1: ... break; }`|
| Jawa | `switch (x) { case 1 -> ... }`(Java 14+) |
| Rdza | `match x { 1 => ..., _ => ... }`|
| Haskell | `case x of 1 -> ...; _ -> ...`|
| Scala | `x match { case 1 => ... }`|
| Szybki | `switch x { case 1: ... default: ... }`|
| Kotlina | `when (x) { 1 -> ... else -> ... }`|
| OCaml | `match x with 1 -> ... \| _ -> ...`|
| Erlang | `case X of 1 -> ...; _ -> ... end`|
| Eliksir | `case x do 1 -> ...; _ -> ... end`|
| Dart | `switch (x) { 1 => ...; _ => ... }`(Dart 3.0) |
| PHP | `match($x) { 1 => ..., default => ... }`(PHP 8.0) |
| C# | `x switch { 1 => ..., _ => ... }`(C# 8+) |
| TypeScript | Brak natywnego dopasowania wzorca (użyj if/else) |
| Pythona | `match x: case 1: ... case _: ...`(Python 3.10+) |
| Prolog | Wbudowane w dopasowanie klauzuli |
| Idź | `switch x { case 1: ... default: ... }`|