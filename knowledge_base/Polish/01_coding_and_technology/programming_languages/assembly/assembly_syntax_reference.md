...
## Funkcje i konwencje wywoływania
```nasm
...
## Streszczenie
Składnia języka asemblera jest definiowana przez architekturę procesora i asembler (NASM, GAS, MASM). x86-64 zapewnia 16 rejestrów ogólnego przeznaczenia, wiele trybów adresowania i setki instrukcji obejmujących przenoszenie danych, arytmetykę, logikę, przepływ sterowania, SIMD i wywołania systemowe. Używana tutaj składnia NASM jest najpowszechniejsza w przypadku programowania w systemie Linux x86-64. Opanowanie asemblera wymaga zrozumienia rejestrów, stosu, konwencji wywoływania i sposobu, w jaki konstrukcje wysokiego poziomu są odwzorowywane na instrukcje maszynowe.