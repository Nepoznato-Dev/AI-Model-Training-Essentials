...
-->
# JavaScript — Informacje o składni
Ten dokument zawiera kompleksowe, uporządkowane omówienie składni języka JavaScript. Uzupełnia główne odniesienia do JavaScript, koncentrując się na wyczerpujących wzorcach składni, tabelach operatorów i mechanice wewnętrznej.
---
...
| `/`| Podział | `7 / 2`| `3.5`| |
| `%`| Reszta | `7 % 2`| `1`| |
| `**`| Potęgowanie | `2 ** 10`| `1024`| ES2016 |
| `++`| Przyrost | `x++`/`++x`| | Post- i pre-inkrementacja |
| `--`| Zmniejszenie | `x--`/`--x`| | Dekrementacja następcza i wstępna |
| `+x`,`-x`| Jednoargumentowy | `+"42"`→`42`| Wpisz przymus na numer |
### Porównanie i równość
| Operator | Imię | Przykład | Notatki |
|---------|------|---------|-------|
| `===`| Ścisła równość | `1 === 1`| Żadnego przymusu — zawsze preferuj to |
| `!==`| Ścisła nierówność | `1 !== "1"`| `true`— różne typy |
| `==`| Abstrakcyjna równość | `1 == "1"`| Wpisz przymus — unikaj |
| `!=`| Abstrakcyjna nierówność | `1 != "1"`| `false`— unikaj |
| `<`,`>`,`<=`,`>=`| Relacyjny | `"b" > "a"`| Leksykografika dla ciągów |
### Operatory logiczne i operatory zerowe
| Operator | Imię | Przykład | Notatki |
|---------|------|---------|-------|
| `&&`| Logiczne ORAZ | `a && b`| Zwraca pierwszą wartość fałszywą lub ostatnią |
| `\|\|`| Logiczne LUB | `a \|\| b`| Zwraca pierwszą prawdziwą lub ostatnią wartość |
| `!`| Logiczne NIE | `!true`| `false`|
| `??`| Zerowe łączenie | `a ?? b`| Zwraca`b`tylko wtedy, gdy`a`to`null`/`undefined`|
| `?.`| Opcjonalne łączenie | `obj?.prop?.method()`| Zwarcia do`undefined`|
### Operatory bitowe
| Operator | Imię | Przykład | Notatki |
|---------|------|---------|-------|
| `&`| ORAZ | `5 & 3`| `1`|
...
| `~`| NIE | `~5`| `-6`|
| `<<`| Lewy Shift | `5 << 1`| `10`|
| `>>`| Podpisano Prawy Shift | `-5 >> 1`| `-3`|
| `>>>`| Przesunięcie w prawo bez znaku | `-1 >>> 0`| `4294967295`|
### Przypisanie i destrukturyzacja
```javascript
// Basic assignment
let x = 10;
...
| Pierwszeństwo | Operatorzy | Łączność |
|------------|---------------|-------------|
| 1 (najwyższy) | `()``[]``.``?.` | Lewy |
| 2 | `new`(bez argumentów) | Jasne |
| 3 | `++``--` (postfiks) | Nie dotyczy |
| 4 | `!``~``+x``-x``typeof``void``delete``++``--`(przedrostek) | Jasne |
| 5 | `**`| Jasne |
| 6 | `*``/``%`| Lewy |
| 7 | `+``-` | Lewy |
| 8 | `<<``>>``>>>`| Lewy |
| 9 | `<``<=``>``>=``in``instanceof` | Lewy |
| 10 | `===``!==``==``!=` | Lewy |
| 11 | `&`| Lewy |
| 12 | `^`| Lewy |
| 13 | `\|`| Lewy |
...
| 16 | `??`| Lewy |
| 17 | `=``+=``-=`itp. | Jasne |
| 18 (najniższy) | `=>`(funkcja strzałkowa) | Jasne |
---
...
## Funkcje
### Deklaracje i wyrażenia funkcji
```javascript
...
## Struktury danych
### Przegląd kolekcji wbudowanych
| Kolekcja | Zamówione | Zmienne | Duplikaty | Typ klucza | Wyszukaj |
|------------|--------|---------|------------|--------------|--------|
| `Array`| Tak | Tak | Tak | Indeks (liczba) | O(n) |
| `Object`| Tak (wstawienie) | Tak | Klawisze: Nie | Ciąg/symbol | O(1) średnio |
| `Map`| Tak (wstawienie) | Tak | Klawisze: Nie | Dowolny | O(1) średnio |
| `Set`| Tak (wstawienie) | Tak | Nie | — | O(1) średnio |
| `WeakMap`| Nie | Tak | Klawisze: Nie | Tylko obiekt | O(1) średnio |
| `WeakSet`| Nie | Tak | Nie | Tylko obiekt | O(1) średnio |
### Tablice
```javascript
// Creation
...
### CommonJS (starsza wersja Node.js)
```javascript
// Export
...
### Rejestr słabych refów i finalizacji
```javascript
...
## Streszczenie
Składnia JavaScriptu jest z pozoru zwodniczo prosta, ale zawiera znaczną głębię. Język ewoluował od prostego narzędzia skryptowego do języka wieloparadygmatowego z klasami, modułami, generatorami, serwerami proxy i wyrafinowanym modelem asynchronicznym. Zrozumienie pierwszeństwa operatorów, pętli zdarzeń, łańcuchów prototypów i niuansów wiązania`this`oddziela kompetentnych programistów od ekspertów. Nowoczesny JavaScript (ES2020+) z opcjonalnym łączeniem w łańcuchy, łączeniem zerowym,`Promise.allSettled`i oczekiwaniem najwyższego poziomu w dalszym ciągu ogranicza schematy, zachowując jednocześnie kompatybilność wsteczną.