...
## Streszczenie
Współczesne idiomy JavaScript kładą nacisk na:`const`/`let`zamiast`var`, destrukturyzacja, funkcje strzałkowe, literały szablonów, async/await, operatory spread/rest, metody tablicowe (`map`/`filter`/`reduce`), moduły ES, opcjonalne łączenie łańcuchowe (`?.`), łączenie wartości zerowych (`??`) i pola klas z prywatnymi członkami. Postępuj zgodnie z ESLint, aby uzyskać jakość kodu, ładniejszy, jeśli chodzi o formatowanie i preferuj wzorce funkcjonalne zamiast stanu zmiennego. Społeczność JavaScript ceni zwięzłość, możliwość układania i nowoczesne funkcje ES2024+.