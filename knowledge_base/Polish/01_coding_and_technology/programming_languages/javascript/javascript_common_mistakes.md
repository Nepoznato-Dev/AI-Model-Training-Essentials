...
---
# JavaScript — typowe błędy i anty-wzorce
Ten dokument kataloguje najczęstsze błędy, pułapki i anty-wzorce w JavaScript. Każdy wpis pokazuje nieprawidłowe podejście, wyjaśnia przyczynę niepowodzenia i podaje prawidłowe rozwiązanie.
---

## 1. Zamieszanie w wiązaniach `this`
```javascript
...
## 2. Arytmetyka zmiennoprzecinkowa
```javascript
...
## 3.`var`Problemy z podnoszeniem i zasięgiem
```javascript
...
## 5. Brak reakcji na odrzucone obietnice
```javascript
...
## 6. Modyfikowanie obiektów podczas iteracji
```javascript
...
## 7. Antywzorzec: Piekło wywołania zwrotnego
```javascript
...
## 9. Obiekty podobne do tablic nie są tablicami
```javascript
...
## 10. Anty-wzorzec: Mutowanie parametrów funkcji
```javascript
...
## 11. Niezrozumienie pętli zdarzeń i mikrozadań
```javascript
...
## 13. Zapominanie o oczyszczeniu słuchaczy zdarzeń
```javascript
// ❌ WRONG — memory leak
class Component {
  mount() {
...
## 14. Płytka kopia kontra głęboka kopia
```javascript
...
## Streszczenie
Elastyczność JavaScriptu wiąże się z pułapkami: powiązanie`this`zależy od miejsca wywołania,`==`wykonuje wymuszanie, obietnice wymagają obsługi błędów, a kolejność pętli zdarzeń (mikrozadania przed makrozadaniami) jest nieoczywista. Nowoczesne zasady są proste: używaj`const`/`let`nigdy`var`, preferuj`===`zamiast`==`, zawsze obsługuj odrzucenia obietnic, używaj funkcji strzałek do wywołań zwrotnych, unikaj mutacji i czyść detektory zdarzeń. Przestrzeganie tych zasad eliminuje większość błędów JavaScript.