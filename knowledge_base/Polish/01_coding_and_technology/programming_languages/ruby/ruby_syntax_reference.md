...
| `==``!=``===``<=>` | Równość | `a == b`| `===`oznacza równość wielkości liter; `<=>`to statek kosmiczny |
| `eql?`| Równość wartości | `a.eql?(b)`| Bardziej rygorystyczne niż`==`(typ kontroli) |
| `equal?`| Tożsamość | `a.equal?(b)`| Ten sam obiekt? |
| `<``>``<=``>=` | Porównanie | `a >= b`| |
| `&&``\|\|``!`| Logiczne | `a && b`| Zwarcie; zwraca ostatnią obliczoną wartość |
| `and``or``not`| Logiczne (niski priorytet) | `a and b`| Unikaj — użyj`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | Bitowe | `a & b`| |
| `=~``!~` | Dopasowanie wyrażeń regularnych | `str =~ /pattern/`| Zwraca indeks lub zero |
| `..``...` | Zakres | `1..10`| `..`włącznie;  Ekskluzywny`...`|
| `?.`| Bezpieczna nawigacja | `user&.name`| Zwraca zero, jeśli użytkownik ma zero |
| `[]``[]=` | Dostęp do elementu | `arr[0]`| |
###Prawda
```ruby
...
## Streszczenie
Składnia Ruby została zaprojektowana z myślą o szczęściu programistów — czyta się ją jak po angielsku i nagradza ekspresję. Siła języka pochodzi z modelu obiektowego (wszystko jest obiektem), bloków i zamknięć, możliwości metaprogramowania oraz modułu Enumerable, który zapewnia bogaty interfejs API przetwarzania kolekcji. Ruby 3.x dodał dopasowywanie wzorców, sygnatury typu RBS i Ractor dla współbieżności, zachowując jednocześnie podstawową filozofię języka: sprawić, by programiści byli produktywni i szczęśliwi.