...
-->
# Ruby — typowe błędy i anty-wzorce
Dokument ten kataloguje najczęstsze błędy, pułapki i antywzorce w Rubim. Każdy wpis pokazuje nieprawidłowe podejście, wyjaśnia przyczynę niepowodzenia i podaje prawidłowe rozwiązanie.
---
...
## 2. Prawdziwość`nil`kontra `false`
```ruby
...
## 3. Interpolacja ciągów a łączenie
```ruby
...
## 4. Nieużywanie bloków/enumeratorów idiomatycznie
```ruby
# ❌ WRONG — C-style loop
i = 0
...
## 5. Nadużycie zmiennej globalnej
```ruby
...
## 6. Klasy podstawowe dotyczące łatania małp
```ruby
...
## 7. Niezrozumienie`return`w blokach
```ruby
...
## 9. Pomieszanie symboli i ciągów znaków
```ruby
...
## 10. Niezamrażanie literałów łańcuchowych
```ruby
...
## Streszczenie
Ekspresyjność Ruby może prowadzić do antywzorców: modyfikowania kolekcji podczas iteracji, małpiego łatania klas podstawowych, zmiennych globalnych i grubych modeli. Sposób Rubiego polega na tym, że: używaj modułów wyliczających (`each`,`map`,`reject`), preferuj symbole i argumenty słów kluczowych, zamrażaj literały łańcuchowe, wyodrębniaj obiekty usług z modeli i używaj udoskonaleń zamiast łatek małp. Ruby nagradza programistów, którzy stosują bloki, pisanie na klawiaturze i zasadę najmniejszego zaskoczenia.