...
---
# Scala — Odniesienie do składni
Ten dokument zawiera obszerne, uporządkowane odniesienia do składni języka Scala (3.x z uwagami na temat różnic w wersji 2.x). Uzupełnia główne odniesienia do języka Scala, skupiając się na wyczerpujących wzorcach składni, systemie typów, konstrukcjach programowania funkcjonalnego i idiomach Scali.
---
...
|---------|------|---------|-------|
| `+``-``*``/``%`| Arytmetyka | `a + b`| |
| `**`| Moc (duża liczba dziesiętna) | `BigDecimal(2) ** 10`| Nie wbudowane dla Int |
| `==``!=` | Równość | `a == b`| Wywołuje`.equals()`— null-safe |
| `eq``ne` | Równość odniesienia | `a eq b`| Ten sam przykład |
| `<``>``<=``>=` | Porównanie | `a >= b`| Wymaga`Ordered`|
| `&&``\|\|``!`| Logiczne | `a && b`| Zwarcie |
| `&``\|``^``~` | Bitowe | `a & b`| |
| `<<``>>``>>>`| Przesunięcie | `a << 2`| |
| `+:``:+``::``:::` | Operacje zbierania | `1 +: list`| Łącznik prawy, jeśli kończy się na`:`|
| `->`| Tworzenie krotki | `"key" -> "value"`| `("key", "value")`|
| `=>`| Funkcja/strzałka | `x => x + 1`| Lambda lub mapowanie typu |
| `<-`| Generator | `x <- List(1,2,3)`| Do zrozumienia |
### Zasady operatora
```scala
...
## Streszczenie
Składnia Scali ujednolica programowanie obiektowe i funkcjonalne w ramach potężnego systemu typów. Dopasowywanie wzorców, wyrażenia for i klasy przypadków zapewniają ekspresyjne modelowanie danych. Cechy umożliwiają elastyczną kompozycję bez ograniczeń pojedynczego dziedziczenia. System typów — z typami rodzajowymi, wariancjami, granicami typów i typami nieprzezroczystymi — wychwytuje błędy w czasie kompilacji, umożliwiając jednocześnie abstrakcję o zerowym koszcie. Scala 3 upraszcza język za pomocą wyliczeń, metod rozszerzeń, typów podanych/używanych i nieprzezroczystych, czyniąc język bardziej przystępnym, zachowując jednocześnie jego pełną moc.