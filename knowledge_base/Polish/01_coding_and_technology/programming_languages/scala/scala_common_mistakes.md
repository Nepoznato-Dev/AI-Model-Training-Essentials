...
# Scala — typowe błędy i anty-wzorce
Dokument ten kataloguje najczęstsze błędy, pułapki i anty-wzorce w Scali wraz z poprawkami.
---

## 1. Używanie`var`zamiast `val`
```scala
...
## 2. Nieużywanie dopasowywania wzorców dla typów zapieczętowanych
```scala
// ❌ WRONG — if/else chain
...
## 4. Przyszłe piekło wywołań zwrotnych
```scala
...
## 5. Nadużycia w zakresie niejawnych konwersji
```scala
...
## 8. Wpisz Erasure z typem ogólnym
```scala
...
## Streszczenie
Możliwości Scali wiążą się z odpowiedzialnością: preferuj`val`zamiast`var`, używaj dopasowywania wzorców dla typów zapieczętowanych, prawidłowo obsługuj`Option`(nigdy nie wywołuj`.get`), używaj for-comprehensions zamiast zagnieżdżonych kontraktów Futures i unikaj wartości null. Scala 3 upraszcza wiele z tych wzorców za pomocą wyliczeń, metod rozszerzeń i typów unii. Kluczowa zasada: jeśli kompilator może to sprawdzić, pozwól kompilatorowi to sprawdzić.