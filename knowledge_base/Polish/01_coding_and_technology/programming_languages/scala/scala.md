...
#Scala
Scala (Scalable Language) to skompilowany język programowania ze statycznym typem, który łączy paradygmaty programowania obiektowego i funkcjonalnego. Stworzona przez Martina Odersky'ego i wydana po raz pierwszy w 2004 roku, Scala działa na JVM (także Scala.js dla JavaScript i Scala Native). Został zaprojektowany, aby rozwiązać problem gadatliwości języka Java przy jednoczesnym zachowaniu pełnej interoperacyjności języka Java.
Scala to język stojący za Apache Spark (framework przetwarzania dużych zbiorów danych) i jest szeroko stosowany w inżynierii danych, systemach rozproszonych i usługach zaplecza. Firmy takie jak Twitter (obecnie X), LinkedIn, Netflix i The Guardian korzystają ze Scali.
---

## Dlaczego Scala ma znaczenie
- **Funkcjonalna + hybryda OOP**: łączy w sobie najlepsze cechy obu paradygmatów w jednym języku.
- **Zwięzły**: Znacznie mniej gadatliwy niż Java — dopasowywanie wzorców, klasy przypadków, wnioskowanie o typach.
- **Apache Spark**: Podstawowy język przetwarzania dużych zbiorów danych w oparciu o platformę Spark.
- **System typów**: zaawansowane funkcje, takie jak niejawne, klasy typów i algebraiczne typy danych.
- **Kompatybilność z JVM**: Używa wszystkich bibliotek Java; działa na tej samej maszynie JVM.
- **Akka / Pekko**: Popularny framework do budowania współbieżnych, rozproszonych systemów.
## Kompromisy
| Ograniczenie | Szczegóły | Typowe obejście |
|----------|---------|--------------------------------|
| **Złożoność** | Język ma wiele cech; może być trudny do opanowania | Zacznij od „dobrych części”; unikaj zbyt sprytnego kodu |
| **Czasy kompilacji** | Wolniejsze niż Java, szczególnie w przypadku typów złożonych | Użyj kompilacji przyrostowej (Bloop, sbt) |
| **Krzywa uczenia się** | Bardziej stromy niż Java czy Python | Inwestuj czas; wypłata jest znacząca |
| **Mniejszy rynek pracy** | Mniej ról niż Java czy Python | Silny w rolach związanych z inżynierią danych i backendem |
| **Niespójny styl** | Różne zespoły piszą bardzo różne Scala | Postępuj zgodnie z przewodnikami dotyczącymi stylu społeczności |
---
...
### Klasy typów (poprzez niejawne)
Scala 2 używa niejawnych parametrów do kodowania klas typów. Scala 3 wprowadza natywną składnię`given`/ `using`.
```scala
...
### Zapieczętowane cechy i wyliczenia (wyliczenia w Scala 3)
```scala
...
### Konfiguracja kompilacji (build.sbt)
```scala
// build.sbt
...
| `sbt compile`| Skompiluj główne źródła |
| `sbt test`| Uruchom wszystkie testy |
| `sbt run`| Uruchom główną klasę |
| `sbt runMain com.example.App`| Uruchom określoną klasę główną |
| `sbt console`| Uruchom REPL z projektem w ścieżce klasy |
| `sbt clean`| Wyczyść skompilowane wyjście |
| `sbt assembly`| Zbuduj gruby plik JAR (z wtyczką sbt-assembly) |
| `sbt scalafmt`| Sformatuj kod za pomocą Scalafmt |
| `sbt scalafmtCheck`| Sprawdź formatowanie kodu |
| `sbt ~compile`| Ciągła kompilacja (ponowna kompilacja w przypadku zmiany) |
### Formatowanie kodu (.scalafmt.conf)
```
# .scalafmt.conf
version = 3.7.14
...
Scala ma płynny dostęp do wszystkich bibliotek Java.
```scala
...
### Natywna współpraca (natywna Scala)
```scala
...
### Wzorzec repozytorium z obsługą błędów
```scala
...
|------|---------|-------|
| **JMH** | Mikrobenchmarking |  Wtyczka`sbt-jmh`|
| **WizualnaVM** | Profilowanie i monitorowanie JVM |  Komenda`jvisualvm`|
| **Profil asynchroniczny** | Profilowanie procesora/pamięci o niskim obciążeniu | Dołącz do działającej maszyny JVM |
| **Twój zestaw** | Profiler komercyjny | Integracja IDE |
| **sbt-scoverage** | Pokrycie kodu | `sbt coverage test coverageReport`|
### Analiza porównawcza z JMH
```scala
...
## Kiedy używać Scali
| Scenariusz | Dlaczego Scala | Lepsza alternatywa |
|---------|----------|--------------------------------|
| Duże dane (Spark) | Podstawowy język Spark | Python (PySpark) dla prostszych potoków |
| Systemy rozproszone (Akka) | Dojrzałe ramy współbieżności | Idź, Erlang/Eliksir |
| Backendy JVM | Zwięzła alternatywa dla Java | Java, Kotlin |
...
## Streszczenie
Scala to potężny język, który umożliwia programowanie funkcjonalne w JVM. Jest to język Apache Spark i mocny wybór w przypadku inżynierii danych, systemów rozproszonych i usług backendu. Krzywa uczenia się jest realna, ale nagrodą jest język, który jest zarówno wyrazisty, jak i wydajny. Zespołom, które już zainwestowały w ekosystem JVM, Scala oferuje bardziej zwięzłą i wydajną alternatywę dla Java.