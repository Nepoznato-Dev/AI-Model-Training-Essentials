...
| `==``!=``===``!==` | Równość | `$a === $b`| `===`sprawdza również typ |
| `<=>`| Statek kosmiczny | `$a <=> $b`| Zwraca -1, 0 lub 1 |
| `<``>``<=``>=` | Porównanie | `$a >= $b`| |
| `&&``\|\|``!`| Logiczne | `$a && $b`| Zwarcie |
| `and``or``xor`| Logiczne (niski priorytet) | `$a and $b`| Unikaj — użyj`&&`/`\|\|`|
| `??`| Połączenie zerowe | `$a ?? 'default'`| Zwraca `'default'`, jeśli`$a`ma wartość null |
| `??=`| Przypisanie łączenia wartości null | `$a ??= 'default'`| Przypisuje tylko wtedy, gdy`$a`ma wartość null |
| `?``:` | Trójskładnikowy | `$a ? $b : $c`| |
| `.`| Konkatenacja | `$a . $b`| |
| `.=`| Łączenie przypisania | `$a .= $b`| |
| `=>`| Przypisanie tablicy | `['key' => 'value']`| |
| `->`| Dostęp dla członków | `$obj->method()`| |
| `?->`| Operator Nullsafe | `$obj?->method()`| Zwraca wartość null, jeśli`$obj`ma wartość null |
| `::`| Dostęp statyczny | `ClassName::method()`| |
| `instanceof`| Wpisz kontrolę | `$obj instanceof ClassName`| |
| `clone`| Klonowanie obiektu | `$copy = clone $obj`| Głęboka kopia wymaga`__clone()`|
### Wpisz Żonglerka
```php
...
## Streszczenie
Składnia PHP ewoluowała radykalnie od swoich początków skryptowych do nowoczesnego, bezpiecznego języka. W PHP 8.x wprowadzono typy unii, typy skrzyżowań, wyliczenia, włókna, klasy tylko do odczytu, atrybuty i najwyższej klasy elementy wywoływalne — przekształcając go z języka skryptowego o luźnym typie w taki, który obsługuje rygorystyczną inżynierię oprogramowania. Siłą języka pozostaje jego skupienie na sieci: dzięki frameworkom takim jak Laravel i Symfony, ogromnemu ekosystemowi i niemal uniwersalnej obsłudze hostingu, składnia PHP obsługuje miliony aplikacji. Nowoczesny PHP nagradza programistów, którzy stosują jego system typów, korzystają z automatycznego ładowania opartego na kompozytorze i piszą czysty, obiektowy kod.