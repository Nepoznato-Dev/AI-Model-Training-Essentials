...
## Dlaczego Ada jest ważna
- **Systemy krytyczne dla bezpieczeństwa**: Zaprojektowane od podstaw dla systemów, w których awaria nie wchodzi w grę.
- **Silne typowanie**: Najbardziej rygorystyczny system typów ze wszystkich języków głównego nurtu — wychwytuje błędy w czasie kompilacji, których nie zauważają inne języki.
- **Wbudowana współbieżność**: Zadania (programowanie współbieżne) są częścią języka, a nie biblioteki.
- **Weryfikacja formalna**: Obsługuje formalne metody sprawdzania poprawności programu.
- **Funkcje niezawodności**: Wbudowane wyjątki, kontrakty (warunki wstępne/końcowe) i kontrole w czasie wykonywania.
- **Nowoczesna Ada**: Ada 2012 i Ada 2022 dodały nowoczesne funkcje przy jednoczesnym zachowaniu gwarancji bezpieczeństwa.
## Kompromisy
| Ograniczenie | Szczegóły | Typowe obejście |
|----------|---------|--------------------------------|
| **Niszowa społeczność** | Mała baza programistów w porównaniu do C, Java lub Python | Wyspecjalizowana, ale kompetentna społeczność |
| **Rozszerzona składnia** | Bardziej gadatliwy niż C lub Python | Zaakceptować jako część projektu zorientowanego na bezpieczeństwo |
| **Ograniczony ekosystem** | Mniej bibliotek niż języków głównego nurtu | Napisz niestandardowy kod; skorzystaj z rozbudowanej biblioteki standardowej |
| **Krzywa uczenia się** | Złożony język z wieloma funkcjami bezpieczeństwa | Zacznij od podzbioru „SPARK” Ady do prac o znaczeniu krytycznym dla bezpieczeństwa |
| **Dostępność kompilatora** | Mniej opcji kompilatora (główny jest GNAT) | GNAT jest darmowy i dobrze utrzymany |
---
...
### Oznaczone typy i dziedziczenie (OOP)
```ada
-- Abstract base type
package Shapes is
...
### Generics (polimorfizm w czasie kompilacji)
```ada
...
### Obliczenia równoległe (Ada 2022)
```ada
...
## Testowanie
### Struktura testowania jednostek
```ada
...
## Interoperacyjność
### Interoperacyjność C (interfejsy)
```ada
with Interfaces.C;
...
## Wzorce projektowe
### Wzór 1: Obserwator z chronionymi obiektami
```ada
...
### Wzorzec 2: Maszyna stanów z rozróżnionymi typami
```ada
...
### Wzorzec 3: RAII z ​​typami kontrolowanymi
```ada
...
### Abstrakcje o zerowym nakładzie finansowym
```ada
...
## Zastosowanie
### SPARK do certyfikacji o znaczeniu krytycznym dla bezpieczeństwa
```ada
...
## Kiedy używać Ada
| Scenariusz | Dlaczego Ada | Lepsza alternatywa |
|---------|---------|--------------------------------|
| Systemy krytyczne dla bezpieczeństwa | Zaprojektowany do tego; wsparcie weryfikacji formalnej | — |
| Lotnictwo / kosmonautyka | Certyfikowane kompilatory; Zgodność z DO-178C | — |
| Systemy obronne | dziedzictwo Departamentu Obrony; funkcje bezpieczeństwa | — |
| Kolej / urządzenia medyczne | Wymagania dotyczące wysokiej integralności | — |
...
## Streszczenie
Ada to język zbudowany z myślą o poprawności. Jego ścisły system typów, wbudowana współbieżność i formalna obsługa weryfikacji sprawiają, że jest to wybór dla systemów, w których awaria jest niedopuszczalna. Chociaż społeczność Ada jest niewielka w porównaniu z językami głównego nurtu, Ada pozostaje niezbędna w lotnictwie, obronności, przestrzeni kosmicznej i innych dziedzinach kluczowych dla bezpieczeństwa. W przypadku tych aplikacji rygorystyczne podejście Ady do inżynierii oprogramowania nie jest ograniczeniem – o to właśnie chodzi.