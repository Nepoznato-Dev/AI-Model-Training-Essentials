...
## 2.`=`vs`Is`do porównania obiektów
```vb
...
## 4. Nieużywanie`Using`do połączeń z bazą danych
```vb
...
## 5. Anty-wzorzec: logika biznesowa w procedurach obsługi zdarzeń
```vb
...
## Streszczenie
Pułapki VB.NET: zawsze używaj`Using`dla zasobów jednorazowych, odróżniaj`=`(wartość) od`Is`(odniesienie), włącz`Option Strict On`, używaj bloków`Using`do połączeń z bazą danych i oddziel logikę biznesową od procedur obsługi zdarzeń interfejsu użytkownika. Nowoczesny VB.NET to potężny język .NET — używaj właściwie jego funkcji.