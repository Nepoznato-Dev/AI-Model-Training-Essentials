...
---
# Perl — typowe błędy i anty-wzorce
Ten dokument kataloguje najczęstsze błędy, pułapki i antywzorce w Perlu wraz z poprawkami.
---

## 1. Nieużywanie`strict`i `warnings`
```perl
...
## 4. Nieużywanie odniesień do złożonych danych
```perl
...
## 5. Regex bez przejrzystości ograniczników
```perl
...
## 6. Nieużywanie nowoczesnych funkcji Perla
```perl
...
## Streszczenie
Elastyczność Perla jest niebezpieczna: zawsze`use strict; use warnings;`, zrozum kontekst (skalar vs lista), używaj zmiennych leksykalnych (`my`), używaj referencji do zagnieżdżonych danych i korzystaj z nowoczesnych funkcji Perla (podpisy,`say`,`//`). Perl nagradza tych, którzy mądrze korzystają z jego bogatego silnika wyrażeń regularnych i ekosystemu CPAN.