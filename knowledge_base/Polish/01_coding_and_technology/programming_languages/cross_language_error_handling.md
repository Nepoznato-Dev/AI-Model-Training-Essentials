...
-->
# Porównanie międzyjęzykowe — obsługa błędów
## Przegląd modeli obsługi błędów
| Modelka | Języki | Mechanizm |
|-------|-----------|----------|
| **Wyjątki (spróbuj/złap)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **Typy wyników/opcji** | Rust, Haskell, OCaml, Scala (także) | `Result<T, E>`,`Maybe a`|
| **Wiele zwracanych wartości** | Idź, Lua | `value, err := f()`|
| **Kody błędów** | C, Fortran | Zwróć kod błędu, sprawdź ręcznie |
| **Błędy dopasowania wzorca** | Eliksir, Erlang | `{:ok, value}`/`{:error, reason}`|
| **Warunki (możliwość ponownego uruchomienia)** | Wspólny Lisp | `restart-case`,`handler-bind`|
| **Brak obsługi błędów** | Scratch, SQL (podstawowy) | Nie dotyczy |
| **Umowy** | Ada, Eiffel |  Warunki`Pre`,`Post`|
...
## Podsumowanie: Paradygmaty obsługi błędów
| Paradygmat | Podejście | Plusy | Wady |
|---------|----------|------|------|
| **Wyjątki** | rzuć/złap | Czysta szczęśliwa ścieżka, ślady stosu | Można zapomnieć, nadużycie przepływu kontroli |
| **Typy wyników** | Zwróć`Ok`/`Err`| Wyraźne, nie można ignorować błędów | Szczegółowo, potrzebny operator`?`|
| **Kody błędów** | Zwróć status | Prosty, kompatybilny z C | Łatwe do zignorowania, bez kontekstu |
| **Wiele zwrotów** | `value, err := f()`| Wyraźne, idiomatyczne | Można zapomnieć sprawdzić`err`|
| **Dopasowanie wzoru** | `{:ok, v}`/`{:error, r}`| Wyczerpujący, funkcjonalny | Pełne bez`with`|
| **Warunki** | Programy obsługi z możliwością ponownego uruchomienia | Najpotężniejszy, możliwy do odzyskania | Złożone, rzadko rozumiane |