...
---
# TypeScript — typowe błędy i anty-wzorce
Ten dokument kataloguje najczęstsze błędy, pułapki i anty-wzorce w TypeScript. Każdy wpis pokazuje nieprawidłowe podejście, wyjaśnia przyczynę niepowodzenia i podaje prawidłowe rozwiązanie.
---
...
## 2. Wpisz asercje zamiast zabezpieczeń typu
```typescript
...
## 3. Nadużycie asercji innej niż null
```typescript
...
## 4. Pomieszanie interfejsów a typów
```typescript
...
## 7. Niespodzianki związane z pisaniem strukturalnym
```typescript
...
## 8. Błędy typu asynchronicznego/oczekiwania
```typescript
...
## 9. Anty-wzorzec: powtarzające się typy (bez użycia typów ogólnych)
```typescript
...
## 10. Nadmierne opcjonalne łączenie łańcuchowe
```typescript
...
## 11. Korzystanie z`namespace`(starsza wersja)
```typescript
...
## 12. Niewykorzystywanie dyskryminowanych związków zawodowych
```typescript
...
## Streszczenie
System typów TypeScriptu jest potężny, ale wymaga dyscypliny. Grzechem głównym jest sięgnięcie po`any`— użyj `unknown`, gdy typ jest naprawdę nieznany i zawęź go za pomocą osłon typu. Unikaj asercji o wartości innej niż null (`!`), obsługując prawidłowo wartość niezdefiniowaną. Używaj interfejsów dla kształtów obiektów, typów dla związków i typów obliczonych. Dyskryminowane związki zawodowe eliminują całe klasy błędów. Włącz`strict: true`od pierwszego dnia — rygorystyczność modernizacji jest bolesna. Celem jest uczynienie nielegalnych państw niereprezentowalnymi na poziomie typu.