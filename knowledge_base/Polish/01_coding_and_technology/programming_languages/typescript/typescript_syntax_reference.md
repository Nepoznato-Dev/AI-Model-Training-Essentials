...
-->
# TypeScript — odniesienie do składni
Ten dokument zawiera obszerne, uporządkowane informacje o składni języka TypeScript. Uzupełnia główne odniesienia do TypeScriptu, koncentrując się na systemie typów, rodzajach ogólnych, typach narzędzi i zaawansowanym programowaniu na poziomie typów.
---

## Podstawy systemu wpisywania typów
### Typy pierwotne i wbudowane
```typescript
// Primitives
...
## Dekoratory (TC39 Stage 3 / TypeScript 5.0+)
```typescript
// Class decorator
...
### Asercje i zawężenia dotyczące stałych
```typescript
...
## Streszczenie
System typów TypeScriptu jest jego cechą definiującą i jest szczególnie głęboki. Od podstawowych adnotacji poprzez typy generyczne, typy warunkowe, typy mapowane, typy literałów szablonów i`infer`, system typów jest zasadniczo językiem funkcjonalnym działającym w czasie kompilacji. Zrozumienie wzajemnych zależności między`interface`a`type`, typami ogólnymi i użytkowymi oraz typowaniem strukturalnym i nominalnym pomaga w pisaniu TypeScriptu, który jest zarówno bezpieczny, jak i ergonomiczny. Język stale ewoluuje, z każdym wydaniem dodając większą moc ekspresji, zachowując jednocześnie pełną kompatybilność wsteczną z JavaScript.