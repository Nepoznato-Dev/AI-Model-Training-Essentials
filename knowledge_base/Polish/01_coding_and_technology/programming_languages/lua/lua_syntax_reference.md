...
| `//`| Podział piętra | `7 // 2`| Zwraca 3 (Lua 5.3+) |
| `==``~=` | Równość | `a == b`| `~=`jest „nierówny” |
| `<``>``<=``>=` | Porównanie | `a >= b`| |
| `and``or``not`| Logiczne | `a and b`| Zwraca operandy, a nie wartości logiczne |
| `..`| Konkatenacja | `"hello" .. " world"`| |
| `#`| Długość | `#t`| Długość ciągu lub tabeli |
| `&``\|``~``<<``>>`| Bitowe | `a & b`| Lua 5.3+ (operacje na liczbach całkowitych) |
### Operatory logiczne — zwroty inne niż logiczne
```lua
...
|------------|-----------|
| 1 (najwyższy) | `^`|
| 2 | `not``#``-`(jednoargumentowy) |
| 3 | `*``/``//``%` |
| 4 | `+``-` |
| 5 | `..`(skojarzenie prawostronne) |
| 6 | `<<``>>` |
| 7 | `&`|
| 8 | `~`(binarny XOR) |
| 9 | `\|`|
...
## Tabele — uniwersalna struktura danych
```lua
...
## Streszczenie
Składnia Lua jest celowo minimalna — cały język można opisać na jednej stronie. Tabele służą jako uniwersalna struktura danych (tablice, słowniki, obiekty, moduły). Metatabele zapewniają rozszerzalność bez złożoności języka. Współprogramy umożliwiają wielozadaniowość kooperacyjną i eleganckie wzorce iteratorów. Siła języka wynika z jego możliwości osadzania: Lua została zaprojektowana do działania w aplikacjach hosta, zapewniając możliwości tworzenia skryptów przy minimalnym nakładzie pracy. W przypadku tworzenia gier, konfiguracji i systemów wbudowanych prostota Lua jest jego największą zaletą — mały język, który robi dokładnie to, czego potrzebuje i nic więcej.