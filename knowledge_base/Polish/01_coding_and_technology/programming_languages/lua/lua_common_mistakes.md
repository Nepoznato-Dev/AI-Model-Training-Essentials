...
---
# Lua — typowe błędy i anty-wzorce
Ten dokument kataloguje najczęstsze błędy, pułapki i anty-wzorce w Lua wraz z poprawkami.
---
...
## 2.`nil`kontra`false`w warunkach
```lua
...
## 7. Nieużywanie metatabli dla OOP
```lua
...
## Streszczenie
Minimalizm Lua tworzy pułapki: indeksowanie oparte na 1, tylko`nil`i`false`są fałszywe, tabele są referencjami (nie wartościami), zmienne są domyślnie globalne, a`pairs`jest nieuporządkowane, podczas gdy`ipairs`jest uporządkowane. Sposób Lua jest następujący: zawsze używaj`local`, używaj`ipairs`dla tablic i`pairs`dla słowników, używaj`table.concat`do budowania ciągów i używaj metatabel dla OOP. W prostocie Lua kryje się jej siła — ale szanuj przypadki Edge.