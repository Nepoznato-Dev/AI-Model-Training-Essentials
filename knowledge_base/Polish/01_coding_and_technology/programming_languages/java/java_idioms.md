...
## Streszczenie
Współczesne idiomy Java kładą nacisk na: rekordy danych, zapieczętowane klasy dla hierarchii, dopasowywanie wzorców, wyrażenia przełączające, bloki tekstu, strumienie do przetwarzania kolekcji, opcjonalne w przypadku nieobecności, try-with-resources do czyszczenia, wirtualne wątki do współbieżności i wstrzykiwania zależności. Postępuj zgodnie z Przewodnikiem po stylach Google Java, użyj`javafmt`lub`Spotless`do formatowania i Error Prone lub Checkstyle do lintingu. Nowoczesna Java (17–21+) z rekordami, zapieczętowanymi klasami, dopasowywaniem wzorców i wirtualnymi wątkami jest zwięzła i wyrazista — obejmuje nowe funkcje.