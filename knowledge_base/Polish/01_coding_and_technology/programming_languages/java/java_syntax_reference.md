...
| `-`| Odejmowanie | `3 - 2`| `1`| |
| `*`| Mnożenie | `3 * 2`| `6`| |
| `/`| Podział | `7 / 2`| `3`| Dzielenie liczb całkowitych dla`int`; `3.5`dla`double`|
| `%`| Reszta | `7 % 2`| `1`| |
...
|---------|------|---------|-------|
| `==`| Równe | `x == y`| Równość odniesienia dla obiektów |
| `!=`| Nierówne | `x != y`| |
| `<`,`>`,`<=`,`>=`| Relacyjny | `x >= y`| |
| `&&`| Logiczne ORAZ | `a && b`| Zwarcie |
| `\|\|`| Logiczne LUB | `a \|\| b`| Zwarcie |
| `!`| Logiczne NIE | `!true`| `false`|
| `instanceof`| Wpisz Sprawdź | `obj instanceof String`| Również dopasowywanie wzorców (Java 16+) |
### Operatory bitowe
| Operator | Imię | Przykład | Notatki |
|---------|------|---------|-------|
| `&`| ORAZ | `5 & 3`| `1`|
...
|------------|-----------|
| 1 (najwyższy) | Postfiks:`expr++``expr--` |
| 2 | Jednoargumentowy:`++expr``--expr``+expr``-expr``~``!` |
| 3 | Obsada:`(Type)expr`|
| 4 | Multiplikatyw:`*``/``%`|
| 5 | Dodatek:`+``-` |
| 6 | Przesunięcie:`<<``>>``>>>`|
| 7 | Relacyjne:`<``>``<=``>=``instanceof`|
| 8 | Równość:`==``!=` |
| 9 | Bitowe AND:`&`|
| 10 | Bitowy XOR:`^`|
| 11 | Bitowe LUB:`\|`|
...
### Lambdy i interfejsy funkcjonalne
```java
// Lambda syntax
...
## Struktury danych
### Struktura kolekcji
| Interfejs | Wdrożenia | Zamówione | Wariant bezpieczny dla wątków |
|----------|----------------|---------|----------------------------------|
| `List`| `ArrayList`,`LinkedList`| Tak | `CopyOnWriteArrayList`|
| `Set`| `HashSet`,`TreeSet`,`LinkedHashSet`| Różnie | `CopyOnWriteArraySet`|
| `Map`| `HashMap`,`TreeMap`,`LinkedHashMap`| Różnie | `ConcurrentHashMap`|
| `Queue`| `ArrayDeque`,`PriorityQueue`| Różnie | `ConcurrentLinkedQueue`|
| `Deque`| `ArrayDeque`| Obydwa końce | `ConcurrentLinkedDeque`|
### Listy
```java
// Creation
...
## Streszczenie
Składnia języka Java ewoluowała dramatycznie od jej początków w 1995 roku. Nowoczesna Java (21+) zawiera rekordy, zapieczętowane klasy, dopasowywanie wzorców, wirtualne wątki i szablony ciągów — dzięki czemu jest bardziej zwięzła i wyrazista niż kiedykolwiek. Podstawowa składnia pozostaje znana (podobna do C, silnie typowana, oparta na klasach), ale dodatki lambda, Stream API i`var`znacznie ograniczyły schemat. Siła języka leży w jego ogromnym ekosystemie, gwarancjach kompatybilności wstecznej i optymalizacji wydajności JVM poprzez kompilację JIT.