...
## 2. Modyfikowanie kolekcji podczas iteracji
```java
...
## 3. Niezamykanie zasobów (spróbuj z zasobami)
```java
...
## 4. Pułapki związane z automatycznym pakowaniem liczb całkowitych
```java
...
## 7. Brak obsługi przerwanego wyjątku
```java
...
## 8. Zwracanie wartości Null zamiast pustych kolekcji
```java
...
## 9. Zmienne obiekty jako klucze HashMap
```java
...
## 10. Anty-wzorzec: przechwytywanie`Exception`lub `Throwable`
```java
...
## 11. Nie zastępowanie`hashCode`podczas zastępowania `equals`
```java
...
## 12. Warunki wyścigu z podwójnym sprawdzaniem blokowania
```java
...
## Streszczenie
Szczegółowość Javy może ukryć subtelne błędy:`==`vs`equals()`, współbieżne modyfikacje, wycieki zasobów, pułapki wydajności autoboxingu i kontrakt równości/hashCode. Nowoczesne podejście do języka Java polega na tym, że: używaj`Objects.equals()`do porównań z zachowaniem wartości null, zawsze używaj funkcji try-with-resources, zwracaj puste kolekcje zamiast wartości null, wychwytuj określone wyjątki i postępuj zgodnie z zasadą pojedynczej odpowiedzialności. Funkcje Java 8+ (strumienie, opcjonalne, odniesienia do metod) ograniczają schematy, czyniąc kod bezpieczniejszym i bardziej wyrazistym.