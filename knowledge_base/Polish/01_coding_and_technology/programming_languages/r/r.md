...
- **Obliczenia statystyczne**: Najbardziej kompleksowy zbiór metod statystycznych w dowolnym języku.
- **Wizualizacja danych**: ggplot2 tworzy grafikę o jakości publikacyjnej. Gramatyka paradygmatu grafiki nie ma sobie równych.
- **Powtarzalne badania**: R Markdown / Quarto pozwala połączyć kod, wyniki i narrację w jednym dokumencie.
- **Standard akademicki**: Stosowany w statystyce, bioinformatyce, epidemiologii, ekologii, ekonomii i naukach społecznych.
- **Tidyverse**: Spójny zestaw pakietów (dplyr, ggplot2, tidyr, readr), dzięki którym analiza danych jest elegancka i spójna.
- **Bezpłatne i otwarte oprogramowanie**: Brak kosztów licencyjnych; aktywnie utrzymywana przez globalną społeczność.
## Kompromisy
| Ograniczenie | Szczegóły | Typowe obejście |
|----------|---------|--------------------------------|
| **Wydajność** | Domyślnie jednowątkowy; powolny dla dużych zbiorów danych | Użyj`data.table`, pakietów równoległych lub Rcpp do integracji z C++ |
| **Wykorzystanie pamięci** | Ładuje całe zbiory danych do pamięci RAM | Użyj`data.table::fread`, pakietu strzałek do przetwarzania poza rdzeniem |
| **Nie jest to język ogólnego zastosowania** | Niezręczne w przypadku tworzenia stron internetowych, programowania systemów lub aplikacji | Do zadań niestatystycznych używaj Pythona, Go lub JavaScriptu
| **Niespójna składnia** | Baza R ma dziwactwa; różne pakiety używają różnych konwencji | Użyj tidyverse dla spójności |
| **Rynek pracy** | Głównie role akademickie/badawcze | Role zajmujące się analityką danych coraz częściej preferują Python |
---
...
### Niestandardowa ocena i metaprogramowanie
```r
library(rlang)
...
### Zarządzanie zależnościami za pomocą Renv
```r
...
## Wzorce projektowe
### Wzorzec 1: Uporządkowana ocena elastycznych funkcji
```r
...
### Wzorzec 2: Programowanie funkcjonalne za pomocą mruczenia
```r
library(purrr)
...
### Wzorzec 3: Potok generowania raportów
```r
library(rmarkdown)
...
## Kiedy używać R
| Scenariusz | Dlaczego R | Lepsza alternatywa |
|---------|-------|--------------------------------|
| Analiza statystyczna | Najbardziej wszechstronne metody statystyczne | Python (statsmodels, scipy) |
| Wizualizacja danych | ggplot2 nie ma sobie równych pod względem jakości publikacji | Python (matplotlib, seaborn) do interaktywnego |
| Badania akademickie | Standard w wielu dziedzinach | — |
| Bioinformatyka | Bioconductor ma ponad 2000 specjalistycznych pakietów | Python dla rurociągów produkcyjnych |
| Raportowanie (R Markdown/Quarto) | Zintegrowana analiza + narracja | Jupyter (Python) |
| Produkcja systemów ML | Nie przeznaczony do wdrożenia | Python, Java |
...
### data.table Głębokie nurkowanie
```r
library(data.table)
...
### Porządne kolumny ewaluacyjne i dynamiczne
```r
library(dplyr)
library(rlang)
...
## Modelowanie statystyczne Głębokie nurkowanie
### Zaawansowana regresja
```r
...
### Uczenie maszynowe za pomocą schludnych modeli
```r
library(tidymodels)
