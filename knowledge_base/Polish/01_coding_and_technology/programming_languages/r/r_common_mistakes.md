...
-->
# R — Typowe błędy i anty-wzorce
Ten dokument kataloguje najczęstsze błędy, pułapki i anty-wzorce w R z poprawkami.
---

## 1. Rosnące ramki danych w pętlach
```r
# ❌ WRONG — O(n²) growth
result <- data.frame()
for (i in 1:1000) {
...
## 3. Niezrozumienie recyklingu wektorów
```r
...
## 5. Nieużywanie `stringsAsFactors = FALSE`
```r
...
## 6. Rodzina`apply`kontra Tidyverse
```r
...
## 8. Przypisanie globalne za pomocą `<<-`
```r
...
## Streszczenie
Interaktywny charakter języka R tworzy pułapki: rosnące ramki danych w pętlach (wstępne alokowanie lub wektoryzacja), niespodzianki związane z powtarzaniem wektorów, propagacja`NA`i możliwość nadpisywania`T`/ `F`. Sposób R to: wektoryzacja operacji, wstępna alokacja podczas zapętlania, jawna obsługa `NA`, używanie tidyverse do manipulacji danymi i unikanie`<<-`. R nagradza tych, którzy myślą wektorowo i korzystają z bogatego ekosystemu pakietów.