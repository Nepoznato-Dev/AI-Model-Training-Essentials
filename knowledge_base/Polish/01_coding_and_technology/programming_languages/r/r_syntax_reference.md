...
| `+``-``*``/``^`| Arytmetyka | `2^10`| |
| `%%`| Moduł | `7 %% 3`| Zwraca 1 |
| `%/%`| Dzielenie całkowite | `7 %/% 3`| Zwraca 2 |
| `==``!=` | Równość | `x == y`| wektoryzowane |
| `<``>``<=``>=` | Porównanie | `x >= y`| wektoryzowane |
| `&``\|``!`| Logiczne (wektorowe) | `x & y`| Elementarnie |
| `&&``\|\|` | Logiczne (skalarne) | `x && y`| Tylko pierwszy element |
| `%in%`| Członkostwo | `x %in% y`| |
| `<-``=` | Zadanie | `x <- 10`| `<-`jest idiomatyczny |
| `->`| Przypisz prawy | `10 -> x`| Rzadko używany |
| `<<-`| Globalne przypisanie | `x <<- 10`| Przypisz w środowisku nadrzędnym |
| `$`| Wyodrębnij element | `df$column`| |
| `[``[[` | Indeksowanie | `x[1]`/`x[[1]]`| `[`zwraca ten sam typ;  Ekstrakty`[[`|
| `%>%`| Fajka (magrittr) | `x %>% f()`| |
| `\|>`| Rura (podstawa R 4.1+) | `x \|> f()`| Rura rodzima |
...
## Streszczenie
Składnia języka R jest przeznaczona do obliczeń statystycznych i analizy danych. Wektorowy charakter operacji eliminuje pętle w przypadku większości zadań. Ekosystem tidyverse (dplyr, ggplot2, tidyr) zapewnia spójną, czytelną gramatykę do manipulacji i wizualizacji danych. Możliwości modelowania statystycznego R — od podstawowych testów t po modele z efektami mieszanymi — nie mają sobie równych pod względem głębokości. W przypadku analityki danych, statystyki i powtarzalnych badań R pozostaje niezbędnym narzędziem.