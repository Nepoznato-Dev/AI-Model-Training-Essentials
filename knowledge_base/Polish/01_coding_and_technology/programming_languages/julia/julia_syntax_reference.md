...
|---------|------|---------|-------|
| `+``-``*``/``^`| Arytmetyka | `2^10`| |
| `÷`| Dzielenie całkowite | `7 ÷ 3`| To samo co`div(7, 3)`|
| `%``\` | Moduł/reszta | `7 % 3`| |
| `==``!=` | Równość | `a == b`| Równość wartości |
| `===`| Tożsamość | `a === b`| Ten sam obiekt |
| `<``>``<=``>=` | Porównanie | `a >= b`| Możliwość połączenia łańcuchowego:`1 < x < 10`|
| `&&``\|\|``!`| Logiczne | `a && b`| Zwarcie |
| `&``\|``⊻``~` | Bitowe | `a & b`| `⊻`to XOR |
| `:`| Zakres | `1:10`| `start:step:stop`|
| `..`| Interwał | `1..10`| Wymaga zestawów interwałów |
| `∈``∉``⊆`| Ustaw członkostwo | `x ∈ S`| `in(x, S)`|
| `∘`| Skład | `f ∘ g`| `(f ∘ g)(x) == f(g(x))`|
| `\|>`| Rura | `x \|> f`| `f(x)`|
| `=>`| Para | `a => b`| Dla dykt |
| `...`| Plamka | `f(args...)`| Rozwiń kolekcję |
...
## Streszczenie
Składnia Julii jest czysta, matematyczna i wyrazista. Wielokrotne wysyłanie pozwala funkcjom zachowywać się inaczej w zależności od wszystkich typów argumentów, umożliwiając elegancką organizację kodu. Makra zapewniają potężne metaprogramowanie bez złożoności makr preprocesora w stylu C. System typów — obejmujący typy parametryczne, typy abstrakcyjne i typy unii — zapewnia zarówno elastyczność, jak i wydajność. Indeksowanie, zrozumienie i składnia bloków do-block Julii sprawiają, że jest to naturalne w obliczeniach matematycznych i naukowych. Język osiąga obietnicę „chodzenia jak Python, uruchamiania jak C” poprzez kompilację JIT z wnioskowaniem o typach.