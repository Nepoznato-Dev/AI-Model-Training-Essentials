...
---
# Kotlin — typowe błędy i anty-wzorce
Ten dokument kataloguje najczęstsze błędy, pułapki i antywzorce w Kotlinie. Każdy wpis pokazuje nieprawidłowe podejście, wyjaśnia przyczynę niepowodzenia i podaje prawidłowe rozwiązanie.
---

## 1. Typy platform i wyjątek NullPointerException
```kotlin
...
## 3.`data class`bez odpowiedniego`equals`/ `hashCode`
```kotlin
...
## 4. Nieprawidłowe użycie funkcji zakresu
```kotlin
...
## 5. Współprogramy: brak obsługi anulowania
```kotlin
...
## 6. Antywzorzec: Kotlin w stylu Java
```kotlin
...
## 7. Kompletność klas zapieczętowanych
```kotlin
...
## 9. Nieużywanie`by lazy`do kosztownej inicjalizacji
```kotlin
...
## 10. Zamieszanie związane z przepływem i danymi na żywo
```kotlin
...
## Streszczenie
Zwięzłość Kotlina może ukryć subtelne problemy: typy platform przed interoperacją Java, awarie `lateinit`, zmienne klasy danych przerywające kolekcje i anulowanie współprogramów. Sposób Kotlina to: zastosuj bezpieczeństwo zerowe (nigdy nie używaj`!!`), użyj odpowiedniej funkcji zakresu (`let`dla transformacji,`apply`dla konfiguracji,`also`dla efektów ubocznych), użyj zapieczętowanych klas z wyczerpującą`when`, obsługuj anulowanie współprogramów i pisz idiomatyczny Kotlin (nie Java ze składnią Kotlina). Kompilator jest Twoim przewodnikiem — jeśli ostrzega, słuchaj.