...
-->
# Kotlin — Informacje o składni
Ten dokument zawiera kompleksowe, uporządkowane informacje o składni Kotlina (2.0+). Uzupełnia główne odniesienia do Kotlina, koncentrując się na wyczerpujących wzorcach składni, bezpieczeństwie zerowym, współprogramach i idiomach Kotlina.
---
...
|---------|------|---------|-------|
| `+``-``*``/``%`| Arytmetyka | `a + b`| |
| `..`| Zakres | `1..10`| Zakres obejmujący |
| `until`| Zakres półotwarty | `1 until 10`| `1..9`|
| `downTo`| Zakres malejący | `10 downTo 1`| |
| `==``!=` | Równość strukturalna | `a == b`| Wywołuje`.equals()`|
| `===``!==` | Równość referencyjna | `a === b`| Ten sam przykład |
| `<``>``<=``>=` | Porównanie | `a >= b`| Wymaga`Comparable`|
| `&&``\|\|``!`| Logiczne | `a && b`| Zwarcie |
| `?:`| Operator Elvisa | `a ?: b`| Zwraca `b`, jeśli`a`ma wartość null |
| `?.`| Bezpieczne połączenie | `a?.b`| Zwraca wartość null, jeśli`a`ma wartość null |
| `!!`| Twierdzenie inne niż null | `a!!`| Zgłasza NPE, jeśli null |
| `as`| Obsada | `a as String`| Zgłasza wyjątek ClassCastException |
| `as?`| Bezpieczny odlew | `a as? String`| Zwraca wartość null w przypadku niepowodzenia |
| `in`| Członkostwo | `x in list`| |
| `!in`| Nie jestem członkiem | `x !in list`| |
| `is`| Wpisz kontrolę | `x is String`| Inteligentne oddanie po sprawdzeniu |
| `!is`| Nie wpisz | `x !is String`| |
...
## Streszczenie
Składnia Kotlina jest zwięzła, bezpieczna i kompatybilna z Javą. Bezpieczeństwo zerowe eliminuje całą klasę błędów w czasie kompilacji. Współprogramy zapewniają naturalny sposób pisania kodu asynchronicznego. Zapieczętowane klasy, klasy danych i funkcje rozszerzeń umożliwiają ekspresyjne modelowanie domen. Siła języka wynika z wzajemnego oddziaływania jego funkcji — funkcji zakresu, DSL, klas wbudowanych i ujednoliconych typów generycznych, które razem tworzą kod, który jest zarówno bezpieczny, jak i przyjemny w pisaniu.