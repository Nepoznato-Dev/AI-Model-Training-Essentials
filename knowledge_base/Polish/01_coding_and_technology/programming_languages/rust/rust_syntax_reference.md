...
|---------|------|---------|-------|
| `+`| Dodatek | `3 + 2`| Również konkatenacja`String`poprzez`format!`|
| `-`| Odejmowanie | `3 - 2`| |
| `*`| Mnożenie | `3 * 2`| |
| `/`| Podział | `7 / 2`| Dzielenie liczb całkowitych:`3`|
| `%`| Reszta | `7 % 2`| `1`|
| `-x`| Negacja | `-5`| |
### Operatory porównania i operatory logiczne
| Operator | Imię | Przykład | Notatki |
|---------|------|---------|-------|
| `==`| Równe | `x == y`| Wymaga`PartialEq`|
| `!=`| Nierówne | `x != y`| |
| `<`,`>`,`<=`,`>=`| Zamawianie | `x >= y`| Wymaga`PartialOrd`|
...
| `\|`| LUB | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `!`| NIE | `!0b1010u8`| Dopełnienie bitowe |
| `<<`| Lewy Shift | `5 << 1`| `10`|
| `>>`| Prawy Shift | `5 >> 1`| `2`|
### Pierwszeństwo operatora (od najwyższego do najniższego)
| Pierwszeństwo | Operatorzy | Notatki |
|------------|----------|-------|
| 1 (najwyższy) |  Ścieżki`()``.` :: | |
| 2 | Wywołania metod, dostęp do pól | Automatyczne derefowanie |
| 3 | `-`(negacja),`!`,`*`(deref) | |
| 4 | `as`(obsada typu) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
| 14 | `..``..=` | Zakres |
| 15 | `=``+=` itp. | Zadanie |
| 16 (najniższy) | `return``break``continue`| |
---
...
| Wpisz | Stos/Sterta | Rozmiar | Zmienność | Notatki |
|------|----------|------|------------|------|
| Elementy pierwotne (`i32`,`f64`,`bool`,`char`) | Stos | Naprawiono | Domyślnie niezmienne | `Copy`|
| `[T; N]`(tablica) | Stos | Naprawiono | Możliwość zmiany za pomocą`mut`| Stały rozmiar, `Copy`, jeśli T wynosi`Copy`|
| `Vec<T>`| Kupa | Dynamiczny | Możliwość zmiany za pomocą`mut`| Uprawy |
| `String`| Kupa | Dynamiczny | Możliwość zmiany za pomocą`mut`| UTF-8, możliwość uprawy |
| `&[T]`(plasterek) | Stos (gruby ptr) | Widok dynamiczny | Zależy od`&`lub`&mut`| Pożyczony widok |
| `HashMap<K, V>`| Kupa | Dynamiczny | Możliwość zmiany za pomocą`mut`| O(1) średnie wyszukiwanie |
| `BTreeMap<K, V>`| Kupa | Dynamiczny | Możliwość zmiany za pomocą`mut`| Wyszukiwanie O(log n), uporządkowane |
| `HashSet<T>`| Kupa | Dynamiczny | — | O(1) średnio, bez duplikatów |
### Wektory
```rust
// Creation
...
### Wspólne standardowe cechy bibliotek
```rust
...
## Współbieżność
### Wątki i przekazywanie wiadomości
```rust
use std::thread;
...
### Asynchronizacja/Oczekiwanie (Tokio)
```rust
// Async function
...
## Streszczenie
Składnia Rusta jest zbudowana w oparciu o gwarancje bezpieczeństwa egzekwowane w czasie kompilacji. System własności, polimorfizm oparty na cechach, dopasowywanie wzorców i abstrakcje o zerowym koszcie tworzą spójny projekt, w którym kompilator jest rygorystycznym, ale uczciwym partnerem. Od podstawowego przepływu sterowania po zaawansowane funkcje, takie jak makra, niebezpieczny kod i inteligentne wskaźniki, każda konstrukcja służy celowi, jakim jest bezpieczne i wydajne programowanie systemów. Zrozumienie zależności pomiędzy okresami życia, pożyczkami i systemem typów jest kluczem do opanowania Rusta.