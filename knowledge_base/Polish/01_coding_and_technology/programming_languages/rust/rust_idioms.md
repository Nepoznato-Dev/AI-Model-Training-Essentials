...
## Streszczenie
Idiomy Rusta podkreślają: własność i pożyczanie,`Result`/`Option`do obsługi błędów, wyliczenia z metodami, cechy abstrakcji, łączenie iteratorów, inteligentne wskaźniki dla stanu współdzielonego i abstrakcje o zerowym koszcie. Postępuj zgodnie z`rustfmt`do formatowania,`clippy`do lintingu i wytycznymi Rust API. Społeczność Rusta ceni bezpieczeństwo, wydajność i poprawność — „jeśli się kompiluje, to działa”. Preferuj`&str`zamiast`String`w argumentach funkcji, użyj`?`do propagacji błędów i uzyskaj wspólne cechy za pomocą`#[derive(Debug, Clone, ...)]`.