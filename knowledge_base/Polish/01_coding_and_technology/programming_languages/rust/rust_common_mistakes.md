...
---
# Rdza — typowe błędy i anty-wzorce
Dokument ten kataloguje najczęstsze błędy, pułapki i antywzorce w Rust. Każdy wpis pokazuje nieprawidłowe podejście, wyjaśnia przyczynę niepowodzenia i podaje prawidłowe rozwiązanie. Kompilator Rusta wychwytuje wiele błędów, ale zrozumienie tych wzorców przyspieszy proces uczenia się.
---

## 1. Walka z modułem sprawdzającym pożyczki — niepotrzebne klonowanie
```rust
...
## 2. Zamieszanie między ciągiem a &str
```rust
...
## 3. Ignorowanie wyników z`unwrap()`w produkcji
```rust
...
## 4. Błędy w adnotacjach na całe życie
```rust
...
## 5. Anty-wzorzec: używanie`Rc`/`RefCell`wszędzie
```rust
...
## 6. Niezrozumienie semantyki ruchu
```rust
...
## 7. Anty-wzorzec: Nadmierny`to_string()`/ `to_owned()`
```rust
...
## 9. Unieważnienie iteratora — niepotrzebne zbieranie
```rust
...
## 10. Anty-wzorzec: Wyliczenie bez metod
```rust
...
## 11. Nieużywanie`#[derive]`dla typowych cech
```rust
...
## 12. Niebezpieczny kod bez dokumentacji
```rust
...
## Streszczenie
Kompilator Rusta jest rygorystyczny, ale sprawiedliwy — większość błędów uczy czegoś o bezpieczeństwie pamięci. Największymi błędami nie są błędy kompilatora, ale anty-wzorce: niepotrzebne klonowanie w celu uspokojenia funkcji sprawdzania pożyczek, używanie`unwrap()`w produkcji, sięganie po`Rc<RefCell<T>>`przed próbą posiadania i przydzielanie ciągów znaków, jeśli wystarczą odniesienia. Nastawienie Rustaceana jest następujące: pozwól kompilatorowi poprowadzić Cię w stronę abstrakcji o zerowych kosztach. Klonuj tylko wtedy, gdy jest to konieczne, propaguj błędy za pomocą`?`, preferuj`&str`zamiast`String`i dokumentuj każdy blok `unsafe`.