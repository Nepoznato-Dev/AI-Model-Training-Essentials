...
-->
# C — Informacje o składni
Ten dokument zawiera kompleksowe, uporządkowane odniesienia do składni języka C (C11/C17 z uwagami C23). Uzupełnia główne odniesienia do języka C, koncentrując się na wyczerpujących wzorcach składni, tablicach operatorów, mechanice wskaźników i zarządzaniu pamięcią.
---
...
| `*`| Mnożenie | `3 * 2`| `6`| |
| `/`| Podział | `7 / 2`| `3`| Obcięcie liczb całkowitych dla typów int |
| `%`| Moduł | `7 % 2`| `1`| Tylko dla typów całkowitych |
| `++`| Przyrost | `i++`/`++i`| | Post- i pre-inkrementacja |
| `--`| Zmniejszenie | `i--`/`--i`| | Dekrementacja następcza i wstępna |
...
| `\|`| LUB | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `~`| Uzupełnij | `~0`| Wszystkie bity odwrócone |
| `<<`| Lewy Shift | `1 << 3`| `8`|
| `>>`| Prawy Shift | `8 >> 3`| `1`(implementacja zdefiniowana dla podpisu) |
### Operatory wskaźników i adresów
| Operator | Imię | Przykład | Notatki |
|---------|------|---------|-------|
| `&`| Adres | `&x`| Zwraca wskaźnik do x |
| `*`| Dereferencja | `*ptr`| Dostęp do wartości przy wskaźniku |
| `->`| Dostęp do członka poprzez wskaźnik | `p->x`| Odpowiednik`(*p).x`|
| `.`| Dostęp dla członków | `s.field`| Bezpośredni dostęp do członka struktury |
| `[]`| Indeks dolny tablicy | `arr[i]`| Odpowiednik`*(arr + i)`|
### Pierwszeństwo operatora (od najwyższego do najniższego)
| Pierwszeństwo | Operatorzy | Łączność |
|------------|---------------|-------------|
| 1 (najwyższy) | `()``[]``->``.` | Od lewej do prawej |
| 2 | `!``~``+``-` (jednoargumentowy)`*``&``sizeof``(type)` | Od prawej do lewej |
...
| 13 | `? :`(trójskładnikowy) | Od prawej do lewej |
| 14 | `=``+=``-=``*=``/=``%=``&=``\|=``^=``<<=``>>=`| Od prawej do lewej |
| 15 (najniższy) | `,`(przecinek) | Od lewej do prawej |
---
...
## Struktury, związki i wyliczenia
```c
// Struct
struct Point {
    double x;
...
## Standardowe podstawowe informacje biblioteczne
```c
...
## Streszczenie
Składnia języka C jest minimalna i bliska sprzętowi — wskaźniki, ręczne zarządzanie pamięcią i cienka warstwa abstrakcji na instrukcjach maszynowych. Siła języka wynika z tej prostoty: prawie nie ma ukrytych kosztów, nie ma narzutów na czas wykonania i pełna kontrola nad układem pamięci. Preprocesor, choć potężny, wymaga dyscypliny, aby móc go bezpiecznie używać. Modern C (C11/C17/C23) dodał ważne funkcje bezpieczeństwa, takie jak `_Static_assert`, `_Generic`,`nullptr`i `typeof`, czyniąc język coraz bezpieczniejszym, zachowując jednocześnie podstawową filozofię zaufania programiście.