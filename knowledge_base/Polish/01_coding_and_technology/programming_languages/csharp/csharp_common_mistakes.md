...
## 1. Nieutylizacja obiektów `IDisposable`
```csharp
// ❌ WRONG — resource leak
...
## 3. Metody asynchronicznej pustki
```csharp
...
## 4. LINQ`.ToList()`Niepotrzebna alokacja
```csharp
...
## 5. Przechwytywanie zmiennej pętli (wersja poprzedzająca C# 5.0)
```csharp
...
## 6. Nieużywanie dopasowywania wzorców
```csharp
...
## 7. Anty-wzorzec: Klasy Boga / Lokalizator usług
```csharp
...
## 10. Nieużywanie rekordów dla typów danych
```csharp
...
## Streszczenie
C# jest dobrze zaprojektowanym językiem, ale ma pułapki: brak usuwania obiektów `IDisposable`, asynchroniczne metody void, niepotrzebne alokacje LINQ i wyjątki odwołań o wartości null. Nowoczesny język C# (8+) zapewnia zaawansowane narzędzia: typy referencyjne dopuszczające wartość null, dopasowywanie wzorców, rekordy, używanie deklaracji i wyrażenia przełączające. Kluczowe zasady: zawsze usuwaj zasoby, nigdy nie używaj async void (z wyjątkiem procedur obsługi zdarzeń), używaj wstrzykiwania konstruktora, preferuj rekordy dla danych i włączaj typy referencyjne dopuszczające wartość null, aby wyłapywać błędy zerowe w czasie kompilacji.