...
W tym przewodniku omówiono wzorce idiomatyczne i najlepsze praktyki dotyczące pisania czystego, nowoczesnego kodu w języku C# (12+).
---

## Nowoczesna składnia C#
```csharp
// ✅ Records for data
...
## Streszczenie
Nowoczesne idiomy C# kładą nacisk na: rekordy danych, dopasowywanie wzorców, bezpieczeństwo null (typy referencyjne dopuszczające wartość null), LINQ dla kolekcji, async/await z CancellationToken, wstrzykiwanie zależności, podstawowe konstruktory, wyrażenia kolekcji i deklaracje `using`. Postępuj zgodnie z konwencjami kodowania C#, użyj`dotnet format`do formatowania i analizatorów Roslyn do jakości kodu. Nowoczesny język C# (12+) jest zwięzły i wyrazisty — obejmuje rekordy, dopasowywanie wzorców i podstawowe konstruktory.