...
-->
# Haskell — typowe błędy i anty-wzorce
Ten dokument kataloguje najczęstsze błędy, pułapki i antywzorce w Haskell wraz z poprawkami.
---

## 1. Nieskończone listy i niespodzianki związane z lenistwem
```haskell
...
## 5. Nieprawidłowe użycie składni rekordu
```haskell
...
## Streszczenie
Czystość i lenistwo Haskella stwarzają wyjątkowe pułapki: wycieki miejsca z leniwej oceny, częściowe funkcje, które ulegają awarii w czasie wykonywania, zamieszanie między ciągiem/tekstem/ciągiem bajtów i nadużywanie operacji we/wy. Sposób Haskella polega na tym: użyj ścisłej oceny akumulatorów (`foldl'`, BangPatterns), unikaj funkcji częściowych (`head`,`read`,`!!`), użyj`Maybe`/`Either`do obsługi błędów, użyj`Text`dla tekstu i zachowaj IO minimalne. System typów kompilatora jest Twoim największym sprzymierzeńcem — pozwól, aby poprowadził Cię do kompletnych, czystych funkcji.