...
Ten dokument kataloguje najczęstsze błędy, pułapki i anty-wzorce w C++. Każdy wpis pokazuje nieprawidłowe podejście, wyjaśnia przyczynę niepowodzenia i podaje prawidłowe rozwiązanie. Nowoczesny C++ (C++ 11/14/17/20) zapewnia narzędzia pozwalające uniknąć większości klasycznych pułapek.
---

## 1. Zasada trzech/pięciu naruszeń
```cpp
...
## 2. Surowy`new`/`delete`Zamiast inteligentnych wskaźników
```cpp
...
## 6. Nieprawidłowe użycie `std::move`
```cpp
...
## 7. Anty-wzorzec: użycie`using namespace std;`w nagłówkach
```cpp
...
## 8. Niezdefiniowane zachowanie spowodowane przepełnieniem podpisu
```cpp
...
## 9. Niezrozumienie kategorii wartości
```cpp
...
## 10. Anty-wzorzec: ręczne zarządzanie zasobami (przed RAII)
```cpp
...
## 11. Rozdęcie szablonu i długi czas kompilacji
```cpp
...
## 12. Nieużywanie`[[nodiscard]]`i `[[maybe_unused]]`
```cpp
...
## Streszczenie
Złożoność C++ stwarza wiele okazji do błędów: Reguła pięciu, dzielenie obiektów, unieważnianie iteratorów, wiszące odniesienia i niezdefiniowane zachowanie spowodowane przepełnieniem podpisu. Nowoczesny C++ jest antidotum: używaj inteligentnych wskaźników zamiast surowych`new`/`delete`, RAII dla wszystkich zasobów,`std::vector`i`std::string`zamiast surowych tablic, przenoś semantykę dla wydajności i `[[nodiscard]]`, aby przechwytywać ignorowane zwracane wartości. Podstawowe wytyczne C++ istnieją nie bez powodu — postępuj zgodnie z nimi i korzystaj z narzędzi takich jak Clang-Tidy i narzędzia dezynfekujące, aby uchwycić to, co przegapiłeś.