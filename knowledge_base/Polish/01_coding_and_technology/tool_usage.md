...
3. Popchnij gałąź:`git push origin feature/new-thing`.
...
- Multi-kursor:`Alt+Click`do umieszczania dodatkowych kursorów.
- Przejdź do definicji:`F12`.
- Zmień nazwę symbolu:`F2`.
- Format dokumentu:`Shift+Alt+F`.
- Rozszerzenia: zainstaluj obsługę języków (Python, Rust, Go itp.), linters i formatery z panelu Rozszerzenia (`Ctrl+Shift+X`).
...
-`Ctrl+Alt+L`(macOS:`Cmd+Option+L`) formatuje kod.
...
### Podstawowe pojęcia
- **Obraz**: szablon tylko do odczytu zbudowany na podstawie pliku`Dockerfile`.
