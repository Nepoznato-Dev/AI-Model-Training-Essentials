...
-->
# Skrócona instrukcja obsługi SQL
Niezbędne polecenia SQL do operacji na bazach danych.
---
...
### Dołącz do lewej/prawej strony```sql
...
### Pełne połączenie zewnętrzne```sql
...
-`DECIMAL(p,s)`- Dokładny dziesiętny (precyzja, skala)
-`FLOAT`- Przybliżona liczba zmiennoprzecinkowa
-`DOUBLE`- Pływak o podwójnej precyzji
### Ciąg
-`CHAR(n)`- Łańcuch o stałej długości
-`VARCHAR(n)`- Łańcuch o zmiennej długości
-`TEXT`- Duży tekst
-`ENUM`- Wartości wyliczone
### Data/godzina
-`DATE`- Data (RRRR-MM-DD)
-`TIME`- Czas (GG:MM:SS)
-`DATETIME`- Data i godzina
-`TIMESTAMP`- Uniksowy znacznik czasu
-`YEAR`- Wartość roku
### Wartość logiczna
-`BOOLEAN`lub`BOOL`- Prawda/Fałsz
### Binarny
-`BLOB`- Duży obiekt binarny
-`BINARY`- Naprawiono plik binarny
-`VARBINARY`- Zmienna binarna
---

## Przydatne funkcje
### Funkcje łańcuchowe```sql
...
✅ **Zrób:**
- Używaj indeksów w często odpytywanych kolumnach
- Wybierz tylko potrzebne kolumny (unikaj`SELECT *`)
- Użyj`EXPLAIN`do analizy wydajności zapytań
- Odpowiednio normalizuj dane
- Użyj przygotowanych instrukcji, aby zapobiec wstrzyknięciu SQL
❌ **Nie rób:**
- Używaj funkcji na indeksowanych kolumnach w klauzulach WHERE
- Utwórz zbyt wiele indeksów (spowalnia zapis)
- Używaj niepotrzebnie `SELECT DISTINCT`
- Ignoruj ​​plany wykonania zapytań
- Przechowuj obliczone wartości, jeśli można je obliczyć
---
...
*Ostatnia aktualizacja: lipiec 2026 r. | Standard SQL (kompatybilny z MySQL/PostgreSQL)*