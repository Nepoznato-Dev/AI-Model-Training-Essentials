...
## Streszczenie
Prognozowanie szeregów czasowych łączy klasyczną statystykę z nowoczesnym uczeniem maszynowym. Metody klasyczne (ARIMA, wygładzanie wykładnicze, Prorok) są łatwe do interpretacji, szybkie i często dokładne. Metody głębokiego uczenia się (LSTM, Transformers) wychwytują złożone wzorce, ale wymagają większej ilości danych i dostrojenia. Kluczowe zasady pozostają takie same niezależnie od metody: poznaj strukturę danych (trend, sezonowość, szum), porównaj z prostą wartością bazową, oceń za pomocą odpowiednich wskaźników i uwzględnij fakt, że przyszłość nie będzie dokładnie replikować przeszłości.