...
Systemy rekomendacji przewidują, co użytkownik będzie chciał w następnej kolejności zobaczyć, kupić lub z czym wejść w interakcję. Zasilają kanały treści w mediach społecznościowych, sugestie produktów w witrynach e-commerce, wybór filmów na platformach streamingowych i wyniki wyszukiwania. Mimo że są niewidoczne dla większości użytkowników, należą do systemów sztucznej inteligencji o największym komercyjnym wpływie na świecie — Netflix szacuje, że jego silnik rekomendacji pozwala zaoszczędzić ponad 1 miliard dolarów rocznie, zmniejszając odpływ abonentów.
---
...
### Wspólne filtrowanie
Pomysł: jeśli użytkownicy A i B zgodzili się w przeszłości, prawdopodobnie zgodzą się w przyszłości.
...
| Wyjaśnialne („zalecane, ponieważ jest podobne do X”) | Mniej nieoczekiwanych zdarzeń |
### Podejścia hybrydowe
...
| Modelka | Podejście |
|-------|--------------|
| **GRU4Rec** | Model oparty na GRU dla rekomendacji opartych na sesji |
...
| **Uwłaczające warstwom** | Dodaj komponent obniżający wartość do modelu |
...
|--------|--------|---------|
| **Netflix** | Rekomendacje filmów/programów telewizyjnych | Odzyskiwanie dwóch wież + głęboki ranking + bandyci kontekstowi dla grafiki |
