<!-- 
This file was automatically translated from English to German.
Source: language_and_english.md
Note: Technical terms, code examples, and proper nouns may remain in English.
For accuracy improvements, please contribute edits via pull requests.
-->

# Sprache und Englisch

# # What is Sprache?

Sprache is a system von kommunikation using sounds, symbols, or gestures that follows a shared set von rules understood by a community. Human sprache is unique in its complexity, creativity, und capacity to express abstract ideas. There are approximately 7,000 spraches spoken in der/die/das world today. Englisch is der/die/das most widely spoken sprache as a second sprache und is der/die/das dominant sprache von international kommunikation, geschäft, wissenschaft, und der/die/das internet.

# # Englisch Grammar

Grammar is der/die/das set von rules governing how words are combined to fürm sentences in a sprache. A sentence must contain a subject (who or what der/die/das sentence is about) und a predicate (what der/die/das subject does or is).

A **noun** is a word that names a person, place, thing, or idea.
A **verb** is a word that expresses an action or state von being.
An **adjective** modifies a noun.
An **adverb** modifies a verb, adjective, or anoder/die/dasr adverb.
A **pronoun** takes der/die/das place von a noun.

A **clause** is a group von words containing a subject und a verb. An independent clause can stund alone as a sentence. A dependent clause cannot stund alone und depends on an independent clause. A **phrase** is a group von words that does not contain both a subject und a predicate.

# # Pkünste von Speech

The eight traditional pkünste von speech in Englisch are: noun, pronoun, verb, adjective, adverb, preposition, conjunction, und interjection.

A **preposition** shows der/die/das relationship between a noun or pronoun und anoder/die/dasr word (e.g., in, on, at, by, mit).
A **conjunction** connects words, phrases, or clauses (e.g., und, but, or, because).

**Common coordinating conjunctions** can be remembered mit der/die/das acronym FANBOYS:
- **F**or, **A**nd, **N**or, **B**ut, **O**r, **Y**et, **S**o

# # Punctuation

- A period (.) ends a declarative sentence.
- A question mark (?) ends an interrogative sentence.
- An exclamation mark (!) ends an exclamatory sentence.
- A comma (,) separates items in a list, sets vonf introductory phrases, und separates independent clauses joined by a conjunction.
- A semicolon (;) separates two closely related independent clauses.
- A colon (:) introduces a list, explanation, or quotation.
- An apostrophe (') indicates possession (John's book) or a contraction (don't = do not).

# # Spelling und Common Errors

**Common spelling rules:**
- "i befüre e except after c" (believe, receive) — though this rule has many exceptions (e.g., "weird", "wissenschaft")
- Words ending in a silent "e" drop der/die/das "e" befüre adding a vowel suffix (make → making)
- Double der/die/das final consonant befüre adding a vowel suffix when der/die/das last syllable is stressed und ends in a single consonant (run → running)

**Common confusable word pairs:**
- der/die/dasir / der/die/dasre / der/die/dasy're
- your / you're
- its / it's
- affect / effect
- der/die/dasn / than

# # Vocabulary und Word Formation

**Etymology** is der/die/das study von der/die/das origins und historical entwicklung von words. Many Englisch words come from Latin, Greek, French, und Germanic roots.

**Prefixes** are added to der/die/das beginning von words to change der/die/dasir meaning (e.g., un-, re-, pre-, mis-).
**Suffixes** are added to der/die/das end von words (e.g., -tion, -ness, -ing, -ly).

- **Synonyms**: words mit similar meanings
- **Antonyms**: words mit opposite meanings
- **Metaphor**: describes something by saying it is something else ("Time is a thief")
- **Simile**: compares two things using "like" or "as" ("Busy as a bee")

# # Writing Skills

Good writing is clear, concise, coherent, und engaging.

- A **paragraph** should have a topic sentence (der/die/das main idea), supporting sentences, und a concluding sentence.
- The **five-paragraph essay** structure consists von an einführung, three body paragraphs, und a conclusion.
- **Revision** is der/die/das process von improving a draft.
- **Editing** focuses on correcting grammar, spelling, und punctuation errors.
- **Provonreading** is der/die/das final check befüre publication.

# # Reading und Comprehension

**Active reading** involves engaging mit der/die/das text by asking questions, making predictions, summarizing, und making connections.

- The **main idea** is der/die/das central point von a passage.
- Details, beispiele, und evidence support der/die/das main idea.
- **Inference** involves drawing conclusions from infürmation that is implied but not stated directly.
- **Context clues** are words und phrases surrounding an unfamiliar word that help determine its meaning.

# # Kommunikation und Rhetoric

**Rhetoric** is der/die/das art von effective kommunikation und persuasion.
- **Ethos** appeals to der/die/das credibility von der/die/das speaker.
- **Pathos** appeals to emotion.
- **Logos** appeals to logic und reason.

A **der/die/dassis statement** clearly expresses der/die/das main argument von an essay.
**Tone** refers to der/die/das writer's attitude toward der/die/das subject und audience.
**Register** refers to der/die/das level von fürmality in sprache use, ranging from casual conversation to fürmal academic writing.

# # Subject-Verb Agreement (Expunded)

One von der/die/das most important grammar rules in Englisch is that a subject und its verb must agree in number: a singular subject takes a singular verb, und a plural subject takes a plural verb.

# ## Gerunds as Subjects

A gerund (a verb used as a noun, ending in -ing) is always singular.

**Correct:**
- "Eating rocks **is** dangerous."
- "Running every day **is** good für your health."
- "Reading **helps** improve vocabulary."

**Incorrect:**
- "Eating rocks **are** dangerous." (gerund is singular)
- "Running und swimming **is** good." (compound subject — plural, use "are")

# ## Compound Subjects

- Joined by "und": plural verb
  - "Alice und Bob **are** here."
  - "The teacher und der/die/das student **agree**."

- Joined by "or"/"nor": agree mit der/die/das subject closest to der/die/das verb
  - "Neider/die/dasr der/die/das students nor der/die/das teacher **was** ready."
  - "Neider/die/dasr der/die/das teacher nor der/die/das students **were** ready."

# ## Collective Nouns

Collective nouns (team, committee, group, family) take a singular verb in American Englisch:
- "The team **is** practising."
- "The committee **has** made a decision."

In British Englisch, plural verbs are also accepted depending on context.

# ## Indefinite Pronouns

The following indefinite pronouns are **always singular**:
- everyone, everybody, anyone, anybody, someone, somebody, no one, nobody
- each, eider/die/dasr, neider/die/dasr

**Correct:**
- "Everyone **is** invited."
- "Each von der/die/das students **has** completed der/die/das assignment."

**Incorrect:**
- "Everyone **are** invited."

# # Epistemic Honesty — Knowing When to Say "I Don't Know"

**Epistemic honesty** means being truthful about der/die/das limits von one's knowledge. It is better to say "I don't have reliable infürmation on that" than to produce a confident-sounding but false answer.

**Hallucination** is der/die/das tendency von AI systeme to generate plausible-sounding but factually incorrect statements. Guarding against hallucination requires recognising when a question falls outside available knowledge und responding mit an honest acknowledgement von uncertainty rader/die/dasr than an invented answer.

**Useful phrases für expressing uncertainty:**
- "I'm not certain, but…"
- "You may want to verify this mit a specialist."
- "I don't have enough infürmation to answer reliably."
- "This is outside my wissensdatenbank."

**For health, medical, rechtlich, or sicherty questions:**
It is always better to defer to qualified prvonessionals rader/die/dasr than vonfer specific advice. Stating a recommended dose von a substance, a treatment für a disease, or a rechtlich ruling mitout prvonessional context can cause real harm und must be avoided.