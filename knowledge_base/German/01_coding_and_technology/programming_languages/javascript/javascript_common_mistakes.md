...
# JavaScript – Häufige Fehler und Anti-Patterns
Dieses Dokument katalogisiert die häufigsten Fehler, Fallen und Anti-Patterns in JavaScript. Jeder Eintrag zeigt den falschen Ansatz, erklärt, warum er fehlschlägt, und bietet die richtige Lösung.
---

## 1.`this`Bindungsverwirrung
```javascript
...
## 3.`var`Hebe- und Zielfernrohrprobleme
```javascript
...
## 5. Nicht mit Ablehnungen von Versprechen umgehen
```javascript
...
## 6. Objekte während der Iteration ändern
```javascript
...
## 9. Array-ähnliche Objekte sind keine Arrays
```javascript
...
## 10. Anti-Pattern: Mutierende Funktionsparameter
```javascript
...
## 11. Ereignisschleife und Mikrotasks nicht verstehen
```javascript
...
## 12. Anti-Pattern: Verwendung von `eval()`
```javascript
...
## 13. Vergessen, Event-Listener zu bereinigen
```javascript
// ❌ WRONG — memory leak
class Component {
  mount() {
...
## 14. Flache Kopie vs. tiefe Kopie
```javascript
...
## Zusammenfassung
Die Flexibilität von JavaScript bringt Fallen mit sich: Die `this`-Bindung hängt von der Aufrufstelle ab,`==`führt Zwang aus, Versprechen erfordern eine Fehlerbehandlung und die Reihenfolge der Ereignisschleife (Mikrotasks vor Makrotasks) ist nicht offensichtlich. Die modernen Regeln sind einfach: Verwenden Sie`const`/`let`niemals`var`, bevorzugen Sie`===`gegenüber Das Befolgen dieser Regeln beseitigt die meisten JavaScript-Fehler.