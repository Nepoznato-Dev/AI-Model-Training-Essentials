...
---
# JavaScript – Syntaxreferenz
Dieses Dokument bietet eine umfassende, strukturierte Syntaxreferenz für JavaScript. Es ergänzt die Hauptreferenz zu JavaScript, indem es sich auf umfassende Syntaxmuster, Operatortabellen und interne Mechanismen konzentriert.
---

## Operatoren und Ausdrücke
### Arithmetische Operatoren
| Betreiber | Name | Beispiel | Ergebnis | Notizen |
|----------|------|---------|--------|-------|
| `+`| Zusatz | `3 + 2`| `5`| Auch String-Verkettung |
| `-`| Subtraktion | `3 - 2`| `1`| |
| `*`| Multiplikation | `3 * 2`| `6`| |
| `/`| Abteilung | `7 / 2`| `3.5`| |
| `%`| Rest | `7 % 2`| `1`| |
| `**`| Potenzierung | `2 ** 10`| `1024`| ES2016 |
| `++`| Inkrement | `x++`/`++x`| | Post- und Pre-Inkrement |
| `--`| Dekrementieren | `x--`/`--x`| | Post- und Pre-Dekrement |
| `+x`,`-x`| Unär | `+"42"`→`42`| Geben Sie Zwang zur Zahl | ein
### Vergleich & Gleichheit
| Betreiber | Name | Beispiel | Notizen |
|----------|------|---------|-------|
| `===`| Strenge Gleichheit | `1 === 1`| Kein Typzwang – bevorzugen Sie immer diesen |
| `!==`| Strenge Ungleichheit | `1 !== "1"`| `true`– verschiedene Typen |
| `==`| Abstrakte Gleichheit | `1 == "1"`| Geben Sie Zwang ein – vermeiden Sie |
| `!=`| Abstrakte Ungleichheit | `1 != "1"`| `false`– vermeiden |
|  `<`, `>`, `<=`,`>=`| Relational | `"b" > "a"`| Lexikografisch für Strings |
### Logische und Null-Operatoren
| Betreiber | Name | Beispiel | Notizen |
|----------|------|---------|-------|
| `&&`| Logisches UND | `a && b`| Gibt den ersten falschen oder letzten Wert | zurück
| `\|\|`| Logisches ODER | `a \|\| b`| Gibt den ersten wahren oder letzten Wert | zurück
| `!`| Logisches NICHT | `!true`| `false`|
| `??`| Nullish Coalescing | `a ?? b`| Gibt`b`nur zurück, wenn`a``null` /`undefined`| ist
| `?.`| Optionale Verkettung | `obj?.prop?.method()`| Kurzschlüsse nach`undefined`|
### Bitweise Operatoren
| Betreiber | Name | Beispiel | Notizen |
|----------|------|---------|-------|
| `&`| UND | `5 & 3`| `1`|
...
| `>>`| Signiert Rechtsverschiebung | `-5 >> 1`| `-3`|
| `>>>`| Rechtsverschiebung ohne Vorzeichen | `-1 >>> 0`| `4294967295`|
### Zuweisung & Destrukturierung
```javascript
// Basic assignment
let x = 10;
...
|------------|-----------|---------------|
| 1 (höchste) | `()``[]``.``?.` | Links |
| 2 | `new`(ohne Argumente) | Richtig |
| 3 | `++``--` (Postfix) | N/A |
| 4 | `!``~``+x``-x``typeof``void``delete``++` Richtig |
| 5 | `**`| Richtig |
| 6 | `*``/``%`| Links |
| 7 | `+``-` | Links |
| 8 | `<<``>>``>>>`| Links |
| 9 | `<``<=``>``>=``in``instanceof` | Links |
| 10 | `===``!==``==``!=` | Links |
| 11 | `&`| Links |
| 12 | `^`| Links |
| 13 | `\|`| Links |
...
| 16 | `??`| Links |
| 17 | `=``+=``-=`usw. | Richtig |
| 18 (niedrigste) | `=>`(Pfeilfunktion) | Richtig |
---
...
## Datenstrukturen
### Übersicht über integrierte Sammlungen
| Sammlung | Bestellt | Veränderlich | Duplikate | Schlüsseltyp | Suche |
|------------|---------|---------|------------|----------|--------|
| `Array`| Ja | Ja | Ja | Index (Nummer) | O(n) |
| `Object`| Ja (Einfügung) | Ja | Schlüssel: Nein | Zeichenfolge/Symbol | O(1) Durchschnitt |
| `Map`| Ja (Einfügung) | Ja | Schlüssel: Nein | Irgendein | O(1) Durchschnitt |
| `Set`| Ja (Einfügung) | Ja | Nein | — | O(1) Durchschnitt |
| `WeakMap`| Nein | Ja | Schlüssel: Nein | Nur Objekt | O(1) Durchschnitt |
| `WeakSet`| Nein | Ja | Nein | Nur Objekt | O(1) Durchschnitt |
### Arrays
```javascript
// Creation
...
## Zusammenfassung
Die Syntax von JavaScript ist oberflächlich betrachtet täuschend einfach, weist jedoch eine erhebliche Tiefe auf. Die Sprache hat sich von einem einfachen Skripttool zu einer Multiparadigmensprache mit Klassen, Modulen, Generatoren, Proxys und einem ausgefeilten asynchronen Modell entwickelt. Das Verständnis der Operatorpriorität, der Ereignisschleife, der Prototypketten und der Nuancen der `this`-Bindung unterscheidet kompetente Entwickler von Experten. Modernes JavaScript (ES2020+) mit optionaler Verkettung, Nullish-Koaleszierung,`Promise.allSettled`und Top-Level-Await reduziert weiterhin die Boilerplate und sorgt gleichzeitig für Abwärtskompatibilität.