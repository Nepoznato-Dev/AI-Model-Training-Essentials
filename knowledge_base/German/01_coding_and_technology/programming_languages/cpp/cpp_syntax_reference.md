...
|----------|------|---------|-------|
| `+``-``*``/``%`| Arithmetik | `a + b`| Gleich wie C |
| `++``--` | Inkrementieren/Dekrementieren | `++i`,`i++`| Bevorzugen Sie die Vorinkrementierung |
| `==``!=``<``>``<=``>=` | Vergleich | `a <=> b`| `<=>`ist ein Drei-Wege-Vergleich (C++20) |
| `&&``\|\|``!`| Logisch | `a && b`| Kurzschluss |
| `&``\|``^``~``<<``>>` | Bitweise | `a & b`| Gleich wie C |
| `?:`| Ternär | `cond ? a : b`| |
### C++-spezifische Operatoren
| Betreiber | Name | Beispiel | Notizen |
|----------|------|---------|-------|
| `::`| Scope-Auflösung | `std::cout`| |
| `.*``->*` | Zeiger auf Mitglied | `obj.*pmf`| |
| `<=>`| Raumschiff (C++20) | `a <=> b`| Gibt`strong_ordering`| zurück
| `co_await`| Coroutine suspendieren | `co_await expr`| C++20 |
| `co_yield`| Coroutine-Ausbeute | `co_yield value`| C++20 |
| `co_return`| Coroutine-Rückkehr | `co_return value`| C++20 |
...
## Kontrollfluss
### Strukturierte Bindungen und moderne Muster
```cpp
...
## Intelligente Zeiger und Speicher
```cpp
...
### Containerübersicht
| Behälter | Kopfzeile | Bestellt | Suche | Notizen |
|-----------|--------|---------|--------|-------|
| `vector`| `<vector>`| Ja | O(n) | Standardauswahl für Sequenzen |
| `deque`| `<deque>`| Ja | O(n) | Schnelles Drücken/Knall an beiden Enden |
| `list`| `<list>`| Ja | O(n) | Doppelt verknüpfte Liste |
| `array`| `<array>`| Ja | O(n) | Feste Größe, Stapelzuweisung |
| `set`/`multiset`| `<set>`| Ja | O(log n) | Rot-schwarzer Baum |
| `map`/`multimap`| `<map>`| Ja (per Schlüssel) | O(log n) | Schlüsselwert, rot-schwarzer Baum |
| `unordered_set`| `<unordered_set>`| Nein | O(1) Durchschnitt | Hash-Tabelle |
| `unordered_map`| `<unordered_map>`| Nein | O(1) Durchschnitt | Hash-Tabelle |
...
## Zusammenfassung
Die C++-Syntax reicht von der Zeigermanipulation auf niedriger Ebene bis zur Metaprogrammierung von Vorlagen auf hoher Ebene und auf Coroutine-basierter Asynchronisierung. Die Weiterentwicklung der Sprache durch C++ 17/20/23 hat die Sicherheit (intelligente Zeiger, `std::optional`, Konzepte), die Ausdruckskraft (strukturierte Bindungen, Bereiche, Die Kernphilosophie bleibt unverändert: Sie sollten nicht für das bezahlen, was Sie nicht verwenden, und die Sprache sollte jede Abstraktionsebene von Bare-Metal bis hin zu domänenspezifischen Sprachen unterstützen.