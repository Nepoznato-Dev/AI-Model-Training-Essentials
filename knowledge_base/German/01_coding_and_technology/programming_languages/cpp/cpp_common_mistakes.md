...
# C++ – Häufige Fehler und Anti-Patterns
Dieses Dokument katalogisiert die häufigsten Fehler, Fallen und Anti-Patterns in C++. Jeder Eintrag zeigt den falschen Ansatz, erklärt, warum er fehlschlägt, und bietet die richtige Lösung. Modernes C++ (C++11/14/17/20) bietet Tools zur Vermeidung der meisten klassischen Fallstricke.
---

## 1. Regel von drei/fünf Verstößen
```cpp
...
## 2. Raw`new`/`delete`anstelle von Smart Pointern
```cpp
...
## 6.`std::move`nicht richtig verwenden
```cpp
...
## 7. Anti-Pattern: Verwendung von`using namespace std;`in Headern
```cpp
...
## 8. Undefiniertes Verhalten durch signierten Überlauf
```cpp
...
## 9. Wertkategorien nicht verstehen
```cpp
...
## 10. Anti-Pattern: Manuelles Ressourcenmanagement (Pre-RAII)
```cpp
...
## 11. Aufgeblähte Vorlagen und lange Kompilierungszeiten
```cpp
...
## 12.`[[nodiscard]]`und`[[maybe_unused]]`werden nicht verwendet
```cpp
...
## Zusammenfassung
Die Komplexität von C++ bietet viele Möglichkeiten für Fehler: die Fünferregel, Objekt-Slicing, Iterator-Invalidierung, fehlende Referenzen und undefiniertes Verhalten durch signierten Überlauf. Modernes C++ ist das Gegenmittel: Verwenden Sie intelligente Zeiger anstelle von rohen`new`/ `delete`, RAII für alle Ressourcen,`std::vector`und Die C++-Kernrichtlinien existieren aus einem bestimmten Grund: Befolgen Sie sie und verwenden Sie Tools wie Clang-Tidy und Sanitizer, um zu erkennen, was Sie verpassen.