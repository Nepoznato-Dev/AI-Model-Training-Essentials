...
Dieses Dokument katalogisiert die häufigsten Fehler, Fallen und Anti-Patterns in C#. Jeder Eintrag zeigt den falschen Ansatz, erklärt, warum er fehlschlägt, und bietet die richtige Lösung.
---

## 1. `IDisposable`-Objekte nicht entsorgen
```csharp
// ❌ WRONG — resource leak
...
## 4. LINQ`.ToList()`Unnötige Zuweisung
```csharp
...
## 5. Schleifenvariable erfassen (vor C# 5.0)
```csharp
...
## 6. Kein Mustervergleich verwenden
```csharp
...
## 7. Anti-Pattern: Gottklassen / Service-Locator
```csharp
...
## 10. Keine Datensätze für Datentypen verwenden
```csharp
...
## Zusammenfassung
C# ist eine gut gestaltete Sprache, weist jedoch Fallstricke auf: keine Entsorgung von `IDisposable`-Objekten, asynchrone Void-Methoden, unnötige LINQ-Zuweisungen und Null-Referenz-Ausnahmen. Modernes C# (8+) bietet leistungsstarke Tools: Nullable-Referenztypen, Mustervergleich, Datensätze, Verwendungsdeklarationen und Switch-Ausdrücke. Die wichtigsten Prinzipien: Ressourcen immer entsorgen, niemals asynchrones Void verwenden (außer Event-Handlern), Konstruktorinjektion verwenden, Datensätze für Daten bevorzugen und nullbare Referenztypen aktivieren, um Nullfehler zur Kompilierungszeit abzufangen.