...
-->
# Rust – Syntaxreferenz
Dieses Dokument bietet eine umfassende, strukturierte Syntaxreferenz für Rust. Es ergänzt die Hauptreferenz von Rust, indem es sich auf umfassende Syntaxmuster, Operatortabellen und interne Mechanismen des Eigentümersystems, Merkmale und Parallelität konzentriert.
---
...
|----------|------|---------|-------|
| `+`| Zusatz | `3 + 2`| Auch `String`-Verkettung über`format!`|
| `-`| Subtraktion | `3 - 2`| |
| `*`| Multiplikation | `3 * 2`| |
| `/`| Abteilung | `7 / 2`| Ganzzahldivision für ganze Zahlen:`3`|
| `%`| Rest | `7 % 2`| `1`|
| `-x`| Negation | `-5`| |
### Vergleich und logische Operatoren
| Betreiber | Name | Beispiel | Notizen |
|----------|------|---------|-------|
| `==`| Gleich | `x == y`| Erfordert`PartialEq`|
| `!=`| Nicht gleich | `x != y`| |
|  `<`, `>`, `<=`,`>=`| Bestellen | `x >= y`| Erfordert`PartialOrd`|
| `&&`| Logisches UND | `a && b`| Kurzschluss |
| `\|\|`| Logisches ODER | `a \|\| b`| Kurzschluss |
...
| `\|`| ODER | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `!`| NICHT | `!0b1010u8`| Bitweises Komplement |
| `<<`| Linksverschiebung | `5 << 1`| `10`|
| `>>`| Rechtsverschiebung | `5 >> 1`| `2`|
### Operatorpriorität (vom höchsten zum niedrigsten)
| Vorrang | Operatoren | Notizen |
|------------|-----------|-------|
| 1 (höchste) | `()`Pfade`.`:: | |
| 2 | Methodenaufrufe, Feldzugriff | Auto-Deref |
| 3 | `-`(Negation), `!`,`*`(Deref) | |
| 4 | `as`(Typumsetzung) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
| 14 | `..``..=` | Reichweite |
| 15 | `=``+=` usw. | Aufgabe |
| 16 (niedrigste) | `return``break``continue`| |
---
...
## Datenstrukturen
### Übersicht über integrierte Typen
| Geben Sie | ein Stapel/Haufen | Größe | Veränderlichkeit | Notizen |
|------|-----------|------|------------|-------|
| Primitive (`i32`,`f64`,`bool`,`char`) | Stapel | Behoben | Standardmäßig unveränderlich | `Copy`|
| `[T; N]`(Array) | Stapel | Behoben | Veränderbar mit`mut`| Feste Größe, `Copy`, wenn T gleich`Copy`| ist
| `Vec<T>`| Haufen | Dynamisch | Veränderbar mit`mut`| Wachsbar |
| `String`| Haufen | Dynamisch | Veränderbar mit`mut`| UTF-8, erweiterbar |
| `&[T]`(Schnitt) | Stapel (fetter PTR) | Dynamische Ansicht | Hängt von`&`oder`&mut`| ab Ausgeliehene Ansicht |
| `HashMap<K, V>`| Haufen | Dynamisch | Veränderbar mit`mut`| O(1) durchschnittliche Suche |
| `BTreeMap<K, V>`| Haufen | Dynamisch | Veränderbar mit`mut`| O(log n)-Suche, geordnet |
| `HashSet<T>`| Haufen | Dynamisch | — | O(1) Durchschnitt, keine Duplikate |
### Vektoren
```rust
// Creation
...
### Definition und Implementierung von Merkmalen
```rust
// Define a trait
trait Summary {
...
### Gemeinsame Merkmale der Standardbibliothek
```rust
...
## Parallelität
### Threads und Nachrichtenübermittlung
```rust
use std::thread;
...
## Zusammenfassung
Die Syntax von Rust basiert auf Sicherheitsgarantien, die zur Kompilierungszeit durchgesetzt werden. Das Eigentumssystem, der merkmalsbasierte Polymorphismus, der Mustervergleich und die Nullkosten-Abstraktionen bilden ein kohärentes Design, bei dem der Compiler ein strenger, aber fairer Partner ist. Vom grundlegenden Kontrollfluss bis hin zu erweiterten Funktionen wie Makros, unsicherem Code und intelligenten Zeigern dient jedes Konstrukt dem Ziel einer sicheren und effizienten Systemprogrammierung. Das Verständnis des Zusammenspiels zwischen Lebensdauern, Ausleihen und dem Typensystem ist der Schlüssel zur Beherrschung von Rust.