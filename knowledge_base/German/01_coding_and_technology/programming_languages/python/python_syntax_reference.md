...
---
# Python – Syntaxreferenz
Dieses Dokument bietet eine umfassende, strukturierte Syntaxreferenz für Python. Es ergänzt die Hauptreferenz zu Python, indem es sich auf umfassende Syntaxmuster, Operatortabellen und interne Mechanismen konzentriert.
---
...
|----------|------|---------|--------|-------|
| `+`| Zusatz | `3 + 2`| `5`| Auch Zeichenfolgen-/Listenverkettung |
| `-`| Subtraktion | `3 - 2`| `1`| Auch unäre Negation |
| `*`| Multiplikation | `3 * 2`| `6`| Auch Zeichenfolgen-/Listenwiederholung |
| `/`| Wahre Teilung | `7 / 2`| `3.5`| Gibt immer float | zurück
| `//`| Bodenaufteilung | `7 // 2`| `3`| Rundet gegen negative Unendlichkeit |
| `%`| Modul | `7 % 2`| `1`| Vorzeichen entspricht Teiler |
| `**`| Potenzierung | `2 ** 10`| `1024`| Rechtsassoziativ |
### Vergleich und boolesche Operatoren
| Betreiber | Name | Beispiel | Notizen |
|----------|------|---------|-------|
| `==`| Gleich | `x == y`| Wertvergleich (keine Identität) |
| `!=`| Nicht gleich | `x != y`| |
|  `<`, `>`, `<=`,`>=`| Bestellen | `x >= y`| Verkettbar:`a < b < c`|
| `is`| Identität | `x is y`| Gleiches Objekt im Speicher |
| `is not`| Keine Identität | `x is not None`| Bevorzugt gegenüber`!= None`|
| `in`| Mitgliedschaft | `x in collection`| O(1) für Mengen/Dikte, O(n) für Listen |
| `and`| Logisches UND | `a and b`| Kurzschluss; gibt den letzten wahren/falschen Wert zurück |
| `or`| Logisches ODER | `a or b`| Kurzschluss; gibt den ersten Wahrheits-/Falschwert zurück |
| `not`| Logisches NICHT | `not x`| |
### Bitweise Operatoren
| Betreiber | Name | Beispiel | Notizen |
|----------|------|---------|-------|
| `&`| UND | `5 & 3`| `1`|
...
| 11 | `not`| Richtig |
| 12 | `and`| Links |
| 13 (niedrigste) | `or`| Links |
### Zuweisung und erweiterte Zuweisung
```python
...
| Sammlung | Bestellt | Veränderlich | Duplikate | Syntax | Suche |
|------------|---------|---------|------------|--------|--------|
| `list`| Ja | Ja | Ja | `[1, 2, 3]`| O(n) |
| `tuple`| Ja | Nein | Ja | `(1, 2, 3)`| O(n) |
| `dict`| Ja (3,7+) | Ja | Schlüssel: Nein | `{"k": "v"}`| O(1) Durchschnitt |
| `set`| Nein | Ja | Nein | `{1, 2, 3}`| O(1) Durchschnitt |
| `frozenset`| Nein | Nein | Nein | `frozenset({1,2})`| O(1) Durchschnitt |
### Listen
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
### Deskriptoren und Eigenschaften
```python
class Validated:
...
### Geben Sie System und Protokolle ein
```python
from typing import (
...
## Zusammenfassung
Diese Syntaxreferenz deckt den gesamten Funktionsumfang von Python ab – von grundlegenden Operatoren und Kontrollflüssen bis hin zu erweiterter Metaprogrammierung, Deskriptoren und dem Typsystem. Die Syntax von Python ist bewusst minimal und lesbar, aber ihre Tiefe ist beträchtlich: Mustervergleich, asynchrone Generatoren, Protokolle und Metaklassen bieten leistungsstarke Werkzeuge für komplexe Anwendungen. Die Sprache entwickelt sich ständig weiter und jede Version fügt Funktionen hinzu, die die Ausdruckskraft verbessern und gleichzeitig die Abwärtskompatibilität gewährleisten.