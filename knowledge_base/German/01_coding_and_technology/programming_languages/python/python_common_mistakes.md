...
-->
# Python – Häufige Fehler und Anti-Patterns
Dieses Dokument katalogisiert die häufigsten Fehler, Fallen und Anti-Patterns in Python. Jeder Eintrag zeigt den falschen Ansatz, erklärt, warum er fehlschlägt, und bietet die richtige Lösung. Wenn Sie diese Fallstricke verstehen, können Sie robusteren Pythonic-Code schreiben.
---
...
**Warum es passiert:** Standardargumente werden einmal zum Zeitpunkt der Funktionsdefinition ausgewertet, nicht bei jedem Aufruf. Ein veränderlicher Standardwert (Liste, Diktat, Satz) wird für alle Aufrufe gemeinsam genutzt.
---

## 2. Ändern einer Liste während der Iteration
```python
...
## 3. Späte Bindung in Abschlüssen
```python
...
## 4. Verwendung von`type()`anstelle von `isinstance()`
```python
...
## 5. Keine Kontextmanager für Ressourcen verwenden
```python
...
## 6. Der `is`-Operator für den Wertevergleich
```python
...
## 8. Pythons Scope Rules (LEGB) nicht verstehen
```python
...
## 10. Anti-Pattern: Verwendung von`except:`Bare Except
```python
...
## 11. Veränderbare Referenzen in den internen Zustand zurückführen
```python
...
## 12.`setdefault`oder`collections.defaultdict`nicht verwenden
```python
...
## 13. Verwechslung von`==`und`=`in den Bedingungen
```python
...
## 14. Die Erschöpfung des Generators nicht verstehen
```python
...
## 15. Anti-Pattern: Gottobjekte / Überladene Klassen
```python
...
## Zusammenfassung
Die Einfachheit von Python täuscht – veränderbare Standardeinstellungen, späte Bindungsschließungen, Bereichsregeln und Ressourcenverwaltung weisen alle subtile Verhaltensweisen auf, die Entwickler in die Falle locken. Die wichtigsten Prinzipien, die Sie sich merken sollten: Standardwerte werden einmal ausgewertet,`is`prüft Identität und nicht Gleichheit, verwenden Sie immer Kontextmanager für Ressourcen, fangen Sie bestimmte Ausnahmen ab und bevorzugen Sie die Komposition gegenüber Gottobjekten. Das Schreiben von Pythonic-Code bedeutet, diese Fallstricke zu verstehen und die Funktionen der Sprache (Kontextmanager, Verständnis, Generatoren, `collections`-Modul) wie vorgesehen zu nutzen.