...
# Sprachübergreifender Vergleich – Kontrollfluss
## If/Else-Anweisungen
```python
# Python
if x > 0:
...
| Rubin | `10.times { \|i\| ... }`oder`(0..9).each { \|i\| ... }`|
| Schnell | `for i in 0..<10`|
| Kotlin | `for (i in 0 until 10)`|
| Haskell | `mapM_ f [0..9]`|
| Perl | `for my $i (0..9)`|
| R | `for (i in 1:10)`|
| PHP | `for ($i = 0; $i < 10; $i++)`|
| SQL | N/A (satzbasiert, keine Schleifen in Standard-SQL) |
| Bash | `for i in {0..9}; do ... done`|
| Lua | `for i = 0, 9 do ... end`|
| MATLAB | `for i = 1:10 ... end`|
| Prolog | N/A (Stattdessen Rekursion) |
| Lispeln | `(dotimes (i 10) ...)`|
| Ada | `for I in 0..9 loop ... end loop;`|
| COBOL | `PERFORM VARYING I FROM 0 BY 1 UNTIL I >= 10`|
| Erlang | N/A (Stattdessen Rekursion + Listen) |
| OCaml | `for i = 0 to 9 do ... done`|
| Montage | Manuelle Schleife mit CMP/JMP |
| Delphi | `for I := 0 to 9 do ... end;`|
| Kratzer | `repeat (10) ... end`|
...
| Schnell | `switch x { case 1: ... default: ... }`|
| Kotlin | `when (x) { 1 -> ... else -> ... }`|
| OCaml | `match x with 1 -> ... \| _ -> ...`|
| Erlang | `case X of 1 -> ...; _ -> ... end`|
| Elixier | `case x do 1 -> ...; _ -> ... end`|
| Dart | `switch (x) { 1 => ...; _ => ... }`(Dart 3.0) |
| PHP | `match($x) { 1 => ..., default => ... }`(PHP 8.0) |
| C# | `x switch { 1 => ..., _ => ... }`(C# 8+) |
| TypeScript | Kein nativer Mustervergleich (if/else verwenden) |
| Python | `match x: case 1: ... case _: ...`(Python 3.10+) |
| Prolog | Eingebaut in Klauselübereinstimmung |
| Geh | `switch x { case 1: ... default: ... }`|