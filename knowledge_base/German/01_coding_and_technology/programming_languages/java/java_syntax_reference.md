...
-->
# Java – Syntaxreferenz
Dieses Dokument bietet eine umfassende, strukturierte Syntaxreferenz für Java. Es ergänzt die wichtigste Java-Referenz, indem es sich auf umfassende Syntaxmuster, Operatortabellen und interne Mechanismen der JVM, OOP und Parallelität konzentriert.
---
...
| `/`| Abteilung | `7 / 2`| `3`| Ganzzahldivision für`int`; `3.5`für`double`|
| `%`| Rest | `7 % 2`| `1`| |
...
|----------|------|---------|-------|
| `==`| Gleich | `x == y`| Referenzgleichheit für Objekte |
| `!=`| Nicht gleich | `x != y`| |
|  `<`, `>`, `<=`,`>=`| Relational | `x >= y`| |
| `&&`| Logisches UND | `a && b`| Kurzschluss |
| `\|\|`| Logisches ODER | `a \|\| b`| Kurzschluss |
| `!`| Logisches NICHT | `!true`| `false`|
| `instanceof`| Typprüfung | `obj instanceof String`| Auch Mustervergleich (Java 16+) |
### Bitweise Operatoren
| Betreiber | Name | Beispiel | Notizen |
|----------|------|---------|-------|
| `&`| UND | `5 & 3`| `1`|
...
| Vorrang | Operatoren |
|------------|-----------|
| 1 (höchste) | Postfix:`expr++``expr--` |
| 2 | Unär:`++expr``--expr``+expr``-expr``~``!` |
| 3 | Besetzung:`(Type)expr`|
| 4 | Multiplikativ:`*``/``%`|
| 5 | Zusatz:`+``-` |
| 6 | Schicht:`<<``>>``>>>`|
...
### Lambdas und funktionale Schnittstellen
```java
// Lambda syntax
...
## Datenstrukturen
### Sammlungs-Framework
| Schnittstelle | Implementierungen | Bestellt | Thread-sichere Variante |
|-----------|----------------|---------|-------|
| `List`| `ArrayList`,`LinkedList`| Ja | `CopyOnWriteArrayList`|
| `Set`| `HashSet`,`TreeSet`,`LinkedHashSet`| Variiert | `CopyOnWriteArraySet`|
| `Map`| `HashMap`,`TreeMap`,`LinkedHashMap`| Variiert | `ConcurrentHashMap`|
| `Queue`| `ArrayDeque`,`PriorityQueue`| Variiert | `ConcurrentLinkedQueue`|
| `Deque`| `ArrayDeque`| Beide Enden | `ConcurrentLinkedDeque`|
### Listen
```java
// Creation
...
## Zusammenfassung
Die Syntax von Java hat sich seit ihren Anfängen im Jahr 1995 dramatisch weiterentwickelt. Modernes Java (21+) bietet Datensätze, versiegelte Klassen, Mustervergleich, virtuelle Threads und String-Vorlagen – und ist damit prägnanter und ausdrucksvoller als je zuvor. Die Kernsyntax bleibt bekannt (C-ähnlich, stark typisiert, klassenbasiert), aber die Ergänzungen von Lambdas, der Stream-API und`var`haben die Boilerplate erheblich reduziert. Die Stärke der Sprache liegt in ihrem riesigen Ökosystem, den Abwärtskompatibilitätsgarantien und den Leistungsoptimierungen der JVM durch JIT-Kompilierung.