...
# Java – Häufige Fehler und Anti-Patterns
Dieses Dokument katalogisiert die häufigsten Fehler, Fallen und Anti-Patterns in Java. Jeder Eintrag zeigt den falschen Ansatz, erklärt, warum er fehlschlägt, und bietet die richtige Lösung.
---

## 1.`equals()`vs.`==`für Objekte
```java
...
## 2. Ändern einer Sammlung während der Iteration
```java
...
## 3. Ressourcen nicht schließen (Try-with-Resources)
```java
...
## 4. Fallstricke beim Autoboxen von Ganzzahlen
```java
...
## 7. InterruptedException wird nicht behandelt
```java
...
## 8. Null anstelle leerer Sammlungen zurückgeben
```java
...
## 9. Veränderliche Objekte als HashMap-Schlüssel
```java
...
## 10. Anti-Pattern:`Exception`oder`Throwable`abfangen
```java
...
## 11.`hashCode`nicht überschreiben, wenn`equals`überschrieben wird
```java
...
## 12. Rennbedingungen mit doppelt überprüfter Verriegelung
```java
...
## Zusammenfassung
Die Ausführlichkeit von Java kann subtile Fehler verbergen:`==`vs. Der moderne Java-Ansatz lautet: Verwenden Sie`Objects.equals()`für nullsichere Vergleiche, verwenden Sie immer Try-with-Resources, geben Sie leere Sammlungen anstelle von Null zurück, fangen Sie bestimmte Ausnahmen ab und befolgen Sie das Prinzip der Einzelverantwortung. Java 8+-Funktionen (Streams, Optional, Methodenreferenzen) reduzieren den Boilerplate und machen den Code gleichzeitig sicherer und ausdrucksvoller.