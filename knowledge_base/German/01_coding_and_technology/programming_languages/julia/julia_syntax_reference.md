...
|----------|------|---------|-------|
| `+``-``*``/``^`| Arithmetik | `2^10`| |
| `÷`| Ganzzahldivision | `7 ÷ 3`| Identisch mit`div(7, 3)`|
| `%``\` | Modul/Rest | `7 % 3`| |
| `==``!=` | Gleichheit | `a == b`| Wertegleichheit |
| `===`| Identität | `a === b`| Gleiches Objekt |
| `<``>``<=``>=` | Vergleich | `a >= b`| Verkettbar:`1 < x < 10`|
| `&&``\|\|``!`| Logisch | `a && b`| Kurzschluss |
| `&``\|``⊻``~` | Bitweise | `a & b`| `⊻`ist XOR |
| `:`| Reichweite | `1:10`| `start:step:stop`|
| `..`| Intervall | `1..10`| Erfordert IntervalSets |
| `∈``∉``⊆`| Mitgliedschaft festlegen | `x ∈ S`| `in(x, S)`|
| `∘`| Zusammensetzung | `f ∘ g`| `(f ∘ g)(x) == f(g(x))`|
| `\|>`| Rohr | `x \|> f`| `f(x)`|
| `=>`| Paar | `a => b`| Für Diktate |
| `...`| Spritzer | `f(args...)`| Sammlung erweitern |
...
## Funktionen und Mehrfachversand
```julia
# Basic function
function add(x, y)
    x + y
...
## Zusammenfassung
Julias Syntax ist klar, mathematisch und ausdrucksstark. Durch den Mehrfachversand können sich Funktionen basierend auf allen Argumenttypen unterschiedlich verhalten, was eine elegante Codeorganisation ermöglicht. Makros bieten leistungsstarke Metaprogrammierung ohne die Komplexität von Präprozessormakros im C-Stil. Das Typsystem – mit parametrischen Typen, abstrakten Typen und Union-Typen – ermöglicht sowohl Flexibilität als auch Leistung. Julias 1-basierte Indizierung, Verständnis und Do-Block-Syntax machen es für das mathematische und wissenschaftliche Rechnen selbstverständlich. Die Sprache erreicht ihr „Walk like Python, run like C“-Versprechen durch JIT-Kompilierung mit Typinferenz.