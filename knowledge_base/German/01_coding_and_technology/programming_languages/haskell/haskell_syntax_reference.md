...
|----------|------|---------|-------|
| `+``-``*``/` | Arithmetik | `x + y`| Num-Typklasse |
| `div``mod` | Ganzzahldivision | `7 \`div\` 2`| Gibt 3 | zurück
| `^``**` | Macht | `2 ^ 10`| `^`für Integral; `**`für Floating |
| `==``/=` | Gleichheit | `x == y`| Eq-Typklasse |
| `<``>``<=``>=` | Vergleich | `x >= y`| Ord-Typklasse |
| `&&``\|\|``not`| Logisch | `a && b`| Kurzschluss |
| `.`| Funktionszusammensetzung | `(f . g) x`| `f (g x)`|
| `$`| Funktionsanwendung | `f $ x + 1`| `f (x + 1)`– niedrige Priorität |
| `<>`| Monoid anhängen | `"hello" <> " world"`| Halbgruppe |
| `<$>`| Funktor fmap | `(+1) <$> Just 5`| `fmap (+1) (Just 5)`|
| `<*>`| Applikativ bewerben | `pure (+) <*> Just 3`| |
| `>>=`| Monadenbindung | `Just 5 >>= \x -> Just (x+1)`| |
| `>>`| Monadenfolge | `putStrLn "hi" >> putStrLn "bye"`| |
| `=<<`| Bind (umgekehrt) | `f =<< mx`| Identisch mit`mx >>= f`|
| `++`| Listenverkettung | `[1,2] ++ [3,4]`| O(n) in der linken Liste |
| `:`| Nachteile | `1 : [2, 3]`| O(1) |
| `!!`| Index | `[1,2,3] !! 1`| O(n) – Teilfunktion |
| `\\`| Listenunterschied | `[1,2,3] \\ [2]`| `[1,3]`|
### Operator-Vorrang (ausgewählt)
| Vorrang | Operatoren |
|------------|-----------|
| 7 (höchste) | `*``/``div``mod` |
| 6 | `+``-` |
...
## Zusammenfassung
Die Syntax von Haskell ist minimalistisch und dennoch außerordentlich ausdrucksstark. Das Typsystem – mit algebraischen Datentypen, Typklassen, GADTs und Typfamilien – dient sowohl als Spezifikations- als auch als Verifizierungstool. Mustervergleich, Lazy Evaluation und Monaden bieten elegante Lösungen für komplexe Probleme. Die Reinheit von Haskell (keine Nebenwirkungen bei Typen) erleichtert das Nachdenken, Testen und Parallelisieren von Code. Obwohl die Lernkurve steil ist, durchdringt Haskells Einfluss moderne Sprachen – von Swifts Optionals über Kotlins versiegelte Klassen bis hin zu Rusts Typsystem. Wenn Sie Haskell beherrschen, werden Sie in jeder Sprache ein besserer Programmierer.