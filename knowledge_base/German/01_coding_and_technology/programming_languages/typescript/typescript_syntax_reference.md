...
-->
# TypeScript – Syntaxreferenz
Dieses Dokument bietet eine umfassende, strukturierte Syntaxreferenz für TypeScript. Es ergänzt die Hauptreferenz zu TypeScript, indem es sich auf das Typsystem, Generika, Dienstprogrammtypen und erweiterte Programmierung auf Typebene konzentriert.
---

## Grundlagen des Typsystems
### Primitive und integrierte Typen
```typescript
// Primitives
...
## Dienstprogrammtypen
### Integrierte Dienstprogrammtypen
```typescript
interface User {
  id: string;
  name: string;
...
## Dekorateure (TC39 Stufe 3 / TypeScript 5.0+)
```typescript
// Class decorator
...
### Konst. Behauptungen und Eingrenzung
```typescript
...
## Zusammenfassung
Das Typsystem von TypeScript ist sein bestimmendes Merkmal und es ist besonders tiefgreifend. Von einfachen Annotationen bis hin zu Generika, bedingten Typen, zugeordneten Typen, Vorlagenliteraltypen und`infer`ist das Typsystem im Wesentlichen eine funktionale Sprache, die zur Kompilierungszeit ausgeführt wird. Das Verständnis des Zusammenspiels zwischen`interface`und `type`, generischen und Utility-Typen sowie struktureller und nominaler Typisierung unterstützt das Schreiben von TypeScript, das sowohl sicher als auch ergonomisch ist. Die Sprache entwickelt sich ständig weiter, wobei jede Version mehr Ausdruckskraft hinzufügt und gleichzeitig die volle Abwärtskompatibilität mit JavaScript beibehält.