...
---
# Swift – Syntaxreferenz
Dieses Dokument bietet eine umfassende, strukturierte Syntaxreferenz für Swift (5.9+). Es ergänzt die Hauptreferenz von Swift, indem es sich auf umfassende Syntaxmuster, Optionen, protokollorientierte Programmierung und moderne Parallelität konzentriert.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Arithmetik | `a + b`| |
| `..``...` | Reichweite | `1...10`| `...`geschlossen; `..<`halboffen |
| `==``!=``<``>``<=``>=` | Vergleich | `a == b`| Erfordert`Equatable`/`Comparable`|
| `===``!==` | Identität | `a === b`| Gleiche Referenz (nur Klassen) |
| `&&``\|\|``!`| Logisch | `a && b`| Kurzschluss |
| `??`| Nil verschmelzt | `a ?? b`| Gibt`b`zurück, wenn`a`Null ist |
| `?.`| Optionale Verkettung | `a?.b`| Gibt Null zurück, wenn`a`Null ist |
| `!`| Auspacken erzwingen | `a!`| Abstürze, wenn Null – vermeiden |
| `&`| Inout-Parameter | `swap(&a, &b)`| Referenz übergeben |
### Nullhandhabungsoperatoren
```swift
...
## Property Wrapper und Ergebnisgeneratoren
```swift
// Property wrapper
@propertyWrapper
struct Trimmed {
...
## Zusammenfassung
Die Syntax von Swift bringt Sicherheit und Ausdruckskraft in Einklang. Optionals verhindern Null-Abstürze, Protokolle ermöglichen eine flexible Komposition ohne Vererbung und moderne Parallelität (async/await, Actors) macht sichere gleichzeitige Programmierung zugänglich. Die Sprache entwickelt sich mit Funktionen wie Parameterpaketen, Makros und Eigentumsrechten weiter – jede Ergänzung macht Swift sicherer und leistungsfähiger und hält gleichzeitig sein Kernversprechen ein: Code, der durch seine Konstruktion sicher ist.