...
---
# C – Syntaxreferenz
Dieses Dokument bietet eine umfassende, strukturierte Syntaxreferenz für C (C11/C17 mit C23-Hinweisen). Es ergänzt die Haupt-C-Referenz, indem es sich auf umfassende Syntaxmuster, Operatortabellen, Zeigermechanik und Speicherverwaltung konzentriert.
---
...
| `/`| Abteilung | `7 / 2`| `3`| Ganzzahlkürzung für int-Typen |
| `%`| Modul | `7 % 2`| `1`| Nur für Ganzzahltypen |
| `++`| Inkrement | `i++`/`++i`| | Post- und Pre-Inkrement |
| `--`| Dekrementieren | `i--`/`--i`| | Post- und Pre-Dekrement |
...
| `\|`| ODER | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `~`| Komplement | `~0`| Alle Bits umgedreht |
| `<<`| Linksverschiebung | `1 << 3`| `8`|
| `>>`| Rechtsverschiebung | `8 >> 3`| `1`(implementierungsdefiniert für signiert) |
### Zeiger- und Adressoperatoren
| Betreiber | Name | Beispiel | Notizen |
|----------|------|---------|-------|
| `&`| Adresse von | `&x`| Gibt einen Zeiger auf x | zurück
| `*`| Dereferenzierung | `*ptr`| Zugriffswert am Zeiger |
| `->`| Mitgliedszugriff über Zeiger | `p->x`| Entspricht`(*p).x`|
| `.`| Mitgliederzugang | `s.field`| Direkter Zugriff auf Strukturmitglieder |
| `[]`| Array-Index | `arr[i]`| Entspricht`*(arr + i)`|
### Operatorpriorität (vom höchsten zum niedrigsten)
| Vorrang | Operatoren | Assoziativität |
|------------|-----------|---------------|
| 1 (höchste) | `()``[]``->``.` | Von links nach rechts |
| 2 | `!``~``+``-` (unär)`*``&``sizeof``(type)` | Von rechts nach links |
...
| 13 | `? :`(ternär) | Von rechts nach links |
| 14 | `=``+=``-=``*=``/=``%=` Von rechts nach links |
| 15 (niedrigste) | `,`(Komma) | Von links nach rechts |
---
...
## Zusammenfassung
Die Syntax von C ist minimal und nah an der Hardware – Zeiger, manuelle Speicherverwaltung und eine dünne Abstraktionsschicht über Maschinenanweisungen. Die Stärke der Sprache liegt in dieser Einfachheit: Es gibt fast keine versteckten Kosten, keinen Laufzeitaufwand und vollständige Kontrolle über das Speicherlayout. Der Präprozessor ist zwar leistungsstark, seine sichere Verwendung erfordert jedoch Disziplin. Modernes C (C11/C17/C23) hat wichtige Sicherheitsfunktionen wie `_Static_assert`, `_Generic`,`nullptr`und