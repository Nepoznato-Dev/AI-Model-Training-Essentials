...
Dieses Dokument katalogisiert die häufigsten Fehler, Fallen und Anti-Patterns in C. Jeder Eintrag zeigt den falschen Ansatz, erklärt, warum er fehlschlägt, und bietet die richtige Lösung. C gibt Ihnen die volle Kontrolle – was bedeutet, dass Sie die volle Verantwortung für die Speichersicherheit, die Überprüfung von Grenzen und undefiniertes Verhalten tragen.
---

## 1. Pufferüberlauf mit`gets()`/ `strcpy()`
```c
...
## 4. Nicht initialisierte Variablen
```c
...
## 5.`scanf()`Ohne Breitenbegrenzung
```c
...
## 8. Sicherheitslücken bei der Formatierung von Zeichenfolgen
```c
...
## 9. Verwechslung von`=`und`==`in den Bedingungen
```c
...
## 10. Zeigerarithmetik nicht verstehen
```c
...
## 11. Anti-Pattern: Magische Zahlen
```c
...
## Zusammenfassung
Die Gefahren von C sind bekannt, aber allgegenwärtig: Pufferüberläufe, baumelnde Zeiger, Ganzzahlüberlauf, Format-String-Angriffe und Speicherlecks. Die Disziplin lautet: Überprüfen Sie immer die Rückgabewerte, geben Sie immer frei, was Sie zuweisen, verwenden Sie immer begrenzte Funktionen (`snprintf`, C belohnt sorgfältige Programmierer mit unübertroffener Leistung und Kontrolle – bestraft jedoch Nachlässigkeit mit undefiniertem Verhalten und Sicherheitslücken.