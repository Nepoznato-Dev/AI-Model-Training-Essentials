...
## Warum Ada wichtig ist
- **Sicherheitskritische Systeme**: Von Grund auf für Systeme entwickelt, bei denen ein Ausfall keine Option ist.
- **Starke Typisierung**: Das strengste Typsystem aller Mainstream-Sprachen – fängt beim Kompilieren Fehler ab, die andere Sprachen übersehen.
- **Integrierte Parallelität**: Tasking (gleichzeitige Programmierung) ist Teil der Sprache, keine Bibliothek.
- **Formale Verifizierung**: Unterstützt formale Methoden zum Nachweis der Programmkorrektheit.
- **Zuverlässigkeitsfunktionen**: Integrierte Ausnahmen, Verträge (Vor-/Nachbedingungen) und Laufzeitprüfungen.
- **Modernes Ada**: Ada 2012 und Ada 2022 haben moderne Funktionen hinzugefügt und gleichzeitig Sicherheitsgarantien beibehalten.
## Die Kompromisse
| Einschränkung | Einzelheiten | Typische Problemumgehung |
|-----------|---------|-----|
| **Nischen-Community** | Kleine Entwicklerbasis im Vergleich zu C, Java oder Python | Spezialisierte, aber sachkundige Community |
| **Ausführliche Syntax** | Ausführlicher als C oder Python | Akzeptanz im Rahmen der sicherheitsgerichteten Gestaltung |
| **Begrenztes Ökosystem** | Weniger Bibliotheken als Mainstream-Sprachen | Schreiben Sie benutzerdefinierten Code. Nutzen Sie die umfangreiche Standardbibliothek |
| **Lernkurve** | Komplexe Sprache mit vielen Sicherheitsfunktionen | Beginnen Sie mit Adas „SPARK“-Teilmenge für sicherheitskritische Arbeiten |
| **Compiler-Verfügbarkeit** | Weniger Compiler-Optionen (GNAT ist die wichtigste) | GNAT ist kostenlos und gepflegt |
---
...
### Markierte Typen und Vererbung (OOP)
```ada
-- Abstract base type
package Shapes is
...
### Generics (Polymorphismus zur Kompilierungszeit)
```ada
...
### Geschützte Objekte (Thread-sichere freigegebene Daten)
```ada
...
## Interoperabilität
### C-Interoperabilität (Schnittstellen)
```ada
with Interfaces.C;
...
## Designmuster
### Muster 1: Beobachter mit geschützten Objekten
```ada
...
### Muster 2: Zustandsmaschine mit diskriminierten Typen
```ada
...
### Muster 3: RAII mit kontrollierten Typen
```ada
...
## Bereitstellung
### SPARK für sicherheitskritische Zertifizierung
```ada
...
## Wann man Ada verwenden sollte
| Szenario | Warum Ada | Bessere Alternative |
|----------|---------|-----|
| Sicherheitskritische Systeme | Dafür konzipiert; formelle Verifizierungsunterstützung | — |
| Luft-/Raumfahrt | Zertifizierte Compiler; DO-178C-Konformität | — |
| Verteidigungssysteme | DoD-Erbe; Sicherheitsmerkmale | — |
| Bahn-/Medizingeräte | Anforderungen an hohe Integrität | — |
| Allgemeine Anwendungsentwicklung | Overkill für unkritische Systeme | Python, Java, Go |
