...
|----------|------|---------|-------|
| `+``-``*``/``^`| Arithmetik | `A * B`| Matrix multiplizieren |
| `.*``./``.^`| Elementweise | `A .* B`| Array-Operation |
| `'`| Transponieren | `A'`| Konjugieren transponieren |
| `.'`| Nicht konjugierte Transponierung | `A.'`| |
| `\`| Linke Kluft | `A \ b`| Lösen Sie Ax = b |
| `/`| Rechts teilen | `b / A`| Lösen Sie xA = b |
| `==``~=` | Gleichheit | `A == B`| Elementweise |
| `<``>``<=``>=` | Vergleich | `A > 0`| Elementweise |
| `&``\|``~`| Logisch (elementweise) | `A & B`| |
| `&&``\|\|` | Kurzschluss | `a && b`| Nur Skalar |
| `:`| Doppelpunkt/Bereich | `1:10`| `start:step:stop`|
| `;`| Zeilentrenner / unterdrücken | `[1; 2; 3]`| |
| `,`| Spaltentrennzeichen | `[1, 2, 3]`| |
---
...
## Zusammenfassung
Die Syntax von MATLAB basiert auf der Matrix: Jede Variable ist ein Array und die Operationen sind für die lineare Algebra optimiert. Der Doppelpunktoperator, die logische Indizierung und vektorisierte Operationen eliminieren Schleifen für die meisten numerischen Aufgaben. Das Plotsystem erzeugt Zahlen in Publikationsqualität mit minimalem Code. Toolboxen erweitern MATLAB in alle technischen Bereiche. Während Python in einige Gebiete vorgedrungen ist, bleibt MATLAB der Standard für technische Berechnungen, Simulink-basierte Modellierung und Rapid Prototyping.