...
# SQL – Versionsverlauf und Entwicklung
## Zeitleiste
| Version | Jahr | Schlüsselthema |
|---------|------|-----------|
| Fortsetzung | 1974 | Ursprüngliche IBM-Forschungssprache (Chamberlin & Boyce) |
| SQL-86 | 1986 | **Erster ANSI-Standard** (SQL-86) |
| SQL-89 | 1989 | Kleinere Überarbeitung (Integritätsbeschränkungen) |
| SQL-92 | 1992 | **Major**:`JOIN`, Unterabfragen,`CASE`,`COALESCE`|
| SQL:1999 | 1999 | **Reguläre Ausdrücke**, rekursive Abfragen, Trigger, BLOBs |
| SQL:2003 | 2003 | **Fensterfunktionen**, XML,`GENERATED`Spalten |
| SQL:2006 | 2006 | XML-Unterstützung,`MERGE`|
| SQL:2008 | 2008 | `INSTEAD OF`löst`TRUNCATE`und`ORDER BY`in Ansichten aus |
| SQL:2011 | 2011 | **Zeitliche Daten** (`AS OF`,`FOR SYSTEM_TIME`),`FETCH FIRST`|
| SQL:2016 | 2016 | **JSON-Unterstützung**, Zeilenmustererkennung |
| SQL:2019 | 2019 | **Polymorphe Tabellenfunktionen**,`LISTAGG`|
| SQL:2023 | 2023 | **`JSON_TABLE`**, `SET`-Operationen, Array-Erweiterungen |
## Wichtige Meilensteine
### SEQUEL und Early SQL (1974–1986)
- **1974**: Donald Chamberlin und Raymond Boyce erstellen SEQUEL bei IBM Research
- **Ziel**: Abfragemanipulation für System R (relationale Datenbank)
- Aufgrund eines Markenkonflikts in SQL (Structured Query Language) umbenannt
- **1986**: Erster ANSI-Standard (SQL-86)
- **1987**: ISO übernimmt SQL-87
### SQL-92 – Die Stiftung (1992)
- **Der wichtigste Standard** – alles moderne SQL basiert darauf
- `INNER JOIN`, `LEFT JOIN`,`RIGHT JOIN`
- Unterabfragen (verschachtelt `SELECT`)
- `CASE`-Ausdruck
-`COALESCE`,`NULLIF`
- `UNIQUE`-, `CHECK`-Einschränkungen
- Schemadefinition (`CREATE SCHEMA`)
### SQL:1999 – Modernes SQL beginnt (1999)
- Reguläre Ausdrücke (`LIKE`, `SIMILAR TO`)
- Rekursive Abfragen (`WITH RECURSIVE`)
- Auslöser
- BLOB/CLOB (Binär-/Zeichen-große Objekte)
- Benutzerdefinierte Typen (UDTs)
-`ORDER BY`in Unterabfragen
### SQL:2003 – Analytics Revolution (2003)
- **Fensterfunktionen**: `ROW_NUMBER()`, `RANK()`, `DENSE_RANK()`, `LAG()`, `LEAD()`,`SUM() OVER()`
- XML-Datentyp und -Funktionen
-`GENERATED ALWAYS AS IDENTITY`
-`SAVEPOINT`(Transaktionskontrolle)
- Hash-Funktionen
### SQL:2011 – Zeitdaten (2011)
- **Temporale Tabellen**:`FOR SYSTEM_TIME AS OF`,`VERSIONING`
-`FETCH FIRST n ROWS ONLY`(Standard `LIMIT`)
-`OFFSET`/`FETCH`Paginierung
### SQL:2016–2023 – JSON und darüber hinaus (2016–heute)
- **2016**: JSON-Datentyp, `JSON_VALUE`, `JSON_QUERY`,`JSON_EXISTS`
- **2019**: Polymorphe Tabellenfunktionen,`LISTAGG`
- **2023**:`JSON_TABLE`(relationale Ansicht von JSON), `SET`-Operationen, Array-Verbesserungen
## Syntaxentwicklung
```sql
