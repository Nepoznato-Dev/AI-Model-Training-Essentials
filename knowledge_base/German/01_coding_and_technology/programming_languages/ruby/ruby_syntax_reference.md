...
-->
# Ruby – Syntaxreferenz
Dieses Dokument bietet eine umfassende, strukturierte Syntaxreferenz für Ruby (3.x). Es ergänzt die Hauptreferenz zu Ruby, indem es sich auf umfassende Syntaxmuster, Blöcke und Abschlüsse, Metaprogrammierung und Ruby-Referenzen konzentriert.
---
...
| `==``!=``===``<=>` | Gleichheit | `a == b`| `===`ist Groß-/Kleinschreibung; `<=>`ist Raumschiff |
| `eql?`| Wertegleichheit | `a.eql?(b)`| Strenger als`==`(Prüftyp) |
| `equal?`| Identität | `a.equal?(b)`| Gleiches Objekt? |
| `<``>``<=``>=` | Vergleich | `a >= b`| |
| `&&``\|\|``!`| Logisch | `a && b`| Kurzschluss; gibt den zuletzt ausgewerteten Wert | zurück
| `and``or``not`| Logisch (niedrige Priorität) | `a and b`| Vermeiden – verwenden Sie`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | Bitweise | `a & b`| |
| `=~``!~` | Regex-Übereinstimmung | `str =~ /pattern/`| Gibt Index oder Null | zurück
| `..``...` | Reichweite | `1..10`| `..`inklusive; `...`exklusiv |
| `?.`| Sichere Navigation | `user&.name`| Gibt Null zurück, wenn der Benutzer null ist |
| `[]``[]=` | Elementzugriff | `arr[0]`| |
### Wahrhaftigkeit
```ruby
...
## Zusammenfassung
Die Syntax von Ruby ist darauf ausgelegt, Entwickler glücklich zu machen – sie liest sich wie Englisch und belohnt Ausdruckskraft. Die Stärke der Sprache beruht auf ihrem Objektmodell (alles ist ein Objekt), Blöcken und Abschlüssen, Metaprogrammierungsfunktionen und dem Enumerable-Modul, das eine umfangreiche Sammlungsverarbeitungs-API bereitstellt. Ruby 3.x hat Mustervergleich, RBS-Typsignaturen und Ractor für Parallelität hinzugefügt und dabei die Kernphilosophie der Sprache beibehalten: Programmierer produktiv und glücklich machen.