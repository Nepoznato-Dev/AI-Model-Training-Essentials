...
## Warum Ruby wichtig ist
- **Entwicklerglück**: Ruby ist so konzipiert, dass es lesbar und unterhaltsam ist. „Ruby soll Programmierer glücklich machen“ – Matz.
- **Ausdrucksstarke Syntax**: Code liest sich wie Englisch. Minimale Interpunktion, natürliche Phrasierung.
- **Ruby on Rails**: Eines der produktivsten Web-Frameworks, die jemals erstellt wurden. Unterstützt GitHub, Shopify, Basecamp, GitLab.
- **Metaprogrammierung**: Ruby kann sich zur Laufzeit selbst ändern – Methoden dynamisch definieren, domänenspezifische Sprachen (DSLs) erstellen.
- **Block-/Iteratormuster**: Rubys Blöcke und Iteratoren machen die Sammlungsverarbeitung elegant.
- **Alles ist ein Objekt**:`3.times { puts "hello" }`– Ganzzahlen haben Methoden.
## Die Kompromisse
| Einschränkung | Einzelheiten | Typische Problemumgehung |
|-----------|---------|-----|
| **Leistung** | Langsamer als kompilierte Sprachen; MRT hat eine GIL | Verwenden Sie JRuby für Parallelität. Auf C-Erweiterungen auslagern |
| **Rückläufige Popularität** | Weniger neue Akzeptanz im Vergleich zu Python, Go, Rust | Immer noch weit verbreitet; stark in Web-Startups und Beratung |
| **Tippen** | Dynamisches Tippen kann zu Laufzeitfehlern führen | Verwenden Sie Sorbet oder RBS für optionale statische Typisierung |
| **Speichernutzung** | Höherer Speicherbedarf als Go oder Rust | Akzeptabel für die meisten Webanwendungen |
| **Stellenmarkt** | Weniger neue Positionen als Python oder JavaScript | Stark in bestimmten Nischen (Schienengeschäfte, Beratung) |
---
...
## Ruby auf Schienen
Rails ist ein Full-Stack-Webframework, das der MVC-Architektur (Model-View-Controller) folgt und Folgendes hervorhebt:
- **Konvention statt Konfiguration**: Sinnvolle Standardeinstellungen – keine Notwendigkeit, alles zu konfigurieren.
- **Wiederholen Sie sich nicht (DRY)**: Verwenden Sie Generatoren, Migrationen und Konventionen, um Wiederholungen zu minimieren.
- **Aktiver Datensatz**: Datenbankobjekte sind Ruby-Objekte. `User.find(1)`ruft einen Benutzer ab.
- **Migrationen**: Datenbankschemaänderungen sind versionierter Ruby-Code.
Rails unterstützt GitHub, Shopify, Stripe (früh), Basecamp, GitLab und viele Startups.
---

## Erweiterte Syntax und Muster
### Ententypisierung und dynamischer Versand
```ruby
...
### Prozesse, Lambdas und Abschlüsse
```ruby
...
### Gemfile – Abhängigkeitsmanagement
```ruby
# Gemfile
...
## Wann man Ruby verwendet
| Szenario | Warum Ruby | Bessere Alternative |
|----------|---------|-----|
| Webanwendungen (Rails) | Schnelle Entwicklung, produktives Framework | Django (Python), Laravel (PHP), Next.js |
| Prototyping | Sehr schnell zu schreiben und zu iterieren | Python, JavaScript |
| Skripterstellung und Automatisierung | Elegante Syntax, leistungsstarke Textverarbeitung | Python, Shell |
| DevOps-Tools (Chef, Puppet) | Etabliertes Ökosystem | Los, Python |
| CLI-Tools | Möglich, aber nicht ideal | Geh, Rust |
| Leistungskritische Systeme | Zu langsam | C, C++, Rust, Go |
