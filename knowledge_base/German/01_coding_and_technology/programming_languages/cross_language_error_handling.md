...
# Sprachübergreifender Vergleich – Fehlerbehandlung
## Übersicht über Fehlerbehandlungsmodelle
| Modell | Sprachen | Mechanismus |
|-------|-----------|-----------|
| **Ausnahmen (try/catch)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **Ergebnis-/Optionstypen** | Rust, Haskell, OCaml, Scala (auch) | `Result<T, E>`,`Maybe a`|
| **Mehrere Rückgabewerte** | Geh, Lua | `value, err := f()`|
| **Fehlercodes** | C, Fortran | Fehlercode zurückgeben, manuell prüfen |
| **Mustervergleichsfehler** | Elixier, Erlang | `{:ok, value}`/`{:error, reason}`|
| **Bedingungen (neustartbar)** | Gemeinsames Lispeln | `restart-case`,`handler-bind`|
| **Keine Fehlerbehandlung** | Scratch, SQL (Grundkenntnisse) | N/A |
| **Verträge** | Ada, Eiffel | `Pre`,`Post`Bedingungen |
...
## Zusammenfassung: Fehlerbehandlungsparadigmen
| Paradigma | Ansatz | Vorteile | Nachteile |
|----------|----------|------|------|
| **Ausnahmen** | werfen/fangen | Sauberer glücklicher Pfad, Stapelspuren | Kann vergessen werden, Kontrollflussmissbrauch |
| **Ergebnistypen** | Rückgabe`Ok`/`Err`| Explizit, Fehler können nicht ignoriert werden | Ausführlich, `?`-Operator erforderlich |
| **Fehlercodes** | Int-Status zurückgeben | Einfach, C-kompatibel | Leicht zu ignorieren, kein Kontext |
| **Mehrfachretouren** | `value, err := f()`| Explizit, idiomatisch | Kann vergessen,`err`| zu überprüfen
| **Mustervergleich** | `{:ok, v}`/`{:error, r}`| Umfangreich, funktional | Ausführlich ohne`with`|
| **Bedingungen** | Neustartbare Handler | Am leistungsstärksten und wiederherstellbarsten | Komplex, selten verstanden |