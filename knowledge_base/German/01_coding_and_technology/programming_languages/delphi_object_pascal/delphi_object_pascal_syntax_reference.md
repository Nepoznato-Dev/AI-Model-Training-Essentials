...
---
# Delphi / Object Pascal – Syntaxreferenz
Dieses Dokument bietet eine umfassende, strukturierte Syntaxreferenz für Delphi/Object Pascal. Es ergänzt die Hauptreferenz von Delphi, indem es sich auf umfassende Syntaxmuster, OOP-Funktionen, Generika, Datensätze und Sprachidiome konzentriert.
---
...
| | `-`| Subtraktion | `A - B`|
| | `*`| Multiplikation | `A * B`|
| | `/`| Reale Teilung | `A / B`(gibt immer Real zurück) |
| | `div`| Ganzzahldivision | `A div B`(gibt Ganzzahl zurück) |
| | `mod`| Modul | `A mod B`|
...
| **Logisch** | `and`| Logisches UND | `A and B`|
| | `or`| Logisches ODER | `A or B`|
| | `not`| Logisches NICHT | `not A`|
| | `xor`| Exklusives ODER | `A xor B`|
| **String** | `+`| Verkettung | `'Hello' + ' ' + 'World'`|
| **Set** | `in`| Mitgliedschaft | `X in [1, 2, 3]`|
| | `+`| Gewerkschaft | `SetA + SetB`|
| | `-`| Unterschied | `SetA - SetB`|
| | `*`| Kreuzung | `SetA * SetB`|
| **Aufgabe** | `:=`| Wert zuweisen | `X := 42`|
| **Adresse** | `@`| Adresse von | `P := @X`|
| | `^`| Dereferenzierung | `P^`|
---
...
## Zusammenfassung
Delphi/Object Pascal kombiniert saubere, lesbare Syntax mit leistungsstarken OOP-Funktionen. Klassen, Schnittstellen, Generika und Datensätze bieten ein vollständiges Toolkit für die Anwendungsentwicklung. Die expliziten Anfangs-/Endblöcke, typisierten Variablen und Bezeichner ohne Berücksichtigung der Groß- und Kleinschreibung machen die Sprache zugänglich. Die VCL/FMX-Frameworks erweitern die Sprache um visuelle Komponentenbibliotheken für eine schnelle Anwendungsentwicklung. Für Windows-Unternehmensanwendungen und Datenbank-Frontends bleibt Delphi eine produktive Wahl.