...
| `<``>``<=``>=` | Vergleich | `a >= b`| |
| `.and.``.or.``.not.`| Logisch | `a .and. b`| |
| `.eqv.``.neqv.` | Logische Äquivalenz | `a .eqv. b`| |
...
## Coarrays (Parallele Programmierung)
```fortran
...
## Zusammenfassung
Die Syntax des modernen Fortran ist sauber, strukturiert und für numerische Berechnungen optimiert. Array-Operationen eliminieren Schleifen für die meisten mathematischen Aufgaben. Module bieten Kapselung und Namespace-Management. OOP-Funktionen (abgeleitete Typen, Vererbung, Polymorphismus) ermöglichen eine saubere Codeorganisation. Coarrays bieten native parallele Programmierung ohne externe Bibliotheken. Die Stärken von Fortran – Leistung, Array-Operationen und numerische Präzision – sind im wissenschaftlichen Rechnen nach wie vor unübertroffen. Für HPC, Computerphysik und technische Simulation ist Fortran weiterhin ein Kraftpaket.