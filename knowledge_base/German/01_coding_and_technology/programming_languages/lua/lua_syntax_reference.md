...
-->
# Lua – Syntaxreferenz
Dieses Dokument bietet eine umfassende, strukturierte Syntaxreferenz für Lua (5.4). Es ergänzt die Hauptreferenz von Lua, indem es sich auf umfassende Syntaxmuster, Tabellen und Metatabellen, Coroutinen und Luas einzigartige Designphilosophie konzentriert.
---
...
| `//`| Bodenaufteilung | `7 // 2`| Gibt 3 zurück (Lua 5.3+) |
| `==``~=` | Gleichheit | `a == b`| `~=`ist „ungleich“ |
| `<``>``<=``>=` | Vergleich | `a >= b`| |
| `and``or``not`| Logisch | `a and b`| Gibt Operanden zurück, keine booleschen Werte |
| `..`| Verkettung | `"hello" .. " world"`| |
| `#`| Länge | `#t`| Länge der Zeichenfolge oder Tabelle |
| `&``\|``~``<<``>>`| Bitweise | `a & b`| Lua 5.3+ (Ganzzahloperationen) |
### Logische Operatoren – Nicht-boolesche Rückgaben
```lua
...
### Rangfolge (vom höchsten zum niedrigsten)
| Vorrang | Operatoren |
|------------|-----------|
| 1 (höchste) | `^`|
| 2 | `not``#``-`(unär) |
...
## Tabellen – Die universelle Datenstruktur
```lua
...
## Zusammenfassung
Die Syntax von Lua ist bewusst minimalistisch gehalten – die gesamte Sprache kann auf einer einzigen Seite beschrieben werden. Tabellen dienen als universelle Datenstruktur (Arrays, Wörterbücher, Objekte, Module). Metatabellen bieten Erweiterbarkeit ohne Sprachkomplexität. Coroutinen ermöglichen kooperatives Multitasking und elegante Iteratormuster. Die Stärke der Sprache liegt in ihrer Einbettbarkeit: Lua ist so konzipiert, dass es in Host-Anwendungen integriert ist und Skriptfunktionen mit minimalem Overhead bereitstellt. Für die Spieleentwicklung, Konfiguration und eingebettete Systeme ist die Einfachheit von Lua seine größte Stärke – eine kleine Sprache, die genau das tut, was sie soll, und nicht mehr.