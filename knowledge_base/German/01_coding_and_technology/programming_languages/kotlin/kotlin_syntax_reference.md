...
-->
# Kotlin – Syntaxreferenz
Dieses Dokument bietet eine umfassende, strukturierte Syntaxreferenz für Kotlin (2.0+). Es ergänzt die Hauptreferenz zu Kotlin, indem es sich auf umfassende Syntaxmuster, Nullsicherheit, Coroutinen und Kotlin-Redewendungen konzentriert.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Arithmetik | `a + b`| |
| `..`| Reichweite | `1..10`| Inklusiv-Sortiment |
| `until`| Halboffenes Sortiment | `1 until 10`| `1..9`|
| `downTo`| Absteigender Bereich | `10 downTo 1`| |
| `==``!=` | Strukturelle Gleichheit | `a == b`| Ruft`.equals()`| auf
| `===``!==` | Referenzielle Gleichheit | `a === b`| Gleiche Instanz |
| `<``>``<=``>=` | Vergleich | `a >= b`| Erfordert`Comparable`|
| `&&``\|\|``!`| Logisch | `a && b`| Kurzschluss |
| `?:`| Elvis-Operator | `a ?: b`| Gibt`b`zurück, wenn`a`null ist |
| `?.`| Sicherer Anruf | `a?.b`| Gibt null zurück, wenn`a`null ist |
| `!!`| Nicht-Null-Behauptung | `a!!`| Löst NPE aus, wenn null |
| `as`| Besetzung | `a as String`| Löst ClassCastException | aus
| `as?`| Sicherer Wurf | `a as? String`| Gibt bei einem Fehler null zurück |
| `in`| Mitgliedschaft | `x in list`| |
| `!in`| Kein Mitglied | `x !in list`| |
| `is`| Typprüfung | `x is String`| Intelligente Besetzung nach der Prüfung |
| `!is`| Geben Sie nicht | ein `x !is String`| |
...
## Zusammenfassung
Die Syntax von Kotlin ist prägnant, sicher und mit Java kompatibel. Null-Sicherheit eliminiert eine ganze Klasse von Fehlern zur Kompilierungszeit. Coroutinen bieten eine natürliche Möglichkeit, asynchronen Code zu schreiben. Versiegelte Klassen, Datenklassen und Erweiterungsfunktionen ermöglichen eine ausdrucksstarke Domänenmodellierung. Die Leistungsfähigkeit der Sprache ergibt sich aus dem Zusammenspiel ihrer Funktionen – Bereichsfunktionen, DSLs, Inline-Klassen und verdinglichte Generika – zusammen ergeben Code, der sowohl sicher als auch angenehm zu schreiben ist.