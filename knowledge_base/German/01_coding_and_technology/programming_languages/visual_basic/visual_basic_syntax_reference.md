...
# Visual Basic – Syntaxreferenz
Dieses Dokument bietet eine umfassende, strukturierte Syntaxreferenz für Visual Basic (VB.NET). Es ergänzt die Hauptreferenz zu Visual Basic, indem es sich auf umfassende Syntaxmuster, OOP-Funktionen, LINQ, Async/Await und .NET-Integrationsidiome konzentriert.
---

## Operatoren und Ausdrücke
| Kategorie | Betreiber | Beschreibung | Beispiel |
|----------|----------|-------------|---------|
| **Arithmetik** | `+`| Zusatz | `a + b`|
| | `-`| Subtraktion | `a - b`|
| | `*`| Multiplikation | `a * b`|
| | `/`| Division (gibt Double zurück) | `a / b`|
| | `\`| Ganzzahldivision | `a \ b`|
| | `Mod`| Modul | `a Mod b`|
| | `^`| Potenzierung | `a ^ b`|
| **Vergleich** | `=`| Gleich | `a = b`|
| | `<>`| Ungleich | `a <> b`|
| | `<`| Weniger als | `a < b`|
| | `>`| Größer als | `a > b`|
| | `<=`| Weniger oder gleich | `a <= b`|
| | `>=`| Größer oder gleich | `a >= b`|
| **Logisch** | `AndAlso`| Kurzschluss UND | `a AndAlso b`|
| | `OrElse`| Kurzschluss ODER | `a OrElse b`|
| | `Not`| Logisches NICHT | `Not a`|
| | `And`| Bitweises UND | `a And b`|
| | `Or`| Bitweises ODER | `a Or b`|
| | `Xor`| Bitweises XOR | `a Xor b`|
| **String** | `&`| Verkettung | `"Hello" & " " & "World"`|
| | `+=`| Anhängen | `s &= " more"`|
| **Aufgabe** | `=`| Zuweisen | `x = 42`|
| **Null** | `Is`| Referenzgleichheit | `obj Is Nothing`|
| | `IsNot`| Referenzungleichung | `obj IsNot Nothing`|
| **Typ** | `TryCast`| Sicherer Cast (gibt nichts zurück) | `TryCast(obj, String)`|
| | `CType`| Konvertieren | `CType(obj, Integer)`|
| | `DirectCast`| Direkter Wurf (wird bei Fehlschlag ausgelöst) | `DirectCast(obj, String)`|
---
...
## Zusammenfassung
Die Syntax von Visual Basic ist klar und englischähnlich, was sie zu einer der am besten lesbaren Programmiersprachen macht. VB.NET bietet vollständigen Zugriff auf das .NET-Ökosystem, einschließlich LINQ für Datenabfragen, Async/Await für gleichzeitige Vorgänge, Generika für Typsicherheit und umfassendes OOP mit Klassen, Schnittstellen und Vererbung. Die Groß-/Kleinschreibung der Sprache und die ausführlichen Schlüsselwörter (End If, End Sub, End Class) verbessern die Lesbarkeit. Für Windows-Unternehmensanwendungen, Office-Automatisierung (VBA) und Rapid Prototyping in .NET bleibt VB.NET eine produktive und leistungsfähige Sprache.