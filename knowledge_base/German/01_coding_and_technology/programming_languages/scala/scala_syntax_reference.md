...
|----------|------|---------|-------|
| `+``-``*``/``%`| Arithmetik | `a + b`| |
| `**`| Potenz (BigDecimal) | `BigDecimal(2) ** 10`| Nicht integriert für Int |
| `==``!=` | Gleichheit | `a == b`| Ruft`.equals()`auf – nullsicher |
| `eq``ne` | Referenzgleichheit | `a eq b`| Gleiche Instanz |
| `<``>``<=``>=` | Vergleich | `a >= b`| Erfordert`Ordered`|
| `&&``\|\|``!`| Logisch | `a && b`| Kurzschluss |
| `&``\|``^``~` | Bitweise | `a & b`| |
| `<<``>>``>>>`| Verschiebung | `a << 2`| |
| `+:``:+``::``:::` | Sammlungseinsätze | `1 +: list`| Rechtsassoziativ, wenn mit`:`| endet
| `->`| Tupelerstellung | `"key" -> "value"`| `("key", "value")`|
| `=>`| Funktion/Pfeil | `x => x + 1`| Lambda- oder Typzuordnung |
| `<-`| Generator | `x <- List(1,2,3)`| Zum Verständnis |
### Betreiberregeln
```scala
...
## Zusammenfassung
Die Syntax von Scala vereint objektorientierte und funktionale Programmierung unter einem leistungsstarken Typsystem. Mustervergleich, For-Comprehensions und Fallklassen ermöglichen eine ausdrucksstarke Datenmodellierung. Merkmale ermöglichen eine flexible Zusammensetzung ohne Einschränkungen bei der Einzelvererbung. Das Typsystem – mit Generika, Varianz, Typgrenzen und undurchsichtigen Typen – fängt Fehler zur Kompilierungszeit ab und ermöglicht gleichzeitig Null-Kosten-Abstraktionen. Scala 3 vereinfacht die Sprache durch Aufzählungen, Erweiterungsmethoden, gegebene/verwendete und undurchsichtige Typen, wodurch die Sprache zugänglicher wird und gleichzeitig ihre volle Leistungsfähigkeit behält.