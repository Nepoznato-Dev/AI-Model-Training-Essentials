...
|----------|------|---------|-------|
| `+``-``*``/``^`| Arithmetik | `2^10`| |
| `%%`| Modul | `7 %% 3`| Gibt 1 | zurück
| `%/%`| Ganzzahldivision | `7 %/% 3`| Gibt 2 | zurück
| `==``!=` | Gleichheit | `x == y`| Vektorisiert |
| `<``>``<=``>=` | Vergleich | `x >= y`| Vektorisiert |
| `&``\|``!`| Logisch (vektorisiert) | `x & y`| Elementweise |
| `&&``\|\|` | Logisch (skalar) | `x && y`| Nur erstes Element |
| `%in%`| Mitgliedschaft | `x %in% y`| |
| `<-``=` | Aufgabe | `x <- 10`| `<-`ist idiomatisch |
| `->`| Rechts zuweisen | `10 -> x`| Selten verwendet |
| `<<-`| Globale Zuweisung | `x <<- 10`| In übergeordneter Umgebung zuweisen |
| `$`| Element extrahieren | `df$column`| |
| `[``[[` | Indizierung | `x[1]`/`x[[1]]`| `[`gibt denselben Typ zurück;  `[[`-Extrakte |
| `%>%`| Pfeife (magrittr) | `x %>% f()`| |
| `\|>`| Rohr (Basis R 4.1+) | `x \|> f()`| Native Pfeife |
...
## Zusammenfassung
Die Syntax von R ist für statistische Berechnungen und Datenanalysen konzipiert. Durch die vektorisierte Natur der Operationen entfallen Schleifen bei den meisten Aufgaben. Das Tidyverse-Ökosystem (dplyr, ggplot2, Tidyr) bietet eine konsistente, lesbare Grammatik für die Datenmanipulation und -visualisierung. Die statistischen Modellierungsfähigkeiten von R – von einfachen T-Tests bis hin zu Mixed-Effects-Modellen – sind in ihrer Tiefe unübertroffen. Für Datenwissenschaft, Statistik und reproduzierbare Forschung bleibt R ein wesentliches Werkzeug.