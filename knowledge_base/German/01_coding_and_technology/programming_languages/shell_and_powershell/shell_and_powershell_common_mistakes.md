...
# Shell und PowerShell – Häufige Fehler und Anti-Patterns
Dieses Dokument katalogisiert die häufigsten Fehler, Fallen und Anti-Patterns in Shell/Bash und PowerShell mit Korrekturen.
---

## 1. Variablen ohne Anführungszeichen (Bash)
```bash
...
## 4. Exit-Codes werden nicht überprüft (Bash)
```bash
...
## 7. Subshell-Variablenbereich (Bash)
```bash
...
## 8. PowerShell: Pipeline-Objekte vs. Strings
```powershell
...
## Zusammenfassung
Shell-Scripting-Traps: Setzen Sie Variablen in Bash immer in Anführungszeichen, verwenden Sie`[[ ]]`anstelle von PowerShell-Traps: Verwenden Sie doppelte Anführungszeichen für die Interpolation, verwenden Sie`-eq`und nicht`==`und denken Sie daran, dass Pipeline-Objekte Eigenschaften haben. Beide belohnen sorgfältiges Zitieren und explizite Fehlerbehandlung.