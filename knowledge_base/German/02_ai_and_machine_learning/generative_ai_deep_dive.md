...
VAEs werden häufig als Komponenten in anderen Modellen verwendet (z. B. verwendet Stable Diffusion ein VAE als Teil seiner Pipeline).
...
| GAN Variant | Schlüsselinnovation |
|-------------|---------------|
| **DCGAN** | Convolutional architectures; Stalltraining |
...
### Latente Verbreitung
Die Diffusion im Pixelraum durchzuführen ist teuer. **Latente Diffusion** (von Stable Diffusion verwendet) führt den Diffusionsprozess stattdessen in einem komprimierten latenten Raum aus.
...
|------------|-------------|
| **Zero-Shot-Klassifizierung** | Bilder ohne Schulung anhand von Textbeschreibungen klassifizieren |
...
| **Schwäche** | Training ist schwierig; Probenahme erfordert MCMC |
...
|-------|----------|-----------------|
| **Sora** (OpenAI) | Diffusionstransformator | Bis zu 1080p; versteht die Physik einigermaßen gut |
