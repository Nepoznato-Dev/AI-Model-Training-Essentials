...
مرجع سريع لأجل Python 3.x بناء الجملة و common patterns.
...
- Use `#` لأجل single-line comments
- Use `"""triple quotes"""` لأجل docstrings و multi-line strings
- Indentation matters (typically 4 spaces)
- Naming conventions: `snake_case` لأجل variables/functions, `PascalCase` لأجل classes
- `__name__ == "__main__"` to check if script is run directly
- Use `virtualenv` or `venv` لأجل project isolation
- Install packages مع `pip install package_name`
