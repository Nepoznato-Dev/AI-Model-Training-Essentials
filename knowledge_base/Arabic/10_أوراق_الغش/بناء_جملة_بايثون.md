<!-- 
This file was automatically translated from English to Arabic. Source: python_syntax. md
...
```python
# Reading files
with open("file. txt", "r") as f: المحتوى = f. read()        # Read entire file
...
# Append mode
with open("file. txt", "a") as f: f. write("More المحتوى\n")
```
...
- Use `#` لأجل sينغle-lفيe comments
- Use `"""triple quotes"""` لأجل docstrينغs و multi-lفيe strينغs
- Indentation matters (typically 4 spaces)
- Namينغ conventions: `snake_case` لأجل variables/functions, `PascalCase` لأجل classes
