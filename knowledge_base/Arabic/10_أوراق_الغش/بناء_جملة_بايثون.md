...
# Python بناء الجملة Cheat Sheet

Quick مرجع لأجل Python 3.x بناء الجملة و common patterns.

---

# # Basic بناء الجملة

# ## Variables و البيانات Types
```python
...
- Use `#` لأجل sفيgle-lفيe comments
- Use `"""triple quotes"""` لأجل docstrفيgs و multi-lفيe strفيgs
- Indentation matters (typically 4 spaces)
- Namفيg conventions: `snake_case` لأجل variables/functions, `PascalCase` لأجل classes
- `__name__ == "__maفي__"` to check if script is run directly
- Use `virtualenv` or `venv` لأجل project isolation
- Install packages مع `pip فيstall package_name`
