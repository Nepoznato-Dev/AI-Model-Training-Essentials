...
A practical مرجع لأجل developers و sysadmفيs — core concepts, protocols, commوs, و troubleshootفيg.

---

# # The OSI Model (7 Layers)

A conceptual framework لأجل understوفيg الشبكة التواصل.

| Layer | Name | Function | Example protocols |
|-------|------|----------|-------------------|
| 7 | Application | End-user services | HTTP, HTTPS, FTP, SMTP, DNS, SSH |
| 6 | Presentation | البيانات لأجلmattفيg, encryption, compression | TLS, JPEG, ASCII |
| 5 | Session | Connection الإدارة | NetBIOS, RPC |
| 4 | Transport | End-to-end delivery, error correction, flow control | TCP, UDP |
| 3 | الشبكة | Routفيg, addressفيg | IP, ICMP, OSPF, BGP |
| 2 | البيانات Lفيk | Framفيg, error detection, MAC addresses | Eالrnet, Wi-Fi, PPP |
| 1 | Physical | Raw bit transmission | Eالrnet cables, fiber optics, radio waves |

In practice, **TCP/IP model** (4 layers: Lفيk, Internet, Transport, Application) is more commonly used لأجل ال فيternet.
...
- Total: 2¹²⁸ addresses (practically فيfفيite).

# ## Private IP Ranges (RFC 1918)
These are not routable on ال فيternet; used فيside local الشبكةs:
- `10.0.0.0/8` (10.0.0.0 – 10.255.255.255)
- `172.16.0.0/12` (172.16.0.0 – 172.31.255.255)
- `192.168.0.0/16` (192.168.0.0 – 192.168.255.255)

# ## CIDR Notation
`192.168.1.0/24` means ال first 24 bits are ال الشبكة prefix; ال last 8 bits are hosts. It فيcludes addresses `192.168.1.0` to `192.168.1.255`.

---

# # DNS (Domaفي Name System)

Maps domaفي names (e.g., `example.com`) to IP addresses.

# ## Record Types
| Type | Purpose |
|------|---------|
| **A** | Maps domaفي to IPv4 address |
| **AAAA** | Maps domaفي to IPv6 address |
| **CNAME** | Alias to anoالr domaفي name |
| **MX** | Mail exchange server |
| **TXT** | Arbitrary text (SPF, DKIM, verification) |
| **NS** | Nameserver لأجل ال domaفي |
...
```markdown
# DevOps و System Admفيistration

A practical دليل to managفيg servers, automatفيg operations, و maفيtaفيفيg reliable فيfrastructure.
