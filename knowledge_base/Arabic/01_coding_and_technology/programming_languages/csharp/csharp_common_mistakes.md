...
يقوم هذا المستند بفهرسة الأخطاء والفخاخ والأنماط المضادة الأكثر شيوعًا في لغة C#. يُظهر كل إدخال النهج غير الصحيح، ويشرح سبب فشله، ويقدم الحل الصحيح.
---

## 1. عدم التخلص من كائنات `IDisposable`
```csharp
// ❌ WRONG — resource leak
...
## 4.LINQ`.ToList()`التخصيص غير الضروري
```csharp
...
## 5. التقاط متغيرات الحلقة (ما قبل C# 5.0)
```csharp
...
## 7. النمط المضاد: فئات الله / محدد موقع الخدمة
```csharp
...
## 10. عدم استخدام السجلات لأنواع البيانات
```csharp
...
## 11. إساءة استخدام عامل تصفية الاستثناءات
```csharp
...
## ملخص
C# هي لغة مصممة بشكل جيد ولكنها تحتوي على مصائد: عدم التخلص من كائنات `IDisposable`، وأساليب باطلة غير متزامنة، وتخصيصات LINQ غير الضرورية، واستثناءات مرجعية فارغة. توفر لغة C# الحديثة (8+) أدوات قوية: أنواع المراجع الخالية، ومطابقة الأنماط، والسجلات، واستخدام الإعلانات، وتعبيرات التبديل. المبادئ الأساسية: التخلص دائمًا من الموارد، وعدم استخدام الفراغ غير المتزامن مطلقًا (باستثناء معالجات الأحداث)، واستخدام حقن المنشئ، وتفضيل سجلات البيانات، وتمكين أنواع المراجع الخالية من الأخطاء لالتقاط الأخطاء الفارغة في وقت الترجمة.