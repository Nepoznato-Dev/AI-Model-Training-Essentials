...
# بيرل - مرجع بناء الجملة
توفر هذه الوثيقة مرجعًا تركيبيًا شاملاً ومنظمًا لـ Perl (5.38+). وهو يكمل مرجع Perl الرئيسي من خلال التركيز على أنماط بناء الجملة الشاملة، والتعبيرات العادية، والمراجع، وقدرة معالجة النص في Perl.
---
...
| `+``-``*``/``%``**` | حسابية | `2 ** 10`| `**`هو الأس |
| `.`| تسلسل السلسلة | `$a . $b`| |
| `x`| تكرار السلسلة | `"ab" x 3`| `"ababab"`|
| `==``!=``<``>``<=``>=` | مقارنة رقمية | `$a == $b`| |
| `eq``ne``lt``gt``le``ge` | مقارنة السلسلة | `$a eq $b`| |
| `<=>`| سفينة الفضاء الرقمية | `$a <=> $b`| إرجاع -1، 0، 1 |
| `cmp`| سفينة الفضاء سلسلة | `$a cmp $b`| |
| `&&``\|\|``!`| منطقي | `$a && $b`| |
| `and``or``not`| منطقي (منخفض الدقة) | `$a and $b`| تجنب استخدام`&&`/`\|\|`|
| `=~``!~` | مباراة Regex | `$str =~ /pattern/`| |
| `..``...` | المدى / الوجه بالتخبط | `1..10`| |
| `?:`| الثلاثي | `$a ? $b : $c`| |
| `//`| محدد-أو | `$a // $b`| `$b`إذا كان`$a`غير محدد |
| `=>`| فاصلة سمينة | `key => 'value'`| اقتباسات تلقائية بكلمات عارية |
### الأختام — الأنواع المتغيرة
```perl
...
## ملخص
بناء جملة بيرل مبني على السياق والعلامات ومبدأ "هناك أكثر من طريقة للقيام بذلك". التعبيرات العادية مدمجة في اللغة، مما يجعل معالجة النص لا مثيل لها في الإيجاز. تتيح المراجع هياكل البيانات المعقدة. يوفر CPAN وحدات لكل شيء. لغة Perl الحديثة (مع`strict`و`warnings`والمتغيرات المعجمية) نظيفة وقابلة للصيانة. إن إرث بيرل لا يزال حيًا في الأنماط والأدوات التي تبنتها كل لغة حديثة.