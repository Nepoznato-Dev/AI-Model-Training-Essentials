...
# دارت - مرجع بناء الجملة
توفر هذه الوثيقة مرجعًا شاملاً ومنظمًا لصيغة Dart (3.x). وهو يكمل مرجع Dart الرئيسي من خلال التركيز على أنماط بناء الجملة الشاملة، والسلامة الخالية، والبرمجة غير المتزامنة، وأنماط التصميم الموجهة نحو Flutter.
---
...
|----------|------|--------|-------|
| `+``-``*``/``%``~/` | حسابية | `7 ~/ 3`| `~/`هو قسمة عدد صحيح |
| `==``!=` | المساواة | `a == b`| |
| `<``>``<=``>=` | مقارنة | `a >= b`| |
| `&&``\|\|``!`| منطقي | `a && b`| ماس كهربائى |
| `??`| اندماج فارغ | `a ?? b`| `b`إذا كانت قيمة`a`فارغة |
| `?.`| الوصول إلى علم فارغ | `a?.b`| فارغة إذا كانت`a`فارغة |
| `!`| تأكيد فارغ | `a!`| يرمي إذا كانت فارغة |
| `..``?..` | تتالي | `obj..a..b`| عمليات السلسلة على نفس الكائن |
| `??=`| تعيين مع علم بالصفر | `a ??= b`| تعيين إذا كانت فارغة |
| `as`| نوع الصب | `obj as String`| يرمي على الفشل |
| `is``is!` | اختبار النوع | `obj is String`| |
| `=>`| وظيفة السهم | `() => expr`| |
### التدوين المتتالي
```dart
...
## ملخص
بناء جملة Dart نظيف ومتسق ومصمم لتحقيق الإنتاجية. تعمل سلامة الصوت الخالية على إزالة الأخطاء المرجعية الفارغة في وقت الترجمة. يوفر المزامنة/الانتظار والتدفقات أنماطًا طبيعية للبرمجة غير المتزامنة. تتيح الفئات والخلطات والفئات المختومة تكوينًا مرنًا للكائنات. تضيف مطابقة الأنماط والسجلات ومعدلات الفصل في Dart 3 مزيدًا من التعبير. في حين أن الغرض الأساسي لـ Dart هو Flutter، فإن تركيبته مناسبة تمامًا لأي مجال تطبيق - جانب الخادم (مع Dart Frog أو Shelf)، وأدوات CLI، وتطبيقات الويب (المترجمة إلى JavaScript).