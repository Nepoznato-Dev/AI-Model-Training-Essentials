...
# بايثون - الأخطاء الشائعة والأنماط المضادة
يقوم هذا المستند بفهرسة الأخطاء والفخاخ والأنماط المضادة الأكثر شيوعًا في بايثون. يُظهر كل إدخال النهج غير الصحيح، ويشرح سبب فشله، ويقدم الحل الصحيح. سيساعدك فهم هذه المخاطر على كتابة تعليمات برمجية بايثونية أكثر قوة.
---

## 1. الحجج الافتراضية القابلة للتغيير
```python
...
**لماذا يحدث ذلك:** يتم تقييم الوسائط الافتراضية مرة واحدة في وقت تعريف الوظيفة، وليس عند كل استدعاء. يتم مشاركة الإعدادات الافتراضية القابلة للتغيير (قائمة، إملاء، مجموعة) عبر جميع الدعوات.
---

## 2. تعديل القائمة أثناء التكرار
```python
...
## 3. الربط المتأخر في عمليات الإغلاق
```python
...
## 4. استخدام`type()`بدلاً من `isinstance()`
```python
...
## 5. عدم استخدام مديري السياق للموارد
```python
...
## 6. عامل التشغيل`is`لمقارنة القيمة
```python
...
## 8. عدم فهم قواعد نطاق لغة بايثون (LEGB)
```python
...
## 10. النمط المضاد: استخدام`except:`Bare باستثناء
```python
...
## 11. إعادة المراجع القابلة للتغيير إلى الحالة الداخلية
```python
...
## 12. عدم استخدام`setdefault`أو `collections.defaultdict`
```python
...
## 15. النمط المضاد: كائنات الإله / الفصول المثقلة
```python
...
## ملخص
إن بساطة Python خادعة - فالافتراضيات القابلة للتغيير، وإغلاق الارتباط المتأخر، وقواعد النطاق، وإدارة الموارد، كلها لها سلوكيات خفية تحاصر المطورين. المبادئ الأساسية التي يجب تذكرها: يتم تقييم الإعدادات الافتراضية مرة واحدة، ويتحقق`is`من الهوية وليس المساواة، ويستخدم دائمًا مديري السياق للموارد، ويلتقط استثناءات محددة، ويفضل التركيب على الكائنات الإلهية. إن كتابة كود بايثون يعني فهم هذه المخاطر واستخدام ميزات اللغة (مديري السياق، والفهم، والمولدات، ووحدة `collections`) على النحو المنشود.