...
|----------|------|--------|--------|-------|
| `+`| اضافة | `3 + 2`| `5`| أيضًا سلسلة/قائمة التسلسل |
| `-`| الطرح | `3 - 2`| `1`| أيضا النفي الأحادي |
| `*`| الضرب | `3 * 2`| `6`| أيضًا تكرار السلسلة/القائمة |
| `/`| القسمة الحقيقية | `7 / 2`| `3.5`| تُرجع دائمًا تعويمًا |
| `//`| تقسيم الارضيات | `7 // 2`| `3`| جولات نحو اللانهاية السلبية |
| `%`| معامل | `7 % 2`| `1`| العلامة تطابق المقسوم عليه |
| `**`| الأس | `2 ** 10`| `1024`| الحق النقابي |
### المقارنة والعوامل المنطقية
| المشغل | الاسم | مثال | ملاحظات |
|----------|------|--------|-------|
| `==`| متساوي | `x == y`| مقارنة القيمة (وليس الهوية) |
| `!=`| غير متساويين | `x != y`| |
| `<`,`>`,`<=`,`>=`| الطلب | `x >= y`| قابل للتسلسل:`a < b < c`|
| `is`| الهوية | `x is y`| نفس الكائن في الذاكرة |
| `is not`| ليست هوية | `x is not None`| يفضل على`!= None`|
| `in`| العضوية | `x in collection`| O(1) للمجموعات/الإملاءات، O(n) للقوائم |
| `and`| منطقي و | `a and b`| ماس كهربائى؛ تُرجع آخر قيمة صحيحة/خطأ |
| `or`| منطقية أو | `a or b`| ماس كهربائى؛ تُرجع أول قيمة صحيحة/خطأ |
| `not`| منطقي لا | `not x`| |
### مشغلي Bitwise
| المشغل | الاسم | مثال | ملاحظات |
|----------|------|--------|-------|
...
| 8 | `^`| اليسار |
| 9 | `\|`| اليسار |
| 10 | `==`,`!=`,`<`,`>`,`<=`,`>=`,`is`,`in`| قابل للتسلسل |
| 11 | `not`| حق |
| 12 | `and`| اليسار |
| 13 (الأدنى) | `or`| اليسار |
### المهمة والمهمة المعززة
```python
...
| جمع | أمر | قابل للتغيير | التكرارات | بناء الجملة | بحث |
|------------|---------|---------|------------|--------|--------|
| `list`| نعم | نعم | نعم | `[1, 2, 3]`| يا(ن) |
| `tuple`| نعم | لا | نعم | `(1, 2, 3)`| يا(ن) |
| `dict`| نعم (3.7+) | نعم | المفاتيح: لا | `{"k": "v"}`| يا (1) متوسط ​​|
| `set`| لا | نعم | لا | `{1, 2, 3}`| يا (1) متوسط ​​|
| `frozenset`| لا | لا | لا | `frozenset({1,2})`| يا (1) متوسط ​​|
### القوائم
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
## ملخص
يغطي مرجع بناء الجملة هذا مجموعة ميزات Python الكاملة - بدءًا من العوامل الأساسية وتدفق التحكم وحتى البرمجة الفوقية المتقدمة والواصفات ونظام الكتابة. إن بناء جملة بايثون بسيط وقابل للقراءة بشكل متعمد، ولكن عمقه كبير: مطابقة الأنماط، والمولدات غير المتزامنة، والبروتوكولات، والفئات الوصفية توفر أدوات قوية للتطبيقات المعقدة. تستمر اللغة في التطور، حيث يضيف كل إصدار ميزات تعمل على تحسين التعبير مع الحفاظ على التوافق مع الإصدارات السابقة.