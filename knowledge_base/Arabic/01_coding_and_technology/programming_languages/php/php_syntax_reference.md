...
| `+``-``*``/``%`| حسابية | `$a + $b`| |
| `**`| الأس | `2 ** 10`| `1024`|
| `==``!=``===``!==` | المساواة | `$a === $b`| `===`نوع الشيكات أيضًا |
| `<=>`| سفينة الفضاء | `$a <=> $b`| إرجاع -1 أو 0 أو 1 |
| `<``>``<=``>=` | مقارنة | `$a >= $b`| |
| `&&``\|\|``!`| منطقي | `$a && $b`| ماس كهربائى |
| `and``or``xor`| منطقي (أسبقية منخفضة) | `$a and $b`| تجنب استخدام`&&`/`\|\|`|
| `??`| اندماج فارغ | `$a ?? 'default'`| إرجاع`'default'`إذا كانت قيمة`$a`فارغة |
| `??=`| تعيين الدمج الفارغ | `$a ??= 'default'`| يتم التعيين فقط إذا كانت قيمة`$a`فارغة |
| `?``:` | الثلاثي | `$a ? $b : $c`| |
| `.`| تسلسل | `$a . $b`| |
| `.=`| تسلسل التعيين | `$a .= $b`| |
| `=>`| مهمة المصفوفة | `['key' => 'value']`| |
| `->`| دخول الأعضاء | `$obj->method()`| |
| `?->`| مشغل Nullsafe | `$obj?->method()`| يُرجع فارغًا إذا كان`$obj`فارغًا |
| `::`| الوصول الثابت | `ClassName::method()`| |
| `instanceof`| اكتب الاختيار | `$obj instanceof ClassName`| |
| `clone`| استنساخ الكائنات | `$copy = clone $obj`| تتطلب النسخة العميقة`__clone()`|
### اكتب شعوذة
```php
...
## ملخص
لقد تطور بناء جملة PHP بشكل كبير من أصول البرمجة النصية إلى لغة حديثة وآمنة للكتابة. قدم PHP 8.x الأنواع الموحدة، وأنواع التقاطع، والتعدادات، والألياف، وفئات القراءة فقط، والسمات، والقابلة للاستدعاء من الدرجة الأولى - مما أدى إلى تحويلها من لغة برمجة مكتوبة بشكل فضفاض إلى لغة تدعم هندسة البرمجيات الصارمة. تظل قوة اللغة هي محور تركيزها على الويب: مع أطر عمل مثل Laravel وSymfony، ونظام بيئي ضخم، ودعم استضافة شبه عالمي، يخدم بناء جملة PHP ملايين التطبيقات. تكافئ PHP الحديثة المطورين الذين يتبنون نظام الكتابة الخاص بها، ويستخدمون التحميل التلقائي القائم على الملحن، ويكتبون تعليمات برمجية نظيفة وموجهة للكائنات.