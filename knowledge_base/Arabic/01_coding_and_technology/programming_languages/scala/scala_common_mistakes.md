...
---
# سكالا — الأخطاء الشائعة والأنماط المضادة
يقوم هذا المستند بفهرسة الأخطاء والفخاخ والأنماط المضادة الأكثر شيوعًا في Scala مع التصحيحات.
---

## 1. استخدام`var`بدلاً من `val`
```scala
...
## 2. عدم استخدام مطابقة الأنماط للأنواع المختومة
```scala
// ❌ WRONG — if/else chain
...
## 5. إساءة استخدام التحويلات الضمنية
```scala
...
## 6. عدم التعامل مع`Option`بشكل صحيح
```scala
...
## 8. محو الكتابة باستخدام الأدوية العامة
```scala
...
## ملخص
تأتي قوة Scala مع المسؤولية: تفضل`val`على `var`، واستخدم مطابقة الأنماط للأنواع المختومة، والتعامل مع`Option`بشكل صحيح (لا تستدعي`.get`أبدًا)، واستخدم الفهم بدلاً من العقود الآجلة المتداخلة، وتجنب القيمة الخالية. يعمل Scala 3 على تبسيط العديد من هذه الأنماط باستخدام التعدادات وطرق الامتداد وأنواع الاتحاد. المبدأ الأساسي: إذا كان المترجم يستطيع التحقق من ذلك، دع المترجم يتحقق منه.