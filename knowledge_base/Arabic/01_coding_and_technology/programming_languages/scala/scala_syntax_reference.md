...
|----------|------|--------|-------|
| `+``-``*``/``%`| حسابية | `a + b`| |
| `**`| الطاقة (BigDecimal) | `BigDecimal(2) ** 10`| غير مدمج لـ Int |
| `==``!=` | المساواة | `a == b`| يستدعي`.equals()`— null-safe |
| `eq``ne` | المساواة المرجعية | `a eq b`| نفس المثال |
| `<``>``<=``>=` | مقارنة | `a >= b`| يتطلب`Ordered`|
| `&&``\|\|``!`| منطقي | `a && b`| ماس كهربائى |
| `&``\|``^``~` | بتوايز | `a & b`| |
| `<<``>>``>>>`| التحول | `a << 2`| |
| `+:``:+``::``:::` | عمليات التجميع | `1 +: list`| ارتباط يمين إذا كان ينتهي بـ`:`|
| `->`| خلق الصفوف | `"key" -> "value"`| `("key", "value")`|
| `=>`| الوظيفة/السهم | `x => x + 1`| لامدا أو نوع التعيين |
| `<-`| مولد | `x <- List(1,2,3)`| للفهم |
### قواعد المشغل
```scala
...
## ملخص
يقوم بناء جملة Scala بتوحيد البرمجة الموجهة للكائنات والبرمجة الوظيفية ضمن نظام كتابة قوي. توفر مطابقة الأنماط والفهم وفئات الحالة نمذجة بيانات معبرة. تتيح السمات تكوينًا مرنًا دون قيود الميراث الفردي. نظام الكتابة - مع الأدوية العامة والتباين وحدود الكتابة والأنواع غير الشفافة - يلتقط الأخطاء في وقت الترجمة مع تمكين التجريدات بدون تكلفة. يعمل Scala 3 على تبسيط اللغة باستخدام التعدادات وطرق الامتداد والأنواع المعطاة/الاستخدامية والأنواع غير الشفافة، مما يجعل اللغة أكثر سهولة مع الاحتفاظ بقوتها الكاملة.