...
-->
# C++ - مرجع بناء الجملة
يوفر هذا المستند مرجعًا شاملاً ومنظمًا لصيغة Modern C++ (C++17/20/23). وهو يكمل المرجع الرئيسي لـ C++ من خلال التركيز على أنماط بناء الجملة الشاملة، وجداول التشغيل، وميكانيكا القوالب، والميزات الحديثة.
---
...
|----------|------|--------|-------|
| `+``-``*``/``%`| حسابية | `a + b`| نفس C |
| `++``--` | زيادة/إنقاص | `++i`,`i++`| تفضل الزيادة المسبقة |
| `==``!=``<``>``<=``>=` | مقارنة | `a <=> b`| `<=>`هي مقارنة ثلاثية (C++20) |
...
| المشغل | الاسم | مثال | ملاحظات |
|----------|------|--------|-------|
| `::`| نطاق القرار | `std::cout`| |
| `.*``->*` | مؤشر إلى عضو | `obj.*pmf`| |
| `<=>`| سفينة الفضاء (C++20) | `a <=> b`| إرجاع`strong_ordering`|
| `co_await`| تعليق كوروتين | `co_await expr`| سي++20 |
| `co_yield`| العائد كوروتين | `co_yield value`| سي++20 |
| `co_return`| عودة كوروتين | `co_return value`| سي++20 |
...
## التحكم في التدفق
### الارتباطات المنظمة والأنماط الحديثة
```cpp
...
## حاويات وخوارزميات STL
### نظرة عامة على الحاوية
| حاوية | رأس | أمر | بحث | ملاحظات |
|-----------|-------|---------|--------|-------|
| `vector`| `<vector>`| نعم | يا(ن) | الاختيار الافتراضي للتسلسلات |
| `deque`| `<deque>`| نعم | يا(ن) | دفع/فرقعة سريعة من كلا الطرفين |
| `list`| `<list>`| نعم | يا(ن) | قائمة مرتبطة بشكل مضاعف |
| `array`| `<array>`| نعم | يا(ن) | حجم ثابت، مكدس مخصص |
| `set`/`multiset`| `<set>`| نعم | يا(سجل ن) | شجرة حمراء سوداء |
| `map`/`multimap`| `<map>`| نعم (بالمفتاح) | يا(سجل ن) | القيمة الرئيسية، شجرة حمراء سوداء |
| `unordered_set`| `<unordered_set>`| لا | يا (1) متوسط ​​| جدول التجزئة |
| `unordered_map`| `<unordered_map>`| لا | يا (1) متوسط ​​| جدول التجزئة |
...
## ملخص
يمتد بناء جملة C++ من معالجة المؤشر منخفض المستوى إلى البرمجة الفوقية للقالب عالي المستوى والمزامنة المستندة إلى coroutine. لقد أدى تطور اللغة من خلال C++17/20/23 إلى تحسين الأمان بشكل كبير (المؤشرات الذكية، `std::optional`، المفاهيم)، والتعبير (الارتباطات المنظمة، والنطاقات، `std::format`)، والأداء (المفاهيم التي تتيح التجريدات بدون تكلفة، وcoroutines لعدم المزامنة دون تحميل المكدس). تبقى الفلسفة الأساسية دون تغيير: يجب ألا تدفع مقابل ما لا تستخدمه، ويجب أن تدعم اللغة كل مستوى من التجريد بدءًا من اللغات المعدنية وحتى اللغات الخاصة بالمجال.