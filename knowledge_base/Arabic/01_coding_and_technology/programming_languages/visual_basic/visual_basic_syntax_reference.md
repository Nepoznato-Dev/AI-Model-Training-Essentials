...
-->
# فيجوال بيسك - مرجع بناء الجملة
يوفر هذا المستند مرجعًا شاملاً ومنظمًا لصيغة Visual Basic (VB.NET). وهو يكمل مرجع Visual Basic الرئيسي من خلال التركيز على أنماط بناء الجملة الشاملة، وميزات OOP، وLINQ، وasync/await، ومصطلحات التكامل .NET.
---

## العوامل والتعبيرات
| الفئة | المشغل | الوصف | مثال |
|----------|---------|-------------|---------|
| **الحساب** | `+`| اضافة | `a + b`|
| | `-`| الطرح | `a - b`|
| | `*`| الضرب | `a * b`|
| | `/`| تقسيم (يرجع مزدوج) | `a / b`|
| | `\`| قسمة الأعداد الصحيحة | `a \ b`|
| | `Mod`| معامل | `a Mod b`|
| | `^`| الأس | `a ^ b`|
| **المقارنة** | `=`| متساوي | `a = b`|
| | `<>`| لا يساوي | `a <> b`|
...
| | `<=`| أقل أو يساوي | `a <= b`|
| | `>=`| أكبر أو يساوي | `a >= b`|
| **منطقي** | `AndAlso`| ماس كهربائى و | `a AndAlso b`|
| | `OrElse`| ماس كهربائى أو | `a OrElse b`|
| | `Not`| منطقي لا | `Not a`|
| | `And`| Bitwise و | `a And b`|
| | `Or`| Bitwise أو | `a Or b`|
| | `Xor`| XOR بالبت | `a Xor b`|
| ** سلسلة ** | `&`| تسلسل | `"Hello" & " " & "World"`|
| | `+=`| إلحاق | `s &= " more"`|
| **المهمة** | `=`| تعيين | `x = 42`|
| **خالية** | `Is`| المساواة المرجعية | `obj Is Nothing`|
| | `IsNot`| عدم المساواة المرجعية | `obj IsNot Nothing`|
| **النوع** | `TryCast`| طاقم الممثلين الآمن (لا يُرجع شيئًا) | `TryCast(obj, String)`|
| | `CType`| تحويل | `CType(obj, Integer)`|
| | `DirectCast`| الإلقاء المباشر (رمية الفشل) | `DirectCast(obj, String)`|
---
...
## ملخص
بناء جملة Visual Basic نظيف ويشبه اللغة الإنجليزية، مما يجعلها واحدة من أكثر لغات البرمجة قابلية للقراءة. يوفر VB.NET الوصول الكامل إلى النظام البيئي .NET بما في ذلك LINQ للاستعلام عن البيانات، والمزامنة/الانتظار للعمليات المتزامنة، والأدوية العامة لسلامة النوع، وOOP الشامل مع الفئات والواجهات والميراث. تعمل عدم حساسية حالة الأحرف والكلمات الأساسية المطولة (End If وEnd Sub وEnd Class) على تحسين إمكانية القراءة. بالنسبة لتطبيقات Windows الخاصة بالمؤسسات، وأتمتة المكاتب (VBA)، والنماذج الأولية السريعة ضمن .NET، تظل VB.NET لغة منتجة وقادرة.