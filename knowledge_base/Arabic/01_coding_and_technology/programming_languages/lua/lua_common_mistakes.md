...
# لوا — الأخطاء الشائعة والأنماط المضادة
يقوم هذا المستند بفهرسة الأخطاء والفخاخ والأنماط المضادة الأكثر شيوعًا في Lua مع التصحيحات.
---
...
## 4. المتغيرات العالمية بشكل افتراضي
```lua
...
## 7. عدم استخدام الجداول الوصفية لـ OOP
```lua
...
## ملخص
تعمل بساطة Lua على إنشاء مصائد: الفهرسة المستندة إلى 1، فقط`nil`و`false` زائفتان، والجداول هي مراجع (وليست قيم)، والمتغيرات عامة بشكل افتراضي، و`pairs` غير مرتبة بينما يتم ترتيب `ipairs`. طريقة Lua هي: استخدم`local`دائمًا، واستخدم`ipairs`للمصفوفات و`pairs` للقواميس، واستخدم`table.concat`لبناء السلسلة، واستخدم الجداول الوصفية لـ OOP. إن بساطة Lua هي قوتها - ولكن احترم الحالات المتطورة.