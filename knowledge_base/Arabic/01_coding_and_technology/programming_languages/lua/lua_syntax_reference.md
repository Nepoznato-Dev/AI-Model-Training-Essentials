...
|----------|------|--------|-------|
| `+``-``*``/``%``^` | حسابية | `2 ^ 10`| `^`هو الأس |
| `//`| تقسيم الارضيات | `7 // 2`| إرجاع 3 (لوا 5.3+) |
| `==``~=` | المساواة | `a == b`| `~=`"غير متساوٍ" |
| `<``>``<=``>=` | مقارنة | `a >= b`| |
| `and``or``not`| منطقي | `a and b`| إرجاع المعاملات، وليس القيم المنطقية |
| `..`| تسلسل | `"hello" .. " world"`| |
| `#`| الطول | `#t`| طول السلسلة أو الجدول |
| `&``\|``~``<<``>>`| بتوايز | `a & b`| لوا 5.3+ (عدد صحيح للعمليات) |
### العوامل المنطقية — عمليات الإرجاع غير المنطقية
```lua
...
## الجداول — بنية البيانات العالمية
```lua
...
## الجداول الوصفية والطرق الوصفية
```lua
...
## ملخص
بناء جملة Lua بسيط عمدًا - يمكن وصف اللغة بأكملها في صفحة واحدة. تعمل الجداول بمثابة بنية البيانات العالمية (المصفوفات والقواميس والكائنات والوحدات النمطية). توفر الجداول الوصفية قابلية للتوسعة دون تعقيد اللغة. تعمل Coroutines على تمكين تعدد المهام التعاونية وأنماط التكرار الأنيقة. تأتي قوة اللغة من قابليتها للتضمين: تم تصميم Lua للعيش داخل التطبيقات المضيفة، مما يوفر إمكانات البرمجة النصية بأقل قدر من الحمل. بالنسبة لتطوير الألعاب، والتكوين، والأنظمة المدمجة، فإن بساطة Lua هي أعظم نقاط قوتها - فهي لغة صغيرة تفعل ما تحتاج إليه بالضبط، وليس أكثر.