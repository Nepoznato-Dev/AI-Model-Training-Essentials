...
|----------|------|--------|-------|
| `+``-``*``/``%``**` | حسابية | `2 ** 10`| `**`هو الأس |
| `==``!=``===``<=>` | المساواة | `a == b`| `===`هي مساواة الحالة؛ `<=>`هي سفينة فضاء |
| `eql?`| المساواة في القيمة | `a.eql?(b)`| أكثر صرامة من`==`(نوع الشيكات) |
| `equal?`| الهوية | `a.equal?(b)`| نفس الكائن؟ |
| `<``>``<=``>=` | مقارنة | `a >= b`| |
| `&&``\|\|``!`| منطقي | `a && b`| ماس كهربائى؛ إرجاع آخر قيمة تم تقييمها |
| `and``or``not`| منطقي (أسبقية منخفضة) | `a and b`| تجنب استخدام`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | بتوايز | `a & b`| |
| `=~``!~` | مباراة Regex | `str =~ /pattern/`| إرجاع الفهرس أو لا شيء |
| `..``...` | النطاق | `1..10`| `..`شاملاً؛ `...`حصريا |
| `?.`| الملاحة الآمنة | `user&.name`| إرجاع لا شيء إذا كان المستخدم لا شيء |
| `[]``[]=` | الوصول إلى العنصر | `arr[0]`| |
### الصدق
```ruby
...
## ملخص
تم تصميم بناء جملة روبي لإسعاد المطورين - فهو يقرأ مثل اللغة الإنجليزية ويكافئ التعبير. تأتي قوة اللغة من نموذج الكائن الخاص بها (كل شيء هو كائن)، والكتل والإغلاقات، وقدرات البرمجة الوصفية، والوحدة النمطية Enumerable التي توفر واجهة برمجة تطبيقات معالجة مجموعة غنية. أضافت Ruby 3.x مطابقة الأنماط، وتوقيعات نوع RBS، وRactor للتزامن، مع الحفاظ على الفلسفة الأساسية للغة: جعل المبرمجين منتجين وسعداء.