...
| نموذج | اللغات | آلية |
|-------|----------|-----------|
| ** الاستثناءات (المحاولة/الالتقاط)** | Java، C++، C#، Python، Ruby، PHP، JavaScript، TypeScript، Kotlin، Swift، Dart، Scala، VB، Delphi | `try { } catch (e) { }`|
| **أنواع النتائج/الخيارات** | روست، هاسكل، أوكامل، سكالا (أيضًا) | `Result<T, E>`,`Maybe a`|
| **قيم الإرجاع المتعددة** | اذهب يا لوا | `value, err := f()`|
| **رموز الخطأ** | ج، فورتران | قم بإرجاع رمز الخطأ، تحقق يدويًا |
| **أخطاء مطابقة النمط** | إكسير، إرلانج | `{:ok, value}`/`{:error, reason}`|
| **الشروط (قابلة لإعادة التشغيل)** | اللثغة المشتركة | `restart-case`,`handler-bind`|
| ** لا توجد معالجة للأخطاء ** | سكراتش، SQL (أساسي) | لا يوجد |
| **العقود** | آدا، إيفل |  شروط`Pre`,`Post`|
## معالجة الأخطاء المستندة إلى الاستثناء
```python
# Python
try:
    result = 10 / 0
...
## ملخص: نماذج التعامل مع الأخطاء
| النموذج | النهج | الايجابيات | سلبيات |
|----------|---------|------|------|
| **الاستثناءات** | رمي/قبض | مسار سعيد نظيف، آثار المكدس | يمكن نسيانها والتحكم في إساءة استخدام التدفق |
| **أنواع النتائج** | إرجاع`Ok`/`Err`| صريح، لا يمكن تجاهل الأخطاء | مطول، مطلوب مشغل`?`|
| **رموز الخطأ** | إرجاع الحالة | بسيط ومتوافق مع C | من السهل التجاهل، لا يوجد سياق |
| **عائدات متعددة** | `value, err := f()`| صريح، اصطلاحي | يمكن أن تنسى التحقق من`err`|
| **مطابقة الأنماط** | `{:ok, v}`/`{:error, r}`| شاملة وعملية | مطول بدون`with`|
| **الشروط** | معالجات قابلة لإعادة التشغيل | أقوى، قابلة للاسترداد | معقدة ونادرا ما تكون مفهومة |