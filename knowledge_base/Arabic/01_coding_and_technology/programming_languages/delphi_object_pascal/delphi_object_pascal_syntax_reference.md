...
| | `-`| الطرح | `A - B`|
| | `*`| الضرب | `A * B`|
| | `/`| القسمة الحقيقية | `A / B`(ترجع دائمًا Real) |
| | `div`| قسمة الأعداد الصحيحة | `A div B`(إرجاع عدد صحيح) |
| | `mod`| معامل | `A mod B`|
| **المقارنة** | `=`| متساوي | `A = B`|
| | `<>`| لا يساوي | `A <> B`|
| | `<`| أقل من | `A < B`|
...
| | `<=`| أقل أو يساوي | `A <= B`|
| | `>=`| أكبر أو يساوي | `A >= B`|
| **منطقي** | `and`| منطقي و | `A and B`|
| | `or`| منطقية أو | `A or B`|
| | `not`| منطقي لا | `not A`|
| | `xor`| حصريا أو | `A xor B`|
| ** سلسلة ** | `+`| تسلسل | `'Hello' + ' ' + 'World'`|
| **مجموعة** | `in`| العضوية | `X in [1, 2, 3]`|
| | `+`| الاتحاد | `SetA + SetB`|
| | `-`| الفرق | `SetA - SetB`|
| | `*`| تقاطع | `SetA * SetB`|
| **المهمة** | `:=`| تعيين القيمة | `X := 42`|
| **العنوان** | `@`| عنوان | `P := @X`|
| | `^`| المرجعية | `P^`|
---
...
## ملخص
يجمع Delphi/Object Pascal بين بناء الجملة النظيف والقابل للقراءة وميزات OOP القوية. توفر الفئات والواجهات والأدوية والسجلات مجموعة أدوات كاملة لتطوير التطبيقات. إن كتل البداية/النهاية الصريحة للغة، والمتغيرات المكتوبة، والمعرفات الخالية من تحسس حالة الأحرف تجعلها سهلة الاستخدام. تعمل أطر عمل VCL/FMX على توسيع اللغة من خلال مكتبات المكونات المرئية للتطوير السريع للتطبيقات. بالنسبة لتطبيقات Windows الخاصة بالمؤسسات والواجهات الأمامية لقاعدة البيانات، تظل دلفي خيارًا منتجًا.