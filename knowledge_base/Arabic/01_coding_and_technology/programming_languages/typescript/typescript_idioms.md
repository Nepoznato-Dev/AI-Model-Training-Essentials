...
## ملخص
تؤكد مصطلحات TypeScript على: التحقق الصارم من النوع، والواجهات لأشكال الكائنات، والاتحادات التمييزية للحالة، وحرس الكتابة للتضييق، والأدوية العامة ذات القيود، و`readonly` للثبات، و`as const` للأنواع الحرفية. اتبع ESLint باستخدام `typescript-eslint`، أجمل للتنسيق، وقم دائمًا بتمكين `strict: true`. يقدّر مجتمع TypeScript سلامة النوع، فإذا تم تجميعه، فمن المحتمل أن يعمل. تفضل`interface`لأشكال الكائنات، و`type` للوحدات والأدوات المساعدة، وتجنب`any`بأي ثمن.