...
|----------|------|--------|-------|
| `+`| اضافة | `3 + 2`| أيضًا تسلسل`String`عبر`format!`|
| `-`| الطرح | `3 - 2`| |
| `*`| الضرب | `3 * 2`| |
| `/`| شعبة | `7 / 2`| تقسيم الأعداد الصحيحة للأعداد الصحيحة:`3`|
| `%`| الباقي | `7 % 2`| `1`|
| `-x`| النفي | `-5`| |
### المقارنة والعوامل المنطقية
| المشغل | الاسم | مثال | ملاحظات |
|----------|------|--------|-------|
| `==`| متساوي | `x == y`| يتطلب`PartialEq`|
| `!=`| غير متساويين | `x != y`| |
| `<`,`>`,`<=`,`>=`| الطلب | `x >= y`| يتطلب`PartialOrd`|
...
| `\|`| أو | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `!`| ليس | `!0b1010u8`| تكملة bitwise |
| `<<`| التحول الأيسر | `5 << 1`| `10`|
| `>>`| التحول الأيمن | `5 >> 1`| `2`|
### أسبقية عامل التشغيل (من الأعلى إلى الأدنى)
| الأسبقية | المشغلين | ملاحظات |
|------------|-----------|-------|
| 1 (الأعلى) |  مسارات`()``.` :: | |
| 2 | استدعاءات الطريقة، الوصول الميداني | تحديد تلقائي |
| 3 | `-`(النفي)، `!`،`*`(deref) | |
| 4 | `as`(نوع الزهر) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
| 13 | `\|\|`| |
| 14 | `..``..=` | النطاق |
| 15 | `=``+=` وما إلى ذلك | الواجب |
| 16 (الأدنى) | `return``break``continue`| |
---
...
## هياكل البيانات
### نظرة عامة على الأنواع المضمنة
| اكتب | المكدس/الكومة | الحجم | قابلية التغيير | ملاحظات |
|------|-----------|------------|-------|------|------|------|------|
| البدائيات (`i32`,`f64`,`bool`,`char`) | كومة | ثابت | غير قابل للتغيير بشكل افتراضي | `Copy`|
| `[T; N]`(مصفوفة) | كومة | ثابت | قابل للتغيير باستخدام`mut`| حجم ثابت،`Copy`إذا كان T هو`Copy`|
| `Vec<T>`| كومة | ديناميكي | قابل للتغيير باستخدام`mut`| قابل للنمو |
| `String`| كومة | ديناميكي | قابل للتغيير مع`mut`| UTF-8، قابل للنمو |
| `&[T]`(شريحة) | كومة (الدهون بتر) | عرض ديناميكي | يعتمد على`&`أو`&mut`| عرض مستعار |
| `HashMap<K, V>`| كومة | ديناميكي | قابل للتغيير مع`mut`| O(1) متوسط ​​البحث |
| `BTreeMap<K, V>`| كومة | ديناميكي | قابل للتغيير مع`mut`| O(log n) بحث، أمر |
| `HashSet<T>`| كومة | ديناميكي | — | O(1) متوسط، لا يوجد تكرارات |
### المتجهات
```rust
// Creation
...
### سمات المكتبة القياسية المشتركة
```rust
...
## ملخص
تم بناء بناء جملة Rust على أساس ضمانات السلامة المطبقة في وقت الترجمة. يشكل نظام الملكية، وتعدد الأشكال القائم على السمات، ومطابقة الأنماط، والتجريدات بدون تكلفة تصميمًا متماسكًا حيث يكون المترجم شريكًا صارمًا ولكن عادلاً. بدءًا من تدفق التحكم الأساسي وحتى الميزات المتقدمة مثل وحدات الماكرو والتعليمات البرمجية غير الآمنة والمؤشرات الذكية، يخدم كل إنشاء هدف برمجة أنظمة آمنة وفعالة. إن فهم التفاعل بين فترات الحياة والاقتراض ونظام الكتابة هو المفتاح لإتقان Rust.