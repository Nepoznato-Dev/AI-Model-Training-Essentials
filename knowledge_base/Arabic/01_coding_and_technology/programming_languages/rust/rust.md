...
تم التصويت لـ Rust على أنها لغة البرمجة "الأكثر تفضيلاً" في استطلاع مطوري Stack Overflow لعدة سنوات متتالية. يتم استخدامه بشكل متزايد في برمجة الأنظمة، وWebAssembly، وأدوات CLI، والبنية التحتية السحابية، وكبديل لـ C/C++ في السياقات الأمنية الحرجة. تقبل نواة Linux الآن كود Rust.
---
...
|-----------|------------------------|---|
| ** منحنى التعلم الحاد ** | الملكية والاقتراض والأعمار لا تشبه أي شيء في اللغات الأخرى | استثمر وقتك في "كتاب الصدأ"؛ المفاهيم انقر مع الممارسة |
| **تجميع بطيء** | يمكن أن تكون أوقات الترجمة طويلة بالنسبة للمشاريع الكبيرة | استخدم`cargo check`للتحقق السريع من الكتابة؛ التجميع التزايدي يساعد |
| ** معالجة الأخطاء المطوَّلة ** |  يتطلب عامل التشغيل`Result<T, E>`و`?` معالجة صريحة | استخدم`anyhow`للتطبيقات،`thiserror`للمكتبات |
| **سوق العمل الأصغر** | وظائف Rust أقل من وظائف Java أو Python أو JavaScript (لكنها تنمو بسرعة) | معظم أدوار Rust موجودة في برمجة الأنظمة أو التشفير أو البنية التحتية |
| ** النظام البيئي غير الناضج ** | مكتبات أقل من Python/Java/JS لبعض المجالات | النظام البيئي ينمو بسرعة. العديد من الصناديق ذات نوعية ممتازة |
---
...
### الملكية والاقتراض
هذا هو الابتكار الأساسي لرست. كل قيمة لها مالك واحد بالضبط. عندما يخرج المالك عن النطاق، يتم إسقاط القيمة.
```rust
...
### الهياكل والتعدادات ومطابقة الأنماط
```rust
struct Point {
    x: f64,
    y: f64,
...
### الأدوية العامة وحدود السمات
تتيح لك الأدوية العامة كتابة التعليمات البرمجية التي تعمل مع أي نوع مع الحفاظ على أمان النوع الكامل. السمات تحدد السلوك المشترك.
```rust
...
### وحدات الماكرو
يحتوي الصدأ على نوعين من وحدات الماكرو: التصريحية (`macro_rules!`) والإجرائية (اشتقاق، سمة، تشبه الوظيفة).
```rust
...
### مطابقة الأنماط المتقدمة وتدميرها
```rust
...
## التزامن والتوازي
### نموذج الموضوع والتزامن
يمنع نظام ملكية Rust سباقات البيانات في وقت الترجمة. تعمل السمتان`Send`و`Sync` على فرض سلامة الخيط.
```rust
use std::thread;
...
| أداة | الغرض |
|------|---------|
| **البضائع** | نظام البناء، مدير الحزم، مشغل الاختبار، منشئ المستندات |
| **crates.io** | تسجيل الطرود (أكثر من 150.000 صندوق) |
| **صدأ** | منسق الكود |
| ** كليبي ** | Linter مع مئات الفحوصات المفيدة |
| **طوكيو** | وقت التشغيل غير المتزامن (معيار الصدأ غير المتزامن) |
| **سيردي** | إطار عمل التسلسل/إلغاء التسلسل |
| ** أكتيكس ويب / أكسوم ** | أطر الويب |
| **ديزل/سقلكس** | ORMs لقاعدة البيانات / منشئو الاستعلام |
---

## متى يجب استخدام الصدأ
| السيناريو | لماذا الصدأ | البديل الأفضل |
|----------|--------|------------------|
| برمجة الأنظمة | سلامة الذاكرة + الأداء | C/C++ إذا كنت لا تحتاج إلى ضمانات السلامة |
| تجميع الويب | دعم WASM الأفضل في فئته | -- |
| أدوات سطر الأوامر | سريع، ثنائي واحد، تجربة مستخدم رائعة | اختر CLIs الأبسط |
| الأنظمة المدمجة | لا يوجد GC، الوصول إلى الأجهزة، السلامة | C لأبسط المضمنة |
| كود الأداء الحرج | يطابق سرعة C/C++ | -- |
| البنية التحتية السحابية | اعتماد متزايد (AWS، Cloudflare) | اذهب للتطوير الأسرع |
| تطوير التطبيقات العامة | منحنى التعلم الحاد يبطئ عملية التطوير | بايثون، جو، جافا |
| الواجهات الخلفية للويب | ممكن ولكن النظام البيئي أصغر سنا | اذهب، Node.js، بايثون |
| علم البيانات / تعلم الآلة | ليس النظام البيئي لهذا | بايثون، ر |
| مخطوطات سريعة / نماذج أولية | مطول جدًا وبطيء في الكتابة | بايثون، جافا سكريبت |
---

## ملخص
Rust هي لغة تجبرك على التفكير في الذاكرة والملكية والتزامن - وتكافئك بالكود الصحيح من خلال البناء. إن منحنى التعلم حقيقي، ولكن المردود كبير: برامج بنفس سرعة لغة C ولكنها خالية من أخطاء المؤشر الفارغ، وسباقات البيانات، وتسرب الذاكرة. الصدأ ليست لغة إنتاجية للأغراض العامة - إنها لغة أنظمة عندما يكون كل من الصحة والأداء مهمًا. ويشير اعتمادها المتزايد في الصناعة (بما في ذلك نواة لينكس وأندرويد) إلى أنها ستكون ذات أهمية متزايدة.