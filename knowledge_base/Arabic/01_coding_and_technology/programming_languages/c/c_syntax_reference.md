...
| `*`| الضرب | `3 * 2`| `6`| |
| `/`| شعبة | `7 / 2`| `3`| اقتطاع عدد صحيح لأنواع int |
| `%`| معامل | `7 % 2`| `1`| فقط للأنواع الصحيحة |
| `++`| زيادة | `i++`/`++i`| | ما بعد وما قبل الزيادة |
| `--`| إنقاص | `i--`/`--i`| | ما بعد وما قبل التخفيض |
...
| `\|`| أو | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `~`| تكملة | `~0`| انقلبت كل البتات |
| `<<`| التحول الأيسر | `1 << 3`| `8`|
| `>>`| التحول الأيمن | `8 >> 3`| `1`(محدد للتنفيذ للتوقيع) |
### مشغلي المؤشر والعنوان
| المشغل | الاسم | مثال | ملاحظات |
|----------|------|--------|-------|
| `&`| عنوان | `&x`| إرجاع المؤشر إلى x |
| `*`| المرجعية | `*ptr`| قيمة الوصول عند المؤشر |
| `->`| وصول الأعضاء عبر المؤشر | `p->x`| أي ما يعادل`(*p).x`|
| `.`| دخول الأعضاء | `s.field`| الوصول المباشر لأعضاء الهيكل |
| `[]`| صفيف منخفض | `arr[i]`| أي ما يعادل`*(arr + i)`|
### أسبقية عامل التشغيل (من الأعلى إلى الأدنى)
| الأسبقية | المشغلين | الترابط |
|------------|-----------|---------------|
| 1 (الأعلى) | `()``[]``->``.` | من اليسار إلى اليمين |
| 2 | `!``~``+``-` (أحادي)`*``&``sizeof``(type)` | من اليمين إلى اليسار |
| 3 | `*``/``%`| من اليسار إلى اليمين |
| 4 | `+``-` | من اليسار إلى اليمين |
| 5 | `<<``>>` | من اليسار إلى اليمين |
| 6 | `<``<=``>``>=` | من اليسار إلى اليمين |
| 7 | `==``!=` | من اليسار إلى اليمين |
| 8 | `&`(bitwise AND) | من اليسار إلى اليمين |
| 9 | `^`| من اليسار إلى اليمين |
| 10 | `\|`| من اليسار إلى اليمين |
| 11 | `&&`| من اليسار إلى اليمين |
| 12 | `\|\|`| من اليسار إلى اليمين |
| 13 | `? :`(ثلاثي) | من اليمين إلى اليسار |
| 14 | `=``+=``-=``*=``/=``%=``&=``\|=``^=``<<=``>>=`| من اليمين إلى اليسار |
| 15 (الأدنى) | `,`(فاصلة) | من اليسار إلى اليمين |
---
...
## ملخص
بناء جملة لغة C بسيط وقريب من مؤشرات الأجهزة وإدارة الذاكرة اليدوية وطبقة تجريد رقيقة فوق تعليمات الجهاز. تأتي قوة اللغة من هذه البساطة: لا توجد أي تكاليف مخفية تقريبًا، ولا توجد تكاليف تشغيل إضافية، وتحكم كامل في تخطيط الذاكرة. على الرغم من أن المعالج الأولي قوي، إلا أنه يتطلب الانضباط لاستخدامه بأمان. أضافت لغة C الحديثة (C11/C17/C23) ميزات أمان مهمة مثل`_Static_assert`و`_Generic`و`nullptr`و `typeof`، مما يجعل اللغة أكثر أمانًا بشكل متزايد مع الحفاظ على فلسفتها الأساسية المتمثلة في الثقة في المبرمج.