...
-->
# JavaScript — الأخطاء الشائعة والأنماط المضادة
يقوم هذا المستند بفهرسة الأخطاء والفخاخ والأنماط المضادة الأكثر شيوعًا في JavaScript. يُظهر كل إدخال النهج غير الصحيح، ويشرح سبب فشله، ويقدم الحل الصحيح.
---
...
## 6. تعديل الكائنات أثناء التكرار
```javascript
...
## 7. النمط المضاد: رد الاتصال بالجحيم
```javascript
...
## 9. الكائنات المشابهة للمصفوفة ليست مصفوفات
```javascript
...
## 10. النمط المضاد: تغيير معلمات الوظيفة
```javascript
...
## 11. عدم فهم حلقة الأحداث والمهام الصغيرة
```javascript
...
## 14. النسخة الضحلة مقابل النسخة العميقة
```javascript
...
## ملخص
تأتي مرونة JavaScript مع مصائد: يعتمد ربط`this`على موقع الاتصال، وينفذ`==`الإكراه، وتتطلب الوعود معالجة الأخطاء، وترتيب حلقة الأحداث (المهام الصغيرة قبل المهام الكبيرة) غير واضح. القواعد الحديثة بسيطة: استخدم`const`/`let`أبدًا `var`، وفضل`===`على `==`، وتعامل دائمًا مع رفض الوعود، واستخدم وظائف الأسهم لعمليات الاسترجاعات، وتجنب التغيير، وتنظيف مستمعي الأحداث. يؤدي اتباع هذه القواعد إلى التخلص من غالبية أخطاء JavaScript.