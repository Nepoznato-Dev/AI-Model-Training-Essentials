...
| `+``-``*``/``^`| حسابية | `2^10`| |
| `%%`| معامل | `7 %% 3`| العوائد 1 |
| `%/%`| قسمة الأعداد الصحيحة | `7 %/% 3`| العوائد 2 |
| `==``!=` | المساواة | `x == y`| متجه |
| `<``>``<=``>=` | مقارنة | `x >= y`| متجه |
| `&``\|``!`| منطقي (متجه) | `x & y`| العنصر الحكيم |
| `&&``\|\|` | منطقي (عددي) | `x && y`| العنصر الأول فقط |
| `%in%`| العضوية | `x %in% y`| |
| `<-``=` | الواجب | `x <- 10`| `<-`اصطلاحي |
| `->`| تعيين الحق | `10 -> x`| نادرا ما يستخدم |
| `<<-`| تخصيص عالمي | `x <<- 10`| تعيين في البيئة الأم |
| `$`| استخراج العنصر | `df$column`| |
| `[``[[` | الفهرسة | `x[1]`/`x[[1]]`|  تقوم`[`بإرجاع نفس النوع؛  مقتطفات`[[`|
| `%>%`| الأنابيب (ماجريتر) | `x %>% f()`| |
| `\|>`| الأنابيب (قاعدة R 4.1+) | `x \|> f()`| الأنابيب الأصلية |
### العمليات الموجهة
```r
...
## ملخص
تم تصميم بناء جملة R للحوسبة الإحصائية وتحليل البيانات. تعمل الطبيعة الموجهة للعمليات على التخلص من الحلقات لمعظم المهام. يوفر النظام البيئي Tidyverse (dplyr، ggplot2، tidyr) قواعد نحوية متسقة وقابلة للقراءة لمعالجة البيانات وتصورها. إن إمكانات النمذجة الإحصائية لـ R - بدءًا من اختبارات t الأساسية وحتى نماذج التأثيرات المختلطة - لا مثيل لها في العمق. بالنسبة لعلوم البيانات والإحصائيات والأبحاث القابلة للتكرار، تظل لغة R أداة أساسية.