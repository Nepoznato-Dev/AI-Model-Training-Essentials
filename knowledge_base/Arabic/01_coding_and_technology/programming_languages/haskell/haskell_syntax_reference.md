...
|----------|------|--------|-------|
| `+``-``*``/` | حسابية | `x + y`| رقم النوع |
| `div``mod` | قسمة الأعداد الصحيحة | `7 \`div\` 2`| العوائد 3 |
| `^``**` | القوة | `2 ^ 10`| `^`للتكامل؛ `**`للعائمة |
| `==``/=` | المساواة | `x == y`| مكافئ typeclass |
| `<``>``<=``>=` | مقارنة | `x >= y`| نوع الطلب |
| `&&``\|\|``not`| منطقي | `a && b`| ماس كهربائى |
| `.`| تكوين الوظيفة | `(f . g) x`| `f (g x)`|
| `$`| تطبيق الدالة | `f $ x + 1`| `f (x + 1)`— أسبقية منخفضة |
| `<>`| إلحاق مونويد | `"hello" <> " world"`| نصف المجموعة |
| `<$>`| فاكتور فماب | `(+1) <$> Just 5`| `fmap (+1) (Just 5)`|
| `<*>`| تطبيقي | `pure (+) <*> Just 3`| |
| `>>=`| ربط موناد | `Just 5 >>= \x -> Just (x+1)`| |
| `>>`| تسلسل موناد | `putStrLn "hi" >> putStrLn "bye"`| |
| `=<<`| ربط (عكس) | `f =<< mx`| نفس`mx >>= f`|
| `++`| قائمة التسلسل | `[1,2] ++ [3,4]`| O(n) في القائمة اليسرى |
| `:`| سلبيات | `1 : [2, 3]`| يا(1) |
| `!!`| الفهرس | `[1,2,3] !! 1`| O(n) — دالة جزئية |
| `\\`| قائمة الفرق | `[1,2,3] \\ [2]`| `[1,3]`|
### أسبقية عامل التشغيل (محدد)
| الأسبقية | المشغلين |
|------------|-----------|
| 7 (الأعلى) | `*``/``div``mod` |
| 6 | `+``-` |
...
## ملخص
بناء جملة هاسكل بسيط ولكنه معبر بشكل غير عادي. يعمل نظام الكتابة - مع أنواع البيانات الجبرية، وفئات الكتابة، وGADTs، وعائلات الكتابة - كأداة للمواصفات والتحقق. توفر مطابقة الأنماط والتقييم البطيء والوحدات الأحادية حلولاً أنيقة للمشكلات المعقدة. إن نقاء هاسكل (لا توجد آثار جانبية في الأنواع) يجعل من السهل التفكير في التعليمات البرمجية واختبارها وموازنتها. في حين أن منحنى التعلم حاد، فإن تأثير هاسكل يتخلل اللغات الحديثة - من اختيارات Swift إلى فصول Kotlin المختومة إلى نظام الكتابة في Rust. إن إتقان هاسكل يجعلك مبرمجًا أفضل في كل اللغات.