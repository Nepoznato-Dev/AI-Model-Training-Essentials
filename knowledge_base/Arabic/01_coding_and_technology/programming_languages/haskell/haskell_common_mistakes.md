...
-->
# هاسكل — الأخطاء الشائعة والأنماط المضادة
يقوم هذا المستند بفهرسة الأخطاء والفخاخ والأنماط المضادة الأكثر شيوعًا في هاسكل مع التصحيحات.
---

## 1. قوائم لا نهائية ومفاجآت الكسل
```haskell
...
## 5. عدم استخدام صيغة السجل بشكل صحيح
```haskell
...
## 7. النمط المضاد: الإفراط في استخدام `IO`
```haskell
...
## ملخص
يؤدي نقاء هاسكل وكسله إلى خلق مخاطر فريدة من نوعها: تسرب المساحة من التقييم البطيء، والوظائف الجزئية التي تتعطل أثناء التشغيل، والارتباك بين السلسلة/النص/سلسلة البايت، والإفراط في استخدام الإدخال/الإخراج. طريقة هاسكل هي: استخدام التقييم الصارم للمراكم (`foldl'`, BangPatterns ) وتجنب الوظائف الجزئية (`head`,`read`,`!!`) واستخدام`Maybe`/`Either`لمعالجة الأخطاء، واستخدام`Text`للنص، والاحتفاظ بـ IO الحد الأدنى. نظام كتابة المترجم هو أعظم حليف لك - دعه يرشدك نحو وظائف كاملة ونقية.