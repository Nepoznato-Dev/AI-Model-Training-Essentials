...
|---------|------|-----------|
| تتمة | 1974 | لغة بحث IBM الأصلية (تشامبرلين وبويس) |
| SQL-86 | 1986 | **معيار ANSI الأول** (SQL-86) |
| SQL-89 | 1989 | مراجعة طفيفة (قيود التكامل) |
| SQL-92 | 1992 | **الرئيسية**: `JOIN`، الاستعلامات الفرعية، `CASE`،`COALESCE`|
| SQL: 1999 | 1999 | **التعبيرات العادية**، الاستعلامات العودية، المشغلات، كائنات تخزين البيانات الكبيرة (BLOBs) |
| سكل: 2003 | 2003 | **وظائف النافذة**، أعمدة XML،`GENERATED`|
| سكل: 2006 | 2006 | دعم XML،`MERGE`|
| سكل: 2008 | 2008 |  مشغلات `INSTEAD OF`، `TRUNCATE`،`ORDER BY`في طرق العرض |
| سكل: 2011 | 2011 | **البيانات الزمنية** (`AS OF`,`FOR SYSTEM_TIME`),`FETCH FIRST`|
| SQL: 2016 | 2016 | ** دعم JSON **، التعرف على أنماط الصف |
| SQL: 2019 | 2019 | **وظائف الجدول متعدد الأشكال**،`LISTAGG`|
| سكل: 2023 | 2023 | **`JSON_TABLE`**، عمليات `SET`، تحسينات المصفوفة |
## المعالم الرئيسية
### التكملة ولغة SQL المبكرة (1974-1986)
- **1974**: قام دونالد تشامبرلين وريموند بويس بإنشاء SEQUEL في أبحاث IBM
- **الهدف**: معالجة الاستعلام لنظام R (قاعدة بيانات علائقية)
- تمت إعادة تسميتها إلى SQL (لغة الاستعلام المنظمة) بسبب تعارض في العلامة التجارية
- **1986**: معيار ANSI الأول (SQL-86)
- **1987**: ISO يعتمد SQL-87
### SQL-92 — المؤسسة (1992)
- **المعيار الأكثر أهمية** — تنحدر جميع SQL الحديثة منه
- `INNER JOIN`، `LEFT JOIN`،`RIGHT JOIN`
- الاستعلامات الفرعية (`SELECT` المتداخلة)
- تعبير `CASE`
- `COALESCE`،`NULLIF`
- قيود `UNIQUE`، `CHECK`
- تعريف المخطط (`CREATE SCHEMA`)
### SQL:1999 — بداية SQL الحديثة (1999)
- التعبيرات العادية (`LIKE`,`SIMILAR TO`)
- الاستعلامات العودية (`WITH RECURSIVE`)
- المشغلات
- BLOB/CLOB (كائنات كبيرة ثنائية/شخصية)
- الأنواع المعرفة من قبل المستخدم (UDTs)
-`ORDER BY`في الاستعلامات الفرعية
### SQL:2003 — ثورة التحليلات (2003)
- **وظائف النافذة**:`ROW_NUMBER()`,`RANK()`,`DENSE_RANK()`,`LAG()`,`LEAD()`,`SUM() OVER()`
- نوع بيانات XML ووظائفها
-`GENERATED ALWAYS AS IDENTITY`
-`SAVEPOINT`(التحكم في المعاملات)
- وظائف التجزئة
### SQL:2011 — البيانات المؤقتة (2011)
- **الجداول الزمنية**:`FOR SYSTEM_TIME AS OF`,`VERSIONING`
-`FETCH FIRST n ROWS ONLY`(`LIMIT` القياسي)
- صفحات`OFFSET`/ `FETCH`
### SQL:2016–2023 — JSON وما بعده (2016 إلى الوقت الحاضر)
- **2016**: نوع بيانات JSON، `JSON_VALUE`، `JSON_QUERY`،`JSON_EXISTS`
- **2019**: وظائف الجدول متعدد الأشكال،`LISTAGG`
- **2023**:`JSON_TABLE`(العرض العلائقي لـ JSON)، وعمليات `SET`، وتحسينات المصفوفة
## تطور بناء الجملة
```sql
