...
|----------|------|--------|-------|
| `+``-``*``/``%`| حسابية | `a + b`| |
| `..``...` | النطاق | `1...10`| `...`مغلق؛ `..<`نصف مفتوح |
| `==``!=``<``>``<=``>=` | مقارنة | `a == b`| يتطلب`Equatable`/`Comparable`|
| `===``!==` | الهوية | `a === b`| نفس المرجع (الفصول فقط) |
| `&&``\|\|``!`| منطقي | `a && b`| ماس كهربائى |
| `??`| اندماج لا شيء | `a ?? b`| إرجاع`b`إذا كانت قيمة`a`صفراً |
| `?.`| تسلسل اختياري | `a?.b`| يُرجع nil إذا كانت قيمة`a`صفراً |
| `!`| قوة بسط | `a!`| الأعطال إذا لم يكن هناك شيء - تجنب |
| `&`| المعلمة الداخلية | `swap(&a, &b)`| تمر بالرجوع |
### لا شيء مشغلي المناولة
```swift
...
## ملخص
يوازن بناء جملة Swift بين الأمان والتعبير. تعمل الاختيارات على التخلص من الأعطال الفارغة، وتتيح البروتوكولات التركيب المرن دون الوراثة، كما أن التزامن الحديث (غير المتزامن/الانتظار، الجهات الفاعلة) يجعل البرمجة المتزامنة الآمنة قابلة للوصول. تستمر اللغة في التطور مع ميزات مثل حزم المعلمات ووحدات الماكرو والملكية - كل إضافة تجعل Swift أكثر أمانًا وقوة مع الحفاظ على وعدها الأساسي: كود آمن من خلال البناء.