...
|----------|------|--------|-------|
| `+``-``*``/``%`| حسابية | `a + b`| |
| `..`| النطاق | `1..10`| مجموعة شاملة |
| `until`| نطاق نصف مفتوح | `1 until 10`| `1..9`|
| `downTo`| النطاق التنازلي | `10 downTo 1`| |
| `==``!=` | المساواة الهيكلية | `a == b`| يدعو`.equals()`|
| `===``!==` | المساواة المرجعية | `a === b`| نفس المثال |
| `<``>``<=``>=` | مقارنة | `a >= b`| يتطلب`Comparable`|
| `&&``\|\|``!`| منطقي | `a && b`| ماس كهربائى |
| `?:`| مشغل الفيس | `a ?: b`| إرجاع`b`إذا كانت قيمة`a`فارغة |
| `?.`| مكالمة آمنة | `a?.b`| إرجاع قيمة فارغة إذا كانت`a`فارغة |
| `!!`| تأكيد غير فارغ | `a!!`| يرمي NPE إذا كان فارغًا |
| `as`| يلقي | `a as String`| يلقي ClassCastException |
| `as?`| صب آمن | `a as? String`| إرجاع فارغة عند الفشل |
| `in`| العضوية | `x in list`| |
| `!in`| ليس عضوا | `x !in list`| |
| `is`| اكتب الاختيار | `x is String`| طاقم ذكي بعد الفحص |
| `!is`| لا اكتب | `x !is String`| |
...
## ملخص
بناء جملة Kotlin موجز وآمن وقابل للتشغيل المتبادل مع Java. تعمل السلامة الخالية على إزالة فئة كاملة من الأخطاء في وقت الترجمة. توفر Coroutines طريقة طبيعية لكتابة تعليمات برمجية غير متزامنة. تتيح الفئات المختومة وفئات البيانات ووظائف الامتداد إمكانية نمذجة المجال التعبيري. تأتي قوة اللغة من التفاعل بين ميزاتها - تجتمع وظائف النطاق، وDSLs، والفئات المضمنة، والأدوية العامة المُحددة لإنتاج تعليمات برمجية آمنة وممتعة في الكتابة.