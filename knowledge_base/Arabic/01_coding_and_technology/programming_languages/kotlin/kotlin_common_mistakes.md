...
-->
# Kotlin - الأخطاء الشائعة والأنماط المضادة
يقوم هذا المستند بفهرسة الأخطاء والفخاخ والأنماط المضادة الأكثر شيوعًا في Kotlin. يُظهر كل إدخال النهج غير الصحيح، ويشرح سبب فشله، ويقدم الحل الصحيح.
---

## 1. أنواع المنصات و NullPointerException
```kotlin
...
## 3.`data class` بدون`equals`/ `hashCode`
```kotlin
...
## 4. عدم استخدام وظائف النطاق بشكل صحيح
```kotlin
...
## 5. Coroutines: عدم التعامل مع الإلغاء
```kotlin
...
## 6. مكافحة النمط: Java-Style Kotlin
```kotlin
...
## 9. عدم استخدام`by lazy`للتهيئة الباهظة الثمن
```kotlin
...
## 10. ارتباك التدفق مقابل البيانات الحية
```kotlin
...
## ملخص
يمكن أن يؤدي إيجاز Kotlin إلى إخفاء المشكلات الدقيقة: أنواع الأنظمة الأساسية من التشغيل المتداخل لـ Java، وتعطل `lateinit`، وفئات البيانات القابلة للتغيير التي تكسر المجموعات، وإلغاء coroutine. طريقة Kotlin هي: احتضان السلامة الخالية (لا تستخدم`!!`أبدًا)، واستخدام وظيفة النطاق الصحيحة (`let` للتحويلات، و`apply` للتكوين، و`also` للتأثيرات الجانبية)، واستخدام الفئات المختومة مع`when`الشامل، والتعامل مع إلغاء coroutine، وكتابة Kotlin الاصطلاحية (وليس Java مع بناء جملة Kotlin). المترجم هو دليلك، إذا كان يحذرك، استمع.