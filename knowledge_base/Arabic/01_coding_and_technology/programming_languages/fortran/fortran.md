...
## لماذا يهم فورتران
- **أداء الحوسبة عالية الأداء**: يقوم مترجمو Fortran بإنتاج بعض من أسرع الأكواد الرقمية المتوفرة — غالبًا ما تتطابق مع C/C++ أو تتجاوزها لعمليات المصفوفة.
- **قواعد الأكواد القديمة**: تمت كتابة عقود من الأكواد العلمية (نماذج المناخ والمحاكاة الفيزيائية) بلغة فورتران.
- **عمليات المصفوفة**: دعم أصلي للمصفوفة متعددة الأبعاد مع تركيب مصمم للحسابات الرياضية.
- **الاستقرار العددي**: تم تحسين اللغة والمترجمات لحساب الفاصلة العائمة.
- **Coarrays**: دعم البرمجة المتوازية المدمج (Fortran 2008+).
- **لا يزال يتطور**: يضيف Fortran 2023 ميزات حديثة مع الحفاظ على التوافق مع الإصدارات السابقة.
##المقايضات
| الحد | التفاصيل | الحل النموذجي |
|-----------|------------------------|---|
| ** المجتمع المتخصص ** | صغيرة ومتخصصة - معظمها علمية/HPC | مجتمع نشط في العلوم الحسابية |
| **نظام بيئي محدود** | مكتبات أقل من Python أو Java أو C++ | استخدم BLAS/LAPACK للعمل العددي؛ قابلية التشغيل البيني C للاحتياجات الأخرى |
| **ليس للأغراض العامة** | ضعيف بالنسبة للويب أو الهاتف المحمول أو واجهة المستخدم الرسومية أو برمجة الأنظمة | استخدام فورتران للحساب. التفاف في Python/C للتطبيقات |
| **الإدراك** | غالبًا ما يُنظر إليها على أنها "عفا عليها الزمن" على الرغم من الإمكانيات الحديثة | التركيز على نقاط قوتها: العددية والحوسبة عالية الأداء |
| **التوظيف** | عدد قليل من مطوري فورتران يدخلون السوق | الخبراء الحاليون في الأوساط الأكاديمية والمختبرات الوطنية |
---

## أساسيات بناء الجملة (فورتران الحديثة)
```fortran
program hello
    implicit none
...
### الأنواع المشتقة وإجراءات ربط النوع
```fortran
...
### الإجراءات العامة والحمولة الزائدة على المشغل
```fortran
module vector_ops
    implicit none
...
## التزامن والتوازي
### المصفوفات (Fortran 2008 التوازي المدمج)
```fortran
...
## الاختبار
### إطار الاختبار (اختبار فورتران أو pFUnit)
```fortran
...
### إمكانية التشغيل البيني C (ISO_C_BINDING)
```fortran
module c_interface
...
### تكامل Python (عبر f2py أو المكتبة المشتركة)
```fortran
...
## أنماط التصميم
### النموذج 1: برمجة المصفوفات (تجنب الحلقات)
```fortran
...
### النموذج 2: الأنواع المشتقة ذات المعلمات
```fortran
...
### النموذج 3: نمط رد الاتصال مع مؤشرات الإجراء
```fortran
module integrator
    implicit none
    abstract interface
...
## ملخص
فورتران هي لغة البرمجة العلمية الأصلية وتظل قوة في مجال الحوسبة عالية الأداء. تعد لغة Fortran الحديثة لغة قادرة ومتطورة مع عمليات صفيف أصلية، ودعم البرمجة المتوازية، وقابلية التشغيل البيني بلغة C. على الرغم من أن مجتمعها صغير ومتخصص، إلا أن Fortran تواصل تشغيل بعض أحمال العمل الحسابية الأكثر تطلبًا في العالم. بالنسبة للحوسبة الرقمية على نطاق واسع، تظل فورتران ذات صلة.