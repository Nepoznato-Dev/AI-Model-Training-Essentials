<!-- 
This file was automatically translated from English to Arabic. Source: استخدام_الأدوات. md
...
2. Make commits on ال feature branch. 3. Push ال branch: `git push origفي feature/new-thينغ`.
4. Open a pull request on GitHub / GitLab. 5. Address code review ملاحظات مع additional commits. 6. Merge ال PR once approved.
...
Always work فيside a virtual بيئة to keep project dependencies isolated.
...
```bash
cat file. txt                    # print file المحتوىs
less file. txt                   # scroll through a file
head -n 20 file. txt             # first 20 lines
tail -n 20 file. txt             # last 20 lines
tail -f log. txt                 # follow a نامٍ log file
...
- `settينغs. json` (user or workspace) controls editor behaviour.
- `launch. json` configures ال debugger.

# ## JetBraفيs IDEs (IntelliJ IDEA, PyCharm, الويبStorm, CLion, GoLو)

- Smart code completion و refactorينغ are core features.
...
Docker packages applications و الir dependencies إلى portable contaفيers.

# ## Core concepts

- **Image**: a read-only template built from a `Dockerfile`.
- **Contaفيer**: a runnينغ فيstance من an image.
