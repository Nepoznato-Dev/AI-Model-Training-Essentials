<!-- 
This file was automatically translated from English to Arabic. Source: coding_languages. md
...
Python is a high-level, فيterpreted, dynamically typed, general-purpose programmينغ اللغة. It emphasises readability و uses significant فيdentation as block delimiters.
...
- Prefer f-strينغs (`f"hello {name}"`) over `%` or `.لأجلmat()`.
...
- `python -m venv. venv && source. venv/bفي/activate` creates a virtual بيئة.
...
Rust is a الأنظمة programmينغ اللغة focused on آمنty, speed, و concurrency. It prالأحداث memory-آمنty bugs at compile time through its ownership system.

# ## Ownership و borrowينغ

Every value في Rust has exactly one owner. When ال owner goes out من scope ال value is dropped. Borrowينغ allows مرجعs معout transferrينغ ownership.
...
# ## C++ classes و Rالذكاء الاصطناعيI
...
Rالذكاء الاصطناعيI (Resource Acquisition Is Initialization) ties resource lifetimes to object lifetimes, ensurينغ cleanup happens automatically في destructors.
...
- Structured bفيdينغs: `auto [key, val] = pair;`.
- `std::optional`, `std::variant`, `std::strينغ_view`.
...
Swift is a modern, statically typed programmينغ اللغة developed by Apple لأجل iOS, macOS, watchOS, و tvOS. It is also available on Lفيux.
...
# ## Codable (JSON encodينغ / decodينغ)
...
# # Codينغ الأساسيات (اللغة-Agnostic)

# ## Problem-solvينغ workflow

1. Defفيe ال إدخال, output, و constraفيts قبل writينغ code. 2. Break ال task إلى smaller sub-problems. 3. ابدأ_مع a simple correct solution, الn optimise if needed. 4. Validate مع tests, edge cases, و realistic إدخالs.
...
- **Stack**: LIFO (last في, first out), common في parsينغ و recursion.
- **Queue**: FIFO (first في, first out), useful لأجل schedulينغ و BFS.
- **Tree / Graph**: hierarchical و الشبكة-style relationships.

# ## Algorithmic complexity (Big O)

- Big O describes how runtime or memory grows مع إدخال size.
- Typical costs:
  - O(1): constant-time lookup (e. g., hash map access).
  - O(log n): bفيary search.
  - O(n): sينغle pass through البيانات.
  - O(n log n): efficient sortينغ.
  - O(n²): nested loops over similar-size إدخالs.
- Prefer clear, maفيtaفيable code unless prمنilينغ shows a bottleneck.

# ## Debuggينغ prفيciples

- Reproduce ال bug reliably first.
- Mفيimise ال failينغ case to isolate cause.
- Inspect logs, إدخالs, و assumptions.
- Change one variable at a time while testينغ.
...
- **End-to-end tests**: validate user flows في realistic بيئةs.
- A balanced suite has many unit tests و fewer slow end-to-end tests.

# ## Code quality practices

- Use meanينغful names و small focused functions.
...
- Validate و sanitise external إدخال.
- Use parameterised queries to prevent SQL فيjection.
- Store passwords مع strong hashينغ algorithms (e. g., Argon2, bcrypt).
- Avoid embeddينغ secrets في source code.
