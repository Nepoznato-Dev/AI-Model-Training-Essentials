...
---
# الدليل المرجعي السريع لـ SQL
أوامر SQL الأساسية لعمليات قاعدة البيانات.
---
...
## الانضمامات
### الانضمام الداخلي```sql
SELECT u.name, o.order_date, o.total
FROM users u
INNER JOIN orders o ON u.id = o.user_id;
```

### انضمام يسار/يمين```sql
...
-`DECIMAL(p,s)`- العلامة العشرية الدقيقة (الدقة، المقياس)
- `FLOAT` - نقطة عائمة تقريبية
- `DOUBLE` - تعويم مزدوج الدقة
### سلسلة
- `CHAR(n)` - سلسلة ذات طول ثابت
-`VARCHAR(n)`- سلسلة ذات طول متغير
- `TEXT` - نص كبير
- `ENUM` - القيم المذكورة
### التاريخ/الوقت
- `DATE` - التاريخ (YYYY-MM-DD)
- `TIME` - الوقت (ساعة:دقيقة:ثواني ثانية)
- `DATETIME` - التاريخ والوقت
- `TIMESTAMP` - الطابع الزمني لنظام التشغيل Unix
- `YEAR` - قيمة السنة
### منطقية
- `BOOLEAN` أو `BOOL` - صواب/خطأ
### ثنائي
- `BLOB` - كائن ثنائي كبير
- `BINARY` - ثنائي ثابت
- `VARBINARY` - ثنائي متغير
---
...
## نصائح الأداء
✅ **افعل:**
- استخدم الفهارس في الأعمدة التي يتم الاستعلام عنها بشكل متكرر
- حدد الأعمدة المطلوبة فقط (تجنب`SELECT *`)
- استخدم`EXPLAIN`لتحليل أداء الاستعلام
- تطبيع البيانات بشكل مناسب
- استخدم البيانات المعدة لمنع حقن SQL
❌ **لا تفعل:**
- استخدم الوظائف على الأعمدة المفهرسة في عبارات WHERE
- إنشاء عدد كبير جدًا من الفهارس (يبطئ الكتابة)
- استخدم`SELECT DISTINCT`دون داع
- تجاهل خطط تنفيذ الاستعلام
- تخزين القيم المحسوبة عندما يمكن حسابها
---
...
*آخر تحديث: يوليو 2026 | معيار SQL (متوافق مع MySQL/PostgreSQL)*