...
---
# Bash ve Shell Komut Dosyası Yazma Hile Sayfası
Çoğu Linux ve macOS sisteminde varsayılan kabuk olan Bash'te kabuk komut dosyaları yazmaya yönelik pratik bir referans. Söz dizimini, ortak kalıpları, metin işlemeyi ve kullanışlı tek satırlık satırları kapsar.
---

## Komut Dosyası Yapısı
Her Bash betiği bir **shebang** satırıyla başlar:
```bash
#!/usr/bin/env bash
...
| Bayrak | Efekt |
|------|--------|
| `set -e`| Bir komut başarısız olursa hemen çıkın |
| `set -u`| Ayarlanmamış değişkenleri hata olarak ele alın |
| `set -o pipefail`| Herhangi bir komut başarısız olursa işlem hattı başarısız olur |
| `set -x`| Çalıştırmadan önce her komutu yazdırın (hata ayıklama modu) |
Bir komut dosyası çalıştırın:`chmod +x script.sh && ./script.sh`veya `bash script.sh`
---
...
| `$#`| Konumsal bağımsız değişkenlerin sayısı |
| `$@`| Tüm konumsal argümanlar (ayrı kelimeler olarak) |
| `$*`| Tüm konumsal bağımsız değişkenler (tek bir dize olarak) |
| `$?`| Son komutun çıkış durumu (0 = başarılı) |
| `$$`| Mevcut sürecin PID'si |
| `$!`| Son arka plan işleminin PID'si |
| `$_`| Önceki komutun son argümanı |
---
...
| Testi | Anlamı |
|------|------------|
| `[[ -f "$x" ]]`| Dosya mevcut ve normal bir dosya |
| `[[ -d "$x" ]]`| Dizin mevcut |
| `[[ -e "$x" ]]`| Dosya/dizin mevcut (herhangi bir şey) |
| `[[ -r "$x" ]]`| Okunabilir |
| `[[ -w "$x" ]]`| Yazılabilir |
| `[[ -x "$x" ]]`| Yürütülebilir |
| `[[ -z "$x" ]]`| Dize boş |
| `[[ -n "$x" ]]`| Dize boş değil |
| `[[ "$a" == "$b" ]]`| Dize eşitliği |
| `[[ "$a" != "$b" ]]`| Dize eşitsizliği |
| `[[ "$a" =~ regex ]]`| Normal ifade eşleşmesi |
| `[[ $a -eq $b ]]`| Tam sayı eşitliği |
| `[[ $a -ne $b ]]`| Tamsayı eşitsizliği |
| `[[ $a -gt $b ]]`| Şundan büyük: |
| `[[ $a -lt $b ]]`| |'dan az |
| `[[ $a -ge $b ]]`| Büyük veya eşittir |
| `[[ $a -le $b ]]`| Küçük veya eşit |
...
| Sözdizimi | Anlamı |
|----------|------------|
| `cmd > file`| Stdout'u dosyaya yönlendir (üzerine yaz) |
| `cmd >> file`| Stdout'u dosyaya yönlendir (ekle) |
| `cmd 2> errors.log`| stderr'i yönlendir |
| `cmd &> all.log`| Hem stdout'u hem de stderr'i yönlendirin |
| `cmd1 \| cmd2`| Cmd1'in stdout'unu cmd2'nin stdin'ine yönlendirin |
| `cmd1 \|& cmd2`| Hem stdout'u hem de stderr'i borulayın |
| `cmd < file`| Dosyayı stdin'e yönlendir |
| `cmd <<EOF ... EOF`| Here-belge (çok satırlı giriş) |
| `cmd <<< "string"`| Here-string (tek satırlı giriş) |
```bash
# Here-document
cat <<EOF
Name: $NAME
...
## Komut Dosyası Desenleri
### Bağımsız Değişken Ayrıştırma
```bash
#!/usr/bin/env bash
set -euo pipefail
...
## Özet
Bash komut dosyası oluşturma, bilgisayarlarla çalışan herkesin verimliliğini artırır. Her bayrağı ezberlemek gerekli değildir; neyin mümkün olduğunu ve belgelerin nerede bulunacağını bilmek yeterlidir. Temel bilgilerle başlayın: değişkenler, koşullar, döngüler, çizgiler. Daha sonra gerektiği gibi metin işleme araçlarını (grep, sed, awk) ekleyin. Sağlam komut dosyaları yazmak için`set -euo pipefail`kullanılması önerilir.