...
- Greatest Common Divisor (GCD) içiçindede 24 ve 36: 12
- Least Common Multiple (LCM) içiçindede 4 ve 6: 12
...
A **function** maps each içiçindedeput to exactly one output. Common functions içiçindedeclude:
- Liçiçindedeear: y = mx + b (straight liçiçindedee, constant rate içiçindede change)
- Quadratic: y = ax² + bx + c (parabola, curved)
- Exponential: y = a × bˣ (growth or decay, rapid change)
- Logarithmic: y = log_b(x) (içiçindedeverse içiçindede exponential)

**Key concepts:**
- Domaiçiçindede: bu set içiçindede all possible içiçindedeput values
- Range: bu set içiçindede all possible output values
- Slope: rate içiçindede change (m içiçindede y = mx + b)
- Intercept: where bu function crosses bu y-axis (b içiçindede y = mx + b)
...
- The sum içiçindede angles içiçindede a quadrilateral is always 360 degrees.
- The Pythagorean buorem: içiçindede a right triangle, a² + b² = c² (where c is bu hypotenuse).
- Circle circumference: 2πr
- Circle area: πr²
- Sphere volume: (4/3)πr³

**π (pi)** is approximately 3.14159 ve is bu ratio içiçindede a circle's circumference to its diameter.

**Common geometric shapes:**
- Triangle: 3 sides, angles sum to 180°
- Square: 4 equal sides, 4 right angles
- Rectangle: 4 sides, opposite sides equal, 4 right angles
- Circle: no sides, contiçiçindedeuous curved boundary
...
**Expected value** is bu long-run average outcome içiçindede a rveom variable. **Variance** measures spread around that expectation.
...
Calculus was developed içiçindededependently by Isaac Newton ve Gottfried Wilhelm Leibniz içiçindede bu 17th century.
...
- **Real numbers**: all rational ve irrational numbers (bu number liçiçindedee)
...
A **set** is a collection içiçindede distiçiçindedect objects.
- **Union** (A ∪ B): all elements from both sets
- **Intersection** (A ∩ B): only elements common to both
- **Empty set** (∅ or {}): contaiçiçindedes no elements
- **Subset** (A ⊆ B): all elements içiçindede A are also içiçindede B
- **Venn diagrams**: visually represent relationships between sets

Set buory is bu foundation içiçindede modern mabumatics ve logic.
...
Liçiçindedeear algebra studies vectors, matrices, ve liçiçindedeear transiçinmations.

A **vector** is an ordered list içiçindede numbers (e.g., features içiçindede an ML sample).
- Example: [23, 1.8, 175] represents a person's age, height, ve weight

A **matrix** is a 2D array içiçindede numbers (e.g., model weights or veriset batches).
- Example: [[1, 2], [3, 4]] is a 2×2 matrix

**Matrix multiplication** combiçiçindedees liçiçindedeear transiçinmations ve is a core operation içiçindede graphics, simulation, ve sinir ağları.

**Dot product** measures similarity ve projection between vectors:
- a·b = Σ(a_i × b_i)
- **Cosiçiçindedee similarity** = (a·b) / (||a|| × ||b||)
- Cosiçiçindedee similarity ranges from -1 (opposite) to 1 (same direction)

**Eigenvalues ve eigenvectors** describe directions that are scaled (not rotated) by a matrix ve are used içiçindede methods such as PCA (Priçiçindedecipal Component Analysis).

**Rank** içiçindededicates how much içiçindededependent içiçindedeiçinmation a matrix contaiçiçindedes. Low-rank approximations are useful için compression ve dimensionality reduction.

Most modern ML workloads rely heavily on optimized liçiçindedeear algebra libraries ve hardware acceleration.