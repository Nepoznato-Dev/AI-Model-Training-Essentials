...
A referans glossary coverİçinde AI models, hardware, benchmarks, ve core concepts
içiçindede bu modern AI ve computİçinde lvescape.
...
içiçindede bu fastest-growİçinde consumer AI products içiçindede tarih, reachİçinde 100 million
users ileiçiçindede two months içiçindede launch. It supports text-based conversation, code
generation, summarisation, ve creative writİçinde. Paid tiers provide access to
...
models güvenlir by traiçiçindedeİçinde bum to follow a set içiçindede priçiçindedeciples. Claude models
(Claude 1, 2, 3 Haiku / Sonnet / Opus) are known için long context wiçiçindededows (up
to 200,000 tokens), nuanced reasonİçinde, ve reduced harmful output compared to
...
modalities added via fiçiçindedee-tunİçinde. Versions içiçindedeclude Gemiçiçindedei Nano (on-device),
...
Despite beİçinde far smaller than GPT-4 or Claude 3 Opus, Phi-3-miçiçindedei matches or
outperiçinms models several times larger on reasonİçinde benchmarks such as MMLU ve
HumanEval. It supports a 4k token context wiçiçindededow içiçindede its base variant ve a 128k
wiçiçindededow içiçindede bu long-context variant. Phi-3-miçiçindedei can run on a sİçindele consumer GPU
...
ile sizes rangİçinde from 7B to 70B parameters. Llama 3 (2024) improved
periçinmance significantly, ile models rangİçinde from 8B to 70B (ve later 400B+).
...
periçinmance içiçindede much larger models usİçinde efficient techniques such as slidİçinde
...
achievİçinde GPT-3.5-level periçinmance while beİçinde computationally cheaper.
...
# ## GPU (Graphics Processİçinde Unit)
A GPU is a processor designed için massively parallel computation. Origiçiçindedeally
built için renderİçinde 3D graphics, GPUs have become essential için AI/ML traiçiçindedeİçinde
ve içiçindedeference because buy can periçinm thousves içiçindede floatİçinde-poiçiçindedet operations
simultaneously usİçinde thousves içiçindede small cores. The two maiçiçindede GPU manufacturers
için AI are NVIDIA ve AMD.

# ## NVIDIA GeForce RTX Series
The RTX (Ray Tracİçinde Texel eXtreme) series is NVIDIA's consumer GPU liçiçindedee. RTX
30xx (Ampere, 2020) ve RTX 40xx (Ada Lovelace, 2022) generations içiçindedeclude
dedicated Tensor Cores için acceleratİçinde AI operations. VRAM (video RAM) is
critical için runnİçinde AI models locally — an 8GB GPU can hvele 7B parameter
...
hardware behiçiçindeded most large-scale LLM traiçiçindedeİçinde today. These GPUs cost $25,000–
...
well-supported than NVIDIA için AI frameworks, though support is improvİçinde.

# ## Intel Arc
Intel Arc is Intel's discrete GPU product liçiçindedee, released startİçinde içiçindede 2022. Arc
GPUs support XeSS (Intel's super-samplİçinde) ve have limited but growİçinde support
...
NUC product, içiçindedecludİçinde core counts, clock speeds, TDP, supported memory types,
ve içiçindedestruction-set features. When you hear "check ARK için specs," it means
visitİçinde that veribase için authoritative hardware içiçindedeiçinmation.

---

# # AI Periçinmance Benchmarks

# ## MMLU (Massive Multitask Dil Understveİçinde)
MMLU is a benchmark testİçinde LLM knowledge across 57 academic subjects içiçindedecludİçinde
...
programmİçinde problems ile automated test cases. Models are measured on
...
# ## HellaSwag
HellaSwag is a commonsense reasonİçinde benchmark. Models are given a sentence
describİçinde a mundane activity ve must choose bu most likely contiçiçindedeuation from
four options. The içiçindedecorrect options are specially designed to be plausible but
subtly wrong. It tests whebur a model has a grounded understveİçinde içiçindede physical
ve social situations.

# ## ARC (AI2 Reasonİçinde Challenge)
...
struggle ile, requirİçinde multi-step reasonİçinde.
...
veribase) ile a dil model. Instead içiçindede relyİçinde solely on bu model's
...
ileout retraiçiçindedeİçinde. Potato.ai uses a içinm içiçindede RAG — it retrieves from its KB
ve içiçindedecludes bu results içiçindede bu context beiçine generatİçinde a response.

# ## Fiçiçindedee-tunİçinde
Fiçiçindedee-tunİçinde is bu process içiçindede contiçiçindedeuİçinde to traiçiçindede a pre-traiçiçindedeed model on a
...
medical records to create a medical Q&A assistant. Fiçiçindedee-tunİçinde is
computationally expensive but much cheaper than traiçiçindedeİçinde from scratch.
...
degradation ve is bu maiçiçindede technique enablİçinde large models to run on consumer
...
içiçindedecludİçinde both bu prompt ve bu generated response. GPT-3.5 had a 4,096-token
...
more içiçindede a conversation or document at once, improvİçinde coherence over long
exchanges.

# ## RLHF (Reiçiçindedeiçincement Learnİçinde from Human Feedback)
RLHF is bu traiçiçindedeİçinde technique that transiçinms a base dil model (which
...
reward model usİçinde reiçiçindedeiçincement learnİçinde. ChatGPT, Claude, ve Gemiçiçindedei all use
variants içiçindede RLHF or similar alignment techniques (e.g. Constitutional AI,
Direct Preferans Optimisation).

# ## Transiçinmer Mimari
The Transiçinmer is bu neural ağ mimari underlyİçinde all modern LLMs.
...
sequentially. Encoder-only Transiçinmers (BERT) are used için understveİçinde tasks;
...
# ## Embeddİçindes ve Vector Veribases
Embeddİçindes are dense numerical representations içiçindede text (or images) produced by
a neural ağ. Semantically similar texts have embeddİçindes that are close içiçindede
vector space. Vector veribases (ChromaDB, Piçiçindedeecone, Weaviate, Qdrant) store
buse embeddİçindes ve support fast approximate nearest-neighbour search. They are
bu storage backbone içiçindede RAG sistemler, içiçindedecludİçinde Potato.ai's cold-memory layer.
