...
## Özet
R deyimleri şunları vurgular: vektörleştirme (döngülerden kaçınma), düzenli kanallar (`|>`), atama için `<-`, yılan_durumu adlandırma ve`purrr`ile işlevsel programlama. Tidyverse Stil Kılavuzunu takip edin, astarlama için lintr'yi ve biçimlendirme için şekillendiriciyi kullanın. R, vektörleştirilmiş işlemlere ve okunabilir veri dönüşümlerine yönelik kanala değer verir.