...
| `+``-``*``/``^`| Aritmetik | `2^10`| |
| `%%`| Modül | `7 %% 3`| İade 1 |
| `%/%`| Tamsayı bölümü | `7 %/% 3`| İade 2 |
| `==``!=` | Eşitlik | `x == y`| Vektörleştirilmiş |
| `<``>``<=``>=` | Karşılaştırma | `x >= y`| Vektörleştirilmiş |
| `&``\|``!`| Mantıksal (vektörleştirilmiş) | `x & y`| Öğe bazında |
| `&&``\|\|` | Mantıksal (skaler) | `x && y`| Yalnızca ilk öğe |
| `%in%`| Üyelik | `x %in% y`| |
| `<-``=` | Ödev | `x <- 10`| `<-`deyimseldir |
| `->`| Sağ atama | `10 -> x`| Nadiren kullanılır |
| `<<-`| Genel atama | `x <<- 10`| Ebeveyn ortamına ata |
| `$`| Öğeyi çıkar | `df$column`| |
| `[``[[` | İndeksleme | `x[1]`/`x[[1]]`| `[`aynı türü döndürür; `[[`özleri |
| `%>%`| Boru (magrittr) | `x %>% f()`| |
| `\|>`| Boru (taban R 4.1+) | `x \|> f()`| Yerli boru |
...
## Özet
R'nin sözdizimi istatistiksel hesaplama ve veri analizi için tasarlanmıştır. Operasyonların vektörleştirilmiş doğası çoğu görev için döngüleri ortadan kaldırır. Düzenli ekosistem (dplyr, ggplot2, tidyr), veri işleme ve görselleştirme için tutarlı, okunabilir bir dilbilgisi sağlar. R'nin temel t testlerinden karma etki modellerine kadar istatistiksel modelleme yetenekleri derinlemesine eşsizdir. Veri bilimi, istatistik ve tekrarlanabilir araştırmalar için R önemli bir araç olmaya devam ediyor.