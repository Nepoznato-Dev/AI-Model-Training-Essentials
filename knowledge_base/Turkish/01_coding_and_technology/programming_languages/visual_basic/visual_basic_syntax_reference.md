...
| | `-`| Çıkarma | `a - b`|
| | `*`| Çarpma | `a * b`|
| | `/`| Bölme (Çift değerini döndürür) | `a / b`|
| | `\`| Tamsayı bölümü | `a \ b`|
| | `Mod`| Modül | `a Mod b`|
| | `^`| Üs | `a ^ b`|
...
| | `>=`| Büyük veya eşit | `a >= b`|
| **Mantıksal** | `AndAlso`| Kısa devre VE | `a AndAlso b`|
| | `OrElse`| Kısa devre VEYA | `a OrElse b`|
| | `Not`| Mantıksal DEĞİL | `Not a`|
| | `And`| Bitsel VE | `a And b`|
| | `Or`| Bitsel VEYA | `a Or b`|
| | `Xor`| Bitsel XOR | `a Xor b`|
| **Dize** | `&`| birleştirme | `"Hello" & " " & "World"`|
| | `+=`| Ekle | `s &= " more"`|
| **Ödev** | `=`| Ata | `x = 42`|
| **Boş** | `Is`| Referans eşitliği | `obj Is Nothing`|
| | `IsNot`| Referans eşitsizliği | `obj IsNot Nothing`|
| **Tür** | `TryCast`| Güvenli yayın (Hiçbir şey döndürmez) | `TryCast(obj, String)`|
| | `CType`| Dönüştür | `CType(obj, Integer)`|
| | `DirectCast`| Doğrudan atış (başarısızlık durumunda atış) | `DirectCast(obj, String)`|
---
...
## Özet
Visual Basic'in sözdizimi temiz ve İngilizceye benzer, bu da onu en okunabilir programlama dillerinden biri yapar. VB.NET, veri sorgulama için LINQ, eşzamanlı işlemler için eşzamansız/beklemede, tür güvenliği için jenerikler ve sınıflar, arayüzler ve mirasla kapsamlı OOP dahil olmak üzere .NET ekosistemine tam erişim sağlar. Dilin büyük/küçük harfe duyarsızlığı ve ayrıntılı anahtar sözcükleri (End If, End Sub, End Class) okunabilirliği artırır. Kurumsal Windows uygulamaları, Office otomasyonu (VBA) ve .NET içerisinde hızlı prototip oluşturma için VB.NET üretken ve yetenekli bir dil olmaya devam ediyor.