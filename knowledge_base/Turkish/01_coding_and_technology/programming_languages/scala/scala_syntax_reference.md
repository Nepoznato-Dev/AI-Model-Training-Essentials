...
|----------|------|-----------|-------|
| `+``-``*``/``%`| Aritmetik | `a + b`| |
| `**`| Güç (BigDecimal) | `BigDecimal(2) ** 10`| Int için yerleşik değil |
| `==``!=` | Eşitlik | `a == b`|`.equals()`çağrıları — null güvenli |
| `eq``ne` | Referans eşitliği | `a eq b`| Aynı örnek |
| `<``>``<=``>=` | Karşılaştırma | `a >= b`|`Ordered`gerektirir |
| `&&``\|\|``!`| mantıksal | `a && b`| Kısa devre |
| `&``\|``^``~` | Bitsel | `a & b`| |
| `<<``>>``>>>`| Shift | `a << 2`| |
| `+:``:+``::``:::` | Toplama operasyonları | `1 +: list`|`:`ile bitiyorsa sağ ilişkisel |
| `->`| Tuple oluşturma | `"key" -> "value"`| `("key", "value")`|
| `=>`| İşlev/ok | `x => x + 1`| Lambda veya tür eşleme |
| `<-`| Jeneratör | `x <- List(1,2,3)`| Anlamak için |
### Operatör Kuralları
```scala
...
## Özet
Scala'nın sözdizimi, nesne yönelimli ve işlevsel programlamayı güçlü bir tür sistemi altında birleştirir. Örüntü eşleştirme, kavramaya yönelik ve vaka sınıfları anlamlı veri modelleme sağlar. Özellikler, tekli kalıtım sınırlamaları olmaksızın esnek kompozisyona olanak tanır. Jenerikler, varyans, tür sınırları ve opak türler içeren tür sistemi, sıfır maliyetli soyutlamalara olanak tanırken derleme zamanında hataları yakalar. Scala 3, numaralandırmalar, uzatma yöntemleri, verilen/kullanılan ve opak türlerle dili basitleştirerek dili tam gücünü korurken daha ulaşılabilir hale getirir.