...
---
# JavaScript — Yaygın Hatalar ve Anti-Kalıplar
Bu belge, JavaScript'teki en yaygın hataları, tuzakları ve anti-kalıpları kataloglamaktadır. Her giriş yanlış yaklaşımı gösterir, neden başarısız olduğunu açıklar ve doğru çözümü sağlar.
---

## 1.`this`Bağlayıcı Karışıklık
```javascript
...
## 3.`var`Kaldırma ve Kapsam Sorunları
```javascript
...
## 5. Söz Reddedilmelerini Ele Almamak
```javascript
...
## 6. Yineleme Sırasında Nesneleri Değiştirmek
```javascript
...
## 7. Anti-Desen: Geri Çağırma Cehennemi
```javascript
...
## 8. `typeof null`'yi Yanlış Anlamak
```javascript
...
## 9. Dizi Benzeri Nesneler Dizi Değildir
```javascript
...
## 10. Anti-Desen: Mutasyona Uğrayan Fonksiyon Parametreleri
```javascript
...
## 11. Olay Döngüsünü ve Mikro Görevleri Anlamamak
```javascript
...
## 12. Desen Karşıtı:`eval()`Kullanımı
```javascript
...
## 13. Etkinlik Dinleyicilerini Temizlemeyi Unutmak
```javascript
// ❌ WRONG — memory leak
class Component {
  mount() {
...
## 14. Sığ Kopyalama ve Derin Kopyalama
```javascript
...
## Özet
JavaScript'in esnekliği tuzaklarla birlikte gelir:`this`bağlama çağrı sitesine bağlıdır,`==`zorlama gerçekleştirir, sözler hata yönetimi gerektirir ve olay döngüsü sıralaması (makro görevlerden önce mikro görevler) açık değildir. Modern kurallar basittir:`const`/`let`kullanın, asla`var`kullanmayın,`==`yerine `===`'yi tercih edin, söz reddetmeleri her zaman ele alın, geri aramalar için ok işlevlerini kullanın, mutasyondan kaçının ve olay dinleyicilerini temizleyin. Bu kurallara uymak, JavaScript hatalarının çoğunu ortadan kaldırır.