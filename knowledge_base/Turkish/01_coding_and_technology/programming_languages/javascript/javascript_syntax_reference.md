...
| `%`| Kalan | `7 % 2`| `1`| |
| `**`| Üs | `2 ** 10`| `1024`| ES2016 |
| `++`| Artış | `x++`/`++x`| | Artış sonrası ve öncesi |
| `--`| Azaltma | `x--`/`--x`| | Azalma sonrası ve öncesi |
| `+x`,`-x`| Tekli | `+"42"`→`42`| Sayıya zorlama yazın |
### Karşılaştırma ve Eşitlik
| Operatör | İsim | Örnek | Notlar |
|----------|------|-----------|-------|
| `===`| Katı Eşitlik | `1 === 1`| Hiçbir tür zorlama yok — her zaman bunu tercih edin |
| `!==`| Katı Eşitsizlik | `1 !== "1"`| `true`— farklı türler |
| `==`| Soyut Eşitlik | `1 == "1"`| Zorlamayı yazın — kaçının |
| `!=`| Soyut Eşitsizlik | `1 != "1"`| `false`— kaçının |
| `<`,`>`,`<=`,`>=`| İlişkisel | `"b" > "a"`| Dizeler için sözlükbilimsel |
### Mantıksal ve Boş İşleçler
| Operatör | İsim | Örnek | Notlar |
|----------|------|-----------|-------|
| `&&`| Mantıksal VE | `a && b`| İlk yanlış veya son değeri döndürür |
| `\|\|`| Mantıksal VEYA | `a \|\| b`| İlk gerçeği veya son değeri döndürür |
| `!`| Mantıksal DEĞİL | `!true`| `false`|
| `??`| Birleşmeyi Sıfırla | `a ?? b`| Yalnızca `a`,`null`/`undefined`| ise `b`'yi döndürür |
| `?.`| Opsiyonel Zincirleme | `obj?.prop?.method()`| `undefined`'ye kısa devreler |
### Bitsel Operatörler
| Operatör | İsim | Örnek | Notlar |
|----------|------|-----------|-------|
| `&`| VE | `5 & 3`| `1`|
...
| `>>`| Sağa Kaydırma İmzalı | `-5 >> 1`| `-3`|
| `>>>`| İmzasız Sağa Kaydırma | `-1 >>> 0`| `4294967295`|
### Atama ve İmha
```javascript
// Basic assignment
let x = 10;
...
|---------------|---------------|---------------|
| 1 (en yüksek) | `()``[]``.``?.` | Sol |
| 2 | `new`(bağımsız değişkenler olmadan) | Sağ |
| 3 | `++``--` (son ek) | Yok |
| 4 | `!``~``+x``-x``typeof``void``delete``++``--`(ön ek) | Sağ |
| 5 | `**`| Sağ |
| 6 | `*``/``%`| Sol |
| 7 | `+``-` | Sol |
| 8 | `<<``>>``>>>`| Sol |
...
## İşlevler
### İşlev Bildirimleri ve İfadeleri
```javascript
...
| Koleksiyon | Sipariş edildi | Değiştirilebilir | Kopyalar | Anahtar Türü | Arama |
|------------|------------|------------|------------|----------|--------|
| `Array`| Evet | Evet | Evet | Dizin (sayı) | O(n) |
| `Object`| Evet (ekleme) | Evet | Tuşlar: Hayır | Dize/Sembol | O(1) ortalama |
| `Map`| Evet (ekleme) | Evet | Tuşlar: Hayır | Herhangi biri | O(1) ortalama |
| `Set`| Evet (ekleme) | Evet | Hayır | — | O(1) ortalama |
| `WeakMap`| Hayır | Evet | Tuşlar: Hayır | Yalnızca nesne | O(1) ortalama |
| `WeakSet`| Hayır | Evet | Hayır | Yalnızca nesne | O(1) ortalama |
### Diziler
```javascript
// Creation
...
## Özet
JavaScript'in sözdizimi görünüşte aldatıcı derecede basittir ancak önemli bir derinlik içerir. Dil, basit bir kodlama aracından sınıflar, modüller, oluşturucular, proxy'ler ve karmaşık bir eşzamansız model içeren çok paradigmalı bir dile dönüştü. Operatör önceliğini, olay döngüsünü, prototip zincirlerini ve`this`bağlamanın nüanslarını anlamak, yetkin geliştiricileri uzman geliştiricilerden ayırır. İsteğe bağlı zincirleme, sıfır birleştirme,`Promise.allSettled`ve üst düzey bekleme özelliklerine sahip modern JavaScript (ES2020+), geriye dönük uyumluluğu korurken ortak metinleri azaltmaya devam ediyor.