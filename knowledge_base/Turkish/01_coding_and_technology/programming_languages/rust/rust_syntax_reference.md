...
|----------|------|-----------|-------|
| `+`| İlave | `3 + 2`| Ayrıca`format!`aracılığıyla`String`birleştirme |
| `-`| Çıkarma | `3 - 2`| |
| `*`| Çarpma | `3 * 2`| |
| `/`| Bölüm | `7 / 2`| Tam sayılar için tam sayı bölümü:`3`|
| `%`| Kalan | `7 % 2`| `1`|
| `-x`| Olumsuzluk | `-5`| |
### Karşılaştırma ve Mantıksal Operatörler
| Operatör | İsim | Örnek | Notlar |
|----------|------|-----------|-------|
| `==`| Eşit | `x == y`|`PartialEq`gerektirir |
| `!=`| Eşit Değil | `x != y`| |
| `<`,`>`,`<=`,`>=`| Sipariş | `x >= y`|`PartialOrd`gerektirir |
| `&&`| Mantıksal VE | `a && b`| Kısa devre |
| `\|\|`| Mantıksal VEYA | `a \|\| b`| Kısa devre |
| `!`| Mantıksal DEĞİL | `!true`| |
### Bitsel Operatörler
| Operatör | İsim | Örnek | Notlar |
|----------|------|-----------|-------|
| `&`| VE | `5 & 3`| `1`|
| `\|`| VEYA | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `!`| DEĞİL | `!0b1010u8`| Bitsel tamamlayıcı |
| `<<`| Sola Kaydırma | `5 << 1`| `10`|
| `>>`| Sağa Kaydırma | `5 >> 1`| `2`|
### Operatör Önceliği (en yüksekten en düşüğe)
| Öncelik | Operatörler | Notlar |
|------------|---------------|-------|
| 1 (en yüksek) | `()`yolları`.`:: | |
| 2 | Yöntem çağrıları, alan erişimi | Otomatik deref |
| 3 | `-`(olumsuzlama),`!`,`*`(deref) | |
| 4 | `as`(tip döküm) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
| 14 | `..``..=` | Menzil |
| 15 | `=``+=` vb. | Ödev |
| 16 (en düşük) | `return``break``continue`| |
---
...
## Veri Yapıları
### Yerleşik Türlere Genel Bakış
| Tür | Yığın/Yığın | Boyut | Değişkenlik | Notlar |
|------|-----------|------|------------|-------|
| İlkeller (`i32`, `f64`, `bool`, `char`) | Yığın | Sabit | Varsayılan olarak değişmez | `Copy`|
| `[T; N]`(dizi) | Yığın | Sabit |`mut`ile değiştirilebilir | Sabit boyutlu, eğer T`Copy`ise`Copy`|
| `Vec<T>`| Yığın | Dinamik |`mut`ile değiştirilebilir | Büyütülebilir |
| `String`| Yığın | Dinamik |`mut`ile değiştirilebilir | UTF-8, büyütülebilir |
| `&[T]`(dilim) | Yığın (yağ ptr) | Dinamik görünüm |`&`veya `&mut`'ye bağlıdır | Ödünç alınan görünüm |
| `HashMap<K, V>`| Yığın | Dinamik |`mut`ile değiştirilebilir | O(1) ortalama arama |
| `BTreeMap<K, V>`| Yığın | Dinamik |`mut`ile değiştirilebilir | O(log n) araması, sıralı |
| `HashSet<T>`| Yığın | Dinamik | — | O(1) ortalama, kopya yok |
### Vektörler
```rust
// Creation
...
### Ortak Standart Kitaplık Özellikleri
```rust
...
## Özet
Rust'un sözdizimi, derleme zamanında uygulanan güvenlik garantilerinin temeli üzerine kurulmuştur. Sahiplik sistemi, özelliğe dayalı polimorfizm, kalıp eşleştirme ve sıfır maliyetli soyutlamalar, derleyicinin katı ama adil bir ortak olduğu tutarlı bir tasarım oluşturur. Temel kontrol akışından makrolar, güvenli olmayan kod ve akıllı işaretçiler gibi gelişmiş özelliklere kadar her yapı, güvenli, verimli sistem programlama hedefine hizmet eder. Yaşam süreleri, ödünç alma ve tür sistemi arasındaki etkileşimi anlamak Rust'ta uzmanlaşmanın anahtarıdır.