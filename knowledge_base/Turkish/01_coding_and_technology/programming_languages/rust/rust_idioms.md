...
---
# Rust — Deyimsel Kalıplar ve En İyi Uygulamalar
Bu kılavuz, temiz, güvenli Rust kodu yazmaya yönelik deyimsel kalıpları ve en iyi uygulamaları kapsar.
---
...
## Özet
Rust deyimleri şunu vurgular: sahiplik ve ödünç alma, hata işleme için`Result`/ `Option`, yöntemlerle numaralandırma, soyutlama için özellikler, yineleyici zincirleme, paylaşılan durum için akıllı işaretçiler ve sıfır maliyetli soyutlamalar. Biçimlendirme için `rustfmt`'yi, astarlama için `clippy`'yi ve Rust API Yönergelerini izleyin. Rust topluluğu güvenliğe, performansa ve doğruluğa değer verir; "derlenirse çalışır." İşlev bağımsız değişkenlerinde`&str`yerine `String`'yi tercih edin, hata yayılımı için`?`kullanın ve`#[derive(Debug, Clone, ...)]`ile ortak özellikleri türetin.