...
|----------|---------------|-----------|
| **İstisnalar (dene/yakala)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **Sonuç/Seçenek türleri** | Rust, Haskell, OCaml, Scala (ayrıca) | `Result<T, E>`,`Maybe a`|
| **Birden çok dönüş değeri** | Git Lua | `value, err := f()`|
| **Hata kodları** | C, Fortran | Hata kodunu döndürün, manuel olarak kontrol edin |
| **Desen eşleştirme hataları** | İksir, Erlang | `{:ok, value}`/`{:error, reason}`|
| **Koşullar (yeniden başlatılabilir)** | Ortak Lisp | `restart-case`,`handler-bind`|
| **Hata işleme yok** | Çizik, SQL (temel) | Yok |
| **Sözleşmeler** | Ada, Eyfel | `Pre`,`Post`koşullar |
## İstisnaya Dayalı Hata İşleme
```python
# Python
try:
    result = 10 / 0
...
## Özet: Paradigmalarda Hata İşleme
| Paradigma | Yaklaşım | Artıları | Eksileri |
|----------|----------|----------|------|
| **İstisnalar** | fırlat/yakala | Mutlu yolu temizleyin, izleri yığınlayın | Unutulabilir, akışın kötüye kullanılması |
| **Sonuç türleri** | Geri Dön`Ok`/`Err`| Açık, hatalar göz ardı edilemez | Ayrıntılı,`?`operatörü gerekli |
| **Hata kodları** | İnt durumunu döndür | Basit, C uyumlu | Göz ardı edilmesi kolay, bağlam yok |
| **Birden fazla dönüş** | `value, err := f()`| Açık, deyimsel | `err`'yi kontrol etmeyi unutabilirsiniz |
| **Desen eşleştirme** | `{:ok, v}`/`{:error, r}`| Kapsamlı, işlevsel |`with`olmadan ayrıntılı |
| **Koşullar** | Yeniden başlatılabilir işleyiciler | En güçlü, kurtarılabilir | Karmaşık, nadiren anlaşılan |