...
-->
# TypeScript — Deyimsel Kalıplar ve En İyi Uygulamalar
Bu kılavuz, temiz, tür açısından güvenli TypeScript kodu yazmaya yönelik deyim kalıplarını ve en iyi uygulamaları kapsar.
---
...
## Özet
TypeScript deyimleri şunları vurgular: katı tür denetimi, nesne şekilleri için arayüzler, durum için ayrımlı birleşimler, daraltma için tür korumaları, kısıtlamalı jenerikler, değişmezlik için`readonly`ve değişmez türler için `as const`. Formatlama için Prettier olan`typescript-eslint`ile ESLint'i takip edin ve her zaman `strict: true`'yi etkinleştirin. TypeScript topluluğu tür güvenliğine değer verir; derlenirse muhtemelen işe yarar. Nesne şekilleri için `interface`'yi, birleşimler ve yardımcı programlar için `type`'yi tercih edin ve ne pahasına olursa olsun `any`'den kaçının.