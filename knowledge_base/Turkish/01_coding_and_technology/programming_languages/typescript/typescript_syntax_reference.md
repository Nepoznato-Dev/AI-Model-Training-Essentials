...
## Tür Sistem Temelleri
### İlkel ve Yerleşik Türler
```typescript
// Primitives
...
### Değişmez Türler ve Birleşimler
```typescript
// Literal types
...
### Yerleşik Yardımcı Program Türleri
```typescript
interface User {
  id: string;
  name: string;
...
## Dekoratörler (TC39 Aşama 3 / TypeScript 5.0+)
```typescript
// Class decorator
...
## Özet
TypeScript'in yazım sistemi onun tanımlayıcı özelliğidir ve oldukça derindir. Temel ek açıklamalardan jeneriklere, koşullu türlere, eşlenen türlere, şablon değişmez türlerine ve `infer`'ye kadar, tür sistemi esas olarak derleme zamanında çalışan işlevsel bir dildir.`interface`ve `type`, genel ve yardımcı program türleri ve yapısal ve nominal yazım arasındaki etkileşimi anlamak, hem güvenli hem de ergonomik TypeScript yazmayı destekler. Dil gelişmeye devam ediyor; her sürüm, JavaScript ile geriye dönük tam uyumluluğu korurken daha fazla ifade gücü sağlıyor.