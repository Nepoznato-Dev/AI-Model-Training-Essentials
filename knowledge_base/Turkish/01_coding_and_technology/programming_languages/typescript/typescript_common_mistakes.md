...
# TypeScript — Yaygın Hatalar ve Anti-Kalıplar
Bu belge TypeScript'teki en yaygın hataları, tuzakları ve anti-kalıpları kataloglamaktadır. Her giriş yanlış yaklaşımı gösterir, neden başarısız olduğunu açıklar ve doğru çözümü sağlar.
---

## 1. `any`'yi aşırı kullanmak
```typescript
...
## 2. Tip Korumaları Yerine İddiaları Yazın
```typescript
...
## 3. Boş Olmayan İddianın Kötüye Kullanımı
```typescript
...
## 4. Arayüzler ve Türlerin Karışıklığı
```typescript
...
## 5. Anti-Desenleri Numaralandırma
```typescript
...
## 8. Eş Zamansız/Bekleme Türü Hataları
```typescript
...
## 9. Anti-Desen: Tekrarlanan Türler (Jenerik Kullanmamak)
```typescript
...
## 10. Aşırı İsteğe Bağlı Zincirleme
```typescript
...
## 12. Ayrımcılığa Uğrayan Sendikalardan Yararlanmamak
```typescript
...
## Özet
TypeScript'in yazım sistemi güçlüdür ancak disiplin gerektirir. Esas günah `any`'ye ulaşmaktır — tür gerçekten bilinmediğinde`unknown`kullanın ve tür korumalarıyla daraltın. Tanımsızı doğru şekilde işleyerek boş olmayan iddialardan (`!`) kaçının. Nesne şekilleri, birleşim türleri ve hesaplanan türler için arayüzleri kullanın. Ayrımcı sendikalar tüm böcek sınıflarını ortadan kaldırır. `strict: true`'yi ilk günden itibaren etkinleştirin; katılığın iyileştirilmesi zahmetlidir. Amaç, yasadışı durumları tür düzeyinde temsil edilemez hale getirmektir.