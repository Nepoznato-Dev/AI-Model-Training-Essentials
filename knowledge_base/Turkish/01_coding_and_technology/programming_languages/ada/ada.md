...
## Ada Neden Önemlidir
- **Güvenlik açısından kritik sistemler**: Arızanın bir seçenek olmadığı sistemler için sıfırdan tasarlanmıştır.
- **Güçlü yazma**: Herhangi bir ana dilin en katı tür sistemi — derleme zamanında diğer dillerin gözden kaçırdığı hataları yakalar.
- **Yerleşik eşzamanlılık**: Görev verme (eşzamanlı programlama) bir kitaplığın değil, dilin bir parçasıdır.
- **Resmi doğrulama**: Programın doğruluğunu kanıtlamaya yönelik resmi yöntemleri destekler.
- **Güvenilirlik özellikleri**: Yerleşik istisnalar, sözleşmeler (ön/son koşullar) ve çalışma zamanı kontrolleri.
- **Modern Ada**: Ada 2012 ve Ada 2022, güvenlik garantilerini korurken modern özellikler ekledi.
## Takaslar
| Sınırlama | Ayrıntılar | Tipik Geçici Çözüm |
|-----------|------------|-----------|
| **Niş topluluk** | C, Java veya Python ile karşılaştırıldığında küçük geliştirici tabanı | Uzmanlaşmış ancak bilgili topluluk |
| **Ayrıntılı söz dizimi** | C veya Python'dan daha ayrıntılı | Güvenlik odaklı tasarımın bir parçası olarak kabul edin |
| **Sınırlı ekosistem** | Ana dillere göre daha az kütüphane | Özel kod yazın; kapsamlı standart kütüphaneyi kullanın |
| **Öğrenme eğrisi** | Birçok güvenlik özelliğine sahip karmaşık dil | Güvenliğin kritik olduğu işler için Ada'nın "SPARK" alt kümesiyle başlayın |
| **Derleyici kullanılabilirliği** | Daha az derleyici seçeneği (TBMM ana seçenektir) | TBMM ücretsiz ve bakımlıdır |
---
...
### Etiketli Türler ve Kalıtım (OOP)
```ada
-- Abstract base type
package Shapes is
...
### Jenerikler (Derleme Zamanı Polimorfizmi)
```ada
...
### Sözleşmeler ve Hususlar (Ada 2012)
```ada
...
### Korunan Nesneler (İş Parçacığı Güvenli Paylaşılan Veriler)
```ada
...
### C Birlikte Çalışabilirlik (Arayüzler)
```ada
with Interfaces.C;
...
## Tasarım Desenleri
### Desen 1: Korunan Nesnelere Sahip Gözlemci
```ada
...
### Desen 2: Ayırt Edilmiş Türlere Sahip Durum Makinesi
```ada
...
### Desen 3: Kontrollü Türlerle RAII
```ada
...
## Dağıtım
### Güvenlik Açısından Kritik Sertifikasyon için SPARK
```ada
...
## Ada Ne Zaman Kullanılmalı
| Senaryo | Neden Ada | Daha İyi Alternatif |
|----------|------------|-----------|
| Güvenlik açısından kritik sistemler | Bunun için tasarlandı; resmi doğrulama desteği | — |
| Havacılık / uzay | Sertifikalı derleyiciler; DO-178C uyumluluğu | — |
| Savunma sistemleri | Savunma Bakanlığı mirası; güvenlik özellikleri | — |
| Demiryolu / tıbbi cihazlar | Yüksek bütünlük gereksinimleri | — |
| Genel uygulama geliştirme | Kritik olmayan sistemler için aşırıya kaçma | Python, Java, Git |
