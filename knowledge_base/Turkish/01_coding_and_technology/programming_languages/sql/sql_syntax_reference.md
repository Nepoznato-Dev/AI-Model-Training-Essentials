...
-->
# SQL — Sözdizimi Referansı
Bu belge, SQL için kapsamlı, yapılandırılmış bir sözdizimi referansı sağlar (PostgreSQL/MySQL/SQLite lehçesi farklılıklarına ilişkin notlarla birlikte ANSI SQL). Kapsamlı sorgu modellerine, pencere işlevlerine, CTE'lere ve veritabanı tasarımına odaklanarak ana SQL referansını tamamlar.
---
...
## Özet
SQL'in sözdizimi 50 yıl boyunca sürekli olarak gelişirken aynı kalmıştır. Temel sorgular (SELECT, JOIN, WHERE, GROUP BY) tüm veritabanlarında standarttır. Pencere işlevleri, CTE'ler ve JSON desteği, modern SQL'in yeteneklerini temsil eder. Etkili SQL kullanımı, veritabanı motorunun sorguları nasıl işlediğini anlamaktan (yürütme planlarını okumak, dizinleri anlamak ve prosedür döngüleri yerine küme tabanlı işlemler yazmak) faydalanır. Güçlü SQL becerileri, verilerle çalışan herkes için değerlidir.