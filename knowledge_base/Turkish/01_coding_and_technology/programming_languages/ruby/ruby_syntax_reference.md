...
-->
# Ruby — Sözdizimi Referansı
Bu belge Ruby (3.x) için kapsamlı, yapılandırılmış bir sözdizimi referansı sağlar. Kapsamlı sözdizimi kalıplarına, bloklara ve kapanışlara, metaprogramlamaya ve Ruby deyimlerine odaklanarak ana Ruby referansını tamamlar.
---
...
| `==``!=``===``<=>` | Eşitlik | `a == b`| `===`büyük/küçük harf eşitliğidir; `<=>`uzay gemisidir |
| `eql?`| Değer eşitliği | `a.eql?(b)`| `==`'den daha katıdır (kontrol türü) |
| `equal?`| Kimlik | `a.equal?(b)`| Aynı nesne mi? |
| `<``>``<=``>=` | Karşılaştırma | `a >= b`| |
| `&&``\|\|``!`| mantıksal | `a && b`| Kısa devre; son değerlendirilen değeri döndürür |
| `and``or``not`| Mantıksal (düşük öncelikli) | `a and b`| Kaçının —`&&`/`\|\|`/`!`kullanın |
| `&``\|``^``~``<<``>>` | Bitsel | `a & b`| |
| `=~``!~` | Normal ifade eşleşmesi | `str =~ /pattern/`| Dizini veya sıfırı döndürür |
| `..``...` | Menzil | `1..10`| `..`dahil; `...`özel |
| `?.`| Güvenli gezinme | `user&.name`| Kullanıcı sıfır ise sıfır değerini döndürür |
| `[]``[]=` | Öğe erişimi | `arr[0]`| |
### Doğruluk
```ruby
...
## Numaralandırılabilir ve Koleksiyonlar
```ruby
# Array
arr = [1, 2, 3, 4, 5]
...
## Özet
Ruby'nin sözdizimi geliştiricinin mutluluğu için tasarlanmıştır; İngilizce gibi okunur ve ifade gücünü ödüllendirir. Dilin gücü nesne modelinden (her şey bir nesnedir), bloklardan ve kapanışlardan, metaprogramlama yeteneklerinden ve zengin bir koleksiyon işleme API'si sağlayan Enumerable modülünden gelir. Ruby 3.x, dilin temel felsefesini korurken aynı zamanda desen eşleştirme, RBS türü imzalar ve eşzamanlılık için Ractor ekledi: programcıları üretken ve mutlu kılmak.