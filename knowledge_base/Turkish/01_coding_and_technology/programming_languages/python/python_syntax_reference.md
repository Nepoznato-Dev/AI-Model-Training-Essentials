...
-->
# Python — Sözdizimi Referansı
Bu belge Python için kapsamlı, yapılandırılmış bir sözdizimi referansı sağlar. Kapsamlı sözdizimi kalıplarına, operatör tablolarına ve dahili mekanizmalara odaklanarak ana Python referansını tamamlar.
---
...
|----------|------|-----------|-----------|-------|
| `+`| İlave | `3 + 2`| `5`| Ayrıca dize/liste birleştirme |
| `-`| Çıkarma | `3 - 2`| `1`| Ayrıca tekli olumsuzluk |
| `*`| Çarpma | `3 * 2`| `6`| Ayrıca dize/liste tekrarı |
| `/`| Gerçek Bölüm | `7 / 2`| `3.5`| Her zaman kayan noktayı döndürür |
| `//`| Kat Bölümü | `7 // 2`| `3`| Negatif sonsuza doğru yuvarlar |
| `%`| Modül | `7 % 2`| `1`| İşaret bölenle eşleşir |
| `**`| Üs | `2 ** 10`| `1024`| Sağ çağrışımlı |
### Karşılaştırma ve Boole Operatörleri
| Operatör | İsim | Örnek | Notlar |
|----------|------|-----------|-------|
| `==`| Eşit | `x == y`| Değer karşılaştırması (kimlik değil) |
| `!=`| Eşit Değil | `x != y`| |
| `<`,`>`,`<=`,`>=`| Sipariş | `x >= y`| Zincirlenebilir:`a < b < c`|
| `is`| Kimlik | `x is y`| Bellekte aynı nesne |
| `is not`| Kimlik Değil | `x is not None`| `!= None`'ye tercih edilir |
| `in`| Üyelik | `x in collection`| Kümeler/sözler için O(1), listeler için O(n) |
| `and`| Mantıksal VE | `a and b`| Kısa devre; son doğru/yanlış değeri döndürür |
| `or`| Mantıksal VEYA | `a or b`| Kısa devre; ilk doğru/yanlış değeri döndürür |
| `not`| Mantıksal DEĞİL | `not x`| |
### Bitsel Operatörler
| Operatör | İsim | Örnek | Notlar |
|----------|------|-----------|-------|
| `&`| VE | `5 & 3`| `1`|
...
| 8 | `^`| Sol |
| 9 | `\|`| Sol |
| 10 | `==`,`!=`,`<`,`>`,`<=`,`>=`,`is`,`in`| Zincirlenebilir |
| 11 | `not`| Sağ |
| 12 | `and`| Sol |
| 13 (en düşük) | `or`| Sol |
### Ödev ve Genişletilmiş Ödev
```python
...
## Veri Yapıları
### Yerleşik Koleksiyonlara Genel Bakış
| Koleksiyon | Sipariş edildi | Değiştirilebilir | Kopyalar | Sözdizimi | Arama |
|------------|------------|------------|------------|-----------|--------|
| `list`| Evet | Evet | Evet | `[1, 2, 3]`| O(n) |
| `tuple`| Evet | Hayır | Evet | `(1, 2, 3)`| O(n) |
| `dict`| Evet (3,7+) | Evet | Tuşlar: Hayır | `{"k": "v"}`| O(1) ortalama |
| `set`| Hayır | Evet | Hayır | `{1, 2, 3}`| O(1) ortalama |
| `frozenset`| Hayır | Hayır | Hayır | `frozenset({1,2})`| O(1) ortalama |
### Listeler
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
## Özet
Bu sözdizimi referansı, temel operatörler ve kontrol akışından gelişmiş metaprogramlamaya, tanımlayıcılara ve tür sistemine kadar Python'un tüm özellik kümesini kapsar. Python'un sözdizimi kasıtlı olarak minimum düzeyde ve okunabilirdir, ancak derinliği önemlidir: desen eşleştirme, eşzamansız oluşturucular, protokoller ve metasınıflar karmaşık uygulamalar için güçlü araçlar sağlar. Dil, her sürümde geriye dönük uyumluluğu korurken ifade gücünü artıran özellikler eklenerek gelişmeye devam ediyor.