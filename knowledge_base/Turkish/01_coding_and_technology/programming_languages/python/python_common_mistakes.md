...
-->
# Python — Yaygın Hatalar ve Anti-Kalıplar
Bu belge Python'daki en yaygın hataları, tuzakları ve anti-kalıpları kataloglamaktadır. Her giriş yanlış yaklaşımı gösterir, neden başarısız olduğunu açıklar ve doğru çözümü sağlar. Bu tuzakları anlamak, daha sağlam Pythonic kod yazmanıza yardımcı olacaktır.
---

## 1. Değişken Varsayılan Bağımsız Değişkenler
```python
...
**Neden olur:** Varsayılan bağımsız değişkenler her çağrıda değil, işlev tanımı zamanında bir kez değerlendirilir. Değişken bir varsayılan (liste, dikte, ayar) tüm çağrılarda paylaşılır.
---

## 2. Yineleme Sırasında Bir Listeyi Değiştirmek
```python
...
## 4.`isinstance()`Yerine`type()`Kullanmak
```python
...
## 5. Kaynaklar için İçerik Yöneticilerini Kullanmamak
```python
...
## 6. Değer Karşılaştırma için`is`Operatörü
```python
...
## 8. Python'un Kapsam Kurallarını Anlamamak (LEGB)
```python
...
## 10. Anti-Desen:`except:`Bare Except'i Kullanma
```python
...
## 11. Değişken Referansları Dahili Duruma Döndürmek
```python
...
## 12.`setdefault`veya`collections.defaultdict`Kullanılmaması
```python
...
## 13. Koşullarda`==`ve `=`'yi Karıştırmak
```python
...
## 14. Jeneratörün Tükendiğini Anlamamak
```python
...
## 15. Anti-Desen: Tanrı Nesneleri / Aşırı Yüklenmiş Sınıflar
```python
...
## Özet
Python'un basitliği aldatıcıdır; değişken varsayılanlar, geç bağlama kapanışları, kapsam kuralları ve kaynak yönetiminin hepsinde geliştiricileri tuzağa düşüren incelikli davranışlar vardır. Hatırlanması gereken temel ilkeler: varsayılanlar bir kez değerlendirilir,`is`eşitliği değil kimliği kontrol eder, kaynaklar için her zaman bağlam yöneticilerini kullanır, belirli istisnaları yakalar ve kompozisyonu tanrı nesnelere tercih eder. Pythonic kod yazmak, bu tuzakları anlamak ve dilin özelliklerini (bağlam yöneticileri, kavramalar, oluşturucular,`collections`modülü) amaçlandığı şekilde kullanmak anlamına gelir.