...
## Özet
Montaj dili sözdizimi, işlemci mimarisi ve montajcı (NASM, GAS, MASM) tarafından tanımlanır. x86-64, 16 genel amaçlı kayıt, çoklu adresleme modları ve veri hareketi, aritmetik, mantık, kontrol akışı, SIMD ve sistem çağrılarını kapsayan yüzlerce talimat sağlar. Burada kullanılan NASM sözdizimi, Linux x86-64 geliştirmesi için en yaygın olanıdır. Montajda uzmanlaşmak, yazmaçları, yığını, çağırma kurallarını ve üst düzey yapıların makine talimatlarıyla nasıl eşleştiğini anlamayı gerektirir.