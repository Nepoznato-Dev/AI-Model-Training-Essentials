...
# Diller Arası Karşılaştırma — Kontrol Akışı
## If/Else İfadeleri
```python
# Python
if x > 0:
...
| Yakut | `10.times { \|i\| ... }`veya`(0..9).each { \|i\| ... }`|
| Hızlı | `for i in 0..<10`|
...
| Ölçek | `x match { case 1 => ... }`|
| Hızlı | `switch x { case 1: ... default: ... }`|
| Kotlin | `when (x) { 1 -> ... else -> ... }`|
| OCaml | `match x with 1 -> ... \| _ -> ...`|
| Erlang | `case X of 1 -> ...; _ -> ... end`|
| İksir | `case x do 1 -> ...; _ -> ... end`|
| Dart | `switch (x) { 1 => ...; _ => ... }`(Dart 3.0) |
| PHP | `match($x) { 1 => ..., default => ... }`(PHP 8.0) |
| C# | `x switch { 1 => ..., _ => ... }`(C# 8+) |
| TypeScript | Yerel kalıp eşleşmesi yok (if/else kullanın) |
| Python | `match x: case 1: ... case _: ...`(Python 3.10+) |
| Giriş | Yerleşik cümle eşleştirme |
| Git | `switch x { case 1: ... default: ... }`|