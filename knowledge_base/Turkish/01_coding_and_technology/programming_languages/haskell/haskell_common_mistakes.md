...
## 1. Sonsuz Listeler ve Tembellik Sürprizleri
```haskell
...
## 5. Kayıt Söz Dizimini Düzgün Kullanmamak
```haskell
...
## 7. Anti-Desen: `IO`'nin Aşırı Kullanımı
```haskell
...
## Özet
Haskell'in saflığı ve tembelliği benzersiz tuzaklar yaratır: tembel değerlendirmeden kaynaklanan alan sızıntıları, çalışma zamanında çöken kısmi işlevler, String/Text/ByteString arasındaki karışıklık ve aşırı IO kullanımı. Haskell yöntemi şudur: akümülatörler için sıkı değerlendirme kullanın (`foldl'`, BangPatterns), kısmi işlevlerden kaçının (`head`,`read`,`!!`), hata işleme için`Maybe`/`Either`kullanın, metin için`Text`kullanın ve saklayın IO minimum. Derleyicinin yazım sistemi en büyük müttefikinizdir; bırakın sizi tam ve saf işlevlere doğru yönlendirsin.