...
|----------|------|-----------|-------|
| `+``-``*``/` | Aritmetik | `x + y`| Sayı tipi sınıfı |
| `div``mod` | Tamsayı bölümü | `7 \`div\` 2`| Geri Dönüyor 3 |
| `^``**` | Güç | `2 ^ 10`|  İntegral için `^`;  yüzer için`**`|
| `==``/=` | Eşitlik | `x == y`| Denklem tipi sınıfı |
| `<``>``<=``>=` | Karşılaştırma | `x >= y`| Ord tip sınıfı |
| `&&``\|\|``not`| mantıksal | `a && b`| Kısa devre |
| `.`| Fonksiyon bileşimi | `(f . g) x`| `f (g x)`|
| `$`| Fonksiyon uygulaması | `f $ x + 1`| `f (x + 1)`— düşük öncelikli |
| `<>`| Monoid eklentisi | `"hello" <> " world"`| Yarıgrup |
| `<$>`| İşlevsel fmap | `(+1) <$> Just 5`| `fmap (+1) (Just 5)`|
| `<*>`| Uygulamalı başvuru | `pure (+) <*> Just 3`| |
| `>>=`| Monad bağlama | `Just 5 >>= \x -> Just (x+1)`| |
| `>>`| Monad dizisi | `putStrLn "hi" >> putStrLn "bye"`| |
| `=<<`| Ciltleme (ters çevrilmiş) | `f =<< mx`|`mx >>= f`ile aynı |
| `++`| Liste birleştirme | `[1,2] ++ [3,4]`| O(n) sol listede |
| `:`| Eksileri | `1 : [2, 3]`| Ç(1) |
| `!!`| Dizin | `[1,2,3] !! 1`| O(n) — kısmi fonksiyon |
| `\\`| Liste farkı | `[1,2,3] \\ [2]`| `[1,3]`|
### Operatör Önceliği (seçili)
| Öncelik | Operatörler |
|---------------|-----------|
| 7 (en yüksek) | `*``/``div``mod` |
| 6 | `+``-` |
...
## Özet
Haskell'in sözdizimi minimal düzeyde olmasına rağmen olağanüstü derecede etkileyicidir. Cebirsel veri türleri, tür sınıfları, GADT'ler ve tür aileleri içeren tür sistemi, hem bir belirtim hem de doğrulama aracı olarak hizmet eder. Desen eşleştirme, tembel değerlendirme ve monadlar karmaşık sorunlara zarif çözümler sunar. Haskell'in saflığı (türlerde yan etki olmaması), kodun akıl yürütmesini, test edilmesini ve paralelleştirilmesini kolaylaştırır. Öğrenme eğrisi dik olsa da Haskell'in etkisi, Swift'in seçeneklerinden Kotlin'in mühürlü sınıflarına ve Rust'un yazım sistemine kadar modern dillere nüfuz ediyor. Haskell'de uzmanlaşmak sizi her dilde daha iyi bir programcı yapar.