...
## 1. Platform Türleri ve NullPointerException
```kotlin
...
## 3.`data class`Uygun`equals`/`hashCode`Olmadan
```kotlin
...
## 4. Kapsam İşlevlerini Doğru Kullanmamak
```kotlin
...
## 5. Eşyordamlar: İptali İşlememek
```kotlin
...
## 6. Anti-Desen: Java Stili Kotlin
```kotlin
...
## 8. Uzatma İşlevi Gölgelendirmesi
```kotlin
...
## 9. Pahalı Başlatma İçin`by lazy`Kullanılmaması
```kotlin
...
## 10. Akış ve Canlı Veri Karışıklığı
```kotlin
...
## Özet
Kotlin'in kısa ve öz olması, ince sorunları gizleyebilir: Java birlikte çalışmadan kaynaklanan platform türleri,`lateinit`çökmeleri, koleksiyonları bozan değişken veri sınıfları ve eşyordam iptali. Kotlin'in yöntemi şudur: null güvenliğini benimseyin (hiçbir zaman`!!`kullanmayın), doğru kapsam işlevini kullanın (dönüşümler için `let`, yapılandırma için `apply`, yan etkiler için `also`), kapsamlı`when`ile mühürlü sınıflar kullanın, eşyordam iptalini yönetin ve deyimsel Kotlin yazın (Kotlin sözdizimi ile Java değil). Derleyici sizin rehberinizdir; eğer uyarıyorsa dinleyin.