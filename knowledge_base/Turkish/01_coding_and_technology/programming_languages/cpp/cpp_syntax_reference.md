...
|----------|------|-----------|-------|
| `+``-``*``/``%`| Aritmetik | `a + b`| C ile aynı |
| `++``--` | Arttırma/Azaltma | `++i`,`i++`| Ön artışı tercih et |
| `==``!=``<``>``<=``>=` | Karşılaştırma | `a <=> b`| `<=>`üç yönlü bir karşılaştırmadır (C++20) |
| `&&``\|\|``!`| mantıksal | `a && b`| Kısa devre |
| `&``\|``^``~``<<``>>` | Bitsel | `a & b`| C ile aynı |
| `?:`| Üçlü | `cond ? a : b`| |
### C++-Özel Operatörler
| Operatör | İsim | Örnek | Notlar |
|----------|------|-----------|-------|
| `::`| Kapsam çözümü | `std::cout`| |
| `.*``->*` | İşaretçiden üyeye | `obj.*pmf`| |
| `<=>`| Uzay Gemisi (C++20) | `a <=> b`| `strong_ordering`'yi döndürür |
| `co_await`| Coroutine askıya alma | `co_await expr`| C++20 |
| `co_yield`| Koroutin verimi | `co_yield value`| C++20 |
| `co_return`| Coroutine dönüşü | `co_return value`| C++20 |
...
## Akışı Kontrol Et
### Yapılandırılmış Ciltlemeler ve Modern Desenler
```cpp
...
## Fonksiyonlar ve Lambdalar
### Modern İşlev Söz Dizimi
```cpp
...
## STL Kapları ve Algoritmalar
### Kapsayıcıya Genel Bakış
| Konteyner | Başlık | Sipariş edildi | Arama | Notlar |
|-----------|-----------|-----------|-----------|-------|
| `vector`| `<vector>`| Evet | O(n) | Diziler için varsayılan seçim |
| `deque`| `<deque>`| Evet | O(n) | Her iki uçta hızlı itme/açma |
| `list`| `<list>`| Evet | O(n) | Çift bağlantılı liste |
| `array`| `<array>`| Evet | O(n) | Sabit boyutlu, yığına ayrılmış |
| `set`/`multiset`| `<set>`| Evet | O(log n) | Kırmızı-siyah ağaç |
| `map`/`multimap`| `<map>`| Evet (anahtarla) | O(log n) | Anahtar/değer çifti, kırmızı-siyah ağaç |
| `unordered_set`| `<unordered_set>`| Hayır | O(1) ortalama | Hash tablosu |
| `unordered_map`| `<unordered_map>`| Hayır | O(1) ortalama | Hash tablosu |
...
## Özet
C++ sözdizimi, düşük seviyeli işaretçi manipülasyonundan yüksek seviyeli şablon metaprogramlamasına ve eşyordam tabanlı eşzamansızlığa kadar uzanır. Dilin C++17/20/23 aracılığıyla evrimi, güvenliği (akıllı işaretçiler, `std::optional`, kavramlar), ifade edilebilirliği (yapılandırılmış bağlamalar, aralıklar, `std::format`) ve performansı (sıfır maliyetli soyutlamalara olanak tanıyan kavramlar, yığın yükü olmadan eşzamansız eşyordamlar) önemli ölçüde iyileştirdi. Temel felsefe değişmeden kalıyor: kullanmadığınız şey için ödeme yapmamalısınız ve dil, çıplak metalden alana özgü dillere kadar her düzeyde soyutlamayı desteklemelidir.