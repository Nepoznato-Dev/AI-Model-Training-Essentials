...
# Fortran — Sözdizimi Referansı
Bu belge Modern Fortran (2008/2018) için kapsamlı, yapılandırılmış bir sözdizimi referansı sağlar. Kapsamlı sözdizimi kalıplarına, dizi işlemlerine, modüllere, OOP'ye ve yüksek performanslı bilgi işleme odaklanarak ana Fortran referansını tamamlar.
---
...
| `<``>``<=``>=` | Karşılaştırma | `a >= b`| |
| `.and.``.or.``.not.`| mantıksal | `a .and. b`| |
| `.eqv.``.neqv.` | Mantıksal eşdeğerlik | `a .eqv. b`| |
...
## Coarray'ler (Paralel Programlama)
```fortran
...
## Özet
Modern Fortran'ın sözdizimi temiz, yapılandırılmış ve sayısal hesaplama için optimize edilmiştir. Dizi işlemleri çoğu matematiksel görev için döngüleri ortadan kaldırır. Modüller kapsülleme ve ad alanı yönetimi sağlar. OOP özellikleri (türetilmiş türler, kalıtım, polimorfizm) temiz kod organizasyonuna olanak tanır. Coarray'ler harici kütüphaneler olmadan yerel paralel programlama sunar. Fortran'ın güçlü yönleri (performans, dizi işlemleri ve sayısal hassasiyet) bilimsel hesaplama açısından eşsizdir. HPC, hesaplamalı fizik ve mühendislik simülasyonu için Fortran bir güç merkezi olmaya devam ediyor.