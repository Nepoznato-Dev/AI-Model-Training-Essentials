...
|----------|------|-----------|-------|
| `+``-``*``/``^`| Aritmetik | `A * B`| Matris çarpımı |
| `.*``./``.^`| Öğe bazında | `A .* B`| Dizi işlemi |
| `'`| Transpoze | `A'`| Eşlenik devrik |
| `.'`| Eşlenik olmayan devrik | `A.'`| |
| `\`| Sol bölme | `A \ b`| Ax = b |
| `/`| Sağa bölme | `b / A`| xA = b |
| `==``~=` | Eşitlik | `A == B`| Öğe bazında |
| `<``>``<=``>=` | Karşılaştırma | `A > 0`| Öğe bazında |
| `&``\|``~`| Mantıksal (öğe bazında) | `A & B`| |
| `&&``\|\|` | Kısa devre | `a && b`| Yalnızca skaler |
| `:`| İki nokta üst üste/aralık | `1:10`| `start:step:stop`|
| `;`| Satır ayırıcı / bastır | `[1; 2; 3]`| |
| `,`| Sütun ayırıcı | `[1, 2, 3]`| |
---
...
## Özet
MATLAB'ın sözdizimi matris önceliklidir; her değişken bir dizidir ve işlemler doğrusal cebir için optimize edilmiştir. İki nokta üst üste operatörü, mantıksal indeksleme ve vektörleştirilmiş işlemler, çoğu sayısal görev için döngüleri ortadan kaldırır. Çizim sistemi, minimum kodla yayın kalitesinde rakamlar üretir. Araç kutuları MATLAB'ı her mühendislik alanına genişletir. Python bazı bölgeleri ele geçirmiş olsa da MATLAB mühendislik hesaplamaları, Simulink tabanlı modelleme ve hızlı prototipleme için standart olmaya devam ediyor.