...
---
# Lua — Sözdizimi Referansı
Bu belge Lua (5.4) için kapsamlı, yapılandırılmış bir sözdizimi referansı sağlar. Kapsamlı sözdizimi kalıplarına, tablolara ve metatablolara, eşyordamlara ve Lua'nın benzersiz tasarım felsefesine odaklanarak ana Lua referansını tamamlar.
---
...
| `//`| Zemin bölümü | `7 // 2`| 3'ü döndürür (Lua 5.3+) |
| `==``~=` | Eşitlik | `a == b`| `~=`"eşit değil" |
| `<``>``<=``>=` | Karşılaştırma | `a >= b`| |
| `and``or``not`| mantıksal | `a and b`| Boolean'ları değil işlenenleri döndürür |
| `..`| birleştirme | `"hello" .. " world"`| |
| `#`| Uzunluk | `#t`| Dize veya tablonun uzunluğu |
| `&``\|``~``<<``>>`| Bitsel | `a & b`| Lua 5.3+ (tamsayı işlemleri) |
### Mantıksal Operatörler — Boolean Olmayan Dönüşler
```lua
...
## Metatablolar ve Meta Yöntemler
```lua
...
## Özet
Lua'nın sözdizimi kasıtlı olarak minimal düzeydedir; dilin tamamı tek bir sayfada anlatılabilir. Tablolar evrensel veri yapısı görevi görür (diziler, sözlükler, nesneler, modüller). Metatablolar dil karmaşıklığı olmadan genişletilebilirlik sağlar. Eşyordamlar işbirlikçi çoklu görevlere ve zarif yineleyici modellerine olanak tanır. Dilin gücü gömülebilirliğinden gelir: Lua, ana bilgisayar uygulamalarının içinde yaşayacak şekilde tasarlanmıştır ve minimum ek yük ile komut dosyası oluşturma yetenekleri sağlar. Oyun geliştirme, konfigürasyon ve gömülü sistemler açısından Lua'nın basitliği onun en büyük gücüdür; tam olarak ihtiyaç duyduğu şeyi yapan, daha fazlasını yapmayan küçük bir dildir.