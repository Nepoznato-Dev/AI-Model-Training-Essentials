...
## 2. Koşullarda`nil`ve`false`Karşılaştırması
```lua
...
## 4. Varsayılan Olarak Genel Değişkenler
```lua
...
## 7. OOP için Metatabloları Kullanmamak
```lua
...
## Özet
Lua'nın minimalizmi tuzaklar yaratır: 1 tabanlı indeksleme, yalnızca`nil`ve`false`sahtedir, tablolar referanstır (değer değil), değişkenler varsayılan olarak geneldir ve`ipairs`sıralanırken`pairs`sırasızdır. Lua yöntemi şudur: her zaman`local`kullanın, diziler için`ipairs`ve sözlükler için`pairs`kullanın, dize oluşturmak için`table.concat`kullanın ve OOP için metatablolar kullanın. Lua'nın sadeliği onun gücüdür; ancak uç durumlara saygı gösterin.