...
---
# Java — Sözdizimi Referansı
Bu belge Java için kapsamlı, yapılandırılmış bir sözdizimi referansı sağlar. Kapsamlı sözdizimi kalıplarına, operatör tablolarına ve JVM, OOP ve eşzamanlılığın dahili mekaniğine odaklanarak ana Java referansını tamamlar.
---
...
|----------|------|-----------|-----------|-------|
| `+`| İlave | `3 + 2`| `5`| Ayrıca String birleştirme |
| `-`| Çıkarma | `3 - 2`| `1`| |
| `*`| Çarpma | `3 * 2`| `6`| |
| `/`| Bölüm | `7 / 2`| `3`|`int`için tam sayı bölümü; `double`için`3.5`|
| `%`| Kalan | `7 % 2`| `1`| |
...
|----------|------|-----------|-------|
| `==`| Eşit | `x == y`| Nesneler için referans eşitliği |
| `!=`| Eşit Değil | `x != y`| |
| `<`,`>`,`<=`,`>=`| İlişkisel | `x >= y`| |
| `&&`| Mantıksal VE | `a && b`| Kısa devre |
| `\|\|`| Mantıksal VEYA | `a \|\| b`| Kısa devre |
| `!`| Mantıksal DEĞİL | `!true`| `false`|
| `instanceof`| Tip Kontrolü | `obj instanceof String`| Ayrıca desen eşleştirme (Java 16+) |
### Bitsel Operatörler
| Operatör | İsim | Örnek | Notlar |
|----------|------|-----------|-------|
| `&`| VE | `5 & 3`| `1`|
...
| Öncelik | Operatörler |
|---------------|-----------|
| 1 (en yüksek) | Sonek:`expr++``expr--` |
| 2 | Tekli:`++expr``--expr``+expr``-expr``~``!` |
| 3 | Oyuncular:`(Type)expr`|
| 4 | Çarpımsal:`*``/``%`|
| 5 | Katkı maddesi:`+``-` |
| 6 | Kaydırma:`<<``>>``>>>`|
| 7 | İlişkisel:`<``>``<=``>=``instanceof`|
| 8 | Eşitlik:`==``!=` |
| 9 | Bit bazında VE:`&`|
| 10 | Bitsel XOR:`^`|
| 11 | Bit bazında VEYA:`\|`|
...
### Lambdalar ve Fonksiyonel Arayüzler
```java
// Lambda syntax
...
| Arayüz | Uygulamalar | Sipariş edildi | İş Parçacığı Güvenli Varyant |
|---------------------|-----|-----------|----------|
| `List`| `ArrayList`,`LinkedList`| Evet | `CopyOnWriteArrayList`|
| `Set`| `HashSet`,`TreeSet`,`LinkedHashSet`| Değişir | `CopyOnWriteArraySet`|
| `Map`| `HashMap`,`TreeMap`,`LinkedHashMap`| Değişir | `ConcurrentHashMap`|
| `Queue`| `ArrayDeque`,`PriorityQueue`| Değişir | `ConcurrentLinkedQueue`|
| `Deque`| `ArrayDeque`| Her iki uç | `ConcurrentLinkedDeque`|
### Listeler
```java
// Creation
...
## Özet
Java'nın sözdizimi 1995'teki kökeninden bu yana önemli ölçüde gelişmiştir. Modern Java (21+), kayıtları, mühürlü sınıfları, kalıp eşleştirmeyi, sanal iş parçacıklarını ve dize şablonlarını içerir; bu da onu her zamankinden daha kısa ve anlamlı hale getirir. Çekirdek sözdizimi tanıdık olmaya devam ediyor (C benzeri, güçlü bir şekilde yazılmış, sınıf tabanlı), ancak lambdaların, Stream API'nin ve `var`'nin eklenmesi, ortak metni önemli ölçüde azalttı. Dilin gücü devasa ekosisteminde, geriye dönük uyumluluk garantilerinde ve JVM'nin JIT derlemesi yoluyla performans optimizasyonlarında yatmaktadır.