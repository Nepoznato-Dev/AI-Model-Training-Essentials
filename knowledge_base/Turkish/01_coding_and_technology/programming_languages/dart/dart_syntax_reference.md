...
---
# Dart — Sözdizimi Referansı
Bu belge Dart (3.x) için kapsamlı, yapılandırılmış bir sözdizimi referansı sağlar. Kapsamlı sözdizimi kalıplarına, sıfır güvenliğine, eşzamansız programlamaya ve Flutter odaklı tasarım kalıplarına odaklanarak ana Dart referansını tamamlar.
---
...
|----------|------|-----------|-------|
| `+``-``*``/``%``~/` | Aritmetik | `7 ~/ 3`| `~/`tam sayı bölümüdür |
| `==``!=` | Eşitlik | `a == b`| |
| `<``>``<=``>=` | Karşılaştırma | `a >= b`| |
| `&&``\|\|``!`| mantıksal | `a && b`| Kısa devre |
| `??`| Boş birleştirme | `a ?? b`|  `b`, eğer`a`boşsa |
| `?.`| Boş değere sahip erişim | `a?.b`|`a`null ise null |
| `!`| Boş iddia | `a!`| Boşsa atar |
| `..``?..` | Çağlayan | `obj..a..b`| Aynı nesne üzerinde zincirleme işlemler |
| `??=`| Boş değere sahip atama | `a ??= b`| Boşsa ata |
| `as`| Döküm türü | `obj as String`| Başarısızlık üzerine fırlatır |
| `is``is!` | Tip testi | `obj is String`| |
| `=>`| Ok işlevi | `() => expr`| |
### Kademeli Gösterim
```dart
...
## Uzatma Yöntemleri ve Kalıpları
```dart
// Extension methods
...
## Özet
Dart'ın söz dizimi temiz, tutarlı ve üretkenlik için tasarlanmıştır. Sağlam boş güvenlik, derleme zamanında boş referans hatalarını ortadan kaldırır. Eşzamansız/beklemede ve akışlar, eşzamansız programlama için doğal modeller sağlar. Sınıflar, karışımlar ve mühürlü sınıflar esnek nesne kompozisyonuna olanak tanır. Dart 3'ün desen eşleştirmesi, kayıtları ve sınıf değiştiricileri ifadeyi artırır. Dart'ın birincil amacı Flutter olsa da sözdizimi, sunucu tarafı (Dart Frog veya Shelf ile), CLI araçları ve web uygulamaları (JavaScript'e derlenmiş) gibi tüm uygulama etki alanları için çok uygundur.