...
|----------|------|-----------|-------|
| `+``-``*``/``%`| Aritmetik | `a + b`| |
| `..``...` | Menzil | `1...10`| `...`kapalı; `..<`yarı açık |
| `==``!=``<``>``<=``>=` | Karşılaştırma | `a == b`|`Equatable`/`Comparable`gerektirir |
| `===``!==` | Kimlik | `a === b`| Aynı referans (yalnızca sınıflar) |
| `&&``\|\|``!`| mantıksal | `a && b`| Kısa devre |
| `??`| Nil birleşme | `a ?? b`|`a`sıfırsa `b`'yi döndürür |
| `?.`| İsteğe bağlı zincirleme | `a?.b`|`a`sıfır ise sıfır değerini döndürür |
| `!`| Paketi açmaya zorla | `a!`| Sıfırsa kilitlenmeler — kaçının |
| `&`| Giriş parametresi | `swap(&a, &b)`| Referansa göre ilet |
### Nil Handling Operatörleri
```swift
...
## Derinlemesine Opsiyonel Seçenekler
```swift
// Optional is an enum
...
## Özet
Swift'in sözdizimi güvenliği ifade gücüyle dengeler. İsteğe bağlı özellikler boş çökmeleri ortadan kaldırır, protokoller miras olmadan esnek kompozisyona olanak tanır ve modern eşzamanlılık (async/await, aktörler) güvenli eşzamanlı programlamayı erişilebilir hale getirir. Dil, parametre paketleri, makrolar ve sahiplik gibi özelliklerle gelişmeye devam ediyor; her ekleme Swift'i daha güvenli ve daha güçlü hale getirirken aynı zamanda temel vaadini de koruyor: yapısı gereği güvenli kod.