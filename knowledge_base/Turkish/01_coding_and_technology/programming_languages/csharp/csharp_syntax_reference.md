...
Bu belge, Modern C# (.NET 7/8'de C# 10/11/12) için kapsamlı, yapılandırılmış bir sözdizimi referansı sağlar. Kapsamlı sözdizimi kalıplarına, LINQ'ya, eşzamansız/beklemede, kalıp eşleştirmeye ve modern özelliklere odaklanarak ana C# referansını tamamlar.
---
...
|----------|------|-----------|-------|
| `+``-``*``/``%`| Aritmetik | `a + b`| `+`ayrıca dizeleri birleştirir |
| `++``--` | Arttırma/Azaltma | `++i`| Ön artışı tercih et |
| `==``!=` | Eşitlik | `a == b`| Geçersiz kılınabilir; kayıtlar değer eşitliğini kullanır |
| `<``>``<=``>=` | İlişkisel | `a >= b`| |
| `&&``\|\|``!`| mantıksal | `a && b`| Kısa devre |
| `&``\|``^``~` | Bitsel | `a & b`| |
| `<<``>>``>>>`| Shift | `a << 2`| `>>>`işaretsiz sağa kaydırma (C# 11) |
| `??`| Boş birleştirme | `a ?? b`|`a`null ise `b`'yi döndürür |
| `?.`| Boş koşullu | `a?.B`|`a`null ise null değerini döndürür |
| `!`| Boşluğu affeden | `a!.B`| Geçersiz değere sahip uyarıyı bastırır |
| `is`| Desen yazın | `x is string s`| Desen eşleştirme |
| `as`| Güvenli yayın | `x as string`| Başarısızlık durumunda null değerini döndürür |
| `nameof`| Ad tam anlamıyla | `nameof(x)`| `"x"`— derleme zamanı |
| `typeof`| Nesne yazın | `typeof(int)`| `System.Int32`|
| `sizeof`| Bayt cinsinden boyut | `sizeof(int)`| `4`(güvenli olmayan bağlam) |
### Operatör Önceliği (en yüksekten en düşüğe)
| Öncelik | Operatörler |
|---------------|-----------|
| 1 (en yüksek) | Birincil:`()``.``?.``[]``()`(çağır)`++``--` (sonek)`new``typeof``sizeof`|
| 2 | Tekli:`+``-``!``~``++``--` (ön ek)`(T)x``await` |
| 3 | Çarpımsal:`*``/``%`|
| 4 | Katkı maddesi:`+``-` |
| 5 | Kaydırma:`<<``>>``>>>`|
| 6 | İlişkisel:`<``>``<=``>=``is``as` |
...
##LINQ
### Sorgu Sözdizimi ve Yöntem Sözdizimi karşılaştırması
```csharp
...
## Dize Enterpolasyonu ve Biçimlendirme
```csharp
...
## Özet
Modern C#, nesne yönelimli, işlevsel ve bileşen yönelimli programlamanın en iyi yönlerini birleştiren çok paradigmalı bir dildir. Sözdizimi, ayrıntılı Java benzeri koddan, kalıp eşleştirme, kayıtlar, koleksiyon ifadeleri ve birincil oluşturucular içeren kısa ifadelere kadar önemli ölçüde gelişmiştir. LINQ, tüm koleksiyonlarda tek tip bir sorgu API'si sağlar. Async/await, asenkron programlamayı doğal hale getirir. Tip sistemi, null yapılabilir referans türleri, kısıtlamalı jenerikler ve kalıp eşleştirme yoluyla güvenliği pragmatizmle dengeler. C#, her sürümde geriye dönük uyumluluğu korurken ortak metinleri azaltan özellikler ekleyerek hızla gelişmeye devam ediyor.