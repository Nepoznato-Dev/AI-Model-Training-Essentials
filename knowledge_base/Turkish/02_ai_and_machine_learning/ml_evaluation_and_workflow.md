...
| **AUC-ROC** | ROC eğrisinin altındaki alan; TPR ve FPR arasındaki ödünleşim | Eşikten bağımsız genel sınıflandırıcı performansı |
...
- **Grid Search**: Önceden tanımlanmış bir hiperparametre kümesinin tüm kombinasyonlarını kapsamlı bir şekilde deneyin. Simple but computationally expensive.
...
- **Bayes Optimizasyonu**: Hedef fonksiyonun olasılıksal bir modelini oluşturur ve hiperparametreleri akıllıca seçer. Libraries: Optuna, Hyperopt, scikit-optimise.
...
6. **Hata analizi**: Modelin yanlış yaptığı örneklere bakın. Kalıpları belirleyin (ör. nadir sınıflar, belirsiz girdiler) ve içgörüleri veri hazırlama veya özellik mühendisliğine geri aktarın.
---
