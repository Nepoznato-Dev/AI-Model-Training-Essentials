...
| INT4 (4 bit) | 0,5 | ~1,9GB | ~3,5 GB | ~6,5GB | ~35 GB |
**Pratik yönergeler:**
...
- Apple Silicon (birleşik bellek), 64GB+ sistemlerde 70B modellerini çalıştırabilir.
### RAM (Sistem Belleği)
...
- 4 bit niceleme için`bitsandbytes`kullanın (`load_in_4bit=True`).
...
Ön doldurma aşamasının yeniden hesaplanmasını önlemek için benzer istemlerde KV önbelleklerini yeniden kullanın. Bazı çerçeveler bunu destekler (örneğin, vLLM, llama.cpp ve`--prompt-cache`).
