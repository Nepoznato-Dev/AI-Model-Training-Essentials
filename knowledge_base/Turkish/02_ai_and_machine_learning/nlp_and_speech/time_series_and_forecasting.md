...
| **ben (d)** | Sabit hale getirmek için farklı adımların sayısı | Kaç kez fark |
...
| **LSTM tabanlı** | Bir sonraki adımı tahmin edin; büyük tahmin hatası = anormallik | Sıralı anomaliler |
### Uygulamalar
...
## Özet
Zaman serisi tahmini, klasik istatistikleri modern makine öğrenimiyle birleştirir. Klasik yöntemler (ARIMA, üstel düzeltme, Prophet) yorumlanabilir, hızlı ve sıklıkla doğrudur. Derin öğrenme yöntemleri (LSTM, Transformers) karmaşık modelleri yakalar ancak daha fazla veri ve ayarlama gerektirir. Yöntem ne olursa olsun temel ilkeler aynı kalır: verilerinizin yapısını anlayın (trend, mevsimsellik, gürültü), basit bir temel çizgiyle karşılaştırın, uygun ölçümlerle değerlendirin ve geleceğin geçmişi tam olarak kopyalayamayacağı gerçeğini hesaba katın.