...
| **Geçiş işlevi** | P(ler' \| ler, a) | s durumunda a eylemi yapıldıktan sonra s' durumuna ulaşma olasılığı |
...
RLHF olmadan dil modeli, her kitabı okumuş ancak bir konuşma sırasında nasıl davranacağını bilmeyen bir öğrenciye benzer. Metin oluşturabilir ancak metin yararsız olabilir, zehirli olabilir veya asıl noktayı tamamen kaçırabilir. RLHF modele *insanların ne istediğini* öğretir; yalnızca metnin neye benzediğini değil.
...
- **Vektörleştirilmiş ortamlar kullanın.** Birçok ortamı paralel olarak çalıştırmak (ör. 8-64), eğim tahminlerini dengeler ve eğitimi büyük ölçüde hızlandırır.
