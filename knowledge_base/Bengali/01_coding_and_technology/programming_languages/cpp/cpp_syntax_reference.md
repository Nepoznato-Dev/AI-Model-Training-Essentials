...
### পাটিগণিত ও তুলনা অপারেটর
| অপারেটর | নাম | উদাহরণ | নোট |
|----------|------|---------|-------|
| `+``-``*``/``%`| পাটিগণিত | `a + b`| C এর মতই |
| `++``--` | বৃদ্ধি/হ্রাস | `++i`,`i++`| প্রাক-বৃদ্ধি পছন্দ করুন |
| `==``!=``<``>``<=``>=` | তুলনা | `a <=> b`| `<=>`হল ত্রিমুখী তুলনা (C++20) |
| `&&``\|\|``!`| যৌক্তিক | `a && b`| শর্ট সার্কিট |
| `&``\|``^``~``<<``>>` | বিটওয়াইজ | `a & b`| C এর মতই |
| `?:`| টারনারি | `cond ? a : b`| |
### C++-নির্দিষ্ট অপারেটর
| অপারেটর | নাম | উদাহরণ | নোট |
|----------|------|---------|-------|
| `::`| স্কোপ রেজোলিউশন | `std::cout`| |
| `.*``->*` | পয়েন্টার-টু-সদস্য | `obj.*pmf`| |
| `<=>`| মহাকাশযান (C++20) | `a <=> b`| ফেরত দেয়`strong_ordering`|
| `co_await`| করুটিন সাসপেন্ড | `co_await expr`| C++20 |
| `co_yield`| করুটিন ফলন | `co_yield value`| C++20 |
| `co_return`| কোরাটিন রিটার্ন | `co_return value`| C++20 |
### অপারেটর ওভারলোডিং
```cpp
class Vector2D {
    double x_, y_;
...
## নিয়ন্ত্রণ প্রবাহ
### স্ট্রাকচার্ড বাইন্ডিং এবং আধুনিক প্যাটার্ন
```cpp
...
## ফাংশন এবং ল্যাম্বডাস
### আধুনিক ফাংশন সিনট্যাক্স
```cpp
...
## STL কন্টেইনার এবং অ্যালগরিদম
### কন্টেইনার ওভারভিউ
| ধারক | হেডার | আদেশ করা | দেখুন | নোট |
|------------|---------|---------|---------|-------|
| `vector`| `<vector>`| হ্যাঁ | O(n) | সিকোয়েন্সের জন্য ডিফল্ট পছন্দ |
| `deque`| `<deque>`| হ্যাঁ | O(n) | উভয় প্রান্তে দ্রুত ধাক্কা/পপ |
| `list`| `<list>`| হ্যাঁ | O(n) | দ্বিগুণ লিঙ্কযুক্ত তালিকা |
| `array`| `<array>`| হ্যাঁ | O(n) | স্থির-আকার, স্ট্যাক-বরাদ্দ |
| `set`/`multiset`| `<set>`| হ্যাঁ | O(লগ n) | লাল-কালো গাছ |
| `map`/`multimap`| `<map>`| হ্যাঁ (চাবি দ্বারা) | O(লগ n) | কী-মান, লাল-কালো গাছ |
| `unordered_set`| `<unordered_set>`| না | O(1) গড় | হ্যাশ টেবিল |
| `unordered_map`| `<unordered_map>`| না | O(1) গড় | হ্যাশ টেবিল |
### মূল অ্যালগরিদম
```cpp
#include <algorithm>
#include <numeric>
...
## সারাংশ
C++ সিনট্যাক্স নিম্ন-স্তরের পয়েন্টার ম্যানিপুলেশন থেকে উচ্চ-স্তরের টেমপ্লেট মেটাপ্রোগ্রামিং এবং কোরোটিন-ভিত্তিক অ্যাসিঙ্ক পর্যন্ত বিস্তৃত। C++17/20/23 এর মাধ্যমে ভাষার বিবর্তন নাটকীয়ভাবে নিরাপত্তার উন্নতি করেছে (স্মার্ট পয়েন্টার,`std::optional`, ধারণা), অভিব্যক্তি (গঠিত বাঁধাই, রেঞ্জ, `std::format`), এবং কর্মক্ষমতা (ধারণাগুলি শূন্য-খরচ বিমূর্ততাকে সক্ষম করে, ওভারহেডের জন্য স্ট্রাকশন ছাড়াই)। মূল দর্শনটি অপরিবর্তিত রয়েছে: আপনি যা ব্যবহার করেন না তার জন্য আপনাকে অর্থ প্রদান করা উচিত নয় এবং ভাষাটি বেয়ার মেটাল থেকে ডোমেন-নির্দিষ্ট ভাষা পর্যন্ত বিমূর্ততার প্রতিটি স্তরকে সমর্থন করবে।