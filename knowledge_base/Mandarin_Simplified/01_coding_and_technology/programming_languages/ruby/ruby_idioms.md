...
＃＃ 概括
Ruby 惯用语强调：开发人员的幸福感、约定优于配置、块和迭代器、鸭子类型和表达语法。遵循 Ruby 样式指南，使用 RuboCop 进行 linting，使用 StandardRB 进行固执己见的格式化。 Ruby 重视可读性——代码读起来应该像英语一样。优先使用可枚举方法而不是手动循环，使用符号作为标识符，并使用块进行迭代和资源管理。