...
---
# Ruby — 常见错误和反模式
本文档列出了 Ruby 中最常见的错误、陷阱和反模式。每个条目都会显示错误的方法，解释其失败的原因，并提供正确的解决方案。
---
...
＃＃ 概括
Ruby 的表现力可能会导致反模式：在迭代期间修改集合、猴子修补核心类、全局变量和胖模型。 Ruby 方式是：使用枚举器（`each`、`map`、`reject`）、首选符号和关键字参数、冻结字符串文字、从模型中提取服务对象以及使用细化而不是猴子补丁。 Ruby 奖励那些拥抱块、鸭子类型和最小意外原则的开发者。