...
# Delphi / Object Pascal — 常见错误和反模式
本文档列出了 Delphi/Object Pascal 中最常见的错误、陷阱和反模式，并进行了更正。
---
...
＃＃ 概括
Delphi 陷阱：始终使用 try/finally 释放对象，区分 `=`（比较）和 `:=`（赋值），字符串索引为 1，在构造函数中调用 `inherited`，并将业务逻辑与 UI 分开。 Delphi 的 VCL/FMX 框架奖励干净的关注点分离。