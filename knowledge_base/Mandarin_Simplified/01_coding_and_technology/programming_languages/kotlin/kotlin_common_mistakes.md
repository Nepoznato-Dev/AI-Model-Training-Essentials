...
## 3.`data class`没有适当的`equals`/ `hashCode`
```kotlin
...
＃＃ 概括
Kotlin 的简洁性可以隐藏微妙的问题：Java 互操作的平台类型、`lateinit` 崩溃、可变数据类破坏集合以及协程取消。 Kotlin 的方法是：拥抱空安全（永远不要使用`!!`），使用正确的作用域函数（`let`用于转换，`apply`用于配置，`also`用于副作用），使用具有详尽`when`的密封类，处理协程取消，并编写惯用的 Kotlin （不是使用 Kotlin 语法的 Java）。编译器是你的向导——如果它发出警告，请听。