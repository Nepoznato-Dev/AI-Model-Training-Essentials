...
---
# R — 语法参考
本文档提供了 R (4.x) 的全面、结构化语法参考。它通过关注详尽的语法模式、tidyverse 生态系统、数据操作、统计建模和可视化来补充主要的 R 参考。
---
...
| `<<-`|全局分配 | `x <<- 10`|在父环境中分配 |
| `$`|提取元素| `df$column`| |
| `[``[[` |索引 | `x[1]`/`x[[1]]`| `[`返回相同类型； `[[`摘录 |
| `%>%`|管道 (magrittr) | `x %>% f()`| |
| `\|>`|管道（基础 R 4.1+）| `x \|> f()`|原生管道|
### 向量化运算
```r
...
＃＃ 概括
R 的语法是为统计计算和数据分析而设计的。操作的矢量化性质消除了大多数任务的循环。 tidyverse 生态系统（dplyr、ggplot2、tidyr）为数据操作和可视化提供了一致、可读的语法。 R 的统计建模功能（从基本 t 检验到混合效应模型）在深度上是无与伦比的。对于数据科学、统计学和可重复研究，R 仍然是一个重要的工具。