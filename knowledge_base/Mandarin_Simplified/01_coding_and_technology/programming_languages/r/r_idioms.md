...
＃＃ 概括
R 习语强调：矢量化（避免循环）、tidyverse 管道 (`|>`)、用于赋值的 `<-`、snake_case 命名以及使用`purrr`进行函数式编程。遵循 Tidyverse 样式指南，使用 lintr 进行 linting，使用 styler 进行格式化。 R 值向量化操作和用于可读数据转换的管道。