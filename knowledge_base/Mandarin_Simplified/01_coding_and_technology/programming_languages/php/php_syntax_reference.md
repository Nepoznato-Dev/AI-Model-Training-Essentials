...
---
# PHP — 语法参考
本文档提供了 PHP (8.x) 的全面、结构化语法参考。它通过关注详尽的语法模式、现代 PHP 功能、面向对象编程和实用习惯用法来补充主要的 PHP 参考。
---
...
| `<``>``<=``>=` |比较| `$a >= $b`| |
| `&&``\|\|``!`|逻辑 | `$a && $b`|短路|
| `and``or``xor`|逻辑（低优先级）| `$a and $b`|避免 — 使用`&&`/`\|\|`|
| `??`|空合并 | `$a ?? 'default'`|如果`$a`为 null，则返回`'default'`|
| `??=`|空合并赋值 | `$a ??= 'default'`|仅当`$a`为 null 时才分配 |
| `?``:` |三元| `$a ? $b : $c`| |
| `.`|连接 | `$a . $b`| |
| `.=`|连接赋值| `$a .= $b`| |
| `=>`|数组赋值 | `['key' => 'value']`| |
| `->`|会员访问 | `$obj->method()`| |
| `?->`|空安全运算符 | `$obj?->method()`|如果`$obj`为 null，则返回 null |
| `::`|静态访问 | `ClassName::method()`| |
| `instanceof`|类型检查 | `$obj instanceof ClassName`| |
| `clone`|对象克隆| `$copy = clone $obj`|深拷贝需要`__clone()` |
### 类型杂耍
```php
...
＃＃ 概括
PHP 的语法已经从其脚本起源戏剧性地发展成为一种现代的、类型安全的语言。 PHP 8.x 引入了联合类型、交集类型、枚举、纤程、只读类、属性和一流的可调用对象，将其从松散类型的脚本语言转变为支持严格软件工程的语言。该语言的优势仍然是其 Web 焦点：凭借 Laravel 和 Symfony 等框架、庞大的生态系统和近乎普遍的托管支持，PHP 的语法为数百万个应用程序提供服务。现代 PHP 奖励那些拥抱其类型系统、使用基于 Composer 的自动加载以及编写干净、面向对象的代码的开发人员。