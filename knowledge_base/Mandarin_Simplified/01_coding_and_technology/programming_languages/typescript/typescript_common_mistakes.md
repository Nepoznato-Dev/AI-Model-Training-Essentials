...
# TypeScript — 常见错误和反模式
本文档列出了 TypeScript 中最常见的错误、陷阱和反模式。每个条目都会显示错误的方法，解释其失败的原因，并提供正确的解决方案。
---
...
＃＃ 概括
TypeScript 的类型系统很强大，但需要纪律。主要的罪过是使用`any`——当类型确实未知时使用 `unknown`，并且使用类型保护来缩小范围。通过正确处理未定义来避免非空断言（`!`）。使用对象形状的接口、联合的类型和计算类型。受歧视的联合消除了整个类别的错误。从第一天起就启用`strict: true`— 改造的严格性是痛苦的。目标是使非法状态在类型级别上无法代表。