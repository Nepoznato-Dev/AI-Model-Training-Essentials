...
## 装饰器（TC39 第 3 阶段 / TypeScript 5.0+）
```typescript
// Class decorator
...
＃＃ 概括
TypeScript 的类型系统是其定义特征，而且非常深入。从基本注释到泛型、条件类型、映射类型、模板文字类型和`infer`，类型系统本质上是一种在编译时运行的函数式语言。了解`interface`与`type`、泛型与实用类型以及结构类型与名义类型之间的相互作用有助于编写既安全又符合人体工程学的 TypeScript。该语言不断发展，每个版本都增加了更多的表达能力，同时保持与 JavaScript 的完全向后兼容性。