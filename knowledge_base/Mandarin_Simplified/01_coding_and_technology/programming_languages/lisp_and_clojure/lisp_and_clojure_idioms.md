...
# Lisp 和 Clojure — 惯用模式和最佳实践
本指南涵盖 Lisp 和 Clojure 的惯用模式。
---
...
＃＃ 概括
Clojure 习惯用法强调：不可变数据、线程宏（`->`、`->>`）、解构、关键字作为函数以及验证规范。 Common Lisp 习语强调：`with-open` 用于资源，`push` /`nreverse`用于列表构建，以及多个值。两者都重视同像性和 REPL 驱动的开发。