...
---
# Lisp 和 Clojure — 语法参考
本文档为 Common Lisp 和 Clojure 提供了全面、结构化的语法参考。它通过关注 S 表达式、宏、持久数据结构和函数式编程模式来补充主要的 Lisp/Clojure 参考。
---
...
＃＃ 概括
Clojure 的语法统一带括号——每种形式都是一个列表。这种同像性使得强大的宏能够将代码转换为数据。持久数据结构提供不变性和结构共享以提高效率。线程宏创建可读的数据管道。并发原语（原子、引用、代理、通道）安全地处理状态更改。 Lisp/Clojure 对现代编程的影响是巨大的——垃圾收集、REPL、宏和函数式编程都可以追溯到 Lisp。