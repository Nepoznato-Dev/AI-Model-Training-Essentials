...
-->
# Go — 语法参考
本文档提供了全面、结构化的 Go 语法参考。它通过关注详尽的语法模式、运算符表以及 goroutine、通道和接口的内部机制来补充主要的 Go 参考。
---
...
＃＃ 概括
Go 的语法有意做到极简——没有继承、没有方法重载、没有异常、没有宏。剩下的是一组干净、正交的构造：用于组合的结构和接口、用于并发的 goroutine 和通道，以及用于可靠性的显式错误处理。该语言的力量不是来自复杂的语法，而是来自其简单原语的优雅交互。泛型（Go 1.18+）增加了类型灵活性，同时又不牺牲 Go 特有的清晰度。