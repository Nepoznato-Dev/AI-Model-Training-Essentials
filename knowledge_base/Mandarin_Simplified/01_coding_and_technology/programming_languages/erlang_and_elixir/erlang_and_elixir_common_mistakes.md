...
---
# Erlang 和 Elixir — 常见错误和反模式
本文档列出了 Erlang 和 Elixir 中最常见的错误、陷阱和反模式，并进行了更正。
---
...
＃＃ 概括
Erlang/Elixir 陷阱：变量是单赋值的，进程需要链接/监视器，GenServer 调用需要显式超时，`receive` 永远阻塞而没有超时，GenServer 回调不应阻塞。 BEAM 方式是：让它崩溃（在监督下），使用`with`进行干净的模式匹配，保持 GenServer 回调快速，并始终处理进程失败。