...
＃＃ 概括
Rust 习惯用法强调：所有权和借用、用于错误处理的`Result`/ `Option`、带有方法的枚举、抽象特征、迭代器链、用于共享状态的智能指针和零成本抽象。按照`rustfmt`进行格式化，按照`clippy`进行 linting，以及 Rust API 指南。 Rust 社区重视安全性、性能和正确性——“如果它能编译，它就能工作。”在函数参数中优先使用`&str`而不是 `String`，使用`?`进行错误传播，并使用`#[derive(Debug, Clone, ...)]`派生共同特征。