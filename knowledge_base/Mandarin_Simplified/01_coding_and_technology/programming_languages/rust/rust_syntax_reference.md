...
# Rust — 语法参考
本文档为 Rust 提供了全面、结构化的语法参考。它通过关注详尽的语法模式、运算符表以及所有权系统、特征和并发的内部机制来补充主要的 Rust 参考。
---
...
|类型 |堆栈/堆|尺寸|可变性 |笔记|
|------|------------|------|------------|--------|
|基元（`i32`、`f64`、`bool`、`char`） |堆栈|固定|默认情况下不可变 | `Copy`|
|  `[T; N]`（数组）|堆栈|固定|与`mut`可变 |固定大小，如果 T 是 `Copy`，则`Copy`|
| `Vec<T>`|堆|动态 |与`mut`可变 |可成长|
| `String`|堆|动态 |与`mut`可变 | UTF-8，可增长 |
|  `&[T]`（切片）|堆栈（胖指针）|动态视图|取决于`&`或`&mut`|借用观点|
| `HashMap<K, V>`|堆|动态 |与`mut`可变 | O(1) 平均查找 |
| `BTreeMap<K, V>`|堆|动态 |与`mut`可变 | O(log n) 查找，有序 |
| `HashSet<T>`|堆|动态 | — | O(1) 平均，无重复 |
### 向量
```rust
// Creation
...
＃＃ 概括
Rust 的语法建立在编译时强制执行的安全保证的基础上。所有权系统、基于特征的多态性、模式匹配和零成本抽象形成了一个连贯的设计，其中编译器是一个严格但公平的合作伙伴。从基本控制流到宏、不安全代码和智能指针等高级功能，每个构造都服务于安全、高效的系统编程的目标。理解生命周期、借用和类型系统之间的相互作用是掌握 Rust 的关键。