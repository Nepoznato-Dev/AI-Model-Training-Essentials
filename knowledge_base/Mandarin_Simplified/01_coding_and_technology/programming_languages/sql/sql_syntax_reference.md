...
＃＃ 概括
SQL 的语法 50 年来一直保持稳定，同时不断发展。核心查询（SELECT、JOIN、WHERE、GROUP BY）是所有数据库的标准查询。窗口函数、CTE 和 JSON 支持代表了现代 SQL 的功能。有效的 SQL 使用得益于了解数据库引擎如何处理查询 - 读取执行计划、理解索引以及编写基于集合的操作而不是过程循环。强大的 SQL 技能对于任何处理数据的人都很有价值。