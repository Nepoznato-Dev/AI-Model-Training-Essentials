...
| | `+=`|追加| `s &= " more"`|
| **作业** | `=`|分配| `x = 42`|
| **空** | `Is`|参考平等| `obj Is Nothing`|
| | `IsNot`|参考不等式| `obj IsNot Nothing`|
| **类型** | `TryCast`|安全转换（不返回任何内容）| `TryCast(obj, String)`|
| | `CType`|转换 | `CType(obj, Integer)`|
| | `DirectCast`|直接投射（失败时抛出）| `DirectCast(obj, String)`|
---
...
＃＃ 概括
Visual Basic 的语法简洁且类似于英语，使其成为最易读的编程语言之一。 VB.NET 提供对 .NET 生态系统的完全访问，包括用于数据查询的 LINQ、用于并发操作的异步/等待、用于类型安全的泛型以及具有类、接口和继承的综合 OOP。该语言的不区分大小写和详细关键字（End If、End Sub、End Class）增强了可读性。对于企业 Windows 应用程序、办公自动化 (VBA) 和 .NET 中的快速原型设计，VB.NET 仍然是一种高效且功能强大的语言。