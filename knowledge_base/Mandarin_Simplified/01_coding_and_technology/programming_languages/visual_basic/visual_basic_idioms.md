...
＃＃ 概括
Visual Basic 习惯用法强调：用于类型安全的 `Option Strict On`、用于资源管理的`Using`块、字符串插值、用于集合的 LINQ、贯穿始终的 `Async/Await`、特定异常处理以及模式匹配和 null 条件运算符等现代功能。 VB 重视可读性——“代码读起来应该像组织良好的规范。”