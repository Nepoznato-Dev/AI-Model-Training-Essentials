...
# JavaScript — 惯用模式和最佳实践
本指南涵盖了编写干净、现代 JavaScript (ES2024+) 代码的惯用模式和最佳实践。
---
...
＃＃ 概括
现代 JavaScript 习语强调：`const`/`let`优于`var`、解构、箭头函数、模板文字、异步/等待、扩展/休息运算符、数组方法 (`map`/`filter`/`reduce`)、ES 模块、可选链接 (`?.`)、无效合并 (`??`) 以及具有私有成员的类字段。遵循 ESLint 的代码质量，遵循 Prettier 的格式，并且更喜欢函数模式而不是可变状态。 JavaScript 社区重视简洁性、可组合性和现代 ES2024+ 功能。