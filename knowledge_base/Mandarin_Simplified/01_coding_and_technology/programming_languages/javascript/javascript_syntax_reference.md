...
|操作员|名称 |示例|笔记|
|----------|------|---------|--------|
| `===`|严格平等| `1 === 1`|无类型强制 - 总是更喜欢这个 |
| `!==`|严格的不平等| `1 !== "1"`| `true`— 不同类型 |
| `==`|抽象平等| `1 == "1"`|类型强制 — 避免 |
| `!=`|抽象不平等| `1 != "1"`| `false`— 避免 |
|  `<`、`>`、`<=`、`>=` |关系 | `"b" > "a"`|字符串词典 |
### 逻辑运算符和空运算符
|操作员|名称 |示例|笔记|
|----------|------|---------|--------|
| `&&`|逻辑与| `a && b`|返回第一个假值或最后一个值 |
| `\|\|`|逻辑或 | `a \|\| b`|返回第一个真值或最后一个值 |
| `!`|逻辑非 | `!true`| `false`|
| `??`|空合并 | `a ?? b`|仅当`a`为`null`/`undefined`时才返回`b`|
| `?.`|可选链接 | `obj?.prop?.method()`|`undefined`短路 |
### 位运算符
|操作员|名称 |示例|笔记|
|----------|------|---------|--------|
| `&`|和| `5 & 3`| `1`|
...
|------------|---------|---------|------------|---------|--------|
| `Array`|是的 |是的 |是的 |索引（数量）| O(n) |
| `Object`|是（插入）|是的 |按键： 否 |字符串/符号 | O(1) 平均 |
| `Map`|是（插入）|是的 |按键： 否 |任何 | O(1) 平均 |
| `Set`|是（插入）|是的 |没有 | — | O(1) 平均 |
| `WeakMap`|没有 |是的 |按键： 否 |仅限对象 | O(1) 平均 |
| `WeakSet`|没有 |是的 |没有 |仅限对象 | O(1) 平均 |
### 数组
```javascript
// Creation
...
＃＃ 概括
JavaScript 的语法表面上看似简单，但内涵却很深。该语言已从简单的脚本工具发展成为具有类、模块、生成器、代理和复杂的异步模型的多范式语言。了解运算符优先级、事件循环、原型链以及`this`绑定的细微差别，将有能力的开发人员与专家开发人员区分开来。具有可选链接、无效合并、`Promise.allSettled`和顶级等待的现代 JavaScript (ES2020+) 继续减少样板文件，同时保持向后兼容性。