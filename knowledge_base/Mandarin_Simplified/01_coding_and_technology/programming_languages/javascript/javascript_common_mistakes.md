...
---
# JavaScript — 常见错误和反模式
本文档列出了 JavaScript 中最常见的错误、陷阱和反模式。每个条目都会显示错误的方法，解释其失败的原因，并提供正确的解决方案。
---
...
＃＃ 概括
JavaScript 的灵活性伴随着陷阱：`this` 绑定取决于调用站点，`==` 执行强制，promise 需要错误处理，并且事件循环顺序（微任务在宏任务之前）不明显。现代规则很简单：使用`const`/`let`绝不使用`var`，更喜欢`===`而不是`==`，始终处理承诺拒绝，使用箭头函数进行回调，避免突变，并清理事件侦听器。遵循这些规则可以消除大多数 JavaScript 错误。