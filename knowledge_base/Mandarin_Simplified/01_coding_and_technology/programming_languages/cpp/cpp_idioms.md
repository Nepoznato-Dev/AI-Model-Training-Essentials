...
-->
# C++ — 惯用模式和最佳实践
本指南涵盖了编写简洁、现代 C++ (20/23) 代码的惯用模式和最佳实践。
---
...
＃＃ 概括
现代 C++ 习惯用法强调：RAII 和智能指针、移动语义、用于类型推导的 `auto`、范围和算法 (C++20)、模板约束的概念、用于缺失/错误的`std::optional`/ `std::expected`、零规则和用于编译时计算的 `constexpr`。按照`clang-format`进行格式化，按照`clang-tidy`进行 linting，并使用`-Wall -Wextra -Werror`进行编译。 C++ 社区重视零成本抽象，并且“不要为不使用的东西付费”。