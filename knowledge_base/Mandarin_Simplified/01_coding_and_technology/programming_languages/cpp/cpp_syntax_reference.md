...
| `::`|范围解析 | `std::cout`| |
| `.*``->*` |指向成员的指针 | `obj.*pmf`| |
| `<=>`|宇宙飞船 (C++20) | `a <=> b`|返回`strong_ordering`|
| `co_await`|协程挂起 | `co_await expr`| C++20 | C++20
| `co_yield`|协程产量 | `co_yield value`| C++20 | C++20
| `co_return`|协程返回| `co_return value`| C++20 | C++20
### 运算符重载
```cpp
class Vector2D {
    double x_, y_;
...
|集装箱|标题 |已订购 |查找 |笔记|
|------------|--------|---------|--------|--------|
| `vector`| `<vector>`|是的 | O(n) |序列的默认选择 |
| `deque`| `<deque>`|是的 | O(n) |两端快速推/弹出 |
| `list`| `<list>`|是的 | O(n) |双向链表|
| `array`| `<array>`|是的 | O(n) |固定大小、堆栈分配 |
| `set`/`multiset`| `<set>`|是的 | O(log n) |红黑树|
| `map`/`multimap`| `<map>`|是（按键）| O(log n) |键值、红黑树|
| `unordered_set`| `<unordered_set>`|没有 | O(1) 平均 |哈希表|
| `unordered_map`| `<unordered_map>`|没有 | O(1) 平均 |哈希表|
### 关键算法
```cpp
#include <algorithm>
#include <numeric>
...
＃＃ 概括
C++ 语法涵盖从低级指针操作到高级模板元编程和基于协程的异步。该语言通过 C++17/20/23 的演变极大地提高了安全性（智能指针、`std::optional`、概念）、表现力（结构化绑定、范围、`std::format`）和性能（实现零成本抽象的概念、无需堆栈开销的异步协程）。核心理念保持不变：您不应该为不使用的东西付费，并且该语言应该支持从裸机到特定于领域的语言的每个抽象级别。