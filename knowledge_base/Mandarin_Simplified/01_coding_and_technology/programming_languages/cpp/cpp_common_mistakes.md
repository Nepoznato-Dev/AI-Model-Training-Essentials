...
-->
# C++ — 常见错误和反模式
本文档列出了 C++ 中最常见的错误、陷阱和反模式。每个条目都会显示错误的方法，解释其失败的原因，并提供正确的解决方案。现代 C++ (C++11/14/17/20) 提供了避免大多数经典陷阱的工具。
---
...
## 7. 反模式：在标头中使用 `using namespace std;`
```cpp
...
＃＃ 概括
C++ 的复杂性创造了许多犯错的机会：五法则、对象切片、迭代器失效、悬空引用以及有符号溢出导致的未定义行为。现代 C++ 是解药：使用智能指针代替原始`new`/`delete`，对所有资源使用 RAII，使用`std::vector`和`std::string`代替原始数组，移动语义以提高效率，并使用`[[nodiscard]]`来捕获被忽略的返回值。 C++ 核心指南的存在是有原因的——遵循它们，并使用 Clang-Tidy 和清理工具等工具来捕捉你错过的内容。