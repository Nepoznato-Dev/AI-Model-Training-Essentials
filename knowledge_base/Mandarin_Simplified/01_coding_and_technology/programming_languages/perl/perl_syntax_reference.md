...
| `==``!=``<``>``<=``>=` |数值比较| `$a == $b`| |
| `eq``ne``lt``gt``le``ge` |字符串比较 | `$a eq $b`| |
| `<=>`|数字宇宙飞船| `$a <=> $b`|返回 -1, 0, 1 |
| `cmp`|弦线宇宙飞船| `$a cmp $b`| |
| `&&``\|\|``!`|逻辑 | `$a && $b`| |
| `and``or``not`|逻辑（低精度）| `$a and $b`|避免 — 使用`&&`/`\|\|`|
| `=~``!~` |正则表达式匹配 | `$str =~ /pattern/`| |
| `..``...` |范围/触发器| `1..10`| |
| `?:`|三元| `$a ? $b : $c`| |
| `//`|定义或 | `$a // $b`| `$b`如果`$a`未定义 |
| `=>`|胖逗号 | `key => 'value'`|自动引用裸字 |
### Sigils — 变量类型
```perl
...
＃＃ 概括
Perl 的语法是围绕上下文、符号和“有不止一种方法可以做到这一点”的原则构建的。语言中内置了正则表达式，使得文本处理的简洁性无与伦比。引用支持复杂的数据结构。 CPAN 为一切提供模块。现代 Perl（带有`strict`、`warnings`和词法变量）干净且可维护。 Perl 的遗产在每种现代语言所采用的模式和工具中得以延续。