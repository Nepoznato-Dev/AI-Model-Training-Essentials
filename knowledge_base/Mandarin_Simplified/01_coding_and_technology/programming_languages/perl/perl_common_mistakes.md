...
# Perl — 常见错误和反模式
本文档列出了 Perl 中最常见的错误、陷阱和反模式，并进行了更正。
---

## 1. 不使用`strict`和 `warnings`
```perl
...
＃＃ 概括
Perl 的灵活性是危险的：总是`use strict; use warnings;`，理解上下文（标量与列表），使用词法变量（`my`），使用嵌套数据的引用，并拥抱现代 Perl 功能（签名，`say`，`//`）。 Perl 奖励那些明智地使用其丰富的正则表达式引擎和 CPAN 生态系统的人。