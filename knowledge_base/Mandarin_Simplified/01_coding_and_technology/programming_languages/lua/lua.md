...
## 为什么 Lua 很重要
- **可嵌入**：设计为嵌入其他应用程序 - 主机提供功能。
- **占用空间小**：整个解释器大小约为 120KB。非常适合嵌入式系统。
- **快速**：最快的解释脚本语言之一。
- **简单**：只有约 20 个关键字。易于学习和集成。
- **游戏开发**：许多游戏引擎和平台的标准脚本语言。
- **Roblox**：为整个 Roblox 生态系统提供动力 - 数以百万计的用户创建的游戏。
## 权衡
|限制|详情 |典型解决方法|
|------------|---------|--------------------|
| **标准库有限** |最少的内置功能 |使用 C/C++ 扩展或使用 LuaRocks 包 |
| **基于 1 的索引** |数组从索引 1 开始（对于程序员来说不常见）|接受作为设计选择；始终如一 |
| **没有课程** |只有表和元表——OOP必须手动实现|使用元表或 OOP 库 |
| **游戏之外的利基** |在网络、数据科学或企业中的有限使用 |用于脚本/嵌入；其他语言的应用程序 |
| **就业市场小** |主要是游戏开发和嵌入式角色 | Roblox 开发是一个不断增长的利基市场 |
---
...
＃＃ 概括
Lua 是典型的嵌入语言。它体积小、速度快且简单，旨在驻留在其他应用程序中并为它们提供脚本编写功能。对于游戏开发、Roblox 和嵌入式系统，Lua 是一个绝佳的选择。它不是一种通用语言，但就其特定的利基（脚本和嵌入）而言，它几乎是无与伦比的。