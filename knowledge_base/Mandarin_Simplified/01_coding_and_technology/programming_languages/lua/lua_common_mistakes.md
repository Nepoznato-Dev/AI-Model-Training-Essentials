...
# Lua — 常见错误和反模式
本文档列出了 Lua 中最常见的错误、陷阱和反模式，并进行了更正。
---
...
＃＃ 概括
Lua 的极简主义造成了陷阱：基于 1 的索引，只有`nil`和`false`是假的，表是引用（不是值），变量默认是全局的，`pairs` 是无序的，而`ipairs`是有序的。 Lua 方式是：始终使用`local`，对数组使用`ipairs`，对字典使用`pairs`，对字符串构建使用`table.concat`，对 OOP 使用元表。 Lua 的简单性就是它的力量——但要尊重边缘情况。