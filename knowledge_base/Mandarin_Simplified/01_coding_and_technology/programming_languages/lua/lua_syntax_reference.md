...
|----------|------|---------|--------|
| `+``-``*``/``%``^` |算术| `2 ^ 10`| `^`是求幂 |
| `//`|楼层划分| `7 // 2`|返回 3 (Lua 5.3+) |
| `==``~=` |平等| `a == b`| `~=`是“不等于”|
| `<``>``<=``>=` |比较| `a >= b`| |
| `and``or``not`|逻辑 | `a and b`|返回操作数，而不是布尔值 |
| `..`|连接 | `"hello" .. " world"`| |
| `#`|长度 | `#t`|字符串或表的长度 |
| `&``\|``~``<<``>>`|按位| `a & b`| Lua 5.3+（整数运算）|
### 逻辑运算符 — 非布尔返回
...
＃＃ 概括
Lua 的语法刻意极简——整个语言可以在一个页面中描述。表作为通用数据结构（数组、字典、对象、模块）。元表提供了可扩展性，但没有语言复杂性。协程支持协作式多任务处理和优雅的迭代器模式。该语言的强大之处在于其可嵌入性：Lua 被设计为驻留在主机应用程序中，以最小的开销提供脚本功能。对于游戏开发、配置和嵌入式系统，Lua 的简单性是其最大的优势——一种小语言，可以准确地完成所需的任务，仅此而已。