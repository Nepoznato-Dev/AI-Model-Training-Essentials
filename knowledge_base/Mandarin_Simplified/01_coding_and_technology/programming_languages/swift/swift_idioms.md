...
＃＃ 概括
Swift 习惯用法强调：安全性（可选、防护）、值类型（结构）、面向协议的设计、模式匹配、异步/等待和不变性（默认为 `let`）。遵循 Swift API 设计指南，使用 SwiftLint 保证代码质量，使用 SwiftFormat 进行格式化。 Swift 重视安全性和表现力——“如果它能编译，它可能是正确的。”