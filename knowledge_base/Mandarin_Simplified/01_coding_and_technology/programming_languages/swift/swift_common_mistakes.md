...
---
# Swift — 常见错误和反模式
本文档列出了 Swift 中最常见的错误、陷阱和反模式。每个条目都会显示错误的方法，解释其失败的原因，并提供正确的解决方案。
---
...
＃＃ 概括
Swift 的安全功能可以防止许多错误，但也引入了自己的陷阱：强制展开会导致崩溃，保留循环隐藏在闭包和委托中，而值类型语义则让来自引用类型语言的开发人员感到惊讶。 Swift 的方法是：永远不要强制展开 (`!`)，使用`guard`进行提前退出，使用`weak`进行委托和闭包捕获，使用`@MainActor`标记 UI 代码，并将大量视图控制器分解为视图模型和服务。 Swift 的类型系统是你的朋友——使用它，而不是反对它。