...
# Python — 语法参考
本文档提供了全面、结构化的 Python 语法参考。它通过关注详尽的语法模式、运算符表和内部机制来补充主要的 Python 参考。
---
...
| `-`|减法| `3 - 2`| `1`|也是一元否定 |
| `*`|乘法| `3 * 2`| `6`|还有字符串/列表重复 |
| `/`|真正的分裂 | `7 / 2`| `3.5`|始终返回浮点数 |
| `//`|地板事业部| `7 // 2`| `3`|朝负无穷方向舍入 |
| `%`|模数| `7 % 2`| `1`|符号匹配除数|
| `**`|求幂| `2 ** 10`| `1024`|右结合|
### 比较和布尔运算符
|操作员|名称 |示例|笔记|
...
| `!=`|不等于| `x != y`| |
|  `<`、`>`、`<=`、`>=` |订购 | `x >= y`|可链接：`a < b < c` |
| `is`|身份| `x is y`|内存中的同一对象|
| `is not`|不是身份| `x is not None`|优于`!= None`|
| `in`|会员资格 | `x in collection`|对于集合/字典，O(1)；对于列表，O(n) |
| `and`|逻辑与| `a and b`|短路；返回最后一个真值/假值 |
| `or`|逻辑或 | `a or b`|短路；返回第一个真/假值 |
| `not`|逻辑非 | `not x`| |
### 位运算符
|操作员|名称 |示例|笔记|
|----------|------|---------|--------|
...
| `tuple`|是的 |没有 |是的 | `(1, 2, 3)`| O(n) |
| `dict`|是（3.7+）|是的 |按键： 否 | `{"k": "v"}`| O(1) 平均 |
| `set`|没有 |是的 |没有 | `{1, 2, 3}`| O(1) 平均 |
| `frozenset`|没有 |没有 |没有 | `frozenset({1,2})`| O(1) 平均 |
### 列表
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
＃＃ 概括
此语法参考涵盖了 Python 的完整功能集 - 从基本运算符和控制流到高级元编程、描述符和类型系统。 Python 的语法刻意简洁且可读，但其深度却很丰富：模式匹配、异步生成器、协议和元类为复杂应用程序提供了强大的工具。该语言不断发展，每个版本都添加了提高表达能力同时保持向后兼容性的功能。