...
| `=`|作业 | `x=10`|`=`周围没有空格 |
| `$var`|变量扩展| `echo $HOME`| |
| `${var}`|支撑扩张 | `${HOME}/docs`|消除歧义 |
| `$(cmd)`|命令替换 | `$(date +%Y)`|优先于反引号 |
| `$((expr))`|算术| `$((2 + 3))`| |
| `${var:-default}`|默认值 | `${PORT:-8080}`|如果未设置或为空 |
| `${var:=default}`|分配默认| `${count:=0}`| |
| `${#var}`|字符串长度 | `${#name}`| |
| `${var%%pattern}`|删除最长的后缀 | `${file%%.*}`|删除扩展 |
| `${var##pattern}`|删除最长的前缀 | `${path##*/}`|获取文件名 |
| `"``"` |双引号| `"$var"`|允许扩展 |
| `'``'` |单引号 | `'$var'`|文字字符串|
### 测试操作员
```bash
...
＃＃ 概括
Bash 和 PowerShell 代表了 shell 脚本的两种范例。 Bash 管道文本——每个命令都会转换字符串。 PowerShell 管道对象——每个命令都会生成带有属性和方法的结构化数据。 Bash 在 Linux/macOS 和 DevOps 中占据主导地位。 PowerShell 对于 Windows 管理和日益跨平台至关重要。两者都是现代技术堆栈中必不可少的工具。 Shell 脚本是连接系统、自动化工作流程和完成工作的粘合剂。