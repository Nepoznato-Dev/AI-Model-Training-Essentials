...
-->
# C# — 常见错误和反模式
本文档列出了 C# 中最常见的错误、陷阱和反模式。每个条目都会显示错误的方法，解释其失败的原因，并提供正确的解决方案。
---

## 1. 不处理`IDisposable`对象
```csharp
// ❌ WRONG — resource leak
...
＃＃ 概括
C# 是一种设计良好的语言，但也有陷阱：不处理`IDisposable`对象、async void 方法、不必要的 LINQ 分配和空引用异常。现代 C# (8+) 提供了强大的工具：可空引用类型、模式匹配、记录、using 声明和 switch 表达式。关键原则：始终处置资源，从不使用 async void（事件处理程序除外），使用构造函数注入，更喜欢数据记录，并启用可空引用类型以在编译时捕获空错误。