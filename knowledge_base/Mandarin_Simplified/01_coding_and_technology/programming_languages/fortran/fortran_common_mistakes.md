...
---
# Fortran — 常见错误和反模式
本文档列出了 Fortran 中最常见的错误、陷阱和反模式，并进行了更正。
---
...
＃＃ 概括
Fortran 的科学计算重点创造了陷阱：基于 1 的索引、隐式类型（始终使用`implicit none`）、数组操作与循环以及意图规范。现代 Fortran (2003+) 有模块、派生类型和 OOP——使用它们。 Fortran 方法是：始终 `implicit none`，为所有参数指定 `intent`，使用数组操作，并释放您分配的内容。