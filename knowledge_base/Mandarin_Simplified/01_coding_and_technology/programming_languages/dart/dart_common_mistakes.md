...
# Dart — 常见错误和反模式
本文档列出了 Dart 中最常见的错误、陷阱和反模式，并进行了更正。
---
...
＃＃ 概括
Dart 的 null 安全性非常强大 - 不要用`!`击败它。对规范实例使用`const`构造函数，始终使用`==`覆盖`hashCode`，使用 Collection-if 和展开运算符进行简洁的列表构建，并从小部件中提取业务逻辑。 Dart 奖励干净、空安全、const 正确的代码。