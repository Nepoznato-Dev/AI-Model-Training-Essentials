...
-->
# Scala — 常见错误和反模式
本文档列出了 Scala 中最常见的错误、陷阱和反模式，并进行了更正。
---
...
＃＃ 概括
Scala 的力量伴随着责任：更喜欢`val`而不是`var`，对密封类型使用模式匹配，正确处理`Option`（永远不要调用`.get`），使用 for 推导式而不是嵌套 Future，并避免 null。 Scala 3 通过枚举、扩展方法和联合类型简化了其中许多模式。关键原则：如果编译器能检查，就让编译器检查。