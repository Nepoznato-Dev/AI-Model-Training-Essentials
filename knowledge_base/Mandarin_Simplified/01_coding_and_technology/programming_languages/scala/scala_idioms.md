...
＃＃ 概括
Scala 惯用语强调：不变性 (`val`)、模式匹配、密封类型、用于安全的 Option/Either、for 推导式、函数集合和类型类（在 Scala 3 中给出/使用）。遵循 Scala 风格指南，使用 scalafmt 进行格式化，使用 scalafix 进行 linting。 Scala 3 更简洁的语法、枚举、不透明类型和扩展方法使该语言更易于使用，同时保留了其功能强大的功能。