...
-->
# Scala — 语法参考
本文档提供了 Scala 的全面、结构化语法参考（3.x 以及有关 2.x 差异的注释）。它通过关注详尽的语法模式、类型系统、函数式编程结构和 Scala 习惯用法来补充主要的 Scala 参考。
---
...
|----------|------|---------|--------|
| `+``-``*``/``%`|算术| `a + b`| |
| `**`|幂（BigDecimal）| `BigDecimal(2) ** 10`|未内置 Int |
| `==``!=` |平等| `a == b`|调用`.equals()`— 空安全 |
| `eq``ne` |参考平等| `a eq b`|相同实例 |
| `<``>``<=``>=` |比较| `a >= b`|需要`Ordered`|
| `&&``\|\|``!`|逻辑 | `a && b`|短路|
| `&``\|``^``~` |按位| `a & b`| |
| `<<``>>``>>>`|班次| `a << 2`| |
| `+:``:+``::``:::` |收藏操作| `1 +: list`|如果以`:`结尾，则右结合 |
| `->`|元组创建 | `"key" -> "value"`| `("key", "value")`|
| `=>`|功能/箭头| `x => x + 1`| Lambda 或类型映射 |
| `<-`|发电机| `x <- List(1,2,3)`|理解 |
### 操作员规则
```scala
...
＃＃ 概括
Scala 的语法在强大的类型系统下统一了面向对象和函数式编程。模式匹配、for 推导式和案例类提供了富有表现力的数据建模。特征可以实现灵活的组合，而没有单一继承的限制。类型系统（具有泛型、方差、类型界限和不透明类型）可以在编译时捕获错误，同时实现零成本抽象。 Scala 3 通过枚举、扩展方法、给定/使用和不透明类型简化了语言，使语言更易于使用，同时保留了其全部功能。