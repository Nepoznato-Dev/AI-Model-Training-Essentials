...
| | `-`| Pengurangan | `a - b`|
| | `*`| Perkalian | `a * b`|
| | `/`| Divisi (mengembalikan Ganda) | `a / b`|
...
| **Logis** | `AndAlso`| Hubungan pendek DAN | `a AndAlso b`|
| | `OrElse`| Hubungan pendek ATAU | `a OrElse b`|
| | `Not`| Logis BUKAN | `Not a`|
| | `And`| Sedikit demi sedikit DAN | `a And b`|
| | `Or`| Sedikit demi sedikit ATAU | `a Or b`|
| | `Xor`| XOR sedikit demi sedikit | `a Xor b`|
| **Tali** | `&`| Penggabungan | `"Hello" & " " & "World"`|
| | `+=`| Tambahkan | `s &= " more"`|
| **Tugas** | `=`| Tetapkan | `x = 42`|
| **Batal** | `Is`| Kesetaraan referensi | `obj Is Nothing`|
| | `IsNot`| Ketimpangan referensi | `obj IsNot Nothing`|
| **Jenis** | `TryCast`| Pemeran aman (tidak menghasilkan apa-apa) | `TryCast(obj, String)`|
| | `CType`| Konversi | `CType(obj, Integer)`|
| | `DirectCast`| Pemeran langsung (lemparan saat gagal) | `DirectCast(obj, String)`|
---
...
## Ringkasan
Sintaks Visual Basic bersih dan mirip bahasa Inggris, menjadikannya salah satu bahasa pemrograman yang paling mudah dibaca. VB.NET menyediakan akses penuh ke ekosistem .NET termasuk LINQ untuk kueri data, async/await untuk operasi bersamaan, generik untuk keamanan tipe, dan OOP komprehensif dengan kelas, antarmuka, dan warisan. Ketidakpekaan huruf besar-kecil dan kata kunci verbose (End If, End Sub, End Class) meningkatkan keterbacaan. Untuk aplikasi Windows perusahaan, otomatisasi Office (VBA), dan pembuatan prototipe cepat dalam .NET, VB.NET tetap menjadi bahasa yang produktif dan mumpuni.