...
Dokumen ini mengkatalogkan kesalahan, jebakan, dan anti-pola paling umum di C++. Setiap entri menunjukkan pendekatan yang salah, menjelaskan mengapa gagal, dan memberikan solusi yang tepat. C++ modern (C++11/14/17/20) menyediakan alat untuk menghindari sebagian besar kesalahan klasik.
---

## 1. Aturan Tiga/Lima Pelanggaran
```cpp
...
## 2.`new`/`delete`Mentah Daripada Smart Pointer
```cpp
...
## 6. Tidak Menggunakan`std::move`dengan Benar
```cpp
...
## 7. Anti-Pola: Menggunakan`using namespace std;`di Header
```cpp
...
## 8. Perilaku Tidak Terdefinisi dari Signed Overflow
```cpp
...
## 9. Tidak Memahami Kategori Nilai
```cpp
...
## 10. Anti-Pola: Manajemen Sumber Daya Manual (Pra-RAII)
```cpp
...
## 11. Template Mengasapi dan Waktu Kompilasi yang Lama
```cpp
...
## 12. Tidak Menggunakan`[[nodiscard]]`dan `[[maybe_unused]]`
```cpp
...
## Ringkasan
Kompleksitas C++ menciptakan banyak peluang untuk kesalahan: Aturan Lima, pemotongan objek, pembatalan iterator, referensi yang menggantung, dan perilaku tidak terdefinisi dari luapan yang ditandatangani. C++ modern adalah penawarnya: gunakan smart pointer alih-alih`new`/`delete`mentah, RAII untuk semua sumber daya,`std::vector`dan`std::string`alih-alih array mentah, pindahkan semantik untuk efisiensi, dan`[[nodiscard]]`untuk menangkap nilai kembalian yang diabaikan. Pedoman Inti C++ ada karena suatu alasan — ikuti pedoman tersebut, dan gunakan alat seperti Clang-Tidy dan pembersih untuk mengetahui apa yang Anda lewatkan.