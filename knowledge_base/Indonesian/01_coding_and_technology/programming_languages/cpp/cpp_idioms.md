...
## Ringkasan
Idiom C++ modern menekankan: RAII dan penunjuk cerdas, semantik perpindahan,`auto`untuk pengurangan tipe, rentang dan algoritma (C++20), konsep untuk batasan templat,`std::optional`/`std::expected`untuk ketidakhadiran/kesalahan, Aturan Nol, dan`constexpr`untuk komputasi waktu kompilasi. Ikuti`clang-format`untuk memformat,`clang-tidy`untuk linting, dan kompilasi dengan`-Wall -Wextra -Werror`. Komunitas C++ menghargai abstraksi tanpa biaya dan "jangan membayar untuk apa yang tidak Anda gunakan".