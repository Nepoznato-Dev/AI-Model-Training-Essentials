...
-->
# C++ — Referensi Sintaks
Dokumen ini memberikan referensi sintaksis yang komprehensif dan terstruktur untuk C++ Modern (C++17/20/23). Ini melengkapi referensi utama C++ dengan berfokus pada pola sintaksis yang lengkap, tabel operator, mekanisme templat, dan fitur modern.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmatika | `a + b`| Sama seperti C |
| `++``--` | Kenaikan/Penurunan | `++i`,`i++`| Lebih suka pra-kenaikan |
| `==``!=``<``>``<=``>=` | Perbandingan | `a <=> b`| `<=>`adalah perbandingan tiga arah (C++20) |
| `&&``\|\|``!`| Logis | `a && b`| Hubungan pendek |
| `&``\|``^``~``<<``>>` | Sedikit demi sedikit | `a & b`| Sama seperti C |
| `?:`| Terner | `cond ? a : b`| |
### C++-Operator Tertentu
| Operator | Nama | Contoh | Catatan |
|----------|------|---------|-------|
| `::`| Resolusi ruang lingkup | `std::cout`| |
| `.*``->*` | Pointer-ke-anggota | `obj.*pmf`| |
| `<=>`| Pesawat Luar Angkasa (C++20) | `a <=> b`| Mengembalikan`strong_ordering`|
| `co_await`| Penangguhan coroutine | `co_await expr`| C++20 |
| `co_yield`| Hasil coroutine | `co_yield value`| C++20 |
| `co_return`| Pengembalian coroutine | `co_return value`| C++20 |
...
## Aliran Kontrol
### Binding Terstruktur & Pola Modern
```cpp
...
## Kontainer & Algoritma STL
### Ikhtisar Kontainer
| Wadah | Tajuk | Dipesan | Pencarian | Catatan |
|-----------|--------|---------|--------|-------|
| `vector`| `<vector>`| Ya | PADA(n) | Pilihan default untuk urutan |
| `deque`| `<deque>`| Ya | PADA(n) | Tekan/pop cepat di kedua ujungnya |
| `list`| `<list>`| Ya | PADA(n) | Daftar tertaut ganda |
| `array`| `<array>`| Ya | PADA(n) | Ukuran tetap, alokasi tumpukan |
| `set`/`multiset`| `<set>`| Ya | HAI(log n) | Pohon merah-hitam |
| `map`/`multimap`| `<map>`| Ya (berdasarkan kunci) | HAI(log n) | Nilai kunci, pohon merah-hitam |
| `unordered_set`| `<unordered_set>`| Tidak | O(1) rata-rata | Tabel hash |
| `unordered_map`| `<unordered_map>`| Tidak | O(1) rata-rata | Tabel hash |
### Algoritma Kunci
```cpp
#include <algorithm>
#include <numeric>
...
## Ringkasan
Sintaks C++ mencakup manipulasi pointer tingkat rendah hingga metaprogramming template tingkat tinggi dan asinkron berbasis coroutine. Evolusi bahasa melalui C++17/20/23 telah meningkatkan keamanan secara dramatis (smart pointer,`std::optional`, konsep), ekspresif (binding terstruktur, rentang,`std::format`), dan performa (konsep yang memungkinkan abstraksi tanpa biaya, coroutine untuk asinkron tanpa overhead tumpukan). Filosofi intinya tetap tidak berubah: Anda tidak perlu membayar untuk apa yang tidak Anda gunakan, dan bahasa tersebut harus mendukung setiap tingkat abstraksi dari bahasa bare metal hingga bahasa khusus domain.