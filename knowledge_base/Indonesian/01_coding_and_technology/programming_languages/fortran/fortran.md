...
## Mengapa Fortran Penting
- **Kinerja HPC**: Kompiler Fortran menghasilkan beberapa kode numerik tercepat yang tersedia — sering kali menyamai atau melampaui C/C++ untuk operasi array.
- **Basis kode lama**: Kode ilmiah selama puluhan tahun (model iklim, simulasi fisika) ditulis dalam Fortran.
- **Operasi array**: Dukungan array multidimensi asli dengan sintaksis yang dirancang untuk komputasi matematis.
- **Stabilitas numerik**: Bahasa dan kompiler dioptimalkan untuk komputasi floating-point.
- **Coarrays**: Dukungan pemrograman paralel bawaan (Fortran 2008+).
- **Masih berkembang**: Fortran 2023 menambahkan fitur modern dengan tetap menjaga kompatibilitas ke belakang.
## Pengorbanan
| Batasan | Detail | Solusi Khas |
|-----------|---------|-------------------|
| **Komunitas khusus** | Kecil dan terspesialisasi — kebanyakan ilmiah/HPC | Komunitas aktif dalam ilmu komputasi |
| **Ekosistem terbatas** | Lebih sedikit perpustakaan daripada Python, Java, atau C++ | Gunakan BLAS/LAPACK untuk pekerjaan numerik; C interoperabilitas untuk kebutuhan lain |
| **Bukan tujuan umum** | Buruk untuk pemrograman web, seluler, GUI, atau sistem | Gunakan Fortran untuk komputasi; bungkus dengan Python/C untuk aplikasi |
| **Persepsi** | Sering dianggap "usang" meskipun memiliki kemampuan modern | Fokus pada kelebihannya: numerik dan HPC |
| **Mempekerjakan** | Beberapa pengembang Fortran memasuki pasar | Pakar yang ada di dunia akademis dan laboratorium nasional |
---

## Dasar Sintaks (Fortran Modern)
```fortran
program hello
    implicit none
...
### Tipe Turunan dan Prosedur Terikat Tipe
```fortran
...
### Prosedur Umum dan Kelebihan Operator
```fortran
module vector_ops
    implicit none
...
### Coarrays (Paralelisme Bawaan Fortran 2008)
```fortran
...
### OpenMP (Paralelisme Memori Bersama)
```fortran
...
### MPI (Paralelisme Memori Terdistribusi)
```fortran
program mpi_example
    use mpi
    implicit none
...
## Pengujian
### Kerangka Uji (fortran-test atau pFUnit)
```fortran
...
## Interoperabilitas
### C Interoperabilitas (ISO_C_BINDING)
```fortran
module c_interface
...
### Integrasi Python (melalui f2py atau perpustakaan bersama)
```fortran
...
## Pola Desain
### Pola 1: Pemrograman Array (Hindari Loop)
```fortran
...
### Pola 2: Tipe Turunan yang Diparameterisasi
```fortran
...
### Pola 3: Pola Panggilan Balik dengan Penunjuk Prosedur
```fortran
module integrator
    implicit none
    abstract interface
...
## Kapan Menggunakan Fortran
| Skenario | Mengapa Fortran | Alternatif Lebih Baik |
|----------|-----------|-------------------|
| HPC / superkomputer | Dioptimalkan untuk kinerja numerik | C++ (dengan hati-hati), Julia |
| Simulasi ilmiah | Kode yang divalidasi selama beberapa dekade | Python (untuk pembuatan prototipe), C++ |
| Model iklim/cuaca | Basis kode lama; kinerja | — |
| Fisika komputasi | Operasi array asli | Python (NumPy), Julia |
| Pemodelan keuangan (HPC) | Performa untuk komputasi skala besar | C++,Python |
...
## Ringkasan
Fortran adalah bahasa pemrograman ilmiah asli dan tetap menjadi yang terdepan dalam komputasi kinerja tinggi. Fortran modern adalah bahasa yang mumpuni dan berkembang dengan operasi array asli, dukungan pemrograman paralel, dan interoperabilitas C. Meskipun komunitasnya kecil dan terspesialisasi, Fortran terus menjalankan beberapa beban kerja komputasi yang paling menuntut di dunia. Untuk komputasi numerik dalam skala besar, Fortran tetap relevan.