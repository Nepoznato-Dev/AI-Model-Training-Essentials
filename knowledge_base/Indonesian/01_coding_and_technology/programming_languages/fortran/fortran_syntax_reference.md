...
---
# Fortran — Referensi Sintaks
Dokumen ini memberikan referensi sintaksis yang komprehensif dan terstruktur untuk Modern Fortran (2008/2018). Ini melengkapi referensi utama Fortran dengan berfokus pada pola sintaksis yang lengkap, operasi array, modul, OOP, dan komputasi kinerja tinggi.
---
...
| `==``/=` | Kesetaraan | `a == b`| `/=`"tidak sama" |
| `<``>``<=``>=` | Perbandingan | `a >= b`| |
| `.and.``.or.``.not.`| Logis | `a .and. b`| |
| `.eqv.``.neqv.` | Kesetaraan logis | `a .eqv. b`| |
| `//`| Penggabungan string | `'hello' // ' world'`| |
---
...
## Ringkasan
Sintaks Fortran modern bersih, terstruktur, dan dioptimalkan untuk komputasi numerik. Operasi array menghilangkan loop untuk sebagian besar tugas matematika. Modul menyediakan enkapsulasi dan manajemen namespace. Fitur OOP (tipe turunan, pewarisan, polimorfisme) memungkinkan organisasi kode yang bersih. Coarrays menawarkan pemrograman paralel asli tanpa perpustakaan eksternal. Kekuatan Fortran — kinerja, operasi susunan, dan presisi numerik — tetap tak tertandingi untuk komputasi ilmiah. Untuk HPC, fisika komputasi, dan simulasi teknik, Fortran terus menjadi pembangkit tenaga listrik.