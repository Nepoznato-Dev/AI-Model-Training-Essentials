...
# Fortran — Kesalahan Umum & Anti-Pola
Dokumen ini berisi katalog kesalahan, jebakan, dan anti-pola paling umum di Fortran beserta koreksinya.
---

## 1. Pengindeksan Berbasis 1
```fortran
...
## 2. Tidak Menggunakan `implicit none`
```fortran
...
## 3. Penugasan Array vs Berdasarkan Elemen
```fortran
...
## 5. Tidak Menggunakan Fitur Fortran Modern
```fortran
...
## 6. Kebocoran Memori dengan Array yang Dapat Dialokasikan
```fortran
...
## Ringkasan
Fokus komputasi ilmiah Fortran menciptakan jebakan: pengindeksan berbasis 1, pengetikan implisit (selalu gunakan`implicit none`), operasi array vs loop, dan spesifikasi maksud. Fortran modern (2003+) memiliki modul, tipe turunan, dan OOP — gunakanlah. Cara Fortran adalah:`implicit none`selalu, tentukan`intent`untuk semua argumen, gunakan operasi array, dan batalkan alokasi apa yang Anda alokasikan.