...
|----------|------|---------|-------|
| `+``-``*``/``%``~/` | Aritmatika | `7 ~/ 3`| `~/`adalah pembagian bilangan bulat |
| `==``!=` | Kesetaraan | `a == b`| |
| `<``>``<=``>=` | Perbandingan | `a >= b`| |
| `&&``\|\|``!`| Logis | `a && b`| Hubungan pendek |
| `??`| Penggabungan nol | `a ?? b`| `b`jika`a`adalah nol |
| `?.`| Akses tanpa sadar | `a?.b`| batal jika`a`adalah nol |
| `!`| Pernyataan nol | `a!`| Melempar jika null |
| `..``?..` | Kaskade | `obj..a..b`| Operasi berantai pada objek yang sama |
| `??=`| Penetapan nol-sadar | `a ??= b`| Tetapkan jika null |
| `as`| Ketik pemeran | `obj as String`| Melempar kegagalan |
| `is``is!` | Ketik tes | `obj is String`| |
| `=>`| Fungsi panah | `() => expr`| |
### Notasi Berjenjang
```dart
...
## Ringkasan
Sintaks Dart bersih, konsisten, dan dirancang untuk produktivitas. Keamanan suara nol menghilangkan kesalahan referensi nol pada waktu kompilasi. Async/menunggu dan aliran memberikan pola alami untuk pemrograman asinkron. Kelas, mixin, dan kelas tersegel memungkinkan komposisi objek yang fleksibel. Pencocokan pola, rekaman, dan pengubah kelas Dart 3 menambah ekspresi. Meskipun tujuan utama Dart adalah Flutter, sintaksisnya sangat cocok untuk domain aplikasi apa pun — sisi server (dengan Dart Frog atau Shelf), alat CLI, dan aplikasi web (dikompilasi ke JavaScript).