...
# Lua — Kesalahan Umum & Anti-Pola
Dokumen ini mengkatalogkan kesalahan, jebakan, dan anti-pola paling umum di Lua beserta koreksinya.
---
...
## 4. Variabel Global Secara Default
```lua
...
## 7. Tidak Menggunakan Metatabel untuk OOP
```lua
...
## Ringkasan
Minimalisme Lua menciptakan jebakan: pengindeksan berbasis 1, hanya`nil`dan`false`yang salah, tabel adalah referensi (bukan nilai), variabel bersifat global secara default, dan`pairs`tidak diurutkan sementara`ipairs`diurutkan. Cara Lua adalah: selalu gunakan`local`, gunakan`ipairs`untuk array dan`pairs`untuk kamus, gunakan`table.concat`untuk pembuatan string, dan gunakan metatabel untuk OOP. Kesederhanaan Lua adalah kekuatannya — namun hormati kasus-kasus edge.