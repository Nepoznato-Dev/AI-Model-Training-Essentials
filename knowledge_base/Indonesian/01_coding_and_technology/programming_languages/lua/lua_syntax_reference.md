...
| `//`| Pembagian lantai | `7 // 2`| Mengembalikan 3 (Lua 5.3+) |
| `==``~=` | Kesetaraan | `a == b`| `~=`"tidak sama" |
| `<``>``<=``>=` | Perbandingan | `a >= b`| |
| `and``or``not`| Logis | `a and b`| Mengembalikan operan, bukan boolean |
| `..`| Penggabungan | `"hello" .. " world"`| |
| `#`| Panjang | `#t`| Panjang string atau tabel |
| `&``\|``~``<<``>>`| Sedikit demi sedikit | `a & b`| Lua 5.3+ (operasi bilangan bulat) |
### Operator Logis — Pengembalian Non-Boolean
```lua
...
## Ringkasan
Sintaks Lua sengaja dibuat minimal — seluruh bahasa dapat dijelaskan dalam satu halaman. Tabel berfungsi sebagai struktur data universal (array, kamus, objek, modul). Metatabel memberikan ekstensibilitas tanpa kerumitan bahasa. Coroutine memungkinkan multitasking kooperatif dan pola iterator yang elegan. Kekuatan bahasa ini berasal dari kemampuan penyematannya: Lua dirancang untuk hidup di dalam aplikasi host, menyediakan kemampuan skrip dengan overhead minimal. Untuk pengembangan game, konfigurasi, dan sistem tertanam, kesederhanaan Lua adalah kekuatan terbesarnya — sebuah bahasa kecil yang melakukan apa yang dibutuhkannya, dan tidak lebih.