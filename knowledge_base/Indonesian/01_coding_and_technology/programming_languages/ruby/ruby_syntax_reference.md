...
# Ruby — Referensi Sintaks
Dokumen ini memberikan referensi sintaksis yang komprehensif dan terstruktur untuk Ruby (3.x). Ini melengkapi referensi utama Ruby dengan berfokus pada pola sintaksis yang lengkap, blok dan penutupan, metaprogramming, dan idiom Ruby.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%``**` | Aritmatika | `2 ** 10`| `**`adalah eksponen |
| `==``!=``===``<=>` | Kesetaraan | `a == b`| `===`adalah persamaan huruf; `<=>`adalah pesawat luar angkasa |
| `eql?`| Nilai kesetaraan | `a.eql?(b)`| Lebih ketat dari`==`(tipe cek) |
| `equal?`| Identitas | `a.equal?(b)`| Objek yang sama? |
| `<``>``<=``>=` | Perbandingan | `a >= b`| |
| `&&``\|\|``!`| Logis | `a && b`| Hubungan pendek; mengembalikan nilai evaluasi terakhir |
| `and``or``not`| Logis (prioritas rendah) | `a and b`| Hindari — gunakan`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | Sedikit demi sedikit | `a & b`| |
| `=~``!~` | Pencocokan ekspresi reguler | `str =~ /pattern/`| Mengembalikan indeks atau nihil |
| `..``...` | Rentang | `1..10`| `..`inklusif; `...`eksklusif |
| `?.`| Navigasi yang aman | `user&.name`| Mengembalikan nihil jika pengguna nihil |
| `[]``[]=` | Akses elemen | `arr[0]`| |
### Kebenaran
```ruby
...
## Ringkasan
Sintaks Ruby dirancang untuk kebahagiaan pengembang — dibaca seperti bahasa Inggris dan menghargai ekspresi. Kekuatan bahasa ini berasal dari model objeknya (semuanya adalah objek), blok dan penutupan, kemampuan metaprogramming, dan modul Enumerable yang menyediakan API pemrosesan koleksi yang kaya. Ruby 3.x telah menambahkan pencocokan pola, tanda tangan tipe RBS, dan Ractor untuk konkurensi, dengan tetap mempertahankan filosofi inti bahasa ini: menjadikan pemrogram produktif dan bahagia.