...
## Mengapa Kotlin Penting
- **Standar Android**: Bahasa pilihan Google untuk Android. Kode Android paling baru adalah Kotlin.
- **100% kompatibel dengan Java**: Gunakan semua pustaka, kerangka kerja, dan alat Java. Bermigrasi secara bertahap.
- **Keamanan nol**: Sistem tipe mencegah pengecualian penunjuk nol pada waktu kompilasi.
- **Ringkas**: Boilerplate jauh lebih sedikit dibandingkan Java — kelas data, fungsi ekstensi, smart cast.
- **Coroutines**: Thread ringan untuk pemrograman asinkron — lebih sederhana daripada CompletableFuture atau callback Java.
- **Multiplatform**: Kotlin Multiplatform memungkinkan Anda berbagi kode antara Android, iOS, web, dan backend.
- **Sisi server**: Ktor, Spring Boot (dukungan penuh Kotlin), dan Exended membuat Kotlin layak untuk backend.
## Pengorbanan
| Batasan | Detail | Solusi Khas |
|-----------|---------|-------------------|
| **Ketergantungan JVM** | Memerlukan JVM (atau Kotlin/Native untuk target non-JVM) | Gunakan gambar asli GraalVM untuk penerapan kecil |
| **Kecepatan kompilasi** | Lebih lambat dari Java untuk proyek besar | Gunakan kompilasi tambahan; Kotlin 2.0 menyempurnakan ini |
| **Kurva pembelajaran untuk pengembang Java** | Fungsi ekstensi, coroutine, DSL adalah konsep baru | Adopsi bertahap; sebagian besar pola Java masih berfungsi |
| **Komunitas lebih kecil dari Java** | Lebih sedikit resource dan perpustakaan khusus Kotlin | Memanfaatkan ekosistem besar di Jawa |
| **Kematangan Multiplatform Kotlin** | Masih berkembang untuk produksi berbagi iOS | Gunakan untuk logika bisnis bersama; pertahankan UI asli |
---
...
### Kelas Data dan Fungsi Ekstensi
```kotlin
...
### Coroutine — Pemrograman Asinkron
```kotlin
...
### Pemrograman meta dengan Refleksi
```kotlin
...
### Penghancuran Tingkat Lanjut dan Pencocokan Pola
```kotlin
...
### Dispatcher dan Konteks Coroutine
```kotlin
import kotlinx.coroutines.*
...
### Saluran — Komunikasi Antar Coroutine
```kotlin
...
### Konkurensi Terstruktur dan Penanganan Kesalahan
```kotlin
...
### Konfigurasi Pembuatan (build.gradle.kts)
```kotlin
plugins {
...
### Perintah Manajemen Ketergantungan
```bash
# Build and run
./gradlew build
./gradlew run
...
## Pola Desain
### Singleton (Aman untuk Thread secara Default)
```kotlin
...
### Pola Repositori dengan Generik
```kotlin
...
## Penerapan
### JAR Gemuk dengan Plugin Shadow
```kotlin
// build.gradle.kts
plugins {
...
### Penerapan Multiplatform Kotlin
```kotlin
...
| Kerangka | Domain |
|-----------|--------|
| **Penulisan Jetpack** | Perangkat UI Android modern |
| **Ktor** | Kerangka web sisi server yang ringan |
| **Sepatu Musim Semi** | Dukungan penuh Kotlin untuk backend perusahaan |
| **Multiplatform Kotlin** | Bagikan kode antara Android, iOS, web, desktop |
| **Terkena** | Pustaka SQL Kotlin (kueri aman untuk mengetik) |
| **Koin** | Kerangka kerja injeksi ketergantungan |
### Alat Pembuatan
| Alat | Tujuan |
|------|---------|
| **Gradle (Kotlin DSL)** | Sistem build — Kotlin adalah bahasa skrip build pilihan |
| **IDE IntelliJ** | IDE oleh JetBrains — dukungan Kotlin terbaik |
---

## Kapan Menggunakan Kotlin
| Skenario | Mengapa Kotlin | Alternatif Lebih Baik |
|----------|-----------|-------------------|
| Pengembangan Android | Bahasa pilihan Google | Java (untuk basis kode lama) |
| Backend JVM | Alternatif Java modern | Jawa, Pergi |
| Lintas platform (logika bersama) | Multiplatform Kotlin | Flutter (untuk berbagi UI) |
| Aplikasi desktop | Mungkin dengan Compose Multiplatform | C#, Swift untuk asli |
| Aplikasi JVM umum | Kurang bertele-tele dibandingkan Java | Java untuk tim yang lebih besar |
| Pemrograman sistem non-JVM | Bukan target utama | Karat, Pergi, C |
| Tampilan depan web | Kotlin/JS ada tetapi terbatas | Skrip Ketik, JavaScript |
