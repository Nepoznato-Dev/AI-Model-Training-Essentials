...
---
# Kotlin — Kesalahan Umum & Anti-Pola
Dokumen ini berisi katalog kesalahan, jebakan, dan anti-pola paling umum di Kotlin. Setiap entri menunjukkan pendekatan yang salah, menjelaskan mengapa gagal, dan memberikan solusi yang tepat.
---

## 1. Tipe Platform dan NullPointerException
```kotlin
...
## 3.`data class`Tanpa`equals`/`hashCode`yang Tepat
```kotlin
...
## 4. Tidak Menggunakan Fungsi Scope dengan Benar
```kotlin
...
## 5. Coroutine: Tidak Menangani Pembatalan
```kotlin
...
## 6. Anti-Pola: Kotlin Bergaya Java
```kotlin
...
## 9. Tidak Menggunakan`by lazy`untuk Inisialisasi Mahal
```kotlin
...
## 10. Aliran vs Kebingungan LiveData
```kotlin
...
## Ringkasan
Ringkasnya Kotlin dapat menyembunyikan masalah halus: jenis platform dari interop Java, error `lateinit`, kelas data yang dapat diubah merusak koleksi, dan pembatalan coroutine. Cara Kotlin adalah: menggunakan keamanan nol (jangan pernah menggunakan`!!`), gunakan fungsi cakupan yang tepat (`let`untuk transformasi,`apply`untuk konfigurasi,`also`untuk efek samping), gunakan kelas tersegel dengan`when`yang lengkap, tangani pembatalan coroutine, dan tulis Kotlin idiomatik (bukan Java dengan sintaksis Kotlin). Kompiler adalah panduan Anda — jika ia memperingatkan, dengarkan.