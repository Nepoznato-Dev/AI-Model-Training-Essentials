...
-->
# Kotlin — Referensi Sintaks
Dokumen ini memberikan referensi sintaksis yang komprehensif dan terstruktur untuk Kotlin (2.0+). Ini melengkapi referensi utama Kotlin dengan berfokus pada pola sintaksis yang lengkap, keamanan null, coroutine, dan idiom Kotlin.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmatika | `a + b`| |
| `..`| Rentang | `1..10`| Kisaran inklusif |
| `until`| Rentang setengah terbuka | `1 until 10`| `1..9`|
| `downTo`| Rentang menurun | `10 downTo 1`| |
| `==``!=` | Kesetaraan struktural | `a == b`| Panggilan`.equals()`|
| `===``!==` | Kesetaraan referensial | `a === b`| Contoh yang sama |
| `<``>``<=``>=` | Perbandingan | `a >= b`| Membutuhkan`Comparable`|
| `&&``\|\|``!`| Logis | `a && b`| Hubungan pendek |
| `?:`| Operator Elvis | `a ?: b`| Mengembalikan`b`jika`a`adalah null |
| `?.`| Panggilan aman | `a?.b`| Mengembalikan null jika`a`adalah null |
| `!!`| Pernyataan bukan nol | `a!!`| Melempar NPE jika null |
| `as`| Pemeran | `a as String`| Melempar ClassCastException |
| `as?`| Pemeran aman | `a as? String`| Mengembalikan null jika gagal |
| `in`| Keanggotaan | `x in list`| |
| `!in`| Bukan anggota | `x !in list`| |
| `is`| Ketik centang | `x is String`| Pemeran pintar setelah pemeriksaan |
| `!is`| Tidak mengetik | `x !is String`| |
...
## Ringkasan
Sintaks Kotlin ringkas, aman, dan dapat dioperasikan dengan Java. Keamanan nol menghilangkan seluruh kelas bug pada waktu kompilasi. Coroutine menyediakan cara alami untuk menulis kode asinkron. Kelas tertutup, kelas data, dan fungsi ekstensi memungkinkan pemodelan domain ekspresif. Kekuatan bahasa ini berasal dari interaksi fitur-fiturnya — fungsi cakupan, DSL, kelas inline, dan generik yang direifikasi digabungkan untuk menghasilkan kode yang aman dan menyenangkan untuk ditulis.