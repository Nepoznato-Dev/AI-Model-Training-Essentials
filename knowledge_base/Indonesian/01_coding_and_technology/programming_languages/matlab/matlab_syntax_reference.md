...
# MATLAB — Referensi Sintaks
Dokumen ini memberikan referensi sintaksis yang komprehensif dan terstruktur untuk MATLAB (R2024+). Ini melengkapi referensi utama MATLAB dengan berfokus pada pola sintaksis yang lengkap, operasi matriks, plotting, dan idiom komputasi ilmiah.
---
...
|----------|------|---------|-------|
| `+``-``*``/``^`| Aritmatika | `A * B`| Perkalian matriks |
| `.*``./``.^`| Dari segi elemen | `A .* B`| Operasi susunan |
| `'`| Ubah urutan | `A'`| Konjugasi transpos |
| `.'`| Transpos non-konjugasi | `A.'`| |
| `\`| Belah kiri | `A \ b`| Selesaikan Kapak = b |
| `/`| Bagi kanan | `b / A`| Selesaikan xA = b |
| `==``~=` | Kesetaraan | `A == B`| Dari segi elemen |
| `<``>``<=``>=` | Perbandingan | `A > 0`| Dari segi elemen |
| `&``\|``~`| Logis (dari segi elemen) | `A & B`| |
| `&&``\|\|` | Hubungan pendek | `a && b`| Hanya skalar |
| `:`| Titik dua/rentang | `1:10`| `start:step:stop`|
| `;`| Pemisah / penekan baris | `[1; 2; 3]`| |
| `,`| Pemisah kolom | `[1, 2, 3]`| |
---
...
## Ringkasan
Sintaks MATLAB adalah yang mengutamakan matriks — setiap variabel adalah array, dan operasinya dioptimalkan untuk aljabar linier. Operator titik dua, pengindeksan logis, dan operasi vektor menghilangkan loop untuk sebagian besar tugas numerik. Sistem plotting menghasilkan angka berkualitas publikasi dengan kode minimal. Toolbox memperluas MATLAB ke dalam setiap domain teknik. Meskipun Python telah merambah beberapa wilayah, MATLAB tetap menjadi standar untuk komputasi teknik, pemodelan berbasis Simulink, dan pembuatan prototipe cepat.