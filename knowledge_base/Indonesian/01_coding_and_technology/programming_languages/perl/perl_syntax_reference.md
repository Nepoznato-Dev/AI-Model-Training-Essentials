...
| `x`| Pengulangan senar | `"ab" x 3`| `"ababab"`|
| `==``!=``<``>``<=``>=` | Perbandingan numerik | `$a == $b`| |
| `eq``ne``lt``gt``le``ge` | Perbandingan string | `$a eq $b`| |
| `<=>`| Pesawat ruang angkasa numerik | `$a <=> $b`| Mengembalikan -1, 0, 1 |
| `cmp`| Pesawat ruang angkasa tali | `$a cmp $b`| |
| `&&``\|\|``!`| Logis | `$a && $b`| |
| `and``or``not`| Logis (sebelumnya rendah) | `$a and $b`| Hindari — gunakan`&&`/`\|\|`|
| `=~``!~` | Pencocokan ekspresi reguler | `$str =~ /pattern/`| |
| `..``...` | Rentang / flip-flop | `1..10`| |
| `?:`| Terner | `$a ? $b : $c`| |
| `//`| Didefinisikan-atau | `$a // $b`| `$b`jika`$a`tidak dapat dipertahankan |
| `=>`| Koma gemuk | `key => 'value'`| Bareword kutipan otomatis |
### Sigil — Tipe Variabel
```perl
...
## Perl Berorientasi Objek (Moo/Moose)
```perl
# Modern OOP with Moo
package User;
use Moo;
...
## Ringkasan
Sintaks Perl dibangun berdasarkan konteks, simbol, dan prinsip "Ada Lebih dari Satu Cara Untuk Melakukannya". Ekspresi reguler dibangun ke dalam bahasa ini, membuat pemrosesan teks menjadi ringkas dan tak tertandingi. Referensi memungkinkan struktur data yang kompleks. CPAN menyediakan modul untuk semuanya. Perl modern (dengan`strict`,`warnings`, dan variabel leksikal) bersih dan dapat dipelihara. Warisan Perl tetap hidup dalam pola dan alat yang diadopsi oleh setiap bahasa modern.