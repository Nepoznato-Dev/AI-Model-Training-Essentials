...
# TypeScript — Kesalahan Umum & Anti-Pola
Dokumen ini mengkatalogkan kesalahan, jebakan, dan anti-pola paling umum di TypeScript. Setiap entri menunjukkan pendekatan yang salah, menjelaskan mengapa gagal, dan memberikan solusi yang tepat.
---

## 1. Terlalu sering menggunakan `any`
```typescript
...
## 2. Ketik Pernyataan Daripada Ketik Penjaga
```typescript
...
## 3. Penyalahgunaan Pernyataan Non-Null
```typescript
...
## 4. Antarmuka vs Jenis Kebingungan
```typescript
...
## 6. Tidak Menggunakan Mode `strict`
```json
...
## 7. Kejutan Pengetikan Struktural
```typescript
...
## 8. Kesalahan Jenis Async/Tunggu
```typescript
...
## 9. Anti-Pola: Tipe Berulang (Tidak Menggunakan Generik)
```typescript
...
## 10. Rangkaian Opsional Berlebihan
```typescript
...
## 11. Menggunakan`namespace`(Warisan)
```typescript
...
## 12. Tidak Memanfaatkan Serikat Pekerja yang Didiskriminasi
```typescript
...
## Ringkasan
Sistem tipe TypeScript sangat kuat tetapi membutuhkan disiplin. Dosa utama adalah mencapai`any`— gunakan`unknown`ketika tipenya benar-benar tidak diketahui, dan sempit dengan pelindung tipe. Hindari pernyataan bukan nol (`!`) dengan menangani undefinisi dengan benar. Gunakan antarmuka untuk bentuk objek, tipe untuk gabungan, dan tipe yang dihitung. Serikat pekerja yang terdiskriminasi menghilangkan seluruh kelompok serangga. Aktifkan`strict: true`sejak hari pertama — penyesuaian yang ketat itu sulit. Tujuannya adalah untuk membuat negara-negara ilegal tidak terwakili pada tingkat jenis.