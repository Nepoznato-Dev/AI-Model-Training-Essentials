...
## Ringkasan
Idiom TypeScript menekankan: pemeriksaan tipe yang ketat, antarmuka untuk bentuk objek, gabungan yang didiskriminasi untuk negara, pelindung tipe untuk penyempitan, generik dengan batasan,`readonly`untuk kekekalan, dan`as const`untuk tipe literal. Ikuti ESLint dengan`typescript-eslint`, Lebih cantik untuk pemformatan, dan selalu aktifkan`strict: true`. Komunitas TypeScript menghargai keamanan tipe — jika dikompilasi, mungkin akan berhasil. Lebih suka`interface`untuk bentuk objek,`type`untuk serikat pekerja dan utilitas, dan hindari`any`dengan cara apa pun.