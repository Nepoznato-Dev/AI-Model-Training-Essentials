...
# Java — Referensi Sintaks
Dokumen ini memberikan referensi sintaksis yang komprehensif dan terstruktur untuk Java. Ini melengkapi referensi utama Java dengan berfokus pada pola sintaksis yang lengkap, tabel operator, dan mekanisme internal JVM, OOP, dan konkurensi.
---
...
|----------|------|---------|--------|-------|
| `+`| Tambahan | `3 + 2`| `5`| Juga penggabungan string |
| `-`| Pengurangan | `3 - 2`| `1`| |
| `*`| Perkalian | `3 * 2`| `6`| |
| `/`| Divisi | `7 / 2`| `3`| Pembagian bilangan bulat untuk`int`; `3.5`untuk`double`|
| `%`| Sisa | `7 % 2`| `1`| |
...
|----------|------|---------|-------|
| `==`| Sama | `x == y`| Kesetaraan referensi untuk objek |
| `!=`| Tidak Sama | `x != y`| |
| `<`,`>`,`<=`,`>=`| Relasional | `x >= y`| |
| `&&`| Logis DAN | `a && b`| Hubungan pendek |
| `\|\|`| Logis ATAU | `a \|\| b`| Hubungan pendek |
| `!`| Logis BUKAN | `!true`| `false`|
| `instanceof`| Ketik Periksa | `obj instanceof String`| Juga pencocokan pola (Java 16+) |
### Operator Bitwise
| Operator | Nama | Contoh | Catatan |
|----------|------|---------|-------|
| `&`| DAN | `5 & 3`| `1`|
...
| Prioritas | Operator |
|------------|-----------|
| 1 (tertinggi) | Perbaikan akhir:`expr++``expr--` |
| 2 | Unary:`++expr``--expr``+expr``-expr``~``!` |
| 3 | Pemeran:`(Type)expr`|
| 4 | Perkalian:`*``/``%`|
| 5 | Aditif:`+``-` |
| 6 | Pergeseran:`<<``>>``>>>`|
| 7 | Relasional:`<``>``<=``>=``instanceof`|
| 8 | Kesetaraan:`==``!=` |
| 9 | Bitwise DAN:`&`|
| 10 | XOR Bitwise:`^`|
| 11 | Bitwise ATAU:`\|`|
...
## Struktur Data
### Kerangka Koleksi
| Antarmuka | Implementasi | Dipesan | Varian Aman Benang |
|-----------|----------------|---------|---------------------|
| `List`| `ArrayList`,`LinkedList`| Ya | `CopyOnWriteArrayList`|
| `Set`| `HashSet`,`TreeSet`,`LinkedHashSet`| Bervariasi | `CopyOnWriteArraySet`|
| `Map`| `HashMap`,`TreeMap`,`LinkedHashMap`| Bervariasi | `ConcurrentHashMap`|
| `Queue`| `ArrayDeque`,`PriorityQueue`| Bervariasi | `ConcurrentLinkedQueue`|
| `Deque`| `ArrayDeque`| Kedua ujungnya | `ConcurrentLinkedDeque`|
### Daftar
```java
// Creation
...
## Ringkasan
Sintaks Java telah berkembang secara dramatis sejak awal mulanya pada tahun 1995. Java modern (21+) menampilkan rekaman, kelas tersegel, pencocokan pola, thread virtual, dan templat string — menjadikannya lebih ringkas dan ekspresif dari sebelumnya. Sintaks inti tetap familier (seperti C, diketik dengan kuat, berbasis kelas), namun penambahan lambda, Stream API, dan`var`telah mengurangi boilerplate secara signifikan. Kekuatan bahasa ini terletak pada ekosistemnya yang besar, jaminan kompatibilitas ke belakang, dan optimalisasi kinerja JVM melalui kompilasi JIT.