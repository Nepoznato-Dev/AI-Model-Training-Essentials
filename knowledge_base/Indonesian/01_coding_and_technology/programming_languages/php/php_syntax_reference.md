...
# PHP — Referensi Sintaks
Dokumen ini memberikan referensi sintaksis yang komprehensif dan terstruktur untuk PHP (8.x). Ini melengkapi referensi utama PHP dengan berfokus pada pola sintaksis yang lengkap, fitur PHP modern, pemrograman berorientasi objek, dan idiom praktis.
---
...
| `+``-``*``/``%`| Aritmatika | `$a + $b`| |
| `**`| Eksponen | `2 ** 10`| `1024`|
| `==``!=``===``!==` | Kesetaraan | `$a === $b`| `===`memeriksa tipe juga |
| `<=>`| Pesawat Luar Angkasa | `$a <=> $b`| Mengembalikan -1, 0, atau 1 |
| `<``>``<=``>=` | Perbandingan | `$a >= $b`| |
| `&&``\|\|``!`| Logis | `$a && $b`| Hubungan pendek |
| `and``or``xor`| Logis (prioritas rendah) | `$a and $b`| Hindari — gunakan`&&`/`\|\|`|
| `??`| Penggabungan nol | `$a ?? 'default'`| Mengembalikan`'default'`jika`$a`adalah null |
| `??=`| Penetapan penggabungan nol | `$a ??= 'default'`| Tetapkan hanya jika`$a`adalah null |
| `?``:` | Terner | `$a ? $b : $c`| |
| `.`| Penggabungan | `$a . $b`| |
| `.=`| Menggabungkan penetapan | `$a .= $b`| |
| `=>`| Penugasan larik | `['key' => 'value']`| |
| `->`| Akses anggota | `$obj->method()`| |
| `?->`| Operator tidak aman | `$obj?->method()`| Mengembalikan null jika`$obj`adalah null |
| `::`| Akses statis | `ClassName::method()`| |
| `instanceof`| Ketik centang | `$obj instanceof ClassName`| |
| `clone`| Kloning objek | `$copy = clone $obj`| Salinan dalam memerlukan`__clone()`|
### Tipe Juggling
```php
...
## Ringkasan
Sintaks PHP telah berkembang secara dramatis dari asal mula skripnya menjadi bahasa modern yang aman untuk mengetik. PHP 8.x memperkenalkan tipe gabungan, tipe persimpangan, enum, fiber, kelas hanya baca, atribut, dan callable kelas satu — mengubahnya dari bahasa skrip yang diketik secara longgar menjadi bahasa yang mendukung rekayasa perangkat lunak yang ketat. Kekuatan bahasa ini tetap menjadi fokus webnya: dengan kerangka kerja seperti Laravel dan Symfony, ekosistem yang sangat besar, dan dukungan hosting yang hampir universal, sintaksis PHP melayani jutaan aplikasi. PHP modern memberi penghargaan kepada pengembang yang menerapkan sistem tipenya, menggunakan pemuatan otomatis berbasis komposer, dan menulis kode berorientasi objek yang bersih.