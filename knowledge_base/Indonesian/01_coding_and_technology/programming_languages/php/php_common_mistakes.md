...
# PHP — Kesalahan Umum & Anti-Pola
Dokumen ini berisi katalog kesalahan, jebakan, dan anti-pola paling umum di PHP beserta koreksinya.
---

## 1. Kesalahan Perbandingan Longgar (`==`).
```php
...
## 3. Tidak Menggunakan Deklarasi Tipe
```php
...
## 4. Variabel Variabel dan Akses Dinamis
```php
...
## 5. Tidak Menangani Kesalahan dengan Benar
```php
...
## 6. Negara Global dan Superglobal
```php
...
## 8. Tidak Menggunakan Autoloading Komposer
```php
...
## 10. Anti-Pola: Logika Bisnis dalam Tampilan
```php
...
## Ringkasan
Fleksibilitas PHP adalah kekuatan dan kelemahannya. Aturan utamanya: selalu gunakan perbandingan ketat (`===`), gunakan pernyataan yang telah disiapkan untuk SQL, aktifkan`declare(strict_types=1)`, gunakan Komposer untuk memuat otomatis, buat ulang ID sesi setelah autentikasi, dan jauhkan logika bisnis dari tampilan. PHP modern (8.x) dengan tipe ketat, tipe gabungan, ekspresi kecocokan, dan argumen bernama sangat jauh berbeda dari PHP 5 — menggunakan fitur-fitur modern.