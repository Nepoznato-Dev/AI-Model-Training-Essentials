...
# COBOL — Riwayat Versi & Evolusi
## Garis Waktu
| Versi | Tahun | Tema Utama |
|---------|------|-----------|
| ALIRAN-MATIC | 1955 | Prekursor bahasa bisnis Grace Hopper |
| COBOL-60 | 1959 | **COBOL Pertama** (komite CODASYL) |
| COBOL-61 | 1961 | Penulis Laporan, peningkatan`PERFORM`|
| COBOL-68 | 1968 | Standar ANSI pertama (X3.1-1968) |
| COBOL-74 | 1974 | `IF`/`ELSE`,`EVALUATE`(saklar), generalisasi |
| COBOL-85 | 1985 | **Pemrograman terstruktur**:`END-IF`,`END-PERFORM`, terminator cakupan |
| COBOL 2002 | 2002 | **OOP**: kelas, metode, warisan,`FUNCTION`|
| COBOL 2014 | 2014 | **XML**,`JSON`(sebagian), tipe`BOOLEAN`|
| COBOL 2023 | 2023 | **Koleksi JSON asli**, UTF-8,`LIST`|
## Tonggak Penting
### Kelahiran COBOL (1959)
- **1959**: CODASYL (Konferensi Bahasa Sistem Data) menciptakan COBOL
- **Pengaruh Grace Hopper**: "Bahasa bisnis harus terlihat seperti bahasa Inggris"
- **Sasaran**: Bahasa bisnis portabel — berjalan di komputer mana pun
- COBOL-60: Versi pertama — penanganan file, laporan, aritmatika
### COBOL 68–74: Standardisasi (1968–1974)
- **COBOL-68**: Standar ANSI pertama
- **COBOL-74**:`EVALUATE`(pernyataan saklar), terstruktur`IF`/`ELSE`
- COBOL menjadi bahasa bisnis yang dominan di seluruh dunia
### COBOL 85: COBOL Terstruktur (1985)
- **Terminator cakupan**:`END-IF`,`END-PERFORM`,`END-READ`
-`EVALUATE`/`WHEN`(beralih)
-`PERFORM`/`END-PERFORM`(loop sebaris)
- Peningkatan `SECTION`
- Ini adalah versi sebagian besar kode COBOL ditulis
### COBOL 2002: COBOL Berorientasi Objek (2002)
- **Kelas dan metode** —`CLASS-ID`,`METHOD-ID`
- **Warisan** —`INHERITS`
- **Antarmuka** —`IMPLEMENTS`
- Kata kunci `FUNCTION`
- Tipe`BOOLEAN`(sebagian)
- Program bersarang
### COBOL 2014–2023: COBOL modern (2014–sekarang)
- **2014**: Dukungan XML, JSON parsial, tipe `BOOLEAN`
- **2023**: **JSON Asli** (PARSE JSON, GENERATE JSON), UTF-8, koleksi `LIST`
- COBOL terus berkembang untuk era modern
## Evolusi Sintaks
```cobol
