...
|----------|------|---------|--------|-------|
| `+`| Tambahan | `3 + 2`| `5`| Juga rangkaian string |
| `-`| Pengurangan | `3 - 2`| `1`| |
| `*`| Perkalian | `3 * 2`| `6`| |
| `/`| Divisi | `7 / 2`| `3.5`| |
| `%`| Sisa | `7 % 2`| `1`| |
| `**`| Eksponen | `2 ** 10`| `1024`| ES2016 |
| `++`| Kenaikan | `x++`/`++x`| | Pasca dan sebelum kenaikan |
| `--`| Penurunan | `x--`/`--x`| | Pasca dan sebelum pengurangan |
| `+x`,`-x`| Unary | `+"42"`→`42`| Ketik paksaan pada angka |
### Perbandingan & Kesetaraan
| Operator | Nama | Contoh | Catatan |
|----------|------|---------|-------|
| `===`| Kesetaraan Ketat | `1 === 1`| Tidak ada paksaan - selalu pilih ini |
| `!==`| Ketimpangan Ketat | `1 !== "1"`| `true`— berbagai jenis |
| `==`| Kesetaraan Abstrak | `1 == "1"`| Ketik paksaan — hindari |
| `!=`| Ketimpangan Abstrak | `1 != "1"`| `false`— hindari |
| `<`,`>`,`<=`,`>=`| Relasional | `"b" > "a"`| Leksikografis untuk string |
### Operator Logis & Nullish
| Operator | Nama | Contoh | Catatan |
|----------|------|---------|-------|
| `&&`| Logis DAN | `a && b`| Mengembalikan nilai falsy pertama atau terakhir |
| `\|\|`| Logis ATAU | `a \|\| b`| Mengembalikan nilai kebenaran pertama atau terakhir |
| `!`| Logis BUKAN | `!true`| `false`|
| `??`| Penggabungan Nullish | `a ?? b`| Mengembalikan`b`hanya jika`a`adalah`null`/`undefined`|
| `?.`| Rangkaian Opsional | `obj?.prop?.method()`| Hubungan pendek ke`undefined`|
### Operator Bitwise
| Operator | Nama | Contoh | Catatan |
|----------|------|---------|-------|
| `&`| DAN | `5 & 3`| `1`|
...
| `>>`| Ditandatangani Shift Kanan | `-5 >> 1`| `-3`|
| `>>>`| Pergeseran Kanan Tanpa Tanda Tangan | `-1 >>> 0`| `4294967295`|
### Penugasan & Penghancuran
```javascript
// Basic assignment
let x = 10;
...
|------------|-----------|---------------|
| 1 (tertinggi) | `()``[]``.``?.` | Kiri |
| 2 | `new`(tanpa argumen) | Benar |
| 3 | `++``--` (postfiks) | T/A |
| 4 | `!``~``+x``-x``typeof``void``delete``++``--`(awalan) | Benar |
| 5 | `**`| Benar |
| 6 | `*``/``%`| Kiri |
| 7 | `+``-` | Kiri |
| 8 | `<<``>>``>>>`| Kiri |
| 9 | `<``<=``>``>=``in``instanceof` | Kiri |
| 10 | `===``!==``==``!=` | Kiri |
| 11 | `&`| Kiri |
| 12 | `^`| Kiri |
| 13 | `\|`| Kiri |
...
| 16 | `??`| Kiri |
| 17 | `=``+=``-=`dll | Benar |
| 18 (terendah) | `=>`(fungsi panah) | Benar |
---
...
| Koleksi | Dipesan | Dapat diubah | Duplikat | Jenis Kunci | Pencarian |
|------------|---------|---------|------------|----------|--------|
| `Array`| Ya | Ya | Ya | Indeks (angka) | PADA(n) |
| `Object`| Ya (penyisipan) | Ya | Kunci: Tidak | String/Simbol | O(1) rata-rata |
| `Map`| Ya (penyisipan) | Ya | Kunci: Tidak | Apa saja | O(1) rata-rata |
| `Set`| Ya (penyisipan) | Ya | Tidak | — | O(1) rata-rata |
| `WeakMap`| Tidak | Ya | Kunci: Tidak | Hanya objek | O(1) rata-rata |
| `WeakSet`| Tidak | Ya | Tidak | Hanya objek | O(1) rata-rata |
### Array
```javascript
// Creation
...
## Ringkasan
Sintaks JavaScript tampak sederhana di permukaan namun mengandung kedalaman yang signifikan. Bahasa ini telah berevolusi dari alat skrip sederhana menjadi bahasa multi-paradigma dengan kelas, modul, generator, proxy, dan model asinkron yang canggih. Memahami prioritas operator, loop peristiwa, rantai prototipe, dan nuansa pengikatan`this`memisahkan pengembang yang kompeten dari pengembang yang ahli. JavaScript modern (ES2020+) dengan rangkaian opsional, penggabungan nullish,`Promise.allSettled`, dan menunggu tingkat atas terus mengurangi boilerplate sambil mempertahankan kompatibilitas ke belakang.