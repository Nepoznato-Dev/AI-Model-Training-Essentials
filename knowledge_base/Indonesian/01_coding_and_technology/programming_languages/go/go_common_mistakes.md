...
## 1. Pengambilan Variabel Loop di Goroutine
```go
...
## 2. Mengabaikan Pengembalian Error
```go
...
## 5. Dereferensi Nol Pointer pada Metode Struktur
```go
...
## 6. Menggunakan`defer`dalam satu Loop
```go
...
## 7. Anti-Pola: Mengembalikan Variabel Bernama Secara Prematur
```go
...
## 11. Tidak Menggunakan`sync.Once`untuk Inisialisasi
```go
...
## Ringkasan
Kesederhanaan Go adalah kelebihannya, namun ia memiliki jebakan halus: kebocoran goroutine dari saluran yang tidak di-buffer, semantik penambahan irisan yang tidak mengubah yang asli, panggilan yang ditangguhkan yang terakumulasi dalam loop, dan iterasi peta yang sengaja diacak. Pepatah Go memandu caranya: menangani setiap kesalahan, menerima antarmuka tetapi mengembalikan struct, menggunakan penerima pointer untuk tipe mutex, dan lebih memilih primitif konkurensi (`sync.Once`,`errgroup`, pembatalan konteks) dibandingkan manajemen goroutine manual. Tulis kode sederhana terlebih dahulu — Hadiahi kejelasan dibandingkan kepintaran.