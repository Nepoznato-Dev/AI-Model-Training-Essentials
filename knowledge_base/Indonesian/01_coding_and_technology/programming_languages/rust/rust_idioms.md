...
---
# Rust — Pola Idiomatik & Praktik Terbaik
Panduan ini mencakup pola idiomatik dan praktik terbaik untuk menulis kode Rust yang bersih dan aman.
---
...
## Ringkasan
Idiom Rust menekankan: kepemilikan dan peminjaman,`Result`/`Option`untuk penanganan kesalahan, enum dengan metode, sifat untuk abstraksi, rangkaian iterator, penunjuk cerdas untuk status bersama, dan abstraksi tanpa biaya. Ikuti`rustfmt`untuk pemformatan,`clippy`untuk linting, dan Pedoman Rust API. Komunitas Rust menghargai keamanan, kinerja, dan kebenaran — "jika dikompilasi, maka akan berhasil." Lebih suka`&str`daripada`String`dalam argumen fungsi, gunakan`?`untuk propagasi kesalahan, dan dapatkan ciri-ciri umum dengan`#[derive(Debug, Clone, ...)]`.