...
# Rust — Referensi Sintaks
Dokumen ini memberikan referensi sintaksis yang komprehensif dan terstruktur untuk Rust. Ini melengkapi referensi utama Rust dengan berfokus pada pola sintaksis yang lengkap, tabel operator, dan mekanisme internal sistem kepemilikan, sifat, dan konkurensi.
---
...
|----------|------|---------|-------|
| `+`| Tambahan | `3 + 2`| Juga penggabungan`String`melalui`format!`|
| `-`| Pengurangan | `3 - 2`| |
| `*`| Perkalian | `3 * 2`| |
| `/`| Divisi | `7 / 2`| Pembagian bilangan bulat menjadi bilangan bulat:`3`|
| `%`| Sisa | `7 % 2`| `1`|
| `-x`| Negasi | `-5`| |
### Perbandingan & Operator Logika
| Operator | Nama | Contoh | Catatan |
|----------|------|---------|-------|
| `==`| Sama | `x == y`| Membutuhkan`PartialEq`|
| `!=`| Tidak Sama | `x != y`| |
| `<`,`>`,`<=`,`>=`| Memesan | `x >= y`| Membutuhkan`PartialOrd`|
| `&&`| Logis DAN | `a && b`| Hubungan pendek |
| `\|\|`| Logis ATAU | `a \|\| b`| Hubungan pendek |
| `!`| Logis BUKAN | `!true`| |
### Operator Bitwise
| Operator | Nama | Contoh | Catatan |
|----------|------|---------|-------|
...
| `\|`| ATAU | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `!`| BUKAN | `!0b1010u8`| Pelengkap bitwise |
| `<<`| Pergeseran Kiri | `5 << 1`| `10`|
| `>>`| Pergeseran Kanan | `5 >> 1`| `2`|
### Prioritas Operator (tertinggi ke terendah)
| Prioritas | Operator | Catatan |
|------------|-----------|-------|
| 1 (tertinggi) |  Jalur`()``.` :: | |
| 2 | Panggilan metode, akses lapangan | Penundaan otomatis |
| 3 | `-`(negasi),`!`,`*`(deref) | |
| 4 | `as`(tipe pemeran) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
| 14 | `..``..=` | Rentang |
| 15 | `=``+=` dll | Tugas |
| 16 (terendah) | `return``break``continue`| |
---
...
## Struktur Data
### Ikhtisar Jenis Bawaan
| Ketik | Tumpukan/Tumpukan | Ukuran | Mutabilitas | Catatan |
|------|-----------|------|------------|-------|
| Primitif (`i32`,`f64`,`bool`,`char`) | Tumpukan | Memperbaiki | Tidak dapat diubah secara default | `Copy`|
| `[T; N]`(susunan) | Tumpukan | Memperbaiki | Dapat diubah dengan`mut`| Ukuran tetap,`Copy`jika T adalah`Copy`|
| `Vec<T>`| tumpukan | Dinamis | Dapat diubah dengan`mut`| Dapat ditanam |
| `String`| tumpukan | Dinamis | Dapat diubah dengan`mut`| UTF-8, dapat ditanam |
| `&[T]`(irisan) | Tumpukan (ptr gemuk) | Tampilan dinamis | Tergantung pada`&`atau`&mut`| Tampilan pinjaman |
| `HashMap<K, V>`| tumpukan | Dinamis | Dapat diubah dengan`mut`| O(1) pencarian rata-rata |
| `BTreeMap<K, V>`| tumpukan | Dinamis | Dapat diubah dengan`mut`| O(log n) pencarian, dipesan |
| `HashSet<T>`| tumpukan | Dinamis | — | O(1) rata-rata, tidak ada duplikat |
### Vektor
```rust
// Creation
...
### Ciri-ciri Perpustakaan Standar Umum
```rust
...
## Ringkasan
Sintaks Rust dibangun atas dasar jaminan keamanan yang diterapkan pada waktu kompilasi. Sistem kepemilikan, polimorfisme berbasis sifat, pencocokan pola, dan abstraksi tanpa biaya membentuk desain yang koheren di mana penyusunnya adalah mitra yang ketat namun adil. Dari aliran kontrol dasar hingga fitur-fitur canggih seperti makro, kode tidak aman, dan pointer cerdas, setiap konstruksi melayani tujuan pemrograman sistem yang aman dan efisien. Memahami interaksi antara masa pakai, peminjaman, dan sistem tipe adalah kunci untuk menguasai Rust.