...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmatika | `a + b`| |
| `..``...` | Rentang | `1...10`| `...`ditutup; `..<`setengah terbuka |
| `==``!=``<``>``<=``>=` | Perbandingan | `a == b`| Memerlukan`Equatable`/`Comparable`|
| `===``!==` | Identitas | `a === b`| Referensi yang sama (hanya kelas) |
| `&&``\|\|``!`| Logis | `a && b`| Hubungan pendek |
| `??`| Nihil menyatu | `a ?? b`| Mengembalikan`b`jika`a`nihil |
| `?.`| Rangkaian opsional | `a?.b`| Mengembalikan nihil jika`a`adalah nihil |
| `!`| Buka paksa | `a!`| Crash jika nihil — hindari |
| `&`| Parameter masuk | `swap(&a, &b)`| Lewati referensi |
### Operator Penanganan Nihil
```swift
...
## Pembungkus Properti & Pembuat Hasil
```swift
// Property wrapper
@propertyWrapper
struct Trimmed {
...
## Ringkasan
Sintaks Swift menyeimbangkan keamanan dengan ekspresi. Opsional menghilangkan error null, protokol memungkinkan komposisi fleksibel tanpa pewarisan, dan konkurensi modern (asinkron/menunggu, aktor) membuat pemrograman serentak yang aman dapat diakses. Bahasa ini terus berkembang dengan fitur-fitur seperti paket parameter, makro, dan kepemilikan — setiap penambahan membuat Swift lebih aman dan lebih kuat sekaligus mempertahankan janji intinya: kode yang aman dalam konstruksi.