...
---
# Python — Referensi Sintaks
Dokumen ini memberikan referensi sintaksis yang komprehensif dan terstruktur untuk Python. Ini melengkapi referensi utama Python dengan berfokus pada pola sintaksis yang lengkap, tabel operator, dan mekanisme internal.
---
...
|----------|------|---------|--------|-------|
| `+`| Tambahan | `3 + 2`| `5`| Juga rangkaian string/daftar |
| `-`| Pengurangan | `3 - 2`| `1`| Juga negasi unary |
| `*`| Perkalian | `3 * 2`| `6`| Juga pengulangan string/daftar |
| `/`| Divisi Sejati | `7 / 2`| `3.5`| Selalu mengembalikan float |
| `//`| Divisi Lantai | `7 // 2`| `3`| Bulatan menuju tak terhingga negatif |
| `%`| Modulus | `7 % 2`| `1`| Tanda pembagi kecocokan |
| `**`| Eksponen | `2 ** 10`| `1024`| Asosiatif kanan |
### Perbandingan & Operator Boolean
| Operator | Nama | Contoh | Catatan |
|----------|------|---------|-------|
| `==`| Sama | `x == y`| Perbandingan nilai (bukan identitas) |
| `!=`| Tidak Sama | `x != y`| |
| `<`,`>`,`<=`,`>=`| Memesan | `x >= y`| Dapat dirantai:`a < b < c`|
| `is`| Identitas | `x is y`| Objek yang sama di memori |
| `is not`| Bukan Identitas | `x is not None`| Lebih disukai daripada`!= None`|
| `in`| Keanggotaan | `x in collection`| O(1) untuk set/dikt, O(n) untuk daftar |
| `and`| Logis DAN | `a and b`| Hubungan pendek; mengembalikan nilai kebenaran/kesalahan terakhir |
| `or`| Logis ATAU | `a or b`| Hubungan pendek; mengembalikan nilai kebenaran/kesalahan pertama |
| `not`| Logis BUKAN | `not x`| |
### Operator Bitwise
| Operator | Nama | Contoh | Catatan |
|----------|------|---------|-------|
| `&`| DAN | `5 & 3`| `1`|
...
| 8 | `^`| Kiri |
| 9 | `\|`| Kiri |
| 10 | `==`,`!=`,`<`,`>`,`<=`,`>=`,`is`,`in`| Dapat dirantai |
| 11 | `not`| Benar |
| 12 | `and`| Kiri |
| 13 (terendah) | `or`| Kiri |
### Penugasan & Penugasan Tambahan
```python
...
### Penutupan & Fungsi Tingkat Tinggi
```python
...
## Struktur Data
### Ikhtisar Koleksi Bawaan
| Koleksi | Dipesan | Dapat diubah | Duplikat | Sintaks | Pencarian |
|------------|---------|---------|------------|--------|--------|
| `list`| Ya | Ya | Ya | `[1, 2, 3]`| PADA(n) |
| `tuple`| Ya | Tidak | Ya | `(1, 2, 3)`| PADA(n) |
| `dict`| Ya (3,7+) | Ya | Kunci: Tidak | `{"k": "v"}`| O(1) rata-rata |
| `set`| Tidak | Ya | Tidak | `{1, 2, 3}`| O(1) rata-rata |
| `frozenset`| Tidak | Tidak | Tidak | `frozenset({1,2})`| O(1) rata-rata |
### Daftar
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
## Ringkasan
Referensi sintaksis ini mencakup rangkaian fitur lengkap Python — mulai dari operator dasar dan aliran kontrol hingga metaprogramming tingkat lanjut, deskriptor, dan sistem tipe. Sintaks Python sengaja dibuat minimal dan mudah dibaca, namun kedalamannya sangat besar: pencocokan pola, generator asinkron, protokol, dan metakelas menyediakan alat yang ampuh untuk aplikasi yang kompleks. Bahasa ini terus berkembang, dengan setiap rilis menambahkan fitur yang meningkatkan ekspresi sekaligus menjaga kompatibilitas ke belakang.