...
## Mengapa Delphi Penting
- **Pengembangan aplikasi yang cepat**: Perancang bentuk visual + kompilasi asli membuat pembuatan GUI Windows menjadi sangat cepat.
- **Kinerja asli**: Mengompilasi langsung ke kode mesin — tidak diperlukan runtime atau VM.
- **Konektivitas basis data**: Komponen basis data yang secara historis sangat baik (dbExpress, FireDAC, ADO).
- **Basis kode lama**: Banyak aplikasi perusahaan yang masih berjalan di Delphi; pemeliharaan adalah keterampilan khusus.
- **Lintas platform (modern)**: Kerangka kerja FireMonkey menargetkan Windows, macOS, iOS, dan Android dari satu basis kode.
## Pengorbanan
| Batasan | Detail | Solusi Khas |
|-----------|---------|-------------------|
| **Komunitas yang menurun** | Jauh lebih kecil dari komunitas C#, Java, atau JavaScript | Forum aktif tapi kecil; dukungan komersial dari Idera |
| **Ekosistem terbatas** | Lebih sedikit perpustakaan pihak ketiga dibandingkan bahasa modern | Gunakan pustaka komponen VCL/FMX; tulis komponen khusus |
| **Utamanya berfokus pada Windows** | Dukungan lintas platform (FMX) kurang matang | Gunakan teknologi C#, Flutter, atau web untuk lintas platform |
| **Biaya perizinan** | IDE Komersial memerlukan lisensi berbayar | Alternatif sumber terbuka: Pascal / Lazarus Gratis |
| **Kesulitan perekrutan** | Lebih sedikit pengembang Delphi yang memasuki pasar | Mempertahankan sistem yang ada; memigrasikan fitur baru ke tumpukan modern |
---
...
### Antarmuka dan Injeksi Ketergantungan
```pascal
unit DIExample;
...
### Arsitektur Komponen
VCL (Visual Component Library) Delphi dan FMX (FireMonkey) dibangun berdasarkan hierarki komponen. Setiap elemen visual mewarisi dari`TComponent`.
```
TObject
└── TPersistent
    └── TComponent
...
### Referensi Petunjuk Kompiler
| Petunjuk | Tujuan | Contoh |
|-----------|---------|---------|
| `{$APPTYPE CONSOLE}`| Aplikasi konsol | `{$APPTYPE CONSOLE}`|
| `{$APPTYPE GUI}`| Aplikasi GUI (default) | `{$APPTYPE GUI}`|
| `{$DEFINE DEBUG}`| Definisikan simbol kondisional | `{$DEFINE DEBUG}`|
| `{$IFDEF symbol}`| Kompilasi bersyarat | `{$IFDEF DEBUG}`|
| `{$R *.dfm}`| Sertakan sumber daya formulir | `{$R *.dfm}`|
| `{$WARNINGS OFF}`| Menekan peringatan | `{$WARNINGS OFF}`|
| `{$HINTS OFF}`| Menekan petunjuk | `{$HINTS OFF}`|
| `{$OPTIMIZATION ON}`| Aktifkan pengoptimal | `{$OPTIMIZATION ON}`|
| `{$STRINGCHECKS ON}`| Aktifkan pemeriksaan rentang string | `{$STRINGCHECKS ON}`|
...
| Fitur | Deskripsi |
|---------|-------------|
| **Titik henti sementara** | Atur pada baris mana pun yang dapat dieksekusi; breakpoint bersyarat didukung |
| **Jendela tontonan** | Pantau nilai variabel secara real-time |
| **Tumpukan panggilan** | Lihat rantai panggilan lengkap dengan variabel lokal |
| **Jendela CPU** | Lihat perakitan yang dihasilkan bersama kode sumber |
| **Tampilan memori** | Periksa memori mentah di alamat mana pun |
| **Titik henti acara** | Istirahat pada pengecualian, pemuatan DLL, acara thread |
| **Debug jarak jauh** | Debug aplikasi yang berjalan pada mesin jarak jauh |
| **Visualisasi data** | Visualisator khusus untuk kumpulan data, string, koleksi |
### Kerangka Pengujian DUnit
```pascal
...
## Pola Desain
### Pola 1: Singleton (Aman untuk Thread)
```pascal
type
  TAppConfig = class
  private
...
### Pola 2: Pengamat (Berbasis Peristiwa)
```pascal
type
...
### Kiat Pengoptimalan VCL/FMX
| Teknik | Dampak | Deskripsi |
|-----------|--------|-------------|
| **Pembaruan Awal/Pembaruan Akhir** | Tinggi | Bungkus pembaruan daftar/kisi untuk mencegah pengecatan ulang |
| **Buffering ganda** | Sedang | Atur`DoubleBuffered := True`untuk mengurangi kedipan |
| **Daftar virtual** | Tinggi | Gunakan`TVirtualStringTree`untuk kumpulan data besar |
| **String magang** | Sedang | Gunakan kembali konstanta string; hindari penggabungan berulang |
| **Penggabungan objek** | Sedang | Gunakan kembali objek yang sering dibuat/dihancurkan |
| **Pemuatan lambat** | Tinggi | Muat data/formulir hanya bila diperlukan |
| **Pengoptimalan kompiler** | Sedang | Aktifkan`{$O+}`untuk versi rilis |
---
...
| Industri | Aplikasi | Mengapa Delphi |
|----------|-------------|-----------|
| **Keuangan** | Platform perdagangan, dasbor perbankan | GUI asli yang cepat, konektivitas database |
| **Perawatan Kesehatan** | Pencitraan medis, manajemen pasien | Komponen VCL, performa asli |
| **Manufaktur** | Sistem SCADA, kontrol industri | Akses perangkat keras langsung, respons waktu nyata |
| **Pemerintah** | Alat administrasi internal | Kontinuitas sistem lama |
| **Telekomunikasi** | Dasbor pemantauan jaringan | Visualisasi data yang cepat |
| **Pendidikan** | Perangkat lunak pendidikan, alat e-learning | Perkembangan pesat, dukungan multimedia |
---

## Kapan Menggunakan Delphi
| Skenario | Mengapa Delphi | Alternatif Lebih Baik |
|----------|-----------|-------------------|
| Pemeliharaan Delphi lama | Basis kode yang ada | — |
| Aplikasi desktop Windows (cepat) | VCL sudah matang dan cepat | C# (WPF/WinForms) |
| Front-end basis data | Komponen data yang sangat baik | C#, Jawa |
| Desktop lintas platform (ceruk) | FireMonkey ada | C#, Flutter, Elektron |
| Pengembangan GUI Windows baru | Mungkin tapi komunitasnya menyusut | C# (WPF/WinUI 3) |
| Pengembangan web | Tidak cocok | JavaScript, Python, C# |
