...
---
# Scala — Kesalahan Umum & Anti-Pola
Dokumen ini mengkatalogkan kesalahan, jebakan, dan anti-pola paling umum di Scala dengan koreksinya.
---

## 1. Menggunakan`var`Daripada `val`
```scala
...
## 2. Tidak Menggunakan Pencocokan Pola untuk Tipe Tersegel
```scala
// ❌ WRONG — if/else chain
...
## 4. Panggilan Balik Masa Depan Neraka
```scala
...
## 5. Penyalahgunaan Konversi Implisit
```scala
...
## 6. Tidak Menangani`Option`dengan Benar
```scala
...
## 8. Ketik Penghapusan dengan Generik
```scala
...
## Ringkasan
Kekuatan Scala hadir dengan tanggung jawab: lebih memilih`val`daripada`var`, menggunakan pencocokan pola untuk tipe yang disegel, menangani`Option`dengan benar (jangan pernah memanggil`.get`), gunakan untuk pemahaman alih-alih Futures bersarang, dan hindari null. Scala 3 menyederhanakan banyak pola ini dengan enum, metode ekstensi, dan tipe gabungan. Prinsip kuncinya: jika kompiler dapat memeriksanya, biarkan kompiler memeriksanya.