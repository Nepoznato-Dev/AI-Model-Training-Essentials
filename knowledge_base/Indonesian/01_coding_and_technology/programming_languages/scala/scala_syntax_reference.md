...
# Scala — Referensi Sintaks
Dokumen ini memberikan referensi sintaksis yang komprehensif dan terstruktur untuk Scala (3.x dengan catatan tentang perbedaan 2.x). Ini melengkapi referensi utama Scala dengan berfokus pada pola sintaksis yang lengkap, sistem tipe, konstruksi pemrograman fungsional, dan idiom Scala.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmatika | `a + b`| |
| `**`| Daya (Desimal Besar) | `BigDecimal(2) ** 10`| Tidak bawaan untuk Int |
| `==``!=` | Kesetaraan | `a == b`| Memanggil`.equals()`— null-safe |
| `eq``ne` | Kesetaraan referensi | `a eq b`| Contoh yang sama |
| `<``>``<=``>=` | Perbandingan | `a >= b`| Membutuhkan`Ordered`|
| `&&``\|\|``!`| Logis | `a && b`| Hubungan pendek |
| `&``\|``^``~` | Sedikit demi sedikit | `a & b`| |
| `<<``>>``>>>`| Pergeseran | `a << 2`| |
| `+:``:+``::``:::` | Operasi pengumpulan | `1 +: list`| Asosiatif kanan jika diakhiri dengan`:`|
| `->`| Pembuatan tupel | `"key" -> "value"`| `("key", "value")`|
| `=>`| Fungsi/panah | `x => x + 1`| Lambda atau ketik pemetaan |
| `<-`| Pembangkit | `x <- List(1,2,3)`| Untuk pemahaman |
### Aturan Operator
```scala
...
## Ringkasan
Sintaks Scala menyatukan pemrograman berorientasi objek dan fungsional di bawah sistem tipe yang kuat. Pencocokan pola, pemahaman, dan kelas kasus menyediakan pemodelan data yang ekspresif. Sifat memungkinkan komposisi yang fleksibel tanpa batasan warisan tunggal. Sistem tipe — dengan generik, varians, batasan tipe, dan tipe buram — menangkap kesalahan pada waktu kompilasi sekaligus mengaktifkan abstraksi tanpa biaya. Scala 3 menyederhanakan bahasa dengan enum, metode ekstensi, tipe yang diberikan/digunakan, dan buram, membuat bahasa lebih mudah didekati sambil mempertahankan kekuatan penuhnya.