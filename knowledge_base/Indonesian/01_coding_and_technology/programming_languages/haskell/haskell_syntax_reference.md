...
# Haskell — Referensi Sintaks
Dokumen ini memberikan referensi sintaksis yang komprehensif dan terstruktur untuk Haskell (GHC 9.x). Ini melengkapi referensi utama Haskell dengan berfokus pada pola sintaksis yang lengkap, sistem tipe, monad, evaluasi malas, dan idiom pemrograman fungsional.
---
...
|----------|------|---------|-------|
| `+``-``*``/` | Aritmatika | `x + y`| Nomor kelas tipe |
| `div``mod` | Pembagian bilangan bulat | `7 \`div\` 2`| Pengembalian 3 |
| `^``**` | Kekuatan | `2 ^ 10`| `^`untuk integral; `**`untuk mengambang |
| `==``/=` | Kesetaraan | `x == y`| Persamaan kelas tipe |
| `<``>``<=``>=` | Perbandingan | `x >= y`| Kelas tipe pesanan |
| `&&``\|\|``not`| Logis | `a && b`| Hubungan pendek |
| `.`| Komposisi fungsi | `(f . g) x`| `f (g x)`|
| `$`| Aplikasi fungsi | `f $ x + 1`| `f (x + 1)`— prioritas rendah |
| `<>`| Penambahan monoid | `"hello" <> " world"`| Semigrup |
| `<$>`| Peta fungsi | `(+1) <$> Just 5`| `fmap (+1) (Just 5)`|
| `<*>`| Berlaku aplikatif | `pure (+) <*> Just 3`| |
| `>>=`| Ikatan Monad | `Just 5 >>= \x -> Just (x+1)`| |
| `>>`| Urutan Monad | `putStrLn "hi" >> putStrLn "bye"`| |
| `=<<`| Ikat (terbalik) | `f =<< mx`| Sama seperti`mx >>= f`|
| `++`| Penggabungan daftar | `[1,2] ++ [3,4]`| O(n) di daftar kiri |
| `:`| Kontra | `1 : [2, 3]`| HAI(1) |
| `!!`| Indeks | `[1,2,3] !! 1`| O(n) — fungsi parsial |
| `\\`| Daftar perbedaan | `[1,2,3] \\ [2]`| `[1,3]`|
### Prioritas Operator (dipilih)
| Prioritas | Operator |
|------------|-----------|
| 7 (tertinggi) | `*``/``div``mod` |
| 6 | `+``-` |
...
## Ringkasan
Sintaks Haskell minimal namun sangat ekspresif. Sistem tipe — dengan tipe data aljabar, kelas tipe, GADT, dan kelompok tipe — berfungsi sebagai spesifikasi dan alat verifikasi. Pencocokan pola, evaluasi malas, dan monad memberikan solusi elegan untuk masalah kompleks. Kemurnian Haskell (tidak ada efek samping dalam tipe) membuat kode lebih mudah untuk dipikirkan, diuji, dan diparalelkan. Meskipun kurva pembelajarannya curam, pengaruh Haskell meresap ke dalam bahasa-bahasa modern — mulai dari opsi Swift, kelas tersegel Kotlin, hingga sistem tipe Rust. Menguasai Haskell membuat Anda menjadi programmer yang lebih baik dalam setiap bahasa.