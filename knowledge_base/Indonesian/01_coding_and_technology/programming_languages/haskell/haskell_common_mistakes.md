...
# Haskell — Kesalahan Umum & Anti-Pola
Dokumen ini berisi katalog kesalahan, jebakan, dan anti-pola paling umum di Haskell beserta koreksinya.
---

## 1. Daftar Tak Terbatas dan Kejutan Kemalasan
```haskell
...
## 5. Tidak Menggunakan Sintaks Record dengan Benar
```haskell
...
## 7. Anti-Pola: Terlalu sering menggunakan `IO`
```haskell
...
## Ringkasan
Kemurnian dan kemalasan Haskell menciptakan kendala unik: kebocoran ruang akibat evaluasi yang malas, sebagian fungsi yang mogok saat runtime, kebingungan antara String/Teks/ByteString, dan penggunaan IO yang berlebihan. Cara Haskell adalah: gunakan evaluasi ketat untuk akumulator (`foldl'`, BangPatterns), hindari fungsi parsial (`head`,`read`,`!!`), gunakan`Maybe`/`Either`untuk penanganan kesalahan, gunakan`Text`untuk teks, dan pertahankan IO minimal. Sistem tipe kompiler adalah sekutu terbesar Anda — biarkan sistem ini memandu Anda menuju fungsi yang total dan murni.