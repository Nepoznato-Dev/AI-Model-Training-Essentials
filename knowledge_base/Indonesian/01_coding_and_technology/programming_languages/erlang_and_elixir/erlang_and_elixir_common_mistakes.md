...
## 2. Tidak Menghubungkan/Memantau Proses (Erlang)
```erlang
...
## 3. Panggilan GenServer Tanpa Batas Waktu (Elixir)
```elixir
...
## 4. Pencocokan Pola di`receive`Tanpa Batas Waktu (Erlang)
```erlang
...
## 5. Tidak Menggunakan`with`untuk Pencocokan Pola Bersarang (Elixir)
```elixir
...
## 6. Anti-Pola: Callback GenServer yang Berjalan Lama
```elixir
...
## Ringkasan
Perangkap Erlang/Elixir: variabel bersifat penugasan tunggal, proses memerlukan tautan/monitor, panggilan GenServer memerlukan batas waktu yang jelas,`receive`memblokir selamanya tanpa batas waktu, dan panggilan balik GenServer tidak boleh diblokir. Cara BEAMnya adalah: biarkan crash (dengan pengawasan), gunakan`with`untuk pencocokan pola yang bersih, pertahankan callback GenServer dengan cepat, dan selalu tangani kegagalan proses.