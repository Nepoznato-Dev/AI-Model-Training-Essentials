...
| `*`| Perkalian | `3 * 2`| `6`| |
| `/`| Divisi | `7 / 2`| `3`| Pemotongan bilangan bulat untuk tipe int |
| `%`| Modulus | `7 % 2`| `1`| Hanya untuk tipe integer |
| `++`| Kenaikan | `i++`/`++i`| | Pasca dan sebelum kenaikan |
| `--`| Penurunan | `i--`/`--i`| | Pasca dan sebelum pengurangan |
...
| `\|`| ATAU | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `~`| Pelengkap | `~0`| Semua bit dibalik |
| `<<`| Pergeseran Kiri | `1 << 3`| `8`|
| `>>`| Pergeseran Kanan | `8 >> 3`| `1`(ditentukan implementasi untuk ditandatangani) |
### Operator Penunjuk & Alamat
| Operator | Nama | Contoh | Catatan |
|----------|------|---------|-------|
| `&`| Alamat | `&x`| Mengembalikan penunjuk ke x |
| `*`| Dereferensi | `*ptr`| Akses nilai pada penunjuk |
| `->`| Akses anggota melalui pointer | `p->x`| Setara dengan`(*p).x`|
| `.`| Akses anggota | `s.field`| Akses anggota struct langsung |
| `[]`| Subskrip array | `arr[i]`| Setara dengan`*(arr + i)`|
### Prioritas Operator (tertinggi ke terendah)
| Prioritas | Operator | Asosiatif |
|------------|-----------|---------------|
| 1 (tertinggi) | `()``[]``->``.` | Kiri ke kanan |
| 2 | `!``~``+``-` (unary)`*``&``sizeof``(type)` | Kanan ke kiri |
| 3 | `*``/``%`| Kiri ke kanan |
| 4 | `+``-` | Kiri ke kanan |
| 5 | `<<``>>` | Kiri ke kanan |
| 6 | `<``<=``>``>=` | Kiri ke kanan |
| 7 | `==``!=` | Kiri ke kanan |
| 8 | `&`(bitwise DAN) | Kiri ke kanan |
| 9 | `^`| Kiri ke kanan |
| 10 | `\|`| Kiri ke kanan |
| 11 | `&&`| Kiri ke kanan |
| 12 | `\|\|`| Kiri ke kanan |
| 13 | `? :`(terner) | Kanan ke kiri |
| 14 | `=``+=``-=``*=``/=``%=``&=``\|=``^=``<<=``>>=`| Kanan ke kiri |
| 15 (terendah) | `,`(koma) | Kiri ke kanan |
---
...
## Ringkasan
Sintaks C minimal dan mirip dengan perangkat keras — penunjuk, manajemen memori manual, dan lapisan abstraksi tipis pada instruksi mesin. Kekuatan bahasa ini berasal dari kesederhanaan ini: hampir tidak ada biaya tersembunyi, tidak ada overhead runtime, dan kontrol penuh atas tata letak memori. Praprosesor, meskipun kuat, memerlukan disiplin agar dapat digunakan dengan aman. C modern (C11/C17/C23) telah menambahkan fitur keselamatan penting seperti`_Static_assert`,`_Generic`,`nullptr`, dan`typeof`, menjadikan bahasa ini secara bertahap lebih aman dengan tetap mempertahankan filosofi intinya yaitu memercayai pemrogram.