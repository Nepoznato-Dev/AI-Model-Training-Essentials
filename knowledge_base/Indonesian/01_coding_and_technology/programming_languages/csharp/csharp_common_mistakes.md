...
Dokumen ini mengkatalogkan kesalahan, jebakan, dan anti-pola paling umum di C#. Setiap entri menunjukkan pendekatan yang salah, menjelaskan mengapa gagal, dan memberikan solusi yang tepat.
---

## 1. Tidak Membuang Objek `IDisposable`
```csharp
// ❌ WRONG — resource leak
...
## 4. LINQ`.ToList()`Alokasi yang Tidak Perlu
```csharp
...
## 5. Menangkap Variabel Loop (Pra-C# 5.0)
```csharp
...
## 6. Tidak Menggunakan Pencocokan Pola
```csharp
...
## 7. Anti-Pola: Kelas Dewa / Pencari Lokasi Layanan
```csharp
...
## 10. Tidak Menggunakan Catatan untuk Tipe Data
```csharp
...
## 11. Penyalahgunaan Filter Pengecualian
```csharp
...
## Ringkasan
C# adalah bahasa yang dirancang dengan baik tetapi memiliki jebakan: tidak membuang objek `IDisposable`, metode async void, alokasi LINQ yang tidak perlu, dan pengecualian referensi nol. C# modern (8+) menyediakan alat canggih: tipe referensi yang dapat dibatalkan, pencocokan pola, catatan, deklarasi penggunaan, dan ekspresi peralihan. Prinsip utamanya: selalu membuang sumber daya, jangan pernah menggunakan async void (kecuali event handler), gunakan injeksi konstruktor, pilih rekaman untuk data, dan aktifkan jenis referensi yang dapat dibatalkan untuk menangkap bug nol pada waktu kompilasi.