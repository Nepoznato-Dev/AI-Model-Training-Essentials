...
|----------|------|---------|-------|
| `+``-``*``/``^`| Aritmatika | `2^10`| |
| `%%`| Modulus | `7 %% 3`| Pengembalian 1 |
| `%/%`| Pembagian bilangan bulat | `7 %/% 3`| Pengembalian 2 |
| `==``!=` | Kesetaraan | `x == y`| Divektorkan |
| `<``>``<=``>=` | Perbandingan | `x >= y`| Divektorkan |
| `&``\|``!`| Logis (divektorkan) | `x & y`| Dari segi elemen |
| `&&``\|\|` | Logis (skalar) | `x && y`| Hanya elemen pertama |
| `%in%`| Keanggotaan | `x %in% y`| |
| `<-``=` | Tugas | `x <- 10`| `<-`adalah idiom |
| `->`| Tetapkan kanan | `10 -> x`| Jarang digunakan |
| `<<-`| Penugasan global | `x <<- 10`| Tetapkan di env induk |
| `$`| Ekstrak elemen | `df$column`| |
| `[``[[` | Pengindeksan | `x[1]`/`x[[1]]`| `[`mengembalikan tipe yang sama;  Ekstrak`[[`|
| `%>%`| Pipa (magrittr) | `x %>% f()`| |
| `\|>`| Pipa (basis R 4.1+) | `x \|> f()`| Pipa asli |
### Operasi Vektor
```r
...
## Ringkasan
Sintaks R dirancang untuk komputasi statistik dan analisis data. Sifat operasi yang divektorkan menghilangkan loop untuk sebagian besar tugas. Ekosistem yang rapi (dplyr, ggplot2, rapir) menyediakan tata bahasa yang konsisten dan mudah dibaca untuk manipulasi dan visualisasi data. Kemampuan pemodelan statistik R — mulai dari uji-t dasar hingga model efek campuran — memiliki kedalaman yang tak tertandingi. Untuk ilmu data, statistik, dan penelitian yang dapat direproduksi, R tetap menjadi alat yang penting.