...
## Mengapa R Penting
- **Komputasi statistik**: Kumpulan metode statistik terlengkap dalam bahasa apa pun.
- **Visualisasi data**: ggplot2 menghasilkan grafik berkualitas publikasi. Tata bahasa paradigma grafis tidak tertandingi.
- **Riset yang dapat direproduksi**: R Markdown / Quarto memungkinkan Anda menggabungkan kode, hasil, dan narasi dalam satu dokumen.
- **Standar akademik**: Digunakan dalam statistik, bioinformatika, epidemiologi, ekologi, ekonomi, dan ilmu sosial.
- **Tidyverse**: Kumpulan paket kohesif (dplyr, ggplot2, rapir, readr) yang membuat analisis data menjadi elegan dan konsisten.
- **Gratis dan sumber terbuka**: Tanpa biaya lisensi; dipelihara secara aktif oleh komunitas global.
## Pengorbanan
| Batasan | Detail | Solusi Khas |
|-----------|---------|-------------------|
| **Kinerja** | Berulir tunggal secara default; lambat untuk kumpulan data besar | Gunakan`data.table`, paket paralel, atau Rcpp untuk integrasi C++ |
| **Penggunaan memori** | Memuat seluruh kumpulan data ke dalam RAM | Gunakan`data.table::fread`, paket panah untuk pemrosesan di luar inti |
| **Bukan bahasa tujuan umum** | Canggung untuk pengembangan web, pemrograman sistem, atau aplikasi | Gunakan Python, Go, atau JavaScript untuk tugas non-statistik |
| **Sintaks tidak konsisten** | Basis R memiliki keunikan; paket yang berbeda menggunakan konvensi yang berbeda | Gunakan rapiverse untuk konsistensi |
| **Pasar kerja** | Sebagian besar peran akademis/penelitian | Peran ilmu data semakin memilih Python |
---
...
### Evaluasi Non-Standar dan Metaprogramming
```r
library(rlang)
...
### Manajemen Ketergantungan dengan renv
```r
...
## Pola Desain
### Pola 1: Evaluasi Rapi untuk Fungsi Fleksibel
```r
...
### Pola 2: Pemrograman Fungsional dengan purrr
```r
library(purrr)
...
### Pola 3: Saluran Pembuatan Laporan
```r
library(rmarkdown)
...
## Kapan Menggunakan R
| Skenario | Mengapa R | Alternatif Lebih Baik |
|----------|-------|-------------------|
| Analisis statistik | Metode statistik terlengkap | Python (statsmodel, scipy) |
| Visualisasi data | ggplot2 tidak tertandingi dalam kualitas publikasi | Python (matplotlib, seaborn) untuk interaktif |
| Penelitian akademis | Standar di banyak bidang | — |
| Bioinformatika | Biokonduktor memiliki 2.000+ paket khusus | Python untuk saluran produksi |
| Pelaporan (Penurunan Harga R/Kuarto) | Analisis + narasi terpadu | Jupyter (Piton) |
...
## Ringkasan
R adalah bahasa statistik. Untuk analisis data, visualisasi, dan pemodelan statistik, kedalaman dan cakupannya masih belum tertandingi. Rapiverse telah memodernisasi bahasanya, dan R Markdown/Quarto membuat penelitian yang dapat direproduksi menjadi mudah. Meskipun Python secara umum telah menguasai ilmu data, R tetap menjadi alat spesialis untuk pekerjaan statistik yang ketat. Bagi siapa pun yang melakukan penelitian kuantitatif, mempelajari R sangatlah penting.
---

## Perselisihan Data Tingkat Lanjut
### data.table Menyelami Lebih Dalam
```r
library(data.table)
...
### Evaluasi rapi dan Kolom Dinamis
```r
library(dplyr)
library(rlang)
...
## Penyelaman Mendalam Pemodelan Statistik
### Regresi Lanjutan
```r
...
### Pembelajaran Mesin dengan model yang rapi
```r
library(tidymodels)
