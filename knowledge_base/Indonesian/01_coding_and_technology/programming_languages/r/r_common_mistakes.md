...
## 1. Menumbuhkan Bingkai Data dalam Loop
```r
# ❌ WRONG — O(n²) growth
result <- data.frame()
for (i in 1:1000) {
...
## 3. Tidak Memahami Daur Ulang Vektor
```r
...
## 5. Tidak Menggunakan `stringsAsFactors = FALSE`
```r
...
## Ringkasan
Sifat interaktif R menciptakan jebakan: menumbuhkan bingkai data dalam loop (pra-alokasi atau vektorisasi), kejutan daur ulang vektor, propagasi `NA`, dan`T`/`F`dapat ditimpa. Cara R adalah: melakukan vektorisasi operasi, melakukan pra-alokasi saat melakukan perulangan, menangani`NA`secara eksplisit, menggunakan rapiverse untuk manipulasi data, dan menghindari`<<-`. R memberi penghargaan kepada mereka yang berpikir dalam vektor dan menggunakan ekosistem paketnya yang kaya.