...
## Bash — Operator & Variabel
### Operator Inti
| Operator | Nama | Contoh | Catatan |
|----------|------|---------|-------|
| `=`| Tugas | `x=10`| Tidak ada spasi di sekitar`=`|
| `$var`| Ekspansi variabel | `echo $HOME`| |
| `${var}`| Ekspansi yang diperkuat | `${HOME}/docs`| Disambiguasi |
| `$(cmd)`| Substitusi perintah | `$(date +%Y)`| Lebih disukai daripada backticks |
| `$((expr))`| Aritmatika | `$((2 + 3))`| |
| `${var:-default}`| Nilai bawaan | `${PORT:-8080}`| Jika tidak disetel atau kosong |
| `${var:=default}`| Tetapkan bawaan | `${count:=0}`| |
| `${#var}`| Panjang senar | `${#name}`| |
| `${var%%pattern}`| Hapus akhiran terpanjang | `${file%%.*}`| Hapus ekstensi |
| `${var##pattern}`| Hapus awalan terpanjang | `${path##*/}`| Dapatkan nama file |
| `"``"` | Kutipan ganda | `"$var"`| Memungkinkan perluasan |
| `'``'` | Kutipan tunggal | `'$var'`| String harfiah |
### Operator Uji
```bash
# File tests
...
## PowerShell — Cmdlet & Saluran Pipa
```powershell
...
## PowerShell — Fungsi & Pola Tingkat Lanjut
```powershell
# Basic function
...
## Ringkasan
Bash dan PowerShell mewakili dua paradigma skrip shell. Teks pipa Bash — setiap perintah mengubah string. PowerShell menyalurkan objek — setiap perintah menghasilkan data terstruktur dengan properti dan metode. Bash mendominasi Linux/macOS dan DevOps. PowerShell sangat penting untuk administrasi Windows dan semakin bersifat lintas platform. Keduanya adalah alat penting dalam tumpukan teknologi modern. Skrip shell adalah perekat yang menghubungkan sistem, mengotomatiskan alur kerja, dan menyelesaikan berbagai hal.