...
-->
# Perbandingan Lintas Bahasa — Penanganan Kesalahan
## Ikhtisar Model Penanganan Kesalahan
| Model | Bahasa | Mekanisme |
|-------|-----------|-----------|
| **Pengecualian (coba/tangkap)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **Jenis Hasil/Opsi** | Rust, Haskell, OCaml, Scala (juga) | `Result<T, E>`,`Maybe a`|
| **Beberapa nilai pengembalian** | Ayo, Lua | `value, err := f()`|
| **Kode kesalahan** | C, Fortran | Kembalikan kode kesalahan, periksa secara manual |
| **Kesalahan pencocokan pola** | Ramuan, Erlang | `{:ok, value}`/`{:error, reason}`|
| **Kondisi (dapat dimulai ulang)** | Cadel Umum | `restart-case`,`handler-bind`|
| **Tidak ada penanganan kesalahan** | Gores, SQL (dasar) | T/A |
| **Kontrak** | Ada, Eiffel |  Kondisi`Pre`,`Post`|
## Penanganan Kesalahan Berbasis Pengecualian
```python
# Python
try:
    result = 10 / 0
...
## Ringkasan: Paradigma Penanganan Kesalahan
| Paradigma | Pendekatan | Kelebihan | Kontra |
|----------|----------|------|------|
| **Pengecualian** | melempar/menangkap | Bersihkan jalur bahagia, tumpukan jejak | Bisa dilupakan, penyalahgunaan aliran kontrol |
| **Jenis hasil** | Kembalikan`Ok`/`Err`| Eksplisit, tidak dapat mengabaikan kesalahan | Verbose, dibutuhkan operator`?`|
| **Kode kesalahan** | Kembalikan status int | Sederhana, kompatibel dengan C | Mudah diabaikan, tanpa konteks |
| **Pengembalian berganda** | `value, err := f()`| Eksplisit, idiomatis | Bisa lupa cek`err`|
| **Pencocokan pola** | `{:ok, v}`/`{:error, r}`| Lengkap, fungsional | Verbose tanpa`with`|
| **Kondisi** | Penangan yang dapat dimulai ulang | Paling kuat, dapat dipulihkan | Kompleks, jarang dipahami |