...
## اسکرپٹ کا ڈھانچہ
ہر باش اسکرپٹ ایک **شیبانگ** لائن سے شروع ہوتا ہے:
```bash
#!/usr/bin/env bash
...
| پرچم | اثر |
|------|---------|
| `set -e`| اگر کوئی کمانڈ ناکام ہو جائے تو فوراً باہر نکلیں |
| `set -u`| غیر سیٹ متغیر کو غلطیوں کے طور پر سمجھیں |
| `set -o pipefail`| پائپ لائن فیل ہو جاتی ہے اگر اس میں کوئی کمانڈ ناکام ہو جاتی ہے |
| `set -x`| ہر کمانڈ کو عمل میں لانے سے پہلے پرنٹ کریں (ڈیبگ موڈ) |
اسکرپٹ چلائیں:`chmod +x script.sh && ./script.sh`یا `bash script.sh`
---
...
| `$0`| اسکرپٹ کا نام |
| `$1`,`$2`, ... | مؤقف کے دلائل |
| `$#`| پوزیشنی دلائل کی تعداد |
| `$@`| تمام پوزیشنی دلائل (الگ الگ الفاظ کے طور پر) |
| `$*`| تمام پوزیشنی دلائل (ایک سٹرنگ کے طور پر) |
| `$?`| آخری کمانڈ کی خارجی حیثیت (0 = کامیابی) |
| `$$`| موجودہ عمل کا PID |
| `$!`| آخری پس منظر کے عمل کا PID |
| `$_`| پچھلے حکم کی آخری دلیل |
---
...
| ٹیسٹ | معنی |
|------|---------|
| `[[ -f "$x" ]]`| فائل موجود ہے اور ایک باقاعدہ فائل ہے۔
| `[[ -d "$x" ]]`| ڈائریکٹری موجود ہے |
| `[[ -e "$x" ]]`| فائل/ڈائریکٹری موجود ہے (کچھ بھی) |
| `[[ -r "$x" ]]`| پڑھنے کے قابل |
| `[[ -w "$x" ]]`| قابل تحریر |
| `[[ -x "$x" ]]`| قابل عمل |
| `[[ -z "$x" ]]`| سٹرنگ خالی ہے |
| `[[ -n "$x" ]]`| سٹرنگ خالی نہیں ہے |
| `[[ "$a" == "$b" ]]`| سٹرنگ مساوات |
| `[[ "$a" != "$b" ]]`| سٹرنگ عدم ​​مساوات |
| `[[ "$a" =~ regex ]]`| ریجیکس میچ |
| `[[ $a -eq $b ]]`| عددی مساوات |
| `[[ $a -ne $b ]]`| عددی عدم مساوات |
| `[[ $a -gt $b ]]`| سے بڑا |
| `[[ $a -lt $b ]]`| سے کم |
| `[[ $a -ge $b ]]`| سے بڑا یا برابر |
| `[[ $a -le $b ]]`| سے کم یا برابر |
### منطقی آپریٹرز
```bash
[[ -f "$a" && -f "$b" ]]   # AND
[[ -f "$a" || -f "$b" ]]   # OR
...
## پائپنگ اور ری ڈائریکشن
| نحو | معنی |
|---------|---------|
| `cmd > file`| stdout فائل پر ری ڈائریکٹ کریں (اوور رائٹ) |
| `cmd >> file`| stdout کو فائل میں ری ڈائریکٹ کریں (ضمیمہ) |
| `cmd 2> errors.log`| ری ڈائریکٹ stderr |
| `cmd &> all.log`| stdout اور stderr | دونوں کو ری ڈائریکٹ کریں۔
| `cmd1 \| cmd2`| cmd1 کا پائپ stdout to cmd2 کے stdin |
| `cmd1 \|& cmd2`| پائپ stdout اور stderr دونوں |
| `cmd < file`| فائل کو stdin پر ری ڈائریکٹ کریں |
| `cmd <<EOF ... EOF`| یہاں دستاویز (ملٹی لائن ان پٹ) |
| `cmd <<< "string"`| یہاں سٹرنگ (سنگل لائن ان پٹ) |
```bash
# Here-document
cat <<EOF
Name: $NAME
...
## خلاصہ
Bash اسکرپٹنگ کمپیوٹر کے ساتھ کام کرنے والے ہر شخص کی پیداواری صلاحیت کو بڑھاتی ہے۔ ہر پرچم کو حفظ کرنا ضروری نہیں ہے - یہ جاننا کہ کیا ممکن ہے اور کہاں سے دستاویزات تلاش کرنا کافی ہے۔ بنیادی باتوں سے شروع کریں: متغیرات، مشروط، لوپس، پائپ۔ پھر ضرورت کے مطابق ٹیکسٹ پروسیسنگ ٹولز (grep، sed، awk) شامل کریں۔ مضبوط اسکرپٹ لکھنے کے لیے`set -euo pipefail`استعمال کرنے کی سفارش کی جاتی ہے۔