...
# SQL کوئیک حوالہ گائیڈ
ڈیٹا بیس آپریشنز کے لیے ضروری SQL کمانڈز۔
---

## بنیادی استفسار کا ڈھانچہ
```sql
...
-`INT`- عدد
-`BIGINT`- بڑا عدد
-`DECIMAL(p,s)`- عین مطابق اعشاریہ (صحت سے متعلق، پیمانہ)
-`FLOAT`- تقریبا تیرتا ہوا نقطہ
-`DOUBLE`- ڈبل صحت سے متعلق فلوٹ
### تار
-`CHAR(n)`- طے شدہ لمبائی کی تار
-`VARCHAR(n)`- متغیر لمبائی کی تار
-`TEXT`- بڑا متن
-`ENUM`- شمار شدہ اقدار
### تاریخ/وقت
...
-`YEAR`- سال کی قیمت
### بولین
-`BOOLEAN`یا`BOOL`- سچ/غلط
### بائنری
-`BLOB`- بائنری بڑی چیز
-`BINARY`- فکسڈ بائنری
-`VARBINARY`- متغیر بائنری
---
...
## کارکردگی کی تجاویز
✅ **کریں:**
- اکثر پوچھے گئے کالموں پر اشاریہ جات کا استعمال کریں۔
- صرف مطلوبہ کالم منتخب کریں (`SELECT *` سے بچیں)
- استفسار کی کارکردگی کا تجزیہ کرنے کے لیے`EXPLAIN`استعمال کریں۔
- ڈیٹا کو مناسب طریقے سے معمول بنائیں
- SQL انجیکشن کو روکنے کے لیے تیار بیانات کا استعمال کریں۔
❌ **نہ کریں:**
- WHERE شقوں میں انڈیکس شدہ کالموں پر فنکشنز کا استعمال کریں۔
- بہت زیادہ اشاریہ جات بنائیں (سست لکھنا)
-`SELECT DISTINCT`غیر ضروری طور پر استعمال کریں۔
- استفسار پر عمل درآمد کے منصوبوں کو نظر انداز کریں۔
- حسابی اقدار کو ذخیرہ کریں جب ان کا حساب لگایا جا سکے۔
---
...
*آخری بار اپ ڈیٹ کیا گیا: جولائی 2026 | ایس کیو ایل سٹینڈرڈ (MySQL/PostgreSQL ہم آہنگ)*