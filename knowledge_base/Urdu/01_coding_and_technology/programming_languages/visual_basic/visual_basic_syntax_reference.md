...
| | `<=`| کم یا برابر | `a <= b`|
| | `>=`| زیادہ یا برابر | `a >= b`|
| **منطقی** | `AndAlso`| شارٹ سرکٹ اور | `a AndAlso b`|
| | `OrElse`| شارٹ سرکٹ یا | `a OrElse b`|
| | `Not`| منطقی نہیں | `Not a`|
| | `And`| بٹ وائز اور | `a And b`|
| | `Or`| Bitwise OR | `a Or b`|
| | `Xor`| Bitwise XOR | `a Xor b`|
| **سٹرنگ** | `&`| کنٹینیشن | `"Hello" & " " & "World"`|
| | `+=`| شامل کریں | `s &= " more"`|
| ** تفویض** | `=`| تفویض | `x = 42`|
| **نقل** | `Is`| حوالہ مساوات | `obj Is Nothing`|
| | `IsNot`| حوالہ عدم مساوات | `obj IsNot Nothing`|
| **قسم** | `TryCast`| محفوظ کاسٹ (کچھ نہیں لوٹاتا) | `TryCast(obj, String)`|
| | `CType`| تبدیل | `CType(obj, Integer)`|
| | `DirectCast`| براہ راست کاسٹ (ناکامی پر پھینکنا) | `DirectCast(obj, String)`|
---
...
## خلاصہ
Visual Basic کا نحو صاف اور انگریزی جیسا ہے، جو اسے سب سے زیادہ پڑھنے کے قابل پروگرامنگ زبانوں میں سے ایک بناتا ہے۔ VB.NET .NET ایکو سسٹم تک مکمل رسائی فراہم کرتا ہے جس میں ڈیٹا کے استفسار کے لیے LINQ، ہم وقتی آپریشنز کے لیے async/انتظار، قسم کی حفاظت کے لیے جنرک، اور کلاسز، انٹرفیسز، اور وراثت کے ساتھ جامع OOP شامل ہیں۔ زبان کی کیس غیر حساسیت اور لفظی کلیدی الفاظ (End If, End Sub, End Class) پڑھنے کی اہلیت کو بڑھاتے ہیں۔ انٹرپرائز ونڈوز ایپلی کیشنز، آفس آٹومیشن (VBA)، اور .NET کے اندر تیزی سے پروٹو ٹائپنگ کے لیے، VB.NET ایک نتیجہ خیز اور قابل زبان ہے۔