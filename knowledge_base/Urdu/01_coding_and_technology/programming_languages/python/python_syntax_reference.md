...
# ازگر - نحوی حوالہ
یہ دستاویز Python کے لیے ایک جامع، ساختی نحوی حوالہ فراہم کرتی ہے۔ یہ مکمل نحوی نمونوں، آپریٹر ٹیبلز، اور اندرونی میکانکس پر توجہ مرکوز کرکے Python کے مرکزی حوالہ کی تکمیل کرتا ہے۔
---
...
|----------|------|---------|---------|-------|
| `+`| اضافہ | `3 + 2`| `5`| اس کے علاوہ سٹرنگ/فہرست جوڑنا |
| `-`| گھٹاؤ | `3 - 2`| `1`| غیر معمولی نفی بھی |
| `*`| ضرب | `3 * 2`| `6`| اس کے علاوہ سٹرنگ/فہرست تکرار |
| `/`| سچی تقسیم | `7 / 2`| `3.5`| ہمیشہ فلوٹ واپس کرتا ہے |
| `//`| فلور ڈویژن | `7 // 2`| `3`| منفی انفینٹی کی طرف راؤنڈ |
| `%`| ماڈیولس | `7 % 2`| `1`| مماثلت تقسیم کرنے والے پر دستخط کریں |
| `**`| وضاحت | `2 ** 10`| `1024`| دائیں ساتھی |
### موازنہ اور بولین آپریٹرز
| آپریٹر | نام | مثال | نوٹس |
|------------|------|---------|---------|
| `==`| برابر | `x == y`| قدر کا موازنہ (شناخت نہیں) |
| `!=`| برابر نہیں | `x != y`| |
| `<`,`>`,`<=`,`>=`| آرڈرنگ | `x >= y`| زنجیر کے قابل:`a < b < c`|
| `is`| شناخت | `x is y`| میموری میں ایک ہی چیز |
| `is not`| شناخت نہیں | `x is not None`|`!= None`پر ترجیح دی گئی |
| `in`| رکنیت | `x in collection`| O(1) سیٹ/ڈکٹس کے لیے، O(n) فہرستوں کے لیے |
| `and`| منطقی اور | `a and b`| شارٹ سرکٹ؛ آخری سچائی/جھوٹی قیمت واپس کرتا ہے |
| `or`| منطقی یا | `a or b`| شارٹ سرکٹ؛ پہلی سچی/جھوٹی قیمت واپس کرتا ہے |
| `not`| منطقی نہیں | `not x`| |
### بٹ وائز آپریٹرز
| آپریٹر | نام | مثال | نوٹس |
|------------|------|---------|---------|
| `&`| اور | `5 & 3`| `1`|
...
| 8 | `^`| بائیں |
| 9 | `\|`| بائیں |
| 10 | `==`,`!=`,`<`,`>`,`<=`,`>=`,`is`,`is`, XQZMARKER2 | زنجیر |
| 11 | `not`| دائیں |
| 12 | `and`| بائیں |
| 13 (سب سے کم) | `or`| بائیں |
### اسائنمنٹ اور بڑھا ہوا اسائنمنٹ
```python
...
## ڈیٹا سٹرکچرز
### بلٹ ان کلیکشنز کا جائزہ
| مجموعہ | حکم دیا | تغیر پذیر | نقلیں | نحو | تلاش کریں |
|------------|---------|---------|------------|---------|---------|
| `list`| جی ہاں | جی ہاں | جی ہاں | `[1, 2, 3]`| O(n) |
| `tuple`| جی ہاں | نہیں | جی ہاں | `(1, 2, 3)`| O(n) |
| `dict`| ہاں (3.7+) | جی ہاں | چابیاں: نہیں | `{"k": "v"}`| O(1) اوسط |
| `set`| نہیں | جی ہاں | نہیں | `{1, 2, 3}`| O(1) اوسط |
| `frozenset`| نہیں | نہیں | نہیں | `frozenset({1,2})`| O(1) اوسط |
### فہرستیں۔
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
## خلاصہ
یہ نحوی حوالہ ازگر کے مکمل فیچر سیٹ کا احاطہ کرتا ہے — بنیادی آپریٹرز سے اور ایڈوانس میٹاپروگرامنگ، ڈسکرپٹرز، اور ٹائپ سسٹم کے ذریعے کنٹرول فلو۔ Python کا نحو جان بوجھ کر کم سے کم اور پڑھنے کے قابل ہے، لیکن اس کی گہرائی کافی ہے: پیٹرن میچنگ، async جنریٹرز، پروٹوکولز، اور میٹا کلاسز پیچیدہ ایپلی کیشنز کے لیے طاقتور ٹولز فراہم کرتے ہیں۔ زبان کا ارتقاء جاری ہے، ہر ریلیز کے ساتھ ایسی خصوصیات شامل ہوتی ہیں جو پسماندہ مطابقت کو برقرار رکھتے ہوئے اظہار کو بہتر کرتی ہیں۔