...
|------------|------|---------|---------|
| `+``-``*``/``%``**` | ریاضی | `2 ** 10`| `**`کفایت ہے |
| `==``!=``===``<=>` | مساوات | `a == b`| `===`کیس کی مساوات ہے۔ `<=>`خلائی جہاز ہے |
| `eql?`| قدر کی مساوات | `a.eql?(b)`|`==`(چیک کی قسم) سے زیادہ سخت |
| `equal?`| شناخت | `a.equal?(b)`| ایک ہی اعتراض؟ |
| `<``>``<=``>=` | موازنہ | `a >= b`| |
| `&&``\|\|``!`| منطقی | `a && b`| شارٹ سرکٹ؛ آخری تشخیص شدہ قیمت واپس کرتا ہے |
| `and``or``not`| منطقی (کم ترجیح) | `a and b`| بچیں — استعمال کریں`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | Bitwise | `a & b`| |
| `=~``!~` | ریجیکس میچ | `str =~ /pattern/`| انڈیکس یا صفر واپس کرتا ہے |
| `..``...` | رینج | `1..10`| `..`شامل؛ `...`خصوصی |
| `?.`| محفوظ نیویگیشن | `user&.name`| اگر صارف صفر ہے تو صفر واپس کرتا ہے |
| `[]``[]=` | عنصر تک رسائی | `arr[0]`| |
### سچائی
```ruby
...
## خلاصہ
روبی کا نحو ڈویلپر کی خوشی کے لیے ڈیزائن کیا گیا ہے — یہ انگریزی کی طرح پڑھتا ہے اور اظہار خیال کا بدلہ دیتا ہے۔ زبان کی طاقت اس کے آبجیکٹ ماڈل (ہر چیز ایک آبجیکٹ ہے)، بلاکس اور بندش، میٹاپروگرامنگ کی صلاحیتوں، اور قابل شمار ماڈیول سے حاصل ہوتی ہے جو ایک بھرپور کلیکشن پروسیسنگ API فراہم کرتا ہے۔ Ruby 3.x نے زبان کے بنیادی فلسفے کو برقرار رکھتے ہوئے پیٹرن میچنگ، RBS قسم کے دستخط، اور Ractor کو ہم آہنگی کے لیے شامل کیا ہے: پروگرامرز کو نتیجہ خیز اور خوش بنائیں۔