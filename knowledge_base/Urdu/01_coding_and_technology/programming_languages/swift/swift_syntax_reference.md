...
|------------|------|---------|---------|
| `+``-``*``/``%`| ریاضی | `a + b`| |
| `..``...` | رینج | `1...10`| `...`بند؛ `..<`آدھا کھلا |
| `==``!=``<``>``<=``>=` | موازنہ | `a == b`|`Equatable`/`Comparable`|
| `===``!==` | شناخت | `a === b`| ایک ہی حوالہ (صرف کلاسز) |
| `&&``\|\|``!`| منطقی | `a && b`| شارٹ سرکٹ |
| `??`| Nil coalescing | `a ?? b`| اگر`a`صفر ہے تو`b`لوٹاتا ہے |
| `?.`| اختیاری زنجیر | `a?.b`| اگر`a`صفر ہے تو صفر واپس کرتا ہے۔
| `!`| زبردستی کھولنا | `a!`| کریشز اگر صفر - گریز |
| `&`| ان آؤٹ پیرامیٹر | `swap(&a, &b)`| حوالہ سے گزریں |
### Nil ہینڈلنگ آپریٹرز
```swift
...
## خلاصہ
سوئفٹ کا نحو تحفظ کو اظہار کے ساتھ متوازن کرتا ہے۔ اختیارات null کریشوں کو ختم کرتے ہیں، پروٹوکول بغیر وراثت کے لچکدار کمپوزیشن کو فعال کرتے ہیں، اور جدید ہم آہنگی (async/await، اداکار) محفوظ کنکرنٹ پروگرامنگ کو قابل رسائی بناتی ہے۔ زبان پیرامیٹر پیک، میکروز اور ملکیت جیسی خصوصیات کے ساتھ تیار ہوتی رہتی ہے - ہر ایک اضافہ اپنے بنیادی وعدے کو برقرار رکھتے ہوئے Swift کو زیادہ محفوظ اور زیادہ طاقتور بناتا ہے: کوڈ جو تعمیر کے لحاظ سے محفوظ ہے۔