...
-->
# C++ — نحوی حوالہ
یہ دستاویز جدید C++ (C++17/20/23) کے لیے ایک جامع، ساختی نحوی حوالہ فراہم کرتی ہے۔ یہ مکمل نحوی نمونوں، آپریٹر ٹیبلز، ٹیمپلیٹ میکانکس، اور جدید خصوصیات پر توجہ مرکوز کرکے مرکزی C++ حوالہ کی تکمیل کرتا ہے۔
---
...
|------------|------|---------|---------|
| `+``-``*``/``%`| ریاضی | `a + b`| اسی طرح C |
| `++``--` | اضافہ/کمی | `++i`,`i++`| پری انکریمنٹ کو ترجیح دیں |
| `==``!=``<``>``<=``>=` | موازنہ | `a <=> b`| `<=>`تین طرفہ موازنہ ہے (C++20) |
| `&&``\|\|``!`| منطقی | `a && b`| شارٹ سرکٹ |
| `&``\|``^``~``<<``>>` | Bitwise | `a & b`| اسی طرح C |
| `?:`| ٹرنری | `cond ? a : b`| |
### C++-مخصوص آپریٹرز
| آپریٹر | نام | مثال | نوٹس |
|------------|------|---------|---------|
| `::`| دائرہ کار قرارداد | `std::cout`| |
| `.*``->*` | پوائنٹر ٹو ممبر | `obj.*pmf`| |
| `<=>`| خلائی جہاز (C++20) | `a <=> b`| واپس کرتا ہے`strong_ordering`|
| `co_await`| کوروٹین معطل | `co_await expr`| C++20 |
| `co_yield`| کوروٹین کی پیداوار | `co_yield value`| C++20 |
| `co_return`| کوروٹین واپسی | `co_return value`| C++20 |
### آپریٹر اوورلوڈنگ
```cpp
class Vector2D {
    double x_, y_;
...
## کنٹرول فلو
### سٹرکچرڈ بائنڈنگز اور جدید پیٹرنز
```cpp
...
## STL کنٹینرز اور الگورتھم
### کنٹینر کا جائزہ
| کنٹینر | ہیڈر | حکم دیا | تلاش کریں | نوٹس |
|------------|---------|---------|---------|-------|
| `vector`| `<vector>`| جی ہاں | O(n) | ترتیب کے لیے پہلے سے طے شدہ انتخاب |
| `deque`| `<deque>`| جی ہاں | O(n) | دونوں سروں پر تیز دھکا/پاپ |
| `list`| `<list>`| جی ہاں | O(n) | دوہری منسلک فہرست |
| `array`| `<array>`| جی ہاں | O(n) | فکسڈ سائز، اسٹیک مختص |
| `set`/`multiset`| `<set>`| جی ہاں | O(log n) | سرخ سیاہ درخت |
| `map`/`multimap`| `<map>`| ہاں (بذریعہ کلید) | O(log n) | کلیدی قدر، سرخ سیاہ درخت |
| `unordered_set`| `<unordered_set>`| نہیں | O(1) اوسط | ہیش ٹیبل |
| `unordered_map`| `<unordered_map>`| نہیں | O(1) اوسط | ہیش ٹیبل |
### کلیدی الگورتھم
```cpp
#include <algorithm>
#include <numeric>
...
## خلاصہ
C++ نحو نچلی سطح کے پوائنٹر ہیرا پھیری سے لے کر اعلیٰ سطح کے ٹیمپلیٹ میٹاپروگرامنگ اور کوروٹین پر مبنی async تک پھیلا ہوا ہے۔ C++17/20/23 کے ذریعے زبان کے ارتقاء نے ڈرامائی طور پر حفاظت (سمارٹ پوائنٹرز، `std::optional`، تصورات)، اظہار خیال (سٹرکچرڈ بائنڈنگز، رینجز،`std::format`)، اور کارکردگی (تصورات جو صفر لاگت کے تجرید کو فعال کرنے کے لیے بغیر اوور ہیڈ کے طور پر) کو بہتر بنایا ہے۔ بنیادی فلسفہ بدستور برقرار ہے: آپ جو استعمال نہیں کرتے ہیں اس کے لیے آپ کو ادائیگی نہیں کرنی چاہیے، اور زبان کو ننگی دھات سے لے کر ڈومین کے لیے مخصوص زبانوں تک ہر سطح کے تجرید کی حمایت کرنی چاہیے۔