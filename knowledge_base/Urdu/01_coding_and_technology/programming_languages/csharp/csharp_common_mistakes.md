...
یہ دستاویز C# میں سب سے عام غلطیوں، ٹریپس اور اینٹی پیٹرن کی فہرست بناتی ہے۔ ہر اندراج غلط نقطہ نظر کو ظاہر کرتا ہے، یہ بتاتا ہے کہ یہ کیوں ناکام ہوتا ہے، اور صحیح حل فراہم کرتا ہے۔
---

## 1.`IDisposable`اشیاء کو ضائع نہیں کرنا
```csharp
// ❌ WRONG — resource leak
...
## 2. سٹرنگز کے لیے`==`بمقابلہ `Equals()`
```csharp
...
## 5. کیپچرنگ لوپ ویری ایبل (پری-C# 5.0)
```csharp
...
## 6. پیٹرن میچنگ کا استعمال نہیں کرنا
```csharp
...
## 7. اینٹی پیٹرن: گاڈ کلاسز/سروس لوکیٹر
```csharp
...
## 10. ڈیٹا کی اقسام کے لیے ریکارڈ استعمال نہیں کرنا
```csharp
...
## خلاصہ
C# ایک اچھی طرح سے ڈیزائن کی گئی زبان ہے لیکن اس میں ٹریپس ہیں:`IDisposable`اشیاء کو ٹھکانے نہ لگانا، async void طریقے، غیر ضروری LINQ مختص کرنا، اور حوالہ سے متعلق مستثنیات۔ جدید C# (8+) طاقتور ٹولز فراہم کرتا ہے: کالعدم حوالہ جات کی قسمیں، پیٹرن کی مماثلت، ریکارڈ، اعلانات کا استعمال، اور اظہارات کو تبدیل کرنا۔ کلیدی اصول: وسائل کو ہمیشہ ضائع کریں، کبھی بھی async void استعمال نہ کریں (ایونٹ ہینڈلرز کے علاوہ)، کنسٹرکٹر انجکشن کا استعمال کریں، ڈیٹا کے لیے ریکارڈ کو ترجیح دیں، اور کمپائل کے وقت null کیڑے پکڑنے کے لیے nullable ریفرنس کی اقسام کو فعال کریں۔