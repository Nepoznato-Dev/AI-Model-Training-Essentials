...
| `+``-``*``/``%``**` | ریاضی | `2 ** 10`| `**`کفایت ہے |
| `.`| سٹرنگ کنکٹنیشن | `$a . $b`| |
| `x`| سٹرنگ کی تکرار | `"ab" x 3`| `"ababab"`|
| `==``!=``<``>``<=``>=` | عددی موازنہ | `$a == $b`| |
| `eq``ne``lt``gt``le``ge` | سٹرنگ موازنہ | `$a eq $b`| |
| `<=>`| عددی خلائی جہاز | `$a <=> $b`| واپسی -1, 0, 1 |
| `cmp`| سٹرنگ خلائی جہاز | `$a cmp $b`| |
| `&&``\|\|``!`| منطقی | `$a && $b`| |
| `and``or``not`| منطقی (کم prec.) | `$a and $b`| بچیں — استعمال کریں`&&`/`\|\|`|
| `=~``!~` | ریجیکس میچ | `$str =~ /pattern/`| |
| `..``...` | رینج / فلپ فلاپ | `1..10`| |
| `?:`| ٹرنری | `$a ? $b : $c`| |
| `//`| وضاحت شدہ-یا | `$a // $b`| `$b`اگر`$a`undef ہے |
| `=>`| موٹا کوما | `key => 'value'`| خودکار حوالہ جات |
### سگلز — متغیر اقسام
```perl
...
## خلاصہ
پرل کا نحو سیاق و سباق، سگلز اور "اسے کرنے کا ایک سے زیادہ طریقہ ہے" کے اصول کے گرد بنایا گیا ہے۔ زبان میں ریگولر ایکسپریشنز بنائے جاتے ہیں، جس سے ٹیکسٹ پروسیسنگ اختصار میں بے مثال ہوتی ہے۔ حوالہ جات پیچیدہ ڈیٹا ڈھانچے کو فعال کرتے ہیں۔ CPAN ہر چیز کے لیے ماڈیول فراہم کرتا ہے۔ جدید پرل (`strict`، `warnings`، اور لغوی متغیرات کے ساتھ) صاف اور برقرار رکھنے کے قابل ہے۔ پرل کی میراث ان نمونوں اور اوزاروں میں زندہ رہتی ہے جو ہر جدید زبان نے اپنایا ہے۔