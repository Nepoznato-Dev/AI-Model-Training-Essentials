...
# کوٹلن - نحوی حوالہ
یہ دستاویز کوٹلن (2.0+) کے لیے ایک جامع، ساختی نحوی حوالہ فراہم کرتی ہے۔ یہ مکمل نحوی نمونوں، null سیفٹی، کوروٹینز، اور کوٹلن محاوروں پر توجہ مرکوز کرکے مرکزی کوٹلن حوالہ کی تکمیل کرتا ہے۔
---
...
| `+``-``*``/``%`| ریاضی | `a + b`| |
| `..`| رینج | `1..10`| شامل رینج |
| `until`| آدھی کھلی رینج | `1 until 10`| `1..9`|
| `downTo`| نزولی رینج | `10 downTo 1`| |
| `==``!=` | ساختی مساوات | `a == b`| کالز`.equals()`|
| `===``!==` | حوالہ جاتی مساوات | `a === b`| ایک ہی مثال |
| `<``>``<=``>=` | موازنہ | `a >= b`|`Comparable`کی ضرورت ہے |
| `&&``\|\|``!`| منطقی | `a && b`| شارٹ سرکٹ |
| `?:`| ایلوس آپریٹر | `a ?: b`| اگر`a`کالعدم ہے تو`b`لوٹاتا ہے |
| `?.`| محفوظ کال | `a?.b`| اگر`a`null ہے تو null لوٹاتا ہے۔
| `!!`| غیر کالعدم دعویٰ | `a!!`| NPE پھینک دیتا ہے اگر null |
| `as`| کاسٹ | `a as String`| ClassCastException پھینک دیتا ہے |
| `as?`| محفوظ کاسٹ | `a as? String`| ناکامی پر null لوٹاتا ہے |
| `in`| رکنیت | `x in list`| |
| `!in`| رکن نہیں | `x !in list`| |
| `is`| ٹائپ چیک | `x is String`| چیک کے بعد اسمارٹ کاسٹ |
| `!is`| قسم نہیں | `x !is String`| |
### آپریٹر اوورلوڈنگ
```kotlin
...
## خلاصہ
کوٹلن کا نحو جامع، محفوظ، اور جاوا کے ساتھ انٹرآپریبل ہے۔ null حفاظت کمپائل کے وقت کیڑے کی ایک پوری کلاس کو ختم کرتی ہے۔ Coroutines غیر مطابقت پذیر کوڈ لکھنے کا قدرتی طریقہ فراہم کرتے ہیں۔ سیل شدہ کلاسز، ڈیٹا کلاسز، اور ایکسٹینشن فنکشنز اظہاری ڈومین ماڈلنگ کو فعال کرتے ہیں۔ زبان کی طاقت اس کی خصوصیات کے باہمی تعامل سے آتی ہے - دائرہ کار کے فنکشنز، DSLs، ان لائن کلاسز، اور ریفائیڈ جنرکس کو مل کر ایسا کوڈ تیار کیا جاتا ہے جو لکھنے میں محفوظ اور لطف اندوز ہوتا ہے۔