...
## 1. پلیٹ فارم کی اقسام اور NullPointerException
```kotlin
...
## 3.`data class`بغیر مناسب`equals`/ `hashCode`
```kotlin
...
## 4. دائرہ کار کے افعال کو درست طریقے سے استعمال نہ کرنا
```kotlin
...
## 5. کوروٹینز: منسوخی کو ہینڈل نہیں کرنا
```kotlin
...
## 6. اینٹی پیٹرن: جاوا اسٹائل کوٹلن
```kotlin
...
## 9. مہنگی شروعات کے لیے`by lazy`استعمال نہیں کرنا
```kotlin
...
## 10. فلو بمقابلہ لائیو ڈیٹا کنفیوژن
```kotlin
...
## خلاصہ
کوٹلن کی جامعیت باریک مسائل کو چھپا سکتی ہے: جاوا انٹراپ سے پلیٹ فارم کی قسمیں،`lateinit`کریشز، ڈیٹا کو تبدیل کرنے والی کلاسیں جمع کرنا، اور کورٹین کینسلیشن۔ کوٹلن طریقہ یہ ہے: null سیفٹی کو گلے لگائیں (`!!` کبھی استعمال نہ کریں)، صحیح اسکوپ فنکشن کا استعمال کریں (تبدیلیوں کے لیے `let`،`apply`کنفیگریشن کے لیے،`also`ضمنی اثرات کے لیے)، مکمل XQZMARKER کے ساتھ مہر بند کلاسز کا استعمال کریں، XQZMARKER5 کو ہینڈ کر سکتے ہیں محاورہ کوٹلن (کوٹلن نحو کے ساتھ جاوا نہیں)۔ مرتب کرنے والا آپ کا رہنما ہے - اگر یہ متنبہ کرتا ہے تو سنیں۔