...
# ڈارٹ - نحوی حوالہ
یہ دستاویز Dart (3.x) کے لیے ایک جامع، ساختی نحوی حوالہ فراہم کرتی ہے۔ یہ مکمل نحوی نمونوں، null سیفٹی، async پروگرامنگ، اور Flutter پر مبنی ڈیزائن کے نمونوں پر توجہ مرکوز کر کے مرکزی ڈارٹ حوالہ کی تکمیل کرتا ہے۔
---
...
|------------|------|---------|---------|
| `+``-``*``/``%``~/` | ریاضی | `7 ~/ 3`| `~/`عددی تقسیم ہے |
| `==``!=` | مساوات | `a == b`| |
| `<``>``<=``>=` | موازنہ | `a >= b`| |
| `&&``\|\|``!`| منطقی | `a && b`| شارٹ سرکٹ |
| `??`| null coalescing | `a ?? b`| `b`اگر`a`کالعدم ہے |
| `?.`| بے خبری تک رسائی | `a?.b`| null اگر`a`null ہے |
| `!`| کالعدم دعویٰ | `a!`| پھینک دیتا ہے اگر null |
| `..``?..` | جھرن | `obj..a..b`| ایک ہی چیز پر سلسلہ آپریشنز |
| `??=`| کالعدم آگاہی تفویض | `a ??= b`| اگر کالعدم ہو تو تفویض کریں |
| `as`| قسم کاسٹ | `obj as String`| ناکامی پر پھینک دیتا ہے |
| `is``is!` | ٹائپ ٹیسٹ | `obj is String`| |
| `=>`| تیر کا فنکشن | `() => expr`| |
### کیسکیڈ نوٹیشن
```dart
...
## خلاصہ
ڈارٹ کا نحو صاف، مستقل اور پیداوری کے لیے ڈیزائن کیا گیا ہے۔ صوتی null حفاظت کمپائل کے وقت null حوالہ کی غلطیوں کو ختم کرتی ہے۔ Async/await اور اسٹریمز غیر مطابقت پذیر پروگرامنگ کے لیے قدرتی نمونے فراہم کرتے ہیں۔ کلاسز، مکسنس، اور سیل شدہ کلاسز لچکدار آبجیکٹ کمپوزیشن کو فعال کرتی ہیں۔ ڈارٹ 3 کے پیٹرن کی مماثلت، ریکارڈز اور کلاس موڈیفائر اظہار میں اضافہ کرتے ہیں۔ جبکہ ڈارٹ کا بنیادی مقصد فلٹر ہے، اس کا نحو کسی بھی ایپلیکیشن ڈومین — سرور سائیڈ (ڈارٹ فراگ یا شیلف کے ساتھ)، سی ایل آئی ٹولز، اور ویب ایپلیکیشنز (جاوا اسکرپٹ پر مرتب) کے لیے موزوں ہے۔