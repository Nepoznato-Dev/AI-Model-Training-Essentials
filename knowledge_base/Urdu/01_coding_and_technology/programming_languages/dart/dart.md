...
-->
#ڈارٹ
ڈارٹ ایک کلائنٹ کی طرف سے تیار کردہ پروگرامنگ لینگویج ہے، جسے گوگل نے پہلی بار 2013 میں ریلیز کیا تھا۔ جبکہ ڈارٹ کو ابتدائی طور پر ویب براؤزرز کے لیے جاوا اسکرپٹ کے ممکنہ متبادل کے طور پر رکھا گیا تھا، لیکن اس نے اپنا بنیادی مقصد **فلٹر** کے پیچھے زبان کے طور پر پایا — موبائل، ویب، ڈیسک ٹاپ، اور سنگل کوڈبا ایپلی کیشنز کو ایمبیڈڈ کرنے کے لیے گوگل کی کراس پلیٹ فارم UI ٹول کٹ۔
ڈارٹ جدید زبانوں کی بہترین خصوصیات کو یکجا کرتا ہے: یہ آبجیکٹ پر مبنی ہے، اس میں اختیاری ٹائپنگ ہے (ڈارٹ 3 کے بعد سے آواز کی حفاظت)،`async`/`await`کے ساتھ غیر مطابقت پذیر پروگرامنگ کو سپورٹ کرتا ہے، اور مقامی مشین کوڈ (موبائل/ڈیسک ٹاپ کے لیے) اور جاوا اسکرپٹ (ویب کے لیے) دونوں کو مرتب کرتا ہے۔
---

## ڈارٹ کیوں اہمیت رکھتا ہے۔
- **پھڑنا**: پھڑپھڑانے کی بنیادی زبان — تیزی سے بڑھنے والے کراس پلیٹ فارم فریم ورک میں سے ایک۔
- **کراس پلیٹ فارم**: آئی او ایس، اینڈرائیڈ، ویب، ونڈوز، میک او ایس، لینکس اور ایمبیڈڈ ڈیوائسز کے لیے سنگل کوڈ بیس۔
- **پیداواری**: گرم دوبارہ لوڈ، بھرپور ویجیٹ لائبریری، اور تاثراتی نحو UI کی ترقی کو تیز بناتا ہے۔
- **ساؤنڈ نال سیفٹی**: کمپائل ٹائم null سیفٹی null حوالہ کی غلطیوں کو ختم کرتی ہے۔
- **کارکردگی**: موبائل کے لیے مقامی ARM کوڈ کو مرتب کرتا ہے۔ پل کی ضرورت نہیں.
- **بڑھتا ہوا ماحولیاتی نظام**: فلٹر کا پیکیج ماحولیاتی نظام تیزی سے پھیل رہا ہے۔
## ٹریڈ آف
| حد | تفصیلات | عام حل |
|------------|---------|-------------------|
| ** پھڑپھڑاہٹ پر مرکوز** | ڈارٹ کا سب سے زیادہ استعمال فلٹر ہے۔ اس سے باہر محدود | پھڑپھڑانے کے لیے استعمال کریں؛ غیر UI کام کے لیے دوسری زبانیں |
| **چھوٹا ماحولیاتی نظام** | React Native یا مقامی پلیٹ فارمز سے کم پیکیجز | تیزی سے بڑھتی ہوئی؛ مقامی APIs کے لیے پلیٹ فارم چینلز |
| **ویب کی کارکردگی** | WASM پر مرتب کردہ ڈارٹ اب بھی پختہ ہو رہا ہے | مسلسل کارکردگی کے لیے CanvasKit رینڈرر کا استعمال کریں |
| **ملازمت کی منڈی** | پھڑپھڑانے والے کردار موجود ہیں لیکن مقامی موبائل سے کم | کراس پلیٹ فارم ڈویلپرز کی بڑھتی ہوئی مانگ |
| ** بیک اینڈ کے لیے نہیں** | ممکن ہے (سرور سائیڈ ڈارٹ) لیکن استعمال کیس نہیں | بیک اینڈز کے لیے Go, Node.js, Python استعمال کریں۔
---
...
### پھڑپھڑانے والے ویجیٹ کے پیٹرنز
```dart
...
### پیٹرن 2: فراہم کنندہ اسٹیٹ مینجمنٹ
```dart
...
### پیٹرن 3: BLoC (کاروباری منطق کا جزو)
```dart
...
## کارکردگی اور اصلاح
### اے او ٹی بمقابلہ جے آئی ٹی تالیف
| موڈ | جب استعمال کیا جاتا ہے | آغاز | رن ٹائم | کیس استعمال کریں |
|------|------------|---------|---------|----------|
| **JIT** (جسٹ ان ٹائم) | ترقی، ڈیبگ موڈ | تیز | قدرے سست | ترقی کے دوران گرم دوبارہ لوڈ |
| **AOT** (وقت سے پہلے) | ریلیز کی تعمیرات | قدرے سست | تیز | پروڈکشن موبائل/ڈیسک ٹاپ ایپس |
| **JS** | ویب اہداف | انحصار کرتا ہے | مختلف ہوتی ہے | فلٹر ویب ایپلیکیشنز |
| **WASM** | ویب (تجرباتی) | انحصار کرتا ہے | تیز | مستقبل کا ویب ہدف |
```dart
...
## تعیناتی۔
### پھڑپھڑاہٹ تعیناتی کے اہداف
| پلیٹ فارم | کمانڈ بنائیں | آؤٹ پٹ |
|------------|---------------|---------|
| **Android** | `flutter build apk`/`flutter build appbundle`| APK/AAB برائے Play Store |
| **iOS** | `flutter build ipa`| ایپ اسٹور کے لیے IPA |
| **ویب** | `flutter build web`| جامد HTML/JS/CSS |
| **ونڈوز** | `flutter build windows`| MSIX یا اسٹینڈ اکیلے exe |
| **macOS** | `flutter build macos`| ایپ بنڈل |
| **لینکس** | `flutter build linux`| بائنری + اثاثے |
```bash
# Build commands
...
## ڈارٹ کب استعمال کریں۔
| منظر نامہ | کیوں ڈارٹ (پھڑپھڑانا) | بہتر متبادل |
|------------|----------------------------|
| کراس پلیٹ فارم موبائل ایپس | پھڑپھڑانا بہترین ہے | مقامی، مقامی سوئفٹ/کوٹلن کا رد عمل ظاہر کریں |
| کراس پلیٹ فارم ڈیسک ٹاپ | لہرانا اس کی حمایت کرتا ہے | الیکٹران، C#، Avalonia |
| ویب ایپلیکیشنز | فلٹر ویب موجود ہے | ری ایکٹ، ویو، اینگولر برائے امیر ویب ایپس |
| ایمبیڈڈ UIs | ایمبیڈڈ کے لیے پھڑپھڑانا | C, LVGL |
| پسدید ترقی | بنیادی استعمال کیس نہیں | Go, Node.js, Python |
| ڈیٹا سائنس / ایم ایل | مناسب نہیں | ازگر، آر |
| سسٹمز پروگرامنگ | مناسب نہیں | C, C++, Rust |
---

## مصنوعی سوال و جواب
### Q1: ڈارٹ کا نال سیفٹی کیسے کام کرتا ہے؟
**A:** ڈارٹ 2.12+ میں ساؤنڈ نال سیفٹی ہے۔ متغیرات پہلے سے طے شدہ طور پر غیر منسوخ ہیں؛ null کی اجازت دینے کے لیے`?`استعمال کریں:
```dart
String name = 'Alice';    // Cannot be null
...
### Q2:`Future`اور`Stream`میں کیا فرق ہے؟
**A:**`Future`ایک واحد async نتیجہ کی نمائندگی کرتا ہے۔ `Stream`async واقعات کی ایک ترتیب کی نمائندگی کرتا ہے:
```dart
...
### Q3: میں فلٹر ایپ میں ریاست کا انتظام کیسے کروں؟
**A:** پیچیدگی پر منحصر متعدد نقطہ نظر:
```dart
...
### Q4: ڈارٹ میں توسیع کے طریقے کیسے کام کرتے ہیں؟
**A:** ایکسٹینشنز بغیر وراثت کے موجودہ اقسام میں فعالیت کا اضافہ کرتی ہیں:
```dart
...
### Q5: میں پرفارمنس ڈارٹ/فلٹر کوڈ کیسے لکھ سکتا ہوں؟
**A:** کلیدی مشقیں:
- جہاں بھی ممکن ہو`const`کنسٹرکٹرز استعمال کریں۔
- ویجٹ کو دوبارہ بنانے سے گریز کریں - `const`، `final`، اور`shouldRebuild`استعمال کریں 
- بڑی فہرستوں کے لیے`ListView`کی بجائے`ListView.builder`استعمال کریں
- فلٹر ڈیو ٹولز کے ساتھ پروفائل
- الگ تھلگ تھریڈز پر مہنگے آپریشنز کے لیے`compute()`استعمال کریں۔
-`setState`کالوں کو کم سے کم کریں - اس بارے میں مخصوص رہیں کہ کس چیز کی تعمیر نو کی ضرورت ہے۔
---

## سوچ کا مسئلہ حل کرنا
### مسئلہ 1: ایک ٹائپ سیف API کلائنٹ بنانا
**مرحلہ 1: مسئلہ کو سمجھیں**
ایک API کلائنٹ بنائیں جو ڈیٹا حاصل کرے اور مناسب طریقے سے ٹائپ کردہ اشیاء کو واپس کرے۔
**مرحلہ 2: نقطہ نظر کی شناخت کریں**
`fromJson` / `toJson`، async/await، اور نتائج کے لیے سیل شدہ کلاسز کے ساتھ ڈارٹ کلاسز کا استعمال کریں۔
**مرحلہ 3: نافذ کریں**```dart
...
**مرحلہ 4: تصدیق کریں**
فرضی HTTP کلائنٹ کے ساتھ ٹیسٹ کریں۔ نیٹ ورک کی ناکامیوں اور برے جوابات کے لیے غلطی سے نمٹنے کی تصدیق کریں۔
### مسئلہ 2: ڈیباؤنس کے ساتھ ایک رد عمل کی تلاش کو نافذ کرنا
**مرحلہ 1: مسئلہ کو سمجھیں**
ایک ایسا سرچ فیلڈ بنائیں جو API سے استفسار کرتا ہے لیکن ضرورت سے زیادہ درخواستوں سے بچنے کے لیے ان پٹ کو ڈیباؤنس کرتا ہے۔
**مرحلہ 2: نقطہ نظر کی شناخت کریں**
`debounceTime` اور`distinct`کے ساتھ ڈارٹ اسٹریمز کا استعمال کریں۔
**مرحلہ 3: نافذ کریں**```dart
import 'dart:async';
...
**مرحلہ 4: ٹیسٹ**
تصدیق کریں کہ تیز ٹائپنگ ڈیباؤنس کی مدت کے بعد صرف ایک API کال کو متحرک کرتی ہے۔
---

## خلاصہ
زندگی میں ڈارٹ کا مقصد پھڑپھڑانا ہے۔ اسٹینڈ لون زبان کے طور پر، یہ قابل ہے لیکن غیر قابل ذکر ہے۔ Flutter کے پیچھے انجن کے طور پر، یہ ڈویلپرز کو ایک کوڈبیس سے ہر بڑے پلیٹ فارم کے لیے خوبصورت، اعلیٰ کارکردگی والی ایپلیکیشنز بنانے کے قابل بناتا ہے۔ اگر آپ کراس پلیٹ فارم موبائل یا ڈیسک ٹاپ ایپلی کیشنز بنا رہے ہیں، تو Dart + Flutter دستیاب بہترین اختیارات میں سے ایک ہے۔ ہر چیز کے لیے دوسری زبانیں زیادہ موزوں ہیں۔