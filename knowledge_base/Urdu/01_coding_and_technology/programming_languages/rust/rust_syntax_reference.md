...
|------------|------|---------|---------|
| `+`| اضافہ | `3 + 2`| اس کے علاوہ`String``format!` کے ذریعے کنکٹنیشن |
| `-`| گھٹاؤ | `3 - 2`| |
| `*`| ضرب | `3 * 2`| |
| `/`| ڈویژن | `7 / 2`| عدد کے لیے عددی تقسیم:`3`|
| `%`| باقی | `7 % 2`| `1`|
| `-x`| نفی | `-5`| |
### موازنہ اور منطقی آپریٹرز
| آپریٹر | نام | مثال | نوٹس |
|------------|------|---------|---------|
| `==`| برابر | `x == y`|`PartialEq`کی ضرورت ہے |
| `!=`| برابر نہیں | `x != y`| |
| `<`,`>`,`<=`,`>=`| آرڈرنگ | `x >= y`|`PartialOrd`کی ضرورت ہے |
...
|------------|------------|---------|
| 1 (سب سے زیادہ) | `()`راستے`.`:: | |
| 2 | میتھڈ کالز، فیلڈ تک رسائی | آٹو ڈیریف |
| 3 | `-`(negation),`!`,`*`(deref) | |
| 4 | `as`(قسم کاسٹ) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
| 14 | `..``..=` | رینج |
| 15 | `=``+=` وغیرہ | تفویض |
| 16 (سب سے کم) | `return``break``continue`| |
---
...
## ڈیٹا سٹرکچرز
### بلٹ ان اقسام کا جائزہ
| قسم | ڈھیر/ڈھیر | سائز | تغیر پذیری | نوٹس |
|------|------------|------|------------|-------|
| قدیم (`i32`,`f64`,`bool`,`char`) | اسٹیک | فکسڈ | ڈیفالٹ کی طرف سے ناقابل تبدیلی | `Copy`|
| `[T; N]`(صفی) | اسٹیک | فکسڈ |`mut`کے ساتھ تغیر پذیر | فکسڈ سائز،`Copy`اگر T ہے`Copy`|
| `Vec<T>`| ڈھیر | متحرک |`mut`کے ساتھ تغیر پذیر | قابل ترقی |
| `String`| ڈھیر | متحرک |`mut`کے ساتھ تغیر پذیر | UTF-8، قابل اضافہ |
| `&[T]`(ٹکڑا) | اسٹیک (چربی پی ٹی آر) | متحرک منظر |`&`یا`&mut`پر منحصر ہے | مستعار منظر |
| `HashMap<K, V>`| ڈھیر | متحرک |`mut`کے ساتھ تغیر پذیر | O(1) اوسط تلاش |
| `BTreeMap<K, V>`| ڈھیر | متحرک |`mut`کے ساتھ تغیر پذیر | O(log n) تلاش، حکم دیا |
| `HashSet<T>`| ڈھیر | متحرک | - | O(1) اوسط، کوئی نقل نہیں |
### ویکٹرز
```rust
// Creation
...
### عام معیاری لائبریری کی خصوصیات
```rust
...
## خلاصہ
زنگ کا نحو کمپائل کے وقت نافذ حفاظتی ضمانتوں کی بنیاد پر بنایا گیا ہے۔ ملکیت کا نظام، خاصیت پر مبنی پولیمورفزم، پیٹرن کی مماثلت، اور صفر لاگت کے تجرید ایک مربوط ڈیزائن بناتے ہیں جہاں مرتب کرنے والا ایک سخت لیکن منصفانہ شراکت دار ہے۔ میکروز، غیر محفوظ کوڈ، اور سمارٹ پوائنٹرز جیسی جدید خصوصیات کے ذریعے بنیادی کنٹرول کے بہاؤ سے، ہر تعمیر محفوظ، موثر سسٹم پروگرامنگ کے ہدف کو پورا کرتی ہے۔ زندگی بھر، قرض لینے، اور قسم کے نظام کے درمیان تعامل کو سمجھنا Rust پر عبور حاصل کرنے کی کلید ہے۔