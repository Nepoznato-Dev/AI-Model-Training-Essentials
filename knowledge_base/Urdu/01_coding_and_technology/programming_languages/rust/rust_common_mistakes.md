...
## 1. قرض لینے والے چیکر سے لڑنا - غیر ضروری کلوننگ
```rust
...
## 3. پیداوار میں`unwrap()`کے ساتھ نتائج کو نظر انداز کرنا
```rust
...
## 5. اینٹی پیٹرن: ہر جگہ`Rc`/`RefCell`کا استعمال
```rust
...
## 7. اینٹی پیٹرن: ضرورت سے زیادہ`to_string()`/ `to_owned()`
```rust
...
## 9. Iterator Invalidation — غیر ضروری طور پر جمع کرنا
```rust
...
## 10. اینٹی پیٹرن: اینوم بغیر طریقوں کے
```rust
...
## 11. عام خصلتوں کے لیے`#[derive]`استعمال نہیں کرنا
```rust
...
## 12. بغیر دستاویزات کے غیر محفوظ کوڈ
```rust
...
## خلاصہ
زنگ کا کمپائلر سخت لیکن منصفانہ ہے — زیادہ تر غلطیاں آپ کو میموری کی حفاظت کے بارے میں کچھ سکھاتی ہیں۔ سب سے بڑی غلطیاں کمپائلر کی غلطیاں نہیں بلکہ اینٹی پیٹرن ہیں: قرض لینے والے کو مطمئن کرنے کے لیے غیر ضروری کلوننگ، پیداوار میں`unwrap()`کا استعمال، ملکیت کی کوشش کرنے سے پہلے`Rc<RefCell<T>>`تک پہنچنا، اور جب حوالہ کافی ہو تو تاریں مختص کرنا۔ رسٹیشین ذہنیت یہ ہے: مرتب کرنے والے کو آپ کو صفر لاگت کے تجریدات کی طرف رہنمائی کرنے دیں۔ صرف اس وقت کلون کریں جب ضروری ہو،`?`کے ساتھ غلطیوں کا پرچار کریں،`String`پر`&str`کو ترجیح دیں، اور ہر`unsafe`بلاک کو دستاویز کریں۔