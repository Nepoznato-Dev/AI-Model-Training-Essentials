...
|------------|------|---------|---------|
| `+``-``*``/``%`| ریاضی | `a + b`| |
| `**`| پاور (بڑا اعشاریہ) | `BigDecimal(2) ** 10`| Int کے لیے بلٹ ان نہیں |
| `==``!=` | مساوات | `a == b`| کالز`.equals()`— null-safe |
| `eq``ne` | حوالہ مساوات | `a eq b`| ایک ہی مثال |
| `<``>``<=``>=` | موازنہ | `a >= b`|`Ordered`کی ضرورت ہے |
| `&&``\|\|``!`| منطقی | `a && b`| شارٹ سرکٹ |
| `&``\|``^``~` | Bitwise | `a & b`| |
| `<<``>>``>>>`| شفٹ | `a << 2`| |
| `+:``:+``::``:::` | مجموعہ آپریشن | `1 +: list`| Right-associative اگر`:`| کے ساتھ ختم ہوتا ہے۔
| `->`| Tuple تخلیق | `"key" -> "value"`| `("key", "value")`|
| `=>`| فنکشن/تیر | `x => x + 1`| لیمبڈا یا ٹائپ میپنگ |
| `<-`| جنریٹر | `x <- List(1,2,3)`| فہم کے لیے |
### آپریٹر کے قواعد
```scala
...
## خلاصہ
اسکالا کا نحو ایک طاقتور قسم کے نظام کے تحت آبجیکٹ اورینٹڈ اور فنکشنل پروگرامنگ کو یکجا کرتا ہے۔ پیٹرن کی مماثلت، فہمی کے لیے، اور کیس کی کلاسیں تاثراتی ڈیٹا ماڈلنگ فراہم کرتی ہیں۔ خصائص واحد وراثت کی حدود کے بغیر لچکدار ساخت کو فعال کرتے ہیں۔ قسم کا نظام — جنرک، تغیر، ٹائپ باؤنڈز، اور مبہم اقسام کے ساتھ — صفر لاگت کے تجرید کو فعال کرتے ہوئے مرتب وقت پر غلطیوں کو پکڑتا ہے۔ Scala 3 زبان کو enums، توسیع کے طریقوں، دیے ہوئے/استعمال، اور مبہم اقسام کے ساتھ آسان بناتا ہے، جس سے زبان کو اس کی مکمل طاقت برقرار رکھتے ہوئے مزید قابل رسائی بنایا جاتا ہے۔