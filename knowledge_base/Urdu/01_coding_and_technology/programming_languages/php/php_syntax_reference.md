...
| `+``-``*``/``%`| ریاضی | `$a + $b`| |
| `**`| وضاحت | `2 ** 10`| `1024`|
| `==``!=``===``!==` | مساوات | `$a === $b`| `===`قسم بھی چیک کرتا ہے |
| `<=>`| خلائی جہاز | `$a <=> $b`| واپسی -1، 0، یا 1 |
| `<``>``<=``>=` | موازنہ | `$a >= $b`| |
| `&&``\|\|``!`| منطقی | `$a && $b`| شارٹ سرکٹ |
| `and``or``xor`| منطقی (کم ترجیح) | `$a and $b`| بچیں — استعمال کریں`&&`/`\|\|`|
| `??`| null coalescing | `$a ?? 'default'`| اگر`$a`کالعدم ہے تو`'default'`لوٹاتا ہے |
| `??=`| null coalescing تفویض | `$a ??= 'default'`| صرف اس صورت میں تفویض کرتا ہے جب`$a`کالعدم ہو |
| `?``:` | ٹرنری | `$a ? $b : $c`| |
| `.`| کنٹینیشن | `$a . $b`| |
| `.=`| مربوط تفویض | `$a .= $b`| |
| `=>`| سرنی تفویض | `['key' => 'value']`| |
| `->`| ممبر تک رسائی | `$obj->method()`| |
| `?->`| Nullsafe آپریٹر | `$obj?->method()`| اگر`$obj`null ہے تو null لوٹاتا ہے۔
| `::`| جامد رسائی | `ClassName::method()`| |
| `instanceof`| ٹائپ چیک | `$obj instanceof ClassName`| |
| `clone`| آبجیکٹ کلوننگ | `$copy = clone $obj`| گہری کاپی کے لیے`__clone()`|
### جگلنگ ٹائپ کریں۔
```php
...
## خلاصہ
پی ایچ پی کا نحو ڈرامائی طور پر اس کی اسکرپٹنگ کی ابتدا سے ایک جدید، ٹائپ سیف زبان میں تبدیل ہوا ہے۔ PHP 8.x نے یونین کی قسمیں، انٹرسیکشن کی قسمیں، enums، fibers، صرف پڑھنے کی کلاسیں، انتسابات، اور فرسٹ کلاس کال ایبلز متعارف کرائے — اسے ایک ڈھیلے ٹائپ شدہ اسکرپٹنگ لینگویج سے ایک ایسی میں تبدیل کرنا جو سخت سافٹ ویئر انجینئرنگ کو سپورٹ کرتی ہے۔ زبان کی طاقت اس کا ویب فوکس بنی ہوئی ہے: Laravel اور Symfony جیسے فریم ورک کے ساتھ، ایک بہت بڑا ماحولیاتی نظام، اور قریب عالمگیر ہوسٹنگ سپورٹ، PHP کا نحو لاکھوں ایپلیکیشنز کو پیش کرتا ہے۔ جدید پی ایچ پی ڈویلپرز کو انعام دیتا ہے جو اس کے ٹائپ سسٹم کو قبول کرتے ہیں، کمپوزر پر مبنی آٹو لوڈنگ کا استعمال کرتے ہیں، اور صاف، آبجیکٹ پر مبنی کوڈ لکھتے ہیں۔