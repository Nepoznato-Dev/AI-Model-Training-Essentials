...
|------------|------|---------|---------|
| `+``-``*``/``^`| ریاضی | `2^10`| |
| `÷`| عددی تقسیم | `7 ÷ 3`| جیسا کہ`div(7, 3)`|
| `%``\` | ماڈیولس/باقی | `7 % 3`| |
| `==``!=` | مساوات | `a == b`| قدر کی مساوات |
| `===`| شناخت | `a === b`| ایک ہی اعتراض |
| `<``>``<=``>=` | موازنہ | `a >= b`| زنجیر کے قابل:`1 < x < 10`|
| `&&``\|\|``!`| منطقی | `a && b`| شارٹ سرکٹ |
| `&``\|``⊻``~` | Bitwise | `a & b`| `⊻`XOR ہے |
| `:`| رینج | `1:10`| `start:step:stop`|
| `..`| وقفہ | `1..10`| IntervalSets کی ضرورت ہے |
| `∈``∉``⊆`| رکنیت مقرر کریں | `x ∈ S`| `in(x, S)`|
| `∘`| ساخت | `f ∘ g`| `(f ∘ g)(x) == f(g(x))`|
| `\|>`| پائپ | `x \|> f`| `f(x)`|
| `=>`| جوڑی | `a => b`| ڈکٹ کے لیے |
| `...`| Splat | `f(args...)`| مجموعہ کو پھیلائیں |
### زنجیروں سے بند موازنہ
```julia
...
## خلاصہ
جولیا کا نحو صاف، ریاضیاتی اور تاثراتی ہے۔ ایک سے زیادہ ڈسپیچ فنکشنز کو تمام دلیل کی اقسام کی بنیاد پر مختلف طریقے سے برتاؤ کرنے کی اجازت دیتا ہے، خوبصورت کوڈ تنظیم کو فعال کرتا ہے۔ میکرو سی طرز کے پری پروسیسر میکروز کی پیچیدگی کے بغیر طاقتور میٹاپروگرامنگ فراہم کرتے ہیں۔ قسم کا نظام - پیرامیٹرک اقسام، تجریدی اقسام، اور یونین کی اقسام کے ساتھ - لچک اور کارکردگی دونوں کو قابل بناتا ہے۔ جولیا کی 1 پر مبنی اشاریہ سازی، فہم، اور ڈو-بلاک نحو اسے ریاضی اور سائنسی کمپیوٹنگ کے لیے قدرتی بناتے ہیں۔ زبان اپنے "ازگر کی طرح چلنا، سی کی طرح چلنا" کے وعدے کو جے آئی ٹی کی تالیف کے ذریعے ٹائپ انفرنس کے ساتھ حاصل کرتی ہے۔