...
## فورٹران کیوں اہمیت رکھتا ہے۔
- **HPC کی کارکردگی**: فورٹران کمپائلرز دستیاب کچھ تیز ترین عددی کوڈ تیار کرتے ہیں — اکثر ارے آپریشنز کے لیے C/C++ سے مماثل یا اس سے زیادہ ہوتے ہیں۔
- **لیگیسی کوڈ بیسز**: فورٹران میں دہائیوں کے سائنسی کوڈ (آب و ہوا کے ماڈلز، فزکس سمولیشنز) لکھے گئے ہیں۔
- **ارے آپریشن**: ریاضی کی گنتی کے لیے ڈیزائن کردہ نحو کے ساتھ مقامی کثیر جہتی صف کی حمایت۔
- **عددی استحکام**: زبان اور مرتب کرنے والے فلوٹنگ پوائنٹ کمپیوٹیشن کے لیے موزوں ہیں۔
- **Coarrays**: بلٹ ان متوازی پروگرامنگ سپورٹ (Fortran 2008+)۔
- **ابھی بھی ترقی پذیر**: فورٹران 2023 پیچھے کی طرف مطابقت کو برقرار رکھتے ہوئے جدید خصوصیات کا اضافہ کرتا ہے۔
## ٹریڈ آف
| حد | تفصیلات | عام حل |
|------------|---------|-------------------|
| **طاق برادری** | چھوٹے اور خصوصی — زیادہ تر سائنسی/HPC | کمپیوٹیشنل سائنس میں ایکٹو کمیونٹی |
| **محدود ماحولیاتی نظام** | ازگر، جاوا، یا C++ | سے کم لائبریریاں عددی کام کے لیے BLAS/LAPACK استعمال کریں۔ C دیگر ضروریات کے لیے انٹرآپریبلٹی |
| **عام مقصد نہیں** | ویب، موبائل، GUI، یا سسٹم پروگرامنگ کے لیے ناقص | حساب کے لیے فورٹران کا استعمال کریں؛ ایپلی کیشنز کے لیے Python/C میں لپیٹیں |
| **خیال** | جدید صلاحیتوں کے باوجود اکثر "متروک" کے طور پر دیکھا جاتا ہے۔ اس کی طاقتوں پر توجہ مرکوز کریں: عددی اور HPC |
| **ہائرنگ** | فورٹران کے چند ڈویلپرز مارکیٹ میں داخل ہو رہے ہیں | اکیڈمیا اور قومی لیبز میں موجودہ ماہرین |
---

## نحوی بنیادی باتیں (جدید فورٹران)
```fortran
program hello
    implicit none
...
### اخذ کردہ اقسام اور قسم کے پابند طریقہ کار
```fortran
...
### عام طریقہ کار اور آپریٹر اوور لوڈنگ
```fortran
module vector_ops
    implicit none
...
### ماڈیولز اور پروگرام آرگنائزیشن
```fortran
module constants
    implicit none
...
### Coarrays (فورٹران 2008 بلٹ ان متوازی)
```fortran
...
### اوپن ایم پی (مشترکہ میموری کی ہم آہنگی)
```fortran
...
### MPI (تقسیم شدہ میموری کی ہم آہنگی)
```fortran
program mpi_example
    use mpi
    implicit none
...
## ٹیسٹنگ
### ٹیسٹ فریم ورک (فورٹران ٹیسٹ یا pFUnit)
```fortran
...
### ازگر کا انٹیگریشن (f2py یا مشترکہ لائبریری کے ذریعے)
```fortran
...
## ڈیزائن پیٹرن
### پیٹرن 1: ارے پروگرامنگ (لوپس سے بچیں)
```fortran
...
### پیٹرن 2: پیرامیٹرائزڈ اخذ کردہ اقسام
```fortran
...
### پیٹرن 3: پروسیجر پوائنٹرز کے ساتھ کال بیک پیٹرن
```fortran
module integrator
    implicit none
    abstract interface
...
## فورٹران کب استعمال کریں۔
| منظر نامہ | کیوں فورٹران | بہتر متبادل |
|------------|------------|-------------------|
| HPC / سپر کمپیوٹنگ | عددی کارکردگی کے لیے آپٹمائزڈ | C++ (دیکھ بھال کے ساتھ)، جولیا |
| سائنسی نقالی | تصدیق شدہ کوڈ کی دہائیاں | ازگر (پروٹو ٹائپنگ کے لیے)، C++ |
| آب و ہوا/موسم کے ماڈل | میراثی کوڈ بیس؛ کارکردگی | - |
| کمپیوٹیشنل فزکس | مقامی صف کے آپریشنز | Python (NumPy)، جولیا |
| فنانشل ماڈلنگ (HPC) | بڑے پیمانے پر حساب کے لیے کارکردگی | C++، ازگر |
| عام درخواست کی ترقی | مناسب نہیں | Python, Java, Go |
| ویب ڈویلپمنٹ | مناسب نہیں | جاوا اسکرپٹ، ازگر |
| ڈیٹا سائنس (انٹرایکٹو) | ورک فلو نہیں | ازگر، آر |
---

## مصنوعی سوال و جواب
### Q1: Fortran 90 اور جدید Fortran (2008+) میں کیا فرق ہے؟
**A:** ماڈرن فورٹران نے بہت سی خصوصیات شامل کیں جو اسے مزید اظہار خیال کرتی ہیں:
```fortran
...
### Q2: Fortran arrays C arrays سے کیسے مختلف ہیں؟
**A:** Fortran arrays بلٹ ان آپریشنز کے ساتھ فرسٹ کلاس اشیاء ہیں:
```fortran
...
### Q3: میں Fortran میں زیادہ سے زیادہ کارکردگی کیسے حاصل کروں؟
**A:** کلیدی مشقیں:
- تمام ڈمی دلائل کے لیے واضح`intent`استعمال کریں۔
- ہر جگہ`implicit none`استعمال کریں۔
- لوپس پر سرنی آپریشنز کو ترجیح دیں۔
- مربوط میموری تک رسائی کے نمونوں کا استعمال کریں۔
- کمپائلر آپٹیمائزیشن فلیگ استعمال کریں:`-O3 -march=native -ffast-math`
-`gprof`یا مرتب کرنے والے مخصوص ٹولز کے ساتھ پروفائل
- ان فنکشنز کے لیے`pure`اور`elemental`استعمال کریں جو کمپائلر کو بہتر بنا سکتا ہے۔
### Q4: میں فورٹران کو C کے ساتھ کیسے انٹرفیس کروں؟
**A:**`iso_c_binding`ماڈیول استعمال کریں:
```fortran
use iso_c_binding
...
### Q5: مجھے فورٹران پروجیکٹس کے لیے کون سا بلڈ سسٹم استعمال کرنا چاہیے؟
**A:** CMake کو بہترین فورٹران سپورٹ حاصل ہے۔ FPM (فورٹران پیکیج مینیجر) جدید مقامی آپشن ہے:
```bash
...
### مسئلہ 1: محدود فرق کے ساتھ PDE کو حل کرنا
**مرحلہ 1: مسئلہ کو سمجھیں**
1D حرارت کی مساوات کو حل کریں: du/dt = alpha * d²u/dx²
**مرحلہ 2: نقطہ نظر کی شناخت کریں**
محدود فرق کا استعمال کرتے ہوئے جگہ اور وقت کو الگ کریں۔ ایک واضح اسکیم استعمال کریں۔
**مرحلہ 3: نافذ کریں**```fortran
...
**مرحلہ 4: تصدیق کریں**
کنزرویشن چیک کریں، گرڈ ریفائنمنٹ کے ساتھ کنورجنشن، اور تجزیاتی حل کے ساتھ موازنہ کریں۔
### مسئلہ 2: میٹرکس ڈیگنلائزیشن
**مرحلہ 1: مسئلہ کو سمجھیں**
ایک سمیٹرک میٹرکس کے eigenvalues اور eigenvectors تلاش کریں۔
**مرحلہ 2: نقطہ نظر کی شناخت کریں**
فورٹران کے انٹرفیس کے ذریعے LAPACK کا`dsyev`روٹین استعمال کریں۔
**مرحلہ 3: نافذ کریں**```fortran
program diagonalize
    use lapack95
    implicit none
...
**مرحلہ 4: تصدیق کریں**
چیک کریں کہ A*v = lambda*v ہر eigen pair کے لیے۔
---

## خلاصہ
فورٹران اصل سائنسی پروگرامنگ زبان ہے اور اعلی کارکردگی والے کمپیوٹنگ میں ایک پاور ہاؤس بنی ہوئی ہے۔ ماڈرن فورٹران مقامی صف کے آپریشنز، متوازی پروگرامنگ سپورٹ، اور سی انٹرآپریبلٹی کے ساتھ ایک قابل، ترقی پذیر زبان ہے۔ جب کہ اس کی کمیونٹی چھوٹی اور ماہر ہے، فورٹران دنیا میں سب سے زیادہ مطالبہ کرنے والے کمپیوٹیشنل ورک بوجھ کو چلانا جاری رکھے ہوئے ہے۔ پیمانے پر عددی کمپیوٹنگ کے لیے، فورٹران متعلقہ رہتا ہے۔