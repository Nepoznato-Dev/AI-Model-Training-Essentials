...
-->
# R — عام غلطیاں اور اینٹی پیٹرن
یہ دستاویز تصحیح کے ساتھ R میں سب سے عام غلطیوں، ٹریپس، اور مخالف پیٹرن کی فہرست بناتی ہے۔
---

## 1. لوپس میں بڑھتے ہوئے ڈیٹا فریم
```r
# ❌ WRONG — O(n²) growth
result <- data.frame()
for (i in 1:1000) {
...
## 3. ویکٹر ری سائیکلنگ کو نہ سمجھنا
```r
...
## 4.`T`اور`F`بمقابلہ`TRUE`اور `FALSE`
```r
...
## 5.`stringsAsFactors = FALSE`استعمال نہیں کرنا
```r
...
## 6.`apply`فیملی بمقابلہ Tidyverse
```r
...
## خلاصہ
R کی انٹرایکٹو فطرت جال پیدا کرتی ہے: لوپس میں بڑھتے ہوئے ڈیٹا فریم (پری ایلوکیٹ یا ویکٹرائز)، ویکٹر ری سائیکلنگ سرپرائز،`NA`پروپیگیشن، اور`T`/`F`اوور رائٹ ایبل ہونا۔ R طریقہ یہ ہے: آپریشنز کو ویکٹرائز کریں، لوپنگ کرتے وقت پہلے سے مختص کریں،`NA`کو واضح طور پر ہینڈل کریں، ڈیٹا میں ہیرا پھیری کے لیے tidyverse کا استعمال کریں، اور`<<-`سے بچیں۔ R ان لوگوں کو انعام دیتا ہے جو ویکٹر میں سوچتے ہیں اور پیکجوں کے اس کے بھرپور ماحولیاتی نظام کو استعمال کرتے ہیں۔