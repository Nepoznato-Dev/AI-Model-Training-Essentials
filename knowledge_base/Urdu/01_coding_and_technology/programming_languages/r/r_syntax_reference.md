...
| `+``-``*``/``^`| ریاضی | `2^10`| |
| `%%`| ماڈیولس | `7 %% 3`| واپسی 1 |
| `%/%`| عددی تقسیم | `7 %/% 3`| واپسی 2 |
| `==``!=` | مساوات | `x == y`| ویکٹرائزڈ |
| `<``>``<=``>=` | موازنہ | `x >= y`| ویکٹرائزڈ |
| `&``\|``!`| منطقی (ویکٹرائزڈ) | `x & y`| عنصر وار |
| `&&``\|\|` | منطقی (اسکیلر) | `x && y`| صرف پہلا عنصر |
| `%in%`| رکنیت | `x %in% y`| |
| `<-``=` | تفویض | `x <- 10`| `<-`محاورہ ہے |
| `->`| دائیں تفویض | `10 -> x`| شاذ و نادر ہی استعمال کیا جاتا ہے |
| `<<-`| عالمی تفویض | `x <<- 10`| والدین env میں تفویض کریں |
| `$`| عنصر نکالیں | `df$column`| |
| `[``[[` | اشاریہ کاری | `x[1]`/`x[[1]]`| `[`ایک ہی قسم واپس کرتا ہے؛ `[[`اقتباسات |
| `%>%`| پائپ (magrittr) | `x %>% f()`| |
| `\|>`| پائپ (بیس R 4.1+) | `x \|> f()`| مقامی پائپ |
### ویکٹرائزڈ آپریشنز
```r
...
## خلاصہ
R کا نحو شماریاتی کمپیوٹنگ اور ڈیٹا کے تجزیہ کے لیے ڈیزائن کیا گیا ہے۔ آپریشنز کی ویکٹرائزڈ نوعیت زیادہ تر کاموں کے لیے لوپس کو ختم کرتی ہے۔ صاف ستھرا ماحولیاتی نظام (dplyr, ggplot2, tidyr) ڈیٹا کی ہیرا پھیری اور تصور کے لیے ایک مستقل، پڑھنے کے قابل گرائمر فراہم کرتا ہے۔ R کی شماریاتی ماڈلنگ کی صلاحیتیں - بنیادی ٹی ٹیسٹ سے لے کر مخلوط اثرات والے ماڈلز تک - گہرائی میں بے مثال ہیں۔ ڈیٹا سائنس، شماریات اور تولیدی تحقیق کے لیے، R ایک ضروری ٹول ہے۔