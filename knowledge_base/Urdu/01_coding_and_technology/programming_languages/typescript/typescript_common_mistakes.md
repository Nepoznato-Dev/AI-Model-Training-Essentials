...
# TypeScript — عام غلطیاں اور اینٹی پیٹرن
یہ دستاویز ٹائپ اسکرپٹ میں سب سے عام غلطیوں، ٹریپس اور اینٹی پیٹرن کی فہرست بناتی ہے۔ ہر اندراج غلط نقطہ نظر کو ظاہر کرتا ہے، یہ بتاتا ہے کہ یہ کیوں ناکام ہوتا ہے، اور صحیح حل فراہم کرتا ہے۔
---

## 1.`any`کا زیادہ استعمال
```typescript
...
## 2. ٹائپ گارڈز کے بجائے دعوے ٹائپ کریں۔
```typescript
...
## 3. غیر کالعدم دعویٰ کا غلط استعمال
```typescript
...
## 4. انٹرفیس بمقابلہ اقسام کنفیوژن
```typescript
...
## 6.`strict`موڈ استعمال نہیں کرنا
```json
...
## 9. اینٹی پیٹرن: دہرائی جانے والی اقسام (جنرک استعمال نہیں کرنا)
```typescript
...
## 10. ضرورت سے زیادہ اختیاری زنجیر
```typescript
...
## 12. امتیازی یونینوں کا فائدہ نہیں اٹھانا
```typescript
...
## خلاصہ
TypeScript کا ٹائپ سسٹم طاقتور ہے لیکن نظم و ضبط کی ضرورت ہے۔ بنیادی گناہ`any`تک پہنچ رہا ہے —`unknown`استعمال کریں جب قسم واقعی نامعلوم ہو، اور ٹائپ گارڈز کے ساتھ تنگ ہو۔ غیر متعینہ دعویٰ (`!`) سے پرہیز کریں۔ آبجیکٹ کی شکلوں کے لیے انٹرفیس استعمال کریں، یونین کے لیے اقسام اور کمپیوٹیڈ اقسام۔ امتیازی یونینیں کیڑے کی پوری کلاسوں کو ختم کرتی ہیں۔ پہلے دن سے`strict: true`کو فعال کریں — دوبارہ تیار کرنے کی سختی تکلیف دہ ہے۔ مقصد غیر قانونی ریاستوں کو قسم کی سطح پر ناقابل نمائندگی بنانا ہے۔