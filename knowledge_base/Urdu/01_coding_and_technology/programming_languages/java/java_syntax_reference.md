...
|----------|------|---------|---------|-------|
| `+`| اضافہ | `3 + 2`| `5`| اس کے علاوہ String concatenation |
| `-`| گھٹاؤ | `3 - 2`| `1`| |
| `*`| ضرب | `3 * 2`| `6`| |
| `/`| ڈویژن | `7 / 2`| `3`|`int`کے لیے عددی تقسیم؛ `3.5`برائے`double`|
| `%`| باقی | `7 % 2`| `1`| |
...
|------------|------|---------|---------|
| `==`| برابر | `x == y`| اشیاء کے لیے حوالہ مساوات |
| `!=`| برابر نہیں | `x != y`| |
| `<`,`>`,`<=`,`>=`| رشتہ دار | `x >= y`| |
| `&&`| منطقی اور | `a && b`| شارٹ سرکٹ |
| `\|\|`| منطقی یا | `a \|\| b`| شارٹ سرکٹ |
| `!`| منطقی نہیں | `!true`| `false`|
| `instanceof`| ٹائپ چیک | `obj instanceof String`| نیز پیٹرن میچنگ (جاوا 16+) |
### بٹ وائز آپریٹرز
| آپریٹر | نام | مثال | نوٹس |
|------------|------|---------|---------|
| `&`| اور | `5 & 3`| `1`|
...
| مقدم | آپریٹرز |
|------------|------------|
| 1 (سب سے زیادہ) | پوسٹ فکس:`expr++``expr--` |
| 2 | یونیری:`++expr``--expr``+expr``-expr``~``!` |
| 3 | کاسٹ:`(Type)expr`|
| 4 | ضرب:`*``/``%`|
| 5 | اضافی:`+``-` |
| 6 | شفٹ:`<<``>>``>>>`|
...
## ڈیٹا سٹرکچرز
### مجموعے کا فریم ورک
| انٹرفیس | نفاذ | حکم دیا | تھریڈ سیف ویرینٹ |
|------------|----------------|------------|
| `List`| `ArrayList`,`LinkedList`| جی ہاں | `CopyOnWriteArrayList`|
| `Set`| `HashSet`,`TreeSet`,`LinkedHashSet`| مختلف ہوتی ہے | `CopyOnWriteArraySet`|
| `Map`| `HashMap`,`TreeMap`,`LinkedHashMap`| مختلف ہوتی ہے | `ConcurrentHashMap`|
| `Queue`| `ArrayDeque`,`PriorityQueue`| مختلف ہوتی ہے | `ConcurrentLinkedQueue`|
| `Deque`| `ArrayDeque`| دونوں سرے | `ConcurrentLinkedDeque`|
### فہرستیں۔
```java
// Creation
...
## خلاصہ
جاوا کا نحو 1995 کی ابتدا سے ڈرامائی طور پر تیار ہوا ہے۔ جدید جاوا (21+) میں ریکارڈز، سیل شدہ کلاسز، پیٹرن میچنگ، ورچوئل تھریڈز، اور سٹرنگ ٹیمپلیٹس شامل ہیں — جو اسے پہلے سے کہیں زیادہ جامع اور اظہار خیال کرتا ہے۔ بنیادی نحو واقف رہتا ہے (سی کی طرح، سختی سے ٹائپ شدہ، کلاس پر مبنی)، لیکن لیمبڈاس، اسٹریم API، اور`var`کے اضافے نے بوائلر پلیٹ کو نمایاں طور پر کم کر دیا ہے۔ زبان کی طاقت اس کے بڑے ماحولیاتی نظام، پسماندہ مطابقت کی ضمانتوں، اور JIT کی تالیف کے ذریعے JVM کی کارکردگی کی اصلاح میں مضمر ہے۔