...
-->
# Java - عام غلطیاں اور اینٹی پیٹرن
یہ دستاویز جاوا میں سب سے عام غلطیوں، ٹریپس، اور اینٹی پیٹرن کی فہرست بناتی ہے۔ ہر اندراج غلط نقطہ نظر کو ظاہر کرتا ہے، یہ بتاتا ہے کہ یہ کیوں ناکام ہوتا ہے، اور صحیح حل فراہم کرتا ہے۔
---

## 1. اشیاء کے لیے`equals()`بمقابلہ `==`
```java
...
## 2. تکرار کے دوران مجموعہ میں ترمیم کرنا
```java
...
## 3. وسائل کو بند نہیں کرنا (وسائل کے ساتھ کوشش کریں)
```java
...
## 7. InterruptedException کو ہینڈل نہیں کرنا
```java
...
## 8. خالی مجموعوں کی بجائے null واپس کرنا
```java
...
## 9. ہیش میپ کیز کے طور پر تبدیل ہونے والی آبجیکٹ
```java
...
## 10. اینٹی پیٹرن:`Exception`یا`Throwable`کو پکڑنا
```java
...
## 11.`equals`کو اوور رائیڈ کرتے وقت`hashCode`کو اوور رائیڈ نہیں کرنا
```java
...
## 12. ڈبل چیک شدہ لاکنگ کے ساتھ ریس کے حالات
```java
...
## خلاصہ
جاوا کی وربوسیٹی ٹھیک ٹھیک کیڑے چھپا سکتی ہے:`==`بمقابلہ `equals()`، ہم آہنگی ترمیم، وسائل کی لیک، آٹو باکسنگ کارکردگی کے جال، اور برابر/ہیش کوڈ معاہدہ۔ جدید جاوا اپروچ یہ ہے: null-safe موازنہ کے لیے`Objects.equals()`استعمال کریں، ہمیشہ وسائل کے ساتھ کوشش کریں، null کے بجائے خالی مجموعے واپس کریں، مخصوص استثناء کو پکڑیں، اور واحد ذمہ داری کے اصول پر عمل کریں۔ Java 8+ خصوصیات (اسٹریمز، اختیاری، طریقہ کار کے حوالے) بوائلر پلیٹ کو کم کرتی ہیں جبکہ کوڈ کو محفوظ اور زیادہ اظہار خیال کرتی ہیں۔