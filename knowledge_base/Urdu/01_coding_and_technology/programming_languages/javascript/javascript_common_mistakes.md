...
-->
# JavaScript — عام غلطیاں اور اینٹی پیٹرن
یہ دستاویز JavaScript میں سب سے عام غلطیوں، ٹریپس، اور مخالف پیٹرن کی فہرست بناتی ہے۔ ہر اندراج غلط نقطہ نظر کو ظاہر کرتا ہے، یہ بتاتا ہے کہ یہ کیوں ناکام ہوتا ہے، اور صحیح حل فراہم کرتا ہے۔
---

## 1.`this`بائنڈنگ کنفیوژن
```javascript
...
## 3.`var`لہرانے اور دائرہ کار کے مسائل
```javascript
...
## 5. وعدوں سے انکار کو ہینڈل نہ کرنا
```javascript
...
## 6. تکرار کے دوران اشیاء میں ترمیم کرنا
```javascript
...
## 9. ارے جیسی آبجیکٹ ارے نہیں ہیں۔
```javascript
...
## 10. اینٹی پیٹرن: میٹیٹنگ فنکشن پیرامیٹرز
```javascript
...
## 11. ایونٹ لوپ اور مائیکروٹاسکس کو نہ سمجھنا
```javascript
...
## 12. اینٹی پیٹرن:`eval()`استعمال کرنا
```javascript
...
## 13. ایونٹ سننے والوں کو صاف کرنا بھول جانا
```javascript
// ❌ WRONG — memory leak
class Component {
  mount() {
...
## خلاصہ
JavaScript کی لچک ٹریپس کے ساتھ آتی ہے:`this`بائنڈنگ کال سائٹ پر منحصر ہے،`==`جبر کرتا ہے، وعدوں کے لیے غلطی سے نمٹنے کی ضرورت ہوتی ہے، اور ایونٹ لوپ آرڈرنگ (میکروٹاسکس سے پہلے مائکروٹاسک) غیر واضح ہے۔ جدید اصول سادہ ہیں:`const`/`let`never`var`استعمال کریں،`==`پر`===`کو ترجیح دیں، ہمیشہ وعدے سے انکار کو ہینڈل کریں، کال بیکس کے لیے تیر کے فنکشن کا استعمال کریں، تبدیلی سے بچیں، اور ایونٹ سننے والوں کو صاف کریں۔ ان اصولوں پر عمل کرنے سے JavaScript کے زیادہ تر کیڑے ختم ہو جاتے ہیں۔