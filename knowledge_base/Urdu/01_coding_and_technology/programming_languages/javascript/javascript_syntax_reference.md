...
|----------|------|---------|---------|-------|
| `+`| اضافہ | `3 + 2`| `5`| اس کے علاوہ سٹرنگ کنکٹنیشن |
| `-`| گھٹاؤ | `3 - 2`| `1`| |
| `*`| ضرب | `3 * 2`| `6`| |
| `/`| ڈویژن | `7 / 2`| `3.5`| |
| `%`| باقی | `7 % 2`| `1`| |
| `**`| وضاحت | `2 ** 10`| `1024`| ES2016 |
| `++`| اضافہ | `x++`/`++x`| | پوسٹ اور پری انکریمنٹ |
| `--`| کمی | `x--`/`--x`| | پوسٹ اور پری کمی |
| `+x`,`-x`| یونیری | `+"42"`→`42`| نمبر پر جبر ٹائپ کریں |
### موازنہ اور مساوات
| آپریٹر | نام | مثال | نوٹس |
|------------|------|---------|---------|
| `===`| سخت مساوات | `1 === 1`| کسی قسم کا جبر نہیں - ہمیشہ اس کو ترجیح دیں |
| `!==`| سخت عدم مساوات | `1 !== "1"`| `true`— مختلف اقسام |
| `==`| خلاصہ مساوات | `1 == "1"`| جبر کی قسم - اجتناب کریں |
| `!=`| خلاصہ عدم مساوات | `1 != "1"`| `false`- بچیں |
| `<`,`>`,`<=`,`>=`| رشتہ دار | `"b" > "a"`| سٹرنگز کے لیے لغت |
### منطقی اور کالعدم آپریٹرز
| آپریٹر | نام | مثال | نوٹس |
|------------|------|---------|---------|
| `&&`| منطقی اور | `a && b`| پہلی غلط یا آخری قیمت |
| `\|\|`| منطقی یا | `a \|\| b`| پہلی سچائی یا آخری قیمت |
| `!`| منطقی نہیں | `!true`| `false`|
| `??`| nullish Coalescing | `a ?? b`|`b`صرف اس صورت میں لوٹاتا ہے جب`a``null` /`undefined`ہے |
| `?.`| اختیاری زنجیر | `obj?.prop?.method()`|`undefined`تک شارٹ سرکٹس |
### بٹ وائز آپریٹرز
| آپریٹر | نام | مثال | نوٹس |
|------------|------|---------|---------|
| `&`| اور | `5 & 3`| `1`|
...
| `~`| نہیں | `~5`| `-6`|
| `<<`| بائیں شفٹ | `5 << 1`| `10`|
| `>>`| دستخط شدہ دائیں شفٹ | `-5 >> 1`| `-3`|
| `>>>`| غیر دستخط شدہ دائیں شفٹ | `-1 >>> 0`| `4294967295`|
### اسائنمنٹ اور ڈیسٹرکچرنگ
```javascript
// Basic assignment
let x = 10;
...
|------------|------------|---------------|
| 1 (سب سے زیادہ) | `()``[]``.``?.` | بائیں |
| 2 | `new`(بغیر args) | دائیں |
| 3 | `++``--` (پوسٹ فکس) | N/A |
| 4 | `!``~``+x``-x``typeof``void``delete``++` `--`prefix) | دائیں |
| 5 | `**`| دائیں |
| 6 | `*``/``%`| بائیں |
| 7 | `+``-` | بائیں |
| 8 | `<<``>>``>>>`| بائیں |
| 9 | `<``<=``>``>=``in``instanceof` | بائیں |
| 10 | `===``!==``==``!=` | بائیں |
| 11 | `&`| بائیں |
| 12 | `^`| بائیں |
| 13 | `\|`| بائیں |
...
| 16 | `??`| بائیں |
| 17 | `=``+=``-=`وغیرہ | دائیں |
| 18 (سب سے کم) | `=>`(تیر کا فنکشن) | دائیں |
---
...
## افعال
### فنکشن ڈیکلریشنز اور ایکسپریشنز
```javascript
...
| مجموعہ | حکم دیا | تغیر پذیر | نقلیں | کلیدی قسم | تلاش کریں |
|------------|---------|---------|------------|------------|---------|
| `Array`| جی ہاں | جی ہاں | جی ہاں | انڈیکس (نمبر) | O(n) |
| `Object`| ہاں (داخل) | جی ہاں | چابیاں: نہیں | سٹرنگ/علامت | O(1) اوسط |
| `Map`| ہاں (داخل) | جی ہاں | چابیاں: نہیں | کوئی بھی | O(1) اوسط |
| `Set`| ہاں (داخل) | جی ہاں | نہیں | - | O(1) اوسط |
| `WeakMap`| نہیں | جی ہاں | چابیاں: نہیں | صرف اعتراض | O(1) اوسط |
| `WeakSet`| نہیں | جی ہاں | نہیں | صرف اعتراض | O(1) اوسط |
### صفیں۔
```javascript
// Creation
...
### کمزور ریف اور فائنلائزیشن رجسٹری
```javascript
...
## خلاصہ
JavaScript کا نحو سطح پر دھوکہ دہی سے آسان ہے لیکن اہم گہرائی پر مشتمل ہے۔ زبان ایک سادہ اسکرپٹنگ ٹول سے کلاسز، ماڈیولز، جنریٹرز، پراکسیز، اور ایک نفیس async ماڈل کے ساتھ ملٹی پیراڈیم زبان میں تیار ہوئی ہے۔ آپریٹر کی ترجیح کو سمجھنا، ایونٹ لوپ، پروٹو ٹائپ چینز، اور`this`بائنڈنگ کی باریکیاں قابل ڈویلپرز کو ماہر سے الگ کرتی ہیں۔ جدید جاوا اسکرپٹ (ES2020+) اختیاری زنجیر کے ساتھ، nullish coalescing، `Promise.allSettled`، اور اعلیٰ سطح کا انتظار پسماندہ مطابقت کو برقرار رکھتے ہوئے بوائلر پلیٹ کو کم کرتا رہتا ہے۔