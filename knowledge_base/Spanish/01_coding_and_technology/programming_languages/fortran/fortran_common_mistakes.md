...
# Fortran: errores comunes y antipatrones
Este documento cataloga los errores, trampas y antipatrones más comunes en Fortran con correcciones.
---

## 1. Indexación basada en 1
```fortran
...
## 3. Asignación de matriz frente a elemento
```fortran
...
## 5. No utilizar las funciones modernas de Fortran
```fortran
...
## 6. Pérdidas de memoria con matrices asignables
```fortran
...
## Resumen
El enfoque informático científico de Fortran crea trampas: indexación basada en 1, tipado implícito (use siempre `implicit none`), operaciones de matriz frente a bucles y especificaciones de intención. Modern Fortran (2003+) tiene módulos, tipos derivados y programación orientada a objetos: utilícelos. La forma de Fortran es:`implicit none`siempre, especifique`intent`para todos los argumentos, use operaciones de matriz y desasigne lo que asigne.