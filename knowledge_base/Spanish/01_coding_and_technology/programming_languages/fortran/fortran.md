...
## Por qué es importante Fortran
- **Rendimiento de HPC**: los compiladores de Fortran producen algunos de los códigos numéricos más rápidos disponibles, que a menudo igualan o superan a C/C++ para operaciones de matriz.
- **Bases de código heredadas**: Décadas de código científico (modelos climáticos, simulaciones físicas) están escritos en Fortran.
- **Operaciones de matriz**: compatibilidad nativa con matrices multidimensionales con sintaxis diseñada para cálculo matemático.
- **Estabilidad numérica**: el lenguaje y los compiladores están optimizados para el cálculo de punto flotante.
- **Coarrays**: soporte de programación paralela integrado (Fortran 2008+).
- **Aún en evolución**: Fortran 2023 agrega características modernas mientras mantiene la compatibilidad con versiones anteriores.
## Las compensaciones
...
## Fundamentos de sintaxis (Fortran moderno)
```fortran
program hello
    implicit none
...
### Tipos derivados y procedimientos vinculados a tipos
```fortran
...
### Procedimientos genéricos y sobrecarga de operadores
```fortran
module vector_ops
    implicit none
...
### Módulos y organización del programa
```fortran
module constants
    implicit none
...
### Coarrays (paralelismo incorporado en Fortran 2008)
```fortran
...
### MPI (Paralelismo de memoria distribuida)
```fortran
program mpi_example
    use mpi
    implicit none
...
### Administrador de paquetes Fortran (fpm)
```toml
# fpm.toml
...
## Pruebas
### Marco de prueba (fortran-test o pFUnit)
```fortran
...
## Interoperabilidad
### Interoperabilidad C (ISO_C_BINDING)
```fortran
module c_interface
...
### Integración con Python (a través de f2py o biblioteca compartida)
```fortran
...
## Patrones de diseño
### Patrón 1: Programación de matrices (evitar bucles)
```fortran
...
### Patrón 2: tipos derivados parametrizados
```fortran
...
### Patrón 3: Patrón de devolución de llamada con punteros de procedimiento
```fortran
module integrator
    implicit none
    abstract interface
...
### Indicadores de optimización del compilador
```bash
...
## Cuándo usar Fortran
| Escenario | ¿Por qué Fortran? Mejor alternativa |
|----------|-----------|-------------------|
| HPC / supercomputación | Optimizado para rendimiento numérico | C++ (con cuidado), Julia |
| Simulaciones científicas | Décadas de código validado | Python (para creación de prototipos), C++ |
| Modelos climáticos/meteorológicos | Bases de código heredadas; rendimiento | — |
| Física computacional | Operaciones de matriz nativa | Python (NumPy), Julia |
| Modelización financiera (HPC) | Rendimiento para computación a gran escala | C++, Pitón |
| Desarrollo de aplicaciones generales | No adecuado | Python, Java, Ir |
