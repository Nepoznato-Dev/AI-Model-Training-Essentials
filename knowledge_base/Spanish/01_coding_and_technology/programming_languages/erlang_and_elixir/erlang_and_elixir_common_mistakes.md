...
## 2. No vincular/monitorear procesos (Erlang)
```erlang
...
## 3. Llamada GenServer sin tiempo de espera (Elixir)
```elixir
...
## 4. Coincidencia de patrón en`receive`sin tiempo de espera (Erlang)
```erlang
...
## 5. No utilizar`with`para la coincidencia de patrones anidados (Elixir)
```elixir
...
## 6. Antipatrón: devoluciones de llamada de GenServer de larga duración
```elixir
...
## Resumen
Trampas de Erlang/Elixir: las variables son de asignación única, los procesos necesitan enlaces/monitores, las llamadas a GenServer necesitan tiempos de espera explícitos,`receive`se bloquea para siempre sin tiempo de espera y las devoluciones de llamadas de GenServer no deberían bloquearse. La forma BEAM es: dejar que falle (con supervisión), usar`with`para una coincidencia de patrones limpia, mantener rápidas las devoluciones de llamadas de GenServer y siempre manejar las fallas del proceso.