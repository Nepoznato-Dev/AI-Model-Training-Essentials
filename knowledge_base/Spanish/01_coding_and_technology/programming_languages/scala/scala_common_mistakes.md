...
-->
# Scala: errores comunes y antipatrones
Este documento cataloga los errores, trampas y antipatrones más comunes en Scala con correcciones.
---

## 1. Usando`var`en lugar de `val`
```scala
...
## 2. No utilizar coincidencia de patrones para tipos sellados
```scala
// ❌ WRONG — if/else chain
...
## 3. Rendimiento de las colecciones
```scala
...
## 4. Infierno de devolución de llamada futura
```scala
...
## 5. Abuso de conversiones implícitas
```scala
...
## 6. No manejar`Option`correctamente
```scala
...
## 8. Borrado de tipos con genéricos
```scala
...
## Resumen
El poder de Scala viene con una responsabilidad: prefiera`val`sobre `var`, use coincidencia de patrones para tipos sellados, maneje`Option`correctamente (nunca llame a `.get`), use for-comprehensiions en lugar de Futures anidados y evite null. Scala 3 simplifica muchos de estos patrones con enumeraciones, métodos de extensión y tipos de unión. El principio clave: si el compilador puede comprobarlo, déjelo comprobarlo.