...
-->
# Scala - Referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para Scala (3.x con notas sobre las diferencias de 2.x). Complementa la referencia principal de Scala al centrarse en patrones de sintaxis exhaustivos, el sistema de tipos, construcciones de programación funcional y modismos de Scala.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmética | `a + b`| |
| `**`| Potencia (Gran Decimal) | `BigDecimal(2) ** 10`| No integrado para Int |
| `==``!=` | Igualdad | `a == b`| Llamadas`.equals()`— seguro para nulos |
| `eq``ne` | Igualdad de referencia | `a eq b`| Misma instancia |
| `<``>``<=``>=` | Comparación | `a >= b`| Requiere`Ordered`|
| `&&``\|\|``!`| Lógico | `a && b`| Cortocircuito |
| `&``\|``^``~` | Bit a bit | `a & b`| |
| `<<``>>``>>>`| Cambio | `a << 2`| |
| `+:``:+``::``:::` | Operaciones de recolección | `1 +: list`| Asociativo derecho si termina en`:`|
| `->`| Creación de tuplas | `"key" -> "value"`| `("key", "value")`|
| `=>`| Función/flecha | `x => x + 1`| Lambda o mapeo de tipos |
| `<-`| Generador | `x <- List(1,2,3)`| Para la comprensión |
### Reglas del operador
```scala
...
## Resumen
La sintaxis de Scala unifica la programación funcional y orientada a objetos bajo un potente sistema de tipos. La coincidencia de patrones, la comprensión y las clases de casos proporcionan un modelado de datos expresivo. Los rasgos permiten una composición flexible sin limitaciones de herencia única. El sistema de tipos (con genéricos, varianza, límites de tipos y tipos opacos) detecta errores en el momento de la compilación y permite abstracciones sin costo. Scala 3 simplifica el lenguaje con enumeraciones, métodos de extensión, dado/uso y tipos opacos, lo que hace que el lenguaje sea más accesible y conserva todo su poder.