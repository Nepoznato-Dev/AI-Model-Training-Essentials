...
- **Sistemas críticos para la seguridad**: Diseñado desde cero para sistemas donde el fallo no es una opción.
- **Escritura fuerte**: el sistema de tipos más estricto de cualquier lenguaje convencional: detecta errores en el momento de la compilación que otros lenguajes pasan por alto.
- **Simultaneidad incorporada**: las tareas (programación concurrente) son parte del lenguaje, no una biblioteca.
- **Verificación formal**: admite métodos formales para demostrar la corrección del programa.
- **Funciones de confiabilidad**: excepciones integradas, contratos (condiciones previas y posteriores) y verificaciones de tiempo de ejecución.
- **Ada moderna**: Ada 2012 y Ada 2022 han agregado características modernas manteniendo las garantías de seguridad.
## Las compensaciones
| Limitación | Detalles | Solución típica |
|-----------|-----------------|-------------------|
| **Comunidad de nicho** | Pequeña base de desarrolladores en comparación con C, Java o Python | Comunidad especializada pero conocedora |
| **Sintaxis detallada** | Más detallado que C o Python | Aceptar como parte del diseño orientado a la seguridad |
| **Ecosistema limitado** | Menos bibliotecas que los idiomas convencionales | Escribir código personalizado; utilizar la extensa biblioteca estándar |
| **Curva de aprendizaje** | Lenguaje complejo con muchas características de seguridad | Comience con el subconjunto "SPARK" de Ada para trabajos críticos para la seguridad |
| **Disponibilidad del compilador** | Menos opciones de compilador (GNAT es el principal) | GNAT es gratuito y está bien mantenido |
---
...
### Tipos etiquetados y herencia (OOP)
```ada
-- Abstract base type
package Shapes is
...
### Genéricos (polimorfismo en tiempo de compilación)
```ada
...
### Objetos protegidos (datos compartidos seguros para subprocesos)
```ada
...
## Interoperabilidad
### Interoperabilidad C (Interfaces)
```ada
with Interfaces.C;
...
## Patrones de diseño
### Patrón 1: Observador con objetos protegidos
```ada
...
### Patrón 2: Máquina de estados con tipos discriminados
```ada
...
### Patrón 3: RAII con tipos controlados
```ada
...
### Abstracciones sin gastos generales
```ada
...
## Implementación
### SPARK para la certificación de seguridad crítica
```ada
...
## Cuándo utilizar Ada
| Escenario | ¿Por qué Ada? Mejor alternativa |
|----------|---------|-------------------|
| Sistemas críticos para la seguridad | Diseñado para esto; soporte de verificación formal | — |
| Aviación/aeroespacial | Compiladores certificados; Cumplimiento de DO-178C | — |
| Sistemas de defensa | herencia del Departamento de Defensa; características de seguridad | — |
| Ferrocarriles/dispositivos médicos | Requisitos de alta integridad | — |
...
## Resumen
Ada es un lenguaje creado para la corrección. Su estricto sistema de tipos, su simultaneidad incorporada y su soporte de verificación formal lo convierten en la opción para sistemas donde las fallas no son aceptables. Si bien su comunidad es pequeña en comparación con los idiomas principales, Ada sigue siendo esencial en la aviación, la defensa, el espacio y otros dominios críticos para la seguridad. Para estas aplicaciones, el enfoque riguroso de Ada hacia la ingeniería de software no es una limitación: es el punto.