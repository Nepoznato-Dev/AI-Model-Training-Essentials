...
-->
# Swift - Referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para Swift (5.9+). Complementa la referencia principal de Swift centrándose en patrones de sintaxis exhaustivos, opciones, programación orientada a protocolos y concurrencia moderna.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmética | `a + b`| |
| `..``...` | Gama | `1...10`| `...`cerrado; `..<`medio abierto |
| `==``!=``<``>``<=``>=` | Comparación | `a == b`| Requiere`Equatable`/`Comparable`|
| `===``!==` | Identidad | `a === b`| Misma referencia (solo clases) |
| `&&``\|\|``!`| Lógico | `a && b`| Cortocircuito |
| `??`| Fusión nula | `a ?? b`| Devuelve`b`si`a`es nulo |
| `?.`| Encadenamiento opcional | `a?.b`| Devuelve nulo si`a`es nulo |
| `!`| Forzar desenvolver | `a!`| Se bloquea si es nulo: evitarlo |
| `&`| Parámetro de entrada y salida | `swap(&a, &b)`| Pasar por referencia |
### Operadores de manipulación nula
```swift
...
## Resumen
La sintaxis de Swift equilibra la seguridad con la expresividad. Las opciones eliminan los fallos nulos, los protocolos permiten una composición flexible sin herencia y la concurrencia moderna (async/await, actores) hace accesible la programación concurrente segura. El lenguaje continúa evolucionando con características como paquetes de parámetros, macros y propiedad; cada adición hace que Swift sea más seguro y poderoso, al tiempo que mantiene su promesa principal: código que es seguro por construcción.