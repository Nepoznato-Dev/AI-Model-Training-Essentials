...
---
# Swift: errores comunes y antipatrones
Este documento cataloga los errores, trampas y antipatrones más comunes en Swift. Cada entrada muestra el enfoque incorrecto, explica por qué falla y proporciona la solución correcta.
---

## 1. Forzar el desenvolvimiento de opciones opcionales
```swift
// ❌ WRONG — crash if nil
...
## 3. No utilizar`enum`con valores asociados
```swift
...
## 4. Malentendido entre los tipos de valor y los tipos de referencia
```swift
...
## 5. Antipatrón: Controlador de vista masiva
```swift
...
## 6. No utilizar`guard`para la salida anticipada
```swift
...
## 8. Confusión en el índice de cadenas
```swift
...
## 9. No utilizar`lazy`para propiedades caras
```swift
...
## 10. Seguridad de subprocesos con `@MainActor`
```swift
...
## Resumen
Las características de seguridad de Swift evitan muchos errores, pero introducen sus propias trampas: forzar el desenvolvimiento provoca fallas, retener ciclos se esconden en cierres y delegados, y la semántica de tipos de valores sorprende a los desarrolladores que provienen de lenguajes de tipo de referencia. La forma Swift es: nunca fuerce el desenvolvimiento (`!`), use`guard`para salidas anticipadas, use`weak`para delegados y capturas de cierre, marque el código de la interfaz de usuario con`@MainActor`y divida controladores de vista masivos en modelos y servicios de vista. El sistema de tipos de Swift es tu amigo: trabaja con él, no en contra de él.