...
-->
# Visual Basic: referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para Visual Basic (VB.NET). Complementa la referencia principal de Visual Basic centrándose en patrones de sintaxis exhaustivos, funciones de programación orientada a objetos, LINQ, async/await y lenguajes de integración .NET.
---
...
| | `<=`| Menos o igual | `a <= b`|
| | `>=`| Mayor o igual | `a >= b`|
| **Lógico** | `AndAlso`| Cortocircuito Y | `a AndAlso b`|
| | `OrElse`| Cortocircuito O | `a OrElse b`|
| | `Not`| NO lógico | `Not a`|
| | `And`| Bit a bit Y | `a And b`|
| | `Or`| Bit a bit O | `a Or b`|
| | `Xor`| XOR bit a bit | `a Xor b`|
| **Cadena** | `&`| Concatenación | `"Hello" & " " & "World"`|
| | `+=`| Agregar | `s &= " more"`|
| **Asignación** | `=`| Asignar | `x = 42`|
| **Nulo** | `Is`| Igualdad de referencia | `obj Is Nothing`|
| | `IsNot`| Desigualdad de referencia | `obj IsNot Nothing`|
| **Tipo** | `TryCast`| Reparto seguro (no devuelve nada) | `TryCast(obj, String)`|
| | `CType`| Convertir | `CType(obj, Integer)`|
| | `DirectCast`| Reparto directo (tira al fallo) | `DirectCast(obj, String)`|
---
...
## Resumen
La sintaxis de Visual Basic es limpia y similar al inglés, lo que lo convierte en uno de los lenguajes de programación más legibles. VB.NET proporciona acceso completo al ecosistema .NET, incluido LINQ para consulta de datos, async/await para operaciones concurrentes, genéricos para seguridad de tipos y programación orientada a objetos integral con clases, interfaces y herencia. La distinción entre mayúsculas y minúsculas del lenguaje y las palabras clave detalladas (End If, End Sub, End Class) mejoran la legibilidad. Para aplicaciones empresariales de Windows, automatización de oficina (VBA) y creación rápida de prototipos dentro de .NET, VB.NET sigue siendo un lenguaje productivo y capaz.