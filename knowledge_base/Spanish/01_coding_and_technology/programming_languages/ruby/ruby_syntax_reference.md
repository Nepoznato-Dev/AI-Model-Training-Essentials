...
# Ruby - Referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para Ruby (3.x). Complementa la referencia principal de Ruby centrándose en patrones de sintaxis exhaustivos, bloques y cierres, metaprogramación y modismos de Ruby.
---
...
| `==``!=``===``<=>` | Igualdad | `a == b`| `===`es igualdad de casos; `<=>`es una nave espacial |
| `eql?`| Igualdad de valores | `a.eql?(b)`| Más estricto que`==`(tipo de cheques) |
| `equal?`| Identidad | `a.equal?(b)`| ¿El mismo objeto? |
| `<``>``<=``>=` | Comparación | `a >= b`| |
| `&&``\|\|``!`| Lógico | `a && b`| Cortocircuito; devuelve el último valor evaluado |
| `and``or``not`| Lógico (baja precedencia) | `a and b`| Evite: utilice`&&`/`\|\|`/`!`|
| `&``\|``^``~``<<``>>` | Bit a bit | `a & b`| |
| `=~``!~` | Coincidencia de expresiones regulares | `str =~ /pattern/`| Índice de devoluciones o nulo |
| `..``...` | Gama | `1..10`| `..`inclusive; `...`exclusivo |
| `?.`| Navegación segura | `user&.name`| Devuelve nulo si el usuario es nulo |
...
## Resumen
La sintaxis de Ruby está diseñada para la felicidad de los desarrolladores: se lee como en inglés y recompensa la expresividad. El poder del lenguaje proviene de su modelo de objetos (todo es un objeto), bloques y cierres, capacidades de metaprogramación y el módulo Enumerable que proporciona una rica API de procesamiento de colecciones. Ruby 3.x ha agregado coincidencia de patrones, firmas de tipo RBS y Ractor para concurrencia, manteniendo al mismo tiempo la filosofía central del lenguaje: hacer que los programadores sean productivos y felices.