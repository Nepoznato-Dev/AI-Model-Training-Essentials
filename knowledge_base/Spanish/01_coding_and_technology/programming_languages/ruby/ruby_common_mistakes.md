...
## 1. Modificar una colección durante la iteración
```ruby
...
## 3. Interpolación de cadenas frente a concatenación
```ruby
...
## 4. No utilizar bloques/enumeradores idiomáticamente
```ruby
# ❌ WRONG — C-style loop
i = 0
...
## 6. Clases básicas de parcheo de monos
```ruby
...
## 7. No entender`return`en bloques
```ruby
...
## 9. Confusión entre símbolo y cadena
```ruby
...
## 10. No congelar literales de cadena
```ruby
...
## Resumen
La expresividad de Ruby puede conducir a antipatrones: modificación de colecciones durante la iteración, parches de clases principales, variables globales y modelos gruesos. La forma Ruby es: usar enumeradores (`each`,`map`,`reject`), preferir símbolos y argumentos de palabras clave, congelar cadenas literales, extraer objetos de servicio de modelos y usar refinamientos en lugar de parches de mono. Ruby recompensa a los desarrolladores que adoptan los bloques, la escritura flexible y el principio de mínima sorpresa.