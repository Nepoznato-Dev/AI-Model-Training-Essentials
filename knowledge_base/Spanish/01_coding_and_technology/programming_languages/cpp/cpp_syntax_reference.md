...
Este documento proporciona una referencia de sintaxis completa y estructurada para C++ moderno (C++17/20/23). Complementa la referencia principal de C++ centrándose en patrones de sintaxis exhaustivos, tablas de operadores, mecánica de plantillas y características modernas.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%`| Aritmética | `a + b`| Igual que C |
| `++``--` | Incremento/Decremento | `++i`,`i++`| Prefiere preincremento |
| `==``!=``<``>``<=``>=` | Comparación | `a <=> b`| `<=>`es una comparación de tres vías (C++20) |
| `&&``\|\|``!`| Lógico | `a && b`| Cortocircuito |
| `&``\|``^``~``<<``>>` | Bit a bit | `a & b`| Igual que C |
| `?:`| Ternario | `cond ? a : b`| |
### Operadores específicos de C++
| Operador | Nombre | Ejemplo | Notas |
|----------|------|---------|-------|
| `::`| Resolución de alcance | `std::cout`| |
| `.*``->*` | Puntero a miembro | `obj.*pmf`| |
| `<=>`| Nave espacial (C++20) | `a <=> b`| Devuelve`strong_ordering`|
| `co_await`| Suspensión de rutina | `co_await expr`| C++20 |
| `co_yield`| Rendimiento de rutina | `co_yield value`| C++20 |
| `co_return`| Retorno de rutina | `co_return value`| C++20 |
...
## Controlar el flujo
### Encuadernaciones estructuradas y patrones modernos
```cpp
...
## Funciones y Lambdas
### Sintaxis de funciones modernas
```cpp
...
## Punteros inteligentes y memoria
```cpp
...
| Contenedor | Encabezado | Ordenado | Búsqueda | Notas |
|-----------|--------|---------|--------|-------|
| `vector`| `<vector>`| Sí | O(n) | Elección predeterminada para secuencias |
| `deque`| `<deque>`| Sí | O(n) | Empuje/pop rápido en ambos extremos |
| `list`| `<list>`| Sí | O(n) | Lista doblemente enlazada |
| `array`| `<array>`| Sí | O(n) | Tamaño fijo, asignación de pila |
| `set`/`multiset`| `<set>`| Sí | O(log n) | Árbol rojo-negro |
| `map`/`multimap`| `<map>`| Sí (por clave) | O(log n) | Valor-clave, árbol rojo-negro |
| `unordered_set`| `<unordered_set>`| No | O(1) promedio | Tabla hash |
| `unordered_map`| `<unordered_map>`| No | O(1) promedio | Tabla hash |
### Algoritmos clave
```cpp
#include <algorithm>
#include <numeric>
...
## Resumen
La sintaxis de C++ abarca desde la manipulación de punteros de bajo nivel hasta la metaprogramación de plantillas de alto nivel y la sincronización basada en rutinas. La evolución del lenguaje a través de C++ 17/20/23 ha mejorado drásticamente la seguridad (punteros inteligentes, `std::optional`, conceptos), la expresividad (enlaces estructurados, rangos, `std::format`) y el rendimiento (conceptos que permiten abstracciones de costo cero, corrutinas para asíncrono sin sobrecarga de pila). La filosofía central permanece sin cambios: no debes pagar por lo que no usas, y el lenguaje debe soportar todos los niveles de abstracción, desde lenguajes básicos hasta lenguajes de dominio específico.