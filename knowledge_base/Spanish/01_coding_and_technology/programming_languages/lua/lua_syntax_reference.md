...
-->
# Lua — Referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para Lua (5.4). Complementa la referencia principal de Lua centrándose en patrones de sintaxis exhaustivos, tablas y metatablas, corrutinas y la filosofía de diseño única de Lua.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%``^` | Aritmética | `2 ^ 10`| `^`es exponenciación |
| `//`| División del piso | `7 // 2`| Devuelve 3 (Lua 5.3+) |
| `==``~=` | Igualdad | `a == b`| `~=`"no es igual" |
| `<``>``<=``>=` | Comparación | `a >= b`| |
| `and``or``not`| Lógico | `a and b`| Devuelve operandos, no booleanos |
| `..`| Concatenación | `"hello" .. " world"`| |
| `#`| Longitud | `#t`| Longitud de cuerda o mesa |
| `&``\|``~``<<``>>`| Bit a bit | `a & b`| Lua 5.3+ (operaciones enteras) |
### Operadores lógicos: devoluciones no booleanas
```lua
...
## Tablas: la estructura de datos universal
```lua
...
## Resumen
La sintaxis de Lua es deliberadamente mínima: todo el lenguaje se puede describir en una sola página. Las tablas sirven como estructura de datos universal (matrices, diccionarios, objetos, módulos). Las metatablas proporcionan extensibilidad sin complejidad del lenguaje. Las corrutinas permiten realizar múltiples tareas cooperativas y patrones iteradores elegantes. El poder del lenguaje proviene de su capacidad de integración: Lua está diseñado para vivir dentro de aplicaciones host, proporcionando capacidades de scripting con una sobrecarga mínima. Para el desarrollo, la configuración y los sistemas integrados de juegos, la simplicidad de Lua es su mayor fortaleza: un lenguaje pequeño que hace exactamente lo que necesita y nada más.