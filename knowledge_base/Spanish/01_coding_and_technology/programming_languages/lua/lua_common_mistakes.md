...
## 3. Semántica de referencia de tabla
```lua
...
## 4. Variables globales por defecto
```lua
...
## 7. No utilizar metatablas para programación orientada a objetos
```lua
...
## Resumen
El minimalismo de Lua crea trampas: indexación basada en 1, solo`nil`y`false`son falsos, las tablas son referencias (no valores), las variables son globales de forma predeterminada y`pairs`está desordenado mientras que`ipairs`está ordenado. La forma de Lua es: usar siempre `local`, usar`ipairs`para matrices y`pairs`para diccionarios, usar`table.concat`para crear cadenas y usar metatablas para programación orientada a objetos. La simplicidad de Lua es su poder, pero respete los casos extremos.