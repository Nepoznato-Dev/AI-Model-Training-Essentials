...
---
# PHP — Referencia de sintaxis
Este documento proporciona una referencia de sintaxis estructurada y completa para PHP (8.x). Complementa la referencia principal de PHP centrándose en patrones de sintaxis exhaustivos, características modernas de PHP, programación orientada a objetos y modismos prácticos.
---
...
| `==``!=``===``!==` | Igualdad | `$a === $b`| `===`también comprueba el tipo |
| `<=>`| Nave espacial | `$a <=> $b`| Devuelve -1, 0 o 1 |
| `<``>``<=``>=` | Comparación | `$a >= $b`| |
| `&&``\|\|``!`| Lógico | `$a && $b`| Cortocircuito |
| `and``or``xor`| Lógico (baja precedencia) | `$a and $b`| Evite: utilice`&&`/`\|\|`|
| `??`| Coalición nula | `$a ?? 'default'`| Devuelve`'default'`si`$a`es nulo |
| `??=`| Asignación coalescente nula | `$a ??= 'default'`| Asigna sólo si`$a`es nulo |
| `?``:` | Ternario | `$a ? $b : $c`| |
| `.`| Concatenación | `$a . $b`| |
| `.=`| Concatenar asignar | `$a .= $b`| |
| `=>`| Asignación de matriz | `['key' => 'value']`| |
| `->`| Acceso de miembros | `$obj->method()`| |
| `?->`| Operador nulo seguro | `$obj?->method()`| Devuelve nulo si`$obj`es nulo |
| `::`| Acceso estático | `ClassName::method()`| |
| `instanceof`| Verificación de tipo | `$obj instanceof ClassName`| |
| `clone`| Clonación de objetos | `$copy = clone $obj`| La copia profunda requiere`__clone()`|
### Tipo malabarismo
```php
...
## Resumen
La sintaxis de PHP ha evolucionado dramáticamente desde sus orígenes de scripting hasta convertirse en un lenguaje moderno con seguridad de tipos. PHP 8.x introdujo tipos de unión, tipos de intersección, enumeraciones, fibras, clases de solo lectura, atributos e invocables de primera clase, transformándolo de un lenguaje de secuencias de comandos poco tipificado a uno que admite una rigurosa ingeniería de software. La fortaleza del lenguaje sigue siendo su enfoque web: con marcos como Laravel y Symfony, un ecosistema masivo y soporte de alojamiento casi universal, la sintaxis de PHP sirve para millones de aplicaciones. PHP moderno recompensa a los desarrolladores que adoptan su sistema de tipos, utilizan la carga automática basada en compositor y escriben código limpio y orientado a objetos.