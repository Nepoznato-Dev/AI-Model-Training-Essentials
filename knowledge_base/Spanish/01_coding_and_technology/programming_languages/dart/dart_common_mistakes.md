...
-->
# Dart: errores comunes y antipatrones
Este documento cataloga los errores, trampas y antipatrones más comunes en Dart con correcciones.
---

## 1. Abuso de seguridad nulo
```dart
// ❌ WRONG — force unwrap
...
## 2. Asíncrono/Espera sin manejo de errores
```dart
...
## 3. No utilizar constructores `const`
```dart
...
## 5. No utilizar Spread y Collection-if
```dart
...
## 6. Antipatrón: widget de estado masivo
```dart
...
## Resumen
La seguridad nula de Dart es poderosa: no la derrotes con `!`. Utilice constructores`const`para instancias canónicas, anule siempre`hashCode`con `==`, utilice operadores de colección-si y de extensión para crear listas concisas y extraiga la lógica empresarial de los widgets. Dart recompensa el código limpio, seguro contra nulos y con corrección constante.