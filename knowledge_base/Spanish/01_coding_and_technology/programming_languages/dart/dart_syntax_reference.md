...
-->
# Dart - Referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para Dart (3.x). Complementa la referencia principal de Dart al centrarse en patrones de sintaxis exhaustivos, seguridad nula, programación asíncrona y patrones de diseño orientados a Flutter.
---
...
|----------|------|---------|-------|
| `+``-``*``/``%``~/` | Aritmética | `7 ~/ 3`| `~/`es división entera |
| `==``!=` | Igualdad | `a == b`| |
| `<``>``<=``>=` | Comparación | `a >= b`| |
| `&&``\|\|``!`| Lógico | `a && b`| Cortocircuito |
| `??`| Coalición nula | `a ?? b`| `b`si`a`es nulo |
| `?.`| Acceso con reconocimiento de nulos | `a?.b`| nulo si`a`es nulo |
| `!`| Aseveración nula | `a!`| Lanza si es nulo |
| `..``?..` | Cascada | `obj..a..b`| Operaciones en cadena sobre el mismo objeto |
| `??=`| Asignación con reconocimiento de nulos | `a ??= b`| Asignar si es nulo |
| `as`| Tipo reparto | `obj as String`| Lanza sobre el fracaso |
| `is``is!` | Prueba de tipo | `obj is String`| |
| `=>`| Función de flecha | `() => expr`| |
### Notación en cascada
```dart
...
## Métodos y patrones de extensión
```dart
// Extension methods
...
## Resumen
La sintaxis de Dart es limpia, consistente y está diseñada para la productividad. La seguridad nula sólida elimina los errores de referencia nula en el momento de la compilación. Async/await y streams proporcionan patrones naturales para la programación asincrónica. Las clases, mixins y clases selladas permiten una composición flexible de objetos. La coincidencia de patrones, los registros y los modificadores de clase de Dart 3 añaden expresividad. Si bien el propósito principal de Dart es Flutter, su sintaxis es adecuada para cualquier dominio de aplicación: del lado del servidor (con Dart Frog o Shelf), herramientas CLI y aplicaciones web (compiladas en JavaScript).