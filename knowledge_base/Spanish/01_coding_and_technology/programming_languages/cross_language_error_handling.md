...
-->
# Comparación entre idiomas: manejo de errores
## Descripción general de los modelos de manejo de errores
| Modelo | Idiomas | Mecanismo |
|-------|-----------|-----------|
| **Excepciones (intentar/captar)** | Java, C++, C#, Python, Ruby, PHP, JavaScript, TypeScript, Kotlin, Swift, Dart, Scala, VB, Delphi | `try { } catch (e) { }`|
| **Tipos de resultados/opciones** | Rust, Haskell, OCaml, Scala (también) | `Result<T, E>`,`Maybe a`|
| **Múltiples valores devueltos** | Vamos, Lúa | `value, err := f()`|
| **Códigos de error** | C, Fortran | Código de error de retorno, verifique manualmente |
| **Errores de coincidencia de patrones** | Elixir, Erlang | `{:ok, value}`/`{:error, reason}`|
| **Condiciones (reiniciables)** | Ceceo común | `restart-case`,`handler-bind`|
| **Sin manejo de errores** | Scratch, SQL (básico) | N/A |
| **Contratos** | Ada, Eiffel |  `Pre`,`Post`condiciones |
...
## Resumen: paradigmas de manejo de errores
| paradigma | Enfoque | Ventajas | Contras |
|----------|----------|------|------|
| **Excepciones** | lanzar/atrapar | Limpie el camino feliz, apile los rastros | Se puede olvidar, controlar el abuso de flujo |
| **Tipos de resultados** | Volver`Ok`/`Err`| Explícito, no se pueden ignorar los errores | Detallado, se necesita el operador`?`|
| **Códigos de error** | Estado de retorno int | Sencillo, compatible con C | Fácil de ignorar, sin contexto |
| **Múltiples devoluciones** | `value, err := f()`| Explícito, idiomático | Puedo olvidarme de comprobar`err`|
| **Coincidencia de patrones** | `{:ok, v}`/`{:error, r}`| Exhaustivo, funcional | Detallado sin`with`|
| **Condiciones** | Manipuladores reiniciables | Más potente, recuperable | Complejo, rara vez comprendido |