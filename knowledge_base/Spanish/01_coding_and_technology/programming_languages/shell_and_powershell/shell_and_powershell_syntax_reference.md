...
## Bash: operadores y variables
### Operadores principales
| Operador | Nombre | Ejemplo | Notas |
|----------|------|---------|-------|
| `=`| Tarea | `x=10`| No hay espacios alrededor`=`|
| `$var`| Expansión variable | `echo $HOME`| |
| `${var}`| Expansión arriostrada | `${HOME}/docs`| Desambigua |
| `$(cmd)`| Sustitución de mando | `$(date +%Y)`| Preferido sobre las comillas invertidas |
| `$((expr))`| Aritmética | `$((2 + 3))`| |
| `${var:-default}`| Valor predeterminado | `${PORT:-8080}`| Si no está configurado o está vacío |
| `${var:=default}`| Asignar predeterminado | `${count:=0}`| |
| `${#var}`| Longitud de la cuerda | `${#name}`| |
| `${var%%pattern}`| Eliminar el sufijo más largo | `${file%%.*}`| Quitar extensión |
| `${var##pattern}`| Eliminar el prefijo más largo | `${path##*/}`| Obtener nombre de archivo |
| `"``"` | Comillas dobles | `"$var"`| Permite ampliación |
| `'``'` | Comillas simples | `'$var'`| Cadena literal |
...
## Bash — Funciones y canalizaciones
```bash
# Function definition
greet() {
    local name="${1:-World}"
...
## PowerShell: cmdlets y canalización
```powershell
...
## PowerShell: funciones y patrones avanzados
```powershell
# Basic function
...
## Resumen
Bash y PowerShell representan dos paradigmas de scripts de shell. Bash canaliza texto: cada comando transforma cadenas. PowerShell canaliza objetos: cada comando produce datos estructurados con propiedades y métodos. Bash domina Linux/macOS y DevOps. PowerShell es fundamental para la administración de Windows y cada vez más multiplataforma. Ambas son herramientas esenciales en una pila tecnológica moderna. Los scripts de Shell son el pegamento que conecta sistemas, automatiza los flujos de trabajo y hace las cosas.