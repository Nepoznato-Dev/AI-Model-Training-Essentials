...
# Java — Referencia de sintaxis
Este documento proporciona una referencia de sintaxis estructurada y completa para Java. Complementa la referencia principal de Java centrándose en patrones de sintaxis exhaustivos, tablas de operadores y mecánica interna de JVM, programación orientada a objetos y concurrencia.
---
...
| `/`| División | `7 / 2`| `3`| División entera para `int`; `3.5`para`double`|
| `%`| Resto | `7 % 2`| `1`| |
...
|----------|------|---------|-------|
| `==`| Igual | `x == y`| Igualdad de referencia para objetos |
| `!=`| No iguales | `x != y`| |
|  `<`, `>`, `<=`,`>=`| Relacional | `x >= y`| |
| `&&`| Y lógico | `a && b`| Cortocircuito |
| `\|\|`| O lógico | `a \|\| b`| Cortocircuito |
| `!`| NO lógico | `!true`| `false`|
| `instanceof`| Escriba cheque | `obj instanceof String`| También coincidencia de patrones (Java 16+) |
...
| Interfaz | Implementaciones | Ordenado | Variante segura para subprocesos |
|-----------|----------------|---------|---------------------|
| `List`| `ArrayList`,`LinkedList`| Sí | `CopyOnWriteArrayList`|
| `Set`|  `HashSet`, `TreeSet`,`LinkedHashSet`| Varía | `CopyOnWriteArraySet`|
| `Map`|  `HashMap`, `TreeMap`,`LinkedHashMap`| Varía | `ConcurrentHashMap`|
| `Queue`|  `ArrayDeque`,`PriorityQueue`| Varía | `ConcurrentLinkedQueue`|
| `Deque`| `ArrayDeque`| Ambos extremos | `ConcurrentLinkedDeque`|
### Listas
```java
// Creation
...
## Resumen
La sintaxis de Java ha evolucionado dramáticamente desde sus orígenes en 1995. Modern Java (21+) presenta registros, clases selladas, coincidencia de patrones, subprocesos virtuales y plantillas de cadenas, lo que lo hace más conciso y expresivo que nunca. La sintaxis principal sigue siendo familiar (tipo C, fuertemente tipada, basada en clases), pero las adiciones de lambdas, Stream API y`var`han reducido significativamente el texto estándar. La fortaleza del lenguaje radica en su ecosistema masivo, garantías de compatibilidad con versiones anteriores y optimizaciones del rendimiento de la JVM a través de la compilación JIT.