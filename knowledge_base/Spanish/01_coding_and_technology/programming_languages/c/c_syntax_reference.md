...
Este documento proporciona una referencia de sintaxis completa y estructurada para C (C11/C17 con notas C23). Complementa la referencia principal de C centrándose en patrones de sintaxis exhaustivos, tablas de operadores, mecánica de punteros y gestión de memoria.
---
...
| `/`| División | `7 / 2`| `3`| Truncamiento de enteros para tipos int |
| `%`| Módulo | `7 % 2`| `1`| Sólo para tipos enteros |
| `++`| Incremento | `i++`/`++i`| | Post-y pre-incremento |
| `--`| Decremento | `i--`/`--i`| | Post y pre-decremento |
...
| `\|`| O | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `~`| Complemento | `~0`| Todos los bits volteados |
| `<<`| Desplazamiento a la izquierda | `1 << 3`| `8`|
| `>>`| Desplazamiento a la derecha | `8 >> 3`| `1`(definido por implementación para firmado) |
### Operadores de puntero y dirección
| Operador | Nombre | Ejemplo | Notas |
|----------|------|---------|-------|
| `&`| Dirección de | `&x`| Devuelve el puntero a x |
| `*`| Desreferencia | `*ptr`| Valor de acceso en el puntero |
| `->`| Acceso de miembros mediante puntero | `p->x`| Equivalente a`(*p).x`|
| `.`| Acceso de miembros | `s.field`| Acceso directo a miembros de estructura |
| `[]`| Subíndice de matriz | `arr[i]`| Equivalente a`*(arr + i)`|
### Precedencia de operadores (de mayor a menor)
| Precedencia | Operadores | Asociatividad |
|------------|-----------|---------------|
| 1 (más alto) | `()``[]``->``.` | De izquierda a derecha |
| 2 | `!``~``+``-` (unario)`*``&``sizeof``(type)` | De derecha a izquierda |
...
| 13 | `? :`(ternario) | De derecha a izquierda |
| 14 | `=``+=``-=``*=``/=``%=``&=``\|=``^=``<<=``>>=`| De derecha a izquierda |
| 15 (más bajo) | `,`(coma) | De izquierda a derecha |
---
...
## Elementos básicos de la biblioteca estándar
```c
...
## Resumen
La sintaxis de C es mínima y cercana al hardware: punteros, administración manual de memoria y una delgada capa de abstracción sobre las instrucciones de la máquina. El poder del lenguaje proviene de esta simplicidad: casi no hay costos ocultos, ni sobrecarga de tiempo de ejecución y control total sobre el diseño de la memoria. El preprocesador, aunque potente, requiere disciplina para utilizarlo de forma segura. Modern C (C11/C17/C23) ha agregado características de seguridad importantes como `_Static_assert`, `_Generic`,`nullptr`y `typeof`, lo que hace que el lenguaje sea cada vez más seguro y al mismo tiempo mantiene su filosofía central de confiar en el programador.