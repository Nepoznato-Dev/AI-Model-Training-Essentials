...
Este documento cataloga los errores, trampas y antipatrones más comunes en C#. Cada entrada muestra el enfoque incorrecto, explica por qué falla y proporciona la solución correcta.
---

## 1. No desechar los objetos `IDisposable`
```csharp
// ❌ WRONG — resource leak
...
## 2.`==`frente a`Equals()`para cadenas
```csharp
...
## 3. Métodos de anulación asíncronos
```csharp
...
## 4. Asignación innecesaria de LINQ `.ToList()`
```csharp
...
## 5. Captura de variable de bucle (anterior a C# 5.0)
```csharp
...
## 6. No utilizar la coincidencia de patrones
```csharp
...
## 7. Antipatrón: Clases de Dios / Localizador de servicios
```csharp
...
## 8. Excepciones de referencia nula
```csharp
...
## 10. No utilizar registros para tipos de datos
```csharp
...
## 11. Abuso del filtro de excepción
```csharp
...
## Resumen
C# es un lenguaje bien diseñado, pero tiene trampas: no eliminar objetos `IDisposable`, métodos nulos asíncronos, asignaciones LINQ innecesarias y excepciones de referencia nula. Modern C# (8+) proporciona herramientas poderosas: tipos de referencia que aceptan valores NULL, coincidencia de patrones, registros, declaraciones de uso y expresiones de cambio. Los principios clave: desechar siempre los recursos, nunca usar async void (excepto controladores de eventos), usar inyección de constructor, preferir registros para datos y habilitar tipos de referencia anulables para detectar errores nulos en el momento de la compilación.