...
# Haskell — Referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para Haskell (GHC 9.x). Complementa la referencia principal de Haskell al centrarse en patrones de sintaxis exhaustivos, el sistema de tipos, mónadas, evaluación diferida y modismos de programación funcional.
---
...
|----------|------|---------|-------|
| `+``-``*``/` | Aritmética | `x + y`| Núm. clase de tipo |
| `div``mod` | División entera | `7 \`div\` 2`| Devuelve 3 |
| `^``**` | Poder | `2 ^ 10`| `^`para integrales; `**`para flotante |
| `==``/=` | Igualdad | `x == y`| Clase de tipo Eq |
| `<``>``<=``>=` | Comparación | `x >= y`| Clase de tipo de orden |
| `&&``\|\|``not`| Lógico | `a && b`| Cortocircuito |
| `.`| Composición de funciones | `(f . g) x`| `f (g x)`|
| `$`| Aplicación de funciones | `f $ x + 1`| `f (x + 1)`— precedencia baja |
| `<>`| Anexo monoide | `"hello" <> " world"`| Semigrupo |
| `<$>`| Funtor fmap | `(+1) <$> Just 5`| `fmap (+1) (Just 5)`|
| `<*>`| Aplicar aplicativo | `pure (+) <*> Just 3`| |
| `>>=`| Enlace de mónada | `Just 5 >>= \x -> Just (x+1)`| |
| `>>`| Secuencia de mónadas | `putStrLn "hi" >> putStrLn "bye"`| |
| `=<<`| Encuadernación (invertida) | `f =<< mx`| Igual que`mx >>= f`|
| `++`| Concatenación de listas | `[1,2] ++ [3,4]`| O(n) en la lista de la izquierda |
| `:`| Contras | `1 : [2, 3]`| O(1) |
| `!!`| Índice | `[1,2,3] !! 1`| O(n) — función parcial |
| `\\`| Diferencia de lista | `[1,2,3] \\ [2]`| `[1,3]`|
### Precedencia del operador (seleccionado)
...
## Resumen
La sintaxis de Haskell es mínima pero extraordinariamente expresiva. El sistema de tipos (con tipos de datos algebraicos, clases de tipos, GADT y familias de tipos) sirve como herramienta de especificación y verificación. La coincidencia de patrones, la evaluación diferida y las mónadas brindan soluciones elegantes a problemas complejos. La pureza de Haskell (sin efectos secundarios en los tipos) hace que el código sea más fácil de razonar, probar y paralelizar. Si bien la curva de aprendizaje es pronunciada, la influencia de Haskell impregna los lenguajes modernos, desde las opciones opcionales de Swift hasta las clases selladas de Kotlin y el sistema de tipos de Rust. Dominar Haskell te convierte en un mejor programador en todos los idiomas.