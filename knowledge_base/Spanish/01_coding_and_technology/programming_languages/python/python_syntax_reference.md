...
# Python — Referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para Python. Complementa la referencia principal de Python centrándose en patrones de sintaxis exhaustivos, tablas de operadores y mecánicas internas.
---
...
|----------|------|---------|--------|-------|
| `+`| Adición | `3 + 2`| `5`| También concatenación de cadenas/listas |
| `-`| Resta | `3 - 2`| `1`| También negación unaria |
| `*`| Multiplicación | `3 * 2`| `6`| También repetición de cadena/lista |
| `/`| Verdadera División | `7 / 2`| `3.5`| Siempre devuelve flotador |
| `//`| División de Piso | `7 // 2`| `3`| Redondea hacia el infinito negativo |
| `%`| Módulo | `7 % 2`| `1`| El signo coincide con el divisor |
| `**`| Exponenciación | `2 ** 10`| `1024`| Asociativo de derecha |
### Comparación y operadores booleanos
| Operador | Nombre | Ejemplo | Notas |
|----------|------|---------|-------|
| `==`| Igual | `x == y`| Comparación de valores (no identidad) |
| `!=`| No iguales | `x != y`| |
|  `<`, `>`, `<=`,`>=`| Realizar pedidos | `x >= y`| Encadenable:`a < b < c`|
| `is`| Identidad | `x is y`| Mismo objeto en memoria |
| `is not`| No identidad | `x is not None`| Preferido sobre`!= None`|
| `in`| Membresía | `x in collection`| O(1) para conjuntos/dictados, O(n) para listas |
| `and`| Y lógico | `a and b`| Cortocircuito; devuelve el último valor verdadero/falso |
| `or`| O lógico | `a or b`| Cortocircuito; devuelve el primer valor verdadero/falso |
| `not`| NO lógico | `not x`| |
...
| 8 | `^`| Izquierda |
| 9 | `\|`| Izquierda |
| 10 |  `==`, `!=`, `<`, `>`, `<=`, `>=`, `is`,`in`| Encadenable |
| 11 | `not`| Derecha |
| 12 | `and`| Izquierda |
| 13 (más bajo) | `or`| Izquierda |
### Asignación y Asignación aumentada
```python
...
| Colección | Ordenado | Mutable | Duplicados | Sintaxis | Búsqueda |
|------------|---------|---------|------------|--------|--------|
| `list`| Sí | Sí | Sí | `[1, 2, 3]`| O(n) |
| `tuple`| Sí | No | Sí | `(1, 2, 3)`| O(n) |
| `dict`| Sí (3,7+) | Sí | Claves: No | `{"k": "v"}`| O(1) promedio |
| `set`| No | Sí | No | `{1, 2, 3}`| O(1) promedio |
| `frozenset`| No | No | No | `frozenset({1,2})`| O(1) promedio |
### Listas
```python
# Creation
nums = [1, 2, 3, 4, 5]
...
## Resumen
Esta referencia de sintaxis cubre el conjunto completo de características de Python, desde operadores básicos y flujo de control hasta metaprogramación avanzada, descriptores y el sistema de tipos. La sintaxis de Python es deliberadamente mínima y legible, pero su profundidad es sustancial: la coincidencia de patrones, los generadores asíncronos, los protocolos y las metaclases proporcionan herramientas poderosas para aplicaciones complejas. El lenguaje continúa evolucionando y cada versión agrega características que mejoran la expresividad y al mismo tiempo mantienen la compatibilidad con versiones anteriores.