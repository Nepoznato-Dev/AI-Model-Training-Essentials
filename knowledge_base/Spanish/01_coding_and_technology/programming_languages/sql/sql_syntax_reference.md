...
# SQL — Referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para SQL (ANSI SQL con notas sobre las diferencias de dialecto PostgreSQL/MySQL/SQLite). Complementa la referencia principal de SQL centrándose en patrones de consulta exhaustivos, funciones de ventana, CTE y diseño de bases de datos.
---

## Sintaxis de consulta principal
### SELECCIONAR
```sql
-- Basic query
...
## Resumen
La sintaxis de SQL se ha mantenido estable durante 50 años mientras evoluciona continuamente. Las consultas principales (SELECT, JOIN, WHERE, GROUP BY) son estándar en todas las bases de datos. Las funciones de ventana, CTE y compatibilidad con JSON representan las capacidades del SQL moderno. El uso eficaz de SQL se beneficia al comprender cómo el motor de la base de datos procesa las consultas: leer planes de ejecución, comprender índices y escribir operaciones basadas en conjuntos en lugar de bucles de procedimientos. Las sólidas habilidades de SQL son valiosas para cualquiera que trabaje con datos.