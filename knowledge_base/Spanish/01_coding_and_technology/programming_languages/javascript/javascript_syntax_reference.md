...
-->
# JavaScript: referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para JavaScript. Complementa la referencia principal de JavaScript centrándose en patrones de sintaxis exhaustivos, tablas de operadores y mecánicas internas.
---
...
|----------|------|---------|--------|-------|
| `+`| Adición | `3 + 2`| `5`| También concatenación de cadenas |
| `-`| Resta | `3 - 2`| `1`| |
| `*`| Multiplicación | `3 * 2`| `6`| |
| `/`| División | `7 / 2`| `3.5`| |
| `%`| Resto | `7 % 2`| `1`| |
| `**`| Exponenciación | `2 ** 10`| `1024`| ES2016 |
| `++`| Incremento | `x++`/`++x`| | Post-y pre-incremento |
| `--`| Decremento | `x--`/`--x`| | Post y pre-decremento |
|  `+x`,`-x`| Unario | `+"42"`→`42`| Escriba coerción al número |
### Comparación e igualdad
| Operador | Nombre | Ejemplo | Notas |
|----------|------|---------|-------|
| `===`| Igualdad estricta | `1 === 1`| Ningún tipo de coerción: siempre prefiero esto |
| `!==`| Desigualdad estricta | `1 !== "1"`| `true`— diferentes tipos |
| `==`| Igualdad abstracta | `1 == "1"`| Tipo coerción - evitar |
| `!=`| Desigualdad abstracta | `1 != "1"`| `false`— evitar |
|  `<`, `>`, `<=`,`>=`| Relacional | `"b" > "a"`| Lexicográfico para cuerdas |
### Operadores lógicos y nulos
| Operador | Nombre | Ejemplo | Notas |
|----------|------|---------|-------|
| `&&`| Y lógico | `a && b`| Devuelve el primer valor falso o el último |
| `\|\|`| O lógico | `a \|\| b`| Devuelve el primer valor verdadero o último |
| `!`| NO lógico | `!true`| `false`|
| `??`| Coalescencia nula | `a ?? b`| Devuelve`b`solo si`a`es`null`/`undefined`|
| `?.`| Encadenamiento opcional | `obj?.prop?.method()`| Cortocircuitos a`undefined`|
...
| `>>`| Firmado desplazamiento a la derecha | `-5 >> 1`| `-3`|
| `>>>`| Desplazamiento a la derecha sin firmar | `-1 >>> 0`| `4294967295`|
### Asignación y desestructuración
```javascript
// Basic assignment
let x = 10;
...
|------------|-----------|---------------|
| 1 (más alto) | `()``[]``.``?.` | Izquierda |
| 2 | `new`(sin argumentos) | Derecha |
| 3 | `++``--` (postfijo) | N/A |
| 4 | `!``~``+x``-x``typeof``void``delete``++``--`(prefijo) | Derecha |
| 5 | `**`| Derecha |
| 6 | `*``/``%`| Izquierda |
| 7 | `+``-` | Izquierda |
| 8 | `<<``>>``>>>`| Izquierda |
| 9 | `<``<=``>``>=``in``instanceof` | Izquierda |
| 10 | `===``!==``==``!=` | Izquierda |
| 11 | `&`| Izquierda |
| 12 | `^`| Izquierda |
| 13 | `\|`| Izquierda |
...
| 16 | `??`| Izquierda |
| 17 | `=``+=``-=`etc. | Derecha |
| 18 (más bajo) | `=>`(función de flecha) | Derecha |
---
...
### Descripción general de las colecciones integradas
| Colección | Ordenado | Mutable | Duplicados | Tipo de clave | Búsqueda |
|------------|---------|---------|------------|----------|--------|
| `Array`| Sí | Sí | Sí | Índice (número) | O(n) |
| `Object`| Sí (inserción) | Sí | Claves: No | Cadena/Símbolo | O(1) promedio |
| `Map`| Sí (inserción) | Sí | Claves: No | Cualquiera | O(1) promedio |
| `Set`| Sí (inserción) | Sí | No | — | O(1) promedio |
| `WeakMap`| No | Sí | Claves: No | Sólo objeto | O(1) promedio |
| `WeakSet`| No | Sí | No | Sólo objeto | O(1) promedio |
### matrices
```javascript
// Creation
...
### Registro de referencia débil y finalización
```javascript
...
## Resumen
La sintaxis de JavaScript es engañosamente simple en la superficie pero contiene una profundidad significativa. El lenguaje ha evolucionado desde una simple herramienta de secuencias de comandos hasta un lenguaje multiparadigma con clases, módulos, generadores, servidores proxy y un sofisticado modelo asíncrono. Comprender la precedencia de los operadores, el bucle de eventos, las cadenas de prototipos y los matices del enlace`this`separa a los desarrolladores competentes de los expertos. JavaScript moderno (ES2020+) con encadenamiento opcional, fusión nula,`Promise.allSettled`y espera de nivel superior continúa reduciendo el texto repetitivo mientras mantiene la compatibilidad con versiones anteriores.