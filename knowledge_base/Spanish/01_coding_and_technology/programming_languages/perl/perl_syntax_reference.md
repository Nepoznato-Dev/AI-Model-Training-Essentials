...
---
# Perl — Referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para Perl (5.38+). Complementa la referencia principal de Perl al centrarse en patrones de sintaxis exhaustivos, expresiones regulares, referencias y el poder de procesamiento de texto de Perl.
---
...
| `x`| Repetición de cuerdas | `"ab" x 3`| `"ababab"`|
| `==``!=``<``>``<=``>=` | Comparación numérica | `$a == $b`| |
| `eq``ne``lt``gt``le``ge` | Comparación de cadenas | `$a eq $b`| |
| `<=>`| Nave espacial numérica | `$a <=> $b`| Devuelve -1, 0, 1 |
| `cmp`| Nave espacial de cuerdas | `$a cmp $b`| |
| `&&``\|\|``!`| Lógico | `$a && $b`| |
| `and``or``not`| Lógico (baja prec.) | `$a and $b`| Evite: utilice`&&`/`\|\|`|
| `=~``!~` | Coincidencia de expresiones regulares | `$str =~ /pattern/`| |
| `..``...` | Gama/chanclas | `1..10`| |
| `?:`| Ternario | `$a ? $b : $c`| |
| `//`| Definido-o | `$a // $b`| `$b`si`$a`no es definido |
| `=>`| Coma gruesa | `key => 'value'`| Autocomillas palabra desnuda |
### Sellos: tipos variables
```perl
...
## Resumen
La sintaxis de Perl se basa en el contexto, los sigilos y el principio de "Hay más de una manera de hacerlo". Las expresiones regulares están integradas en el lenguaje, lo que hace que el procesamiento de texto sea incomparable en cuanto a concisión. Las referencias permiten estructuras de datos complejas. CPAN proporciona módulos para todo. El Perl moderno (con `strict`,`warnings`y variables léxicas) es limpio y fácil de mantener. El legado de Perl sigue vivo en los patrones y herramientas que cada lenguaje moderno ha adoptado.