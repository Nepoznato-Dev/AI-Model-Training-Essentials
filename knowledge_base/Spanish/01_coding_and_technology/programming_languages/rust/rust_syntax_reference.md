...
-->
# Rust - Referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para Rust. Complementa la referencia principal de Rust centrándose en patrones de sintaxis exhaustivos, tablas de operadores y mecánica interna del sistema de propiedad, rasgos y concurrencia.
---
...
|----------|------|---------|-------|
| `+`| Adición | `3 + 2`| También concatenación`String`a través de`format!`|
| `-`| Resta | `3 - 2`| |
| `*`| Multiplicación | `3 * 2`| |
| `/`| División | `7 / 2`| División de números enteros para números enteros:`3`|
| `%`| Resto | `7 % 2`| `1`|
| `-x`| Negación | `-5`| |
### Comparación y operadores lógicos
| Operador | Nombre | Ejemplo | Notas |
|----------|------|---------|-------|
| `==`| Igual | `x == y`| Requiere`PartialEq`|
| `!=`| No iguales | `x != y`| |
|  `<`, `>`, `<=`,`>=`| Realizar pedidos | `x >= y`| Requiere`PartialOrd`|
| `&&`| Y lógico | `a && b`| Cortocircuito |
| `\|\|`| O lógico | `a \|\| b`| Cortocircuito |
| `!`| NO lógico | `!true`| |
...
| `\|`| O | `5 \| 3`| `7`|
| `^`| XOR | `5 ^ 3`| `6`|
| `!`| NO | `!0b1010u8`| Complemento bit a bit |
| `<<`| Desplazamiento a la izquierda | `5 << 1`| `10`|
| `>>`| Desplazamiento a la derecha | `5 >> 1`| `2`|
### Precedencia de operadores (de mayor a menor)
| Precedencia | Operadores | Notas |
|------------|-----------|-------|
| 1 (más alto) | `()`rutas`.`:: | |
| 2 | Llamadas a métodos, acceso a campos | Autoderef |
| 3 | `-`(negación), `!`,`*`(deref) | |
| 4 | `as`(tipo fundido) | |
| 5 | `*``/``%`| |
| 6 | `+``-` | |
...
| 14 | `..``..=` | Gama |
| 15 | `=``+=` etc. | Tarea |
| 16 (más bajo) | `return``break``continue`| |
---
...
### Descripción general de tipos integrados
| Tipo | Pila/Montón | Tamaño | Mutabilidad | Notas |
|------|-----------|------|------------|-------|
| Primitivas (`i32`, `f64`, `bool`, `char`) | Pila | Fijo | Inmutable por defecto | `Copy`|
| `[T; N]`(matriz) | Pila | Fijo | Mutable con`mut`| Tamaño fijo,`Copy`si T es`Copy`|
| `Vec<T>`| Montón | Dinámico | Mutable con`mut`| Cultivable |
| `String`| Montón | Dinámico | Mutable con`mut`| UTF-8, cultivable |
| `&[T]`(rebanada) | Pila (ptr gordo) | Vista dinámica | Depende de`&`o`&mut`| Vista prestada |
| `HashMap<K, V>`| Montón | Dinámico | Mutable con`mut`| O(1) búsqueda promedio |
| `BTreeMap<K, V>`| Montón | Dinámico | Mutable con`mut`| Búsqueda O (log n), ordenada |
| `HashSet<T>`| Montón | Dinámico | — | O(1) promedio, sin duplicados |
### Vectores
```rust
// Creation
...
## Rasgos y genéricos
### Definición e implementación de rasgos
```rust
// Define a trait
trait Summary {
...
### Rasgos comunes de la biblioteca estándar
```rust
...
## Manejo de errores
### Patrones de resultados y opciones
```rust
...
## Resumen
La sintaxis de Rust se basa en garantías de seguridad que se aplican en el momento de la compilación. El sistema de propiedad, el polimorfismo basado en rasgos, la coincidencia de patrones y las abstracciones de costo cero forman un diseño coherente en el que el compilador es un socio estricto pero justo. Desde el flujo de control básico hasta funciones avanzadas como macros, código inseguro y punteros inteligentes, cada construcción cumple el objetivo de una programación de sistemas segura y eficiente. Comprender la interacción entre vidas, préstamos y el sistema de tipos es la clave para dominar Rust.