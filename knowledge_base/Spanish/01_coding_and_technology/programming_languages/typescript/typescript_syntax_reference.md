...
### Tipos de utilidades integradas
```typescript
interface User {
  id: string;
  name: string;
...
### Tipos de utilidades personalizadas
```typescript
...
## Decoradores (TC39 Etapa 3 / TypeScript 5.0+)
```typescript
// Class decorator
...
### Afirmaciones constantes y reducción
```typescript
...
## Resumen
El sistema de tipos de TypeScript es su característica definitoria y es notablemente profundo. Desde anotaciones básicas hasta genéricos, tipos condicionales, tipos mapeados, tipos literales de plantilla y `infer`, el sistema de tipos es esencialmente un lenguaje funcional que opera en tiempo de compilación. Comprender la interacción entre`interface`y `type`, tipos genéricos y de utilidad, y tipificación estructural y nominal permite escribir TypeScript de forma segura y ergonómica. El lenguaje continúa evolucionando y cada versión agrega más poder expresivo al mismo tiempo que mantiene total compatibilidad con JavaScript.