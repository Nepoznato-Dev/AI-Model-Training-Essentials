...
# TypeScript: errores comunes y antipatrones
Este documento cataloga los errores, trampas y antipatrones más comunes en TypeScript. Cada entrada muestra el enfoque incorrecto, explica por qué falla y proporciona la solución correcta.
---
...
## 2. Afirmaciones de tipo en lugar de protecciones de tipo
```typescript
...
## 3. Abuso de afirmaciones no nulas
```typescript
...
## 4. Confusión de interfaces y tipos
```typescript
...
## 7. Sorpresas de tipificación estructural
```typescript
...
## 8. Errores de tipo asíncrono/en espera
```typescript
...
## 9. Antipatrón: tipos repetidos (sin usar genéricos)
```typescript
...
## 10. Encadenamiento opcional excesivo
```typescript
...
## 12. No aprovechar los sindicatos discriminados
```typescript
...
## Resumen
El sistema de tipos de TypeScript es poderoso pero requiere disciplina. El pecado capital es recurrir a `any`: utilizar`unknown`cuando el tipo sea realmente desconocido y limitarse a los tipos de protección. Evite afirmaciones que no sean nulas (`!`) manejando indefinidamente correctamente. Utilice interfaces para formas de objetos, tipos para uniones y tipos calculados. Los sindicatos discriminados eliminan clases enteras de errores. Habilite`strict: true`desde el primer día: el rigor de la modernización es doloroso. El objetivo es hacer que los estados ilegales sean irrepresentables a nivel de tipo.