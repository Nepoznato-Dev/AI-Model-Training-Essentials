...
---
# R — Referencia de sintaxis
Este documento proporciona una referencia de sintaxis completa y estructurada para R (4.x). Complementa la referencia principal de R centrándose en patrones de sintaxis exhaustivos, el ecosistema tidyverse, manipulación de datos, modelado estadístico y visualización.
---
...
|----------|------|---------|-------|
| `+``-``*``/``^`| Aritmética | `2^10`| |
| `%%`| Módulo | `7 %% 3`| Devuelve 1 |
| `%/%`| División entera | `7 %/% 3`| Devuelve 2 |
| `==``!=` | Igualdad | `x == y`| Vectorizado |
| `<``>``<=``>=` | Comparación | `x >= y`| Vectorizado |
| `&``\|``!`| Lógico (vectorizado) | `x & y`| Por elementos |
| `&&``\|\|` | Lógico (escalar) | `x && y`| Sólo primer elemento |
| `%in%`| Membresía | `x %in% y`| |
| `<-``=` | Tarea | `x <- 10`| `<-`es idiomático |
| `->`| Asignar derecho | `10 -> x`| Rara vez usado |
| `<<-`| Asignación global | `x <<- 10`| Asignar en el entorno principal |
| `$`| Extraer elemento | `df$column`| |
| `[``[[` | Indexación | `x[1]`/`x[[1]]`| `[`devuelve el mismo tipo;  Extractos`[[`|
| `%>%`| Tubería (magrittr) | `x %>% f()`| |
| `\|>`| Tubería (base R 4.1+) | `x \|> f()`| Tubería nativa |
...
## Resumen
La sintaxis de R está diseñada para computación estadística y análisis de datos. La naturaleza vectorizada de las operaciones elimina los bucles en la mayoría de las tareas. El ecosistema tidyverse (dplyr, ggplot2, tidyr) proporciona una gramática coherente y legible para la manipulación y visualización de datos. Las capacidades de modelado estadístico de R (desde pruebas t básicas hasta modelos de efectos mixtos) no tienen comparación en profundidad. Para la ciencia de datos, la estadística y la investigación reproducible, R sigue siendo una herramienta esencial.