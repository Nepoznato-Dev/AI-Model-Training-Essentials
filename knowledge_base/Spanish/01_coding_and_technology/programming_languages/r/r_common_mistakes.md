...
## 1. Crecimiento de marcos de datos en bucles
```r
# ❌ WRONG — O(n²) growth
result <- data.frame()
for (i in 1:1000) {
...
## 3. No entender el reciclaje de vectores
```r
...
## 5. No usar `stringsAsFactors = FALSE`
```r
...
## Resumen
La naturaleza interactiva de R crea trampas: marcos de datos crecientes en bucles (preasignados o vectorizados), sorpresas en el reciclaje de vectores, propagación de`NA`y`T`/`F`que se pueden sobrescribir. La forma R es: vectorizar operaciones, preasignar cuando se realiza un bucle, manejar`NA`explícitamente, usar tidyverse para la manipulación de datos y evitar `<<-`. R recompensa a quienes piensan en vectores y utilizan su rico ecosistema de paquetes.