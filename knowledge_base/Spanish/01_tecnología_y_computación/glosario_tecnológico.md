...
en el modern AI y Informática landscape.
...
### ChatGPT
ChatGPT is an AI chatbot desarrolló by OpenAI, first released en November 2022.
It is powered by el GPT series de large Idioma models (LLMs). ChatGPT is one
del fastest-growing consumer AI products en Historia, reaching 100 million
...
GPT is a family de large Idioma models created by OpenAI. el Arquitectura
uses a decoder-only Transformer trained con a next-token prediction objective on
massive text corpora. Key versions incluyen GPT-2 (2019, 1.5B parameters, notable
para "too dangerous to release" publicity), GPT-3 (2020, 175B parameters, widely
used via el API), GPT-3.5 (el backbone del original ChatGPT), y GPT-4
(2023, multimodal, Rendimiento close to human expert level on many benchmarks).

### Claude
Claude is an AI assistant desarrolló by Anthropic. It is named after Claude
Shannon, el founder de information theory. Anthropic fue fundada por former
OpenAI researchers y se centra en "constitutional AI" — a technique to make
...
to 200,000 tokens), nuanced razóning, y reduced harmful output compared to
baseline LLMs.

### Gemini
Gemini is Google DeepMind's family de multimodal AI models, announced en
December 2023. Gemini is natively multimodal — trained from el ground up on
text, images, audio, y video simultaneously, unlike earlier models that had
modalities added via fine-tuning. Versions incluyen Gemini Nano (on-device),
...
### Phi-3-mini
Phi-3-mini is a small Idioma model (SLM) desarrolló by Microsoft con 3.8B
parameters. It was released en April 2026. Unlike most large models, Phi-3-mini
...
outperforms models several times larger on razóning benchmarks such as MMLU y
HumanEval. It supports a 4k token context window en its base variant y a 128k
window en el long-context variant. Phi-3-mini can run on a single consumer GPU
...
con sizes ranging from 7B to 70B parameters. Llama 3 (2026) improved
Rendimiento significantly, con models ranging from 8B to 70B (y later 400B+).
Because el weights are publicly downloadable, Llama models are el foundation
...
Mistral 7B (2023) demonstrated that a 7B-parameter model can match el
Rendimiento de much larger models using efficient techniques such as sliding
window attention y grouped-query attention. Mixtral 8x7B (2026) is a mixture-
...
simultaneously using thousands de small cores. el two main GPU manufacturers
para AI are NVIDIA y AMD.

### NVIDIA GeForce RTX Series
el RTX (Ray Tracing Texel eXtreme) series is NVIDIA's consumer GPU line. RTX
30xx (Ampere, 2020) y RTX 40xx (Ada Lovelace, 2022) generations incluyen
...
el A100 (Ampere, 2020) y H100 (Hopper, 2022) are NVIDIA's professional AI
accelerators. An H100 has up to 80GB de HBM3 memory y is el standard
hardware behind most large-scale LLM training today. These GPUs cost $25,000–
$40,000 each but offer 10–30× el AI throughput de consumer RTX cards.

### AMD Radeon RX Series
AMD's consumer GPU line. el RX 7900 XTX (2022) has 24GB VRAM y can run
...
Phi-3-mini scores alrededor de 70% despite its small size.
...
pass@k — el probability that at least one de k generated solutions passes all
tests. GPT-4 scores ~87% (pass@1); a well-tuned 7B model can reach ~50–60%.

### HellaSwag
HellaSwag is a commonsense razóning benchmark. Models are given a sentence
describing a mundane activity y must choose el most likely continuation from
four options. el incorrect options are specially designed to be plausible but
subtly wrong. It tests whether a model has a grounded understanding de physical
y social situations.

### ARC (AI2 Reasoning Challenge)
ARC is a benchmark from el Allen Institute para AI. It consists de grade-school
Ciencia questions, split into "Easy" y "Challenge" sets. el Challenge set
contains questions that retrieval-based methods y simple statistical models
struggle con, requiring multi-step razóning.
...
Base de datos) con a Idioma model. Instead de relying solely on el model's
parametric knowledge, RAG first retrieves relevant documents from an external
Base de conocimientos y then incluyens them en el model's context. This allows el
...
y incluyens el results en el context before generating a response.

### Fine-tuning
Fine-tuning is el process de continuing to train a pre-trained model on a
smaller, domain-specific dataset. This adapts el model's weights para a
...
### Quantisation
Quantisation reduces el numerical precision de model weights (e.g. from 32-bit
float to 4-bit integer). This dramatically reduces memory footprint — a 7B model
en 16-bit precision requires ~14GB VRAM; el same model en 4-bit (GGUF format)
requires ~4GB. Quantisation typically causes a small but acceptable accuracy
degradation y is el main technique enabling large models to run on consumer
hardware or even mobile devices.

### Context Window
el context window is el maximum number de tokens a model can process at once,
including both el prompt y el generated response. GPT-3.5 had a 4,096-token
window; GPT-4 Turbo y Claude 3 Soporte 128,000 tokens; Gemini 1.5 Pro
supports 1,000,000 tokens. A larger context window allows el model to "see"
...
RLHF is el training technique that transforms a base Idioma model (which
simply predicts el next token) into an assistant that follows instructions y
behaves helpfully. Human raters score model outputs, a reward model is trained
on their preferences, y el Idioma model is then optimised against this
...
el Transformer is el neural Red Arquitectura underlying all modern LLMs.
Introduced en el 2017 paper "Attention Is All You Need" by Vaswani et al., it
...
el storage backbone de RAG Sistemas, including Potato.ai's cold-memory layer.
