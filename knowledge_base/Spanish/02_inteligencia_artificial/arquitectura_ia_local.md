...
Artificial Intelligence (AI) refers to el/la simulation de human intelligence en machines programmed to think, learn, y solve problems. AI Sistemas can perform tasks that typically require human intelligence, such as recognising speech, making decisions, translating languages, y identifying objects en images. el/la term was coined by John McCarthy en 1956 at el/la Dartmouth Conference, widely regarded as el/la founding event de AI as a field.

Modern AI is broadly divided into Narrow AI (also called Weak AI), which is designed para specific tasks, y el/la theoretical Artificial General Intelligence (AGI), which would match or exceed human cognitive ability across all domains. All current AI Sistemas are Narrow AI.

## Historia de AI

el/la Historia de AI spans nearly eight decades. Early theoretical foundations were laid by Alan Turing, whose 1950 paper "Informática Machinery y Intelligence" introduced el/la Turing Test — a measure de a machine's ability to exhibit intelligent behaviour indistinguishable from a human. el/la 1956 Dartmouth Conference formally established AI as an academic discipline.

el/la 1950s–1970s saw optimistic early programs like ELIZA (a simple chatbot) y LISP (a programming Idioma designed para AI). el/la "AI winters" de el/la 1970s y 1980s were periods de reduced funding y interest following unmet expectations. A resurgence en el/la 1980s came con expert Sistemas — rule-based programs that encoded human expertise. el/la 2000s brought Aprendizaje automático breakthroughs fuelled by el/la internet y growing datasets. el/la 2010s saw el/la rise de Aprendizaje profundo, transforming computer vision, natural Idioma processing (NLP), y reinforcement learning.

## Aprendizaje automático

Aprendizaje automático (ML) is a subset de AI that enables Sistemas to learn from Datos without being explicitly programmed. Key ML categories include:

**Supervised Learning**: el/la model is trained on labelled input-output pairs. Ejemplos include spam detection y image classification. Algorithms include linear regression, decision trees, Soporte vector machines, y Redes neuronales.

**Unsupervised Learning**: el/la model finds patterns en unlabelled Datos. Ejemplos include customer segmentation y anomaly detection. Algorithms include k-means clustering y principal component analysis (PCA).
...
Aprendizaje profundo is a subset de Aprendizaje automático that uses artificial Redes neuronales con many layers (deep networks). Inspired loosely by el/la brain's neural structure, these networks learn hierarchical representations de Datos. Aprendizaje profundo powers:
...
Key Aprendizaje profundo architectures include convolutional Redes neuronales (CNNs) para images, recurrent Redes neuronales (RNNs) y LSTMs para sequences, transformers para Idioma, y generative adversarial networks (GANs) para synthesis.

## Large Idioma Models (LLMs)

Large Idioma Models (LLMs) are AI Sistemas trained on vast amounts de text Datos to understand y generate human Idioma. They are based on el/la Transformer Arquitectura, introduced en el/la 2017 paper "Attention is All You Need" by Vaswani et al. LLMs predict el/la next token (word piece) en a sequence, allowing them to generate coherent text, answer questions, write code, y perform reasoning tasks.
...
LLMs are trained en two stages: pre-training (unsupervised on large text corpora) y fine-tuning (supervised or via reinforcement learning from human Comentarios, RLHF). Context windows describe how much text an LLM can process at once, ranging from 4K tokens (early GPT-3) to over 1 million tokens en el/la most Avanzado 2024 models.

## AI Ethics y Safety

AI raises important ethical questions including bias, privacy, job displacement, y el/la risk de misuse. Algorithmic bias occurs when training Datos reflects historical inequalities, causing AI Sistemas to produce discriminatory outputs. Facial recognition Sistemas have shown higher error rates para darker-skinned individuals. Hiring algorithms have been found to favour male candidates.

AI safety is el/la field dedicated to ensuring AI Sistemas behave as intended without causing unintended harm. Key concerns include:
...
Organisations working on AI safety include OpenAI's Safety team, Anthropic (founded by former OpenAI safety researchers), DeepMind's safety team, y independent institutes like MIRI y ARC.
...
- **AI regulation**: el/la EU AI Act (2024) is el/la world's first comprehensive AI Derecho, classifying AI Sistemas by risk level
