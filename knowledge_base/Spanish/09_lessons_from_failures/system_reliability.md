---
# Metadatos
título: "Problemas de confiabilidad del sistema"
descripción: "Fallos del sistema y patrones de confiabilidad"
...
# Clasificación
Etiquetas: [sistema, confiabilidad, lecciones de los fracasos]
...
Este documento consolida problemas de confiabilidad que incluyen pérdidas de memoria, condiciones de carrera, errores de programación concurrentes y fallas de diseño del sistema.
---

## Pérdidas de memoria
Las pérdidas de memoria ocurren cuando los programas asignan memoria pero no la liberan cuando ya no la necesitan, lo que provoca un crecimiento gradual del consumo de memoria.
### Oyentes de eventos olvidados
**Mal ejemplo (JavaScript):**```javascript
...
**Solución:** Utilice referencias débiles para referencias anteriores.```python
import weakref
...
Las condiciones de carrera ocurren cuando el comportamiento del software depende del tiempo relativo de los eventos, como el orden de ejecución de los subprocesos.
### Condición de carrera verificar y luego actuar
**Mal ejemplo (Python):**```python
...
### Condición de carrera de lectura-modificación-escritura
**Mal ejemplo:**```python
...
### Condición de carrera de inicialización diferida
**Mal ejemplo:**```python
...
**Prevención:** Adquiera siempre candados en un orden coherente```python
# Both threads:
lock_a.acquire()
lock_b.acquire()
# ... critical section ...
...
**Solución:** Utilice programación justa o antigüedad prioritaria
---
...
### Gestión de memoria
1. **Utilice RAII**: La adquisición de recursos es un patrón de inicialización
2. **Administradores de contexto**: use declaraciones`with`para recursos
3. **Referencias débiles**: para cachés y referencias anteriores
4. **Creación de perfiles**: creación de perfiles de memoria regulares en producción
### Simultaneidad
1. **Minimizar estado compartido**: prefiere pasar mensajes
2. **Usar abstracciones de alto nivel**: grupos de subprocesos, async/await
3. **Orden de bloqueo**: evite interbloqueos con un orden consistente
4. **Prueba simultáneamente**: utiliza pruebas de estrés y detectores de carrera
### Rendimiento
1. **Perfil primero**: Mida antes de optimizar
2. **Elección de algoritmo**: seleccione las estructuras de datos apropiadas
3. **Operaciones por lotes**: Reduzca los viajes de ida y vuelta a bases de datos/API
4. **Almacenamiento en caché**: almacene en caché los cálculos costosos de manera adecuada
---

## Temas relacionados
- **Vulnerabilidades de seguridad**: consulte`security_vulnerabilities.md`para problemas relacionados con la seguridad
- **Calidad del código**: Consulte`code_quality_issues.md`para conocer inquietudes sobre el mantenimiento
- **Diseño de API**: consulte`../07_api_system_design/api_system_design.md`para conocer los patrones de arquitectura del sistema.
- **Fallos de AI/LLM**: consulte`ai_llm_failures.md`para problemas de confiabilidad específicos de AI
---

## Patrones de confiabilidad del sistema adicionales
### Agotamiento de recursos
**Qué es:** Agotar los recursos del sistema (identificadores de archivos, conexiones, memoria) mediante una asignación ilimitada.
**Mal ejemplo:**```python
...
### Fallos en cascada
**Qué es:** La falla en un componente desencadena fallas en los componentes dependientes.
**Mal ejemplo:**```python
...
## Problemas de seguimiento y observabilidad
### Faltan controles de salud
**Mal ejemplo:**```python
...
### Problemas de desviación del reloj
**Mal ejemplo:**```python
...
**Qué probar:**
1. **Fallos de instancia**: elimine pods/VM aleatorios
2. **Problemas de red**: agregar latencia, descartar paquetes
3. **Agotamiento de recursos**: llenar el disco, agotar la memoria
4. **Fallos de dependencia**: simulacros de interrupciones del servicio
5. **Desviación del reloj**: desincronizar los relojes del servidor
**Ejemplo de experimento del caos:**```yaml
...
### Días de juego
Ejercicios regulares para probar la resiliencia del sistema:
1. **Horario**: Días de juego trimestrales
2. **Alcance**: entorno similar al de producción
3. **Escenarios**: múltiples fallas simultáneas
4. **Métricas**: mide el tiempo de detección y el tiempo de recuperación
5. **Aprendizajes**: documentar y mejorar los runbooks
---
...
### Residuos de aprovisionamiento excesivo
**Mal ejemplo:**```markdown
...
### Prevención del agotamiento durante las guardias
**Malas prácticas:**
- La misma persona de guardia todas las semanas.
- Sin camino de escalada
- Alertas de elementos no procesables
- Autopsias que asignan culpas
**Buenas Prácticas:**```markdown
On-Call Rotation:
...
**Buen libro de ejecución:**```markdown
## Database Slow Query
...
1. Identify slow queries:
   ```sql
SELECCIONAR * DE pg_stat_actividad 
   DONDE estado = 'activo' 
   ORDENAR POR query_start;   ```

2. Check for locks:
   ```sql
SELECCIONE * DESDE pg_locks DONDE concedido = falso;   ```
