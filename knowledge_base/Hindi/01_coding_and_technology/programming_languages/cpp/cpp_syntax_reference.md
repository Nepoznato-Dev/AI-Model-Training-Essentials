...
# सी++ - सिंटैक्स संदर्भ
यह दस्तावेज़ आधुनिक C++ (C++17/20/23) के लिए एक व्यापक, संरचित वाक्यविन्यास संदर्भ प्रदान करता है। यह संपूर्ण सिंटैक्स पैटर्न, ऑपरेटर टेबल, टेम्पलेट मैकेनिक्स और आधुनिक सुविधाओं पर ध्यान केंद्रित करके मुख्य C++ संदर्भ को पूरक करता है।
---

## ऑपरेटर्स और अभिव्यक्तियाँ
### अंकगणित एवं तुलना संचालक
| ऑपरेटर | नाम | उदाहरण | नोट्स |
|-------|------|------|-------|
| `+``-``*``/``%`| अंकगणित | `a + b`| सी के समान |
| `++``--` | वृद्धि/कमी | `++i`,`i++`| पूर्व-वृद्धि को प्राथमिकता दें |
| `==``!=``<``>` तुलना | `a <=> b`| `<=>`तीन-तरफ़ा तुलना है (C++20) |
| `&&``\|\|``!`| तार्किक | `a && b`| शॉर्ट-सर्किट |
| `&``\|``^``~``<<``>>` | बिटवाइज़ | `a & b`| सी के समान |
| `?:`| टर्नरी | `cond ? a : b`| |
### सी++-विशिष्ट ऑपरेटर
| ऑपरेटर | नाम | उदाहरण | नोट्स |
|-------|------|------|-------|
| `::`| दायरा संकल्प | `std::cout`| |
| `.*``->*` | सूचक-से-सदस्य | `obj.*pmf`| |
| `<=>`| अंतरिक्ष यान (सी++20) | `a <=> b`| रिटर्न`strong_ordering`|
| `co_await`| कोरूटाइन सस्पेंड | `co_await expr`| सी++20 |
| `co_yield`| कोरटाइन उपज | `co_yield value`| सी++20 |
| `co_return`| कोरटाइन वापसी | `co_return value`| सी++20 |
### ऑपरेटर ओवरलोडिंग
```cpp
class Vector2D {
    double x_, y_;
...
### संरचित बाइंडिंग और आधुनिक पैटर्न
```cpp
...
## फ़ंक्शंस और लैम्ब्डा
### आधुनिक फ़ंक्शन सिंटैक्स
```cpp
...
## टेम्पलेट और अवधारणाएँ
### टेम्पलेट बुनियादी बातें
```cpp
// Function template
template<typename T>
...
## एसटीएल कंटेनर और एल्गोरिदम
### कंटेनर अवलोकन
| कंटेनर | हेडर | आदेश दिया | लुकअप | नोट्स |
|--------|-------|------|--------|-------|
| `vector`| `<vector>`| हाँ | ओ(एन) | अनुक्रमों के लिए डिफ़ॉल्ट विकल्प |
| `deque`| `<deque>`| हाँ | ओ(एन) | दोनों सिरों पर तेज धक्का/पॉप |
| `list`| `<list>`| हाँ | ओ(एन) | दोगुनी लिंक्ड सूची |
| `array`| `<array>`| हाँ | ओ(एन) | निश्चित-आकार, स्टैक-आवंटित |
| `set`/`multiset`| `<set>`| हाँ | ओ(लॉग एन) | लाल-काला पेड़ |
| `map`/`multimap`| `<map>`| हाँ (कुंजी द्वारा) | ओ(लॉग एन) | कुंजी-मूल्य, लाल-काला पेड़ |
| `unordered_set`| `<unordered_set>`| नहीं | ओ(1) औसत | हैश तालिका |
| `unordered_map`| `<unordered_map>`| नहीं | ओ(1) औसत | हैश तालिका |
### प्रमुख एल्गोरिदम
```cpp
#include <algorithm>
#include <numeric>
...
## सारांश
C++ सिंटैक्स निम्न-स्तरीय पॉइंटर हेरफेर से लेकर उच्च-स्तरीय टेम्पलेट मेटाप्रोग्रामिंग और कॉरआउटिन-आधारित एसिंक तक फैला हुआ है। C++17/20/23 के माध्यम से भाषा के विकास में नाटकीय रूप से सुरक्षा (स्मार्ट पॉइंटर्स, `std::optional`, अवधारणाएं), अभिव्यंजना (संरचित बाइंडिंग, रेंज, `std::format`), और प्रदर्शन (शून्य-लागत अमूर्त को सक्षम करने वाली अवधारणाएं, स्टैक ओवरहेड के बिना एसिंक के लिए कॉरआउट) में सुधार हुआ है। मूल दर्शन अपरिवर्तित रहता है: आपको उस चीज़ के लिए भुगतान नहीं करना चाहिए जिसका आप उपयोग नहीं करते हैं, और भाषा को नंगे धातु से लेकर डोमेन-विशिष्ट भाषाओं तक अमूर्तता के हर स्तर का समर्थन करना चाहिए।