...
| **एन+1 क्वेरी समस्या** | ORM-जनरेटेड क्वेरीज़ बेहद अप्रभावी हो सकती हैं | जटिल प्रश्नों के लिए कस्टम SQL लिखें; व्याख्या विश्लेषण के साथ प्रोफ़ाइल |
...
| अंक | लक्षण | ठीक करें |
|-------|---------|-----|
| बड़ी मेज पर अनुक्रमिक स्कैन |  व्याख्या में`Seq Scan`| उचित अनुक्रमणिका जोड़ें |
...
| **3NF** | 2NF + कोई सकर्मक निर्भरता नहीं | कर्मचारी विभाग का नाम dept_id पर निर्भर करता है, कर्मचारी पर नहीं |
---
...
|----|---|----|
| **बी-ट्री** (डिफ़ॉल्ट) | समानता और सीमा प्रश्न | `WHERE age > 25 AND age < 35`|
| **हैश** | केवल सटीक समानता | `WHERE email = 'x@y.com'`|
| **जिन** | पूर्ण-पाठ खोज, सरणियाँ, JSON | `WHERE description @@ 'search term'`|
| **गिस्ट** | ज्यामितीय/स्थानिक डेटा | `WHERE location <-> point(x,y) < 1000`|
...
|---------|--------|-------|---|-------|
| स्वतः-वृद्धि | `BIGSERIAL`/`GENERATED ALWAYS`| `AUTO_INCREMENT`| `IDENTITY`| `INTEGER PRIMARY KEY AUTOINCREMENT`|
| स्ट्रिंग कॉनकैट | `\|\|`| `CONCAT()`| `+`या`CONCAT()`| `\|\|`|
| दिनांक कार्य | `NOW()`,`AGE()`| `NOW()`,`DATEDIFF()`| `GETDATE()`,`DATEDIFF()`| `DATE('now')`|
| JSON समर्थन | बहुत बढ़िया (`jsonb`) | अच्छा (`JSON`) | अच्छा (`JSON`) | बेसिक (`JSON1`) |
| पूर्ण-पाठ खोज | बिल्ट-इन (`tsvector`) | अंतर्निर्मित | अंतर्निर्मित | सीमित |
...
### Q1:`WHERE`और`HAVING`के बीच क्या अंतर है?
**ए:**`WHERE`समूहीकरण से पहले पंक्तियों को फ़िल्टर करता है;  एकत्रीकरण के बाद`HAVING`समूहों को फ़िल्टर करता है:
```sql
...
### Q2: विंडो फ़ंक्शन ग्रुप BY से किस प्रकार भिन्न हैं?
**ए:** विंडो फ़ंक्शंस पंक्तियों को ढहाए बिना गणना करते हैं:
```sql
...
### Q3: मैं धीमी क्वेरी को कैसे अनुकूलित करूं?
**ए:** मुख्य रणनीतियाँ:
-`WHERE`,`JOIN`, और`ORDER BY`में उपयोग किए गए कॉलम पर इंडेक्स जोड़ें 
-`SELECT *`से बचें - केवल आवश्यक कॉलम चुनें
- क्वेरी योजनाओं को पढ़ने के लिए`EXPLAIN`/`EXPLAIN ANALYZE`का उपयोग करें
- जहां संभव हो सबक्वेरी को जॉइन से बदलें
- पठनीयता के लिए सीटीई का उपयोग करें (आमतौर पर कोई प्रदर्शन दंड नहीं)
- WHERE में अनुक्रमित कॉलम पर फ़ंक्शंस से बचें:`WHERE date >= '2024-01-01'`का उपयोग करें न कि`WHERE YEAR(date) = 2024`का
### प्रश्न4: सीटीई क्या हैं और मुझे उनका उपयोग कब करना चाहिए?
**ए:** सामान्य तालिका अभिव्यक्तियाँ नामित अस्थायी परिणाम सेट बनाती हैं:
```sql
-- CTE for readability
...
### Q5: मैं NULL मानों को सही ढंग से कैसे प्रबंधित करूं?
**ए:** NULL अज्ञात का प्रतिनिधित्व करता है - यह स्वयं सहित किसी भी चीज़ के बराबर नहीं है:
```sql
-- NULL comparisons
...
### समस्या 1: प्रति समूह शीर्ष एन ढूँढना
**चरण 1: समस्या को समझें**
प्रत्येक विभाग में सबसे अधिक वेतन पाने वाले 3 कर्मचारियों का पता लगाएं।
**चरण 2: दृष्टिकोण को पहचानें**
विभाग द्वारा विभाजित`ROW_NUMBER()`के साथ एक विंडो फ़ंक्शन का उपयोग करें।
**चरण 3: कार्यान्वयन**```sql
WITH ranked AS (
    SELECT name, department, salary,
           ROW_NUMBER() OVER (
...
**चरण 4: सत्यापित करें**
जांचें कि प्रत्येक विभाग में अधिकतम 3 पंक्तियाँ हों। यदि आवश्यक हो तो`DENSE_RANK()`के साथ संबंधों को संभालें।
### समस्या 2: साल-दर-साल विकास रिपोर्ट बनाना
**चरण 1: समस्या को समझें**
मासिक राजस्व और साल-दर-साल वृद्धि प्रतिशत की गणना करें।
**चरण 2: दृष्टिकोण को पहचानें**
ग्रुपिंग के लिए`DATE_TRUNC`और पिछले वर्ष की तुलना के लिए`LAG()`विंडो फ़ंक्शन का उपयोग करें।
**चरण 3: कार्यान्वयन**```sql
WITH monthly AS (
    SELECT DATE_TRUNC('month', order_date) AS month,
...
**चरण 4: सत्यापित करें**
जांचें कि पहले 12 महीनों में पिछले वर्ष के लिए शून्य है। ज्ञात आंकड़ों के मुकाबले वृद्धि प्रतिशत को मान्य करें।
### समस्या 3: पंक्तियों को स्तंभों की ओर मोड़ना
**चरण 1: समस्या को समझें**
स्थिति गणना को पंक्तियों से स्तंभों में बदलें।
**चरण 2: दृष्टिकोण को पहचानें**
सशर्त एकत्रीकरण (`SUM` के अंदर `CASE`) का उपयोग करें।
**चरण 3: कार्यान्वयन**```sql
...
**चरण 4: विस्तार करें**
प्रतिशत कॉलम और चालू योग जोड़ें।
---
