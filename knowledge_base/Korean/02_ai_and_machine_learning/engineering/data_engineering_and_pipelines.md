...
| 접근 | 작동 원리 | 최고의 대상 | 도구 |
|----------|-------------|----------|-------|
| **ETL**(추출 → 변환 → 로드) | 창고에 로드하기 *전* 데이터 변환 | 컴퓨팅이 제한된 기존 창고 | 인포매티카, Talend, Apache NiFi |
...
| **차원** | 설명 속성 |  `products`(제품ID, 이름, 카테고리, 가격), `customers`(고객_ID, 이름, 도시) |
---
