...
- **제어**: 모델 버전, 사용자 정의 및 미세 조정을 완벽하게 제어할 수 있습니다.
---

## 하드웨어 요구 사항
### GPU 메모리(VRAM)
가장 중요한 자원입니다. 메모리의 모델 크기 ≒ **매개변수 × 매개변수당 바이트**.
...
- **Q4_K_M**(4비트): 대부분의 작업에 허용되는 가장 작은 품질입니다.
...
- 4비트 양자화에는 `bitsandbytes`을 사용합니다(`load_in_4bit=True`).
...
## 배포 아키텍처
### 단일 장치 모드
하나의 모델은 하나의 시스템(노트북, 스마트폰, 에지 장치)에서 실행됩니다. 개인 비서, 메모 작성 앱, 코드 완성에 사용됩니다.
### 하이브리드 엣지-클라우드
로컬 모델은 일반적인 쿼리를 처리합니다. 복잡한 질문에는 클라우드 모델로 대체합니다. 이는 대부분의 경우 속도/비공개, 극단적인 경우에 대한 기능이라는 두 가지 장점을 모두 제공합니다.
### 분산 추론(다중 GPU)
더 큰 모델의 경우 여러 GPU에 걸쳐 레이어를 분할하거나(텐서 병렬 처리) 여러 장치에 걸쳐 컨텍스트를 분할합니다(파이프라인 병렬 처리). `-ngl`과 함께 llama.cpp를 사용하거나 `--num-gpu-layers`과 함께 ExLlamaV2를 사용하세요.
### 모바일 배포
...
## 모니터링 및 관찰 가능성
- GPU 사용률을 추적합니다(Linux에서는 `nvidia-smi`, macOS에서는 Activity Monitor).
