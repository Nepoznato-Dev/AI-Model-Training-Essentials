...
---
# 다트 — 일반적인 실수 및 안티 패턴
이 문서에는 Dart에서 가장 흔히 발생하는 실수, 함정, 안티 패턴에 대한 수정 사항이 정리되어 있습니다.
---
...
## 요약
Dart의 null 안전성은 강력합니다. `!`로 이를 무너뜨리지 마세요. 정식 인스턴스에는`const`생성자를 사용하고, 항상 `hashCode`를 `==`로 재정의하고, 간결한 목록 작성을 위해 collection-if 및 스프레드 연산자를 사용하고, 위젯에서 비즈니스 로직을 추출하세요. Dart는 깨끗하고 null 안전하며 const가 올바른 코드를 보상합니다.