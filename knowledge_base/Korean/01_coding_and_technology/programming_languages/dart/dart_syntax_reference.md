...
-->
# Dart — 구문 참조
이 문서는 Dart(3.x)에 대한 포괄적이고 구조화된 구문 참조를 제공합니다. 이는 철저한 구문 패턴, null 안전, 비동기 프로그래밍 및 Flutter 지향 디자인 패턴에 중점을 두어 기본 Dart 참조를 보완합니다.
---
...
| `<``>``<=``>=` | 비교 | `a >= b`| |
| `&&``\|\|``!`| 논리적 | `a && b`| 단락 |
| `??`| Null 병합 | `a ?? b`|  `a`가 null인 경우`b`|
| `?.`| Null 인식 액세스 | `a?.b`| `a`가 null인 경우 null |
| `!`| 널 어설션 | `a!`| null인 경우 발생 |
| `..``?..` | 캐스케이드 | `obj..a..b`| 동일한 객체에 대한 체인 작업 |
| `??=`| Null 인식 할당 | `a ??= b`| null인 경우 할당 |
| `as`| 유형 캐스트 ​​| `obj as String`| 실패 시 발생 |
| `is``is!` | 유형 테스트 | `obj is String`| |
| `=>`| 화살표 기능 | `() => expr`| |
### 캐스케이드 표기법
```dart
...
## 요약
Dart의 구문은 깔끔하고 일관되며 생산성을 위해 설계되었습니다. 확실한 널 안전은 컴파일 타임에 널 참조 오류를 제거합니다. Async/await 및 스트림은 비동기 프로그래밍을 위한 자연스러운 패턴을 제공합니다. 클래스, 믹스인 및 봉인 클래스를 사용하면 유연한 개체 구성이 가능합니다. Dart 3의 패턴 일치, 레코드 및 클래스 수정자는 표현력을 추가합니다. Dart의 주요 목적은 Flutter이지만 해당 구문은 서버 측(Dart Frog 또는 Shelf 포함), CLI 도구, 웹 애플리케이션(JavaScript로 컴파일) 등 모든 애플리케이션 도메인에 매우 적합합니다.