...
## 요약
Clojure 관용구는 불변 데이터, 스레딩 매크로(`->`,`->>`), 구조화 해제, 함수로서의 키워드 및 검증 사양을 강조합니다. 일반적인 Lisp 관용구는 리소스의 경우 `with-open`, 목록 작성의 경우`push`/`nreverse`및 여러 값을 강조합니다. 둘 다 동질성과 REPL 기반 개발을 중요하게 생각합니다.