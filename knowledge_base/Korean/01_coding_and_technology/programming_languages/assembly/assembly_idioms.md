...
## 요약
어셈블리 관용구는 다음을 강조합니다. 매직 넘버에 대한 명명된 상수, 반복 패턴을 위한 매크로, 데이터 레이아웃을 위한 구조체 정의, ABI 호환 프롤로그/에필로그, 가독성을 위한 로컬 레이블, 방어 입력 검증 및 효율적인 작업을 위한`xor`/ `lea`. 어셈블리는 무엇보다 명확성을 보상합니다. "읽을 수 없으면 오전 3시에 디버깅할 수 없습니다."