...
## 요약
어셈블리 언어 구문은 프로세서 아키텍처와 어셈블러(NASM, GAS, MASM)에 의해 정의됩니다. x86-64는 16개의 범용 레지스터, 다중 주소 지정 모드 및 데이터 이동, 산술, 논리, 제어 흐름, SIMD 및 시스템 호출을 포괄하는 수백 개의 명령을 제공합니다. 여기에 사용된 NASM 구문은 Linux x86-64 개발에 가장 일반적입니다. 어셈블리를 마스터하려면 레지스터, 스택, 호출 규칙 및 상위 수준 구성이 기계 명령어에 매핑되는 방식을 이해해야 합니다.