...
| `&&``\|\|``!`| 논리적 | `a && b`| 단락 |
| `&``\|``⊻``~` | 비트별 | `a & b`|  `⊻`는 XOR입니다 |
| `:`| 범위 | `1:10`| `start:step:stop`|
| `..`| 간격 | `1..10`| IntervalSets 필요 |
| `∈``∉``⊆`| 회원 설정 | `x ∈ S`| `in(x, S)`|
| `∘`| 구성 | `f ∘ g`| `(f ∘ g)(x) == f(g(x))`|
| `\|>`| 파이프 | `x \|> f`| `f(x)`|
| `=>`| 쌍 | `a => b`| 딕셔너리의 경우 |
...
## 요약
Julia의 구문은 깨끗하고 수학적이며 표현력이 뛰어납니다. 다중 디스패치는 함수가 모든 인수 유형에 따라 다르게 동작하도록 하여 우아한 코드 구성을 가능하게 합니다. 매크로는 C 스타일 전처리기 매크로의 복잡성 없이 강력한 메타프로그래밍을 제공합니다. 파라메트릭 유형, 추상 유형 및 공용체 유형을 포함하는 유형 시스템은 유연성과 성능을 모두 지원합니다. Julia의 1 기반 인덱싱, 이해 및 do-block 구문은 수학 및 과학 컴퓨팅에 자연스럽게 적용됩니다. 이 언어는 유형 추론을 통한 JIT 컴파일을 통해 "Python처럼 걷고, C처럼 실행합니다"라는 약속을 달성합니다.