...
## 5. `stringsAsFactors = FALSE`를 사용하지 않음
```r
...
## 요약
R의 대화형 특성은 함정을 만듭니다. 루프에서 데이터 프레임 증가(사전 할당 또는 벡터화), 벡터 재활용 놀라움,`NA`전파 및`T`/`F`덮어쓰기 가능. R 방식은 작업을 벡터화하고, 루프 시 사전 할당하고, `NA`를 명시적으로 처리하고, 데이터 조작에 tidyverse를 사용하고, `<<-`를 방지하는 것입니다. R은 벡터로 생각하고 풍부한 패키지 생태계를 사용하는 사람들에게 보상합니다.