...
-->
# C — 일반적인 실수 및 안티 패턴
이 문서는 C에서 가장 흔히 발생하는 실수, 함정 및 안티 패턴을 나열합니다. 각 항목은 잘못된 접근 방식을 보여주고 실패 이유를 설명하며 올바른 솔루션을 제공합니다. C는 완전한 제어권을 제공합니다. 이는 메모리 안전, 경계 확인 및 정의되지 않은 동작에 대한 모든 책임을 의미합니다.
---

## 1.`gets()`/ `strcpy()`의 버퍼 오버플로
```c
...
## 요약
C의 위험은 잘 알려져 있지만 항상 존재합니다: 버퍼 오버플로, 매달린 포인터, 정수 오버플로, 형식 문자열 공격 및 메모리 누수. 규칙은 다음과 같습니다: 항상 반환 값을 확인하고, 항상 할당한 것을 해제하고, 항상 제한된 함수(`snprintf`,`strncpy`,`fgets`)를 사용하고,`gets()`를 사용하지 말고, 모든 변수를 초기화하고, Valgrind 및 AddressSanitizer와 같은 도구를 사용하세요. C는 세심한 프로그래머에게 비교할 수 없는 성능과 제어력을 제공하지만 부주의함은 정의되지 않은 동작과 보안 취약점으로 처벌합니다.