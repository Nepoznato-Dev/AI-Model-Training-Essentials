...
-->
# COBOL — 구문 참조
이 문서는 COBOL(2014+)에 대한 포괄적이고 구조화된 구문 참조를 제공합니다. 이는 철저한 구문 패턴, 데이터 설명, 파일 처리 및 비즈니스 논리에 중점을 두어 기본 COBOL 참조를 보완합니다.
---
...
## 요약
COBOL의 구문은 설계상 장황합니다. 모든 명령문은 영어처럼 읽혀집니다. PIC 절이 포함된 데이터 설명은 필드 형식을 정확하게 정의합니다. 4개 부서 구조는 업무 분리를 강제합니다. 파일 처리는 언어에 내장되어 있습니다. 비즈니스 데이터 처리의 경우 COBOL의 십진수 산술 및 레코드 지향 I/O는 타의 추종을 불허합니다. COBOL을 이해한다는 것은 엔터프라이즈 컴퓨팅의 기초를 이해한다는 것을 의미합니다.