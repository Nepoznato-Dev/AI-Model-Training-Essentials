...
## 요약
Go 관용어는 단순성, 명시적인 오류 처리, 작은 인터페이스, 상속에 대한 구성, 컨텍스트가 있는 고루틴, "인터페이스 수락, 구조체 반환"을 강조합니다. 서식 지정은 `gofmt`, 린팅은 `golangci-lint`, 공식 Go Code Review Comments 가이드를 따르세요. Go 커뮤니티는 가독성, 단순성 및 "명확한 방법"을 중요하게 생각합니다. 영리한 코드를 피하세요. Go 코드는 지루하고 예측 가능해야 합니다.