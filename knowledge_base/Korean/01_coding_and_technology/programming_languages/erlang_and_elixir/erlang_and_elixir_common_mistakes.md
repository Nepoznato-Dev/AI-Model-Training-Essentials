...
# Erlang & Elixir — 일반적인 실수 및 안티 패턴
이 문서는 Erlang과 Elixir의 가장 일반적인 실수, 함정, 안티 패턴을 수정하여 목록화합니다.
---
...
## 3. 시간 제한 없이 GenServer 호출(Elixir)
```elixir
...
## 4. 시간 초과 없이 `receive`에서 패턴 일치(Erlang)
```erlang
...
## 5. 중첩 패턴 일치에 `with`를 사용하지 않음(Elixir)
```elixir
...
## 요약
Erlang/Elixir 트랩: 변수는 단일 할당이고 프로세스에는 링크/모니터가 필요하며 GenServer 호출에는 명시적인 시간 초과가 필요하고 `receive`는 시간 초과 없이 영원히 차단되며 GenServer 콜백은 차단되어서는 안 됩니다. BEAM 방식은 다음과 같습니다. (감독을 통해) 충돌을 일으키고, 깔끔한 패턴 일치를 위해 `with`를 사용하고, GenServer 콜백을 빠르게 유지하고, 항상 프로세스 실패를 처리합니다.