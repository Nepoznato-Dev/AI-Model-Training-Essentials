...
---
# Rust — 구문 참조
이 문서는 Rust에 대한 포괄적이고 구조화된 구문 참조를 제공합니다. 이는 철저한 구문 패턴, 연산자 테이블, 소유권 시스템의 내부 메커니즘, 특성 및 동시성에 중점을 두어 주요 Rust 참조를 보완합니다.
---
...
| 운영자 | 이름 | 예 | 메모 |
|------------|------|---------|-------|
| `+`| 추가 | `3 + 2`| 또한 `format!`를 통한`String`연결 |
| `-`| 빼기 | `3 - 2`| |
| `*`| 곱셈 | `3 * 2`| |
| `/`| 부문 | `7 / 2`| 정수에 대한 정수 나누기:`3`|
| `%`| 나머지 | `7 % 2`| `1`|
| `-x`| 부정 | `-5`| |
### 비교 및 ​​논리 연산자
| 운영자 | 이름 | 예 | 메모 |
...
| 유형 | 스택/힙 | 사이즈 | 가변성 | 메모 |
|------|------------|------|------------|-------|
| 기본 요소(`i32`,`f64`,`bool`,`char`) | 스택 | 고정 | 기본적으로 불변 | `Copy`|
|  `[T; N]`(어레이) | 스택 | 고정 | `mut`로 변경 가능 | 고정 크기, T가 `Copy`인 경우`Copy`|
| `Vec<T>`| 힙 | 동적 | `mut`로 변경 가능 | 재배 가능 |
| `String`| 힙 | 동적 | `mut`로 변경 가능 | UTF-8, 확장 가능 |
|  `&[T]`(슬라이스) | 스택(지방 PTR) | 동적보기 |`&`또는 `&mut`에 따라 다름 | 빌린 보기 |
| `HashMap<K, V>`| 힙 | 동적 | `mut`로 변경 가능 | O(1) 평균 조회 |
| `BTreeMap<K, V>`| 힙 | 동적 | `mut`로 변경 가능 | O(log n) 조회, 순서 |
| `HashSet<T>`| 힙 | 동적 | — | 평균 O(1)개, 중복 없음 |
### 벡터
```rust
// Creation
...
## 요약
Rust의 구문은 컴파일 타임에 시행되는 안전 보장을 기반으로 구축되었습니다. 소유권 시스템, 특성 기반 다형성, 패턴 일치 및 비용이 들지 않는 추상화는 컴파일러가 엄격하지만 공정한 파트너인 일관된 디자인을 형성합니다. 기본 제어 흐름부터 매크로, 안전하지 않은 코드, 스마트 포인터와 같은 고급 기능까지 모든 구성은 안전하고 효율적인 시스템 프로그래밍의 목표를 달성합니다. 수명, 차용 및 유형 시스템 간의 상호 작용을 이해하는 것이 Rust를 마스터하는 열쇠입니다.