...
-->
# C# — 일반적인 실수 및 안티 패턴
이 문서에는 C#에서 가장 일반적인 실수, 함정 및 안티패턴이 나열되어 있습니다. 각 항목은 잘못된 접근 방식을 보여주고, 실패 이유를 설명하며, 올바른 솔루션을 제공합니다.
---

## 1.`IDisposable`객체를 폐기하지 않음
```csharp
// ❌ WRONG — resource leak
...
## 요약
C#은 잘 설계된 언어이지만`IDisposable`개체를 처리하지 않음, 비동기 void 메서드, 불필요한 LINQ 할당 및 null 참조 예외 등의 함정이 있습니다. 최신 C#(8+)은 null 허용 참조 유형, 패턴 일치, 레코드, 선언 사용, 스위치 식 등 강력한 도구를 제공합니다. 핵심 원칙은 항상 리소스를 삭제하고, async void를 절대 사용하지 않고(이벤트 핸들러 제외), 생성자 주입을 사용하고, 데이터에 대한 레코드를 선호하고, 컴파일 타임에 null 버그를 잡을 수 있도록 null 허용 참조 유형을 활성화하는 것입니다.