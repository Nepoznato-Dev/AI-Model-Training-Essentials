...
-->
# Fortran — 구문 참조
이 문서는 Modern Fortran(2008/2018)에 대한 포괄적이고 구조화된 구문 참조를 제공합니다. 이는 철저한 구문 패턴, 배열 작업, 모듈, OOP 및 고성능 컴퓨팅에 중점을 두어 주요 Fortran 참조를 보완합니다.
---
...
| `==``/=` | 평등 | `a == b`|  `/=`는 "같지 않음" |
| `<``>``<=``>=` | 비교 | `a >= b`| |
| `.and.``.or.``.not.`| 논리적 | `a .and. b`| |
| `.eqv.``.neqv.` | 논리적 동등성 | `a .eqv. b`| |
| `//`| 문자열 연결 | `'hello' // ' world'`| |
---
...
## 요약
최신 Fortran의 구문은 깔끔하고 구조적이며 수치 계산에 최적화되어 있습니다. 배열 연산은 대부분의 수학 작업에서 루프를 제거합니다. 모듈은 캡슐화 및 네임스페이스 관리를 제공합니다. OOP 기능(파생 유형, 상속, 다형성)을 통해 깔끔한 코드 구성이 가능합니다. Coarrays는 외부 라이브러리 없이 기본 병렬 프로그래밍을 제공합니다. 성능, 배열 작업 및 수치 정밀도와 같은 Fortran의 강점은 과학 컴퓨팅 분야에서 타의 추종을 불허합니다. HPC, 계산 물리학 및 엔지니어링 시뮬레이션의 경우 Fortran은 계속해서 강력한 성능을 발휘하고 있습니다.