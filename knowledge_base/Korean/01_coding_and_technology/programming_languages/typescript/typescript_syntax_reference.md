...
## 데코레이터(TC39 Stage 3 / TypeScript 5.0+)
```typescript
// Class decorator
...
## 요약
TypeScript의 유형 시스템은 TypeScript를 정의하는 기능이며 특히 심층적입니다. 기본 주석부터 제네릭, 조건부 유형, 매핑된 유형, 템플릿 리터럴 유형 및 `infer`까지 유형 시스템은 본질적으로 컴파일 타임에 작동하는 기능적 언어입니다. `interface`와 `type`, 제네릭과 유틸리티 유형, 구조적 유형과 명목형 유형 간의 상호 작용을 이해하면 안전하고 인체공학적인 TypeScript 작성이 지원됩니다. 언어는 계속해서 발전하고 있으며 각 릴리스마다 JavaScript와의 완전한 하위 호환성을 유지하면서 더 많은 표현력을 추가하고 있습니다.