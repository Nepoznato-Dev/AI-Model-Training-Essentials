...
---
# Swift — 일반적인 실수 및 안티 패턴
이 문서는 Swift에서 가장 흔히 발생하는 실수, 함정, 안티패턴을 나열합니다. 각 항목은 잘못된 접근 방식을 보여주고, 실패 이유를 설명하며, 올바른 솔루션을 제공합니다.
---
...
## 요약
Swift의 안전 기능은 많은 버그를 방지하지만 자체 함정을 도입합니다. 강제 언래핑은 충돌을 일으키고, 클로저와 대리자에 사이클을 숨기고, 값 유형 의미론은 참조 유형 언어에서 오는 개발자를 놀라게 합니다. Swift 방식은 강제로 풀지 않음(`!`), 조기 종료에`guard`사용, 대리자 및 클로저 캡처에`weak`사용,`@MainActor`로 UI 코드 표시, 대규모 뷰 컨트롤러를 뷰 모델 및 서비스로 분할하는 것입니다. Swift의 유형 시스템은 당신의 친구입니다. 그것에 반대하지 말고 함께 작업하세요.