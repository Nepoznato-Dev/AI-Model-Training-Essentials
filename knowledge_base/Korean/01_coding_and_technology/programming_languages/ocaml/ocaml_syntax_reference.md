...
-->
# OCaml — 구문 참조
이 문서는 OCaml(4.x/5.x)에 대한 포괄적이고 구조화된 구문 참조를 제공합니다. 이는 철저한 구문 패턴, 대수 데이터 유형, 패턴 일치, 모듈 및 함수형 프로그래밍 관용구에 중점을 두어 기본 OCaml 참조를 보완합니다.
---
...
## 요약
OCaml의 구문은 깔끔하고 수학적입니다. 대수적 데이터 유형과 철저한 패턴 일치는 불가능한 상태를 표현할 수 없게 만듭니다. 펑터가 있는 모듈 시스템은 런타임 비용 없이 강력한 추상화를 제공합니다. 유형 추론은 장황하지 않은 안전성을 의미합니다. Rust, F#, TypeScript 및 Swift에 대한 OCaml의 영향은 부인할 수 없습니다. 시스템 프로그래밍, 컴파일러 및 공식 검증의 경우 OCaml이 여전히 최고의 선택입니다.