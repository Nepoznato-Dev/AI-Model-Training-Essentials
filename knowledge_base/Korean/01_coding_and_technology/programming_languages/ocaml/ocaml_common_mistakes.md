...
## 요약
OCaml 트랩: 비완전한 패턴 일치(컴파일러 경고), `=`는 구조적 동일성인 반면 `==`는 물리적이며 정수 나누기 잘림, 예외 대신 `Option`를 사용하고 꼬리 재귀 및 접기를 선호하여 변경 가능한 상태를 피합니다. OCaml의 컴파일러는 여러분의 동맹입니다. 경고에 귀를 기울이세요.