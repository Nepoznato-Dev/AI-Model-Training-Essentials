...
# Haskell — 일반적인 실수와 안티 패턴
이 문서는 Haskell에서 가장 흔히 발생하는 실수, 함정, 안티 패턴을 수정하여 목록화합니다.
---
...
## 요약
Haskell의 순수성과 게으름은 독특한 함정을 만듭니다. 지연 평가로 인한 공간 누수, 런타임에 충돌하는 부분 함수, 문자열/텍스트/바이트 문자열 간의 혼란, IO의 남용 등이 있습니다. Haskell 방식은 누산기(`foldl'`, BangPatterns)에 대해 엄격한 평가를 사용하고, 부분 함수(`head`,`read`,`!!`)를 피하고, 오류 처리에`Maybe`/`Either`를 사용하고, 텍스트에`Text`를 사용하고, IO를 최소화하는 것입니다. 컴파일러의 유형 시스템은 여러분의 가장 큰 동맹입니다. 전체적이고 순수한 함수를 향해 여러분을 안내해 드립니다.