...
---
# Haskell — 관용적 패턴 및 모범 사례
이 가이드는 깔끔하고 관용적인 Haskell 코드를 작성하기 위한 관용적 패턴과 모범 사례를 다룹니다.
---
...
## 요약
Haskell 관용어는 강력한 유형, 패턴 일치, 효과를 위한 모나드, 순수 함수 및 지연 평가(필요할 경우 엄격하게)를 강조합니다. Haskell 스타일 가이드를 따르고 제안에는 `hlint`를 사용하고 서식 지정에는 `fourmolu`를 사용하세요. Haskell은 정확성과 구성을 중요하게 생각합니다. "불법적인 상태를 표현할 수 없도록 만듭니다."