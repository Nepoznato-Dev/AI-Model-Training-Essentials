...
| | `<=`| 작거나 같음 | `a <= b`|
| | `>=`| 크거나 같음 | `a >= b`|
| **논리적** | `AndAlso`| 단락 AND | `a AndAlso b`|
| | `OrElse`| 단락 또는 | `a OrElse b`|
| | `Not`| 논리적 NOT | `Not a`|
| | `And`| 비트 AND | `a And b`|
| | `Or`| 비트 OR | `a Or b`|
| | `Xor`| 비트별 XOR | `a Xor b`|
...
| | `+=`| 추가 | `s &= " more"`|
| **과제** | `=`| 할당 | `x = 42`|
| **널** | `Is`| 참조 평등 | `obj Is Nothing`|
| | `IsNot`| 기준 불평등 | `obj IsNot Nothing`|
| **유형** | `TryCast`| 안전한 캐스트(아무것도 반환하지 않음) | `TryCast(obj, String)`|
| | `CType`| 변환 | `CType(obj, Integer)`|
| | `DirectCast`| 직접 시전(실패 시 던지기) | `DirectCast(obj, String)`|
---
...
## 요약
Visual Basic의 구문은 깔끔하고 영어와 유사하여 가장 읽기 쉬운 프로그래밍 언어 중 하나입니다. VB.NET은 데이터 쿼리를 위한 LINQ, 동시 작업을 위한 async/await, 유형 안전성을 위한 제네릭, 클래스, 인터페이스 및 상속이 포함된 포괄적인 OOP를 포함하여 .NET 생태계에 대한 전체 액세스를 제공합니다. 언어의 대소문자 구분 및 자세한 키워드(End If, End Sub, End Class)는 가독성을 향상시킵니다. 엔터프라이즈 Windows 응용 프로그램, Office 자동화(VBA) 및 .NET 내 신속한 프로토타이핑의 경우 VB.NET은 여전히 ​​생산적이고 유능한 언어입니다.