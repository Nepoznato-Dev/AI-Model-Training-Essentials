...
-->
# Visual Basic — 일반적인 실수 및 안티 패턴
이 문서에는 Visual Basic(VB.NET)의 가장 일반적인 실수, 함정 및 안티 패턴에 대한 수정 사항이 정리되어 있습니다.
---
...
## 요약
VB.NET 트랩: 일회용 리소스에는 항상 `Using`를 사용하고, `=`(값)를 `Is`(참조)와 구별하고, `Option Strict On`를 활성화하고, 데이터베이스 연결에`Using`블록을 사용하고, UI 이벤트 핸들러에서 비즈니스 로직을 분리합니다. 최신 VB.NET은 강력한 .NET 언어입니다. 해당 기능을 적절하게 사용하십시오.