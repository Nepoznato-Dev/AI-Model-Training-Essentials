...
Quick 참조 Python 3.x 구문 common patterns.

---

# # Basic 구문

# ## Variables 데이 터 Types
```python
...
- Use `#` sle-le comments
- Use `"""triple quotes"""` docstrs multi-le strs
- Indentation matters (typically 4 spaces)
- Nam conventions: `snake_case` variables/functions, `PascalCase` classes
- `__name__ == "__ma__"` to check if script is run directly
- Use `virtualenv` or `venv` project isolation
- Install packages 함께 `pip stall package_name`
