...
A practical 참조 developers sysadms — core concepts, protocols, comms, troubleshoot.

---

# # The OSI Model (7 Layers)

A conceptual framework underst 네트워크 사소통.

| Layer | Name | Function | Example protocols |
|-------|------|----------|-------------------|
| 7 | Application | End-user services | HTTP, HTTPS, FTP, SMTP, DNS, SSH |
| 6 | Presentation | 데이 터 matt, encryption, compression | TLS, JPEG, ASCII |
| 5 | Session | Connection 관리 | NetBIOS, RPC |
| 4 | Transport | End-to-end delivery, error correction, flow control | TCP, UDP |
| 3 | 네트워크 | Rout, address | IP, ICMP, OSPF, BGP |
| 2 | 데이 터 Lk | Fram, error detection, MAC addresses | Ernet, Wi-Fi, PPP |
...
- 합계: ~4.3 billion addresses (but exhausted practice).

# ## IPv6
- 128-bit address, written hex: `2001:0db8:85a3:0000:0000:8a2e:0370:7334`
- 합계: 2¹²⁸ addresses (practically fite).

# ## Private IP Ranges (RFC 1918)
These are not routable on ternet; used side local 네트워크s:
- `10.0.0.0/8` (10.0.0.0 – 10.255.255.255)
- `172.16.0.0/12` (172.16.0.0 – 172.31.255.255)
- `192.168.0.0/16` (192.168.0.0 – 192.168.255.255)

# ## CIDR Notation
`192.168.1.0/24` means first 24 bits are 네트워크 prefix; last 8 bits are hosts. It 포함하다s addresses `192.168.1.0` to `192.168.1.255`.
...
Firewall (Linux ip표/nf표)
bash
sudo ufw status # Ubuntu: simple firewall
sudo ip표 -L -n # List rules
Network Statistics
bash
...
Is the firewall blocking the port? (Check ip표/ufw or 클라우드 security groups).
...
A practical 가 이 드 to manag servers, automat operations, mata reliable frastructure.
...
2 different media types (e.g., SSD + tape, or local + 클라우드).

1 copy off-site (e.g., 클라우드 or remote data centre).
...
Use secrets (via Docker secrets or 환경 with care).
...
Deploy: Update staging/production 환경.
...
Ansible
Agentless configuration management using YA기계 학습.
...
Use variables and secrets (AWS_SECRET_ACCESS_KEY via 환경, not hardcoded).
