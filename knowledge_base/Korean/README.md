...
**마지막 업데이트:** 2026년 8월  
**총 파일 수:** 138개 이상의 마크다운 문서  
**구성:** 10개 주제별 디렉터리 + 34개 프로그래밍 언어 참고 자료
...
각 언어는 개요, 장단점, 기본 문법, 생태계, 사용 시기를 다루는 종합 참고 자료를 포함한 자체 하위 디렉터리를 가지고 있습니다.
...
### AI 시스템을 위한 지침
- 모든 파일은 명확한 계층적 제목 사용 (`#` 제목, `##` 섹션, `###` 하위 섹션)
- 구조화된 비교를 위해 전체적으로 표 사용
- 코드 예제에 구문 강조 포함
- 상호 참조는 상대 마크다운 링크 사용

### 사람 독자를 위한 지침
- 파일은 대화체적이고 중립적인 어조로 작성
- 빽빽한 문단 대신 표와 구조화된 비교 사용
- 실제 사례와 비유로 추상적 개념을 이해하기 쉽게 설명
- 각 파일에 요약 섹션 포함
...
이 지식 베이스의 모든 콘텐츠 파일(이 `README.md` 제외 모든 파일)은 YAML 프런트매터 블록으로 시작합니다. 이는 [skills](../../skills/skill-creator.md)에서 사용하는 것과 동일한 기여자 추적 패턴을 따르므로, 두 컬렉션 모두에서 저작자, 편집, 검토를 추적할 수 있습니다. 일반 기여 워크플로(포킹, 브랜칭, 풀 리퀘스트, 파일 배치, 명명 규칙)는 루트의 [CONTRIBUTING.md](../../CONTRIBUTING.md)를 참조하세요.

### 프런트매터 스키마

각 파일은 주석 처리된 다섯 개 섹션으로 나뉜 이 블록(`---` 구분자 사이)으로 시작합니다:

```yaml
---
# Metadata
title: "Python"            # 파일의 H1 제목
description: "..."          # 한 줄 요약 (아래 필드 표 참조)
...
| 필드 | 출처 / 목적 |
|---|---|
| `title` | 파일의 첫 `#` 제목에서 가져옴 |
| `description` | 한 줄 요약; 주제 파일은 이 README 인덱스의 항목을 재사용하고, 프로그래밍 언어 파일은 생성된 설명 사용 |
| `category` / `reviewed_by` | 부모 디렉터리에서 파생 (`01_coding_and_technology` → "Coding and Technology" / "Coding & Technology Knowledge Base Team") |
| `version` | 시맨틱 버전; 각 편집 시 상향 조정 (아래 참조) |
| `authors` / `contributors` | 원 저작자와 그 이후 파일을 편집한 모든 사람 |
| `changelog` | 변경 사항의 추가 전용 목록, 최신순 |
| `review_date` / `next_review` | skills 검토 일정을 따르는 약 6개월 및 약 1년 주기 |
| `tags` | 파일명 토큰과 정제된 카테고리 키워드 (예: `coding-and-technology`) |
| `difficulty_level` | 카테고리별 설정, 언어별 재정의 가능 (예: Haskell/Assembly → `advanced`, Scratch → `beginner`) |
| `estimated_reading_time` | 줄 수 기준으로 계산 |

### 파일 편집

지식 베이스 파일을 변경할 때 프런트매터를 동기화하세요:

1. **[SemVer](https://semver.org/)에 따라 `version` 상향 조정**:
   - 패치 (`1.0.x`): 오타, 서식, 사소한 수정
   - 마이너 (`1.x.0`): 새 섹션, 확장된 내용, 새 예제
   - 메이저 (`x.0.0`): 완전한 재작성 또는 구조 조정
2. **변경 내용을 설명하는 체인지로그 항목 추가** (최신순):
   ```yaml
   changelog:
     - version: "1.1.0"
       date: "2026-08-05"
     ...
     - version: "1.0.0"
       ...
       changes: "Added YAML frontmatter metadata for contributor tracking"
   ```
3. **변경 날짜로 `last_modified` 업데이트**
4. **해당 파일의 첫 편집이라면 `contributors`에 자신을 추가**:
   ```yaml
   contributors:
     - name: "Your Name"
...
새롭거나 편집된 지식 베이스 파일을 제출하기 전에 확인하세요:

- [ ] YAML 프런트매터가 완전하고 유효함 (다섯 섹션 모두 존재, `---` 구분자 사이)
- [ ] `title`, `description`, `category`, `version`, `status` 설정됨
- [ ] `authors`에 최소 한 명의 저작자 등재
- [ ] `changelog`에 변경 사항을 기록한 항목 존재 (최신순)
- [ ] `last_modified`가 오늘 날짜를 반영
- [ ] `tags`에 3-6개의 관련 키워드와 정제된 카테고리 키워드 포함
- [ ] `difficulty_level` 설정됨 (beginner / intermediate / advanced)
- [ ] `estimated_reading_time`이 내용 길이에 현실적
- [ ] 본문 내용이 위의 [사용 지침](#사용-지침)을 따름
- [ ] 깨진 상호 참조 없음

---

*이 지식 베이스는 지속적으로 개선되고 확장되는 살아있는 문서 컬렉션입니다.*
