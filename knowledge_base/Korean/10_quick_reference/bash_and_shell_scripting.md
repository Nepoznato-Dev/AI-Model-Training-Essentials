...
## 요약
Bash 스크립팅은 컴퓨터를 사용하는 모든 사람에게 힘을 더해줍니다. 모든 플래그를 외울 필요는 없습니다. 가능한 것이 무엇인지, 어디서 찾을 수 있는지 알아야 합니다. 변수, 조건부, 루프, 파이프 등 기본 사항부터 시작하세요. 그런 다음 필요에 따라 텍스트 처리 도구(grep, sed, awk)를 추가하세요. 그리고 항상 `set -euo pipefail`를 사용하세요. 앞으로는 감사하게 될 것입니다.