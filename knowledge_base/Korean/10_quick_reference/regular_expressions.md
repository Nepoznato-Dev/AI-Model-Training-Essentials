...
| `*`| 이전 항목 중 0개 이상 |
| `+`| 위 항목 중 1개 이상 |
| `?`| 이전 중 0 또는 1(`*?` ,`+?`를 사용하면 수량자를 게으르게 만듭니다) |
| `\|`| 교대(OR) |
| `()`| 그룹화 및 캡처 |
| `[]`| 캐릭터 클래스 |
| `{}`| 수량자 범위 |
...
## 요약
Regex는 텍스트의 패턴 일치를 위한 도구입니다. 간단하게 시작하세요. 대부분의 실제 패턴은 문자 클래스, 수량자, 앵커 및 그룹의 조합일 뿐입니다. 패턴을 코드에 넣기 전에 테스트 도구를 사용하여 패턴을 확인하세요. 그리고 기억하세요: 정규 표현식이 너무 복잡해져서 읽을 수 없다면, 대신에 적절한 파서를 사용해야 할 때일 것입니다.